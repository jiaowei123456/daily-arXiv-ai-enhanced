...
TL;DR: MEDOPENCLAW is an auditable runtime for VLMs to operate dynamically in medical viewers, and MEDFLOWBENCH is a full-study medical imaging benchmark evaluating agentic capabilities across viewer-only, tool-use, and open-method tracks, revealing that current VLMs struggle with precise spatial grounding when using professional tools despite being able to navigate viewers.
...
Method: Proposed MEDOPENCLAW runtime for VLMs to operate dynamically within standard medical tools/viewers (e.g., 3D Slicer), and MEDFLOWBENCH benchmark covering multi-sequence brain MRI and lung CT/PET that systematically evaluates medical agentic capabilities across viewer-only, tool-use, and open-method tracks.

Result: State-of-the-art LLMs/VLMs (Gemini 3.1 Pro, GPT-5.4) can successfully navigate viewers to solve basic study-level tasks, but their performance paradoxically degrades when given access to professional support tools due to lack of precise spatial grounding.
...
TL;DR: SITH is a data-free, training-free framework that analyzes CLIP's vision transformer weights to provide semantically coherent explanations of attention heads and enables precise model editing without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing interpretability methods for vision-language models rely on activations, making them dataset-dependent, vulnerable to data bias, and limited to coarse head-level explanations. There's a need for weight-space analysis that provides fine-grained, data-free interpretability.

Method: SITH analyzes CLIP's vision transformer by decomposing each attention head's value-output matrix into singular vectors. It uses COMP (Coherent Orthogonal Matching Pursuit) to explain these vectors as sparse, semantically coherent combinations of human-interpretable concepts, providing intra-head explanations.

Result: SITH yields coherent, faithful intra-head explanations validated through reconstruction fidelity and interpretability experiments. It enables precise weight-space model edits that amplify or suppress specific concepts, improving downstream performance without retraining. Analysis shows fine-tuning primarily reweights a stable semantic basis rather than learning new features.

Conclusion: SITH provides a novel data-free approach to vision-language model interpretability that offers fine-grained insights into attention mechanisms, enables targeted model editing, and reveals that model adaptation occurs through reweighting of existing semantic representations rather than learning fundamentally new features.
...
Method: ReDiPrune operates before the vision-language projector, selecting tokens directly from vision encoder outputs using a lightweight scoring rule that jointly considers text-conditioned relevance and max-min diversity to ensure selected tokens are both query-relevant and non-redundant.
...
Conclusion: ReDiPrune is an effective plug-and-play token pruning method that reduces computational costs while improving accuracy by preserving rich visual features and selecting informative tokens before projection, requiring no retraining or architectural modifications.
...
TL;DR: A scale-aware 3D fusion framework that registers object meshes with transformer-predicted global point clouds to create metrically consistent digital twins using VLM-guided geometric anchors and physics-aware registration.


<details>
  <summary>Details</summary>
Motivation: Transformer-based feedforward reconstruction methods produce dimensionless point clouds with scale ambiguity and inconsistent coordinate conventions, preventing reliable fusion with locally reconstructed object meshes for embodied AI training and evaluation.

Method: Proposes a scale-aware 3D fusion framework with VLM-guided geometric anchor mechanism to recover real-world metric scale, and a geometry-aware registration pipeline with gravity-aligned vertical estimation, Manhattan-world structural constraints, and collision-free local refinement.

Result: Demonstrates improved cross-network object alignment and geometric consistency in real indoor kitchen environments, with applications to multi-primitive fitting and metric measurement. Introduces an open-source indoor digital twin dataset with metrically scaled scenes and registered object-centric mesh annotations.

Conclusion: The proposed framework successfully resolves coordinate mismatch between transformer-predicted global point clouds and object meshes, enabling construction of metrically consistent digital twins for embodied AI applications.
...
TL;DR: The paper introduces a cognitive taxonomy for multimodal ICL, creates UniICL-760K dataset and UniICL-Bench, and proposes Context-Adaptive Prototype Modulator to stabilize few-shot adaptation, achieving competitive results on multimodal ICL tasks.


<details>
  <summary>Details</summary>
Motivation: In-context learning in unified multimodal models suffers from sensitivity to example selection/formatting, cross-modal interference, and varying cognitive demands, leading to non-monotonic and task-dependent efficacy that needs systematic diagnosis and stabilization.

Method: 1) Introduces six-level capability-oriented taxonomy for multimodal ICL demonstrations; 2) Constructs UniICL-760K corpus with curated 8-shot episodes across 15 subtasks; 3) Develops UniICL-Bench for controlled evaluation; 4) Proposes Context-Adaptive Prototype Modulator (lightweight plug-and-play module) to stabilize few-shot adaptation.
...
Conclusion: The cognitive taxonomy provides systematic diagnosis for multimodal ICL behaviors, the curated dataset enables rigorous evaluation, and the proposed architectural intervention effectively stabilizes few-shot adaptation, advancing multimodal ICL research.
...
TL;DR: BCMDA framework addresses domain shift and confirmation bias in semi-supervised medical image segmentation by combining virtual domain bridging with knowledge transfer and prototypical alignment for pseudo label correction.


<details>
  <summary>Details</summary>
Motivation: Address two key challenges in mixed domain semi-supervised medical image segmentation: 1) distributional differences between labeled and unlabeled data hindering knowledge transfer, and 2) inefficient learning from unlabeled data causing severe confirmation bias.

Method: Proposes BCMDA framework with two main components: 1) Knowledge Transfer via Virtual Domain Bridging (KTVDB) using bidirectional correlation maps to synthesize labeled/unlabeled images, fixed/progressive dynamic MixUp strategies, and dual bidirectional CutMix for knowledge transfer; 2) Prototypical Alignment and Pseudo Label Correction (PAPLC) using learnable prototype cosine similarity classifiers for bidirectional prototype alignment and prototypical pseudo label correction.
...
Conclusion: BCMDA effectively addresses domain shift and confirmation bias in semi-supervised medical image segmentation, achieving superior performance with limited annotations through virtual domain bridging and prototypical alignment techniques.
...
TL;DR: LLaVA-LE is a specialized vision-language model for lunar exploration, trained on a new lunar dataset (LUCID) with 96k images and 81k QA pairs, achieving 3.3x performance gain over base LLaVA through two-stage fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Multimodal vision-language models have shown promise but remain unexplored for planetary science due to lack of large-scale datasets pairing real planetary imagery with detailed scientific descriptions.

Method: Created LUCID dataset (96k high-resolution panchromatic lunar images with captions + 81k QA pairs), then fine-tuned LLaVA using two-stage curriculum: (1) concept alignment for domain-specific terrain description, and (2) instruction-tuned visual question answering.

Result: LLaVA-LE achieves 3.3x overall performance gain over Base LLaVA and 2.1x over Stage 1 model, with reasoning score of 1.070 exceeding judge's reference score, demonstrating effectiveness of domain-specific multimodal data and instruction tuning.

Conclusion: Domain-specific multimodal data and instruction tuning significantly advance VLMs for planetary exploration, with LLaVA-LE showing strong performance for lunar terrain analysis and potential for broader planetary science applications.
...
TL;DR: Lookalike3D: A multiview image transformer for detecting identical, similar, and different object pairs in indoor scenes using semantic priors from foundation models, with 3DTwins dataset showing 104% IoU improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: Current 3D object understanding and generation methods overlook repeated objects in real-world scenes, which provide pervasive and complementary information for consistent 3D perception.

Method: Lookalike3D multiview image transformer that leverages strong semantic priors from large image foundation models to classify object pairs as identical, similar, or different using multiview images as input.

Result: 104% IoU improvement over baselines on the 3DTwins dataset containing 76k manually annotated identical, similar, and different object pairs from ScanNet++.

Conclusion: Lookalike object detection enables improved downstream tasks like joint 3D object reconstruction and part co-segmentation, turning repeated objects into powerful cues for consistent, high-quality 3D perception.
...
TL;DR: 3D Gaussian Splatting enables accurate 3D point measurements via multi-view triangulation without requiring stereoscopic workstations or specialized operator skills, outperforming traditional mesh-based methods.


<details>
  <summary>Details</summary>
Motivation: While 3DGS provides superior visual quality for novel view synthesis, its potential for accurate geometric measurement remains underutilized. Current point measurement methods rely on demanding stereoscopic workstations or incomplete/inaccurate 3D meshes.

Method: Leverage 3DGS's ability to render exact source views and smoothly interpolate between them. Users pick congruent points across different views, then triangulate these points to generate precise 3D measurements. This approach enables multi-view intersection (more than two views) for higher accuracy.

Result: Achieves RMSEs in the 1-2 cm range on well-defined points. Significantly outperforms mesh-based methods: 0.037 m vs 0.062 m on thin structures, and 0.013 m RMSE on sharp corners where mesh methods failed entirely. Accuracy matches or exceeds traditional stereoscopic methods on standard hardware.

Conclusion: 3DGS enables intuitive, accurate 3D point measurement without specialized stereoscopic equipment, offering a practical alternative to traditional methods while maintaining high geometric accuracy comparable to or better than stereoscopic approaches.
...
TL;DR: QuatRoPE is a novel positional embedding method for 3D spatial reasoning that encodes pairwise spatial relations linearly with object count, using dot products in attention layers, plus IGRE to isolate its influence from LLM's original positional embeddings.


<details>
  <summary>Details</summary>
Motivation: Spatial reasoning is crucial for intelligent embodied agents but limited by scarce 3D scene-language data. Existing methods either struggle with extracting spatial relations from prematurely fused absolute position features or suffer from poor scalability due to quadratic encoding of all pairwise relations.

Method: Proposes QuatRoPE, a positional embedding method with linear input length relative to object count that explicitly calculates pairwise spatial relations through dot products in attention layers. Also introduces Isolated Gated RoPE Extension (IGRE) to limit QuatRoPE's influence to object-related tokens, preserving LLM's original positional embeddings.

Result: Extensive experiments demonstrate effectiveness. The method maintains spatial consistency and geometric integrity while being scalable with linear complexity.

Conclusion: QuatRoPE with IGRE provides an efficient, scalable solution for 3D spatial reasoning that preserves LLM capabilities while enabling explicit spatial relation computation with linear complexity.
...
TL;DR: Landmark-based gaze estimation methods offer lightweight, interpretable alternatives to deep CNNs, with comparable cross-domain generalization to ResNet18 despite lower within-domain performance due to landmark detection noise.


<details>
  <summary>Details</summary>
Motivation: Deep CNNs for gaze estimation are computationally expensive and lack interpretability, while geometric landmark-based methods are lightweight but their performance limits and generalization capabilities remain underexplored in modern benchmarks.

Method: Standardized pipeline to extract and normalize facial landmarks from three large-scale datasets (Gaze360, ETH-XGaze, GazeGene), training lightweight regression models including Extreme Gradient Boosted trees, holistic MLP, and siamese MLP designed to capture binocular geometry.
...
Conclusion: Sparse geometric features encode sufficient information for robust gaze estimation, enabling efficient, interpretable, and privacy-friendly edge applications while maintaining competitive cross-domain generalization.
...
TL;DR: Self-supervised confidence framework for 3D Gaussian Splatting improves mesh extraction in scenes with view-dependent effects while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting accelerates mesh extraction but struggles with view-dependent effects; existing solutions sacrifice efficiency through multi-view techniques, iterative extraction, or large models.

Method: Introduces self-supervised confidence framework with learnable confidence values balancing photometric/geometric supervision; adds losses penalizing per-primitive color/normal variance; improves appearance model by decoupling D-SSIM loss terms.

Result: Achieves state-of-the-art results for unbounded meshes while maintaining high computational efficiency.

Conclusion: Proposed confidence framework provides simple, efficient alternative to complex methods, enabling accurate mesh extraction in challenging scenes with view-dependent effects.
...
TL;DR: The paper introduces a psychophysically-informed framework using ambiguous images to compare semantic boundary placement between humans and machine classifiers, revealing differences in bias and sensitivity.


<details>
  <summary>Details</summary>
Motivation: To understand where humans and machine classifiers draw semantic boundaries between concepts when visual evidence is ambiguous, using this as an interpretability probe for vision models and to bridge human psychophysics with machine vision.

Method: Developed a framework that interpolates between concepts in CLIP embedding space to generate continuous spectra of ambiguous images, enabling precise measurement of semantic boundary placement for both humans and machine classifiers.

Result: Machine classifiers show greater bias toward seeing "rabbit" compared to humans, who align more closely with the CLIP embedding used for synthesis. Human sensitivity is more strongly affected by guidance scale than machine classifiers.

Conclusion: Controlled ambiguity serves as an effective diagnostic tool for bridging human psychophysics, image classification, and generative models, offering insights into human-model alignment, robustness, interpretability, and image synthesis methods.
...
TL;DR: OpenCap Monocular enables 3D kinematics and kinetics estimation from single smartphone videos, achieving clinical-grade accuracy for mobility assessment without specialized lab equipment.


<details>
  <summary>Details</summary>
Motivation: Traditional biomechanical assessment requires expensive, specialized lab equipment (motion capture, force plates), limiting clinical translation and accessibility. There's a need for scalable, accurate tools for quantifying human movement and musculoskeletal forces in real-world settings.

Method: Algorithm refines 3D human pose estimates from monocular pose estimation model (WHAM) via optimization, computes kinematics of a biomechanically constrained skeletal model, and estimates kinetics via physics-based simulation combined with machine learning.

Result: Validated against marker-based motion capture and force plates: low kinematic error (4.8° mean absolute error for rotational DOF; 3.4 cm for pelvis translations), outperforming regression-only baseline by 48% rotational and 69% translational accuracy. Estimated ground reaction forces comparable to/better than prior two-camera system. Demonstrated clinically meaningful accuracy for knee extension moment (sit-to-stand) and knee adduction moment (walking).

Conclusion: OpenCap Monocular provides accessible, free biomechanical assessment via smartphone app, web app, and cloud computing, enabling scalable quantification of kinematics and kinetics for mobility-related conditions without specialized lab equipment.
...
TL;DR: TIGeR is a multi-modal transformer model that creates unified geo-temporal embeddings for images, enabling joint reasoning about visual appearance, geolocation, and time for applications like geo-time-aware image retrieval.


<details>
  <summary>Details</summary>
Motivation: Real-world applications in digital forensics, urban monitoring, and environmental analysis require complex capabilities beyond standard geo-localization and time prediction, such as retrieving images at the same location but different specified times.

Method: Proposes TIGeR, a multi-modal-transformer-based model that maps image, geolocation, and time into a unified geo-temporal embedding space, supporting flexible input configurations and performing multiple tasks with the same representation.

Result: TIGeR outperforms strong baselines and state-of-the-art methods by up to 16% on time-of-year prediction, 8% on time-of-day prediction, and 14% in geo-time aware retrieval recall, demonstrating benefits of unified geo-temporal modeling.

Conclusion: By preserving location identity under appearance changes, TIGeR enables retrieval based on where and when a scene is rather than purely visual similarity, advancing capabilities for joint visual-geo-temporal reasoning.
...
TL;DR: This review paper analyzes synthetic cardiac MRI generation methods, comparing GANs, VAEs, diffusion models, and flow-matching techniques through fidelity, utility, and privacy metrics, highlighting their applications in overcoming data scarcity and privacy concerns.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the scarcity of annotated medical imaging data in cardiac MRI, overcome challenges like limited labeled datasets, vendor variability, and privacy risks from model memorization, while improving clinical workflows through synthetic data generation.

Method: The review compares multiple synthetic CMRI generation approaches: GANs, VAEs, diffusion probabilistic models, and flow-matching techniques. It examines mask-conditioned generation for structural fidelity, diffusion/flow-matching for boundary preservation, vendor-style conditioning for cross-domain generalization, and privacy mechanisms like membership inference attacks and differential privacy.

Result: The review finds that anatomically constrained synthetic data can enhance segmentation accuracy and robustness across multi-vendor settings. Mask-conditioned generation improves structural fidelity, diffusion/flow-matching models offer strong boundary preservation, and privacy-preserving techniques help mitigate risks of data leakage.

Conclusion: The paper concludes by highlighting current limitations and advocating for integrated, evaluation-driven frameworks that balance fidelity, utility, and privacy for reliable clinical workflows in synthetic cardiac MRI generation.
...
TL;DR: DRoPS uses static pre-scans as geometric priors with surface-aligned Gaussian grids and CNN-based motion parameterization for improved dynamic scene reconstruction from extreme viewpoints.
...
Result: Extensive experiments show DRoPS significantly outperforms current state-of-the-art methods in both rendering quality and 3D tracking accuracy, particularly for scenes with extreme viewpoints and highly articulated motions.

Conclusion: By effectively leveraging static pre-scans through surface-aligned Gaussian grids and CNN-based motion parameterization, DRoPS successfully constrains the solution space and ensures geometric consistency, enabling robust dynamic scene reconstruction from challenging viewpoints.
...
TL;DR: AVControl is a lightweight, extendable framework for multimodal video and audio generation control that uses separate LoRA adapters for each control modality on a parallel canvas, requiring no architectural changes to the base LTX-2 model.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for controlling video and audio generation either use monolithic models with fixed control sets or require costly architectural modifications for each new modality, lacking flexibility and efficiency.

Method: Built on LTX-2 joint audio-visual foundation model, AVControl trains each control modality as a separate LoRA adapter on a parallel canvas that provides reference signals as additional tokens in attention layers, enabling modular control without architectural changes.

Result: Outperforms all evaluated baselines on VACE Benchmark for depth- and pose-guided generation, inpainting, and outpainting; shows competitive results on camera control and audio-visual benchmarks; supports diverse independently trained modalities with high efficiency.

Conclusion: AVControl provides an efficient, modular framework for multimodal video and audio generation control that is both compute- and data-efficient, requiring minimal training data and steps per modality while supporting diverse control types including novel audio-visual controls.
...
TL;DR: Calibri is a parameter-efficient method that calibrates Diffusion Transformers (DiTs) using a single learned scaling parameter and evolutionary optimization, improving generative quality while reducing inference steps.
...
Method: Calibri introduces a single learned scaling parameter to DiT blocks, frames DiT calibration as a black-box reward optimization problem, and solves it efficiently using an evolutionary algorithm that modifies only ~100 parameters.
...
Conclusion: The proposed Calibri method demonstrates that minimal parameter modifications through evolutionary optimization can significantly enhance Diffusion Transformer performance and efficiency in generative tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current AAA CT segmentation models fail due to misdirected attention to irrelevant structures or insufficient focus on thin, low-contrast targets, requiring better control over where models look during training.

Method: Proposes an XAI-guided encoder shaping framework that computes dense attribution-based encoder focus maps (XAI fields) from final encoder blocks, aligns predicted probability mass to XAI fields for focus-output agreement, and routes fields into lightweight refinement pathways with confidence priors that modulate logits at inference.

Result: Substantial improvements over base SAM setup on clinically validated challenging cases curated for failure-prone scenarios, demonstrating practical effectiveness for reliable segmentation in complex scenarios.

Conclusion: Explicitly optimizing encoder focus via XAI guidance is a practical and effective principle for improving segmentation reliability in complex medical imaging scenarios like AAA CT segmentation.
...
TL;DR: GoldiCLIP is a data-efficient vision-language model framework that achieves state-of-the-art performance using only 30M images (300x less data than leading methods) through balanced supervision signals combining self-distillation, VQA objectives, and uncertainty-based loss weighting.


<details>
  <summary>Details</summary>
Motivation: Large-scale VLMs require billion-sample datasets, creating significant barriers to progress. Existing approaches only address subsets of weaknesses in contrastive pretraining, necessitating a more comprehensive solution that balances supervision quality.

Method: Three key innovations: (1) text-conditioned self-distillation aligning both text-agnostic and text-conditioned features; (2) encoder-integrated decoder with VQA objective enabling generalization beyond caption-like queries; (3) uncertainty-based weighting mechanism automatically balancing heterogeneous losses.

Result: Achieves SOTA among data-efficient approaches with 30M images (300x less data): improves by 2.2 points on MSCOCO retrieval, 2.0 on fine-grained retrieval, and 5.9 on question-based retrieval, while remaining competitive with billion-scale models.

Conclusion: GoldiCLIP demonstrates that high-quality supervision balancing multiple signal types enables effective vision-language learning with dramatically reduced data requirements, challenging the necessity of billion-scale datasets for VLM development.
...
<details>
  <summary>Details</summary>
Motivation: Pin site infections are common complications in external fixation that cause pain and morbidity. Current identification and management need improvement to enhance patient experience.

Method: Created pin site infection dataset and developed attention-based deep learning model with Efficient Redundant Reconstruction Convolution (ERRC) to focus on relevant regions and reduce parameters while enhancing feature richness.
...
Result: Achieves 7X increase in Mean Absolute Error compared to clean images, successfully transfers across seven state-of-the-art crowd models with transfer ratios from 0.55 to 1.69, while maintaining competitive visual quality and balancing effectiveness with imperceptibility.

Conclusion: The proposed framework demonstrates effective cross-paradigm adversarial attacks in crowd counting, highlighting security vulnerabilities across different architectural paradigms and providing a balanced approach between attack effectiveness and visual imperceptibility.
...
Result: Superior performance in visual quality (lower FID and FVD) and camera adherence (lower ATE and ARE) compared to state-of-the-art baselines, achieving stable generation for videos up to 32 seconds.

Conclusion: DCARL effectively addresses long-trajectory video generation challenges by combining divide-and-conquer structural stability with autoregressive high-fidelity generation, enabling scalable, controllable long video synthesis.
...
TL;DR: WAFT-Stereo is a warping-based stereo matching method that eliminates the need for cost volumes, achieving state-of-the-art performance on major benchmarks while being significantly faster than competitive methods.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the conventional reliance on cost volumes in stereo matching methods, proposing that warping operations can replace cost volumes for improved efficiency without sacrificing performance.

Method: WAFT-Stereo uses warping-based operations instead of constructing explicit cost volumes. The method leverages warping to establish correspondences between stereo images, eliminating the computational overhead of traditional cost volume construction and processing.
...
Conclusion: Cost volumes are not necessary for strong stereo matching performance and can be effectively replaced by warping operations, resulting in both superior accuracy and significant efficiency improvements.
...
TL;DR: Comprehensive benchmarking of 20 multimodal LLMs for 2D neuroimaging shows technical imaging attributes are nearly solved, but diagnostic reasoning remains challenging, with Gemini-2.5-Pro and GPT-5-Chat achieving best diagnostic performance and Gemini-2.5-Flash offering optimal efficiency-performance trade-off.


<details>
  <summary>Details</summary>
Motivation: Despite advances in multimodal large language models for image-based decision support, their reliability and operational trade-offs in neuroimaging applications remain insufficiently understood, necessitating comprehensive benchmarking to evaluate performance across multiple dimensions including diagnostic accuracy, calibration, structured output validity, and computational efficiency.

Method: Multi-phase benchmarking framework using curated MRI and CT datasets covering multiple sclerosis, stroke, brain tumors, other abnormalities, and normal controls. Models generate multiple simultaneous outputs (diagnosis, subtype, imaging modality, specialized sequence, anatomical plane). Evaluation across four directions: discriminative classification with abstention, calibration, structured-output validity, and computational efficiency, with fair comparison controlled for selection bias.

Result: Technical imaging attributes (modality and plane) are nearly solved, while diagnostic reasoning, especially subtype prediction, remains challenging. Tumor classification is most reliable, stroke moderately solvable, multiple sclerosis and rare abnormalities difficult. Few-shot prompting improves performance but increases token usage, latency, and cost. Gemini-2.5-Pro and GPT-5-Chat achieve strongest overall diagnostic performance; Gemini-2.5-Flash offers best efficiency-performance trade-off. Among open-weight models, MedGemma-1.5-4B approaches proprietary model performance under few-shot prompting while maintaining perfect structured output.

Conclusion: The study provides practical insights into performance, reliability, and efficiency trade-offs for multimodal LLMs in neuroimaging, establishing that while technical imaging attributes are well-handled, diagnostic reasoning remains a significant challenge, with specific models identified as optimal for different use cases, supporting standardized evaluation frameworks for clinical applications.
...
TL;DR: CORA is a 3D vision foundation model for cardiovascular risk assessment that learns from unlabeled CCTA volumes via pathology-centric self-supervised learning, outperforming state-of-the-art models on diagnostic tasks and improving 30-day MACE risk stratification when combined with LLMs.


<details>
  <summary>Details</summary>
Motivation: Clinical translation of automated CCTA analysis is limited by scarcity of expert-annotated datasets, and existing label-free pretraining strategies (like masked image modeling) are biased toward global anatomical statistics rather than spatially localized pathological features of coronary plaques.

Method: CORA uses a pathology-centric, synthesis-driven self-supervised framework with an anatomy-guided lesion synthesis engine that explicitly trains the model to detect simulated vascular abnormalities, biasing representation learning toward clinically relevant disease features rather than dominant background anatomy.

Result: Trained on 12,801 unlabeled CCTA volumes and evaluated across nine independent hospitals, CORA consistently outperformed state-of-the-art 3D vision foundation models across diagnostic and anatomical tasks (plaque characterization, stenosis detection, coronary artery segmentation) with up to 29% performance gain. When coupled with a large language model, it significantly improved 30-day MACE risk stratification.

Conclusion: CORA establishes a scalable and extensible foundation for unified anatomical assessment and cardiovascular risk prediction, demonstrating the effectiveness of pathology-centric self-supervised learning for medical imaging foundation models.
...
TL;DR: Comparison of YOLOv11, SSD, and RT-DETRv2 for smoke detector recognition in automatic inspection systems, with YOLOv11n achieving best performance (mAP@0.5: 0.884) using real and semi-synthetic training data.


<details>
  <summary>Details</summary>
Motivation: Smoke detectors are difficult to inspect due to high ceilings and problematic locations. An automatic inspection system using drones would enable faster revisions, reduce worker risk from heights, and lower costs. This study focuses on the smoke detector recognition component.

Method: Compared three object detection architectures: YOLOv11, SSD, and RT-DETRv2 with various backbone sizes. Evaluated training strategies using real and semi-synthetic data with different augmentation methods. Tested on two challenging datasets containing motion blur, low resolution, and incomplete objects.

Result: YOLOv11n achieved the best performance with average mAP@0.5 score of 0.884. The study provides comprehensive evaluation of different detection models and training strategies for smoke detector recognition in challenging conditions.

Conclusion: YOLOv11n is the most effective detector for smoke recognition in automatic inspection systems. The research demonstrates successful integration of real and synthetic data for training, with publicly available code, models, and dataset to support further development.
...
TL;DR: OptiSAR-Net++ introduces a novel cross-domain remote sensing visual grounding framework that handles both optical and SAR images using efficient patch-level low-rank adaptation mixture of experts, CLIP-based contrastive matching, and text-guided fusion modules.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing visual grounding methods are limited to single-sensor domains (either optical or SAR), which restricts their real-world applicability where multi-sensor data fusion is needed for comprehensive scene understanding.

Method: Proposes OptiSAR-Net++ with: 1) Patch-level Low-Rank Adaptation Mixture of Experts (PL-MoE) for efficient cross-domain feature decoupling; 2) CLIP-based contrastive paradigm with dynamic adversarial negative sampling to replace inefficient Transformer decoding; 3) Text-guided dual-gate fusion module (TGDF-SSA) for semantic-visual alignment; 4) Region-aware auxiliary head for enhanced spatial modeling.

Result: Achieves state-of-the-art performance on both the newly introduced OptSAR-RSVG benchmark and the existing DIOR-RSVG benchmark, demonstrating superior localization accuracy and computational efficiency compared to previous methods.

Conclusion: The proposed OptiSAR-Net++ effectively addresses cross-domain remote sensing visual grounding challenges through innovative architectural designs, establishing a new benchmark for multi-sensor RSVG with improved accuracy and efficiency.
...
TL;DR: A comprehensive framework for surgical phase recognition in pituitary tumor surgery videos using self-supervised learning, temporal modeling, and scalable annotation achieves 90% accuracy with a collaborative platform for data collection and model improvement.
...
Conclusion: The framework successfully addresses surgical phase recognition challenges through integrated self-supervised learning and temporal modeling, with a collaborative platform enabling scalable data collection and continuous model improvement.
...
TL;DR: SSL pretraining on knee radiographs shows mixed results: image-only SSL improves linear probing but not full fine-tuning for KL grade diagnosis, while multimodal SSL fails due to severe bias in hospital data (93% KL grade 3). However, multimodal SSL significantly improves prognostic modeling for 4-year OA incidence and progression.


<details>
  <summary>Details</summary>
Motivation: To assess whether self-supervised learning (SSL) improves knee osteoarthritis modeling for diagnosis and prognosis compared to standard ImageNet-pretrained initialization, addressing limitations of current approaches.
...
Result: For diagnosis: Image-only SSL improved accuracy during linear probing but not during full fine-tuning compared to ImageNet. Multimodal SSL failed to improve grading due to severe bias (93% KL grade 3). For prognosis: Multimodal SSL significantly outperformed ImageNet baselines in predicting 4-year incidence/progression (MOST AUROC: 0.701 vs. 0.599 at 10% labeled data).

Conclusion: Uncurated hospital image-text data is ineffective for learning diagnosis due to severity bias but provides strong signal for prognostic modeling when downstream task aligns with pretraining data distribution. Task alignment between pretraining and downstream objectives is crucial for SSL success.
...
TL;DR: A large-scale polarized reflection dataset of 218 real-world objects captured with a multi-camera, multi-light system enables improved inverse rendering models for material understanding and real-world generalization.


<details>
  <summary>Details</summary>
Motivation: Current inverse rendering approaches rely on synthetic datasets with simplified illumination and limited material realism, preventing generalization to real-world images due to scarcity of real measured reflectance data.

Method: Created a large-scale polarized reflection dataset using an 8-camera, 346-light Light Stage with cross/parallel polarization, capturing 218 everyday objects across five acquisition dimensions (multiview, multi-illumination, polarization, reflectance separation, material attributes) yielding over 1.2M high-resolution images with diffuse-specular separation and analytically derived material properties.

Result: The dataset enables training and evaluation of state-of-the-art inverse and forward rendering models, demonstrating significant improvements in intrinsic decomposition, relighting, and sparse-view 3D reconstruction with better material separation, illumination fidelity, and geometric consistency.

Conclusion: This work establishes a new foundation for physically grounded material understanding and enables real-world generalization beyond synthetic training regimes, addressing core challenges in inverse rendering through comprehensive real-world reflectance data.
...
TL;DR: TIGFlow-GRPO: A two-stage generative framework for human trajectory forecasting that combines Conditional Flow Matching with behavioral rule alignment through Flow-GRPO post-training, improving social compliance and physical feasibility while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing Conditional Flow Matching approaches for trajectory forecasting focus primarily on supervised fitting, which may insufficiently reflect social norms and scene constraints in generated trajectories. There's a need to better align flow-based trajectory generation with behavioral rules to produce more realistic and socially compliant predictions.

Method: Two-stage framework: 1) CFM-based predictor with Trajectory-Interaction-Graph (TIG) module to model fine-grained visual-spatial interactions and strengthen context encoding; 2) Flow-GRPO post-training that reformulates deterministic flow rollout as stochastic ODE-to-SDE sampling for trajectory exploration, with composite reward combining view-aware social compliance and map-aware physical feasibility.

Result: Experiments on ETH/UCY and SDD datasets show improved forecasting accuracy and long-horizon stability while generating trajectories that are more socially compliant and physically feasible compared to existing approaches.

Conclusion: The proposed TIGFlow-GRPO framework effectively connects flow-based trajectory modeling with behavior-aware alignment, providing a promising approach for generating realistic and socially compliant trajectories in dynamic multimedia environments.
...
TL;DR: A generative framework using autoregressive diffusion models to predict infinite-horizon raw gaze trajectories in videos, conditioned on saliency-aware visual features, outperforming existing methods in long-range spatio-temporal accuracy.
...
Conclusion: The generative framework enables infinite-horizon raw gaze prediction that captures fine-grained temporal dynamics and long-range dependencies, advancing video scene understanding and multimodal interaction capabilities.
...
TL;DR: TIES is a dynamic token selection framework for Vision-Language-Action models that improves policy performance while reducing visual token usage by 78% through tau-guided inter-layer efficient selection.


<details>
  <summary>Details</summary>
Motivation: Existing token reduction methods for VLA models rely on static attention magnitude selection, but high-attention tokens are task-dependent and can degrade policy performance, creating a need for dynamic, robust token selection.
...
Conclusion: TIES provides an effective dynamic token selection approach that challenges the assumption of static attention-based token reduction, achieving both improved policy performance and significant computational efficiency gains.
...
TL;DR: BiFM is a bidirectional flow matching framework that jointly learns image generation and inversion in a single model, enabling efficient few-step sampling with improved editing quality.
...
Method: BiFM directly estimates average velocity fields in both "image → noise" and "noise → image" directions, constrained by a shared instantaneous velocity field. Uses continuous time-interval supervision stabilized by bidirectional consistency objective and lightweight time-interval embedding.
...
Conclusion: BiFM provides a unified framework for joint generation and inversion that integrates seamlessly into popular diffusion/flow matching backbones, addressing limitations of existing few-step methods.
...
TL;DR: Proposes Select-Hypothesize-Verify framework to verify neuron concept accuracy, addressing misleading neuron interpretations in neural networks.
...
Conclusion: Verification of neuron functions is essential for accurate interpretation; the proposed framework addresses limitations of existing approaches by validating whether generated concepts truly reflect neuron functionality.
...
TL;DR: xLARD is a self-correcting text-to-image generation framework that uses multimodal LLMs to provide explainable latent rewards for refining image generation through structured feedback and differentiable reward mapping.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with complex prompts involving fine-grained semantics and spatial relations due to their feed-forward nature, which requires anticipating alignment without fully understanding the output. However, evaluating generated images is easier than generating them, creating an asymmetry that can be leveraged for self-correction.

Method: xLARD introduces a lightweight corrector that refines latent representations based on structured feedback from multimodal LLMs. A key innovation is a differentiable mapping from latent edits to interpretable reward signals, enabling continuous latent-level guidance from non-differentiable image-level evaluations. The framework uses model-generated references and explainable latent rewards to guide generation.

Result: Experiments across diverse generation and editing tasks demonstrate that xLARD improves semantic alignment and visual fidelity while maintaining generative priors. The framework shows effectiveness in handling complex prompts with fine-grained semantics and spatial relations.

Conclusion: xLARD successfully leverages the asymmetry between generation difficulty and evaluation tractability to create a self-correcting framework that enables models to understand, assess, and correct themselves during generation, leading to better alignment with complex prompts.
...
Method: PASDiff uses Physics-Aware Semantic Diffusion with training-free approach. It employs inverse intensity weighting and Retinex theory for photometric constraints to recover visibility and natural chromaticity. The Style-Agnostic Structural Injection (SASI) extracts structures from off-the-shelf facial priors while filtering out photometric biases, harmonizing identity features with physical constraints.

Result: Extensive experiments demonstrate PASDiff significantly outperforms existing methods, achieving superior balance among natural illumination, color recovery, and identity consistency. The authors also construct WildDark-Face benchmark with 700 real-world low-light facial images with complex degradations.
...
TL;DR: MoE-GRPO: RL-based framework using Group Relative Policy Optimization to improve expert routing diversity in MoE-based Vision-Language Models, addressing deterministic top-K routing limitations and expert overfitting.


<details>
  <summary>Details</summary>
Motivation: Deterministic top-K routing in MoE-based Vision-Language Models overlooks optimal expert combinations and causes expert overfitting, limiting model performance and diversity.

Method: Formulates expert selection as sequential decision-making problem optimized with Group Relative Policy Optimization (GRPO), plus modality-aware router guidance to enhance training stability by discouraging exploration of infrequently activated experts per modality.

Result: Extensive experiments on multi-modal image and video benchmarks show MoE-GRPO consistently outperforms standard top-K routing and variants by promoting diverse expert selection, mitigating expert overfitting, and enabling task-level expert specialization.

Conclusion: MoE-GRPO effectively addresses deterministic routing limitations through RL-based optimization, improving expert diversity and specialization in MoE-based Vision-Language Models while maintaining computational efficiency.
...
TL;DR: MAML framework for few-shot 3D left atrial wall segmentation from LGE-MRI using multi-task meta-training with boundary-aware loss, showing improved performance over fine-tuning and robustness to domain shifts.


<details>
  <summary>Details</summary>
Motivation: Left atrial wall segmentation from LGE-MRI is difficult due to thin geometry, low contrast, and limited expert annotations, creating a need for few-shot learning approaches that can work with minimal labeled data.

Method: Model-Agnostic Meta-Learning (MAML) framework for K-shot (K=5,10,20) 3D segmentation, meta-trained on wall task with auxiliary left/right atrial cavity tasks, using boundary-aware composite loss to emphasize thin-structure accuracy.

Result: MAML outperformed supervised fine-tuning: at 5-shot, DSC 0.64 vs 0.52, HD95 5.70mm vs 7.60mm; approached fully supervised reference at 20-shot (0.69 vs 0.71 DSC). Robust under domain shift: 5-shot MAML achieved 0.59 DSC/5.99mm HD95 on synthetic shift and 0.57 DSC/6.01mm HD95 on local cohort.
...
TL;DR: EWAD: First unified event-based video anomaly detection framework with new benchmarks, dynamic sampling, density-modulated temporal modeling, and RGB-to-event knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: Event-based vision has ideal properties for VAD (low redundancy, motion focus, privacy), but lacks dedicated datasets and effective modeling strategies, hindering progress in the field.

Method: EVent-centric spatiotemporal Video Anomaly Detection (EWAD) framework with: 1) event density aware dynamic sampling for temporally informative segments; 2) density-modulated temporal modeling for contextual relations from sparse event streams; 3) RGB-to-event knowledge distillation to enhance event-based representations under weak supervision.

Result: Extensive experiments on three benchmarks demonstrate significant improvements over existing approaches, highlighting the potential and effectiveness of event-driven modeling for video anomaly detection.

Conclusion: EWAD establishes event-based VAD as a unified research direction with publicly available benchmarks, showing event-based vision's natural advantages for anomaly detection tasks.
...
Method: Two-stage framework: Stage 1 pre-trains a 3D U-Net with ResNeXt encoder and instance normalization on LA cavity segmentation; Stage 2 transfers weights and adapts to wall segmentation using progressive layer-unfreezing to preserve endocardial features while enabling wall-specific refinement.

Result: Substantial improvements over baseline: wall Dice improved from 0.623 to 0.814, Surface Dice at 1 mm from 0.553 to 0.731, HD95 decreased from 2.95 mm to 2.55 mm, ASSD from 0.71 mm to 0.63 mm. With reduced supervision (70 volumes), achieved Dice 0.78 and HD95 3.15 mm, exceeding multi-class benchmarks.

Conclusion: Anatomically grounded task transfer with controlled fine-tuning improves boundary accuracy for thin LA wall segmentation in 3D LGE-MRI, demonstrating the value of leveraging cavity segmentation as an anatomical prior.
...
TL;DR: A novel method for dynamic 3D scene reconstruction using 3D Gaussian Splatting that preserves local geometric structure across time through view-space ray grouping and spatial distribution constraints, eliminating reliance on external priors.


<details>
  <summary>Details</summary>
Motivation: Current dynamic 3D reconstruction methods using 3D Gaussian Splatting fail to model realistic motion aligned with physical dynamics, especially for monocular video datasets. This misalignment undermines local geometric structure and reconstruction quality, forcing reliance on external priors like optical flow or 2D tracks.
...
Conclusion: The proposed approach successfully preserves local geometric structure in 4D scenes through view-space ray grouping and spatial constraints, enabling physically plausible motion modeling without external priors and improving dynamic 3D reconstruction quality.
...
TL;DR: Proposes a scalable distributed vehicle control method for emergency vehicle transit that uses only local information, avoids centralized training, and provides deterministic safety guarantees through distributed conflict resolution.
...
Method: A distributed vehicle control method where vehicles adjust driving behaviors using only local information online, with theoretical proof that local information is approximately equivalent to global information, plus a distributed conflict resolution mechanism to ensure safety and avoid decision conflicts.
...
Conclusion: The proposed distributed approach overcomes limitations of centralized and learned methods by providing scalable, real-time decision-making with deterministic safety guarantees and natural adaptability to varying traffic conditions.
...
<details>
  <summary>Details</summary>
Motivation: Existing vision-language models struggle with fine-grained visual details and complex object relationships in zero-shot REC, while large language models cannot directly process visual features. There's a need to bridge the gap between low-level image regions and high-level semantic understanding.

Method: Proposes SGREC: 1) Uses a VLM to construct query-driven scene graphs encoding spatial relationships, descriptive captions, and object interactions; 2) Bridges gap between image regions and semantic understanding; 3) Uses LLM to infer target object from structured textual representation with detailed explanations.

Result: Achieves state-of-the-art top-1 accuracy on zero-shot REC benchmarks: RefCOCO val (66.78%), RefCOCO+ testB (53.43%), and RefCOCOg val (73.28%), demonstrating strong visual scene understanding.

Conclusion: SGREC effectively addresses limitations of existing VLMs and LLMs by using query-driven scene graphs as structured intermediaries, achieving superior performance while maintaining interpretability through detailed LLM explanations.
...
TL;DR: Dual-loss framework combining Center Loss and ArcFace Loss improves fine-grained rice leaf disease classification, achieving 99%+ accuracy on three CNN backbones without major architectural changes.


<details>
  <summary>Details</summary>
Motivation: Early rice disease detection is crucial for food security, but traditional deep learning with cross entropy loss struggles with high intra-class variance and inter-class similarity in plant pathology datasets.

Method: Proposed dual-loss framework combining Center Loss and ArcFace Loss applied to three CNN architectures (InceptionNetV3, DenseNet201, EfficientNetB0) trained on Rice Leaf Dataset.

Result: Achieved accuracies of 99.6%, 99.2%, and 99.2% respectively, demonstrating significant performance gains over traditional approaches.

Conclusion: Angular margin-based and center-based constraints substantially boost discriminative feature embeddings, providing an efficient, practical solution for real-world farming deployment without major architectural modifications.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of achieving high-quality 3D reconstruction from few input views while maintaining fast training times, combining the efficiency of TensorRF with regularization techniques from few-shot NeRF methods.

Method: Integrates TensorRF's tensor-based representation for acceleration with FreeNeRF's frequency-driven regularization, adding frequency and occlusion masks to improve stability and quality under sparse views.

Result: On Synthesis NeRF benchmark: PSNR improved from 21.45 dB (TensorRF) to 23.70 dB, with fine-tuned version reaching 24.52 dB while maintaining 10-15 minute training time. On THuman 2.0: achieved 27.37-34.00 dB with only 8 input images.

Conclusion: Few TensoRF provides an efficient and data-effective solution for real-time 3D reconstruction across diverse scenes, demonstrating significant quality improvements over baseline TensorRF while preserving its fast training speed.
...
TL;DR: GDPO-Listener: A framework for generating expressive 3D head motions for both speakers and listeners in dyadic interactions using Auto-Regressive Flow Matching and Group reward-Decoupled Policy Optimization to overcome kinematic stillness and enable semantic text control.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D head motion generation in dyadic interactions suffer from the 'Regression-to-the-Mean' problem in listener motions, collapsing into static faces, and lack parameter space for complex nonverbal motions.
...
Conclusion: GDPO-Listener achieves highly expressive speaking and listening motion generation by addressing the Regression-to-the-Mean problem through innovative architectural and optimization techniques, enabling realistic 3D head motion synthesis for dyadic interactions.
...
TL;DR: BiHumanML3D is the first bilingual text-to-motion benchmark, and BiMD with Cross-Lingual Alignment (CLA) is a diffusion model that significantly outperforms monolingual baselines by explicitly aligning semantic representations across languages.


<details>
  <summary>Details</summary>
Motivation: Text-to-motion generation lacks bilingual datasets and suffers from poor cross-lingual semantic understanding in existing language models, hindering cross-linguistic applications.

Method: Created BiHumanML3D benchmark via LLM-assisted annotation and manual correction; proposed Bilingual Motion Diffusion (BiMD) with Cross-Lingual Alignment (CLA) that explicitly aligns semantic representations across languages.

Result: BiMD with CLA achieves FID 0.045 vs 0.169 and R@3 82.8% vs 80.8%, significantly outperforming monolingual diffusion models and translation baselines on BiHumanML3D, including zero-shot code-switching scenarios.

Conclusion: The dataset and alignment strategy are critical for cross-lingual motion synthesis, demonstrating the necessity of bilingual benchmarks and explicit cross-lingual semantic alignment for high-quality motion generation.
...
TL;DR: VideoTIR: Reinforcement Learning-based framework for efficient long video understanding in MLLMs using multi-level toolkits and TAGPO optimization to reduce hallucinations and redundant tool calls.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from hallucinations in long video understanding due to imbalance between textual and visual tokens. Existing SFT-based tool-calling methods require vast fine-grained data and have constrained trajectories. Need for efficient long video understanding that can properly use comprehensive toolkits.

Method: VideoTIR leverages RL to encourage proper usage of multi-level toolkits for efficient LVU. Uses Zero-RL and SFT cold-starting to enable MLLMs to retrieve meaningful video segments/images/regions. Introduces Toolkit Action Grouped Policy Optimization (TAGPO) with stepwise reward assignment and reuse of failed rollouts to reduce redundant tool-calling. Develops sandbox-based trajectory synthesis framework for high-quality trajectory data generation.

Result: Extensive experiments on three long-video QA benchmarks demonstrate effectiveness and efficiency of the method. The approach enhances long video understanding both accurately and efficiently compared to existing methods.

Conclusion: VideoTIR provides a novel RL-based solution for long video understanding in MLLMs that addresses hallucination issues through proper toolkit usage optimization and efficient tool-calling processes, outperforming existing approaches on benchmark evaluations.
...
TL;DR: PCGR is an interpretable framework for multimodal misinformation detection that uses concept graphs and hierarchical attention for structured reasoning, achieving state-of-the-art performance.
...
Conclusion: PCGR provides an interpretable and evolvable framework that addresses limitations of traditional detectors by enabling structured concept-based reasoning with transparent evidence-to-conclusion chains.
...
TL;DR: CARE is a training-free controllable medical image restoration framework that balances structure preservation and prior-guided enhancement using a dual-latent strategy with adaptive control during inference.


<details>
  <summary>Details</summary>
Motivation: Existing medical image restoration methods require task-specific retraining and lack controllability over the trade-off between faithful reconstruction and prior-driven enhancement, which can be problematic in clinical settings where aggressive restoration may introduce hallucinated details or alter diagnostically important structures.

Method: CARE uses a dual-latent restoration strategy with one branch enforcing data fidelity and anatomical consistency, and another branch leveraging a generative prior to recover missing/degraded information. A risk-aware adaptive controller dynamically adjusts branch contributions based on restoration uncertainty and local structural reliability, enabling conservative or enhancement-focused restoration modes without additional training.
...
Conclusion: CARE offers a practical step toward safer, more controllable, and more deployment-ready medical image restoration by providing explicit control over the structure-preservation vs. enhancement trade-off during inference without requiring model retraining.
...
TL;DR: GeoNDC is a neural data cube that encodes planetary-scale Earth observation data as a continuous spatiotemporal implicit neural field, enabling efficient storage, querying, and reconstruction with high compression ratios and fidelity.


<details>
  <summary>Details</summary>
Motivation: Massive spatiotemporal Earth observation archives are organized as discrete raster files, making them costly to store, transmit, and query. There's a need for more efficient representations that support on-demand access and analysis.

Method: GeoNDC encodes planetary-scale Earth observation data as a continuous spatiotemporal implicit neural field, enabling on-demand queries and continuous-time reconstruction without full decompression. It uses neural networks to represent the data as a queryable function.

Result: GeoNDC achieves 95:1 compression ratio on 20-year MODIS archive (0.44GB vs optimized baseline), with high spectral fidelity (mean R² > 0.98, mean RMSE = 0.021). It supports direct spatiotemporal queries on consumer hardware, recovers cloud-free dynamics under simulated occlusion (R² > 0.85), and attains near-perfect accuracy on biophysical products (R² > 0.98).
...
TL;DR: MoRGS is an online 4D reconstruction framework that improves dynamic scene modeling by explicitly reasoning about per-Gaussian motion using optical flow cues, motion offset fields, and motion confidence to separate static from dynamic regions.


<details>
  <summary>Details</summary>
Motivation: Existing online 4D reconstruction methods using 3D Gaussian Splatting fail to learn per-Gaussian motion that reflects true scene dynamics. Without explicit motion cues, appearance and motion are optimized solely under photometric loss, causing per-Gaussian motion to chase pixel residuals rather than true 3D motion.

Method: MoRGS leverages optical flow on sparse key views as lightweight motion cues to regularize per-Gaussian motion beyond photometric supervision. It learns a per-Gaussian motion offset field to reconcile discrepancies between projected 3D motion and observed flow across views and time. Additionally, it introduces per-Gaussian motion confidence that separates dynamic from static Gaussians and weights Gaussian attribute residual updates.
...
Conclusion: MoRGS effectively addresses the limitation of existing online 4D reconstruction methods by explicitly modeling per-Gaussian motion with optical flow supervision, motion offset fields, and motion confidence, leading to improved dynamic scene reconstruction with better temporal consistency.
...
TL;DR: GaussFusion improves 3D Gaussian splatting reconstructions using geometry-informed video generation to fix artifacts like floaters, flickering, and blur, achieving state-of-the-art performance with real-time capability.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian splatting (3DGS) suffers from artifacts including floaters, flickering, and blur caused by camera pose errors, incomplete coverage, and noisy geometry initialization, especially in real-world scenarios.

Method: Introduces a geometry-informed video-to-video generator that refines 3DGS renderings by processing Gaussian primitive video buffers encoding depth, normals, opacity, and covariance. Includes an artifact synthesis pipeline to simulate diverse degradation patterns for robustness.
...
Conclusion: GaussFusion effectively mitigates 3DGS artifacts through geometry-informed video generation, offering both high-quality reconstruction and real-time performance for practical applications.
...
TL;DR: A closed-loop Event-SVE system combines spatially variant exposure cameras with stereo neuromorphic event cameras for real-time 3D combustion monitoring under extreme HDR, smoke-obscured conditions with microsecond resolution.


<details>
  <summary>Details</summary>
Motivation: Real-time monitoring of high-energy propellant combustion is challenging due to extreme HDR, microsecond-scale particle motion, and heavy smoke that cause saturation, motion blur, and unstable particle extraction in conventional imaging systems.
...
Result: The system achieves maximum calibration error of 0.56% and captures multimodal equivalent-radius statistics in boron-based propellant experiments. It successfully captures fast separation transients that are difficult to observe with conventional sensors.

Conclusion: The proposed framework provides a practical, calibration-consistent route to microsecond-resolved 3D combustion measurement under smoke-obscured HDR conditions, addressing key challenges in high-energy propellant combustion monitoring.
...
TL;DR: A method for high-quality dynamic Gaussian Splatting from monocular videos using SE(3) B-spline motion bases with adaptive control points and multi-view diffusion guidance.


<details>
  <summary>Details</summary>
Motivation: To achieve high-quality dynamic scene reconstruction from monocular videos by explicitly modeling continuous position and orientation deformation of dynamic Gaussians, going beyond previous methods that lack explicit continuous motion modeling.

Method: Uses SE(3) B-spline motion bases with compact control points to model continuous deformation; includes adaptive control mechanism to dynamically adjust motion bases and control points; employs soft segment reconstruction to mitigate long-interval motion interference; utilizes multi-view diffusion model to provide multi-view cues and prevent overfitting to training views.
...
Conclusion: The proposed approach effectively models complex dynamic motions with computational efficiency, achieving high-quality dynamic Gaussian Splatting from monocular videos through explicit SE(3) deformation modeling, adaptive control, and multi-view guidance.
...
TL;DR: GIFT is a training-free framework for video LLMs that selects frames based on their irreplaceability, combining relevance and diversity, with budget-aware refinement to optimize frame selection for long-form video understanding.
...
Method: GIFT uses Directed Diversity to quantify frame uniqueness conditioned on relevance, forming a unified irreplaceability score. Budget-Aware Refinement employs adaptive iterative process: first secures core frames with highest irreplaceability, then prioritizes building temporal context as budget expands.
...
Conclusion: GIFT provides an effective training-free framework for frame selection in VLMs by assessing intrinsic irreplaceability, addressing limitations of existing greedy methods and improving long-form video understanding performance.
...
TL;DR: Z-Erase is the first concept erasure method for single-stream diffusion transformers that prevents generation collapse through stream disentanglement and Lagrangian-guided adaptive modulation.


<details>
  <summary>Details</summary>
Motivation: Concept erasure is crucial for safety in T2I models, but existing methods fail on single-stream architectures like Z-Image where text and image tokens are processed as a unified sequence, causing generation collapse.

Method: 1) Stream Disentangled Concept Erasure Framework that decouples updates to enable existing methods on single-stream models; 2) Lagrangian-Guided Adaptive Erasure Modulation, a constrained algorithm balancing erasure-preservation trade-off.

Result: Z-Erase successfully overcomes generation collapse and achieves state-of-the-art performance across a wide range of tasks, with rigorous convergence analysis proving it can converge to a Pareto stationary point.
...
TL;DR: RLVR for MLLMs faces token interdependence challenge between perception and reasoning tokens; proposed Token-Reweighting (ToR) strategy dynamically reweights critical tokens during training, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: MLLM responses interleave perception-related tokens (visual grounding) with reasoning-related tokens (symbolic reasoning), creating interdependent capacities that make isolated optimization insufficient for RLVR extension to multimodal models.

Method: Proposed Token-Reweighting (ToR) strategy that explicitly models interdependence by identifying critical perception and reasoning tokens and dynamically reweighting them during RLVR training; implemented as plug-and-play on existing methods like GRPO and DAPO.

Result: ToR delivers consistent performance gains across multiple multi-modal reasoning benchmarks, achieving state-of-the-art performance with both accurate visual grounding and coherent reasoning; empirical analysis shows optimizing either perception- or reasoning-only tokens consistently underperforms full optimization.

Conclusion: The interdependence between perception and reasoning tokens in MLLMs necessitates explicit modeling; ToR effectively addresses this challenge through dynamic token reweighting, enabling successful RLVR extension to multimodal models with improved performance.
...
TL;DR: HCD uses channel-level causal masks and MMI objective to separate causal from spurious features for improved OOD generalization.


<details>
  <summary>Details</summary>
Motivation: Deep learning models develop shortcut dependencies on domain-specific features, causing inconsistent OOD performance. Current invariance learning methods struggle with highly mixed features in deep latent spaces.

Method: Hierarchical Causal Dropout (HCD) uses channel-level causal masks to enforce feature sparsity, separating causal from spurious features via causal intervention at representation level. Training guided by Matrix-based Mutual Information objective to minimize MI between features and domain labels while maximizing MI with class labels. StyleMix-driven VICReg module ensures stability by preventing masks from filtering essential causal data.

Result: HCD outperforms existing top-tier methods on OOD benchmarks.

Conclusion: HCD effectively addresses shortcut learning problem by separating causal from spurious features through hierarchical causal dropout mechanism, achieving superior OOD generalization.
...
TL;DR: CLVA addresses object hallucination in multimodal LLMs by identifying attention drift in deep layers and reinforcing critical mid-layer visual anchors to suppress regressive noise, achieving improved performance without training or significant computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing methods for addressing object hallucination in multimodal LLMs lack sufficient interpretability regarding attention drift in final model stages. The authors investigate layer-wise evolution of visual features to understand the root causes of hallucination.

Method: CLVA (Cross-Layer Visual Anchors) is a training-free method that reinforces critical mid-layer features while suppressing regressive noise. It identifies that hallucination stems from deep layer attention regressing toward initial visual noise from early layers, and output reliability depends on acquiring visual anchors at intermediate layers. The approach pulls deep layer attention back to correct visual regions using essential anchors captured from attention dynamics.

Result: The method demonstrates outstanding performance across diverse architectures and benchmarks without significant increase in computational time and GPU memory. It effectively addresses object hallucination by leveraging insights about attention drift and visual anchor acquisition.

Conclusion: CLVA provides an effective, training-free solution to object hallucination in multimodal LLMs by addressing the fundamental issue of attention regression in deep layers and reinforcing critical mid-layer visual features, offering improved interpretability and performance without computational penalties.
...
TL;DR: THEMIS is a multimodal benchmark for evaluating MLLMs on visual fraud reasoning in academic scenarios, featuring 4,000+ questions across 7 real-world scenarios, 5 fraud types with 16 fine-grained manipulations, and multi-dimensional capability assessment.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the complexity and real-world relevance needed to properly evaluate multimodal LLMs for visual fraud reasoning in academic contexts, creating a gap between benchmark performance and practical application.

Method: Developed THEMIS benchmark with: 1) 4,000+ questions from real retracted-paper cases and synthetic multimodal data across 7 scenarios; 2) 5 fraud types with 16 fine-grained manipulation operations; 3) Mapping fraud types to 5 core visual fraud reasoning capabilities for multi-dimensional evaluation.

Result: Evaluation of 16 leading MLLMs shows even the best model (GPT-5) achieves only 56.15% overall performance, demonstrating THEMIS presents a stringent test that reveals distinct model strengths and weaknesses across core capabilities.

Conclusion: THEMIS bridges the gap between existing benchmarks and real-world academic fraud complexity, providing a comprehensive evaluation framework that should advance MLLM development for complex visual fraud reasoning tasks.
...
TL;DR: Pixelis is a pixel-space agent that learns through action using executable operations on images/videos, trained via three-phase approach with supervised fine-tuning, curiosity-coherence reward fine-tuning, and pixel test-time RL, achieving consistent improvements across benchmarks while maintaining safety controls.


<details>
  <summary>Details</summary>
Motivation: Current vision-language systems are static observers limited to describing pixels without acting or safely adapting under distribution shift. This passivity restricts physically grounded visual intelligence. Learning through action rather than static description is essential for generalizable visual intelligence beyond curated data.

Method: Three-phase training: (1) Supervised Fine-Tuning learns pixel-tool grammar from Chain-of-Thought-Action traces with masked imitation loss that upweights operation/argument tokens and uses auxiliary heads to stabilize pixel-grounded arguments; (2) Curiosity-Coherence Reward Fine-Tuning optimizes dual-drive objective combining prediction-error curiosity with adjacent-step coherence and mild efficiency prior under KL anchor; (3) Pixel Test-Time RL performs label-free adaptation by retrieving neighbors, voting over complete trajectories, and updating toward short, high-fidelity exemplars with KL-to-EMA safety control.

Result: Across six public image and video benchmarks, Pixelis yields consistent improvements: average relative gain of +4.08% over same 8B baseline (peaking at +6.03% on VSI-Bench), while producing shorter, auditable toolchains and maintaining in-corridor KL during test-time learning.

Conclusion: Acting within pixels rather than abstract tokens grounds multimodal perception in the physical world, links visual reasoning with actionable outcomes, and enables embodied adaptation without external feedback, addressing limitations of static vision-language systems.
...
TL;DR: RL-MBA: Reinforcement learning framework for multimodal active learning that dynamically adapts modality weights and sample selection based on difficulty, outperforming baselines on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning requires large labeled datasets, which are expensive. Active learning helps by selecting informative samples, but existing multimodal AL methods assume static modality importance and fixed selection rules, ignoring the dynamic nature where modality value and instance difficulty change during training.

Method: RL-MBA models sample selection as a Markov Decision Process with reinforcement learning. Key components: (1) Adaptive Modality Contribution Balancing (AMCB) dynamically adjusts modality weights via reinforcement feedback, and (2) Evidential Fusion for Difficulty-Aware Policy Adjustment (EFDA) estimates sample difficulty using uncertainty-based evidential fusion to prioritize informative samples.
...
Conclusion: RL-MBA effectively addresses the dynamic nature of multimodal active learning by adaptively balancing modality contributions and prioritizing difficult samples, leading to better performance and fairness compared to static approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current RLVR-based training for multimodal reward models requires expensive labeled multimodal preference data, limiting scalability. There's a need for methods that can scale MRM training without extensive multimodal annotations.
...
Result: MSRL substantially improves MRM performance: from 66.6% to 75.9% on VL-RewardBench (visual understanding) and from 70.2% to 75.7% on GenAI-Bench (visual generation), without requiring additional multimodal preference annotations.

Conclusion: MSRL provides an effective framework for scalable reinforcement learning of multimodal reward models, overcoming data limitations by transferring reward reasoning capabilities from text to multimodal domains through progressive training stages.
...
TL;DR: A lightweight, procedural data augmentation method using Moire interference patterns synthesized via closed-form math, achieving improved robustness with negligible computational cost and no external data.


<details>
  <summary>Details</summary>
Motivation: Current data augmentation methods often rely on computationally expensive diffusion-based synthesis or complex feature mixing strategies that require external datasets. The authors aim to develop a more efficient, storage-free alternative that doesn't depend on external data sources.

Method: Procedural augmentation using analytic interference patterns (Moire patterns) generated on-the-fly via closed-form mathematical formulation. Patterns are synthesized directly in memory, mixed with training images during training, and immediately discarded, creating a storage-free pipeline with minimal computational overhead (0.0026 seconds per image).
...
Conclusion: Analytic interference patterns provide a practical and efficient alternative to data-driven generative augmentation methods, offering structured perturbations across wide spatial frequencies with negligible computational cost and no external data requirements.
...
TL;DR: AnyDoc is a framework for multi-task document generation using HTML/CSS format, featuring a scalable data synthesis pipeline that creates DocHTML dataset (265K samples across 111 categories/32 styles), and fine-tunes MLLMs with height-aware reinforcement learning to handle content overflow.


<details>
  <summary>Details</summary>
Motivation: To advance AI-driven document generation by creating a unified framework capable of handling multiple generation tasks across diverse document categories, overcoming limitations of existing human-crafted datasets which have limited coverage and scale.

Method: 1) Develops scalable data synthesis pipeline to automatically generate HTML/CSS documents, creating DocHTML dataset with comprehensive metadata. 2) Fine-tunes multi-modal large language models (MLLMs) for three tasks: intention-to-document, document derendering, and element-to-document. 3) Implements height-aware reinforcement learning (HARL) post-training with reward function based on predicted vs. target document height differences to penalize and mitigate content overflow.

Result: AnyDoc outperforms both general-purpose MLLMs and task-specific baselines across all three document generation tasks in qualitative and quantitative experiments, demonstrating effective handling of content overflow through HARL.

Conclusion: AnyDoc successfully establishes a comprehensive framework for multi-task document generation with scalable data synthesis, effective MLLM fine-tuning, and innovative overflow mitigation through height-aware reinforcement learning, advancing the field of AI-driven document creation.
...
<details>
  <summary>Details</summary>
Motivation: While 3D Vision Foundation Models (3DVFMs) show strong zero-shot capabilities in visual geometry estimation, they struggle with generalizable novel view synthesis (NVS) due to pose-geometry discrepancies and degraded primitives in pose-free settings.

Method: Two key contributions: (1) Self-Consistent Pose Alignment (SCPA) - a training-time feedback loop ensuring pixel-aligned supervision to resolve pose-geometry discrepancy; (2) Rating-based Opacity Matching (ROM) - leverages local 3D geometry consistency knowledge from a sparse-view NVS teacher model to filter out degraded primitives.
...
TL;DR: RPCC is a Bayesian framework for robust principal component completion that identifies sparse foreground indirectly through support determination, eliminating post-hoc thresholding and handling occlusion scenarios better than traditional RPCA.


<details>
  <summary>Details</summary>
Motivation: Traditional RPCA assumes sparse foreground is additive to low-rank background, but in many applications (like video foreground extraction), sparse foreground actually occludes/replaces background elements. This mismatch requires a new approach that handles occlusion properly.

Method: Proposes robust principal component completion (RPCC) framework using variational Bayesian inference applied to fully probabilistic Bayesian sparse tensor factorization. Identifies sparse component indirectly by determining its support, with convergence to hard classifier eliminating post-hoc thresholding.

Result: Method delivers near-optimal estimates on synthetic data and robust foreground-extraction performance on real color video datasets, plus anomaly-detection performance on hyperspectral datasets. Convergence to hard classifier eliminates thresholding requirements.

Conclusion: RPCC provides a principled Bayesian solution to the occlusion problem in robust decomposition, offering improved handling of sparse foreground that replaces rather than adds to background, with practical benefits for video foreground extraction and anomaly detection.
...
TL;DR: DAPASS is a source-free unsupervised domain adaptation framework for panoramic semantic segmentation that addresses geometric distortions and annotation costs through confidence-guided pseudo-label denoising and contextual resolution adversarial alignment.


<details>
  <summary>Details</summary>
Motivation: Panoramic semantic segmentation faces challenges from geometric distortions in 360° projections and high annotation costs. While UDA from pinhole-camera datasets helps, real-world privacy/proprietary constraints require source-free adaptation, which exacerbates domain shift issues leading to unreliable pseudo-labels and poor minority class performance.
...
Result: State-of-the-art performances on outdoor (Cityscapes-to-DensePASS) and indoor (Stanford2D3D) benchmarks: 55.04% mIoU (+2.05% improvement) and 70.38% mIoU (+1.54% improvement), respectively.

Conclusion: DAPASS effectively addresses source-free panoramic semantic segmentation challenges through synergistic pseudo-label refinement and adversarial resolution alignment, demonstrating significant performance gains on both outdoor and indoor benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal deepfake detectors suffer from dataset and generator bias due to training primarily on curated synthetic forgeries, limiting scalability and robustness to unseen manipulations. There's a need for approaches that can learn detection cues without relying on synthetic training data.

Method: SAVe uses self-supervised learning on authentic videos by: 1) Generating identity-preserving, region-aware self-blended pseudo-manipulations to emulate tampering artifacts across multiple facial granularities, 2) Modeling lip-speech synchronization via an audio-visual alignment component to detect temporal misalignment patterns characteristic of audio-visual forgeries.

Result: Experiments on FakeAVCeleb and AV-LipSync-TIMIT datasets demonstrate competitive in-domain performance and strong cross-dataset generalization, showing the effectiveness of self-supervised learning for multimodal deepfake detection.

Conclusion: Self-supervised learning on authentic videos provides a scalable paradigm for multimodal deepfake detection that avoids dataset and generator bias, enabling robust detection of subtle visual artifacts and cross-modal inconsistencies in deepfakes.
...
TL;DR: Ranking optimization outperforms binary DPO for vision-language models by better handling visual faithfulness in responses, requiring vision encoder finetuning for effectiveness.


<details>
  <summary>Details</summary>
Motivation: Binary DPO's "winner-takes-all" approach is suboptimal for vision-language models where response quality depends on visual content; losing responses may still be faithful to visual inputs, which standard DPO formulation lacks nuance for.

Method: Investigates ranking optimization as alternative to binary DPO, focusing on video-text alignment using detailed captions; generates challenging, totally ordered caption chains at scale through repeated caption degradation.

Result: Ranking optimization outperforms binary DPO for long-form content generation and assessment; approaches require finetuning of vision encoder to be effective, challenging DPO as purely language-reweighting process.

Conclusion: Ranking optimization better handles visual faithfulness nuances than binary DPO for vision-language models, and vision encoder finetuning is essential for effectiveness, contradicting DPO as purely language-focused approach.
...
Method: Introduce EgoXtreme dataset captured entirely from egocentric perspective with three challenging scenarios: industrial maintenance, sports, and emergency rescue, designed to introduce severe perceptual ambiguities through extreme lighting, heavy motion blur, and smoke.

Result: State-of-the-art generalizable pose estimators fail to generalize to extreme conditions, especially under low light. Image restoration (deblurring) offers no positive improvement, while tracking-based approaches show performance gains, indicating temporal information is meaningful in fast-motion scenarios.
...
TL;DR: FD² is a fine-grained dataset distillation framework that addresses limitations of existing decoupled methods by localizing discriminative regions and constructing fine-grained representations to preserve intra-class diversity and inter-class differences.


<details>
  <summary>Details</summary>
Motivation: Existing decoupled dataset distillation methods rely on coarse class-label supervision and optimize samples within each class similarly, which is problematic for fine-grained datasets where this approach yields distilled samples that retain large intra-class variation with subtle inter-class differences and become overly similar within the same class, limiting localized discriminative cues and hurting recognition performance.

Method: FD² localizes discriminative regions and constructs fine-grained representations for distillation. During pretraining, counterfactual attention learning aggregates discriminative representations to update class prototypes. During distillation, a fine-grained characteristic constraint aligns each sample with its class prototype while repelling others, and a similarity constraint diversifies attention across same-class samples.
...
Conclusion: FD² effectively addresses the limitations of existing decoupled dataset distillation methods for fine-grained datasets by preserving discriminative cues and maintaining intra-class diversity while enhancing inter-class separation, demonstrating strong performance across various datasets.
...
TL;DR: Researchers created a public skin disease dataset from Bangladesh with 1612 images (250 distinct + augmented) covering 5 common diseases, and tested ML/DL models for automated diagnosis.


<details>
  <summary>Details</summary>
Motivation: Skin disease detection is challenging in resource-limited regions like Bangladesh due to insufficient dermatological expertise and diagnostic instruments, potentially leading to severe health consequences including death. There's a need for automated detection systems using AI/ML techniques.

Method: Developed a publicly available dataset of 1612 images (250 distinct images with augmentation) collected from patients at Faridpur Medical College, Bangladesh, focusing on 5 prevalent skin diseases: Contact Dermatitis, Vitiligo, Eczema, Scabies, and Tinea Ringworm. Applied several machine learning and deep learning models for classification.

Result: Created a dataset with specific image counts: 302 Dermatitis, 381 Eczema, 301 Scabies, 316 Tinea Ringworm, and 312 Vitiligo images. The dataset, though regionally collected, covers diseases common across South Asia and potentially valuable for global ML-based dermatology applications.

Conclusion: This research provides a valuable public dataset for ML/DL researchers working on automated skin disease diagnosis, addressing the critical need for accessible dermatological diagnostic tools in resource-limited settings, with potential global applications especially in South Asian regions.
...
Method: TANL dynamically evaluates activation levels across a corpus dataset during testing, mining candidate labels with high activation responses. It identifies high-confidence test images online, accumulates their assignment probabilities to construct a label activation metric, and selects distribution-adaptive activated negative labels. A batch-adaptive variant uses activation information within the current testing batch. An activation-aware score function emphasizes negative labels with stronger activations.

Result: TANL significantly improves OOD detection performance across diverse backbones and task settings. On ImageNet benchmark, it reduces FPR95 from 17.5% to 9.8%. The method is training-free, test-efficient, and theoretically justified.

Conclusion: TANL effectively addresses the limitation of poor activation in traditional negative label approaches by dynamically mining test-time activated negative labels, achieving state-of-the-art OOD detection performance with theoretical grounding and practical efficiency.
...
TL;DR: Photon is a framework for 3D medical visual question answering that uses adaptive token compression with instruction-conditioned scheduling and surrogate gradients to reduce computational costs while maintaining volumetric continuity and accuracy.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs struggle with 3D medical imaging due to high computational costs; existing methods using 2D slices or fixed-length token compression disrupt volumetric continuity and obscure subtle findings.

Method: Photon represents 3D volumes with variable-length token sequences using instruction-conditioned token scheduling, surrogate gradient propagation, custom backpropagation with gradient restoration, and regularization objectives to mitigate language-only bias.
...
Conclusion: Photon provides an effective framework for 3D medical VQA that balances computational efficiency with accuracy through adaptive token compression and specialized training techniques.
...
TL;DR: TITAnD reformulates trajectory anomaly detection as a vision problem using Hyperspectral Trajectory Images and a Cyclic Factorized Transformer to unify dense/sparse GPS analysis and enable efficient multi-month detection.


<details>
  <summary>Details</summary>
Motivation: Current trajectory anomaly detection methods are fragmented: dense GPS methods preserve fine-grained evidence but have quadratic cost making multi-month analysis intractable, while scalable sparse stay-point methods discard this evidence. This forces separate architectures for each regime and prevents knowledge transfer.

Method: Proposes TITAnD with two key innovations: 1) Hyperspectral Trajectory Image (HTI) representation - a day × time-of-day grid with channels encoding spatial, semantic, temporal, and kinematic information that unifies dense/sparse modalities; 2) Cyclic Factorized Transformer (CFT) - factorizes attention along within-day and across-day axes to encode cyclic inductive bias of human routines while reducing attention cost by orders of magnitude.

Result: TITAnD achieves best AUC-PR across both sparse and dense benchmarks, surpassing vision models like UNet while being 11-75x faster than standard Transformer with comparable memory. Enables dense multi-month anomaly detection for the first time.

Conclusion: Vision reformulation combined with structure-aware modeling is essential for trajectory anomaly detection. The HTI representation unifies dense/sparse modalities under single framework, while CFT's cyclic factorization enables efficient multi-month analysis previously intractable with dense GPS methods.
...
### [81] [A Semantically Disentangled Unified Model for Multi-category 3D Anomaly Detection](https://arxiv.org/abs/2603.25159)
*SuYeon Kim,Wongyu Lee,MyeongAh Cho*

Main category: cs.CV

TL;DR: A novel unified 3D anomaly detection model that addresses Inter-Category Entanglement (ICE) by reconstructing features conditioned on disentangled semantic representations, achieving state-of-the-art performance on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Unified models for 3D anomaly detection suffer from Inter-Category Entanglement (ICE), where latent features from different categories overlap, causing incorrect semantic priors during reconstruction and unreliable anomaly scores.
...
Conclusion: The proposed method effectively addresses ICE in unified 3D anomaly detection models, enhancing reliability and performance through semantic disentanglement and conditioned reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Current video datasets lack depth in fine-grained physical activities needed for skill learning, requiring specialized sports-focused data with instructional content.

Method: Introduced SportSkills dataset with 360k+ instructional videos across 55 sports, containing 630k+ visual demonstrations paired with instructional narrations. Developed representation learning and mistake-conditioned video retrieval task formulation.
...
Conclusion: SportSkills enables fine-grained physical action understanding and bridges representation learning with actionable feedback generation through mistake-conditioned instructional video retrieval.
...
TL;DR: KGFP is a framework that predicts functional failures in object detectors by measuring semantic misalignment between detector features and visual foundation model embeddings, improving safety-critical object recall.


<details>
  <summary>Details</summary>
Motivation: Object detectors in safety-critical applications can fail silently without warning, missing critical objects like pedestrians. Traditional OOD detection methods identify unfamiliar inputs but don't predict functional failures of the detector itself.
...
Conclusion: KGFP provides an effective representation-based monitoring framework for predicting functional failures in object detectors, improving safety in critical applications by detecting when detectors are likely to miss safety-critical objects.
...
<details>
  <summary>Details</summary>
Motivation: Existing self-supervised learning approaches for point clouds focus on semantic awareness but transfer poorly to instance localization tasks, creating a gap that hinders the development of true 3D foundation models capable of supporting all downstream tasks.

Method: PointINS introduces an orthogonal offset branch to jointly learn semantic understanding and geometric reasoning. It employs two complementary regularization strategies: Offset Distribution Regularization (ODR) aligns predicted offsets with geometric priors, and Spatial Clustering Regularization (SCR) enforces local coherence using pseudo-instance masks.

Result: Extensive experiments across five datasets show PointINS achieves +3.5% mAP improvement for indoor instance segmentation and +4.1% PQ gain for outdoor panoptic segmentation compared to existing methods.

Conclusion: PointINS successfully bridges the gap between semantic and instance awareness in point cloud representations, paving the way for scalable 3D foundation models that can support diverse downstream tasks without requiring full finetuning.
...
<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging shows promise for autonomous driving but faces challenges including variable lighting conditions, wide depth-of-field ranges, dynamic scenes with fast-moving objects, real-time operation requirements, and limited computational resources on embedded platforms.
...
Result: The combination of application-specific challenges determines both the criteria for selecting appropriate HSI technologies and the development of custom vision algorithms that leverage spectral and spatial information from sensors.

Conclusion: HSI in autonomous driving requires careful technology selection and algorithm development to address domain-specific challenges while meeting real-time computational constraints.
...
TL;DR: ET-SAM is an efficient SAM-based framework for unified scene text detection and layout analysis that uses a lightweight point decoder to generate word heatmaps, reducing point prompts and accelerating inference by 3× while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Previous SAM-based methods for scene text detection and layout analysis rely on pixel-level text segmentation to sample thousands of foreground points as prompts, resulting in high inference latency and limited data utilization due to dependence on pixel-level annotations.

Method: Proposes ET-SAM with two decoders: 1) A lightweight point decoder that produces word heatmaps to generate few foreground points instead of thousands, eliminating excessive point prompts; 2) A joint training strategy leveraging heterogeneous text-level annotations from multiple datasets (multi-level, word-level only, line-level only) combined as unified training set; 3) Three sets of learnable task prompts in both decoders to mitigate dataset discrepancies.

Result: Achieves about 3× inference acceleration compared to previous SAM-based architecture while obtaining competitive performance on HierText; improves average F-score by 11.0% on Total-Text, CTW1500, and ICDAR15 datasets.

Conclusion: ET-SAM effectively addresses the inefficiency of previous SAM-based methods by reducing point prompts through word heatmaps and enabling joint training on heterogeneous datasets, achieving significant speed improvements while maintaining or improving detection performance.
...
### [87] [Insights on back marking for the automated identification of animals](https://arxiv.org/abs/2603.25535)
*David Brunner,Marie Bordes,Elisabeth Mayrhuber,Stephan M. Winkler,Viktoria Dorfer,Maciej Oczak*

Main category: cs.CV

TL;DR: Study analyzes effective back mark design for individual pig monitoring using machine learning, identifying key design considerations for optimal algorithm recognition.


<details>
  <summary>Details</summary>
Motivation: There's limited research on designing back marks for uniform-looking species like pigs, particularly for machine learning-based monitoring solutions that need guidelines for mark design that algorithms can effectively recognize.

Method: Trained a ResNet-50 neural network to classify ten pigs with unique back marks, then analyzed the model's predictions to identify significant design considerations for effective mark recognition.

Result: Analysis revealed that back marks must remain unambiguous under motion blur, diverse view angles, and occlusions caused by animal behavior. Design must also consider common data augmentation strategies like color, flip, and crop augmentations used during model training.

Conclusion: The insights provide valuable guidance for optimizing back mark design to support effective individual-level monitoring in both research studies and real-world applications using machine learning algorithms.
...
### [88] [Knowledge-Guided Adversarial Training for Infrared Object Detection via Thermal Radiation Modeling](https://arxiv.org/abs/2603.25170)
...
Method: Model thermal radiation relations based on rank order of gray values for different classes, quantify stability of inter-class thermal radiation relations, and propose Knowledge-Guided Adversarial Training (KGAT) that embeds infrared physical knowledge into adversarial training process.
...
Conclusion: Leveraging infrared physical knowledge (thermal radiation relations) through KGAT provides a principled approach to improve robustness of infrared object detection systems against various threats.
...
TL;DR: CLiBD introduces hierarchy-aware multimodal learning for biodiversity identification, improving taxonomic classification by 14% over baselines with better robustness to noise and missing modalities.
...
Method: Two end-to-end variants: CLiBD-HiR introduces Hierarchical Information Regularization (HiR) to shape embedding geometry across taxonomic levels; CLiBD-HiR-Fuse adds a lightweight fusion predictor supporting image-only, DNA-only, or joint inference with resilience to modality corruption.
...
Conclusion: Explicitly encoding biological hierarchy together with flexible fusion is key for practical biodiversity foundation models, enabling robust taxonomic prediction from imperfect inputs like specimen images and DNA barcodes.
...
TL;DR: AG-EgoPose is a dual-stream framework for egocentric 3D human pose estimation that integrates short- and long-range motion context with spatial features from fisheye camera input using parallel spatial and temporal streams fused via transformer decoder.


<details>
  <summary>Details</summary>
Motivation: Egocentric 3D human pose estimation is challenging due to severe perspective distortion, limited body visibility, and complex camera motion in first-person viewpoints. Existing methods relying on single-frame analysis or limited temporal fusion fail to effectively leverage rich motion context available in egocentric videos.

Method: Dual-stream framework with parallel spatial and temporal streams: 1) Spatial stream uses weight-sharing ResNet-18 encoder-decoder to generate 2D joint heatmaps and joint-specific spatial feature tokens; 2) Temporal stream uses ResNet-50 backbone with action recognition backbone to capture motion dynamics; 3) Complementary representations are fused and refined in transformer decoder with learnable joint tokens for joint-level integration of spatial and temporal evidence while maintaining anatomical constraints.
...
Conclusion: AG-EgoPose effectively addresses egocentric 3D pose estimation challenges by integrating short- and long-range motion context with fine-grained spatial cues through a novel dual-stream framework with transformer-based fusion, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with visual reasoning and default to verbalizing perceptual content, which limits fine-grained spatial and visual understanding. Existing approaches either rely on external modules or inefficient pixel-space reasoning.
...
Result: LanteRn shows consistent improvements in visual grounding and fine-grained reasoning across three perception-centric benchmarks (VisCoT, V*, and Blink).

Conclusion: Internal latent representations provide a promising direction for more efficient multimodal reasoning, enabling LMMs to interleave language with compact visual representations for improved visual understanding.
...
TL;DR: VolDiT is the first purely transformer-based 3D diffusion model for volumetric medical image synthesis, replacing convolutional U-Nets with volumetric patch embeddings and global self-attention, featuring a timestep-gated control adapter for segmentation mask conditioning.


<details>
  <summary>Details</summary>
Motivation: Existing 3D medical image generation methods rely on convolutional U-Net backbones in latent diffusion frameworks, which impose strong locality biases and limited receptive fields, constraining scalability, global context integration, and flexible conditioning.
...
Result: The model demonstrates improved global coherence, superior generative fidelity, and enhanced controllability compared to state-of-the-art 3D latent diffusion models based on U-Nets on high-resolution 3D medical image synthesis tasks.
...
TL;DR: The paper proposes two simple modifications to improve compositionality in contrastive vision-language models without harming their basic capabilities: using short concept-centric captions and introducing cross-modal attention-pooling.
...
Method: Two key modifications: 1) Using short concept-centric caption parts aligned with images (obtained via standard NLP software), and 2) Introducing parameter-free cross-modal attention-pooling to obtain concept-centric visual embeddings. Combined with simple auxiliary contrastive losses.

Result: Achieves state-of-the-art performance on standard compositionality benchmarks while maintaining or improving strong zero-shot and retrieval capabilities. No increase in inference cost.

Conclusion: The proposed approach effectively addresses compositionality limitations in V&L models through simple architectural changes that preserve fundamental capabilities, offering a practical solution without benchmark-specific data generation.
...
TL;DR: AnyID is an ultra-fidelity identity-preservation video generation framework that unifies heterogeneous identity inputs and enables precise attribute-level controllability through a primary-referenced generation paradigm with differential prompts.


<details>
  <summary>Details</summary>
Motivation: Current identity-preserving video generation methods are limited to single identity references, restricting creative flexibility and creating ambiguous settings that make it difficult to faithfully reproduce identities across novel contexts.

Method: AnyID features two core contributions: 1) a scalable omni-referenced architecture that unifies heterogeneous identity inputs (faces, portraits, videos) into a cohesive representation, and 2) a primary-referenced generation paradigm with differential prompts for precise attribute-level controllability. The framework is trained on a large-scale curated dataset and fine-tuned with reinforcement learning using human preference data.

Result: Extensive evaluations validate that AnyID achieves ultra-high identity fidelity and superior attribute-level controllability across different task settings, outperforming existing methods.

Conclusion: AnyID addresses the limitations of single-reference identity-preserving video generation by providing a flexible framework that handles diverse input formats while maintaining high fidelity and precise controllability, enabling more creative expression with beloved characters.
...
TL;DR: CardioDiT: A 4D latent diffusion transformer framework for synthesizing temporally coherent cine cardiac MRI without architectural factorization of space and time.


<details>
  <summary>Details</summary>
Motivation: Current generative models for cine cardiac MRI often factorize space and time or use auxiliary mechanisms like anatomical masks, which introduce structural biases that limit global context integration and can cause spatiotemporal discontinuities or physiologically inconsistent cardiac dynamics.

Method: CardioDiT uses a spatiotemporal VQ-VAE to encode 2D+t slices into compact latents, then employs a diffusion transformer to model these latents jointly as complete 3D+t volumes, coupling space and time throughout the generative process without architectural factorization.
...
Conclusion: Explicit 4D modeling with a diffusion transformer provides a principled foundation for spatiotemporal cardiac image synthesis, enabling continuous cardiac dynamics without the limitations of architectural factorization approaches.
...
TL;DR: TacSIm is a large-scale dataset and benchmark for tactical style imitation in football that focuses on replicating real-world team tactical behaviors from broadcast footage, with evaluation metrics for spatial and temporal similarity.


<details>
  <summary>Details</summary>
Motivation: Current football imitation research primarily optimizes reward-based objectives (goals scored, win rates) but pays less attention to accurately replicating real-world team tactical behaviors. There's a need for better tactical style imitation evaluation.

Method: TacSIm projects beginning positions and actions of all 22 players from Premier League broadcast footage onto a standard pitch coordinate system. It provides explicit style imitation tasks and evaluation protocols using spatial occupancy similarity and movement vector similarity metrics.
...
Conclusion: TacSIm establishes a rigorous benchmark for measuring and modeling style-aligned tactical imitation tasks in football using unified data and metrics from broadcast to simulation.
...
Method: Proposes CIV-DG, a causal framework leveraging Conditional Instrumental Variables to disentangle pathological semantics from scanner-induced artifacts. It relaxes strict random assignment assumptions of standard IV methods to accommodate clinical scenarios with endogenous hospital selection. Implemented via Deep Generalized Method of Moments architecture with conditional critic to minimize moment violations and enforce instrument-error orthogonality within demographic strata.
...
Conclusion: CIV-DG provides an effective solution to structural confounding in medical AI by addressing selection bias through conditional causal mechanisms, enabling robust cross-site generalization where conventional domain generalization approaches fail.
...
TL;DR: FreeLOC is a training-free, layer-adaptive framework that addresses out-of-distribution problems in long-video generation using video diffusion models, achieving state-of-the-art results without additional training.


<details>
  <summary>Details</summary>
Motivation: Pre-trained video diffusion models trained on short clips struggle with long-video generation due to two out-of-distribution problems: frame-level relative position OOD and context-length OOD, leading to degraded visual quality.

Method: Proposes two core techniques: Video-based Relative Position Re-encoding (VRPR) for frame-level relative position OOD using hierarchical temporal re-encoding, and Tiered Sparse Attention (TSA) for context-length OOD using structured attention density. Includes layer-adaptive probing to selectively apply methods based on each transformer layer's sensitivity.
...
Conclusion: FreeLOC effectively addresses OOD problems in long-video generation with video diffusion models through a training-free, layer-adaptive approach that maintains both local detail and long-range dependencies without additional training.
...
TL;DR: SDD-YOLO is a specialized small-target detection framework for ground-to-air anti-UAV surveillance, featuring a P2 high-resolution detection head, DFL-free/NMS-free architecture, and MuSGD training strategy, achieving 86.0% mAP@0.5 on the new DroneSOD-30K dataset with high inference speeds.


<details>
  <summary>Details</summary>
Motivation: Detecting small UAVs from ground-to-air perspective faces challenges including extremely low pixel occupancy, cluttered backgrounds, and real-time constraints. Existing YOLO-based detectors lack adequate feature resolution for sub-pixel targets and introduce deployment complexities.

Method: Proposes SDD-YOLO framework with: 1) P2 high-resolution detection head operating at 4× downsampling for fine-grained spatial details; 2) DFL-free, NMS-free architecture from YOLO26 for streamlined inference; 3) MuSGD hybrid training strategy with ProgLoss and STAL to mitigate gradient oscillation on sparse small-target signals; 4) DroneSOD-30K dataset with ~30K annotated images covering diverse conditions.

Result: SDD-YOLO-n achieves 86.0% mAP@0.5 on DroneSOD-30K, surpassing YOLOv5n baseline by 7.8 percentage points. Inference speeds: 226 FPS on NVIDIA RTX 5090 and 35 FPS on Intel Xeon CPU, demonstrating exceptional efficiency for edge deployment.

Conclusion: SDD-YOLO effectively addresses G2A anti-UAV surveillance challenges through specialized architectural design and training strategies, achieving state-of-the-art performance on small-target detection with high computational efficiency suitable for real-world deployment.
...
Method: Two-stage pipeline: 1) Detection via object mask proposals scored against rendered templates using pre-trained features, with multi-view matching and geometric consistency filtering; 2) Pose estimation via iterative hypothesis refinement with feature-metric scoring and cross-view attention, followed by occlusion-aware contour registration minimizing reprojection errors.
...
Conclusion: Novel flexible pipeline combining foundational models, multi-view geometry, and contour-based refinement enables training-free, marker-less detection and tracking of surgical instruments with high accuracy.
...
TL;DR: SAF (Spatial Alignment Framework) addresses the failure of transformation-based adversarial attacks on structured tasks like segmentation/detection by synchronously transforming labels with inputs to maintain spatial alignment.


<details>
  <summary>Details</summary>
Motivation: Existing transformation-based adversarial attacks (TAAs) work well for classification but fail on structured tasks (segmentation, detection) due to spatial misalignment between transformed inputs and untransformed labels, causing erroneous gradients.

Method: Proposes Spatial Alignment Framework (SAF) with Spatial Alignment (SA) algorithm that synchronously transforms labels with inputs during adversarial attack generation, maintaining spatial consistency for structured tasks.

Result: SAF significantly improves TAA transferability on structured tasks: reduces mIoU on Cityscapes from 24.50 to 11.34, on Kvasir-SEG from 49.91 to 31.80, and reduces mAP on COCO from 17.89 to 5.25.

Conclusion: Spatial alignment is crucial for effective transformation-based adversarial attacks on structured tasks; SAF provides a unified framework that enables highly transferable attacks by maintaining spatial consistency between inputs and labels.
...
<details>
  <summary>Details</summary>
Motivation: Existing preemptive robustification methods have practical limitations including reliance on surrogate classifiers, high computational costs from iterative optimization or trained generators, and poor interpretability. The paper aims to develop a simpler, more practical approach based on the correlation between texture intensity and robustness.

Method: Proposes using simple image sharpening as a pre-attack defense mechanism. This approach is surrogate-free (doesn't require well-trained classifiers), optimization-free (no iterative optimization needed), generator-free (no trained generators required), and human-interpretable (simple image processing operation).

Result: Extensive experiments show that sharpening yields remarkable robustness gains with low computational cost, especially effective in transfer scenarios where attacks are transferred between different models.

Conclusion: Image sharpening provides an efficient, practical preemptive robustification approach that addresses limitations of previous methods while maintaining effectiveness against adversarial perturbations.
...
<details>
  <summary>Details</summary>
Motivation: Spatial transcriptomics is expensive, creating need for methods to infer spatial gene expression from cheaper whole slide images. Existing graph neural network approaches have structural limitations due to pre-defined sparse graphs that miss potential interactions between spot pairs.

Method: FEAST models tissue as a fully connected graph using attention mechanisms to consider all pairwise interactions. It introduces negative-aware attention to model both excitatory and inhibitory biological interactions, and an off-grid sampling strategy to capture richer morphological context from intermediate regions beyond standard spot images.
...
Conclusion: FEAST addresses limitations of existing methods by using fully connected attention with negative-aware modeling and off-grid sampling, enabling more accurate spatial gene expression prediction with interpretable biological insights.
...
<details>
  <summary>Details</summary>
Motivation: Existing LiDAR point cloud compression methods struggle with extreme sparsity of high-precision geometric details, which hinders efficient context modeling and limits compression speed and performance.

Method: Two lightweight modules: 1) Geometry Re-Densification Module that iteratively densifies encoded sparse geometry, extracts dense-scale features, then sparsifies them for predictive coding; 2) Cross-scale Feature Propagation Module that leverages occupancy cues from multiple resolution levels to guide hierarchical feature propagation and reduce redundant feature extraction. Also includes integer-only inference pipeline for bit-exact cross-platform consistency.

Result: Demonstrates competitive compression performance at real-time speed, with bit-exact cross-platform consistency and avoidance of entropy-coding collapse observed in existing neural compression methods.

Conclusion: The proposed FastPCC framework provides an efficient solution for predictive lossless coding of LiDAR point clouds, achieving real-time performance while maintaining competitive compression ratios through lightweight modules and integer-only inference.
...
TL;DR: ViewSplat introduces view-adaptive 3D Gaussian splatting for novel view synthesis from unposed images, using dynamic MLPs to predict view-dependent residual updates for Gaussian primitives, achieving state-of-the-art fidelity with fast inference.


<details>
  <summary>Details</summary>
Motivation: Current feed-forward 3D Gaussian splatting methods suffer from a fidelity gap due to limited network capacity to regress static Gaussian primitives that satisfy all viewpoints. The authors aim to overcome this bottleneck by moving from static primitive regression to view-adaptive dynamic splatting.

Method: ViewSplat learns a view-adaptable latent representation that initially predicts base Gaussian primitives alongside dynamic MLP weights. During rendering, these MLPs take target view coordinates as input and predict view-dependent residual updates for each Gaussian attribute (3D position, scale, rotation, opacity, and color), enabling primitive rectification of initial estimation errors.

Result: Extensive experiments show ViewSplat achieves state-of-the-art fidelity while maintaining fast inference (17 FPS) and real-time rendering (154 FPS), significantly improving over static primitive regression approaches.

Conclusion: The view-adaptive dynamic splatting paradigm successfully addresses the fidelity limitations of static Gaussian primitive regression, enabling high-quality novel view synthesis from unposed images with efficient rendering performance.
...
TL;DR: EagleNet introduces energy-aware fine-grained relationship learning to generate context-aware enriched text embeddings for text-video retrieval by modeling text-frame interactions and incorporating frame contextual information.


<details>
  <summary>Details</summary>
Motivation: Existing text-video retrieval methods focus on video representations or cross-modal alignment, but recent approaches that enrich text expressiveness ignore rich interactions among internal video frames, leading to disparities between text and video representations.

Method: Proposes Energy-Aware Fine-Grained Relationship Learning Network (EagleNet) with: 1) Fine-Grained Relationship Learning (FRL) that constructs text-frame graphs to learn relationships and aggregate text candidates into enriched embeddings; 2) Energy-Aware Matching (EAM) to model text-frame interaction energy; 3) Sigmoid loss replacement for more effective cross-modal alignment and stable training.

Result: Extensive experiments demonstrate superiority across MSRVTT, DiDeMo, MSVD, and VATEX datasets.

Conclusion: EagleNet effectively addresses the limitation of ignoring frame contextual information in text enrichment for video retrieval, achieving improved performance through energy-aware fine-grained relationship learning and better cross-modal alignment.
...
TL;DR: V2U4Real is the first large-scale real-world multi-modal dataset for Vehicle-to-UAV cooperative perception, addressing limitations of ground-only V2V/V2I systems by combining ground vehicle and UAV perspectives to overcome occlusions and extend sensing range.


<details>
  <summary>Details</summary>
Motivation: Existing cooperative perception systems (V2V/V2I) are limited to ground-level collaboration, which cannot fully address large-scale occlusions or long-range perception in complex environments. There's a need for cross-view cooperation between ground vehicles and aerial platforms to enhance perception robustness.

Method: Created V2U4Real dataset collected by a ground vehicle and UAV equipped with multi-view LiDARs and RGB cameras, covering urban streets, university campuses, and rural roads under diverse traffic scenarios. Dataset includes over 56K LiDAR frames, 56K multi-view camera images, and 700K annotated 3D bounding boxes across four object classes.

Result: Established benchmarks for single-agent 3D object detection, cooperative 3D object detection, and object tracking. Comprehensive evaluations demonstrate the effectiveness of V2U cooperation in enhancing perception robustness and long-range awareness.

Conclusion: V2U4Real enables research in cross-view cooperative perception, showing that Vehicle-to-UAV cooperation can overcome limitations of ground-only systems by providing complementary perspectives that mitigate occlusions and extend perception range in complex environments.
...
TL;DR: GLIC proposes a graph neural network-based learned image compression framework with dual-scale graphs and adaptive connectivity to overcome rigid receptive fields of CNNs/Transformers, achieving state-of-the-art compression performance.


<details>
  <summary>Details</summary>
Motivation: Current learned image compression methods using CNNs or Transformers have rigid receptive fields and static connectivity patterns that couple non-redundant pixels due to Euclidean proximity, limiting their ability to adaptively capture spatially varying redundancy across images.

Method: Proposes Graph-based Learned Image Compression (GLIC) using Graph Neural Networks with dual-scale graphs for flexible, data-driven receptive fields and adaptive connectivity that dynamically adjusts neighbor count per node based on local content complexity.

Result: GLIC achieves state-of-the-art performance with BD-rate reductions of 19.29%, 21.69%, and 18.71% relative to VTM-9.1 on Kodak, Tecnick, and CLIC datasets respectively.
...
TL;DR: MacroData: A large-scale dataset of 400K multi-reference image pairs (up to 10 references) organized across four dimensions, with MacroBench benchmark for evaluation, showing substantial improvements in multi-reference generation tasks.
...
Method: Introduce MacroData (400K samples with up to 10 reference images) organized across four dimensions: Customization, Illustration, Spatial reasoning, and Temporal dynamics. Also propose MacroBench benchmark (4,000 samples) for standardized evaluation across task dimensions and input scales.
...
Conclusion: MacroData addresses the fundamental data bottleneck in multi-reference image generation, providing comprehensive coverage of the multi-reference space with structured supervision, while MacroBench enables standardized evaluation. Both will be publicly released.
...
TL;DR: HeSS is a two-stage sparsification method for VGGT that quantifies head-wise sensitivity to sparsification via Hessian approximation and reallocates attention budgets accordingly, reducing accuracy degradation under high sparsity.


<details>
  <summary>Details</summary>
Motivation: Global attention layers in VGGT suffer from quadratic computational costs, and existing sparsification methods cause substantial accuracy degradation due to applying uniform sparsity patterns across all attention heads, ignoring their heterogeneous sensitivity characteristics.

Method: Two-stage pipeline: 1) Measure head-wise sparsification sensitivity using Head Sensitivity Score (HeSS), which approximates Hessian with respect to two error terms on a calibration set; 2) HeSS-Guided Sparsification that reallocates total attention budget by assigning denser attention to sensitive heads and sparser attention to robust ones.

Result: HeSS effectively captures head-wise sparsification sensitivity, attention heads exhibit heterogeneous sensitivity characteristics, and the method mitigates performance degradation under high sparsity while maintaining strong robustness across varying sparsification levels.

Conclusion: The proposed HeSS method addresses the accuracy degradation problem in VGGT sparsification by exploiting heterogeneous head sensitivity, enabling efficient attention computation without substantial performance loss.
...
TL;DR: Comprehensive study of five circular-aware methods for image rotation estimation shows probabilistic approaches (circular Gaussian distribution) are most robust across architectures, while classification achieves best accuracy on matched backbones but has stability issues.
...
Result: Best configuration (classification with EfficientViT-B3) achieved 1.23° MAE on DRC-D dataset; circular Gaussian distribution with MambaOut Base achieved similar 1.24° with greater robustness. On COCO 2014, best configuration reached 3.71° MAE, improving over prior work, and 2.84° on larger COCO 2017 dataset.
...
<details>
  <summary>Details</summary>
Motivation: Existing sketch video colorization methods have three core limitations: inflexible user control due to heavy reliance on single reference frames, poor instance controllability leading to misalignment in multi-character scenarios, and degraded detail fidelity in fine-grained regions.

Method: Three key innovations: 1) Canvas Guidance Condition for flexible placement of reference elements and background, 2) Instance Matching Mechanism integrating instance features with sketches for precise multi-character control, 3) Adaptive Decoupled Control Module injecting semantic features from characters, backgrounds, and text conditions into the diffusion process.
...
Conclusion: InstanceAnimator successfully addresses the three core limitations of existing sketch video colorization methods, providing a robust framework for multi-instance scenarios with improved flexibility, alignment, and detail preservation.
...
<details>
  <summary>Details</summary>
Motivation: CLIP's computational demands necessitate lightweight student models, but existing distillation methods fail to preserve the structural relationships between teacher and student embeddings across modalities.

Method: Proposes two novel methods: Vertical Relational Distillation (VRD) for modality-level consistency of distillation strength, and Cross Relational Distillation (XRD) for bidirectional symmetry of cross-modal similarity distributions.
...
Conclusion: Explicit modeling of multi-directional relational dependencies enables more faithful CLIP distillation, producing efficient student models that better preserve teacher's structural relationships.
...
TL;DR: PTM-ST is a phased distillation framework that addresses limitations in multimodal dataset distillation by modeling teacher learning dynamics across training stages, improving stability and performance.
...
Method: Proposes Phased Teacher Model with Shortcut Trajectory (PTM-ST) - a phased distillation framework with stage-aware teacher modeling and shortcut-based trajectory construction to accurately fit teacher learning dynamics across distinct training phases.
...
Conclusion: PTM-ST effectively addresses critical challenges in multimodal dataset distillation by capturing teacher model dynamics across training stages, leading to improved stability, expressiveness, and performance in distilled datasets.
...
TL;DR: FSGNet: A lightweight infrared small target detection framework with frequency-aware and semantic guidance mechanisms to address semantic degradation in U-Net architectures, improving small target localization in complex backgrounds.
...
Method: Proposes FSGNet with three key components: 1) Multi-directional interactive attention module in encoder for fine-grained directional features, 2) Multi-scale frequency-aware module using Fast Fourier Transform to filter background clutter, 3) Global pooling module with semantic guidance flows to propagate high-level semantic information across decoder stages.
...
Conclusion: FSGNet effectively addresses semantic degradation in U-Net for IRSTD through frequency-aware filtering and semantic guidance mechanisms, achieving state-of-the-art performance while maintaining lightweight design for practical applications.
...
TL;DR: PMT introduces a fast Transformer-based decoder that works with frozen Vision Foundation Model features, achieving competitive segmentation performance while maintaining encoder sharing and low latency for both image and video tasks.


<details>
  <summary>Details</summary>
Motivation: To reconcile encoder-only segmentation models' simplicity and speed with the practical benefits of frozen VFM features that enable multi-task encoder sharing for large-scale deployment.

Method: Proposes Plain Mask Decoder (PMD), a fast Transformer-based segmentation decoder that operates on top of frozen VFM features, creating the Plain Mask Transformer (PMT) that preserves encoder-only simplicity while keeping encoder frozen and shareable.

Result: PMT matches frozen-encoder state-of-the-art on image segmentation while running ~3x faster, and performs on par with fully finetuned methods for video segmentation while being up to 8x faster than state-of-the-art frozen-encoder models.

Conclusion: PMT successfully reconciles encoder-only simplicity and speed with frozen VFM features, enabling practical multi-task encoder sharing while maintaining competitive performance and low latency for both image and video segmentation.
...
TL;DR: LaMP introduces a dual-expert VLA framework that embeds 3D scene flow as a latent motion prior to improve robotic manipulation, outperforming existing methods on multiple benchmarks with enhanced robustness to spatial perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action models regress actions directly from 2D semantic visual features, forcing implicit learning of complex 3D physical interactions which degrades under unfamiliar spatial dynamics.
...
Result: LaMP consistently outperforms VLA baselines on LIBERO, LIBERO-Plus, and SimplerEnv-WidowX benchmarks, achieving highest reported average success rates. On LIBERO-Plus OOD perturbations, it shows 9.7% average gain over strongest prior baseline.

Conclusion: Embedding 3D scene flow as a latent motion prior in a dual-expert framework effectively addresses limitations of implicit 3D physical interaction learning in VLA models, improving performance and robustness in robotic manipulation tasks.
...
TL;DR: A hierarchical framework for teaching VLMs 3D spatial understanding through four progressive levels, using automated generation of 5M images with 45M objects for supervised fine-tuning, achieving SOTA on spatial reasoning benchmarks.
...
Method: 1) Hierarchical framework decomposing 3D spatial learning into four progressively complex levels (geometric perception to abstract reasoning); 2) Automated pipeline generating 3D spatial VQA pairs from ~5M images with 45M+ objects; 3) RGB-D VLM incorporating metric-scale point maps as auxiliary inputs.

Result: Achieves state-of-the-art performance on multiple spatial understanding/reasoning benchmarks, surpassing specialized spatial models and large proprietary systems (Gemini-2.5-pro, GPT-5). Analysis reveals clear dependencies among hierarchical task levels.

Conclusion: The hierarchical framework effectively teaches VLMs 3D spatial intelligence, with multi-level task design facilitating emergence of spatial reasoning capabilities. The approach demonstrates superior performance over existing systems and provides insights into spatial intelligence development.
...
TL;DR: VideoWeaver is the first multimodal multi-view video-to-video translation framework that enables consistent appearance across multiple synchronized camera views for embodied AI applications, overcoming limitations of single-view approaches and scaling beyond fixed camera setups.


<details>
  <summary>Details</summary>
Motivation: Prior video-to-video translation works only operate on single views, while embodied AI tasks use multiple synchronized cameras. Applying single-view models independently leads to inconsistent appearance across views, and standard transformer architectures don't scale to multi-view settings due to quadratic attention costs.

Method: VideoWeaver is initially trained as a single-view flow-based V2V model, then extended to multi-view by grounding all views in a shared 4D latent space from Pi3 spatial foundation model. Views are trained at distinct diffusion timesteps to enable learning of joint and conditional view distributions, allowing autoregressive synthesis of new viewpoints.
...
Conclusion: VideoWeaver successfully addresses the multi-view consistency problem in video-to-video translation, enabling realistic resimulation of embodied AI demonstrations across multiple synchronized cameras, which is crucial for transferring pretrained robot policies to new environments without additional data collection.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for simultaneous transformation and correspondence estimation in point cloud registration suffer from local minima due to non-convex coupled objectives, while global solvers have prohibitive convergence times due to loose lower bounds in Branch-and-Bound search.

Method: Proposes DC-Reg framework with a holistic concave underestimator for the coupled transformation-assignment objective using Difference of Convex programming. Unlike prior term-wise relaxations, this captures joint structural interaction between transformation and correspondence variables, enabling tight lower bounds via efficient Linear Assignment Problems evaluated at search box vertices.

Result: DC-Reg achieves significantly faster convergence and superior robustness to extreme noise and outliers compared to state-of-the-art global techniques, validated on synthetic data and 3DMatch benchmark for both 2D similarity and 3D rigid registration.

Conclusion: The holistic DC decomposition approach provides a principled way to tighten Branch-and-Bound search for globally optimal point cloud registration, overcoming limitations of existing methods and enabling robust registration under challenging conditions with partial overlaps and large misalignments.
...
TL;DR: CIAR introduces a cloud-device collaboration framework for AR image generation that uses on-device uncertainty quantification to identify redundant tokens, enabling selective cloud processing and achieving 2.18x speed-up with 70% fewer cloud requests while maintaining image quality.
...
Method: CIAR uses on-device token uncertainty quantification with continuous probability intervals (instead of discrete solution sets) to identify redundant tokens. It incorporates an Interval-enhanced decoding module and distribution alignment training strategy to accelerate decoding while maintaining visual fidelity and semantic consistency.

Result: CIAR achieves 2.18x speed-up and reduces cloud requests by 70% while preserving image quality compared to existing methods, demonstrating efficient cloud-device collaboration for AR image generation.

Conclusion: The CIAR framework effectively addresses computational bottlenecks in AR image generation by leveraging on-device uncertainty quantification and selective cloud processing, enabling practical on-device deployment with significant performance improvements.
...
TL;DR: GridVAD is a training-free pipeline for video anomaly detection that uses Vision-Language Models as anomaly proposers, with self-consistency filtering and spatial-temporal grounding modules to generate pixel-level anomaly masks without domain-specific training.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) are powerful open-set reasoners but are fragile as direct anomaly detectors in video surveillance due to uncalibrated anomaly priors, leading to missed detections and false alarms. The problem is not the VLM itself but how it is used - VLMs should function as anomaly proposers rather than direct detectors.
...
Conclusion: VLMs should be used as anomaly proposers rather than direct detectors, with purpose-built spatial and temporal modules for grounding and propagation. GridVAD demonstrates that training-free pipelines can achieve competitive anomaly detection performance through proper architectural design, offering a more robust and efficient alternative to direct VLM application.
...
TL;DR: AdaSFormer is a serialized transformer framework for indoor monocular semantic scene completion that addresses memory constraints and detail reconstruction challenges through adaptive receptive fields, center-relative positional encoding, and convolution-modulated layer normalization.


<details>
  <summary>Details</summary>
Motivation: Indoor monocular semantic scene completion is more challenging than outdoor due to complex spatial layouts and severe occlusions. Transformers are suitable for global dependencies but have high memory costs and difficulty reconstructing fine-grained details, limiting their use in indoor MSSC.
...
Conclusion: AdaSFormer successfully addresses transformer limitations for indoor MSSC through specialized architectural designs, achieving superior performance on benchmark datasets while making the code publicly available for community use.
...
TL;DR: The paper addresses real-world image restoration by creating a large-scale dataset covering nine degradation types and training an open-source model to compete with closed-source alternatives, while introducing RealIR-Bench for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing restoration models have poor generalization to real-world scenarios due to limited training data scale and distribution. While large-scale closed-source models show strong restoration ability, they require substantial data and computational costs.

Method: Construct a large-scale dataset covering nine common real-world degradation types, train a state-of-the-art open-source model, and introduce RealIR-Bench containing 464 real-world degraded images with tailored evaluation metrics focusing on degradation removal and consistency preservation.
...
Conclusion: The work successfully narrows the gap between open-source and closed-source image restoration models by providing a comprehensive dataset, benchmark, and high-performing open-source model for real-world degradation scenarios.
...
TL;DR: CHIRP dataset and CORVID method for automated individual re-identification and behavior monitoring of wild birds using computer vision.


<details>
  <summary>Details</summary>
Motivation: Long-term behavioral monitoring of individual animals is crucial for conservation and evolutionary biology, but automated behavior monitoring in wild populations remains challenging due to lack of comprehensive datasets covering necessary computer vision tasks.

Method: Introduces CHIRP dataset from wild Siberian jays supporting re-id, action recognition, keypoint estimation, detection, and segmentation; and CORVID method for individual identification based on segmentation and classification of colored leg rings with probability-based tracking.
...
Conclusion: Provides a blueprint for curating real-world datasets from ethically approved biological studies to bridge computer vision research and biological applications.
...
TL;DR: TQD addresses the Motion-Vision Quality Dilemma in video generation by using timestep-aware sampling to train on imbalanced data, achieving better performance than using perfect data.
...
Method: Timestep-aware Quality Decoupling (TQD) modifies the data sampling distribution based on timestep selection in the training process. Motion-rich data is sampled more at higher timesteps, while high visual quality data is sampled more at lower timesteps, matching the model's hierarchical learning dynamics.
...
Conclusion: The Motion-Vision Quality Dilemma can be effectively addressed through timestep-aware training strategies, making it possible to achieve superior video generation performance without requiring perfect, balanced training data.
...
TL;DR: BFMD is the first badminton dataset with full-match dense multimodal annotations enabling accurate shot captioning and match-level tactical analysis.


<details>
  <summary>Details</summary>
Motivation: Existing badminton datasets focus on short clips with task-specific annotations, lacking full-match data with dense multimodal annotations needed for accurate shot captioning and comprehensive match-level tactical analysis.

Method: Created BFMD dataset with 19 broadcast matches (over 20 hours) covering singles and doubles, with 1,687 rallies and 16,751 hit events, each annotated with shot captions and hierarchical multimodal annotations. Developed VideoMAE-based multimodal captioning framework with Semantic Feedback mechanism leveraging shot semantics to guide caption generation.
...
Conclusion: BFMD addresses limitations of existing datasets by providing comprehensive full-match multimodal annotations, enabling improved shot captioning and match-level tactical analysis in badminton.
...
TL;DR: PAWS extracts object articulation information from hand-object interactions in egocentric videos without requiring supervised 3D data or manual annotations.
...
Result: The method achieves significant improvements over baselines on public datasets including HD-EPIC and Arti4D. Extracted articulations benefit downstream tasks like fine-tuning 3D articulation prediction models and enabling robot manipulation.

Conclusion: PAWS provides a scalable approach to articulation perception by leveraging hand-object interactions from egocentric videos, demonstrating practical utility for robotics, simulation, and animation applications.
...
TL;DR: Multimodal fusion of operating microscope (OPMI) and intraoperative OCT (iOCT) enables real-time instrument tracking and tool-tissue distance estimation in vitreoretinal surgery with significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: To leverage complementary imaging modalities (OPMI and iOCT) for comprehensive surgical scene understanding in ophthalmic surgery, specifically for precise instrument tracking and tool-tissue distance estimation in vitreoretinal procedures.

Method: Proposed a multimodal, temporal, real-time capable network architecture with cross-attention fusion module to merge OPMI (extracted via YoloNAS) and iOCT (extracted via CNN encoder) features, plus a region-based recurrent module for temporal coherence.

Result: Achieved 95.79% mAP50 for instrument localization/keypoint detection, real-time processing (22.5 ms/frame), and significant improvement in tool-tissue distance estimation accuracy (from 284 μm to 33 μm for distances below 1 mm).

Conclusion: Multimodal feature fusion enhances multi-task prediction accuracy compared to single-modality processing, demonstrates potential for image-guided vitreoretinal surgery, but highlights challenges for more reliable surgical scene understanding.
...
TL;DR: The paper introduces a comprehensive framework for height-aware remote sensing understanding in Large Multimodal Models, addressing the critical gap in vertical dimension reasoning through new benchmarks and a baseline model.


<details>
  <summary>Details</summary>
Motivation: Current Large Multimodal Models in Earth Observation neglect the critical "vertical" dimension, limiting their reasoning capabilities in complex remote sensing geometries and disaster scenarios where physical spatial structures often outweigh planar visual textures.

Method: 1) Developed a scalable, VLM-driven data generation pipeline using systematic prompt engineering and metadata extraction; 2) Constructed two complementary benchmarks: GeoHeight-Bench for relative height analysis and GeoHeight-Bench+ for holistic terrain-aware reasoning; 3) Proposed GeoHeightChat, the first height-aware remote sensing LMM baseline that synergizes visual semantics with implicitly injected height geometric features.

Result: The framework successfully bridges the "vertical blind spot" gap, with GeoHeightChat serving as a strong proof of concept that demonstrates effective mitigation of height perception limitations and unlocks a new paradigm of interactive height reasoning in existing optical models.

Conclusion: The work establishes a comprehensive evaluation framework for height-aware remote sensing understanding, validates the necessity of height perception through novel benchmarks and a baseline model, and demonstrates that integrating height geometric features with visual semantics can effectively address the vertical dimension gap in Earth Observation LMMs.
...
TL;DR: UNIC introduces an instance-specific neural deformation field method for real-time garment animation that avoids complex topology handling and improves deformation quality over previous approaches.


<details>
  <summary>Details</summary>
Motivation: Physics-based garment simulation is computationally expensive and unsuitable for real-time applications, while existing learning-based methods using graph neural networks fail to capture intricate deformations of complex garment meshes with complex topologies.

Method: UNIC learns instance-specific neural deformation fields to animate garment meshes, mapping 3D points to their deformation offsets. This approach avoids handling complex garment topologies and injects natural smoothness constraints, focusing on generalizing to new motion sequences rather than new garments.

Result: Extensive experiments demonstrate UNIC's effectiveness and efficiency over baseline methods on various garment meshes, achieving real-time performance suitable for interactive applications like video games.

Conclusion: UNIC provides a practical solution for real-time garment animation by leveraging instance-specific neural deformation fields, overcoming limitations of physics-based and graph neural network approaches while maintaining high deformation quality.
...
TL;DR: DeepFAN, a transformer-based model for lung nodule classification, achieved high diagnostic performance (AUC 0.939-0.954) and significantly improved junior radiologists' diagnostic metrics by 7.6-12.6% in a clinical trial.
...
Result: DeepFAN achieved AUC of 0.939 (internal test) and 0.954 (clinical trial). Junior radiologists' performance improved significantly: 10.9% AUC, 10.0% accuracy, 7.6% sensitivity, and 12.6% specificity. Inter-reader consistency improved from fair to moderate (k: 0.313 vs 0.421). Explainability analysis showed higher contributions from global features.

Conclusion: DeepFAN effectively assisted junior radiologists in lung nodule classification, potentially helping homogenize diagnostic quality and reduce unnecessary follow-up of indeterminate pulmonary nodules, with clinical trial registration ChiCTR2400084624.
...
TL;DR: This paper benchmarks demographic fairness of multimodal LLMs for face verification, finding specialized models outperform general ones and revealing complex bias patterns different from traditional face recognition systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the demographic fairness of Multimodal Large Language Models (MLLMs) when used for face verification, as their fairness remains largely unexplored despite their increasing use in this domain.

Method: The method involves benchmarking nine open-source MLLMs from six model families (2B to 8B parameters) on IJB-C and RFW face verification protocols across four ethnicity groups and two gender groups. They measure verification accuracy using Equal Error Rate and True Match Rate at multiple operating points, and quantify demographic disparity with four False Match Rate-based fairness metrics.

Result: Results show that FaceLLM-8B, the only face-specialized model, substantially outperforms general-purpose MLLMs on both benchmarks. Bias patterns differ from traditional face recognition, with different groups being most affected depending on benchmark and model. The most accurate models are not necessarily the fairest, and models with poor overall accuracy can appear fair due to uniformly high error rates across all demographic groups.

Conclusion: The study reveals complex demographic fairness patterns in MLLMs for face verification, highlighting that specialized models perform better but fairness considerations differ from traditional systems, and accuracy-fairness trade-offs exist that require careful evaluation beyond simple accuracy metrics.
...
TL;DR: Automated pipeline translates natural language imaging system descriptions into validated forward models using spec.md format and three agents, achieving expert-level quality across 6 modalities and enabling novel compositional designs.
...
Conclusion: The automated pipeline democratizes computational imaging system design by eliminating expertise bottlenecks and enabling novel compositional designs beyond traditional single-modality tools.
...
<details>
  <summary>Details</summary>
Motivation: Existing CVGL methods treat the problem as image retrieval using contrastive learning, which requires large batches and hard negative mining, ignores geometric map structure, and suffers from coverage mismatch where street-view landmarks may fall outside fixed satellite crops, making retrieval ambiguous and limiting spatial inference.

Method: Proposes Just Zoom In: an autoregressive zooming approach that starts from a coarse satellite view and makes sequential zoom-in decisions to select a terminal satellite cell at target resolution, eliminating contrastive losses and hard negative mining. Also introduces a realistic benchmark with crowd-sourced street views and high-resolution satellite imagery.

Result: Achieves state-of-the-art performance on the new benchmark, improving Recall@1 within 50m by 5.5% and Recall@1 within 100m by 9.6% over the strongest contrastive-retrieval baseline.

Conclusion: Sequential coarse-to-fine spatial reasoning is more effective for cross-view geo-localization than contrastive retrieval formulations, demonstrating the value of explicit spatial inference over maps and addressing coverage mismatch issues.
...
Result: LEMMA achieves state-of-the-art performance with significant reductions: up to 71x fewer trainable parameters, up to 88.5% reduction in GFLOPs, and up to 84.65% reduction in inference time compared to existing models. Specific results include 93.42% IoU on Oil Spill dataset and 98.97% mIoU on Mastr1325.

Conclusion: LEMMA provides an effective lightweight solution for semantic segmentation in marine environments, enabling practical real-time applications for disaster response, environmental surveillance, and coastal monitoring under resource constraints.
...
<details>
  <summary>Details</summary>
Motivation: Current unified models accept multi-modal inputs but produce only single-modality outputs, lacking interleaved generation capability due to training data scarcity and difficulty modeling long-range cross-modal context.
...
Conclusion: The proposed decomposition approach and data construction strategy enable effective interleaved generation, demonstrating that high-quality multi-modal output can be achieved through careful architectural design and synthetic data creation.
...
TL;DR: TRACE is a framework for object motion path editing in videos that allows users to design desired trajectories in a single anchor frame and generates temporally consistent edited videos, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Prior video editing methods focus on appearance manipulation or require point-track-based trajectory control, which is challenging for users, especially in videos with camera motion. There's a need for a practical, easy-to-use approach for controllable object-centric motion editing.
...
Conclusion: TRACE provides a practical framework for object motion path editing that enables users to easily design trajectories in a single anchor frame and generates high-quality, temporally consistent edited videos, addressing limitations of prior methods.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal Diffusion Large Language Models (MDLLMs) suffer from structural vulnerability to multimodal hallucinations due to an algorithmic flaw: the decoder ranks candidate tokens based solely on textual likelihood without verifying localized visual support, creating an objective mismatch where language probability acts as a misspecified proxy for multimodal tasks.

Method: VISAGE introduces a training-free decoding framework that estimates proxy discrepancy by quantifying spatial entropy of cross-attention distributions. It enforces localization consensus across attention heads, penalizing spatially uniform distributions and re-ranking token commitments to favor visually grounded outcomes.

Result: VISAGE demonstrates robustness across hallucination-sensitive and general-purpose benchmarks, achieving relative gains of 8.59% on MMMU-val and 7.75% on HallusionBench, with analytical stability guarantees for bounded objective loss under estimation error.

Conclusion: The VISAGE framework successfully addresses multimodal hallucinations in MDLLMs by recalibrating the objective mismatch at inference time through cross-attention analysis, providing a training-free solution that improves visual grounding without model retraining.
...
TL;DR: Hybrid Memory paradigm addresses limitations of current video world models in handling dynamic subjects that hide and re-emerge, introducing HM-World dataset and HyDRA architecture for improved motion continuity and subject consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video world models treat environments as static canvases and struggle when dynamic subjects hide out of sight and later re-emerge, leading to frozen, distorted, or vanishing subjects. Current memory mechanisms lack proper handling of motion continuity during out-of-view intervals.

Method: Proposes Hybrid Memory paradigm requiring models to act as precise archivists for static backgrounds and vigilant trackers for dynamic subjects. Introduces HM-World dataset (59K high-fidelity clips with decoupled camera/subject trajectories, 17 scenes, 49 subjects, exit-entry events) and HyDRA architecture that compresses memory into tokens with spatiotemporal relevance-driven retrieval mechanism for selective attention to motion cues.

Result: Extensive experiments on HM-World demonstrate that the proposed method significantly outperforms state-of-the-art approaches in both dynamic subject consistency and overall generation quality.

Conclusion: Hybrid Memory paradigm effectively addresses the challenge of preserving identity and motion of hidden subjects in video world models, with HM-World dataset enabling rigorous evaluation and HyDRA architecture providing specialized memory mechanisms for improved hybrid coherence.
...
TL;DR: AnyHand is a large-scale synthetic RGB-D dataset for 3D hand pose estimation that addresses limitations in existing datasets by providing extensive coverage with occlusions, arm details, and aligned depth, enabling significant performance improvements in both RGB-only and RGB-D settings.


<details>
  <summary>Details</summary>
Motivation: Existing real-world datasets for 3D hand pose estimation are limited in coverage, and prior synthetic datasets lack comprehensive features like occlusions, arm details, and aligned depth at scale, creating a bottleneck for advancing the field.

Method: Created a large-scale synthetic dataset (AnyHand) with 2.5M single-hand and 4.1M hand-object interaction RGB-D images featuring rich geometric annotations. Also developed a lightweight depth fusion module that integrates into existing RGB-based models.

Result: Using AnyHand for training yields significant gains on FreiHAND and HO-3D benchmarks in RGB-only settings, shows strong generalization to out-of-domain HO-Cap without fine-tuning, and achieves superior performance on HO-3D with RGB-D models using the depth fusion module.

Conclusion: AnyHand effectively addresses data limitations in 3D hand pose estimation, demonstrating that large-scale synthetic data with comprehensive features can substantially improve performance and generalization, while the depth fusion module enables effective RGB-D integration.
...
TL;DR: PixelSmile: A diffusion framework for fine-grained facial expression editing using symmetric joint training and contrastive learning to achieve precise linear control while preserving identity.


<details>
  <summary>Details</summary>
Motivation: Fine-grained facial expression editing has been limited by intrinsic semantic overlap between expressions, making precise control difficult. Existing methods struggle with disentangling expression semantics while preserving identity.
...
Result: Extensive experiments demonstrate superior disentanglement and robust identity preservation. Achieves precise and stable linear expression control, naturally supporting smooth expression blending. Validated on the constructed FFE dataset with continuous affective annotations.

Conclusion: PixelSmile effectively addresses semantic overlap in expression editing, enabling continuous, controllable, and fine-grained facial expression editing with strong identity preservation and smooth blending capabilities.
...
TL;DR: PackForcing: A unified framework for efficient long-video generation using hierarchical KV-cache compression with three-partition strategy (sink, mid, recent tokens) to address linear memory growth and temporal repetition in autoregressive video diffusion models.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion models face three key bottlenecks: intractable linear KV-cache growth, temporal repetition, and compounding errors during long-video generation, limiting their practical application for extended video synthesis.

Method: Three-partition KV-cache strategy: (1) Sink tokens preserve early anchor frames at full resolution; (2) Mid tokens achieve 32x spatiotemporal compression via dual-branch network with progressive 3D convolutions and low-resolution VAE re-encoding; (3) Recent tokens kept at full resolution. Dynamic top-k context selection for mid tokens and Temporal RoPE Adjustment to re-align position gaps.
...
Conclusion: PackForcing demonstrates that hierarchical context compression enables efficient long-video generation with bounded memory, proving short-video supervision is sufficient for high-quality, long-video synthesis while maintaining temporal coherence and dynamic quality.
...
TL;DR: BizGenEval is a systematic benchmark for evaluating commercial visual content generation models across 5 document types and 4 capability dimensions, revealing significant gaps between current models and professional requirements.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on natural image synthesis but fail to evaluate models under the structured, multi-constraint requirements of real-world commercial design tasks, creating a need for systematic evaluation of practical visual content creation capabilities.

Method: Created BizGenEval benchmark spanning 5 document types (slides, charts, webpages, posters, scientific figures) and 4 capability dimensions (text rendering, layout control, attribute binding, knowledge-based reasoning), with 400 curated prompts and 8000 human-verified checklist questions for rigorous assessment.

Result: Large-scale benchmarking of 26 popular image generation systems (commercial APIs and open-source models) revealed substantial capability gaps between current generative models and the requirements of professional visual content creation.

Conclusion: BizGenEval serves as a standardized benchmark for real-world commercial visual content generation, highlighting the need for improved models that can handle complex visual and semantic constraints required in professional design tasks.
...
TL;DR: SlotVTG is a lightweight framework that enhances MLLMs' object-centric reasoning for video temporal grounding, improving out-of-domain generalization without full retraining.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs for Video Temporal Grounding have coarse recognition capabilities requiring task-specific fine-tuning, which causes models to memorize dataset-specific shortcuts rather than learning to ground in actual visual content, leading to poor out-of-domain generalization. Object-centric learning offers a solution but existing approaches require expensive full retraining.
...
Conclusion: SlotVTG provides an efficient framework for enhancing MLLMs' object-centric reasoning capabilities for video temporal grounding, addressing the out-of-domain generalization problem without requiring expensive full retraining of the entire model pipeline.
...
TL;DR: LIGHT is a diffusion-based method for human-object interaction animation that uses data-driven guidance emerging from asynchronous denoising schedules rather than hand-crafted contact priors.


<details>
  <summary>Details</summary>
Motivation: Generating realistic human-object interaction animations is challenging due to the need to jointly model dynamic human actions and diverse object geometries. Prior diffusion-based approaches rely on hand-crafted contact priors or human-imposed kinematic constraints, which limits their effectiveness and generalization.
...
Result: Pace-induced guidance more effectively mirrors the benefits of contact priors than conventional classifier-free guidance, achieving higher contact fidelity, more realistic HOI generation, and stronger generalization to unseen objects and tasks.

Conclusion: LIGHT demonstrates that data-driven guidance emerging from denoising pace itself can effectively replace hand-crafted contact priors for human-object interaction animation, offering improved contact awareness and generalization capabilities.
...
TL;DR: The paper proposes learning generative models of table tennis players' tactical racket strokes to create a common latent space that encodes individual characteristics including skill levels, enabling automated skill assessment.


<details>
  <summary>Details</summary>
Motivation: Skill assessment is crucial but challenging because skill is latent to observed actions. The paper aims to quantify skill in complex human behaviors, specifically in dyadic sports like table tennis where skill manifests in nuanced execution conditioned on game context.

Method: Learn generative models of each player's tactical racket strokes and jointly embed them in a common latent space. Train on large-scale 3D-reconstructed professional matches, conditioning on comprehensive game context including player positioning and opponent behaviors.

Result: The learned player space reflects distinct play styles and attributes representing skill. A simple relative ranking network trained on these embeddings achieves both relative and absolute skill predictions.
...
TL;DR: PSDesigner is an automated graphic design system that mimics human designer workflows, using specialized components to collect assets and execute tool calls on PSD files, trained on a large annotated dataset called CreativePSD.


<details>
  <summary>Details</summary>
Motivation: Existing automated design systems simplify professional workflows, resulting in limited flexibility and intuitiveness. There's a need for a system that can faithfully translate user intentions into editable design files while emulating human designer creativity.

Method: PSDesigner builds upon multiple specialized components to collect theme-related assets based on user instructions, autonomously infers and executes tool calls to manipulate design files (PSD files). The system is trained on CreativePSD, a large dataset of high-quality PSD files annotated with operation traces across diverse design scenarios and artistic styles.
...
Conclusion: PSDesigner successfully addresses the limitations of existing automated design systems by emulating human designer workflows and leveraging a comprehensive annotated dataset, providing a more flexible and intuitive solution for automated graphic design.
...
TL;DR: MegaFlow is a zero-shot optical flow model that uses pre-trained Vision Transformer features for global matching to handle large displacements, achieving state-of-the-art performance without task-specific fine-tuning.
...
Result: Achieves state-of-the-art zero-shot performance on multiple optical flow benchmarks and competitive performance on long-range point tracking benchmarks, demonstrating robust transferability.

Conclusion: MegaFlow presents a unified paradigm for generalizable motion estimation by leveraging pre-trained vision priors, offering strong performance in large displacement scenarios without domain-specific fine-tuning.
...
TL;DR: Vega is a vision-language-world-action model for autonomous driving that uses autoregressive processing for vision/language inputs and diffusion for world modeling/trajectory generation, enabling personalized driving through diverse user instructions.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language-action models for autonomous driving lack flexibility to follow diverse user instructions for personalized driving, as they primarily use language only for scene description or reasoning rather than instruction-following.

Method: Proposes Vega model with: 1) InstructScene dataset (~100K scenes with diverse driving instructions and trajectories; 2) Unified architecture using autoregressive paradigm for vision/language inputs and diffusion paradigm for world modeling/trajectory generation; 3) Joint attention for cross-modal interactions; 4) Individual projection layers for different modalities.

Result: Extensive experiments show superior planning performance and strong instruction-following abilities, demonstrating the model's capability to handle personalized driving scenarios.

Conclusion: Vega paves the way for more intelligent and personalized driving systems by effectively integrating vision, language, world modeling, and action generation in a unified framework that can follow diverse user instructions.
...
TL;DR: RefAlign is a representation alignment framework for reference-to-video generation that explicitly aligns diffusion Transformer reference features to a visual foundation model's semantic space using contrastive alignment loss during training, improving identity consistency and semantic discriminability without inference overhead.


<details>
  <summary>Details</summary>
Motivation: Existing R2V methods use auxiliary semantic features alongside VAE latents to provide guidance and reduce pixel-level information leakage, but still suffer from copy-paste artifacts and multi-subject confusion due to modality mismatch across heterogeneous encoder features.

Method: Proposes RefAlign framework with reference alignment loss that pulls reference features and VFM features of the same subject closer (improving identity consistency) while pushing apart features of different subjects (enhancing semantic discriminability). Applied only during training with no inference overhead.

Result: Extensive experiments on OpenS2V-Eval benchmark show RefAlign outperforms current state-of-the-art methods in TotalScore, validating effectiveness of explicit reference alignment for R2V tasks.

Conclusion: RefAlign achieves better balance between text controllability and reference fidelity through explicit representation alignment, demonstrating that aligning DiT reference-branch features to VFM semantic space effectively addresses modality mismatch issues in R2V generation.
...
TL;DR: MuRF is a training-free, architecture-agnostic method that enhances vision foundation models by fusing multi-resolution features at inference time to leverage complementary inductive biases from different scales.


<details>
  <summary>Details</summary>
Motivation: Current vision foundation models use single-scale inference despite visual perception's multi-resolution nature where low-resolution views excel at global semantics and high-resolution views capture fine details. The single-scale paradigm overlooks these complementary inductive biases.

Method: Multi-Resolution Fusion (MuRF) processes images at multiple resolutions through a frozen VFM and fuses the resulting features into a unified representation. It's training-free, architecture-agnostic, and works with various VFM families including DINOv2 and SigLIP2.

Result: Empirical validation shows MuRF enhances performance across a broad spectrum of computer vision tasks by leveraging complementary multi-resolution information, demonstrating successful generalization across different VFM families.

Conclusion: MuRF provides a fundamental, universally effective enhancement to visual representation that harnesses the synergy of multi-resolution processing without requiring model retraining, making it a practical improvement for existing vision foundation models.
...
TL;DR: LGTM introduces a feed-forward framework that uses compact Gaussian primitives with per-primitive textures to decouple geometric complexity from rendering resolution, enabling high-fidelity 4K novel view synthesis without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing feed-forward 3D Gaussian Splatting methods suffer from quadratic growth in primitive count with increasing resolution, fundamentally limiting scalability and making high-resolution synthesis like 4K intractable.

Method: LGTM predicts compact Gaussian primitives coupled with per-primitive textures, decoupling geometric complexity from rendering resolution. This feed-forward framework avoids the quadratic primitive growth of pixel-aligned approaches.

Result: The method enables high-fidelity 4K novel view synthesis without per-scene optimization, a capability previously unattainable for feed-forward methods, while using significantly fewer Gaussian primitives than existing approaches.

Conclusion: LGTM overcomes the resolution scaling barrier in feed-forward 3D Gaussian Splatting by introducing a texture-coupled primitive approach that enables scalable high-resolution synthesis with reduced geometric complexity.
...
TL;DR: ShotStream is a causal multi-shot video generation architecture for interactive storytelling that achieves real-time 16 FPS generation through novel distillation techniques and dual-cache memory mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current bidirectional video generation architectures suffer from limited interactivity and high latency, making them unsuitable for real-time interactive storytelling applications where users need to dynamically guide narrative development.

Method: 1) Reformulate multi-shot video generation as next-shot generation conditioned on historical context; 2) Fine-tune text-to-video model into bidirectional next-shot generator; 3) Distill into causal student via Distribution Matching Distillation; 4) Introduce dual-cache memory mechanism with global context cache (inter-shot consistency) and local context cache (intra-shot consistency) with RoPE discontinuity indicator; 5) Two-stage distillation strategy with intra-shot self-forcing using ground-truth histories and inter-shot self-forcing using self-generated histories.
...
Conclusion: ShotStream represents a significant advancement in interactive video generation, enabling real-time multi-shot storytelling with high coherence and low latency, paving the way for practical interactive narrative applications.
...
<details>
  <summary>Details</summary>
Motivation: To investigate why depression detection models achieve high performance without revealing what drives predictions, and to identify systematic biases in semi-structured interview protocols.

Method: Analyzed three depression detection datasets (ANDROIDS, DAIC-WOZ, E-DAIC) to identify bias from interviewer prompts in semi-structured interviews. Compared models trained on full conversations vs. participant-only utterances to examine evidence distribution.

Result: Models trained on interviewer turns exploit fixed prompts and positions to distinguish depressed from control subjects, achieving high classification scores without using participant language. Restricting to participant utterances distributes decision evidence more broadly and reflects genuine linguistic cues.

Conclusion: Semi-structured interview protocols introduce systematic, architecture-agnostic bias by allowing models to leverage script artifacts rather than patient language. Analyses must localize decision evidence by time and speaker to ensure models learn from participants' genuine linguistic cues.
...
TL;DR: Network pruning works well for non-generative language tasks but fails for generative tasks due to amplified deviations in probability space during sequential generation.
...
Method: Analyze pruning from a representation-hierarchy perspective, decomposing LM computation into three sequential spaces: embedding, logit, and probability spaces, examining pruning-induced perturbations across these spaces.

Result: Embedding and logit spaces are robust to pruning, but the nonlinear logit-to-probability transformation amplifies deviations, which accumulate across time steps during generation, causing substantial degradation. The categorical-token probability subspace stability supports pruning effectiveness for non-generative tasks.

Conclusion: The analysis disentangles pruning effects across tasks, explaining the task-dependent performance discrepancy and providing practical guidance for pruning application in language models.
...
TL;DR: Fine-tuning a small 1.2B parameter LLM for systematic review screening significantly improves performance over base models, achieving 86.40% agreement with human coders.


<details>
  <summary>Details</summary>
Motivation: Traditional systematic reviews are time-consuming due to manual screening of thousands of titles/abstracts. While LLMs have been explored for efficiency, inconsistent results from prompting alone suggest insufficient context for optimal performance.

Method: Fine-tuned a small 1.2 billion parameter open-weight LLM specifically for study screening using a dataset where humans rated over 8500 titles/abstracts for potential inclusion in a systematic review.
...
Conclusion: Fine-tuning LLMs shows promise for title and abstract screening in large-scale systematic reviews, addressing limitations of prompting-only approaches through domain-specific training.
...
<details>
  <summary>Details</summary>
Motivation: Clinical documentation is critical for patient safety and care continuity, but EHR administrative burden contributes to physician burnout. This is particularly challenging for low-resource languages like Finnish, necessitating effective NLP solutions for medical transcription.

Method: Fine-tuned LLaMA 3.1-8B on a small validated corpus of simulated clinical conversations from Metropolia University of Applied Sciences using controlled preprocessing and optimization. Evaluated effectiveness via sevenfold cross-validation with BLEU, ROUGE-L, and BERTScore metrics.

Result: Fine-tuned model achieved BLEU = 0.1214, ROUGE-L = 0.4982, and BERTScore F1 = 0.8230. Results show low n-gram overlap but strong semantic similarity with reference transcripts, indicating effective translation of medical discourse in spoken Finnish.

Conclusion: Fine-tuning is an effective approach for medical transcription in Finnish, supporting feasibility of privacy-oriented domain-specific LLMs for clinical documentation. Study provides directions for future work in this area.
...
TL;DR: New dataset of English sentences annotated with UMR aspect labels over AMR graphs, with baseline experiments establishing benchmarks for automatic aspect prediction.
...
Method: Created new dataset with UMR aspect labels over AMR graphs; developed annotation scheme and guidelines for labeling eventive predicates according to UMR aspect lattice; implemented multi-step adjudication pipeline for consistency; conducted baseline experiments with three modeling approaches.

Result: Established initial benchmarks for automatic UMR aspect prediction; demonstrated utility of dataset for future automation; provided foundation for integrating aspect into semantic meaning representations more broadly.

Conclusion: The new UMR aspect-annotated dataset enables improved aspect prediction and integration into semantic representations, addressing previous limitations in aspect annotation and automation.
...
TL;DR: Synthetic rewriting of Portuguese text boosts language model performance, but only when applied to high-quality source data; the benefit is scale-dependent with stronger effects at larger model sizes.


<details>
  <summary>Details</summary>
Motivation: Most synthetic data generation studies focus on English and lack systematic control over source data quality. This research aims to understand how synthetic rewriting interacts with source data quality specifically for Portuguese language model pretraining.

Method: Used ClassiCC-PT Portuguese corpus with STEM/Educational quality scores to create two 10B-token subsets (high and low quality). Rewrote each into four styles using a 7B instruction-tuned model, generating ~40B tokens per condition. Trained two English-centric base models (1.1B and 7B parameters) on each condition and evaluated on PoETa V2 benchmark (44 Portuguese tasks).

Result: At 7B scale: rewriting high-quality data yields +3.4 NPM gain over unmodified data; rewriting low-quality data provides only +0.5 NPM gain. At 1.1B scale: weaker interaction, with unmodified low-quality data performing comparably to rewritten high-quality data.
...
TL;DR: ARRoL accelerates RLVR training by pruning low-quality rollouts during generation and steering surviving ones toward correctness-balanced distributions, achieving both accuracy gains and training speedups.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods like GRPO and DAPO suffer from high computational costs due to sampling many rollouts per prompt, and sparse relative advantages from nearly all-correct or all-incorrect samples that yield weak learning signals.

Method: ARRoL trains a lightweight quality head on-the-fly to predict success probability of partial rollouts, uses it for early pruning decisions, and employs system design that prunes inside inference engine while re-batching remaining rollouts for efficient log-probability computation and policy updates.
...
Conclusion: ARRoL effectively addresses computational inefficiency and sparse learning signals in RLVR through online rollout pruning and correctness-balancing, demonstrating significant improvements in both training efficiency and model accuracy.
...
TL;DR: LogSigma wins SemEval-2026 Task 3 by using learned homoscedastic uncertainty to automatically balance Valence and Arousal regression objectives across languages, achieving 1st place on five datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional ABSA predicts discrete sentiment labels, but DimABSA requires continuous Valence and Arousal scores. The challenge is that Valence and Arousal have different prediction difficulties across languages and domains, requiring adaptive task balancing.
...
Result: Achieves 1st place on five datasets across both tracks. Learned variance weights vary substantially across languages (0.66x for German to 2.18x for English), demonstrating language-dependent optimal task balancing.

Conclusion: Optimal task balancing for DimABSA is language-dependent and cannot be determined a priori. Learned homoscedastic uncertainty effectively addresses varying Valence-Arousal difficulty profiles across languages.
...
TL;DR: The paper introduces decoding-constrained beam search to efficiently compute deterministic lower bounds on near-verbatim extraction risk in LLMs, addressing the computational expense of existing probabilistic extraction methods.


<details>
  <summary>Details</summary>
Motivation: Standard greedy-decoding extraction methods fail to capture how extraction risk varies across sequences, while probabilistic extraction methods are only tractable for verbatim memorization and miss near-verbatim instances that pose similar privacy and copyright risks. Quantifying near-verbatim extraction risk is computationally expensive, requiring ~100,000 Monte Carlo samples per sequence.

Method: The authors introduce decoding-constrained beam search, which yields deterministic lower bounds on near-verbatim extraction risk at a cost comparable to ~20 Monte Carlo samples per sequence. This approach efficiently quantifies extraction risk for both verbatim and near-verbatim sequences.

Result: The method surfaces information invisible to verbatim methods: many more extractable sequences, substantially larger per-sequence extraction mass, and patterns in how near-verbatim extraction risk manifests across model sizes and types of text.

Conclusion: Decoding-constrained beam search provides an efficient solution for quantifying near-verbatim extraction risk in LLMs, revealing previously invisible extraction patterns and enabling better assessment of privacy and copyright risks.
...
TL;DR: Adaptive Chunking framework improves RAG performance by selecting optimal chunking strategies per document using five novel intrinsic metrics, achieving 72% answer correctness (vs 62-64%) and 30% more answered questions.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems rely on "one-size-fits-all" chunking approaches that fail to capture document nuances, and there's no dedicated evaluation framework for chunking strategies independent of downstream performance.

Method: Introduces Adaptive Chunking framework with five novel intrinsic metrics (RC, ICC, DCC, BI, SC) to assess chunking quality, plus two new chunkers (LLM-regex splitter and split-then-merge recursive splitter) with targeted post-processing techniques.

Result: On diverse corpus spanning legal, technical, and social science domains, adaptive chunking increased RAG answer correctness to 72% (from 62-64%) and increased successfully answered questions by over 30% (65 vs. 49) without changing models or prompts.

Conclusion: Adaptive, document-aware chunking guided by complementary intrinsic metrics offers a practical and effective path to more robust RAG systems, demonstrating that chunking strategy selection significantly impacts RAG performance.
...
<details>
  <summary>Details</summary>
Motivation: Machine Translation and Quality Estimation systems perform well in general domains but degrade significantly under domain mismatch, creating a need for effective adaptation methods to specialized domains without requiring extensive computational resources or large datasets.

Method: The dissertation presents four main methods: 1) Similarity-based data selection for MT using small targeted in-domain subsets; 2) Staged QE training pipeline combining domain adaptation with lightweight data augmentation; 3) Study of subword tokenization and vocabulary alignment in fine-tuning; 4) QE-guided in-context learning for large language models that selects examples without parameter updates.

Result: Small targeted in-domain subsets outperform larger generic datasets for MT; staged QE training improves performance across domains, languages, and resource settings; aligned tokenization-vocabulary setups lead to stable training and better translation quality; QE-guided in-context learning outperforms standard retrieval methods and supports reference-free setups.

Conclusion: Domain adaptation for MT and QE depends on careful data selection, representation alignment, and efficient adaptation strategies. The dissertation provides practical methods for building reliable domain-specific systems that address computational cost, data efficiency, and performance stability across various adaptation scenarios.
...
TL;DR: MoFA is a model-driven framework that uses LLM-based reasoning for feature selection in industrial ML systems, incorporating semantic and quantitative feature information to satisfy operational constraints without requiring labeled data.


<details>
  <summary>Details</summary>
Motivation: Traditional feature selection methods rely on labeled data and statistical heuristics, making them difficult to apply in production environments where labeled data are limited and multiple operational constraints must be satisfied.

Method: MoFA performs sequential, reasoning-based feature selection using structured prompts that incorporate feature definitions, importance scores, correlations, and metadata (feature groups/types). It uses interpretable, constraint-aware reasoning to select features.

Result: In three real-world applications: (1) Improved accuracy while reducing feature group complexity for True Interest and Time-Worthiness Prediction; (2) Discovered high-order interaction terms yielding substantial engagement gains in Value Model Enhancement online experiments; (3) Selected compact, high-value feature subsets improving both model accuracy and inference efficiency for Notification Behavior Prediction.

Conclusion: MoFA demonstrates the practicality and effectiveness of LLM-based reasoning for feature selection in real production systems, addressing limitations of traditional methods in industrial settings.
...
TL;DR: Exons-Detect is a training-free AI text detection method that uses exon-aware token reweighting to identify and amplify informative tokens, achieving state-of-the-art performance with improved robustness to adversarial attacks and varying input lengths.
...
Conclusion: The proposed Exons-Detect method effectively addresses limitations of existing training-free approaches by focusing on informative tokens, providing both high detection accuracy and robustness while maintaining interpretability through translation scores.
...
TL;DR: Multilingual LLMs interpret instructions differently across languages due to social register variations in imperative mood, with declarative rewriting reducing cross-linguistic variance by 81%.


<details>
  <summary>Details</summary>
Motivation: To investigate why identical semantic instructions produce opposite interaction topologies across languages (cooperative in English vs competitive in Spanish) in multilingual LLMs, suggesting that models process instructions as social acts rather than technical specifications.
...
Result: Imperative mood carries different obligatory force across speech communities; declarative rewriting reduced cross-linguistic variance by 81% (p=0.029). Rewriting three of eleven imperative blocks shifted Spanish instruction topology from competitive to cooperative with spillover effects.

Conclusion: Models process instructions as social acts with language-dependent register effects. Imperative mood creates language-dependent alignment, while declarative factual descriptions transfer across languages. This suggests constitutional AI principles in imperative mood may create language-dependent alignment.
...
TL;DR: Computational analysis of Slovene historical newspapers using topic modeling, LLM-based sentiment analysis, and entity graphs to examine collective identity representations in late 19th/early 20th century public discourse.


<details>
  <summary>Details</summary>
Motivation: To examine how collective identities, political orientations, and national belonging were represented in Slovene public discourse at the turn of the 20th century, using computational methods to analyze historical newspaper data that is often noisy and challenging to process.

Method: Combines BERTopic for thematic pattern identification, evaluation of four instruction-following LLMs for sentiment classification in OCR-degraded historical Slovene (selecting GaMS3-12B-Instruct), NER graph creation for entity-place relationships, and mixed methods approach combining quantitative network analysis with critical discourse analysis.

Result: Identified shared concerns and clear ideological differences between conservative-Catholic (Slovenec) and liberal-progressive (Slovenski narod) newspapers; found GaMS3-12B-Instruct most suitable for sentiment analysis but with limitations (better on neutral than positive/negative sentiment); revealed meaningful variation in portrayal of collective identities; demonstrated value of combining computational methods with critical interpretation.

Conclusion: The study demonstrates the value of combining scalable computational methods with critical interpretation for digital humanities research on noisy historical newspaper data, providing insights into the emergence and development of intertwined historical political and socionomic identities in Slovene public discourse.
...
TL;DR: LLMs verbalize confidence scores detached from actual accuracy; calibration and confidence signals are encoded orthogonally; reasoning disrupts confidence (Reasoning Contamination Effect); adaptive steering pipeline improves calibration alignment.


<details>
  <summary>Details</summary>
Motivation: LLMs produce verbalized confidence scores that are poorly calibrated (detached from actual accuracy), but the geometric relationships underlying this behavior remain poorly understood. Understanding these mechanisms could enable better calibration of LLM confidence outputs.

Method: Mechanistic interpretability analysis using linear probes and contrastive activation addition (CAA) steering to examine internal representations. Evaluated across three open-weight models and four datasets. Introduced two-stage adaptive steering pipeline that reads internal accuracy estimates and steers verbalized output to match them.

Result: Calibration and verbalized confidence signals are encoded linearly but orthogonally across all models/datasets. Reasoning process disrupts verbalized confidence direction (Reasoning Contamination Effect), exacerbating miscalibration. The adaptive steering pipeline substantially improves calibration alignment across all evaluated models.

Conclusion: LLM confidence miscalibration stems from orthogonal encoding of calibration vs. confidence signals, with reasoning processes contaminating confidence outputs. The proposed adaptive steering pipeline effectively aligns verbalized confidence with internal accuracy estimates, offering a promising approach for improving LLM calibration.
...
TL;DR: The paper introduces oMind framework for adapting LLMs to mental health domain, addressing challenges of data quality, training paradigms, and multi-turn dialogue evaluation through a novel SFT dataset and benchmark.


<details>
  <summary>Details</summary>
Motivation: LLMs show promise for mental health applications but face specific challenges: lack of high-quality interpretable training data, limited training paradigms focused only on core capabilities, and inadequate evaluation methods for multi-turn dialogues in this sensitive domain.

Method: Developed oMind framework with: 1) Training pipeline using structured knowledge retrieval, LLM-based pruning, and review actions to create ~164k multi-task SFT dataset; 2) oMind-Chat benchmark with expert-annotated turn-level and conversation-level rubrics for multi-turn evaluation; 3) Aligned LLM agents for diverse capabilities including conversations.

Result: oMind LLMs consistently outperform baselines on both core capabilities and conversation tasks. oMind-LLM shows significantly better reasoning with up to 80% win rate in comparative evaluations.

Conclusion: The oMind framework successfully addresses key challenges in adapting LLMs to mental health domain through high-quality data generation, comprehensive training paradigms, and robust multi-turn evaluation benchmarks, demonstrating superior performance over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Standard calibration metrics (ECE, Brier score) conflate two distinct capacities: how much a model knows (Type-1 sensitivity) and how well it knows what it knows (Type-2 metacognitive sensitivity). This conflation prevents proper evaluation of whether models truly "know what they don't know" versus merely appearing well-calibrated due to confidence criterion placement.

Method: Introduced a Type-2 Signal Detection Theory evaluation framework using meta-d' and metacognitive efficiency ratio (M-ratio) to decompose Type-1 and Type-2 capacities. Applied to four LLMs (Llama-3-8B-Instruct, Mistral-7B-Instruct-v0.3, Llama-3-8B-Base, Gemma-2-9B-Instruct) across 224,000 factual QA trials, with temperature manipulation experiments to dissociate confidence policy from metacognitive capacity.

Result: (1) Metacognitive efficiency varies substantially across models even with similar Type-1 sensitivity (Mistral has highest d' but lowest M-ratio); (2) Metacognitive efficiency is domain-specific with different weakest domains per model; (3) Temperature manipulation shifts Type-2 criterion while meta-d' remains stable for two models; (4) AUROC_2 and M-ratio produce fully inverted model rankings, showing they answer fundamentally different evaluation questions.

Conclusion: The meta-d' framework reveals which models truly "know what they don't know" versus which merely appear well-calibrated due to criterion placement, providing crucial distinctions for model selection, deployment, and human-AI collaboration. Standard aggregate metrics mask important domain-specific metacognitive differences.
...
TL;DR: Proposes methods to extract speech evaluation features from weakly-supervised ASR models, enabling expansion to low-resource languages without requiring phonemic frame-synchronous models.


<details>
  <summary>Details</summary>
Motivation: Standard ASR models for speech evaluation require phonemic frame-synchronous features, but these are unavailable for low-resource languages due to limited training data. Weakly-supervised models support many languages but are frame-asynchronous and not phonemic, creating incompatibility for feature extraction.

Method: 1) Compute phoneme posteriors by mapping ASR hypotheses to a phoneme confusion network; 2) Use word-level instead of phoneme-level speaking rate and duration features; 3) Combine phoneme and frame-level features using cross-attention architecture that eliminates need for phoneme time alignment.

Result: The proposed approach performs comparably with standard frame-synchronous features on both English (speechocean762) and low-resource Tamil datasets, demonstrating effectiveness for speech evaluation in low-resource scenarios.

Conclusion: The method successfully overcomes incompatibilities between weakly-supervised ASR models and speech evaluation feature extraction, enabling expansion of speech evaluation to low-resource languages without requiring phonemic frame-synchronous models.
...
TL;DR: Systematic comparison shows automatic prompt optimization (GEPA) can match expert prompt engineering in some NLP tasks, with task-dependent results and no statistical differences in most expert-optimized comparisons.


<details>
  <summary>Details</summary>
Motivation: To determine whether automatic prompt optimization can replace expert prompt engineering in linguistic tasks, given LLM performance's high sensitivity to prompt design.

Method: First systematic comparison of hand-crafted zero-shot expert prompts, base DSPy signatures, and GEPA-optimized DSPy signatures across translation, terminology insertion, and language quality assessment tasks, evaluating five model configurations.
...
Conclusion: Automatic prompt optimization can achieve comparable performance to expert prompt engineering in many linguistic tasks, though the comparison is asymmetric (GEPA uses gold-standard splits while expert prompts rely on domain expertise without labeled data).
...
TL;DR: Lightweight LLMs can serve as effective low-latency security judges for prompt attacks in production guardrails through structured reasoning processes, with modest gains from multi-model aggregation.


<details>
  <summary>Details</summary>
Motivation: There's a deployment gap in LLM security: lightweight classifiers struggle with generalization under distribution shift, while high-capacity LLM judges are too slow/costly for live enforcement. Need reliable, low-latency security judges for production guardrails.

Method: Use lightweight general-purpose LLMs as security judges with careful prompt/output design, guiding them through structured reasoning: explicit intent decomposition, safety-signal verification, harm assessment, and self-reflection. Also evaluate Mixture-of-Models (MoM) aggregation.

Result: General-purpose LLMs (e.g., gemini-2.0-flash-lite-001) can serve as effective low-latency judges for live guardrails. Method is deployed in production for Singapore's public service chatbots. MoM aggregation shows only modest performance gains over single-model judges.

Conclusion: Lightweight LLMs with structured reasoning can bridge the deployment gap for prompt-attack detection in production environments, offering reliable low-latency security enforcement where traditional approaches fail.
...
TL;DR: Cross-Preference Learning (CPL) is a preference-based training framework that improves context-aware machine translation by explicitly modeling when and how contextual information benefits translation quality through intra- and cross-condition preferences.


<details>
  <summary>Details</summary>
Motivation: Context-aware machine translation doesn't consistently outperform sentence-level MT because contextual signals are unevenly beneficial across sentences, and existing training objectives don't explicitly model this variability, limiting models' adaptive exploitation of context.

Method: Proposes Cross-Preference Learning (CPL), a preference-based training framework that integrates both intra-condition preferences (within same input condition) and cross-condition preferences (between different input conditions) into the preference optimization objective to explicitly capture when and how contextual information improves translation.

Result: Experimental validation on several public context-aware MT tasks using multiple models (Qwen3-4B, Qwen3-8B, Llama-3-8B) demonstrates consistent improvements in translation quality and robustness across both input conditions without architectural modifications.

Conclusion: CPL effectively addresses the limitation of existing context-aware MT approaches by providing explicit supervision on contextual benefit variability, enabling models to adaptively exploit context and achieve better translation performance.
...
TL;DR: LLMs struggle to maintain implicit consistency in multi-turn interactions, failing to preserve unstated goals across dialogue turns without explicit context, highlighting limitations for persona-driven applications.
...
Method: Used a 20-question-style riddle game paradigm where LLMs secretly select a target and respond to users' guesses with "yes/no" answers, evaluating their ability to maintain implicit consistency across turns.

Result: LLMs struggle to preserve latent consistency - their implicit "goals" shift across turns unless explicitly provided their selected target in context.
...
<details>
  <summary>Details</summary>
Motivation: Data scarcity is identified as a primary limitation in dialectal NLP research, particularly for Basque dialects. The paper aims to address this limitation by systematically compiling available dialectal resources.

Method: The authors create a catalog distinguishing two data source types: 1) Online data originally written in dialects (news, tweets, dictionaries, atlases, grammar rules, videos), and 2) Standard-to-dialect adapted data. For manual adaptation, they adapt the XNLI test split into three Basque dialects. For automatic adaptation, they evaluate the BasPhyCowest dataset through manual assessment by native speakers.

Result: The paper yields: 1) A systematic catalog of contemporary Basque dialectal data, 2) A high-quality parallel gold standard evaluation dataset (XNLI adapted to three dialects), and 3) Quality assessment of automatically adapted data (BasPhyCowest) to determine its viability as silver data.

Conclusion: The paper provides a comprehensive resource catalog addressing data scarcity in Basque dialectal NLP, offering both gold-standard manually adapted data and insights into the viability of automatically adapted silver data for dialectal applications.
...
Method: Created CPGBench framework collecting 3,418 CPG documents from 9 countries/regions and 2 international organizations across 24 specialties, extracting 32,155 clinical recommendations with metadata. Generated multi-turn conversations for each recommendation to evaluate 8 leading LLMs' detection and adherence capabilities, supplemented by human evaluation with 56 clinicians.
...
Conclusion: CPGBench is the first benchmark systematically revealing which clinical recommendations LLMs fail to detect or adhere to, highlighting crucial gaps that must be addressed for safe and responsible LLM deployment in clinical practice.
...
TL;DR: FRED Difficulty Metrics (Fertility Ratio, Retrieval Proxy, Pre-training Exposure, Corpus Diversity) explain variability in low-resource MT performance by quantifying dataset characteristics rather than model capability.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the problem of inconsistent and difficult-to-contextualize performance in extremely low-resource machine translation, where it's unclear whether reported breakthroughs result from superior methodologies or artifacts of benchmark collection, particularly for researchers working with specific language groups like ancient languages.

Method: The authors introduce FRED Difficulty Metrics: Fertility Ratio (F) measuring tokenization coverage, Retrieval Proxy (R) quantifying train-test overlap, Pre-training Exposure (E) assessing pre-training data exposure, and Corpus Diversity (D) evaluating dataset variety. These dataset-intrinsic metrics are used to analyze performance variability across language pairs.

Result: The metrics reveal that significant performance variability in low-resource MT is explained by train-test overlap and pre-training exposure rather than model capability. Extinct and non-Latin indigenous languages suffer from poor tokenization coverage (high token fertility), highlighting limitations of transferring models from high-resource languages lacking shared vocabulary.

Conclusion: By providing FRED indices alongside performance scores, the paper enables more transparent evaluation of cross-lingual transfer and establishes a more reliable foundation for the extremely low-resource MT community, allowing better contextualization of results across different language pairs.
...
TL;DR: LLMs trained on synthetic data fail to generalize to natural sentences, while models trained on natural data perform robustly on both natural and synthetic test suites, demonstrating natural data's superiority for capturing abstract linguistic patterns.
...
Method: Used Blackbird Language Matrices (structured datasets) with two data sources: (1) natural sentences extracted from Universal Dependencies, and (2) synthetic sentences. Compared models trained on each data type and evaluated their generalization across both natural and synthetic test suites.

Result: Models trained on synthetic data achieved ceiling performance on synthetic test suites but failed to generalize to natural sentences. Models trained on natural data showed robust performance on both natural and synthetic test suites.

Conclusion: Natural data is superior for training LLMs to capture abstract linguistic patterns, and structured evaluation setups (like BLMs) are valuable for probing LLMs' syntactic and semantic knowledge.
...
<details>
  <summary>Details</summary>
Motivation: Current scientific evaluation benchmarks use static, single-turn QA formats that are inadequate for measuring LLM performance in complex scientific tasks requiring multi-step iteration and experimental interaction. There's a need for systematic assessment of LLMs' dynamic reasoning in real-world research scenarios.

Method: Introduces MolQuest, an agent-based evaluation framework built on authentic chemical experimental data. It formalizes molecular structure elucidation as a multi-turn interactive task requiring models to: 1) proactively plan experimental steps, 2) integrate heterogeneous spectral sources (NMR, MS), and 3) iteratively refine structural hypotheses within a vast chemical space.

Result: Contemporary frontier models show significant limitations: SOTA models achieve only ~50% accuracy, while most other models remain below 30% threshold. This reveals a critical gap in LLMs' strategic scientific reasoning and abductive reasoning abilities.

Conclusion: MolQuest provides a reproducible, extensible framework for science-oriented LLM evaluation. The findings highlight the need for future research toward AI that can actively participate in the scientific process, addressing current limitations in strategic scientific reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate pragmatic communication in large language models under strict partial information settings, where multiple agents must coordinate with complementary but incomplete views to achieve a shared goal that no single agent can fully observe.

Method: Formalizes the problem as a multi-sender pragmatic reasoning task and provides a diagnostic framework that decomposes failures into spatial grounding, belief modeling, and pragmatic communication errors, including a taxonomy of behavioral failure profiles.
...
Conclusion: Multi-agent coordination remains a fundamentally unsolved challenge for current language models, as evidenced by the disconnect between reasoning capability and collaborative success in pragmatic communication tasks.
...
TL;DR: WSF-ARG+ is the first dataset combining hate speech with check-worthiness information, accompanied by an LLM-in-the-loop framework that reduces human annotation effort while maintaining quality, showing that incorporating check-worthiness improves hate speech detection performance.


<details>
  <summary>Details</summary>
Motivation: Hateful content online often uses fact-like misinformation, creating challenges for content moderation that requires assessing both harmfulness and veracity. Current approaches fail to jointly address hate speech and misinformation, deepening prejudice and increasing moderator workload.

Method: Released WSF-ARG+ dataset combining hate speech with check-worthiness information. Introduced novel LLM-in-the-loop framework for annotating check-worthy claims, tested with 12 open-weight LLMs of varying sizes and architectures, validated through extensive human evaluation.

Result: LLM-in-the-loop framework reduces human effort without compromising annotation quality. Hate speech messages with check-worthy claims show significantly higher harassment and hate. Incorporating check-worthiness labels improves LLM-based hate speech detection by up to 0.213 macro-F1 (average 0.154 macro-F1 for large models).

Conclusion: The WSF-ARG+ dataset and LLM-in-the-loop framework effectively address the joint challenge of hate speech and misinformation detection, demonstrating that check-worthiness information is valuable for improving hate speech detection systems and reducing content moderator workload.
...
TL;DR: SGPE tokenizer reduces token usage by 61.7% for Sinhala and 27% for Hindi compared to OpenAI's o200k, extending context windows up to 4.38× for Abugida scripts while guaranteeing no syllable splitting.


<details>
  <summary>Details</summary>
Motivation: Standard BPE tokenizers struggle with complex Abugida scripts, breaking conjuncts into meaningless sub-character units, which degrades LLM reasoning efficiency and raises inference costs, creating a "Token Tax" for Global South languages.

Method: Proposed WWHO three-layer architecture and SGPE algorithm that separates linguistic rules from statistical compression, enabling multilingual tokenization with Syllable-aware Grapheme Pair Encoding.

Result: For Sinhala: TWR 1.274 (61.7% reduction vs o200k); For Hindi: TWR 1.181 (27% reduction vs o200k); Mixed-script: TWR 1.240 (36.7-60.2% reductions vs various models); Achieves 4.38× context window extension.

Conclusion: SGPE effectively extends usable context windows for Abugida languages while ensuring Linguistic Zero-Breakage Guarantee, addressing the token inefficiency problem of standard BPE tokenizers for complex scripts.
...
TL;DR: The paper argues that writing is essential for cognitive development and deep learning, and that outsourcing writing to AI tools like ChatGPT risks losing these cognitive benefits. It explores AI-text detection, pedagogical adaptations, and the need for machine-generated language recognition as a critical literacy skill.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the increasing use of generative AI tools like ChatGPT in educational and professional settings, which threatens to reduce writing to a mere formality. The authors are concerned that this automation strips writing of its cognitive value and undermines the learning process that occurs through the act of writing itself.

Method: The paper draws on cognitive psychology, educational theory, and real classroom practices to analyze what is lost when machines write for humans. It explores AI-text detection capabilities and proposes pedagogical adaptations rather than outright bans on AI tools.

Result: The analysis reveals that the writing process—characterized as messy, slow, and often frustrating—is where deep human learning occurs. The paper identifies current possibilities in AI-text detection and argues for smarter pedagogical approaches that leverage rather than prohibit AI tools.

Conclusion: Writing cannot be faked without losing its cognitive value. The ability to recognize machine-generated language should become a critical 21st-century literacy skill. Educators should adapt pedagogy to preserve the learning benefits of writing while acknowledging AI's role in modern communication.
...
TL;DR: LLMs can be fine-tuned as token compressors/decompressors that translate long texts into compact discrete latent codes (Z-tokens) with variable-length, content-adaptive compression, achieving up to 18× token reduction while maintaining reconstruction fidelity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of token inefficiency in long-context reasoning by developing a method that can compress lengthy texts into compact representations while preserving semantic content, enabling more efficient processing of long documents and queries.

Method: A self-expressive autoencoding framework that fine-tunes a pretrained LLM with lightweight LoRA-based adapter heads to translate long texts into discrete, variable-length latent codes (Z-tokens) and reconstruct the original text exactly from them, with content-adaptive compression where semantically dense segments receive more tokens while redundant regions are aggressively compressed.

Result: Achieves up to 18 times token reduction on Wikipedia, CNN/DailyMail, HotpotQA, and Qulac-style long-query datasets while preserving reconstruction fidelity and downstream task performance, supporting applications including prompt compression and autoregressive generation in Z-token space.

Conclusion: Off-the-shelf LLMs can function as excellent token compressors/decompressors through simple fine-tuning, offering a potential pathway toward token-efficient long-context reasoning with content-adaptive compression that maintains semantic integrity.
...
TL;DR: TAPO is a reinforcement learning framework that uses English as a pivot language to improve multilingual mathematical reasoning by decoupling language understanding from reasoning through step-level relative advantage.


<details>
  <summary>Details</summary>
Motivation: LLMs show strong English mathematical reasoning but underperform in multilingual contexts due to language understanding deficiencies, creating a need to bridge this performance gap.

Method: Translation-Augmented Policy Optimization (TAPO) built on GRPO, using English as pivot with understand-then-reason paradigm and step-level relative advantage to decouple understanding from reasoning, integrating translation quality rewards.

Result: TAPO outperforms baselines in multilingual mathematical reasoning and translation tasks, shows good generalization to unseen languages and out-of-domain tasks, and effectively synergizes language understanding with reasoning.

Conclusion: TAPO successfully bridges the multilingual performance gap in LLM mathematical reasoning through explicit language alignment and decoupled optimization of understanding and reasoning components.
...
TL;DR: Systematic testing shows minimal prompt context yields highest performance gains in LLM text classification, with diminishing returns and potential accuracy decreases from excessive context; substantial heterogeneity across models/tasks requires individual validation.


<details>
  <summary>Details</summary>
Motivation: While LLMs show promise for cost-effective text classification in social sciences with variable performance, there's a need to understand how to maximize accuracy through prompt engineering, particularly examining prompt context optimization.

Method: Systematically varied three aspects of prompt engineering (label descriptions, instructional nudges, and few-shot examples) across two different examples, testing performance impacts of increasing prompt context.

Result: Minimal increase in prompt context yields highest performance gains; further context increases provide only marginal improvements and can sometimes decrease accuracy. Substantial heterogeneity observed across models, tasks, and batch sizes.

Conclusion: Individual validation of each LLM coding task is essential rather than relying on general rules, as optimal prompt context varies significantly across different applications and excessive context can be counterproductive.
...
TL;DR: LLM-based synthetic data generation fails for Romansh due to confusion between its 6 varieties; aligning augmentation direction with resource gradient yields better results, surpassing Gemini 3 Pro by 23 BLEU in lowest-resource variety.


<details>
  <summary>Details</summary>
Motivation: Standard LLM-based synthetic data generation for low-resource MT fails for Romansh because LLMs confuse its 6 distinct language varieties, requiring a different approach.
...
Result: Approach surpasses Gemini 3 Pro by 23 BLEU in lowest-resource Romansh variety; human evaluation confirms first model generating fluent translations in individual Romansh varieties.

Conclusion: For languages with multiple varieties like Romansh, data augmentation should follow resource gradients rather than relying on LLM-based synthetic data generation from higher-resource languages.
...
TL;DR: This paper critically assesses 12 fake news detection approaches across 10 diverse datasets, evaluating in-domain, multi-domain, and cross-domain performance to understand generalization challenges in text-only English fake news detection.


<details>
  <summary>Details</summary>
Motivation: Despite advances in fake news detection, the problem has become more sophisticated due to LLMs and social media amplification. There's a need to systematically evaluate how well current detection methods generalize across different domains and datasets, especially given domain shift and out-of-distribution challenges in real-world scenarios.

Method: The study evaluates 12 representative approaches spanning traditional ML, deep learning, transformers, and specialized cross-domain architectures on 10 publicly available datasets. They treat fake news detection as binary classification ("Real" vs "Fake") with harmonized labels, treating each dataset as a distinct domain. They conduct in-domain, multi-domain, and cross-domain experiments to simulate real-world domain shift scenarios.

Result: Fine-tuned models perform well in-domain but struggle to generalize across domains. Cross-domain architectures can reduce this generalization gap but are data-hungry. LLMs show promise through zero- and few-shot learning capabilities. Results should be interpreted as robustness evaluations within the English text-only protocol due to dataset confounds and possible pre-training exposure.

Conclusion: Current fake news detection methods face significant generalization challenges across domains. While cross-domain architectures help, they require substantial data. LLMs offer a promising alternative through their few-shot capabilities. The study provides a critical assessment framework but acknowledges limitations in label harmonization and dataset-specific confounds.
...
TL;DR: VLMs generate visually-grounded stories with human-like surface fluency but systematic differences in narrative coherence compared to human-written narratives.


<details>
  <summary>Details</summary>
Motivation: To understand how vision-language models (VLMs) organize discourse in visually-grounded narratives compared to human writers, assessing narrative coherence beyond surface fluency.

Method: Comparative analysis using metrics for narrative coherence (coreference, discourse relations, topic continuity, character persistence, multimodal grounding) on Visual Writing Prompts corpus, computing narrative coherence scores for human vs. VLM-generated stories.

Result: VLMs show broadly similar coherence profiles that systematically differ from humans; individual metric differences are subtle but become clearer when considered jointly.

Conclusion: Despite human-like surface fluency, VLM narratives exhibit systematic differences in discourse organization for visually-grounded stories, revealing limitations in narrative coherence.
...
TL;DR: PICon is an evaluation framework that systematically probes persona agents for contradictions using logically chained multi-turn questioning, revealing that even supposedly consistent LLM-based persona agents fail to match human consistency across internal, external, and retest dimensions.


<details>
  <summary>Details</summary>
Motivation: LLM-based persona agents are increasingly used as scalable proxies for human participants, but there's no systematic method to verify whether their responses remain contradiction-free and factually accurate throughout interactions, creating a reliability gap.

Method: PICon applies interrogation methodology principles to probe persona agents through logically chained multi-turn questioning, evaluating consistency along three dimensions: internal consistency (freedom from self-contradiction), external consistency (alignment with real-world facts), and retest consistency (stability under repetition).

Result: Evaluation of seven groups of persona agents alongside 63 human participants revealed that even systems previously reported as highly consistent fail to meet human baselines across all three dimensions, showing contradictions and evasive responses under chained questioning.

Conclusion: PICon provides both conceptual foundation and practical methodology for evaluating persona agents before trusting them as human participant substitutes, highlighting current limitations and establishing systematic verification standards.
...
TL;DR: Analysis of arXiv papers reveals LLM-driven shifts in word usage patterns, with current classifiers struggling to identify specific LLM sources in multi-class classification due to model similarities, while real-world LLM usage shows heterogeneous and dynamic patterns.


<details>
  <summary>Details</summary>
Motivation: To investigate previously overlooked word usage shifts in academic papers likely driven by large language models (LLMs), and to understand the challenges in identifying specific LLM sources due to model similarities and evolving usage patterns.

Method: Analysis of arXiv papers using a direct, highly interpretable linear approach that accounts for differences between models and prompts, enabling quantitative assessment of LLM-driven word usage effects.
...
Conclusion: LLMs are driving measurable shifts in academic writing patterns, but current detection methods face challenges due to model similarities and evolving usage, highlighting the need for more sophisticated approaches to understand and track LLM influence in academic literature.
...
TL;DR: LLM-based automated scoring systems show robustness to most construct-irrelevant factors but are vulnerable to text duplication and heavily penalize off-topic responses.


<details>
  <summary>Details</summary>
Motivation: Automated scoring systems are widely used but vulnerable to construct-irrelevant factors; with increasing LLM adoption, there's renewed focus on hallucinations and robustness in LLM-based scoring approaches.

Method: Investigated effects of construct-irrelevant factors on a dual-architecture LLM-based scoring system designed for short essay-like open-response items in a situational judgment test.

Result: System was robust to padding with meaningless text, spelling errors, and writing sophistication. Text duplication resulted in lower scores (contradicting previous non-LLM studies), and off-topic responses were heavily penalized.

Conclusion: Results provide encouraging support for LLM-based scoring system robustness when designed with construct relevance in mind.
...
TL;DR: This paper presents a system-level framework for self-improving language models, conceptualizing the process as a closed-loop lifecycle with four core processes and an autonomous evaluation layer, where models autonomously drive their own improvement through data generation, selection, optimization, and refinement.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from two key challenges: (1) the increasing cost and scalability limitations of human supervision as LLMs advance, and (2) the diminishing informativeness of human feedback as models approach human-level capabilities. Simultaneously, models' growing autonomous decision-making capabilities create opportunities to automate components of the development process.

Method: The paper introduces a unified framework organizing self-improvement techniques into a closed-loop lifecycle with four tightly coupled processes: data acquisition, data selection, model optimization, and inference refinement, plus an autonomous evaluation layer. The model itself drives each stage while the evaluation layer monitors progress and guides improvement across stages.

Result: The paper systematically reviews and analyzes representative methods for each component from a technical standpoint, providing a comprehensive framework for understanding existing self-improvement techniques and their interrelationships within a unified system-level perspective.

Conclusion: The paper concludes by discussing current limitations and outlining a vision for future research toward fully self-improving LLMs, emphasizing the need for continued development of autonomous evaluation and closed-loop improvement systems as models approach and potentially surpass human-level capabilities.
...
TL;DR: S2D2 is a training-free self-speculative decoding framework for block-diffusion language models that uses the same pretrained model as both drafter and verifier by exploiting its autoregressive behavior at block size 1, improving accuracy-speed tradeoffs without additional training or extra test-time compute.


<details>
  <summary>Details</summary>
Motivation: Standard confidence-thresholded decoding in block-diffusion language models is brittle in the few-step regime needed for practical acceleration: aggressive thresholds hurt quality while conservative thresholds require unnecessary denoising steps. Existing approaches either require additional training or incur extra test-time compute.

Method: S2D2 inserts a speculative verification step into standard block-diffusion decoding using the same pretrained model as both drafter (block-diffusion mode) and verifier (autoregressive mode at block size 1). It uses lightweight routing policies to decide when verification is worth its cost, creating a hybrid decoding trajectory where diffusion proposes tokens in parallel while autoregressive mode acts as a local sequence-level critic.

Result: Across three mainstream block-diffusion families, S2D2 consistently improves accuracy-speed tradeoff over strong confidence-thresholding baselines. On SDAR: up to 4.7× speedup over autoregressive decoding, up to 1.57× over tuned dynamic decoding baseline while improving accuracy by up to 4.5 points. On LLaDA2.1-Mini: remains complementary to built-in self-correction, including conservative setting where it's 4.4× faster than static baseline with slightly higher accuracy.

Conclusion: S2D2 provides an effective training-free solution to the brittleness of confidence-thresholded decoding in block-diffusion language models, achieving significant speedups while maintaining or improving accuracy through self-speculative verification using the same pretrained model in dual roles.
...
TL;DR: Natural-Language Agent Harnesses (NLAHs) externalize agent control logic as portable natural language artifacts, executed by Intelligent Harness Runtime (IHR) through explicit contracts and adapters.


<details>
  <summary>Details</summary>
Motivation: Agent performance depends on harness engineering, but current harness designs are buried in controller code and runtime-specific conventions, making them hard to transfer, compare, and study scientifically.
...
Conclusion: High-level control logic of agent harnesses can be externalized as portable executable artifacts using natural language specifications and a shared runtime system.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal event forecasting methods overlook dynamic acquisition and fusion of multimodal knowledge. Current approaches fail to capture time-sensitive structural information across modalities and struggle with evolving multimodal fusion features due to static coattention mechanisms.

Method: DyMRL integrates time-specific structural features from Euclidean, hyperbolic, and complex spaces into a relational message-passing framework. It uses pretrained models for time-sensitive visual/linguistic intelligence and incorporates dual fusion-evolution attention mechanisms for dynamic multimodal fusion.
...
Conclusion: The proposed DyMRL approach effectively addresses dynamic multimodal knowledge acquisition and fusion challenges in event forecasting by leveraging multispace representation learning and dynamic attention mechanisms.
...
TL;DR: The paper introduces metrics to measure symmetry content in unconstrained ML models for physics, analyzes how symmetry information is learned, establishes a framework for diagnosing spectral failures, and demonstrates that strategic injection of minimal inductive biases improves stability and accuracy while preserving expressivity.


<details>
  <summary>Details</summary>
Motivation: Unconstrained ML models for physical simulations often achieve competitive performance without enforcing exact symmetries, learning approximate equivariant behavior through data augmentation. However, there's a need for rigorous metrics to quantify symmetry content and understand how these models process symmetry information across architectural layers.

Method: Introduces rigorous metrics to measure symmetry content in learned representations and assess equivariant condition accuracy. Applies these metrics to two unconstrained transformer-based models: a graph neural network for atomistic simulations and a PointNet-style architecture for particle physics. Analyzes symmetry information processing across layers and learning during training.

Result: Establishes a framework for diagnosing spectral failure modes in ML models. Demonstrates that strategic injection of minimum required inductive biases leads to superior stability and accuracy while preserving the high expressivity and scalability of unconstrained architectures and guaranteeing physical fidelity.

Conclusion: Rigorous symmetry metrics enable understanding of how unconstrained models learn equivariant behavior. Strategic minimal inductive bias injection provides a balanced approach that maintains model expressivity while ensuring physical fidelity, offering superior performance compared to either fully constrained or completely unconstrained approaches.
...
TL;DR: ERL is a self-improvement framework that enables LLM agents to adapt to specialized environments by reflecting on past task experiences to generate transferable heuristics that guide future task execution.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based autonomous agents struggle with environment adaptation and don't leverage accumulated experience, approaching each new task from scratch despite having past interactions.

Method: Experiential Reflective Learning (ERL) framework: agents reflect on task trajectories and outcomes to generate heuristics, then retrieve relevant heuristics at test time based on current task context and inject them into the agent's execution guidance.
...
Conclusion: Reflecting on single-attempt experiences to extract transferable heuristics enables effective agent self-improvement, demonstrating that experiential learning through heuristic generation and selective retrieval significantly enhances LLM agent adaptation and performance.
...
TL;DR: Neural network framework learns mesh-free discrete differential operators from local stencil geometry, achieving polynomial consistency with favorable accuracy-cost trade-off for irregular particle configurations.


<details>
  <summary>Details</summary>
Motivation: Classical meshless methods face a trade-off between computational cost and accuracy: low-cost methods have limited accuracy while high-accuracy methods require substantial per-stencil computation. There's a need for mesh-free operators that maintain accuracy while being computationally efficient and robust to irregular geometries.
...
Result: The learned operators show improved accuracy over Smoothed Particle Hydrodynamics and favorable accuracy-cost trade-off relative to high-order consistent mesh-free methods in moderate-accuracy regimes. Operators are resolution-agnostic, reusable across particle configurations and governing equations, and successfully applied to solve weakly compressible Navier-Stokes equations.

Conclusion: Neural networks can effectively learn mesh-free discrete differential operators that achieve polynomial consistency while being robust to irregular geometries, offering a practical alternative to classical meshless methods with better computational efficiency and accuracy balance.
...
TL;DR: A Physics-Informed Neural Network (PINN) digital twin framework for dynamic, tray-wise modeling of binary distillation columns under transient conditions, combining Aspen simulation data with embedded thermodynamic constraints for superior accuracy over data-only baselines.


<details>
  <summary>Details</summary>
Motivation: To develop a more accurate and physically consistent digital twin for industrial distillation processes that can handle transient operations while embedding fundamental thermodynamic principles, overcoming limitations of purely data-driven approaches that may violate physical laws.

Method: Proposes a PINN digital twin framework that embeds thermodynamic constraints (vapor-liquid equilibrium via modified Raoult's law, tray-level mass/energy balances, McCabe-Thiele methodology) directly into neural network loss function via physics residual terms. Uses adaptive loss-weighting to balance data fidelity and physics consistency. Trained on high-fidelity synthetic dataset from Aspen HYSYS (961 timestamped measurements over 8 hours for 16 sensor streams).

Result: Achieves RMSE of 0.00143 for HX mole fraction prediction (R² = 0.9887), representing 44.6% reduction over best data-only baseline (LSTM, MLP, GRU, Transformer, DeepONet). Strictly satisfies thermodynamic constraints. Accurately captures column dynamics including feed tray responses, reflux ratio variations, and pressure transients under transient perturbations.

Conclusion: The PINN digital twin framework establishes a robust foundation for real-time soft sensing, model-predictive control, and anomaly detection in industrial distillation processes by combining simulation data with embedded physics, outperforming purely data-driven approaches while maintaining physical consistency.
...
TL;DR: LLM agents can edit training code directly for hyperparameter optimization, but classical methods like CMA-ES and TPE outperform LLMs on fixed search spaces. However, LLM-based code editing narrows the gap, and a hybrid approach (Centaur) combining CMA-ES with small LLMs achieves best results.


<details>
  <summary>Details</summary>
Motivation: To compare classical hyperparameter optimization (HPO) algorithms against LLM-based methods for tuning language model hyperparameters, and to explore whether LLMs can effectively edit training code directly in unconstrained search spaces.

Method: Used autoresearch repository as testbed to compare classical HPO (CMA-ES, TPE) against LLM-based methods. Introduced Centaur, a hybrid approach that shares CMA-ES's internal state (mean vector, step-size, covariance matrix) with an LLM to combine classical optimization with domain knowledge.

Result: Classical HPO methods outperform LLM-based agents on fixed search spaces. LLM-based code editing narrows the gap substantially. Centaur hybrid achieves best results, with 0.8B variant outperforming 27B variant. Reliability matters more than exploration breadth. Small LLMs suffice when paired with strong classical optimizers.

Conclusion: Hybrid approaches combining classical optimization algorithms with LLMs (even small ones) are most effective for hyperparameter optimization. LLMs struggle with optimization state tracking but provide valuable domain knowledge, while classical methods lack domain knowledge but excel at optimization mechanics.
...
TL;DR: Hierarchical federated learning framework for underwater anomaly detection addresses acoustic communication constraints through three-tier architecture with compressed updates and selective fog cooperation, achieving full sensor participation and significant energy savings.


<details>
  <summary>Details</summary>
Motivation: Standard flat federated learning faces limitations in underwater deployments due to expensive long-range acoustic transmissions and reduced sensor participation when only a subset can reach the gateway directly, necessitating an energy-efficient solution.
...
Result: Hierarchical learning preserves full sensor participation (vs. 48% in flat FL), selective cooperation reduces energy by 31-33% while matching accuracy, compressed uploads reduce total energy by 71-95%, and framework remains competitive on real benchmarks.

Conclusion: The hierarchical federated learning framework provides practical design guidance for underwater deployments under severe acoustic constraints, balancing detection quality, communication energy, and network participation through localized communication and selective cooperation.
...
### [206] [GraphER: An Efficient Graph-Based Enrichment and Reranking Method for Retrieval-Augmented Generation](https://arxiv.org/abs/2603.24925)
*Ruizhong Miao,Yuying Wang,Rongguang Wang,Chenyang Li,Tao Sheng,Sujith Ravi,Dan Roth*

Main category: cs.LG

TL;DR: GraphER is a graph-based enrichment and reranking method for RAG systems that captures multiple proximity forms beyond semantic similarity without requiring knowledge graphs, enabling seamless integration with standard vector stores.


<details>
  <summary>Details</summary>
Motivation: Current semantic search in RAG systems is insufficient for complex information needs where evidence is scattered across multiple sources. Agentic retrieval strategies expand search space but inefficiently explore without leveraging data structure, while knowledge graph approaches capture richer proximities but incur high maintenance costs and are incompatible with standard vector stores.

Method: GraphER independently enriches data objects during offline indexing and performs graph-based reranking over candidate objects at query time. It captures multiple forms of proximity beyond semantic similarity without requiring a knowledge graph, making it compatible with standard vector stores. The approach is retriever-agnostic and introduces negligible latency overhead.
...
Conclusion: GraphER addresses limitations of existing approaches by providing a graph-based enrichment and reranking method that captures richer proximities without the maintenance costs of knowledge graphs, enabling seamless integration with production vector stores while maintaining efficiency.
...
### [207] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
*Kaan Durmaz,Jan Schuchardt,Sebastian Schmidt,Stephan Günnemann*

Main category: cs.LG

TL;DR: Random cropping in vision models amplifies differential privacy by probabilistically excluding sensitive localized content, providing stronger privacy guarantees without modifying training procedures.


<details>
  <summary>Details</summary>
Motivation: Random cropping is widely used in computer vision but its privacy implications have been unexplored. When sensitive content (faces, license plates) is spatially localized, random cropping can probabilistically exclude it from model inputs, potentially amplifying differential privacy without architectural changes.

Method: Introduce patch-level neighboring relation for vision data and derive tight privacy bounds for DP-SGD combined with random cropping. Analyze patch inclusion probability and how it composes with minibatch sampling to yield lower effective sampling rate. Validate empirically across segmentation architectures and datasets.

Result: Patch-level amplification improves privacy-utility trade-off across multiple segmentation architectures and datasets. The analysis quantifies how random cropping reduces effective sampling rate, providing stronger differential privacy guarantees.

Conclusion: Aligning privacy accounting with domain structure and existing sources of randomness (like random cropping) yields stronger privacy guarantees at no additional cost, demonstrating the importance of incorporating domain-specific knowledge into privacy analysis.
...
### [208] [Training LLMs for Multi-Step Tool Orchestration with Constrained Data Synthesis and Graduated Rewards](https://arxiv.org/abs/2603.24709)
...
TL;DR: A framework combining RL environment with real API response cache and graduated reward design improves multi-step tool orchestration performance by addressing data synthesis and partial correctness signaling challenges.


<details>
  <summary>Details</summary>
Motivation: Multi-step tool orchestration is challenging for LLMs due to frequent failures in full sequence execution, particularly parameter value errors. Existing training approaches face obstacles: environments focus on simple per-turn function calls with simulated data, and binary rewards lack signals for partial correctness.

Method: Two-part framework: 1) Reinforcement learning environment backed by large-scale cache of real API responses enabling efficient data synthesis pipeline for valid multi-step orchestration traces with controllable complexity; 2) Graduated reward design decomposing correctness into atomic validity (individual function call correctness at increasing granularity) and orchestration (correct tool sequencing with dependency respect).
...
Conclusion: The framework effectively addresses key challenges in training LLMs for multi-step tool orchestration by providing realistic training data through API response caching and nuanced reward signals through graduated reward decomposition, leading to improved performance on complex orchestration tasks.
...
### [209] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: ACOF is an actor-critic optimization framework for analog sizing that separates proposal and evaluation roles to bring designer-like guidance into the optimization loop, improving search efficiency and quality.


<details>
  <summary>Details</summary>
Motivation: Analog design optimization is slow due to expensive simulation cycles and the need to find solutions in narrow regions of large search spaces. Existing optimizers lack the judgment designers use to decide where to search next.
...
TL;DR: SPARTA: A contrastive learning framework for creating robust spatiotemporal embeddings from sparse weather data, outperforming autoencoders on downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Weather data is high-dimensional and multimodal, requiring compression for efficient downstream tasks. Current contrastive learning approaches don't adequately address sparse data common in real-world weather collection, nor compare sufficiently with alternative compression methods like autoencoders.

Method: SPARTA framework with: 1) contrastive loss aligning sparse samples with complete ones, 2) temporally aware batch sampling, 3) cycle-consistency loss for latent space structure, 4) graph neural network fusion for domain-specific physical knowledge injection.

Result: Contrastive learning proves feasible and advantageous for sparse geoscience data compression, enhancing performance on downstream tasks compared to alternative methods.

Conclusion: SPARTA demonstrates that contrastive learning with sparse data augmentation creates more robust embeddings for weather data, improving efficiency and performance for forecasting and extreme-weather detection applications.
...
<details>
  <summary>Details</summary>
Motivation: To move beyond analogical phase-transition language in grokking by providing falsifiable finite-size inputs and quantitative testing methods, addressing the lack of concrete evidence for phase-transition claims.

Method: Treats group order p of ℤ_p as an extensive variable, uses held-out spectral head-tail contrast as representation-level order parameter, applies condensed-matter-style diagnostic chain including coarse-grid sweeps and dense near-critical addition audit with Binder-like crossings and susceptibility comparison.

Result: Binder-like crossings reveal a shared finite-size boundary, and susceptibility comparison strongly disfavors smooth-crossover interpretation (ΔAIC = 16.8 in near-critical audit), supporting phase-transition framework.

Conclusion: Phase-transition language in grokking can be tested as a quantitative finite-size claim rather than invoked as analogy alone, though transition order remains unresolved.
...
<details>
  <summary>Details</summary>
Motivation: Standard vision models treat objects as independent Euclidean points, failing to capture hierarchical structure where parts exist within wholes. Current approaches lack geometric structure to represent asymmetric causality in visual hierarchies.

Method: Introduces Worldline Slot Attention that models objects as persistent trajectories through spacetime worldlines, where each object has multiple slots at different hierarchy levels sharing spatial position but differing in temporal coordinates. Compares Euclidean vs Lorentzian geometric structures for representing worldlines.
...
Conclusion: Hierarchical object discovery requires geometric structure encoding asymmetric causality, an inductive bias absent from Euclidean space but natural to Lorentzian light cones. This is achieved with only 11K parameters, demonstrating the importance of causal structure over tree structure for visual hierarchies.
...
TL;DR: FFzero is a forward-only learning framework that enables neural network training without backpropagation or automatic differentiation, using layer-wise local learning and prototype-based representations for physical neural networks.
...
Method: FFzero combines layer-wise local learning, prototype-based representations, and directional-derivative-based optimization using forward evaluations only, without backpropagation or automatic differentiation.

Result: The framework shows that local learning is effective under forward-only optimization where backpropagation fails, generalizes to multilayer perceptron and convolutional neural networks across classification and regression tasks, and provides a viable path toward backpropagation-free in-situ physical learning.

Conclusion: FFzero enables stable neural network training without backpropagation, offering a practical solution for physical neural networks where traditional gradient-based methods are difficult to implement.
...
TL;DR: Benchmark study evaluating interpretability methods across clinical prediction tasks reveals attention mechanisms are efficient for faithful interpretation, black-box methods are computationally infeasible for time-series, and many approaches are unreliable.


<details>
  <summary>Details</summary>
Motivation: Clinical decisions require explicit justification, making model interpretability essential for auditing deep clinical models. As model architectures and explainability methods expand, critical questions remain about whether architectural features like attention improve explainability and whether interpretability approaches generalize across clinical tasks.
...
Conclusion: The study provides guidelines for improving interpretability within clinical predictive pipelines and offers an open-source implementation via PyHealth to support reproducibility and extensibility in clinical interpretability research.
...
<details>
  <summary>Details</summary>
Motivation: To develop a simpler, faster, and fully intrinsic framework for flow matching on homogeneous spaces without dealing with complicated geometry.

Method: Reformulates homogeneous space flow matching as a task on underlying Lie groups by lifting data distributions, then reduces to Euclidean flow matching on Lie algebras.

Result: Eliminates need for premetrics or geodesics, creating a simpler and faster framework compared to Riemannian Flow Matching.

Conclusion: Provides a general, intrinsic framework for flow matching on homogeneous spaces that avoids complex geometric computations.
...
TL;DR: Multi-answer reinforcement learning trains language models to generate multiple plausible answers in a single forward pass, improving diversity and efficiency over single-answer approaches.


<details>
  <summary>Details</summary>
Motivation: Standard LM training collapses answer distributions to single dominant modes, which fails for real-world tasks with multiple valid answers (medical diagnosis, ambiguous QA, incomplete information). Current approaches require computationally intensive repeated sampling to generate non-modal answers.

Method: Multi-answer reinforcement learning modifies RL objectives to enable models to explicitly generate multiple candidate answers in a single forward pass, internalizing inference-time search into the generative process.
...
Conclusion: Multi-answer RL provides a principled and compute-efficient alternative to inference-time scaling procedures like best-of-k, enabling distributional reasoning over multiple answers during inference.
...
TL;DR: The paper investigates two ML approaches for optimizing real-time staffing in warehouse sortation systems: custom Transformer-based offline RL policies and LLMs operating on abstracted state descriptions, both achieving performance improvements over historical baselines.


<details>
  <summary>Details</summary>
Motivation: To develop AI-assisted operational decision-making for semi-automated warehouse sortation systems, addressing the need for real-time staffing optimization at different levels of abstraction with varying trade-offs between detailed state representations and human-readable inputs.

Method: Two complementary approaches: 1) Custom Transformer-based policies trained using offline reinforcement learning on detailed historical state representations, evaluated in learned simulators; 2) LLMs operating on abstracted, human-readable state descriptions, systematically comparing prompting techniques, automatic prompt optimization, and fine-tuning strategies including supervised fine-tuning with Direct Preference Optimization on simulator-generated preferences.

Result: Offline RL achieved 2.4% throughput improvement over historical baselines in learned simulators, translating to significant savings in high-volume operations. LLMs with supervised fine-tuning combined with Direct Preference Optimization achieved performance matching or slightly exceeding historical baselines in hand-crafted simulators, though prompting alone proved insufficient.

Conclusion: Both approaches offer viable paths toward AI-assisted operational decision-making: offline RL excels with task-specific architectures and detailed state representations, while LLMs support human-readable inputs and can incorporate manager preferences through iterative feedback loops.
...
TL;DR: HYPER-TINYPW replaces stored pointwise convolution weights with generated weights using a shared micro-MLP, achieving 6.31x compression while maintaining performance on ECG and audio tasks for microcontroller deployment.


<details>
  <summary>Details</summary>
Motivation: Neural network deployment on microcontrollers is constrained by limited flash and SRAM memory, where 1x1 pointwise convolutions dominate memory usage even after INT8 quantization across vision, audio, and wearable sensing applications.

Method: A compression-as-generation approach where a shared micro-MLP synthesizes pointwise convolution kernels at load time from tiny per-layer codes, caching them for execution with standard integer operators. The method enforces a shared latent basis across layers to remove cross-layer redundancy while keeping the first pointwise layer in INT8 for stability.

Result: On three ECG benchmarks, HYPER-TINYPW achieves 6.31x compression (84.15% fewer bytes) while matching the performance of a roughly 1.4 MB CNN at about 225 kB. Under 32-64 kB budgets, it sustains balanced detection where compact baselines degrade. On Speech Commands audio dataset, it reaches 96.2% test accuracy (98.2% best validation).

Conclusion: HYPER-TINYPW shifts the macro-F1 versus flash Pareto frontier for microcontroller deployment, demonstrating that compression-as-generation is effective for resource-constrained ML systems where per-layer redundancy dominates, with broad applicability to 1D biosignals, on-device speech, and embedded sensing tasks.
...
### [219] [CVA: Context-aware Video-text Alignment for Video Temporal Grounding](https://arxiv.org/abs/2603.24934)
*Sungho Moon,Seunghun Lee,Jiwan Seo,Sunghoon Im*

Main category: cs.LG

TL;DR: CVA is a novel video temporal grounding framework that addresses alignment robustness to irrelevant background context through query-aware data augmentation, contrastive boundary discrimination, and hierarchical transformer architecture, achieving SOTA performance with ~5 point R1 improvement.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of achieving temporally sensitive video-text alignment that remains robust to irrelevant background context in video temporal grounding, particularly mitigating false negatives caused by query-agnostic mixing and contextual shifts.

Method: Three key components: 1) Query-aware Context Diversification (QCD) - data augmentation using video-text similarity-based pool to mix only semantically unrelated content; 2) Context-invariant Boundary Discrimination (CBD) loss - contrastive loss enforcing semantic consistency at temporal boundaries; 3) Context-enhanced Transformer Encoder (CTE) - hierarchical architecture combining windowed self-attention and bidirectional cross-attention with learnable queries for multi-scale temporal context.

Result: Achieves state-of-the-art performance on major VTG benchmarks (QVHighlights and Charades-STA) with significant improvement of approximately 5 points in Recall@1 (R1) scores over previous SOTA methods, demonstrating effectiveness in mitigating false negatives.

Conclusion: The synergy of data-centric (QCD) and architectural (CTE) enhancements with boundary-aware contrastive learning (CBD) enables robust video-text alignment that is temporally sensitive and context-invariant, establishing a new SOTA in video temporal grounding.
...
### [220] [A Systematic Empirical Study of Grokking: Depth, Architecture, Activation, and Regularization](https://arxiv.org/abs/2603.25009)
*Shalima Binta Manir,Anamika Paul Rupa*

Main category: cs.LG

TL;DR: Grokking dynamics in neural networks are primarily driven by interactions between optimization stability and regularization, not architecture, with weight decay being the dominant control parameter requiring a narrow "Goldilocks" regime for delayed generalization to occur.


<details>
  <summary>Details</summary>
Motivation: To disentangle the confounding roles of architecture, optimization, and regularization in grokking (delayed transition from memorization to generalization), as prior studies have not systematically separated these factors.

Method: Controlled study on modular addition (mod 97) with matched and carefully tuned training regimes across models, systematically varying architecture (depth, activation functions), optimization parameters, and regularization (weight decay) while controlling for confounds.
...
Conclusion: Grokking is an interaction-driven phenomenon governed by optimization and regularization, challenging architecture-centric interpretations and providing unified empirical account of delayed generalization dynamics.
...
### [221] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*

Main category: cs.LG

TL;DR: First high-probability minimax optimal regret bound for OCO with two-point bandit feedback under strongly convex losses.


<details>
  <summary>Details</summary>
Motivation: Resolve the open problem highlighted by Agarwal et al. (2010) regarding achieving tight high-probability regret bounds for strongly convex functions in OCO with two-point bandit feedback, where the challenge stems from heavy-tailed bandit gradient estimators making standard concentration analysis difficult.

Method: Develop new techniques to handle heavy-tailed nature of bandit gradient estimators in two-point feedback setting, enabling proper concentration analysis for strongly convex functions.

Result: Achieve minimax optimal high-probability regret bound of O(d(log T + log(1/δ))/μ) for μ-strongly convex losses, optimal in both time horizon T and dimension d.

Conclusion: Resolves long-standing open problem in bandit convex optimization, providing first tight high-probability regret guarantees for strongly convex functions with two-point feedback.
...
### [222] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*

Main category: cs.LG

TL;DR: Epistemic Compression: Robustness in high-stakes domains requires matching model complexity to data shelf life, not scaling parameters, using architecture-based parsimony enforced through a Regime Index.


<details>
  <summary>Details</summary>
Motivation: Foundation models fail in high-stakes domains (medicine, finance, policy) due to the Fidelity Paradox - extra capacity amplifies noise in changing environments where reliability matters most.

Method: Introduces Epistemic Compression principle and Regime Index that separates Shifting Regime (unstable, data-poor) from Stable Regime (invariant, data-rich), enforcing parsimony through architectural design rather than post-hoc regularization.
...
Conclusion: High-stakes AI requires shift from scaling for its own sake to principled parsimony, matching model complexity to data shelf life through architectural constraints.
...
### [223] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
TL;DR: Intern-S1-Pro is a 1-trillion parameter scientific multimodal foundation model that combines general AI capabilities with specialized scientific expertise across 100+ tasks in chemistry, materials, life sciences, and earth sciences, featuring advanced agent capabilities and efficient RL training infrastructure.


<details>
  <summary>Details</summary>
Motivation: To create the first one-trillion-parameter scientific multimodal foundation model that bridges the gap between general AI capabilities and specialized scientific expertise, addressing the need for comprehensive scientific intelligence across multiple domains.

Method: Scaling to 1-trillion parameters using XTuner and LMDeploy infrastructure for efficient Reinforcement Learning training while maintaining precision consistency between training and inference; integrating multimodal capabilities with advanced agent functionalities.

Result: Achieved comprehensive enhancement across general and scientific domains with stronger reasoning and image-text understanding; mastered over 100 specialized scientific tasks; demonstrated top-tier performance among open-source models for general capabilities and outperformed proprietary models in specialized scientific tasks.
...
### [224] [The Order Is The Message](https://arxiv.org/abs/2603.25047)
*Jordan LeDoux*

Main category: cs.LG

TL;DR: Fixed-ordering training strategies achieve near-perfect test accuracy with only 0.3% of input space, far exceeding IID baseline performance, while adversarial ordering prevents learning entirely, revealing ordering structure as a critical factor in learning efficiency.


<details>
  <summary>Details</summary>
Motivation: To investigate how example ordering affects learning efficiency and generalization in modular arithmetic tasks, challenging the assumption that IID sampling is optimal and exploring whether structured ordering can enable learning from minimal data below established sample complexity bounds.

Method: Controlled experiment on modular arithmetic (p=9973) with identical training data (0.3% of input space) but varying only example ordering: fixed-ordering strategies, IID baseline, and adversarially structured ordering, analyzing learning dynamics across epochs and Fourier representations.

Result: Fixed-ordering strategies achieve 99.5% test accuracy by epochs 487 and 659 respectively, while IID baseline reaches only 0.30% after 5,000 epochs. Adversarial ordering suppresses learning entirely. Generalizing models construct Fourier representations with fundamental frequency matching the Fourier dual of ordering structure, consistent across all seeds and initializations.

Conclusion: Example ordering structure significantly impacts learning efficiency, enabling generalization from minimal data below theoretical bounds. This reveals a training efficiency channel that bypasses content-level auditing and suggests grokking phenomena may be reinterpreted through ordering effects rather than sudden generalization.
...
### [225] [SIGMA: Structure-Invariant Generative Molecular Alignment for Chemical Language Models via Autoregressive Contrastive Learning](https://arxiv.org/abs/2603.25062)
*Xinyu Wang,Fei Dou,Jinbo Bi,Minghu Song*

Main category: cs.LG

TL;DR: SIGMA addresses modality mismatch in linearized molecular generation by aligning latent states of equivalent partial graphs via token-level contrastive learning, with IsoBeam pruning isomorphic redundancy during inference.
...
Conclusion: SIGMA resolves trajectory divergence in linearized molecular generation by enabling strict recognition of geometric symmetries through contrastive alignment and isomorphic pruning, maintaining the efficiency of string formulations while improving structural fidelity.
...
### [226] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
*Mohammad A. Farmani,Hoshin V. Gupta,Ali Behrangi,Muhammad Jawad,Sadaf Moghisi,Guo-Yue Niu*

Main category: cs.LG

TL;DR: The Mass-Conserving Perceptron (MCP) framework embeds physical hydrological processes into AI models, showing that progressive inclusion of process representations improves predictive skill while maintaining interpretability, with performance gains being hydroclimate-dependent.


<details>
  <summary>Details</summary>
Motivation: Machine learning models in hydrology often achieve high accuracy but lack physical interpretability, creating a need for physics-aware AI frameworks that enforce conservation principles while learning process relationships from data.
...
Result: Progressively augmenting MCP's internal physical structure generally improves predictive performance. Vertical drainage substantially improves skill in arid and snow-dominated basins but reduces performance in rainfall-dominated regions, while surface ponding has small effects. Best-performing MCP configurations approach LSTM benchmark skill while maintaining explicit physical interpretability.
...
### [227] [Layer-Specific Lipschitz Modulation for Fault-Tolerant Multimodal Representation Learning](https://arxiv.org/abs/2603.25103)
...
<details>
  <summary>Details</summary>
Motivation: Multimodal systems in industrial/safety-critical environments need reliability under sensor failures, signal degradation, or cross-modal inconsistencies.

Method: Two-stage self-supervised training: pre-train multimodal convolutional autoencoder on clean data, then expand with learnable compute block (dense layers for correction, contrastive objectives for anomaly identification) with layer-specific Lipschitz modulation and gradient clipping.
...
### [228] [SEVerA: Verified Synthesis of Self-Evolving Agents](https://arxiv.org/abs/2603.25111)
*Debangshu Banerjee,Changming Xu,Gagandeep Singh*

Main category: cs.LG

TL;DR: SEVerA framework combines formal verification with learning to create self-evolving LLM agents that guarantee safety/correctness while improving task performance.


<details>
  <summary>Details</summary>
Motivation: Existing self-evolving LLM agent frameworks lack formal safety/correctness guarantees, raising reliability and security concerns when deployed autonomously on unseen inputs.

Method: Introduces Formally Guarded Generative Models (FGGM) with formal output contracts, and SEVerA framework with three stages: Search (synthesize parametric programs), Verification (prove correctness), and Learning (optimize soft objectives while preserving correctness).
...
Conclusion: Formal behavioral constraints not only guarantee correctness but also steer synthesis toward higher-quality agents, demonstrating the viability of combining formal verification with learning for safe self-evolving AI systems.
...
### [229] [Vision Hopfield Memory Networks](https://arxiv.org/abs/2603.25157)
...
TL;DR: Vision Hopfield Memory Network (V-HMN) is a brain-inspired vision backbone that integrates hierarchical memory mechanisms with iterative refinement for improved interpretability, data efficiency, and biological plausibility compared to Transformer and Mamba architectures.


<details>
  <summary>Details</summary>
Motivation: Current vision/multimodal backbones (Transformers, Mamba) lack biological plausibility, require massive training data, and offer limited interpretability. The paper aims to bridge brain-inspired computation with large-scale ML by developing a foundation model based on computational principles of the human brain.

Method: V-HMN integrates hierarchical memory mechanisms: local Hopfield modules for associative memory at patch level, global Hopfield modules as episodic memory for contextual modulation, and predictive-coding-inspired refinement for iterative error correction. Memory retrieval exposes input-pattern relationships for interpretability, while pattern reuse improves data efficiency.

Result: V-HMN achieved competitive results on public computer vision benchmarks compared to widely adopted backbones, while offering better interpretability, higher data efficiency, and stronger biological plausibility.

Conclusion: V-HMN demonstrates potential as a next-generation vision foundation model and provides a blueprint for multimodal backbones in text/audio domains, bridging brain-inspired computation with large-scale machine learning.
...
### [230] [Train at Moving Edge: Online-Verified Prompt Selection for Efficient RL Training of Large Reasoning Model](https://arxiv.org/abs/2603.25184)
*Jiahao Wu,Ning Lu,Shengcai Liu,Kun Wang,Yanting Yang,Li Qing,Ke Tang*

Main category: cs.LG

TL;DR: HIVE is a dual-stage prompt selection framework that improves RL efficiency for LLMs by identifying high-utility prompts before rollout, focusing on the "learning edge" where intermediate difficulty meets high uncertainty.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for post-training LLMs (like GRPO) require multiple rollouts per prompt, creating prohibitive computational costs since many prompts provide negligible gradients and low utility. The need exists for efficient prompt selection before rollout to reduce computational overhead.

Method: HIVE (History-Informed and online-VErified prompt selection) uses a dual-stage framework: 1) Coarse selection using historical reward trajectories, and 2) Real-time pruning using prompt entropy as a proxy to filter out instances with stale utility. The approach targets the "learning edge" - prompts with intermediate difficulty and high uncertainty that shift during training.

Result: Evaluation across multiple math reasoning benchmarks and models shows HIVE yields significant rollout efficiency improvements without compromising performance compared to standard RL approaches.

Conclusion: HIVE effectively addresses computational inefficiency in RL for LLMs by intelligently selecting high-utility prompts before rollout, leveraging the evolving nature of the "learning edge" to maintain training effectiveness while reducing computational costs.
...
### [231] [Gap Safe Screening Rules for Fast Training of Robust Support Vector Machines under Feature Noise](https://arxiv.org/abs/2603.25221)
*Tan-Hau Nguyen,Thu-Le Tran,Kien Trung Nguyen*

Main category: cs.LG

TL;DR: First safe sample screening rules for robust SVMs that reduce computational cost without affecting optimal solution by identifying samples with uncertainty sets guaranteed to lie entirely on one side of margin hyperplane.


<details>
  <summary>Details</summary>
Motivation: Robust SVMs improve reliability against feature noise but suffer from increased computational cost; need methods to reduce training complexity while preserving robustness guarantees.

Method: Develop safe screening rules for R-SVMs using Lagrangian duality (instead of Fenchel-Rockafellar duality) to identify training samples whose uncertainty sets lie entirely on one side of margin hyperplane; establish ideal screening rule and practical GAP-based adaptation.
...
Conclusion: Safe screening rules can effectively accelerate robust SVM training without compromising solution optimality, enabling practical deployment of robust models with reduced computational burden.
...
### [232] [Offline Decision Transformers for Neural Combinatorial Optimization: Surpassing Heuristics on the Traveling Salesman Problem](https://arxiv.org/abs/2603.25241)
...
<details>
  <summary>Details</summary>
Motivation: Neural Combinatorial Optimization relies on online RL which hampers deployment and underutilizes decades of algorithmic knowledge; offline RL can learn superior strategies directly from existing heuristic solution datasets.

Method: Apply offline RL framework Decision Transformer to learn from heuristic solution datasets; integrate Pointer Network to handle instance-dependent variable action space for node selection; employ expectile regression for optimistic conditioning of Return-to-Go to handle instances with widely varying optimal values.

Result: Method consistently produces higher-quality tours than the four classical heuristics it was trained on, demonstrating the potential of offline RL to exceed performance embedded in existing domain knowledge.

Conclusion: Offline RL with Decision Transformer can unlock and exceed the performance embedded in existing algorithmic knowledge for combinatorial optimization problems like TSP, offering a promising alternative to online RL approaches.
...
### [233] [How Pruning Reshapes Features: Sparse Autoencoder Analysis of Weight-Pruned Language Models](https://arxiv.org/abs/2603.25325)
*Hector Borobia,Elies Seguí-Mas,Guillermina Tormo-Carbó*

Main category: cs.LG

TL;DR: Pruning preferentially preserves rare features over frequent ones in language models, with Wanda pruning maintaining feature structure better than magnitude pruning, and geometric feature survival doesn't predict causal importance.


<details>
  <summary>Details</summary>
Motivation: To systematically understand how weight pruning affects the internal representations of large language models, particularly using Sparse Autoencoders as interpretability probes to analyze feature geometry changes.

Method: Used SAEs as interpretability probes across three model families (Gemma 3 1B, Gemma 2 2B, Llama 3.2 1B), two pruning methods (magnitude and Wanda), and six sparsity levels (0-60%). Investigated five research questions: seed stability, feature survival, SAE transferability, feature fragility, and causal relevance.

Result: Rare SAE features survive pruning far better than frequent ones (rho = -1.0 in 11/17 conditions). Wanda pruning preserves feature structure up to 3.7x better than magnitude pruning. Pre-trained SAEs remain viable on Wanda-pruned models up to 50% sparsity. Geometric feature survival doesn't predict causal importance.
...
### [234] [From Intent to Evidence: A Categorical Approach for Structural Evaluation of Deep Research Agents](https://arxiv.org/abs/2603.25342)
*Shuoling Liu,Zhiquan Tan,Kun Yi,Hui Wu,Yihan Li,Jiangpeng Yan,Liyuan Chen,Kai Chen,Qiang Yang*

Main category: cs.LG

TL;DR: The paper introduces a category theory-based framework to model deep research agents and proposes a mechanism-aware benchmark with 296 questions to stress-test agents along four structural axes, revealing persistent low performance (19.9% SOTA) and exposing limitations in multi-hop structural synthesis.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of deep research agents relies on ad hoc empirical benchmarks that fail to rigorously model agent behavior or adequately stress-test long-horizon synthesis and ambiguity resolution. There's a need for formal theoretical grounding and systematic evaluation.

Method: Formalizes deep research agent behavior using category theory, modeling workflow as composition of structure-preserving maps (functors). Introduces a mechanism-aware benchmark with 296 questions designed to stress-test agents along four interpretable axes: sequential connectivity chains, V-structure pullback intersections, topological ordering of retrieved substructures, and ontological falsification via Yoneda Probe.

Result: Evaluation of 11 leading models shows persistently low baseline performance with state-of-the-art achieving only 19.9% average accuracy. Reveals stark dichotomy: agents succeed at dynamic topological re-ordering and ontological verification but collapse on multi-hop structural synthesis. Massive performance variance indicates reliance on brittle heuristics rather than systemic understanding.

Conclusion: While top-tier autonomous agents can organically unify search and reasoning, achieving generalized mastery over complex structural information remains a formidable open challenge. The work demonstrates the need for more fundamental structural understanding beyond current heuristic approaches.
...
### [235] [Hessian-informed machine learning interatomic potential towards bridging theory and experiments](https://arxiv.org/abs/2603.25373)
...
<details>
  <summary>Details</summary>
Motivation: Local curvature of potential energy surfaces is critical for predicting experimental observables but remains computationally prohibitive for complex systems, creating a need for efficient methods to capture Hessian information in machine learning potentials.

Method: Developed Hi-MLIP (Hessian-informed Machine Learning Interatomic Potential) with HINT (Hessian INformed Training) protocol that includes Hessian pre-training, configuration sampling, curriculum learning, and stochastic projection Hessian loss to reduce Hessian label requirements by 2-4 orders of magnitude.

Result: Hi-MLIP significantly improves transition-state search accuracy, achieves chemical accuracy for Gibbs free-energy predictions in data-scarce regimes, accurately treats strongly anharmonic hydrides, reproduces phonon renormalization and superconducting critical temperatures in agreement with experiments while bypassing computational bottlenecks.

Conclusion: The framework establishes a practical route to enhance curvature awareness of machine learning interatomic potentials, bridging simulation and experimental observables across diverse systems by efficiently capturing Hessian information.
...
### [236] [GlowQ: Group-Shared LOw-Rank Approximation for Quantized LLMs](https://arxiv.org/abs/2603.25385)
*Selim An,Il hong Suh,Yeseong Kim*

Main category: cs.LG

TL;DR: GlowQ introduces group-shared low-rank approximation for quantized LLMs, caching a single shared right factor per input-sharing group to reduce latency and memory overhead while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing low-rank correction methods for quantized LLMs (like LQER, QERA, ASER) restore all layers and insert error-correction modules into every decoder block, which increases latency and memory overhead significantly.

Method: GlowQ uses group-shared low-rank approximation that caches a single shared right factor per input-sharing group and restores only groups/layers with highest accuracy benefit. It computes high-precision projection once per group and reuses it across modules. GlowQ-S is a selective variant that applies cached shared module only where it provides largest benefit.
...
Conclusion: GlowQ provides an efficient group-shared low-rank approximation approach that significantly reduces latency and memory overhead for quantized LLMs while maintaining or improving accuracy, offering a practical solution for deployment of low-bit quantized models.
...
### [237] [Maximum Entropy Behavior Exploration for Sim2Real Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2603.25464)
*Jiajun Hu,Nuria Armengol Urpi,Jin Cheng,Stelian Coros*

Main category: cs.LG

TL;DR: FB-MEBE is an online zero-shot RL algorithm for quadrupedal control that combines unsupervised behavior exploration with a regularization critic to generate diverse, natural policies deployable directly to hardware.


<details>
  <summary>Details</summary>
Motivation: Zero-shot RL requires diverse pretraining datasets, but undirected exploration yields low-diversity data leading to poor downstream performance and policies unsuitable for hardware deployment without further finetuning.
...
Conclusion: The proposed FB-MEBE algorithm effectively addresses the exploration challenge in online zero-shot RL for quadrupedal control, enabling direct hardware deployment of learned policies through behavior diversity maximization and regularization.
...
### [238] [Not a fragment, but the whole: Map-based evaluation of data-driven Fire Danger Index models](https://arxiv.org/abs/2603.25469)
*Shahbaz Alvi,Italo Epicoco,Jose Maria Costa Saura*

Main category: cs.LG

TL;DR: Proposes novel evaluation method for wildfire forecasting models that aligns with operational decision-making, focusing on both fire prediction accuracy and false alarm reduction, and demonstrates ensemble ML improves both metrics.
...
Method: Revisits daily FDI model evaluation paradigm and proposes novel evaluation method aligned with real-world decision-making; systematically assesses performance in fire prediction accuracy and false positives; uses ensemble of ML models.

Result: Ensemble of ML models improves both fire identification and reduces false positives compared to individual models.

Conclusion: Proposed evaluation framework provides more operationally relevant assessment of wildfire forecasting models, and ensemble ML approaches offer superior performance in both fire detection and false alarm reduction for practical fire management applications.
...
### [239] [Causal-INSIGHT: Probing Temporal Models to Extract Causal Structure](https://arxiv.org/abs/2603.25473)
*Benjamin Redden,Hui Wang,Shuyan Li*

Main category: cs.LG

TL;DR: Causal-INSIGHT is a model-agnostic framework that extracts directed temporal influence structures from trained predictors using intervention-inspired input clamping, with Qbic for graph selection.
...
Method: Causal-INSIGHT uses systematic, intervention-inspired input clamping at inference time on pre-trained predictors, constructs directed temporal influence signals from model responses, and employs Qbic (a sparsity-aware graph selection criterion) to balance predictive fidelity and structural complexity without ground-truth labels.
...
Conclusion: Causal-INSIGHT provides an effective post-hoc interpretation framework for extracting model-implied directed temporal influence structures from trained predictors, offering insights into how models leverage temporal dependencies for prediction.
...
### [240] [How Class Ontology and Data Scale Affect Audio Transfer Learning](https://arxiv.org/abs/2603.25476)
*Manuel Milling,Andreas Triantafyllopoulos,Alexander Gebhard,Simon Rampp,Björn W. Schuller*

Main category: cs.LG

TL;DR: Audio-to-audio transfer learning study shows that while more pre-training data (samples and classes) helps, similarity between pre-training and downstream tasks has greater impact on transfer performance.


<details>
  <summary>Details</summary>
Motivation: Despite widespread use of transfer learning in deep learning, there are still open questions about its inner workings and understanding when/how well it works, particularly in audio domains.

Method: Pre-trained various models on ontology-based subsets of AudioSet, then fine-tuned them on three computer audition tasks: acoustic scene recognition, bird activity recognition, and speech command recognition.

Result: Increasing pre-training samples and classes has positive impact, but similarity between pre-training and downstream tasks generally surpasses this effect, enabling models to learn comparable features.

Conclusion: Task similarity is a more critical factor than sheer pre-training data volume for successful audio-to-audio transfer learning, providing insights into when and how transfer learning works effectively.
...
### [241] [Interpretable PM2.5 Forecasting for Urban Air Quality: A Comparative Study of Operational Time-Series Models](https://arxiv.org/abs/2603.25495)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan,Sultan Alamri*

Main category: cs.LG

TL;DR: Lightweight interpretable models (SARIMAX, Facebook Prophet, NeuralProphet) with leakage-aware workflow achieve competitive PM2.5 forecasting in Beijing, with Facebook Prophet showing best performance under walk-forward refitting and SARIMAX with residual correction performing best in frozen regime.


<details>
  <summary>Details</summary>
Motivation: Current air-quality forecasting frameworks often rely on complex, data-intensive, and computationally demanding models. This study investigates whether lightweight and interpretable forecasting approaches can provide competitive performance for hourly PM2.5 prediction.
...
Conclusion: Lightweight additive forecasting strategies can remain highly competitive for urban air-quality prediction, offering a practical balance between accuracy, interpretability, and computational efficiency, with Facebook Prophet and SARIMAX showing particular promise in different deployment scenarios.
...
### [242] [Missing-Aware Multimodal Fusion for Unified Microservice Incident Management](https://arxiv.org/abs/2603.25538)
*Wenzhuo Qian,Hailiang Zhao,Ziqi Wang,Zhipeng Gao,Jiayi Chen,Zhiwei Ling,Shuiguang Deng*

Main category: cs.LG

TL;DR: ARMOR: A robust self-supervised framework for microservice incident management that handles missing modalities through asymmetric encoders and missing-aware gated fusion, achieving SOTA performance even with severe data loss.


<details>
  <summary>Details</summary>
Motivation: Existing unified incident management frameworks assume perfect multimodal data completeness, but in practice, network fluctuations and agent failures frequently cause missing modalities. Current approaches using static placeholders introduce imputation noise that masks anomalies and degrades diagnostic performance.
...
Conclusion: ARMOR provides a practical solution for real-world microservice incident management by addressing the critical challenge of missing modalities through robust self-supervised learning, enabling reliable diagnostics even with incomplete data.
...
### [243] [An Integrative Genome-Scale Metabolic Modeling and Machine Learning Framework for Predicting and Optimizing Biofuel-Relevant Biomass Production in Saccharomyces cerevisiae](https://arxiv.org/abs/2603.25561)
...
Result: Random Forest and XGBoost achieved R² of 0.99989 and 0.9990 respectively; variational autoencoder revealed four distinct metabolic clusters; SHAP identified glycolysis, TCA cycle, and lipid biosynthesis as key determinants; in silico overexpression achieved 0.979 gDW/hr biomass flux; Bayesian optimization produced 12-fold increase (0.0858 to 1.041 gDW/hr); GAN proposed stoichiometrically feasible novel flux configurations.
...
### [244] [Revisiting On-Policy Distillation: Empirical Failure Modes and Simple Fixes](https://arxiv.org/abs/2603.25562)
*Yuqian Fu,Haohuan Huang,Kaiwen Jiang,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: The paper analyzes on-policy distillation (OPD) for LLM post-training, identifies failure modes of sampled-token OPD in long-horizon settings, and proposes teacher top-K local support matching with truncated reverse-KL to improve stability and performance.


<details>
  <summary>Details</summary>
Motivation: On-policy distillation is appealing for LLM post-training because it evaluates teacher feedback on student-generated rollouts rather than fixed teacher traces, but sampled-token OPD becomes fragile in long-horizon settings due to unreliable teacher guidance as rollouts drift from teacher-common prefixes.

Method: The paper revisits OPD from estimator and implementation perspectives, identifies three failure modes of sampled-token OPD, and proposes teacher top-K local support matching implemented as truncated reverse-KL with top-p rollout sampling and special-token masking.

Result: The proposed objective yields more stable optimization and better downstream performance than sampled-token OPD across single-task math reasoning and multi-task agentic-plus-math training, demonstrating improved reliability in long-horizon settings.

Conclusion: Token-level OPD has bias-variance tradeoffs, and the proposed teacher top-K local support matching with truncated reverse-KL addresses the identified failure modes, providing more robust on-policy distillation for LLM post-training in long-horizon scenarios.
...
### [245] [Spatiotemporal System Forecasting with Irregular Time Steps via Masked Autoencoder](https://arxiv.org/abs/2603.25597)
*Kewei Zhu,Yanze Xin,Jinwei Hu,Xiaoyuan Cheng,Yiming Yang,Sibo Cheng*

Main category: cs.LG

TL;DR: Proposes a Physics-Spatiotemporal Masked Autoencoder for predicting high-dimensional dynamical systems with irregular time steps, achieving improved accuracy and efficiency without data imputation.
...
Conclusion: The model shows potential for capturing complex spatiotemporal patterns without requiring domain-specific knowledge, with applications in climate modelling, fluid dynamics, ocean forecasting, environmental monitoring, and scientific computing.
...
### [246] [Anchored-Branched Steady-state WInd Flow Transformer (AB-SWIFT): a metamodel for 3D atmospheric flow in urban environments](https://arxiv.org/abs/2603.25635)
...
TL;DR: AB-SWIFT is a transformer-based model with branched architecture for urban air flow prediction, outperforming existing methods on atmospheric simulations with varying urban geometries and stratifications.


<details>
  <summary>Details</summary>
Motivation: Deep learning surrogate models for urban air flow struggle with high variations in urban geometry and large mesh sizes, while CFD computations remain costly.
...
Conclusion: The proposed AB-SWIFT model effectively addresses challenges in urban air flow modeling, providing an accurate deep learning alternative to costly CFD computations.
...
### [247] [Uncertainty-Guided Label Rebalancing for CPS Safety Monitoring](https://arxiv.org/abs/2603.25670)
*John Ayotunde,Qinghua Xu,Guancheng Wang,Lionel C. Briand*

Main category: cs.LG

TL;DR: U-Balance is a supervised approach that leverages behavioral uncertainty to rebalance imbalanced datasets for CPS safety monitoring, using uncertainty-guided label rebalancing to enrich the minority class without synthetic data generation.


<details>
  <summary>Details</summary>
Motivation: Safety monitoring in Cyber-Physical Systems suffers from extreme class imbalance (rare unsafe events), where standard rebalancing techniques perform poorly on time-series telemetry data. Behavioral uncertainty in CPS operations is correlated with safety outcomes but unexplored for safety monitoring.

Method: U-Balance trains a GatedMLP-based uncertainty predictor that summarizes telemetry windows into distributional kinematic features and outputs uncertainty scores. It then applies uncertainty-guided label rebalancing (uLNR) that probabilistically relabels safe-labeled windows with unusually high uncertainty as unsafe, enriching the minority class without synthetic data. Finally, a safety predictor is trained on the rebalanced dataset.
...
Conclusion: U-Balance effectively addresses class imbalance in CPS safety monitoring by leveraging behavioral uncertainty through uncertainty-guided label rebalancing, significantly improving safety prediction performance without generating unrealistic synthetic samples or overfitting on the minority class.
...
### [248] [Longitudinal Digital Phenotyping for Early Cognitive-Motor Screening](https://arxiv.org/abs/2603.25673)
*Diego Jimenez-Oviedo,Ruben Vera-Rodriguez,Ruben Tolosana,Juan Carlos Ruiz-Garcia,Jaime Herreros-Rodriguez*

Main category: cs.LG

TL;DR: AI-driven longitudinal framework uses tablet interaction data to identify three distinct cognitive-motor development profiles in children (low, medium, high performance) with high stability in low-performance cluster, enabling early screening and intervention.


<details>
  <summary>Details</summary>
Motivation: Traditional cognitive-motor development assessments are subjective and static, lacking continuous monitoring. Digital biomarkers from tablet interactions offer objective, continuous data for early detection of atypical development to enable timely interventions.

Method: Longitudinal framework analyzing tablet-based interactions from children aged 18 months to 8 years across six cognitive-motor tasks. Applied dimensionality reduction (t-SNE) and unsupervised clustering (K-Means++) to identify developmental phenotypes and track individual transitions between profiles over time.

Result: Identified three distinct developmental profiles: low, medium, and high performance. Longitudinal tracking revealed >90% retention in low-performance cluster during early years, indicating persistence of early deficits. Higher-performance clusters showed greater variability, potentially reflecting engagement factors.

Conclusion: Validates unsupervised learning on touchscreen data for uncovering heterogeneous developmental paths. Identified profiles serve as scalable, data-driven proxies for cognitive growth, providing foundation for early screening tools and personalized pediatric interventions.
...
### [249] [On Neural Scaling Laws for Weather Emulation through Continual Training](https://arxiv.org/abs/2603.25687)
*Shashank Subramanian,Alexander Kiefer,Arnur Nigmetov,Amir Gholami,Dmitriy Morozov,Michael W. Mahoney*

Main category: cs.LG

TL;DR: Neural scaling laws for weather forecasting models show predictable performance trends with compute-optimal training regimes using Swin Transformers and continual training with cooldowns.


<details>
  <summary>Details</summary>
Motivation: To study neural scaling laws in Scientific Machine Learning, specifically for weather forecasting, to understand performance prediction and efficient resource allocation for foundation models in this domain.

Method: Adopted minimal Swin Transformer architecture with continual training using constant learning rates and periodic cooldowns; systematically explored model and dataset sizes under various compute budgets to construct IsoFLOP curves.

Result: Models follow predictable scaling trends, outperform standard cosine learning rate schedules, and cooldown phases improve downstream performance for multi-step rollouts and sharper predictions; identified compute-optimal training regimes.

Conclusion: Neural scaling serves as an important diagnostic for efficient resource allocation in weather forecasting models, with extrapolated trends highlighting potential performance limits at larger scales.
...
TL;DR: The paper presents a unified framework treating deterministic memory access as a special case of stochastic sampling, revealing how increasing stochastic demands reduce data-access efficiency and can drive systems into entropy-limited operation, with analysis of emerging probabilistic compute-in-memory architectures for trustworthy AI.
...
Conclusion: The paper outlines pathways toward scalable hardware for trustworthy AI by analyzing probabilistic compute-in-memory approaches that address the memory system challenges created by the shift toward probabilistic computation in AI workloads.
...
TL;DR: The paper investigates transferring knowledge from a non-neural random forest classifier to a neural network student, enabling joint optimization and unified inference across ML tasks.


<details>
  <summary>Details</summary>
Motivation: To extend student-teacher learning beyond neural networks by transferring knowledge from non-neural ML pipelines (specifically random forests) to neural networks, allowing for joint optimization of pipeline components and a single unified inference engine for multiple ML tasks.

Method: Extends student-teacher learning by using random forest classifiers as teachers to train neural network students. Experiments with various NN topologies on 100 OpenML tasks where random forests performed well. Investigates using random forests for selecting optimal NN hyper-parameters.

Result: For majority of tasks, student neural networks can successfully mimic teacher random forests when appropriate NN hyper-parameters are selected. Random forests can also be effectively used for hyper-parameter selection.

Conclusion: Knowledge transfer from non-neural ML models to neural networks is feasible, enabling unified optimization and inference. Proper hyper-parameter selection is crucial for successful knowledge distillation from random forests to neural networks.
...
### [252] [Enhancing Online Support Group Formation Using Topic Modeling Techniques](https://arxiv.org/abs/2603.24765)
*Pronob Kumar Barman,Tera L. Reynolds,James Foulds*

Main category: cs.IR

TL;DR: Two novel ML models (gDMR and gSTM) automate personalized support group formation in online health communities by integrating text, demographics, and network data, outperforming baselines and enabling scalable peer support.


<details>
  <summary>Details</summary>
Motivation: Traditional support group formation in online health communities faces scalability issues, static categorization, and insufficient personalization, limiting effective peer support for diverse health concerns.

Method: Proposed gDMR (Group-specific Dirichlet Multinomial Regression) and gSTM (Group-specific Structured Topic Model) integrate user-generated text, demographic profiles, and interaction data via node embeddings from user networks to automate personalized, semantically coherent group formation.

Result: Both models substantially outperform baseline methods (LDA, DMR, STM) in predictive accuracy (held-out log likelihood), semantic coherence (UMass metric), and internal group consistency on MedHelp.org dataset with 2M+ posts. gDMR leverages relational patterns for practical implementation, while gSTM emphasizes sparsity for distinct thematic groups.

Conclusion: The models provide scalable solutions that reduce manual curation, enhance peer interactions in OHCs, and have implications for patient engagement, community resilience, and health outcomes, with qualitative validation showing alignment with manually coded health themes.
...
### [253] [DIET: Learning to Distill Dataset Continually for Recommender Systems](https://arxiv.org/abs/2603.24958)
*Jiaqing Zhang,Hao Wang,Mingjia Yin,Bo Chen,Qinglin Jia,Rui Zhou,Ruiming Tang,ChaoYi Ma,Enhong Chen*

Main category: cs.IR

TL;DR: DIET proposes streaming dataset distillation for recommender systems, maintaining a compact evolving distilled dataset that preserves training-critical signals from streaming data, enabling efficient model iteration with 60× cost reduction.


<details>
  <summary>Details</summary>
Motivation: Full retraining of deep recommender models on massive streaming behavioral logs is prohibitively expensive, slowing down model development. There's a need for data-efficient approaches that approximate full-data training behavior without repeatedly processing the entire evolving data stream.

Method: DIET formulates streaming dataset distillation for recommender systems, maintaining a compact distilled dataset that evolves alongside streaming data. It models distilled data as evolving training memory updated stage-wise, with principled initialization from influential samples and selective updates guided by influence-aware memory addressing within a bi-level optimization framework.
...
Conclusion: Streaming dataset distillation serves as a scalable and reusable data foundation for recommender system development, enabling efficient model iteration while maintaining training fidelity through evolving distilled datasets.
...
### [254] [Unbiased Multimodal Reranking for Long-Tail Short-Video Search](https://arxiv.org/abs/2603.24975)
*Wenyi Xu,Feiran Zhu,Songyang Li,Renzhe Zhou,Chao Zhang,Chenglei Dai,Yuren Mao,Yunjun Gao,Yi Zhang*

Main category: cs.IR

TL;DR: LLM-driven multimodal reranking framework addresses Matthew effect in short-video search by using LLM world knowledge to assess content quality without relying on sparse user behavior data, improving long-tail query performance.


<details>
  <summary>Details</summary>
Motivation: Short-video search suffers from severe Matthew effect on long-tail queries where sparse user behavior data causes models to amplify low-quality content (clickbait, shallow content). LLMs offer a new paradigm with inherent world knowledge to assess content quality independent of sparse user interactions.

Method: Two-stage training process: 1) Use multimodal evidence to construct high-quality annotations for supervised fine-tuning; 2) Incorporate pairwise preference optimization to help model learn partial orderings among candidates. At inference, experience scores promote high-quality but underexposed videos in reranking and guide page-level optimization through reinforcement learning.

Result: Consistent improvements over strong baselines in offline metrics (AUC, NDCG@K, human preference judgement). Online A/B test covering 15% traffic demonstrates gains in both user experience and consumption metrics.

Conclusion: LLM-driven multimodal reranking framework effectively addresses the Matthew effect in long-tail video search scenarios, confirming practical value through improved user experience and consumption metrics.
...
### [255] [Sparton: Fast and Memory-Efficient Triton Kernel for Learned Sparse Retrieval](https://arxiv.org/abs/2603.25011)
*Thong Nguyen,Cosimo Rulli,Franco Maria Nardini,Rossano Venturini,Andrew Yates*

Main category: cs.IR

TL;DR: Sparton is a fused GPU kernel for Learned Sparse Retrieval models that eliminates the memory bottleneck of LM heads by performing early online reduction, achieving significant speedups and memory savings.


<details>
  <summary>Details</summary>
Motivation: LSR models like Splade suffer from massive memory bottlenecks due to materializing large vocabulary-sized logit matrices (30k-250k tokens), limiting model scaling and creating I/O overhead that throttles throughput.
...
Conclusion: Sparton effectively addresses the memory bottleneck in LSR models through kernel fusion and early reduction, enabling more efficient scaling of vocabulary sizes without sacrificing retrieval effectiveness.
...
### [256] [Hyena Operator for Fast Sequential Recommendation](https://arxiv.org/abs/2603.25027)
*Jiahao Liu,Lin Li,Zhiyuan Li,Kaixi Hu,Kaize Shi,Jingling Yuan*

Main category: cs.IR

TL;DR: HyenaRec is a novel sequential recommender that replaces quadratic attention with polynomial-based kernels and gated convolutions, achieving linear scaling with sequence length while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Attention-based sequential recommendation models suffer from quadratic complexity, making long user histories prohibitively expensive. While sub-quadratic operators like Hyena exist for language modeling, they face challenges in recommendation due to limited representation capacity on sparse, long user sequences.

Method: HyenaRec integrates polynomial-based kernel parameterization with gated convolutions. It designs convolutional kernels using Legendre orthogonal polynomials for smooth long-term temporal dependency modeling, complemented by a gating mechanism to capture fine-grained short-term behavioral bursts, creating a hybrid architecture.

Result: HyenaRec consistently outperforms Attention-, Recurrent-, and other baselines in ranking accuracy across multiple real-world datasets. It trains significantly faster (up to 6x speedup) with particularly pronounced advantages on long-sequence scenarios while maintaining accuracy.

Conclusion: Polynomial-based kernel parameterization serves as a principled and scalable alternative to attention for sequential recommendation, offering linear scaling with sequence length without sacrificing accuracy.
...
### [257] [AuthorityBench: Benchmarking LLM Authority Perception for Reliable Retrieval-Augmented Generation](https://arxiv.org/abs/2603.25092)
...
<details>
  <summary>Details</summary>
Motivation: RAG systems are vulnerable to low-authority sources that can propagate misinformation, but it's unclear whether LLMs can perceive information authority as a distinct capability beyond semantic understanding.

Method: Introduced AuthorityBench with three datasets: DomainAuth (10K web domains with PageRank authority), EntityAuth (22K entities with popularity-based authority), and RAGAuth (120 queries with documents of varying authority). Evaluated five LLMs using three judging methods (PointJudge, PairJudge, ListJudge) across multiple output formats.

Result: ListJudge and PairJudge with PointScore output achieve strongest correlation with ground-truth authority; ListJudge offers optimal cost-effectiveness. Incorporating webpage text consistently degrades judgment performance, indicating authority is distinct from textual style. Authority-guided filtering significantly improves RAG answer accuracy.

Conclusion: LLMs can perceive information authority, and this capability is practically important for reliable knowledge retrieval in RAG systems, with AuthorityBench providing a comprehensive evaluation framework.
...
### [258] [MCLMR: A Model-Agnostic Causal Learning Framework for Multi-Behavior Recommendation](https://arxiv.org/abs/2603.25126)
...
Conclusion: MCLMR provides a principled causal learning framework that addresses fundamental challenges in multi-behavior recommendation through causal intervention, adaptive aggregation, and bias-aware representation alignment, offering a model-agnostic solution that can be integrated into various MBR architectures.
...
### [259] [ColBERT-Att: Late-Interaction Meets Attention for Enhanced Retrieval](https://arxiv.org/abs/2603.25248)
*Raj Nath Patel,Sourav Dutta*

Main category: cs.IR

TL;DR: ColBERT-Att enhances the ColBERT late interaction retrieval model by incorporating attention weights to better capture term importance, improving recall accuracy across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current late interaction models like ColBERT fail to leverage attention weights that could capture the importance of similarities between query and document terms, potentially leading to better relevance understanding and improved retrieval performance.

Method: Proposes ColBERT-Att, which explicitly integrates attention mechanism into the late interaction framework, using attention weights to weight the similarities between query and document terms.

Result: Empirical evaluation shows improvements in recall accuracy on MS-MARCO, as well as across a wide range of BEIR and LoTTE benchmark datasets.

Conclusion: Integrating attention mechanisms into late interaction frameworks enhances retrieval performance by better capturing term importance and relevance relationships between queries and documents.
...
### [260] [Supercharging Federated Intelligence Retrieval](https://arxiv.org/abs/2603.25374)
...
TL;DR: Federated RAG system using Flower framework with confidential compute environment for secure knowledge retrieval across private data silos, featuring cascading inference with third-party models.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG assumes centralized document access, which fails when knowledge is distributed across private data silos that cannot share raw documents due to privacy/security constraints.
...
Result: Enables confidential remote LLM inference even with honest-but-curious or compromised servers, maintaining privacy while leveraging distributed knowledge sources.

Conclusion: Proposed system addresses privacy-preserving RAG across distributed data silos through federated learning architecture with confidential computing and secure cascading inference techniques.
...
### [261] [Pseudo Label NCF for Sparse OHC Recommendation: Dual Representation Learning and the Separability Accuracy Trade off](https://arxiv.org/abs/2603.24750)
*Pronob Kumar Barman,Tera L. Reynolds. James Foulds*

Main category: cs.IR

TL;DR: Pseudo Label NCF improves recommendation for online health communities under extreme interaction sparsity by using survey-derived pseudo labels to enhance Neural Collaborative Filtering architectures, achieving better ranking performance and more interpretable embedding spaces.


<details>
  <summary>Details</summary>
Motivation: Online health communities face discovery challenges due to extreme interaction sparsity where users have minimal prior interactions for personalization. Traditional recommendation systems struggle in cold-start conditions with limited user-group interactions.

Method: Extends Neural Collaborative Filtering (NCF) architectures (Matrix Factorization, MLP, NeuMF) with auxiliary pseudo label objective derived from survey group feature alignment. Uses 16-dimensional intake vectors for users and structured feature profiles for groups, mapping cosine similarity to [0,1] pseudo labels. Learns dual embedding spaces: main embeddings for ranking and pseudo label embeddings for semantic alignment.

Result: All pseudo label variants improve ranking performance: MLP improves HR@5 from 2.65% to 5.30%, NeuMF from 4.46% to 5.18%, and MF from 4.58% to 5.42%. Pseudo label embeddings show higher cosine silhouette scores (MF: 0.0394→0.0684, NeuMF: 0.0263→0.0653). Negative correlation observed between embedding separability and ranking accuracy, indicating interpretability-performance trade-off.

Conclusion: Survey-derived pseudo labels effectively improve recommendation under extreme sparsity while producing interpretable task-specific embedding spaces. The approach addresses cold-start problems in online health communities and demonstrates the value of auxiliary semantic alignment objectives in recommendation systems.
