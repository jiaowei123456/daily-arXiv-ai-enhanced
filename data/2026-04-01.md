...
TL;DR: DF-ACBlurGAN is a structure-aware conditional GAN for generating periodic patterns with controlled repetition, spacing, and boundary coherence, applied to biomaterial surface design.


<details>
  <summary>Details</summary>
Motivation: Existing generative models struggle with internally repeated and periodic structures because they optimize for local texture statistics and semantic realism rather than global structural consistency. This is particularly problematic for applications like microtopographical biomaterial surfaces that require strict control over repetition scale, spacing, and boundary coherence.

Method: DF-ACBlurGAN integrates frequency-domain repetition scale estimation, scale-adaptive Gaussian blurring, and unit-cell reconstruction to balance sharp local features with stable global periodicity. The model is conditioned on biological response labels and trained under weak supervision with class imbalance.

Result: Evaluation across multiple biomaterial datasets shows improved repetition consistency and controllable structural variation compared to conventional generative approaches. The model successfully synthesizes designs aligned with target functional outcomes.

Conclusion: The proposed structure-aware conditional GAN framework effectively addresses the challenge of generating periodic patterns with controlled repetition characteristics, demonstrating practical value for biomaterial design applications requiring precise structural control.
...
TL;DR: OccSim is the first occupancy world model-driven 3D simulator that generates continuous 3D occupancy maps over 4km without requiring pre-recorded logs or HD maps, enabling scalable autonomous driving simulation.


<details>
  <summary>Details</summary>
Motivation: Current data-driven autonomous driving simulation is constrained by heavy reliance on pre-recorded driving logs or HD maps, which severely limits scalability and restricts open-ended generation capabilities to finite collected datasets.

Method: OccSim uses two modules: 1) W-DiT based static occupancy world model that handles ultra-long-horizon generation of static environments by explicitly introducing known rigid transformations in architecture design, and 2) Layout Generator that populates dynamic foreground with reactive agents based on synthesized road topology.

Result: OccSim can generate over 3,000 continuous frames (4+ km), representing >80x improvement in stable generation length over previous occupancy world models. Data from OccSim pre-trains 4D semantic occupancy forecasting models to achieve up to 67% zero-shot performance on unseen data (11% better than asset-based simulators). Scaling OccSim dataset to 5x size increases zero-shot performance to ~74% (22.1% improvement over asset-based simulators).

Conclusion: OccSim breaks the scalability bottleneck of autonomous driving simulation by eliminating dependency on pre-recorded data, enabling massive, diverse simulation streams that significantly improve downstream model performance through scalable synthetic data generation.
...
TL;DR: Fisheye3R adapts multi-view 3D reconstruction foundation models to handle fisheye camera inputs without performance loss on perspective images, using novel learning schemes that work with limited fisheye training data.


<details>
  <summary>Details</summary>
Motivation: Existing foundation models for multi-view 3D reconstruction degrade on fisheye images due to non-linear projection models, and there's insufficient fisheye training data with ground truth to retrain models effectively.
...
Result: Extensive experiments across three foundation models (VGGT, π³, and MapAnything) show consistent improvements in camera pose, depth, point map, and field-of-view estimation on fisheye images without regression on perspective images.

Conclusion: Fisheye3R successfully extends multi-view 3D reconstruction foundation models to handle fisheye inputs despite limited training data, maintaining perspective performance while improving fisheye accuracy through novel adaptation techniques.
...
TL;DR: A signed Graph Neural Network with sparse edge mask and class-specific saliency decodes category-specific functional connectivity states from 7T fMRI data, revealing biologically meaningful subnetworks along visual pathways.


<details>
  <summary>Details</summary>
Motivation: To understand how large-scale brain networks represent visual categories and bridge the gap between perception and cortical organization by extending voxel-level category selectivity to connectivity-based representations.
...
Conclusion: The framework successfully bridges machine learning and neuroscience by providing a connectivity-based representation of visual processing that extends beyond traditional voxel-level category selectivity.
...
TL;DR: Stepper is a unified framework for text-driven immersive 3D scene synthesis that uses stepwise panoramic expansion to overcome limitations in visual fidelity and explorability, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for text-driven 3D scene synthesis face a trade-off between visual fidelity and explorability: autoregressive expansion suffers from context drift, while panoramic video generation is limited to low resolution.

Method: Stepper uses stepwise panoramic scene expansion with a novel multi-view 360° diffusion model for consistent, high-resolution expansion, coupled with a geometry reconstruction pipeline that enforces geometric coherence. It's trained on a new large-scale, multi-view panorama dataset.

Result: Stepper achieves state-of-the-art fidelity and structural consistency, outperforming prior approaches in immersive 3D scene synthesis from text.

Conclusion: Stepper sets a new standard for immersive scene generation by circumventing the limitations of existing approaches through its unified framework for text-driven 3D scene synthesis.
...
TL;DR: Hybrid quantum-classical models for aluminum TIG welding defect classification show competitive performance compared to classical CNN, demonstrating potential for near-term industrial quality control applications.


<details>
  <summary>Details</summary>
Motivation: Advancing automated quality control in industrial settings through hybrid quantum-classical machine learning approaches for defect detection in aluminum TIG welding.

Method: Two hybrid approaches: 1) Quantum kernel method using CNN-extracted features encoded via parameterized quantum feature map, with quantum kernel matrix computed for SVM optimization solved by VQLS; 2) Angle encoding of features in variational quantum circuit with classical optimizer training. Both compared against classical CNN model on binary and multiclass classification tasks.

Result: CNN demonstrates robust performance, but hybrid quantum-classical models perform competitively, highlighting their potential for real-world industrial applications.

Conclusion: Hybrid quantum-classical approaches show promise for near-term practical applications in industrial defect detection and quality assurance, though classical CNN remains robust.
...
TL;DR: Feed-forward human performance capture method that renders novel views from monocular RGB video by maintaining and progressively updating a canonical space that accumulates appearance information over time, using probabilistic regression to resolve conflicts between past and current observations.


<details>
  <summary>Details</summary>
Motivation: The key challenge in monocular human performance capture is the lack of sufficient observations, especially for unseen regions. The authors aim to leverage the fact that as subjects move continuously over time, more body parts become observable, allowing accumulation of appearance information.

Method: Maintains a canonical space that is progressively updated with each incoming frame to accumulate appearance information over time. Formulates rendering as probabilistic regression to effectively utilize historical context while respecting current deformation, resolving conflicts between past and current observations.

Result: Produces sharper reconstructions than deterministic regression approaches and enables plausible synthesis even in regions with no prior observations. Demonstrated effectiveness on in-domain (4D-Dress) and out-of-distribution (MVHumanNet) datasets.

Conclusion: The proposed feed-forward method successfully addresses the limited observation problem in monocular human performance capture by maintaining a progressively updated canonical space and using probabilistic regression, achieving high-quality novel view synthesis even for unseen regions.
...
TL;DR: MEDiC combines pixel reconstruction, CLIP token distillation, and global alignment in a multi-objective MIM framework, achieving 73.9% kNN accuracy on ImageNet-1K with systematic design space exploration.


<details>
  <summary>Details</summary>
Motivation: Current masked image modeling methods operate in either raw pixel space or latent feature space, but not both. The authors aim to combine these complementary approaches in a single framework to leverage the strengths of both reconstruction and distillation objectives.

Method: MEDiC framework with three complementary objectives: 1) patch-level token distillation from frozen CLIP encoder, 2) global CLS alignment, and 3) pixel reconstruction via lightweight decoder. Systematic investigation includes hierarchical clustering with relative position bias for evolved masking, and analysis of loss weight sensitivity.

Result: Achieves 73.9% kNN and 85.1% fine-tuning accuracy with ViT-Base at 300 epochs on ImageNet-1K. Key findings: all three objectives provide complementary information; evolved masking doesn't outperform simple block masking in teacher-guided setting; optimal loss weights are extremely fragile with small perturbations causing up to 17% accuracy drops.

Conclusion: Combining pixel reconstruction and feature distillation in a multi-objective framework is effective for masked image modeling. The teacher's semantic awareness reduces the benefit of sophisticated masking strategies, and careful loss weight tuning is critical for optimal performance.
...
TL;DR: UltraG-Ray introduces a novel ultrasound scene representation using learnable 3D Gaussian fields coupled with physics-based B-mode synthesis, achieving improved realism in novel view synthesis for ultrasound imaging.


<details>
  <summary>Details</summary>
Motivation: Current novel view synthesis methods in ultrasound struggle with complex tissue and view-dependent acoustic effects, leaving a substantial gap between simulation and reality. Physics-based approaches that incorporate ultrasound image formation processes are needed to address these limitations.

Method: The authors propose UltraG-Ray, which uses a learnable 3D Gaussian field representation that explicitly encodes ultrasound-specific parameters (attenuation, reflection) and couples this with an efficient physics-based module for B-mode synthesis. The method employs a novel ray casting scheme that naturally captures view-dependent attenuation effects.

Result: The method demonstrates consistent gains in image quality metrics compared to state-of-the-art approaches, with up to 15% increase in MS-SSIM, showing clear improvement in the realism of synthesized ultrasound images.

Conclusion: UltraG-Ray successfully bridges the gap between simulation and reality in ultrasound novel view synthesis by combining learnable 3D Gaussian representations with physics-based rendering, enabling physically informed B-mode image generation with increased realism.
...
TL;DR: MMFace-DiT introduces a unified dual-stream diffusion transformer for multimodal face generation that synergistically fuses spatial (mask/sketch) and semantic (text) modalities through parallel processing and shared attention, achieving 40% improvement over SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal face generation models use ad hoc designs that extend pre-trained text-to-image pipelines with auxiliary control modules or stitch separate uni-modal networks, leading to architectural constraints, parameter duplication, and failure under conflicting modalities or mismatched latent spaces, limiting synergistic fusion across semantic and spatial domains.

Method: MMFace-DiT uses a unified dual-stream diffusion transformer with dual-stream transformer blocks that process spatial (mask/sketch) and semantic (text) tokens in parallel, deeply fusing them through a shared Rotary Position-Embedded (RoPE) Attention mechanism to prevent modal dominance and ensure strong adherence to both text and structural priors. A novel Modality Embedder enables a single cohesive model to dynamically adapt to varying spatial conditions without retraining.

Result: MMFace-DiT achieves a 40% improvement in visual fidelity and prompt alignment over six state-of-the-art multimodal face generation models, establishing a flexible new paradigm for end-to-end controllable generative modeling.

Conclusion: MMFace-DiT introduces a unified architecture for synergistic multimodal face synthesis that overcomes limitations of existing ad hoc designs, enabling unprecedented spatial-semantic consistency for controllable face generation through its dual-stream transformer with shared RoPE attention and dynamic modality adaptation.
...
TL;DR: Noise pretraining of implicit neural representations reveals that unstructured noise (Uniform, Gaussian) improves signal fitting but yields poor deep image priors, while spectral noise matching natural image statistics achieves optimal balance for both fitting and inverse imaging tasks.


<details>
  <summary>Details</summary>
Motivation: To understand why data-driven initialization methods for implicit neural representations (INRs) work well - whether they encode classical statistical signal priors or more complex features - by analyzing the effects of noise pretraining on INR performance.

Method: Experimental analysis using noise pretraining on diverse noise classes (Gaussian, Dead Leaves, Spectral) and measuring INR ability to fit unseen signals and encode priors for inverse imaging tasks (denoising) on image and video data.

Result: Unstructured noise (Uniform, Gaussian) dramatically improves signal fitting capacity but yields poor deep image priors for denoising. Spectral noise with 1/|f|^α structure matching natural images achieves excellent balance of signal fitting and inverse imaging capabilities, performing on par with best data-driven initialization methods.

Conclusion: Noise pretraining reveals that spectral structure matching natural image statistics is crucial for effective INR initialization, enabling more efficient training in applications lacking sufficient domain-specific data while maintaining both fitting and inverse imaging capabilities.
...
TL;DR: Streaming retrieval architecture with on-device epsilon-net filter removes redundant frames, enabling efficient cross-modal video retrieval on edge devices with weak encoders.
...
Method: On-device epsilon-net filter retains only semantically novel frames to build denoised embedding index; cross-modal adapter and cloud re-ranker compensate for compact encoder's weak alignment.

Result: Single-pass streaming filter outperforms offline alternatives (k-means, farthest-point, uniform, random) across eight vision-language models (8M-632M) on two egocentric datasets (AEA, EPIC-KITCHENS); achieves 45.6% Hit@5 on held-out data using 8M on-device encoder at estimated 2.7 mW.

Conclusion: The proposed streaming retrieval architecture effectively addresses redundancy in continuous video streams, enabling efficient cross-modal retrieval on resource-constrained edge devices while maintaining competitive performance.
...
TL;DR: A generative video inpainting method that removes humans and shadows from egocentric walking tour videos using a semi-synthetic dataset and fine-tuned Casper diffusion model.


<details>
  <summary>Details</summary>
Motivation: Egocentric walking tour videos contain rich environmental data but are contaminated by frequent human presence and shadows, limiting their usefulness for environment modeling applications.

Method: Constructed a semi-synthetic dataset of video clip pairs (environment-only backgrounds and composite clips with humans+shadows), then fine-tuned the Casper video diffusion model for object and effects inpainting.

Result: The resulting model significantly outperforms Casper both qualitatively and quantitatively at removing humans from walking tour clips with complex backgrounds and significant human presence.

Conclusion: The generated human-free clips enable successful construction of 3D/4D models of urban locations, demonstrating practical utility for environment modeling applications.
...
TL;DR: DASES is a falsification-driven framework for autonomous scientific discovery that uses adversarial evaluation to prevent models from gaming static tests, discovering a new loss function (FNG-CE) that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the problem that in autonomous scientific discovery, once the evaluation criteria are fixed, powerful search algorithms can learn to pass tests without actually understanding the underlying mechanisms. This creates a "dangerous regime" where models can "win the exam without learning the mechanism." The authors aim to develop an evaluation framework that actively pushes against candidates through adaptive falsification rather than passively certifying them through static validation.

Method: The authors introduce DASES (Dynamic Adversarial Scientific Evaluation System), a falsification-driven framework with three co-evolving components: 1) an Innovator that generates executable scientific artifacts, 2) an Abyss Falsifier that creates scientifically admissible counterexample environments, and 3) a Mechanistic Causal Extractor that analyzes the underlying mechanisms. These components evolve under a fixed scientific contract, creating an adversarial evaluation process where candidates must survive increasingly challenging falsification attempts.

Result: In a controlled loss-discovery problem with a single editable locus, DASES successfully rejects artifacts that would have been accepted by static validation. The framework identifies the first candidate that survives the admissible falsification frontier and discovers FNG-CE (Falsification-Navigated Gradient Cross-Entropy), a novel loss function. FNG-CE transfers beyond the synthetic discovery environment and consistently outperforms standard CE (Cross-Entropy) and CE+L2 across standard benchmarks including ImageNet.

Conclusion: The DASES framework demonstrates that adversarial falsification-driven evaluation is superior to static validation for autonomous scientific discovery. By making "the abyss stare back" through adaptive counterexample generation, DASES prevents models from gaming evaluation metrics and ensures genuine mechanistic understanding. The discovery of FNG-CE, which outperforms established loss functions, validates the approach and suggests that falsification-driven frameworks can lead to more robust and transferable scientific discoveries.
...
TL;DR: LA-Sign: A looped transformer framework with geometry-aware alignment for skeleton-based isolated sign language recognition that uses recurrence instead of deeper layers for refinement, achieving SOTA results with fewer unique parameters.


<details>
  <summary>Details</summary>
Motivation: Existing deep feed-forward architectures for skeleton-based ISLR lack mechanisms for recurrent refinement and structured representation, while requiring increased model capacity through deeper stacking rather than efficient refinement processes.

Method: Proposes LA-Sign with looped transformer framework using recurrence for depth, geometry-aware contrastive objective in adaptive hyperbolic space, encoder-decoder looping design, and adaptive Poincare alignment for multi-scale semantic organization.

Result: Achieves state-of-the-art results on WLASL and MSASL benchmarks while using fewer unique layers, demonstrating effectiveness of recurrent latent refinement and geometry-aware representation learning.

Conclusion: Recurrent latent refinement through looping and geometry-aware representation in hyperbolic space provides effective alternative to deeper stacking for skeleton-based sign language recognition, enabling better motion understanding with fewer parameters.
...
TL;DR: The paper analyzes the modality gap in multi-modal models, showing it emerges from contrastive loss optimization and that reducing this gap improves robustness without affecting clean accuracy.


<details>
  <summary>Details</summary>
Motivation: To understand why the modality gap exists in multi-modal models and whether closing this gap improves downstream performance, particularly robustness.

Method: Theoretical analysis of contrastive loss optimization showing it yields orthogonal modality separation, plus empirical experiments with post-processing that moves one modality toward the other's mean.

Result: The modality gap is monotonically related to robustness - decreasing the gap maintains clean accuracy while improving robustness to perturbations. Simple post-processing significantly increases robustness for real-world VLMs.

Conclusion: The modality gap is an inherent property of contrastive learning that can be leveraged to improve model robustness through simple post-processing techniques.
...
TL;DR: WorldFlow3D introduces a novel flow-matching approach for unbounded 3D world generation that treats 3D generation as flowing through data distributions rather than conditional denoising, enabling faster convergence and better control over scene structure and texture.


<details>
  <summary>Details</summary>
Motivation: Unbounded 3D world generation is becoming foundational for scene modeling in computer vision, graphics, and robotics, but existing methods are limited by conditional denoising approaches and lack efficient control mechanisms.

Method: WorldFlow3D builds on flow matching principles to model 3D generation as flowing through 3D data distributions. It uses a latent-free flow approach that generates accurate 3D structure as an intermediate distribution to guide complex structure and high-quality texture generation. The method enables controllability through vectorized scene layout conditions for geometric structure and scene attributes for visual texture control.

Result: The method demonstrates effectiveness on both real outdoor driving scenes and synthetic indoor scenes, showing cross-domain generalizability and high-quality generation on real data distributions. It achieves favorable scene generation fidelity over existing approaches in all tested settings for unbounded scene generation.

Conclusion: WorldFlow3D presents a novel flow-based approach to unbounded 3D world generation that offers faster convergence, better control over scene structure and texture, and superior performance compared to existing methods across diverse domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing video editing methods focus on prescribing motion trajectories or altering appearance, but lack the ability to move an object's 3D motion trajectory while preserving its relative motion. The main challenge is obtaining paired video data for this specific scenario.
...
Conclusion: TrajectoryAtlas and TrajectoryMover provide a solution for moving object 3D motion trajectories in videos while preserving relative motion, overcoming the paired data generation challenge that limited previous methods.
...
<details>
  <summary>Details</summary>
Motivation: Real-world food recognition faces dual imbalance: (1) severe data imbalance within food classes (long-tailed distributions), and (2) imbalanced numbers of new food classes introduced at each incremental learning step, which existing continual learning methods don't address.

Method: DIME learns lightweight adapters for each task using parameter-efficient fine-tuning, then integrates them through a class-count guided spectral merging strategy with rank-wise threshold modulation to preserve dominant knowledge while allowing adaptive updates.
...
Conclusion: DIME effectively addresses the dual imbalance problem in continual food recognition through adapter merging, maintaining a single merged adapter for efficient deployment while significantly outperforming existing methods.
...
TL;DR: SparseDriveV2 improves scoring-based planning by using a factorized trajectory representation (paths × velocity profiles) and two-stage scoring, achieving state-of-the-art performance on autonomous driving benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the uncertainty in end-to-end multi-modal planning by investigating whether static trajectory vocabularies can match the performance of dynamic proposal methods when made sufficiently dense, challenging the assumption that dynamic generation is fundamentally necessary.

Method: Two complementary innovations: (1) scalable factorized vocabulary representation that decomposes trajectories into geometric paths and velocity profiles for combinatorial action space coverage, and (2) scalable two-stage scoring strategy with coarse factorized scoring over paths/velocity profiles followed by fine-grained scoring on composed trajectories.

Result: Achieves 92.0 PDMS and 90.1 EPDMS on NAVSIM, and 89.15 Driving Score with 70.00% Success Rate on Bench2Drive using a lightweight ResNet-34 backbone, demonstrating state-of-the-art performance for scoring-based planning methods.

Conclusion: Static trajectory vocabularies can achieve comparable performance to dynamic proposal methods when made sufficiently dense through factorized representation and efficient scoring, challenging the necessity of dynamic generation in multi-modal planning.
...
TL;DR: LatentPilot introduces a novel VLN paradigm that learns action-conditioned visual dynamics by leveraging future observations during training (but not at inference) through a flywheel-style training mechanism and visual latent tokens that enable the agent to "dream ahead" about how actions affect subsequent observations.


<details>
  <summary>Details</summary>
Motivation: Existing VLN models lack effective understanding of the causal relationship between actions and visual dynamics because they primarily reason over past/current observations while ignoring future visual changes. Humans can imagine near-future outcomes using action-dynamics causality, which improves environmental understanding and navigation decisions.

Method: 1) Flywheel-style training mechanism that iteratively collects on-policy trajectories and retrains the model to match agent behavior distribution, with expert takeover for excessive deviation. 2) Learns visual latent tokens without explicit supervision that attend globally in continuous latent space and are carried across steps, serving as both current output and next input to enable "dreaming ahead" about action effects.

Result: Achieves new state-of-the-art results on R2R-CE, RxR-CE, and R2R-PE benchmarks. Real-robot tests across diverse environments demonstrate superior understanding of environment-action dynamics.

Conclusion: LatentPilot successfully addresses the limitation of existing VLN models by enabling them to learn action-conditioned visual dynamics through future observation exploitation during training, without requiring future frames at inference, leading to improved navigation performance and better understanding of environment-action relationships.
...
TL;DR: Cross-modality distillation from CT to X-ray for disease classification shows limited success; plain logit-KD performs best on original split but fails under resampling, revealing task instability and need for rigorous evaluation protocols.


<details>
  <summary>Details</summary>
Motivation: To investigate whether CT can serve as training-only supervision for X-ray disease classifiers without requiring CT at inference time, addressing the gap in cross-modality computer-aided diagnosis models.

Method: Cross-modality teacher-student distillation using JDCNet as pilot scaffold, with patient-level paired data, logit-KD controls, attention transfer, feature hints, and Monte Carlo resampling for robustness testing.

Result: Plain cross-modal logit-KD achieved 0.875 accuracy/0.714 macro-F1 on original split but dropped to 0.500 balanced accuracy under resampling; late fusion and same-modality distillation performed best in resampled protocol.

Conclusion: No validated CT-to-X-ray architecture emerged; instead, the study provides a reproducible pilot protocol highlighting task definition challenges, failure modes, and minimum requirements for credible cross-modality transfer claims.
...
TL;DR: Modified MedSAM foundation model adapted for multi-class brain tissue segmentation (gray matter, white matter, background) using FSL preprocessing and minimal architectural changes, achieving Dice scores up to 0.8751 on IXI dataset.


<details>
  <summary>Details</summary>
Motivation: Accurate brain tissue segmentation from MRI is essential for neuroanatomy studies, neurological disorder diagnosis, and disease progression monitoring. Traditional methods like FSL FAST require task-specific adjustments and struggle with diverse imaging conditions, while foundation models like MedSAM offer promising prompt-based approaches that can be adapted for medical imaging tasks.

Method: Proposed a modified MedSAM model for multi-class brain tissue segmentation. Preprocessing pipeline includes: 1) skull stripping with FSL BET, 2) tissue probability mapping with FSL FAST, 3) converting to 2D axial, sagittal, coronal slices with multi-class labels (background, gray matter, white matter). Extended MedSAM's mask decoder to three classes while freezing the pre-trained image encoder and fine-tuning only the prompt encoder and decoder.

Result: Achieved Dice scores up to 0.8751 on the IXI dataset, demonstrating effective adaptation of foundation models for multi-class medical image segmentation with minimal architectural modifications.

Conclusion: Foundation models like MedSAM can be successfully adapted for multi-class medical image segmentation with minimal architectural changes. The approach shows promise for extending to more diverse medical imaging scenarios in future work, potentially reducing the need for task-specific adjustments required by traditional methods.
...
TL;DR: SplatHLoc: A hierarchical visual relocalization framework using Feature Gaussian Splatting as scene representation, with adaptive viewpoint retrieval and hybrid feature matching for improved accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Point-based hierarchical relocalization methods are limited by sparse image observations and weak feature matching, which hinders their effectiveness in camera pose estimation when revisiting known scenes.

Method: Uses Feature Gaussian Splatting as scene representation; proposes adaptive viewpoint retrieval to synthesize virtual candidates aligned with query viewpoints; introduces hybrid feature matching combining Gaussian-rendered features (better for coarse stage) and directly extracted image features (better for fine stage).

Result: Extensive experiments on indoor and outdoor datasets show SplatHLoc enhances robustness of visual relocalization and sets new state-of-the-art performance.

Conclusion: SplatHLoc effectively addresses limitations of point-based methods through novel scene representation and hybrid matching strategy, achieving superior visual relocalization performance.
...
TL;DR: SLVMEval is a benchmark for meta-evaluating text-to-video evaluation systems on long videos (up to 3 hours), testing their ability to accurately assess video quality in human-perceptible degradation scenarios.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video evaluation systems lack proper assessment on long videos, and there's a need to determine if these systems can accurately evaluate video quality in scenarios that are easily perceptible to humans.

Method: The authors create a synthetic long-video meta-evaluation benchmark using dense video-captioning datasets. They synthetically degrade source videos to create controlled "high-quality vs low-quality" pairs across 10 distinct aspects, then use crowdsourcing to filter pairs where degradation is clearly perceptible, establishing a final testbed for pairwise comparison-based meta-evaluation.

Result: Human evaluators achieve 84.7%-96.8% accuracy in identifying better long videos. In 9 out of 10 aspects, existing evaluation systems perform worse than human assessment, revealing significant weaknesses in text-to-long-video evaluation.

Conclusion: The SLVMEval benchmark exposes limitations in current text-to-video evaluation systems for long videos, demonstrating that these systems fall short of human-level performance in most quality assessment aspects, highlighting the need for improved evaluation methodologies for long-form video content.
...
TL;DR: 3D object reconstruction from orthographic views using Harris corner detection, envelope intersection, and computational geometry with OpenGL rendering.


<details>
  <summary>Details</summary>
Motivation: To develop a method for reconstructing 3D objects from 2D orthographic views, which is useful in CAD, engineering design, and computer graphics applications where 3D models need to be generated from traditional 2D engineering drawings.

Method: 1. Apply Harris corner detector on input orthographic views to obtain control points. 2. Project control points perpendicular to respective views to construct 3D envelopes. 3. Compute intersection points of mutually perpendicular envelopes to obtain 3D point cloud. 4. Use computational geometry techniques to regenerate surfaces from the 3D points. 5. Render final 3D object using OpenGL.

Result: Successfully reconstructs 3D objects from orthographic views by converting 2D corner features into 3D points through envelope intersection, then generating surfaces for visualization.

Conclusion: The proposed method effectively reconstructs 3D objects from orthographic views using corner detection and geometric intersection techniques, providing a practical approach for converting traditional 2D engineering drawings into 3D models.
...
TL;DR: Adaptive context compression framework for LLMs improves long-running interaction performance by selectively retaining essential information while controlling context growth.


<details>
  <summary>Details</summary>
Motivation: LLMs suffer performance degradation in long-running interactions due to increasing context length, memory saturation, and computational overhead, necessitating efficient context management.

Method: Adaptive context compression framework integrating importance-aware memory selection, coherence-sensitive filtering, and dynamic budget allocation to retain essential conversational information.

Result: Consistent improvements in conversational stability and retrieval performance while reducing token usage and inference latency on LOCOMO, LOCCO, and LongBench benchmarks.
...
TL;DR: Multi-Layer Memory Framework for long-horizon dialogue systems addresses semantic drift and memory retention issues through layered memory decomposition with adaptive retrieval and retention regularization.


<details>
  <summary>Details</summary>
Motivation: Long-horizon dialogue systems suffer from semantic drift and unstable memory retention across extended sessions, requiring better memory management to maintain coherence and reasoning stability.
...
Result: Achieved 46.85 Success Rate, 0.618 overall F1 with 0.594 multi-hop F1, 56.90% six-period retention while reducing false memory rate to 5.1% and context usage to 58.40% on LOCOMO, LOCCO, and LoCoMo benchmarks.

Conclusion: The framework enhances long-term retention and reasoning stability under constrained context budgets, confirming effectiveness in mitigating semantic drift and improving memory management in extended dialogue sessions.
...
TL;DR: LightHarmony3D enables physically consistent lighting and shadows for mesh insertion in 3D Gaussian Splatting scenes using a generative module to predict HDR environment maps.


<details>
  <summary>Details</summary>
Motivation: While 3DGS enables high-fidelity scene reconstruction, inserting external mesh objects with physically consistent lighting and shadows remains challenging for AR/VR, virtual staging, and digital content creation applications.

Method: Proposes a generative module that predicts a full 360° HDR environment map at insertion locations via single forward pass, leveraging generative priors instead of iterative optimization for efficient illumination capture and physically grounded shading.

Result: Achieves state-of-the-art realism and multi-view consistency across multiple real-world reconstruction datasets, with the framework also introducing the first dedicated benchmark for mesh insertion in 3DGS.

Conclusion: LightHarmony3D provides an effective solution for illumination-consistent mesh insertion in 3DGS scenes, enabling physically consistent lighting and shadows while maintaining multi-view coherence for immersive applications.
...
<details>
  <summary>Details</summary>
Motivation: Infrared target detection faces challenges due to low contrast, target camouflage in complex backgrounds, and false alarms from distractors with similar features, requiring improved detection accuracy and reduced false alarm rates.

Method: Proposes CCDNet with three key components: 1) Weighted Multi-branch Perceptrons (WMPs) backbone for multi-level feature aggregation, 2) Aggregation-and-Refinement Fusion Neck (ARFN) for bidirectional target-background relation reconstruction, and 3) Contrastive-aided Distractor Discriminator (CaDD) for adaptive similarity computation to distinguish real targets from distractors.

Result: Extensive experiments on infrared image datasets demonstrate that CCDNet outperforms other state-of-the-art methods in detection accuracy and false alarm reduction.

Conclusion: CCDNet effectively addresses infrared target detection challenges by combining feature aggregation, target-background relation modeling, and distractor discrimination, achieving superior performance for applications like wilderness rescue and maritime search.
...
TL;DR: M2H-MX is a real-time multi-task perception model for monocular spatial understanding that improves depth and semantic predictions through register-gated global context and controlled cross-task interaction, enabling 60.7% reduction in trajectory error when integrated with monocular SLAM.


<details>
  <summary>Details</summary>
Motivation: Monocular cameras are attractive for robotic perception due to low cost and ease of deployment, but achieving reliable real-time spatial understanding from single image streams remains challenging. While multi-task dense prediction models have improved depth and semantic estimation, translating these advances into stable monocular mapping systems is non-trivial.
...
Result: On NYUDv2, M2H-MX-L achieves state-of-the-art results: semantic mIoU improved by 6.6% and depth RMSE reduced by 9.4% over representative multi-task baselines. When deployed in real-time monocular mapping on ScanNet, M2H-MX reduces average trajectory error by 60.7% compared to strong monocular SLAM baseline while producing cleaner metric-semantic maps.

Conclusion: Modern multi-task dense prediction can be reliably deployed for real-time monocular spatial perception in robotic systems, demonstrating that improved dense prediction accuracy translates directly to enhanced system-level performance in monocular mapping applications.
...
TL;DR: Diffusion Mental Averages (DMA) enables diffusion models to generate sharp, realistic "mental averages" of concepts by averaging within the model's semantic space through trajectory alignment, rather than blurry image averaging.


<details>
  <summary>Details</summary>
Motivation: Prior methods for averaging image collections produce blurry results when applied to diffusion samples, as they operate outside the model and ignore the generative process. The authors aim to enable diffusion models to produce their own sharp, realistic concept averages.

Method: DMA averages within the diffusion model's semantic space by optimizing multiple noise latents so their denoising trajectories progressively converge toward shared coarse-to-fine semantics, yielding a single sharp prototype. For multimodal concepts, samples are clustered in semantically-rich spaces like CLIP, then bridged into diffusion space using Textual Inversion or LoRA.

Result: DMA delivers consistent, realistic averages even for abstract concepts, producing sharp prototypes that serve as visual summaries and provide insights into model biases and concept representation.

Conclusion: DMA represents the first approach to produce consistent, realistic concept averages from diffusion models by working within the model's semantic space rather than averaging images externally, offering both practical visual summaries and analytical insights into model behavior.
...
TL;DR: Proposes TSONet for monocular building height estimation from PhiSat-2 imagery, introducing a new dataset (PHDataset) and achieving state-of-the-art performance through two-stream architecture with cross-stream feature exchange and ordinal height refinement.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges in monocular building height estimation: ambiguous height cues, large inter-city building morphology variations, and long-tailed height distributions. PhiSat-2 offers promising global coverage with 4.75m resolution and 7-band spectral data, but its potential hasn't been systematically evaluated for this task.

Method: Two-Stream Ordinal Network (TSONet) jointly models footprint segmentation and height estimation. Key components: Cross-Stream Exchange Module (CSEM) for footprint-aware feature interaction, and Feature-Enhanced Bin Refinement (FEBR) module for ordinal height refinement. Uses ordinal regression with footprint assistance.

Result: TSONet achieves best performance on PHDataset: reduces MAE by 13.2% and RMSE by 9.7%, improves IoU by 14.0% and F1-score by 10.1% over strongest competing methods. Ablation studies confirm effectiveness of CSEM, FEBR, and joint ordinal regression with footprint assistance.

Conclusion: PhiSat-2 benefits monocular building height estimation through balanced spatial detail and multispectral observations. Study confirms PhiSat-2's potential for this task and provides dedicated dataset (PHDataset) and effective method (TSONet) for future research.
...
TL;DR: FlexMem is a training-free approach for long video understanding in MLLMs that mimics human video watching behavior using visual memory mechanisms, enabling infinite-length video processing without input limits.


<details>
  <summary>Details</summary>
Motivation: Long video understanding is a major challenge for Multimodal Large Language Models (MLLMs) due to input length limitations and computational constraints of existing methods that process all video information at once.

Method: FlexMem uses a dual-pathway compression design for memory transfer and writing of visual KV caches, along with different memory reading strategies for diverse video tasks including streaming. It mimics human video watching by continually processing content and recalling relevant memory fragments.

Result: On a single 3090 GPU, FlexMem achieves significant improvements over existing efficient video understanding methods, processes over 1k frames, and helps base MLLMs achieve comparable or better performance than SOTA models like GPT-4o and Gemini-1.5 Pro on some benchmarks.

Conclusion: FlexMem provides an effective, training-free solution for infinite-length video understanding in MLLMs by implementing a flexible memory mechanism that overcomes input limitations while maintaining computational efficiency.
...
TL;DR: Omni-NegCLIP fine-tunes CLIP to better understand presence-based and absence-based negation expressions through modified contrastive objectives, achieving significant improvements without sacrificing general image-text retrieval performance.
...
Method: Proposes Omni-NegCLIP with two contrastive objectives: presence-based objective pulls image embeddings to original captions while pushing away from presence-negated captions; absence-based objective aligns images with both original and absence-negated captions while maintaining semantic distinction. Fine-tunes front transformer layers of CLIP text encoder based on observation that these layers have stronger learning ability for negated text.

Result: Improves performance on presence-based negation by up to 52.65% and absence-based negation by up to 12.50% compared to pretrained CLIP. Maintains or improves general image-text retrieval capability by up to 19.62%. Outperforms prior works in comprehensive negation understanding.

Conclusion: Omni-NegCLIP effectively enhances CLIP's understanding of multiple negation types through targeted fine-tuning of front transformer layers with specialized contrastive objectives, achieving substantial improvements while preserving general multimodal capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing fairness algorithms require protected attribute information (like race, gender) which limits real-world application; need for bias mitigation methods that don't need sensitive attributes.

Method: Proposed Non-Protected Attribute-based Debiasing (NPAD) algorithm uses auxiliary information from non-protected attributes with two loss functions: Debiasing via Attribute Cluster Loss (DACL) and Filter Redundancy Loss (FRL).
...
Conclusion: NPAD provides effective bias mitigation without requiring protected attribute information, addressing practical limitations of existing fairness algorithms.
...
TL;DR: ConInfer is a context-aware inference framework for training-free open-vocabulary remote sensing segmentation that performs joint prediction across multiple spatial units while modeling their semantic dependencies, improving segmentation consistency and accuracy over patch-wise methods.


<details>
  <summary>Details</summary>
Motivation: Existing training-free open-vocabulary remote sensing segmentation approaches use independent patch-level predictions, which are misaligned with the intrinsic characteristics of remote sensing data. Real-world remote sensing scenes are large-scale with strong spatial and semantic correlations, making isolated patch-wise predictions insufficient for accurate segmentation.

Method: ConInfer is a context-aware inference framework that performs joint prediction across multiple spatial units while explicitly modeling their inter-unit semantic dependencies. It incorporates global contextual cues to enhance segmentation consistency, robustness, and generalization in complex remote sensing environments.

Result: Extensive experiments on multiple benchmark datasets show that ConInfer consistently surpasses state-of-the-art per-pixel VLM-based baselines like SegEarth-OV, achieving average improvements of 2.80% on open-vocabulary semantic segmentation and 6.13% on object extraction tasks.

Conclusion: The proposed ConInfer framework addresses the limitations of independent patch-wise predictions in remote sensing segmentation by incorporating global contextual information and modeling semantic dependencies between spatial units, leading to significant improvements in segmentation accuracy and consistency for open-vocabulary remote sensing applications.
...
TL;DR: MaskAdapt is a two-stage residual learning framework for physics-based humanoid control that enables flexible motion adaptation through body-part masking and targeted residual policies.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for flexible motion adaptation in physics-based humanoid control, where traditional methods often require full retraining or lack precision in modifying specific body parts while preserving existing behaviors.

Method: Two-stage residual learning: 1) Train mask-invariant base policy using stochastic body-part masking with regularization for consistent action distributions; 2) Train residual policy on frozen base controller to modify only targeted body parts while preserving original behaviors elsewhere.

Result: MaskAdapt demonstrates strong robustness and adaptability, producing diverse behaviors under masked observations and delivering superior targeted motion adaptation compared to prior work, with applications in motion composition and text-driven partial goal tracking.

Conclusion: The MaskAdapt framework provides an effective approach for flexible motion adaptation in physics-based humanoid control, enabling precise modification of specific body parts while maintaining stability and preserving original behaviors in other regions.
...
TL;DR: MELT addresses frequency bias and hard negative interference in Composed Image Retrieval through rare semantic localization and diffusion-based denoising.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods suffer from frequency bias (Rare Sample Neglect) and susceptibility to interference from hard negative samples and noise, limiting their effectiveness in handling rare modification semantics and robust similarity estimation.

Method: Proposes MELT network that assigns increased attention to rare modification semantics in multimodal contexts and applies diffusion-based denoising to hard negative samples with high similarity scores to enhance multimodal fusion and matching.
...
Conclusion: MELT effectively addresses frequency bias and hard negative interference in CIR through rare semantic localization and robust similarity estimation, demonstrating state-of-the-art performance on standard benchmarks.
...
TL;DR: A gaze-guided CLIP model with adaptive language prompts for fine-grained deepfake attribution and detection, achieving state-of-the-art performance on novel generative methods like diffusion models.


<details>
  <summary>Details</summary>
Motivation: Current deepfake attribution/detection methods have poor generalization to novel generative techniques (diffusion, flow models) due to limited visual modality exploration and lack of synergy between attribution and detection tasks.

Method: 1) Novel fine-grained benchmark for DFAD evaluation on novel generators; 2) Gaze-aware CLIP model leveraging gaze vector distribution differences between pristine/forged faces; 3) Visual perception encoder mining global forgery embeddings; 4) Gaze-aware image encoder fusing forgery gaze prompts with image embeddings; 5) Language refinement encoder with adaptive-enhanced word selector for precise vision-language matching.

Result: Outperforms state-of-the-art by 6.56% ACC and 5.32% AUC in average performance under attribution and detection settings on the proposed benchmark.

Conclusion: The gaze-guided approach with adaptive language prompts effectively enhances generalization to unseen face forgery attacks, demonstrating superior performance in fine-grained deepfake attribution and detection across novel generative methods.
...
TL;DR: MotionScale is a 4D Gaussian Splatting framework for reconstructing dynamic scenes from monocular videos, featuring scalable motion fields and progressive optimization for large scenes and long sequences.


<details>
  <summary>Details</summary>
Motivation: Existing neural rendering methods struggle with accurate 3D geometry recovery and temporally consistent motion reconstruction in complex environments from monocular videos.

Method: Uses 4D Gaussian Splatting with cluster-centric basis transformations for scalable motion fields, plus progressive optimization with two decoupled stages: background extension (handles new regions, refines poses, models shadows) and foreground propagation (enforces motion consistency via three-stage refinement).
...
Conclusion: MotionScale enables realistic reconstruction of dynamic 4D scenes from monocular videos with improved scalability, accuracy, and temporal coherence for complex environments.
...
TL;DR: The paper adapts self-consistency from natural language reasoning to visual domains, specifically for LLM-generated motion graphics trajectories, using geometric transformation groups to identify consistent trajectories and improve generation accuracy by 4-6%.


<details>
  <summary>Details</summary>
Motivation: Self-consistency has been effective for improving LLM performance on natural language reasoning tasks, but its application to visual domains remains unexplored. The authors aim to extend self-consistency to visual reasoning, particularly for motion graphics trajectory generation and verification.

Method: Given a prompt, the method samples diverse motion trajectories from an LLM, then identifies consistent trajectories via clustering. Key insight: model shape families as prototype trajectories paired with geometric transformation groups (rigid, similarity, affine). Two trajectories are consistent if one can be transformed into the other under allowable warps. An algorithm automatically recovers shape families using hierarchical relationships between candidate transformation groups.

Result: The approach improves LLM-based trajectory generation accuracy by 4-6% and extends to verification with 11% precision gains over VLM baselines. The method demonstrates effective adaptation of self-consistency to visual domains.

Conclusion: Self-consistency can be successfully adapted to visual domains using geometric transformation groups to model shape families, significantly improving both generation and verification of LLM-produced motion graphics trajectories.
...
TL;DR: A bilevel meta-learning method that performs feature-space augmentation to improve classifier robustness against spurious correlations, requiring minimal training time and showing semantically meaningful feature edits.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks often rely on spurious features for predictions, making them brittle under distribution shift and on minority-group examples. While ERM-trained feature extractors learn informative representations, classifier heads often fail to handle spurious correlations properly.

Method: A bilevel meta-learning approach that learns support-side feature edits in the backbone's output space. The method performs augmentation directly in feature space rather than pixel space, enabling efficient inner-loop updates that improve classifier performance on hard examples and worst-group scenarios.

Result: The method achieves improved worst-group performance and better handling of spurious correlations with high efficiency, requiring only a few minutes of training on a single GPU. CLIP-based visualizations confirm that learned feature-space updates induce semantically meaningful shifts aligned with spurious attributes.

Conclusion: Feature-space augmentation via bilevel meta-learning provides an efficient and stable approach to improve classifier robustness against spurious correlations, addressing distribution shift issues while maintaining the rich representations learned by ERM-trained backbones.
...
TL;DR: FOSCU integrates Duo-Diffusion (3D latent diffusion with ControlNet) to generate synthetic MRI volumes and segmentation labels, addressing data scarcity in medical image segmentation.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces challenges including restricted access to clinical datasets, costly annotation, and data shortage through PACS, which impede development of robust segmentation algorithms.

Method: FOSCU integrates Duo-Diffusion (a 3D latent diffusion model with ControlNet) that simultaneously generates synthetic MRI volumes and segmentation labels, and an enhanced 3D U-Net training pipeline. Duo-Diffusion uses segmentation-conditioned diffusion to ensure spatial consistency and anatomical detail.

Result: Experimental evaluation on 720 abdominal MRI scans shows models trained with combined real and synthetic data yield a mean Dice score gain of 0.67% over those using only real data, and achieve a 36.4% reduction in Fréchet Inception Distance (FID), reflecting enhanced image fidelity.

Conclusion: FOSCU effectively addresses data scarcity in medical image segmentation by generating high-quality synthetic data that improves model performance and image fidelity when combined with real data.
...
TL;DR: CIPHER is a deepfake detection framework that reuses and fine-tunes discriminators from image generation models to capture generation-agnostic artifacts, achieving superior cross-model detection performance across nine state-of-the-art generative models.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of GANs and diffusion models has created synthetic faces that are increasingly realistic, amplifying risks of misinformation, fraud, and identity abuse. There's an urgent need for detectors that remain robust across diverse generative models as conventional detectors often fail to capture generation-agnostic artifacts.
...
Result: CIPHER demonstrates superior cross-model detection performance across nine state-of-the-art generative models, achieving up to 74.33% F1-score and outperforming existing ViT-based detectors by over 30% in F1-score on average. It maintains robust performance on challenging datasets where baseline methods fail, with up to 88% F1-score on CIFAKE compared to near-zero performance from conventional detectors.
...
TL;DR: Proposes a unified framework that jointly models positional and semantic uncertainties in real-time maps and integrates them into trajectory prediction to improve robustness and performance.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of inherent uncertainty in real-time maps for trajectory prediction, which arises from positional inaccuracies (sensor limitations, environmental occlusions) and semantic errors (misinterpretations of scene context).

Method: Develops a dual-head architecture that independently estimates semantic and positional predictions in a dual-pass manner, deriving prediction variances as uncertainty indicators end-to-end. These uncertainties are fused with semantic and positional predictions to enhance trajectory forecast robustness.

Result: Evaluated on nuScenes dataset across four map estimation methods and two trajectory prediction baselines. The method effectively quantifies map uncertainties through both positional and semantic dimensions and consistently improves performance across minADE, minFDE, and Miss Rate metrics.

Conclusion: Proposed uncertainty-aware framework successfully addresses map uncertainty challenges in trajectory prediction, demonstrating improved robustness and performance through joint modeling of positional and semantic uncertainties.
...
TL;DR: StereoVGGT is a feature backbone for stereo vision that adapts the Visual Geometry Grounded Transformer (VGGT) to mitigate geometric degradation and leverage its embedded camera calibration knowledge, achieving state-of-the-art performance on stereo matching tasks.


<details>
  <summary>Details</summary>
Motivation: Current stereo vision backbones (MDE models or VFMs) lack explicit camera pose supervision during pretraining, creating a performance bottleneck since geometric knowledge is essential for stereo vision. While VGGT has 3D priors including camera poses, direct application yields suboptimal performance due to geometric detail degradation during feature extraction.

Method: Proposes StereoVGGT, a feature backbone tailored for stereo vision that leverages frozen VGGT with a training-free feature adjustment pipeline to mitigate geometric degradation and harness the latent camera calibration knowledge embedded in VGGT.

Result: StereoVGGT-based stereo matching network achieved 1st rank among all published methods on the KITTI benchmark, demonstrating it serves as a highly effective backbone for stereo vision tasks.

Conclusion: StereoVGGT successfully bridges the gap between VGGT's geometric priors and stereo vision requirements by addressing geometric degradation issues, enabling state-of-the-art performance and validating its effectiveness as a specialized backbone for stereo vision tasks.
...
TL;DR: TriDerm is a multimodal framework that learns interpretable wound representations for rare skin disorder RDEB by integrating wound images, masks, and expert reports, using expert triplet comparisons to evaluate embedding spaces and outperforming off-the-shelf foundation models.


<details>
  <summary>Details</summary>
Motivation: RDEB is a rare genetic skin disorder where clinicians need to find similar cases using images and clinical text, but off-the-shelf foundation models don't reliably capture clinically meaningful features for this heterogeneous, long-tail disease, and structured measurement of agreement with experts is challenging.

Method: TriDerm integrates wound imagery, boundary masks, and expert reports. For vision: adapts visual foundation models using wound-level attention pooling and non-contrastive representation learning. For text: prompts LLMs with comparison queries and recovers medically meaningful representations via soft ordinal embeddings (SOE). Evaluates embedding spaces with expert ordinal comparisons (triplet judgments).
...
Conclusion: TriDerm demonstrates that multimodal integration of wound imagery and expert reports with expert-guided evaluation via triplet comparisons enables clinically meaningful representations for rare diseases, providing a framework that outperforms existing foundation models and makes tools publicly available.
...
<details>
  <summary>Details</summary>
Motivation: The democratization of prompt-based AI image editing has increased risks of malicious content fabrication and misinformation, but forgery localization methods for these emerging techniques remain under-explored due to data scarcity.

Method: 1) Developed an automated mask annotation framework using keypoint alignment and semantic space similarity to generate ground-truth masks; 2) Constructed PromptForge-350k dataset covering four SOTA prompt-based editing models; 3) Proposed ICL-Net with triple-stream backbone and intra-image contrastive learning to capture robust forensic features.

Result: Achieved 62.5% IoU on PromptForge-350k, outperforming SOTA by 5.1%; showed strong robustness (<1% IoU drop) against common degradations; demonstrated promising generalization to unseen editing models with 41.5% average IoU.

Conclusion: The work addresses data scarcity in forgery localization for prompt-based AI editing through a novel dataset and network architecture, achieving superior performance and demonstrating robustness and generalization capabilities.
...
TL;DR: Extend3D: Training-free pipeline for 3D scene generation from single image using object-centric 3D generative model with extended latent space, patch-wise generation, and iterative refinement via under-noising concept.


<details>
  <summary>Details</summary>
Motivation: Object-centric 3D generative models have fixed-size latent spaces that limit representation of wide scenes; need to generate complete 3D scenes from single images without requiring training.

Method: Extend latent space in x/y directions, divide into overlapping patches, apply object-centric model to each patch with coupling at each time step. Use point cloud prior for initialization, iteratively refine occluded regions via SDEdit with under-noising concept. Optimize extended latent during denoising with 3D-aware objectives for geometry and texture.

Result: Method yields better results than prior methods as evidenced by human preference and quantitative experiments, demonstrating improved 3D scene generation from single images.

Conclusion: Extend3D enables training-free 3D scene generation from single images by extending object-centric models' latent spaces, using patch-wise generation with coupling, and introducing under-noising for 3D completion and optimization for sub-scene consistency.
...
TL;DR: AA-Splat is an anti-aliased feed-forward 3D Gaussian Splatting method that addresses rendering artifacts at out-of-distribution sampling rates through opacity-balanced band-limiting design.
...
Method: AA-Splat employs an opacity-balanced band-limiting (OBBL) design with two key components: 1) 3D band-limiting post-filter that integrates multi-view maximal frequency bounds into the feed-forward reconstruction pipeline to band-limit 3D scene representations and eliminate degenerate Gaussians; 2) Opacity Balancing (OB) that seamlessly integrates pixel-aligned Gaussian primitives into rendering to compensate for increased overlap between expanded Gaussian primitives.

Result: AA-Splat demonstrates drastic improvements with average 5.4-7.5dB PSNR gains on novel view synthesis performance over state-of-the-art baseline DepthSplat at all resolutions between 4× and 1/4×.

Conclusion: AA-Splat enables robust anti-aliased rendering at any resolution for feed-forward 3D Gaussian Splatting, significantly outperforming existing methods and addressing critical limitations in sparse-view 3D reconstruction and novel view synthesis.
...
TL;DR: HIRE framework dynamically detects hallucination representations in vision-language models and performs hallucination-eliminating edits with minimal computational overhead, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models suffer from hallucination issues where outputs contradict visual facts. Existing mitigation methods either require substantial retraining resources (retraining methods) or introduce dual inference overhead (Contrastive Decoding), limiting practical applicability.
...
Result: Achieves state-of-the-art performance on existing benchmarks with extensive experiments demonstrating effectiveness, efficient hallucination elimination capability, and powerful controllability over hallucinations.

Conclusion: HIRE provides an efficient and robust solution for hallucination mitigation in vision-language models with minimal computational overhead compared to existing methods.
...
TL;DR: AGFT is an alignment-guided fine-tuning framework that enhances zero-shot adversarial robustness in vision-language models while preserving cross-modal semantic structure through soft alignment distributions and distribution consistency calibration.


<details>
  <summary>Details</summary>
Motivation: Pre-trained VLMs have strong zero-shot generalization but are vulnerable to adversarial attacks. Existing adversarial fine-tuning methods disrupt cross-modal alignment, weakening visual-textual correspondence and degrading zero-shot performance.

Method: AGFT uses probabilistic predictions of the original model for text-guided adversarial training, aligning adversarial visual features with textual embeddings via soft alignment distributions. It includes a distribution consistency calibration mechanism that adjusts robust model output to match temperature-scaled pre-trained model predictions.
...
Conclusion: AGFT effectively enhances adversarial robustness in VLMs while preserving cross-modal alignment, addressing limitations of label-based methods that disrupt semantic structure.
...
TL;DR: ViTAS introduces a multi-stage pipeline for radiology report summarization that selectively focuses on pathology-relevant visual patches rather than full images, achieving state-of-the-art performance by demonstrating that less but more relevant visual input is superior for multimodal summarization.
...
Method: ViTAS (Visual-Text Attention Summarizer) is a multi-stage pipeline combining: ensemble-guided MedSAM2 lung segmentation, bidirectional cross-attention for multi-view fusion, Shapley-guided adaptive patch clustering, and hierarchical visual tokenization feeding a Vision Transformer (ViT). The approach selectively focuses on pathology-relevant visual patches rather than full images.
...
Conclusion: Selectively focusing on pathology-relevant visual patches yields substantially better performance than using full images. The findings demonstrate that less but more relevant visual input is not only sufficient but superior for multimodal radiology summarization, challenging the assumption that more visual input is always better.
...
TL;DR: Proposes an extrinsic-aware cross-attention framework for camera-LiDAR calibration that directly aligns image patches and LiDAR point groups in their native domains without projecting LiDAR to 2D depth maps, achieving superior accuracy and robustness under large misalignments.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based camera-LiDAR fusion methods project LiDAR points into 2D depth maps for feature fusion, which distorts 3D geometry and degrades performance when extrinsic initialization is far from ground truth. This limitation motivates a direct cross-modal alignment approach that preserves native domain geometry.

Method: An extrinsic-aware cross-attention framework that directly aligns image patches and LiDAR point groups in their native domains. The attention mechanism explicitly injects extrinsic parameter hypotheses into correspondence modeling, enabling geometry-consistent cross-modal interaction without relying on projected 2D depth maps.

Result: Extensive experiments on KITTI and nuScenes benchmarks show the method consistently outperforms state-of-the-art approaches in accuracy and robustness. Under large extrinsic perturbations, achieves accurate calibration in 88% of KITTI cases and 99% of nuScenes cases, substantially surpassing the second-best baseline.

Conclusion: The proposed extrinsic-aware cross-attention framework provides a more robust and accurate solution for camera-LiDAR extrinsic calibration by directly modeling cross-modal correspondences in native domains, avoiding the geometric distortions inherent in 2D projection-based methods.
...
TL;DR: This paper proposes an imperceptible visual prompt injection attack method against closed-source multimodal large language models (MLLMs) that embeds adversarial instructions into images through bounded text overlays and optimized visual perturbations.


<details>
  <summary>Details</summary>
Motivation: MLLMs deployed in real-world applications are vulnerable to prompt injection attacks, but existing methods rely on textual or perceptible visual prompts that are observable by humans. The authors aim to develop imperceptible visual prompt injection attacks against powerful closed-source MLLMs.

Method: The method adaptively embeds malicious prompts into input images via bounded text overlays for semantic guidance. Imperceptible visual perturbations are iteratively optimized to align the feature representation of attacked images with malicious visual and textual targets at both coarse- and fine-grained levels. The visual target is instantiated as a text-rendered image and progressively refined during optimization to better represent desired semantics and improve transferability.
...
Conclusion: The proposed imperceptible visual prompt injection method effectively attacks closed-source MLLMs by embedding adversarial instructions through optimized visual perturbations and text overlays, highlighting security vulnerabilities in multimodal AI systems.
...
TL;DR: Multimodal models (Paligemma, Llava, Qwen) combining visual and textual features for ID document presentation attack detection show limited effectiveness despite theoretical advantages over traditional visual-only systems.


<details>
  <summary>Details</summary>
Motivation: Traditional PAD systems relying solely on visual features are insufficient against sophisticated spoofing attacks on ID documents, necessitating more robust multimodal approaches that incorporate both visual and contextual information.

Method: Utilized pre-trained multimodal models (Paligemma, Llava, Qwen) to combine deep visual embeddings with contextual textual metadata (document type, issuer, date) for enhanced presentation attack detection on ID documents.

Result: Experimental results indicate that the multimodal models struggle to accurately detect presentation attacks on ID documents, showing limited effectiveness despite the theoretical advantages of combining visual and textual modalities.

Conclusion: While multimodal integration represents a promising direction for PAD enhancement, current pre-trained models face challenges in accurately detecting presentation attacks on ID documents, suggesting need for specialized training or architectural improvements.
...
TL;DR: A²BFR is an attribute-aware blind face restoration framework that unifies high-fidelity reconstruction with prompt-controllable generation using diffusion transformers and semantic attribute learning.


<details>
  <summary>Details</summary>
Motivation: Blind face restoration (BFR) is ill-posed, leading to ambiguous solutions. Current diffusion-based methods lack controllability, while text-guided editing lacks reliable restoration. There's a need to unify high-fidelity reconstruction with controllable attribute manipulation.

Method: A²BFR uses a Diffusion Transformer backbone with unified image-text cross-modal attention, jointly conditioning on degraded inputs and textual prompts. It introduces attribute-aware learning using facial attribute embeddings from an attribute-aware encoder, and semantic dual-training leveraging pairwise attribute variations from the AttrFace-90K dataset to enforce attribute discrimination while preserving fidelity.

Result: A²BFR achieves state-of-the-art performance, outperforming diffusion-based BFR baselines by -0.0467 LPIPS (better perceptual quality) and +52.58% attribute accuracy. It enables fine-grained, prompt-controllable restoration even under severe degradations.

Conclusion: The proposed A²BFR framework successfully unifies high-fidelity face restoration with prompt-controllable generation, addressing the limitations of both traditional BFR methods and text-guided editing approaches through attribute-aware learning and semantic dual-training.
...
TL;DR: VLMs show systematic bias in optical illusion perception, predicting illusions as "real" regardless of counterfactual modifications. A tool-guided inference framework with generic image manipulation tools and illusion-type routing addresses this without training, showing strong cross-structural generalization.


<details>
  <summary>Details</summary>
Motivation: Vision-language models exhibit systematic bias when processing optical illusions, overwhelmingly predicting illusions as "real" even when images are counterfactually modified. This failure mode reveals limitations in VLM perceptual reasoning that need to be addressed without requiring model retraining.

Method: Tool-guided inference framework using off-the-shelf VLMs with access to generic image manipulation tools (line drawing, region cropping, side-by-side comparison, channel isolation) and an illusion-type-routing system prompt. Tools produce new immutable image resources in a persistent registry, allowing reference and composition of prior annotated views throughout reasoning chains.

Result: The framework achieves strong cross-structural generalization, with consistent performance from validation to test sets containing structurally unfamiliar illusion variants. Three empirical observations: (1) strong positive-detection bias from imbalanced training data, (2) dissociation between pixel-accurate spatial reasoning and logical inference over self-generated annotations, (3) pronounced sensitivity to image compression artifacts compounding false positives.

Conclusion: Generic tool-plus-routing design effectively addresses VLM bias in optical illusion perception without training, demonstrating cross-structural generalization. The approach reveals important empirical observations about VLM reasoning limitations that warrant further investigation into training data biases, spatial-logical dissociation, and compression artifact sensitivity.
...
TL;DR: SeGPruner is a semantic-aware and geometry-guided token reduction framework that improves 3D question answering efficiency by pruning redundant visual tokens from multi-view images while maintaining semantic and spatial coverage.


<details>
  <summary>Details</summary>
Motivation: Current 3D QA pipelines using VLMs suffer from severe token redundancy when aggregating multi-view observations, leading to inefficient inference under constrained token budgets. Existing token pruning methods are limited to 2D inputs or indirect geometric cues, failing to explicitly retain semantically critical objects and maintain sufficient spatial coverage for robust 3D reasoning.

Method: SeGPruner combines two complementary selectors: 1) Saliency-aware Token Selector preserves semantically salient tokens using attention-based importance scoring, and 2) Geometry-aware Token Diversifier complements these with spatially diverse tokens by jointly considering semantic relevance and 3D geometric distance. This balances object-level evidence and global scene coverage under aggressive token reduction.
...
TL;DR: EarthEmbeddingExplorer is an interactive web app that transforms static Earth observation foundation models and embedding datasets into dynamic, practical workflows for scientific discovery through cross-modal queries.
...
Method: Developed EarthEmbeddingExplorer, a cloud-native web application with interactive interface supporting cross-modal queries (natural language, visual, and geolocation) to explore precomputed Earth embeddings, enabling seamless transition from models/data to application.

Result: Created a publicly available web application (https://modelscope.ai/studios/Major-TOM/EarthEmbeddingExplorer) that democratizes access to Earth embeddings, provides comprehensive hands-on guidance, and demonstrates practical workflows for deriving scientific insights from retrieval results.

Conclusion: EarthEmbeddingExplorer successfully bridges the gap between academic Earth observation assets and practical applications, empowering researchers to leverage state-of-the-art models and data archives for real-world analysis through an accessible, interactive platform.
...
TL;DR: NeoNet is an end-to-end 3D deep learning framework for noninvasive prediction of perineural invasion in cholangiocarcinoma, integrating segmentation, synthetic data generation, and classification modules to overcome limited imaging criteria.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of noninvasive diagnosis of perineural invasion (PNI) in cholangiocarcinoma, which lacks clear imaging criteria, while minimizing invasive diagnostic procedures that carry risks of patient injury and infection.

Method: Integrated three-module framework: (1) NeoSeg with Tumor-Localized ROI Crop algorithm for segmentation, (2) NeoGen using 3D Latent Diffusion Model with ControlNet conditioned on anatomical masks to generate synthetic patches and balance dataset to 1:1 ratio, (3) NeoCls with PNI-Attention Network (PattenNet) using frozen LDM encoder and specialized 3D Dual Attention Blocks to detect subtle PNI patterns.
...
Conclusion: The proposed NeoNet framework demonstrates effective noninvasive prediction of perineural invasion in cholangiocarcinoma through integrated deep learning modules, addressing data imbalance and subtle pattern detection challenges without relying on predefined image features.
...
TL;DR: A novel context-driven HTR framework enables inference-time writer adaptation using only a few examples without parameter updates, outperforming writer-independent models.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art HTR models struggle with underrepresented handwriting styles; existing writer adaptation methods require computationally expensive fine-tuning or parameter updates with gradient computation and backpropagation.

Method: Propose a context-driven HTR framework inspired by multimodal in-context learning, enabling inference-time writer adaptation using few examples without parameter updates; design a compact 8M-parameter CNN-Transformer; combine context-driven and standard OCR training strategies.
...
Conclusion: The proposed context-driven framework effectively handles unseen and atypical writers through few-shot in-context adaptation, offering computational efficiency and eliminating the need for careful hyperparameter tuning associated with traditional adaptation methods.
...
TL;DR: FedDBP is a federated prototype learning method that addresses feature fidelity-discriminability trade-off and single global prototype limitation through dual-branch feature projection and personalized prototype fusion.


<details>
  <summary>Details</summary>
Motivation: Existing federated prototype learning methods fail to balance feature fidelity and discriminability, and are limited by using only a single global prototype, which hinders performance in heterogeneous federated learning scenarios.

Method: FedDBP uses a client-side Dual-Branch feature projector with L2 alignment and contrastive learning for balanced feature fidelity and discriminability, and a server-side Personalized global prototype fusion approach using Fisher information to identify important channels of local prototypes.

Result: Extensive experiments demonstrate FedDBP's superiority over ten existing advanced federated learning methods, showing improved performance in heterogeneous federated learning settings.

Conclusion: FedDBP effectively addresses the limitations of existing federated prototype learning methods by balancing feature fidelity and discriminability while enabling personalized prototype fusion, making it a superior approach for heterogeneous federated learning.
...
TL;DR: The paper proposes a square superpixel generation method using multi-scale square blocks to overcome irregular shape limitations, enabling parallel processing and integration with deep learning architectures like GNNs and ViTs.


<details>
  <summary>Details</summary>
Motivation: Existing superpixel algorithms produce irregularly shaped regions that are not well-aligned with regular operators like convolutions, limiting parallel implementation and end-to-end optimization in deep learning pipelines.

Method: Develops a square superpixel approach using multi-scale square blocks, computes purity scores based on pixel-intensity similarity for each block, and selects high-quality blocks to approximate superpixels while avoiding irregular shape difficulties.

Result: Experimental results on downstream tasks demonstrate consistent performance improvements, validating the effectiveness of the proposed square superpixel method.

Conclusion: Square superpixels can be readily integrated as graph nodes in GNNs or tokens in ViTs, facilitating multi-scale information aggregation and structured visual representation while enabling efficient parallel processing and learnable feature extraction.
...
TL;DR: VecAttention introduces vector-wise sparse attention for video models, leveraging vertical-vector sparsity patterns to achieve better accuracy-efficiency trade-offs than existing sparse attention methods.
...
Conclusion: VecAttention provides an effective framework for efficient video modeling by exploiting vertical-vector sparsity patterns, offering superior accuracy-efficiency trade-offs for long-context video tasks.
...
TL;DR: AR system using HoloLens 2 enables precise intraoperative relocalization of positive tumor margins in head and neck cancer surgery through markerless surface registration, reducing guidance errors from 14.2 mm to 3.2 mm.


<details>
  <summary>Details</summary>
Motivation: Positive surgical margins are common in head and neck squamous cell carcinoma, but current intraoperative re-resection methods are imprecise because margin locations are typically communicated verbally from pathology, leading to inaccurate guidance.
...
Result: Markerless registration achieved target registration errors comparable to marker-based baseline (median 1.8 mm vs. 1.7 mm; maximum < 4 mm). In margin relocalization tasks, AR guidance reduced error from 14.2 mm (verbal guidance) to 3.2 mm, with all AR localizations within 5 mm error.
...
TL;DR: Two-stage nighttime dehazing framework combining transmittance correction with structure-texture layered optimization in YUV space to address multiple degradation factors simultaneously.


<details>
  <summary>Details</summary>
Motivation: Nighttime hazy images suffer from severe quality degradation including low visibility, color distortion, and reduced contrast due to atmospheric scattering, particle absorption, and non-uniform artificial illumination. Existing methods only address subsets of these issues without comprehensive joint optimization.

Method: Two-stage framework: 1) Transmittance correction with boundary-constrained initial maps, region-adaptive compensation/normalization, quadratic Gaussian filtering in YUV for atmospheric light estimation, improved nighttime imaging model. 2) STAR-YUV decomposition separating image into structure/texture layers, gamma correction and MSRCR for structure enhancement, Laplacian-of-Gaussian for texture enhancement, two-phase fusion strategy with nonlinear Retinex-based fusion and linear blending.

Result: The paper proposes a comprehensive framework that jointly addresses multiple degradation factors in nighttime dehazing, though specific quantitative results are not provided in the abstract.

Conclusion: The proposed two-stage framework effectively addresses the full spectrum of nighttime hazy image degradation factors through integrated transmittance correction and structure-texture optimization, offering a more comprehensive solution than existing methods.
...
TL;DR: A unified framework for multi-task GenAI inference on edge devices using shared models with LoRA weights as runtime inputs and QUAD quantization-aware training.


<details>
  <summary>Details</summary>
Motivation: Deploying Large Vision Models (LVMs) for GenAI tasks on resource-constrained mobile devices is challenging due to high memory/compute requirements. Existing mobile deployment pipelines compile separate model binaries for each LoRA + foundation model, causing redundant storage and increased runtime overhead.

Method: 1) Treat LoRA weights as runtime inputs rather than embedding them into compiled model graph, enabling dynamic task switching without recompilation. 2) Introduce QUAD (Quantization with Unified Adaptive Distillation), a quantization-aware training strategy that aligns multiple LoRA adapters under a shared quantization profile. 3) Implement lightweight runtime stack compatible with mobile NPUs.

Result: Experimental results demonstrate up to 6x reduction in memory footprint and 4x latency improvements while maintaining high visual quality across multiple GenAI tasks. The system works across multiple chipsets.

Conclusion: The proposed unified framework enables efficient multi-task GenAI inference on edge devices through dynamic LoRA weight handling and shared quantization profiles, significantly reducing memory and latency overhead while preserving task performance.
...
TL;DR: A pose-aware generative framework with pose estimation supervision enhances anatomical accuracy and cultural fidelity in generating Bharatanatyam dance postures, outperforming standard cGAN and diffusion models.


<details>
  <summary>Details</summary>
Motivation: Preserving intangible cultural dances like Bharatanatyam requires accurate digital generation of codified adavus and key postures while maintaining anatomical and stylistic integrity for documentation, analysis, and global dissemination.

Method: Proposed pose-aware generative framework integrated with pose estimation module, using keypoint-based loss and pose consistency constraints. Evaluated four configurations: standard cGAN, cGAN with pose supervision, conditional diffusion, and conditional diffusion with pose supervision, all conditioned on key posture class labels with geometric structure optimization.

Result: Pose supervision significantly enhances quality, realism, and authenticity of generated Bharatanatyam postures in both cGAN and conditional diffusion settings, aligning generated poses with ground-truth keypoint structures and promoting cultural fidelity.

Conclusion: The framework provides a scalable approach for digital preservation, education, and dissemination of traditional dance forms, enabling high-fidelity generation without compromising cultural precision, with code publicly available.
...
TL;DR: The paper proposes AMDiT (Adaptive Margin Discrepancy Training) to improve EmoDC (Emotion Diffusion Classifier) for facial expression recognition, enhancing both accuracy and adversarial robustness over standard diffusion training and discriminative classifiers.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning-based FER methods rely on discriminative classifiers that learn shortcuts and are vulnerable to distribution shifts. While diffusion models offer enhanced robustness, standard training fails to penalize incorrect categorical descriptions, leading to suboptimal performance.

Method: Introduces Emotion Diffusion Classifier (EmoDC) using conditional generative diffusion model. Proposes Adaptive Margin Discrepancy Training (AMDiT) that dynamically adjusts margins for each sample to enforce minimum margin between noise-prediction errors for correct vs. incorrect categories, improving discriminative capability.

Result: AMDiT significantly improves EmoDC accuracy over base model with standard denoising diffusion training across RAF-DB basic/compound subsets, SFEW-2.0, and AffectNet in 100-step evaluations. EmoDC outperforms state-of-the-art discriminative classifiers in robustness against noise and blur.

Conclusion: The proposed AMDiT effectively addresses limitations of fixed-margin training by adapting to varying difficulty of noise prediction across images, resulting in improved FER performance and enhanced adversarial robustness compared to existing methods.
...
TL;DR: FlowID is an identity-preserving facial reconstruction method that uses image generation models to remove violent death artifacts from faces while preserving identity information for identification purposes.


<details>
  <summary>Details</summary>
Motivation: Traditional image editing tools for processing post-mortem photos of violent death victims are time-consuming and produce suboptimal results, while current identification processes cannot be immediately carried out. There is a need for efficient, identity-preserving facial reconstruction to aid in victim identification.

Method: FlowID combines single-image fine-tuning of generative models to adapt to out-of-distribution injured faces with attention-based masking that localizes edits to damaged regions while preserving identity-critical features. This enables artifact removal while retaining identity information.

Result: FlowID outperforms state-of-the-art open-source methods while maintaining low memory requirements, making it suitable for local deployment without compromising data privacy. The authors also introduce InjuredFaces, a novel benchmark for identity-preserving facial reconstruction under severe facial damage.

Conclusion: FlowID provides an effective solution for identity-preserving facial reconstruction of violent death victims, enabling better identification processes while maintaining privacy through local deployment capabilities.
...
TL;DR: Video-Oasis is a diagnostic suite that systematically evaluates existing video understanding benchmarks, revealing that 54% of samples are solvable without visual/temporal context and SOTA models perform near random guessing on remaining samples, providing guidelines for future benchmark construction and model evaluation.


<details>
  <summary>Details</summary>
Motivation: Current video understanding benchmarks lack systematic evaluation of what constitutes genuine video understanding, making it difficult to attribute performance gains to visual perception, linguistic reasoning, or knowledge priors rather than superficial cues.

Method: Developed Video-Oasis, a sustainable diagnostic suite that systematically evaluates existing video understanding benchmarks by analyzing whether samples require visual input or temporal context, and testing state-of-the-art models on genuinely challenging samples.

Result: Found that 54% of existing benchmark samples are solvable without visual input or temporal context, and on the remaining genuinely challenging samples, state-of-the-art models perform barely above random guessing levels.

Conclusion: Video-Oasis provides practical guidelines for benchmark construction and rigorous evaluation of video understanding architectures, serving as a standard for future research to ensure models develop genuine spatio-temporal reasoning capabilities.
...
TL;DR: Unify-Agent: A unified multimodal agent that reframes image generation as an agentic pipeline for world-grounded image synthesis, addressing limitations of frozen parametric knowledge in handling long-tail and knowledge-intensive concepts.


<details>
  <summary>Details</summary>
Motivation: Current unified multimodal models rely primarily on frozen parametric knowledge, making them struggle with real-world image generation involving long-tail and knowledge-intensive concepts. The authors are inspired by the success of agents on real-world tasks to address this limitation.

Method: Unify-Agent reframes image generation as an agentic pipeline consisting of prompt understanding, multimodal evidence searching, grounded recaptioning, and final synthesis. The authors construct a tailored multimodal data pipeline and curate 143K high-quality agent trajectories for world-grounded image synthesis to supervise the full agentic generation process.

Result: Extensive experiments show that Unify-Agent substantially improves over its base unified model across diverse benchmarks and real-world generation tasks, while approaching the world knowledge capabilities of the strongest closed-source models. The authors introduce FactIP, a benchmark covering 12 categories of culturally significant and long-tail factual concepts requiring external knowledge grounding.

Conclusion: As an early exploration of agent-based modeling for world-grounded image synthesis, this work highlights the value of tightly coupling reasoning, searching, and generation for reliable open-world agentic image synthesis.
...
TL;DR: BigEarthNet.txt is a large-scale multi-sensor remote sensing dataset with 464k co-registered Sentinel-1/Sentinel-2 images and 9.6M text annotations, designed to advance instruction-driven vision-language learning in Earth observation.


<details>
  <summary>Details</summary>
Motivation: Vision-language models perform well on computer vision but poorly on remote sensing due to lack of large-scale, diverse RS image-text datasets with rich textual annotations. Existing datasets have limited sensor diversity, short captions, and minimal annotation types.

Method: Created BigEarthNet.txt dataset with 464,044 co-registered Sentinel-1 SAR and Sentinel-2 multispectral images, featuring 9.6M text annotations including: geographically anchored captions describing LULC classes and spatial relations, visual question answering pairs, and referring expression detection instructions. Conducted comparative statistical analysis and established manually-verified benchmark split.

Result: BigEarthNet.txt surpasses existing RS image-text datasets in textual richness and annotation variety. Vision-language models show limitations on complex LULC tasks, but fine-tuning with BigEarthNet.txt yields consistent performance gains across all evaluated tasks.

Conclusion: BigEarthNet.txt addresses critical data limitations in remote sensing vision-language learning, providing a comprehensive benchmark for evaluating and improving VLMs on Earth observation tasks, with demonstrated effectiveness in enhancing model performance through fine-tuning.
...
TL;DR: SSFL for diatom classification under decentralized heterogeneous data with partially overlapping class sets; introduces PreDi partitioning scheme to disentangle label-space heterogeneity into prevalence and disparity dimensions; proposes PreP-WFL to strengthen rare-class representations.


<details>
  <summary>Details</summary>
Motivation: Label-scarce visual classification under decentralized and heterogeneous data is challenging, especially with partially overlapping class sets. Existing SSFL studies assume same data heterogeneity pattern throughout pre-training and fine-tuning, and current partitioning schemes fail to generate pure partially class-disjoint settings, limiting controllable simulation of real-world label-space heterogeneity.

Method: 1) Introduce SSFL for diatom classification as representative real-world instance; 2) Systematically investigate stage-specific data heterogeneity (cross-site variation in unlabeled data volume during pre-training and label-space misalignment during fine-tuning); 3) Propose PreDi partitioning scheme that disentangles label-space heterogeneity into orthogonal dimensions (class Prevalence and class-set size Disparity); 4) Develop PreP-WFL (Prevalence-based Personalized Weighted Federated Learning) to adaptively strengthen rare-class representations in low-prevalence scenarios.

Result: SSFL consistently outperforms local-only training under both homogeneous and heterogeneous settings. Pronounced heterogeneity in unlabeled data volume is associated with improved representation pre-training. Under label-space heterogeneity, prevalence dominates performance while disparity has smaller effect. PreP-WFL effectively mitigates degradation, with gains increasing as prevalence decreases.

Conclusion: The findings provide mechanistic basis for characterizing label-space heterogeneity in decentralized recognition systems. The PreDi scheme enables separate analysis of prevalence and disparity effects, and PreP-WFL effectively addresses performance degradation in low-prevalence scenarios, advancing SSFL for real-world heterogeneous data settings.
...
TL;DR: MacTok is a masked augmenting 1D continuous tokenizer that prevents posterior collapse in variational image tokenizers by using random and semantic masking with representation alignment, achieving efficient high-fidelity image tokenization with only 64-128 tokens.


<details>
  <summary>Details</summary>
Motivation: Variational continuous image tokenizers with KL regularization often suffer from posterior collapse when using fewer tokens, where the encoder fails to encode informative features into the compressed latent space, limiting their efficiency and representation quality.

Method: MacTok combines random masking to regularize latent learning and DINO-guided semantic masking to emphasize informative image regions, forcing robust semantic encoding from incomplete visual evidence. It uses global and local representation alignment to preserve discriminative information in a highly compressed 1D latent space.
...
Conclusion: Masking and semantic guidance together effectively prevent posterior collapse in continuous image tokenizers, enabling efficient high-fidelity tokenization with dramatically reduced token counts while maintaining competitive generation quality.
...
TL;DR: DynMask introduces a Motion Spectral Descriptor (MSD) to measure local dynamic complexity in motion, using it to guide masking, attention, and decoding in masked motion generators for improved text-to-motion synthesis, especially on dynamically complex motions.


<details>
  <summary>Details</summary>
Motivation: Current masked generative models for text-to-motion synthesis treat motion frames uniformly during masking, attention, and decoding, which poorly matches motion's varying local dynamic complexity. These models degrade disproportionately on dynamically complex motions, and frame-wise generation error correlates strongly with motion dynamics.

Method: Introduces Motion Spectral Descriptor (MSD), a parameter-free measure of local dynamic complexity computed from short-time spectrum of motion velocity. MSD guides content-focused masking during training, provides spectral similarity prior for self-attention, and can modulate token-level sampling during iterative decoding. Built on masked motion generators as DynMask.

Result: DynMask improves motion generation most clearly on dynamically complex motions while also yielding stronger overall FID scores on HumanML3D and KIT-ML datasets compared to baseline masked motion generators.

Conclusion: Respecting local motion complexity through complexity-aware techniques like MSD is a useful design principle for masked motion generation, addressing the mismatch between uniform frame treatment and motion's varying dynamic complexity.
...
TL;DR: CutClaw is an autonomous multi-agent framework that uses multiple Multimodal Language Models to automatically edit hours-long raw footage into meaningful short videos with synchronized music, narrative consistency, and visual appeal.


<details>
  <summary>Details</summary>
Motivation: Manual video editing is time-consuming and repetitive, creating challenges for filmmakers and content creators who need to produce engaging short videos from long raw footage with proper audio alignment and narrative flow.

Method: Uses a multi-agent system with MLLMs: 1) hierarchical multimodal decomposition to capture fine-grained details and global structures, 2) Playwriter Agent orchestrates storytelling and anchors visual scenes to musical shifts, 3) Editor and Reviewer Agents collaboratively optimize final cuts based on aesthetic and semantic criteria.

Result: CutClaw significantly outperforms state-of-the-art baselines in generating high-quality, rhythm-aligned videos, demonstrating effective autonomous video editing capabilities.

Conclusion: The framework successfully automates the video editing process by leveraging multiple specialized MLLM agents to transform raw footage into polished short videos with synchronized music and narrative coherence, addressing the challenges of manual editing.
...
<details>
  <summary>Details</summary>
Motivation: Vision-based surgical skill assessment faces challenges due to high annotation costs and poor generalization of existing regression models to new surgical tasks and environments, while substantial unlabeled video data is available, motivating unsupervised domain adaptation methods.

Method: CoRe-DA framework uses contrastive regression-based adaptation with relative-score supervision and target-domain self-training to learn domain-invariant representations for surgical skill assessment across domain shifts.
...
Conclusion: CoRe-DA enables scalable surgical skill assessment with reliable cross-domain generalization where existing methods underperform, establishing the first benchmark for unsupervised domain adaptation in surgical skill assessment regression.
...
TL;DR: Proposed clinical DVH metric loss (CDM loss) with bit-mask ROI encoding for H&N 3D dose prediction, improving clinical alignment and training efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for 3D dose prediction use voxel-wise regression losses that poorly align with clinical plan evaluation based on DVH metrics, creating a need for clinically guided loss formulations.

Method: Developed CDM loss incorporating differentiable D-metrics and surrogate V-metrics with lossless bit-mask ROI encoding to improve training efficiency; evaluated on 174 H&N patients using temporal split (137 training, 37 testing).
...
Conclusion: Direct optimization of clinically used DVH metrics enables 3D dose predictions better aligned with clinical criteria than conventional supervision; CDM loss with efficient ROI bit-mask encoding provides practical, scalable framework for H&N dose prediction.
...
TL;DR: SkeletonContext is a prompt-based framework for zero-shot skeleton-based action recognition that enriches skeletal motion with language-driven contextual semantics to bridge the gap between skeleton features and semantic representations.


<details>
  <summary>Details</summary>
Motivation: Existing methods align skeleton features with textual embeddings but lack contextual cues (like objects involved in actions), creating a gap between skeleton and semantic representations that makes it difficult to distinguish visually similar actions.

Method: Proposes SkeletonContext with two key modules: 1) Cross-Modal Context Prompt Module that uses a pretrained language model to reconstruct masked contextual prompts guided by LLMs, transferring linguistic context to the skeleton encoder; 2) Key-Part Decoupling Module to decouple motion-relevant joint features for robust action understanding without explicit object interactions.

Result: Extensive experiments on multiple benchmarks show SkeletonContext achieves state-of-the-art performance under both conventional and generalized zero-shot settings, validating its effectiveness in reasoning about context and distinguishing fine-grained, visually similar actions.

Conclusion: SkeletonContext successfully bridges the gap between skeleton and semantic representations by incorporating language-driven contextual semantics, enabling better cross-modal alignment and improved zero-shot action recognition performance.
...
TL;DR: CANVAS is a comprehensive benchmark dataset for subcellular-resolution whole mouse brain light-sheet fluorescence microscopy, providing high-resolution 3D data with six cell-type markers, extensive cell annotations, and a leaderboard to accelerate development of scalable analysis methods for petabyte-scale neuroimaging data.


<details>
  <summary>Details</summary>
Motivation: Recent advances in intact tissue processing and light-sheet fluorescence microscopy enable unprecedented subcellular-resolution whole-brain 3D imaging, but the petabyte-scale data lacks scalable processing methods and existing computer vision models struggle to generalize to this specialized neuroimaging data.

Method: The authors present CANVAS - a comprehensive benchmark dataset containing high-resolution whole mouse brain LSFM data with six neuronal and immune cell-type markers, along with extensive cell annotations throughout the brain and a leaderboard for method evaluation. They also demonstrate challenges in generalization by testing baseline models on existing architectures.

Result: CANVAS is established as the first and largest LSFM benchmark capturing intact mouse brain tissue at subcellular level with extensive cell annotations. Baseline models show poor generalization due to heterogeneity in cellular morphology across phenotypes and anatomical locations.

Conclusion: CANVAS provides a crucial resource to accelerate development of scalable data processing methods and foundational models for subcellular-resolution whole-brain microscopy, addressing the current bottleneck in analyzing petabyte-scale neuroimaging data and enabling more accurate biological interpretation.
...
TL;DR: The paper investigates how skin tone affects lesion segmentation in dermatology AI, finding that low lesion-skin contrast (not global skin tone) is the key driver of segmentation errors, suggesting fairness improvements should focus on handling low-contrast lesions.


<details>
  <summary>Details</summary>
Motivation: While fairness concerns regarding skin tone have been studied for lesion classification, the influence of skin tone on the segmentation stage remains under-quantified and is frequently assessed using coarse, discrete skin tone categories.

Method: Evaluated three segmentation architectures (UNet, DeepLabV3 with ResNet50 backbone, and DINOv2) on two public dermoscopic datasets (HAM10000 and ISIC2017). Introduced continuous pigment analysis using pixel-wise ITA values as distributions. Used Wasserstein distances between within-image distributions for skin-only, lesion-only, and whole-image regions to quantify lesion-skin contrast and relate it to segmentation performance across multiple metrics.

Result: Global skin tone metrics (Fitzpatrick grouping or mean ITA) show weak association with segmentation quality. Low lesion-skin contrast is consistently associated with larger segmentation errors in models, indicating that boundary ambiguity and low contrast are key drivers of failure.

Conclusion: Fairness improvements in dermoscopic segmentation should prioritize robust handling of low-contrast lesions, and distribution-based pigment measures provide a more informative audit signal than discrete skin-tone categories.
...
TL;DR: FED-Bench: A comprehensive benchmark for facial expression editing with 747 high-quality triplets, FED-Score evaluation protocol, benchmarking of 18 models, and a 20k+ training dataset to address limitations in current facial expression editing evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing facial expression editing benchmarks lack high-quality facial images and corresponding editing instructions, while current evaluation metrics exhibit systemic biases favoring lazy or overfit editing, creating a need for more rigorous testing and accurate evaluation.

Method: 1) Constructed benchmark of 747 triplets (original image, editing instruction, ground-truth) through cascaded scalable pipeline; 2) Introduced FED-Score evaluation protocol with three dimensions: Alignment (instruction following), Fidelity (image quality/identity preservation), and Relative Expression Gain (magnitude of expression changes); 3) Benchmarked 18 image editing models; 4) Provided 20k+ in-the-wild facial training set using scalable benchmark engine.

Result: Current approaches struggle to simultaneously achieve high fidelity and accurate expression manipulation, with fine-grained instruction following identified as the primary bottleneck. Fine-tuning baseline models with the 20k+ training set achieved significant performance gains.

Conclusion: FED-Bench addresses critical gaps in facial expression editing evaluation through comprehensive benchmarking, bias-mitigating evaluation protocol, and scalable data generation, revealing current limitations and providing resources for future model development.
...
### [86] [End-to-End Image Compression with Segmentation Guided Dual Coding for Wind Turbines](https://arxiv.org/abs/2603.29927)
*Raül Pérez-Gonzalo,Andreas Espersen,Søren Forchhammer,Antonio Agudo*

Main category: cs.CV

TL;DR: An end-to-end deep learning framework for wind turbine inspection that jointly performs blade segmentation and dual-mode compression, preserving high fidelity in blade regions while aggressively compressing background.


<details>
  <summary>Details</summary>
Motivation: Transferring large volumes of high-resolution wind turbine inspection images creates bottlenecks in defect assessment. Efficient coding must preserve blade region fidelity while compressing background.

Method: Proposes an integrated framework with: (1) BU-Netv2+P segmentation network with CRF-regularized loss for precise blade localization, (2) hyperprior-based autoencoder for lossy compression, (3) extended bits-back coder with hierarchical models for lossless blade reconstruction, and (4) ROI framework that removes sequential dependency in bits-back coding by reusing background-coded bits.

Result: Demonstrates superior compression performance and efficiency on large-scale wind turbine dataset, offering practical solution for automated inspections while ensuring defect detection is not compromised.

Conclusion: First fully integrated learning-based ROI codec combining segmentation, lossy, and lossless compression, enabling parallelized dual-mode compression for wind turbine inspection applications.
...
### [87] [Better than Average: Spatially-Aware Aggregation of Segmentation Uncertainty Improves Downstream Performance](https://arxiv.org/abs/2603.29941)
...
TL;DR: Systematic analysis of pixel-wise uncertainty aggregation strategies for segmentation UQ, proposing novel spatial-aware methods and a meta-aggregator for robust OoD/failure detection across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Current uncertainty quantification (UQ) in image segmentation lacks systematic study of aggregation strategies, leading to inconsistent practices and suboptimal performance in downstream tasks like OoD and failure detection.

Method: 1) Formal analysis of existing aggregation strategies' properties and limitations; 2) Proposal of novel spatial-aware aggregation methods; 3) Benchmarking on ten diverse datasets; 4) Development of a meta-aggregator integrating multiple strategies.

Result: Spatial-aware aggregators outperform traditional methods (like Global Average) in both OoD and failure detection tasks. Performance varies with dataset characteristics, but the proposed meta-aggregator achieves robust performance across all datasets.

Conclusion: Aggregation strategy selection significantly impacts downstream task performance in segmentation UQ. Spatial-aware methods are generally superior, and a meta-aggregator approach provides dataset-agnostic robustness, establishing best practices for uncertainty aggregation.
...
TL;DR: Synthetic data significantly improves hand-object interaction detection in egocentric vision, especially when real labeled data is scarce, with alignment between synthetic and real data being crucial for effectiveness.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of real labeled data for hand-object interaction detection in egocentric images and explore how synthetic data can bridge this gap.

Method: Extensive experimentation on VISOR, EgoHOS, and ENIGMA-51 datasets using synthetic data with only 10% of real labeled data, plus systematic study of synthetic-real alignment across objects, grasps, and environments.

Result: Significant improvements in Overall AP: +5.67% on VISOR, +8.24% on EgoHOS, and +11.69% on ENIGMA-51 compared to models trained exclusively on real data.

Conclusion: Synthetic data effectively improves HOI detection when real labeled data is limited, with better synthetic-real alignment leading to consistent performance gains; the work releases a new data generation pipeline and HOI-Synth benchmark.
...
TL;DR: A novel view synthesis method for dynamic scenes from monocular videos that combines recurrent mapping with plane sweeps to achieve fine-grained 6-DOF camera control, outperforming both scene-specific Gaussian Splatting and diffusion-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods for synthesizing novel views from monocular videos of dynamic scenes have limitations: scene-specific methods with explicit motion priors fail in highly dynamic regions where multi-view information is scarce, while diffusion-based approaches suffer from geometric inconsistencies and both require substantial computational resources.

Method: Adapts generalizable models for static novel view synthesis to dynamic inputs with two key components: (1) a recurrent loop enabling unbounded and asynchronous mapping between input and target videos, and (2) efficient use of plane sweeps over dynamic inputs to disentangle camera and scene motion for fine-grained six-degrees-of-freedom camera controls.

Result: The model was trained and evaluated on UCSD dataset and Kubric-4D-dyn (a new monocular dynamic dataset with longer, higher resolution sequences and more complex dynamics). It outperformed four Gaussian Splatting-based scene-specific approaches and two diffusion-based approaches in reconstructing fine-grained geometric details across both static and dynamic regions.

Conclusion: The proposed approach successfully addresses limitations of existing methods by combining recurrent mapping with plane sweeps, achieving superior geometric consistency and fine-grained camera control for dynamic novel view synthesis from monocular videos.
...
TL;DR: SHIFT is a training-free attack that exploits the trajectory recovery vulnerability in diffusion-based watermarking by using stochastic resampling to deflect generative trajectories, achieving 95-100% attack success rates without compromising visual quality.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based watermarking methods rely on faithful reconstruction of diffusion trajectories for verification, creating a fundamental vulnerability that can be exploited by attackers.

Method: SHIFT uses stochastic diffusion resampling to deflect the generative trajectory in latent space, statistically decoupling the reconstructed image from the original watermark-embedded trajectory while preserving visual quality and semantic consistency.

Result: Extensive experiments on nine representative watermarking methods across noise-space, frequency-domain, and optimization-based paradigms show SHIFT achieves 95%--100% attack success rates with minimal semantic quality loss.

Conclusion: SHIFT exposes a fundamental vulnerability in diffusion-based watermarking that relies on trajectory recovery, demonstrating that current methods are insufficiently robust against training-free attacks that exploit this common weakness.
...
TL;DR: TSHA is a comprehensive benchmark for indoor safety hazards assessment addressing limitations of existing benchmarks through diverse real-world data and rigorous evaluation protocols.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for vision-language models in indoor safety assessment suffer from three key limitations: reliance on synthetic datasets creating domain gaps, oversimplified safety tasks limiting generalization, and absence of rigorous evaluation protocols for complex scenarios.

Method: Introduced TSHA benchmark comprising 81,809 training samples from four complementary sources: existing indoor datasets, internet images, AIGC images, and newly captured images. Includes challenging test set with 1707 samples including videos and panoramic images with multiple hazards.
...
Conclusion: TSHA benchmark addresses critical limitations in indoor safety assessment, demonstrates substantial performance improvements for trained models, and highlights the importance of comprehensive real-world datasets for robust safety hazard evaluation.
...
TL;DR: IQPIR introduces Image Quality Prior from NR-IQA models to guide restoration toward perceptually optimal outputs, addressing limitations of ground-truth supervision that converges to average quality rather than maximum perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Existing image restoration methods rely on ground-truth supervision, but GT images can have inconsistent perceptual fidelity, causing models to converge to average quality rather than achieving the highest perceptual quality attainable.

Method: Proposes IQPIR framework with three key mechanisms: (1) quality-conditioned Transformer using NR-IQA scores as conditioning signals, (2) dual-branch codebook structure disentangling common and HQ-specific features, and (3) discrete representation-based quality optimization to mitigate over-optimization in continuous latent spaces.

Result: Extensive experiments on real-world image restoration demonstrate superior performance over cutting-edge methods and show generalizable quality-guided enhancement for existing methods.

Conclusion: IQPIR effectively addresses limitations of GT supervision by leveraging NR-IQA-derived quality priors to guide restoration toward perceptually optimal outputs, offering a plug-and-play enhancement compatible with existing architectures.
...
<details>
  <summary>Details</summary>
Motivation: Public safety applications require timely detection of violent behavior, but current automated video analysis faces latency, privacy, and resource limitations, especially in edge-computing environments.

Method: System-level comparison of skeleton-based motion analysis (privacy-aware, low overhead) and vision-language models (contextual understanding, zero-shot reasoning) implemented on GPU-enabled edge device with demonstrator-based evaluation.

Result: Results show complementary strengths: skeleton-based detection offers fast, privacy-aware monitoring while vision-language models provide semantic understanding for complex situations, motivating a hybrid architecture.
...
TL;DR: MAPLE is a hierarchical multi-label classification framework for remote sensing that addresses multi-path label dependencies through hierarchical semantic initialization, graph-based encoding, and adaptive multi-modal fusion with level-aware objectives.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchical multi-label classification approaches struggle with multi-path settings where images may activate multiple taxonomic branches, leading to underutilization of hierarchical information in remote sensing applications.

Method: MAPLE integrates three components: (1) hierarchical semantic initialization from graph-aware textual descriptions, (2) graph-based structure encoding via graph convolutional networks (GCNs), and (3) adaptive multi-modal fusion that dynamically balances semantic priors and visual evidence. It also uses an adaptive level-aware objective that automatically selects appropriate losses per hierarchy level.
...
Conclusion: MAPLE effectively and efficiently models hierarchical semantics for Earth observation by addressing multi-path label dependencies through its integrated framework of semantic initialization, graph encoding, and adaptive fusion with level-aware optimization.
...
TL;DR: A multi-feature fusion framework combining MSCN, CLIP embeddings, and MLBP features achieves superior and more consistent performance for detecting synthetic images across diverse generative models compared to single-feature approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional single-feature detection methods lack robustness against diverse and evolving generative AI models that produce increasingly realistic synthetic images, necessitating more comprehensive approaches.

Method: A multi-feature fusion framework that combines complementary cues from three distinct spaces: MSCN features for low-level statistical deviations, CLIP embeddings for high-level semantic coherence, and Multi-scale Local Binary Patterns for mid-level texture anomalies.

Result: Extensive experiments on four benchmark datasets show individual feature spaces exhibit significant performance variability across different generators, while fusion of all three representations yields superior and more consistent performance, particularly in challenging mixed-model scenarios, outperforming state-of-the-art methods.
...
TL;DR: SceneTeract is a framework for verifying 3D scene functionality under agent-specific constraints by coupling semantic reasoning with geometric checks, enabling evaluation of synthetic environments and VLMs' affordance reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Current embodied AI systems lack robust methods to assess functional affordances of 3D environments for diverse users, creating a gap between semantic understanding and physical feasibility in interactive scenes.

Method: SceneTeract uses a grounded verification engine that decomposes complex activities into atomic actions and validates each step against accessibility requirements (reachability, clearance, navigability) using explicit physical and geometric simulations conditioned on embodied agent profiles.

Result: The framework uncovered frequent functional failures in synthetic indoor environments and revealed systematic mismatches between semantic confidence and physical feasibility in frontier VLMs. It also successfully served as a reward engine for VLM post-training to distill geometric constraints.

Conclusion: SceneTeract bridges perception and physical reality in embodied 3D scene understanding by providing a verification suite that enables scalable assessment of scene functionality and improves VLM reasoning about physical constraints.
...
TL;DR: AutoFormBench benchmark dataset of 407 real-world forms for evaluating form element detection models, with YOLOv11 showing best performance across all element classes.
...
Method: Created AutoFormBench dataset of 407 annotated real-world forms from government, healthcare, and enterprise domains; systematically compared classical OpenCV approaches with four YOLO architectures (YOLOv8, YOLOv11, YOLOv26-s, YOLOv26-l) for localizing and classifying fillable form elements (checkboxes, input lines, text boxes) across diverse PDF documents.

Result: YOLOv11 demonstrates consistently superior performance in both F1 score and Jaccard accuracy across all element classes (checkboxes, input lines, text boxes) and tolerance levels.

Conclusion: The AutoFormBench benchmark provides a valuable resource for form element detection research, with YOLOv11 emerging as the most effective architecture for this task across diverse document types and domains.
...
TL;DR: A robust multi-class handwritten digit classification framework combining diffusion-driven feature denoising with hybrid features (NNMF + CNN) for improved noise and adversarial attack resistance.


<details>
  <summary>Details</summary>
Motivation: To develop a robust classification framework for handwritten digits that maintains effectiveness against noise and adversarial attacks, inspired by previous work on brain tumor classification.

Method: 1) Convert images to interpretable exemplifications using Nonnegative Matrix Factorization (NNMF). 2) Extract deep features using CNN. 3) Combine into hybrid representation. 4) Apply diffusion operation in feature space by adding Gaussian noise. 5) Train feature denoiser network to reverse noise and reconstruct clean features. 6) Use denoised features for multi-class classification.

Result: The diffusion-based hybrid model outperforms CNN baseline models in both baseline and adversarial settings (evaluated with AutoAttack), demonstrating effectiveness and robustness while maintaining strong classification performance.

Conclusion: Feature-level diffusion defense provides reliable multi-class handwritten digit classification with enhanced robustness to noise and adversarial attacks, validating the effectiveness of the hybrid approach combining NNMF and CNN features with diffusion denoising.
...
TL;DR: Synthetic fractal data can effectively train DL models for dynamic MRI reconstruction, performing comparably to cardiac MRI-trained models while avoiding clinical dataset limitations.


<details>
  <summary>Details</summary>
Motivation: To overcome privacy, licensing, and availability limitations of cardiac MR training datasets by developing an alternative using synthetically generated fractal data for training deep learning reconstruction models.

Method: Generated training dataset using quaternion Julia fractals to create 2D+time images, simulated multi-coil MRI acquisition to produce paired fully sampled and radially undersampled k-space data, trained 3D UNet deep artifact suppression model on fractal data (F-DL), compared with identical model trained on cardiac MRI data (CMR-DL), evaluated on prospectively acquired radial real-time cardiac MRI from 10 patients, compared against compressed sensing and low-rank deep image prior methods.

Result: No significant difference in qualitative ranking between F-DL and CMR-DL (p=0.9), both outperformed CS and LR-DIP (p<0.001). Ventricular volumes and function from F-DL were similar to CMR-DL with no significant bias and acceptable limits of agreement compared to reference cine imaging. LR-DIP had significant bias (p=0.016) and wider limits of agreement.

Conclusion: DL models trained using synthetic fractal data can reconstruct real-time cardiac MRI with image quality and clinical measurements comparable to models trained on true cardiac MRI data. Fractal training data provide an open, scalable alternative to clinical datasets and may enable development of more generalizable DL reconstruction models for dynamic MRI.
...
TL;DR: AiS is a two-stage generative framework that separates structural abstraction from visual stylization, enabling deeper artistic style transfer beyond surface appearance by first creating an abstracted structural proxy then rendering it in the target style.


<details>
  <summary>Details</summary>
Motivation: Conventional style transfer methods preserve input geometry and struggle to capture deeper abstraction behaviors in artistic styles, especially for illustrative and nonphotorealistic styles that involve deliberate reinterpretation of structure rather than mere texture/color changes.

Method: Two-stage framework: 1) Derives intermediate abstraction proxy that reinterprets target's structure according to style's abstraction logic, capturing semantic structure while relaxing geometric fidelity; 2) Renders abstraction proxy to produce final stylized output. Both stages use shared image space analogy, learning transformations from visual exemplars without explicit geometric supervision.

Result: AiS supports wider range of stylistic transformations, improves controllability, enables more expressive stylization by decoupling abstraction from appearance and treating abstraction as an explicit, transferable process.

Conclusion: By separating structural abstraction from visual stylization and treating abstraction as an explicit process, AiS enables deeper artistic style transfer that captures the structural reinterpretation inherent in many artistic styles, going beyond conventional texture-based approaches.
...
TL;DR: Proposes a method for generating long-duration character videos with consistent multi-view appearance using compact anchor frames, addressing copy-pasting and multi-reference conflicts through superset content anchoring and RoPE-based weak conditioning.


<details>
  <summary>Details</summary>
Motivation: Digital character generation faces challenges in producing long-duration videos with consistent multi-view appearance and expressive identity. Existing approaches either provide insufficient context for identity preservation or use non-character-centric information as memory, leading to suboptimal consistency.

Method: Represents character visual attributes through compact anchor frames as stable references. Introduces two key mechanisms: 1) Superset Content Anchoring - provides intra- and extra-training clip cues to prevent duplication, and 2) RoPE as Weak Condition - encodes positional offsets to distinguish multiple anchors. Also constructs a scalable pipeline to extract anchors from massive videos.

Result: Method generates high-quality character videos exceeding 10 minutes in duration, achieving expressive identity and appearance consistency across different views, surpassing existing methods.

Conclusion: The proposed anchor-based approach effectively addresses challenges in character video generation, enabling long-duration, multi-view consistent content while maintaining expressive identity through innovative mechanisms that prevent copy-pasting and resolve multi-reference conflicts.
...
TL;DR: EC-Bench is a new benchmark for evaluating enumeration, counting, and temporal evidence grounding in long-form videos, revealing significant performance gaps between current MLLMs and humans.
...
Method: Introduces EC-Bench with 152 videos longer than 30 minutes and 1,699 queries paired with explicit evidence spans, evaluating 22 multimodal large language models on enumeration, counting, and temporal evidence grounding.

Result: Best MLLM achieves only 29.98% accuracy on Enumeration and 23.74% on Counting, while human performance reaches 78.57% and 82.97% respectively, showing strong relationships between enumeration accuracy, temporal grounding, and counting performance.
...
TL;DR: DEUS is a novel Open World Object Detection framework that improves unknown object discovery using ETF-based subspace separation and energy-based known distinction loss to address limitations in existing OWOD methods.


<details>
  <summary>Details</summary>
Motivation: Existing OWOD methods struggle with effectively learning and recognizing unknown object representations due to heavy reliance on known class predictions for unknown detection. Additionally, memory replay techniques that mitigate catastrophic forgetting of old classes often sacrifice knowledge of newly learned classes, creating interference between previous and current knowledge.

Method: DEUS consists of two key components: 1) ETF-Subspace Unknown Separation (EUS) that leverages Equiangular Tight Frame geometric properties to create orthogonal subspaces for cleaner separation between known and unknown object representations, using energies from both spaces; 2) Energy-based Known Distinction (EKD) loss that enforces separation between previous and current classifiers to minimize knowledge interference during memory replay.

Result: DEUS demonstrates outstanding performance improvements in unknown detection on OWOD benchmarks while maintaining competitive known class performance, effectively addressing the dual challenges of unknown object discovery and catastrophic forgetting.

Conclusion: DEUS successfully tackles the core challenges of Open World Object Detection by providing a more effective framework for unknown object representation learning while minimizing interference between old and new class knowledge during incremental learning.
...
TL;DR: NeuroBRIDGE: A graph neural network framework using Riemannian tangent space alignment and behavioral-conditioned Koopman dynamics to predict adolescent substance use initiation from longitudinal functional connectomes.


<details>
  <summary>Details</summary>
Motivation: Early identification of adolescents at risk for substance use initiation is crucial but challenging because existing predictors treat brain connectivity as static or cross-sectional, missing how networks evolve over time and interact with behavior.

Method: Proposed NeuroBRIDGE framework: 1) Aligns longitudinal functional connectomes in Riemannian tangent space, 2) Uses graph neural networks, 3) Incorporates dual-time attention mechanisms, 4) Couples with behavioral-conditioned Koopman dynamics to capture temporal changes.

Result: Evaluated on ABCD dataset, NeuroBRIDGE improved future substance use initiation prediction compared to relevant baselines while providing interpretable insights into neural pathways.

Conclusion: The framework refines understanding of neurodevelopmental risk for substance use initiation and informs targeted prevention strategies by capturing dynamic brain-behavior relationships over time.
...
<details>
  <summary>Details</summary>
Motivation: Current surgical VQA research focuses on static frame analysis, overlooking rich temporal semantics. Surgical video QA is further challenged by low visual contrast, knowledge-driven nature, diverse analytical needs across temporal windows, and hierarchy from basic perception to high-level intraoperative assessment.

Method: Proposes SurgTEMP framework with: (1) query-guided token selection module building hierarchical visual memory (spatial and temporal memory banks), and (2) Surgical Competency Progression (SCP) training scheme. These enable effective modeling of variable-length surgical videos while preserving procedure-relevant cues and temporal coherence.

Result: Introduces CholeVidQA-32K dataset with 32K QA pairs and 3,855 video segments (~128h) from laparoscopic cholecystectomy, organized into three-level hierarchy (Perception, Assessment, Reasoning) spanning 11 tasks. SurgTEMP achieves substantial performance improvements over state-of-the-art open-source multimodal and video LLMs in comprehensive evaluations.

Conclusion: SurgTEMP advances the state of video-based surgical VQA by effectively addressing temporal modeling challenges and supporting diverse downstream assessment tasks through its hierarchical visual memory and progressive training approach.
...
TL;DR: SurgRec is a scalable, reproducible pretraining framework for surgical video understanding with two variants (MAE and JEPA), trained on a large multi-source corpus of 10,535 videos, achieving superior performance across 16 downstream datasets compared to SSL baselines and vision-language models.
...
Method: Proposes SurgRec with two variants (SurgRec-MAE and SurgRec-JEPA) using a unified pretraining pipeline with balanced sampling. Curates a large multi-source corpus of 10,535 videos (214.5M frames) spanning endoscopy, laparoscopy, cataract, and robotic surgery. Standardizes reproducible benchmark across 16 downstream datasets with consistent data splits.
...
Conclusion: SurgRec provides a reproducible, scalable foundation for building more general surgical video models, with all code, models, and data to be publicly released to advance the surgical video understanding community.
...
TL;DR: MAGNet is a Transformer-style graph neural network that adaptively learns brain structure-function interactions by fusing structural MRI morphological features with functional connectivity from fMRI, outperforming baselines on ABCD dataset.
...
Method: MAGNet uses source-based morphometry from structural MRI to extract inter-regional morphological features and fuses them with functional network connectivity from resting-state fMRI. It employs a hybrid graph integrating direct and indirect pathways, local-global attention to refine connectivity importance, and a joint loss that simultaneously enforces cross-modal coherence and optimizes prediction objectives end-to-end.
...
TL;DR: Pilot study extends bimodal brain tumor survival framework by adding FLAIR MRI as third modality, finding preliminary evidence that trimodal early fusion may add prognostic value despite limited sample size constraints.


<details>
  <summary>Details</summary>
Motivation: Multimodal deep learning has improved prognostic accuracy for brain tumors by integrating histopathology and genomic data, but the contribution of volumetric MRI within unified survival frameworks remains unexplored.

Method: Extends bimodal framework by incorporating FLAIR MRI from BraTS2021 as third modality. Uses TCGA-GBMLGG cohort (664 patients) to evaluate three unimodal models, nine bimodal configurations, and three trimodal configurations across early, late, and joint fusion strategies.

Result: Trimodal early fusion achieves exploratory Composite Score (CS = 0.854) with ΔCS of +0.011 over bimodal baseline, though not statistically significant (p = 0.250). MRI achieves reasonable unimodal discrimination (CS = 0.755) but doesn't substantially improve bimodal pairs, while providing measurable uplift in three-way combination. All MRI experiments constrained to 19 test patients yield wide bootstrap confidence intervals (e.g., [0.400,1.000]) precluding definitive conclusions.

Conclusion: Preliminary evidence suggests a third imaging modality may add prognostic value even with limited sample sizes, and that additional modalities require sufficient multimodal context to contribute effectively.
...
TL;DR: An integrated HMD-based AR surgical navigation framework that tracks markers on patients/instruments, enables tool calibration and image-to-patient registration, achieving sub-5mm accuracy in phantom surgical use cases.


<details>
  <summary>Details</summary>
Motivation: Current HMD-AR surgical navigation systems require complex integration of multiple technologies, demanding specialized expertise that hinders scientific progress. There's a need for an adaptable, integrated framework that simplifies development of AR surgical navigation applications.

Method: Developed a configurable AR surgical navigation framework that: 1) tracks 2D patterns as reference markers on patients and instruments, 2) calibrates surgical tools using pivot and reference-based techniques, 3) performs image-to-patient registration via point-based matching and manual positioning. Evaluated on HoloLens 2 and Magic Leap 2 with phantom surgical use cases (needle insertion and rib fracture localization).

Result: Achieved mean tooltip calibration accuracy of 1 mm, registration accuracy of 3 mm, and targeting accuracy below 5 mm in both surgical use cases. Framework demonstrated adaptability across two different HMD platforms and surgical applications.

Conclusion: The framework provides an easy-to-use, configurable tool for HMD-based AR surgical navigation that can be extended to diverse surgical applications. Its public availability facilitates further research and development in this field.
...
<details>
  <summary>Details</summary>
Motivation: Existing polarization-based COD methods rely on complex visual encoders and fusion mechanisms, increasing model complexity while failing to fully explore how polarization can explicitly guide hierarchical RGB representation learning.

Method: Proposes CPGNet with: 1) lightweight polarization interaction module for joint modeling of complementary cues, 2) conditional polarization guidance mechanism for dynamic modulation of RGB features using polarization priors, 3) polarization edge-guided frequency refinement strategy to enhance high-frequency components, and 4) iterative feedback decoder for coarse-to-fine feature calibration.
...
Conclusion: CPGNet effectively addresses limitations of existing polarization-based COD approaches by introducing explicit polarization guidance mechanisms that reduce complexity while improving detection performance through better exploitation of polarization cues.
...
TL;DR: GeoCodeBench is a PhD-level benchmark for evaluating AI coding capabilities in 3D geometric vision, revealing current models (best: GPT-5 at 36.6% pass rate) struggle significantly with complex 3D scientific coding tasks.


<details>
  <summary>Details</summary>
Motivation: Current AI coding models struggle with complex 3D geometric vision tasks, limiting their potential to transform research workflows. There's a need for rigorous evaluation to measure progress toward reliable 3D scientific coding capabilities.

Method: Created GeoCodeBench by curating fill-in-the-function implementation tasks from recent 3D vision papers. Used a tool to extract candidate functions from official repositories, performed human screening to select core 3D geometric components, and generated diverse edge-case unit tests for automatic scoring.

Result: Evaluated 8 representative models; GPT-5 achieved best performance at only 36.6% pass rate. Tasks organized into General 3D capability (geometric transformations, mechanics/optics) and Research capability (novel algorithms, geometric logic). Research tasks are significantly harder. Context ablation shows full-paper inputs underperform Method-section-only inputs.

Conclusion: GeoCodeBench reveals substantial gap between current AI coding capabilities and dependable 3D geometric vision coding. The benchmark serves as a rigorous testbed for advancing from generic coding to trustworthy scientific coding in 3D vision.
...
TL;DR: Video diffusion models exhibit emergent reasoning for maze solving; they commit to motion plans early in denoising, struggle with long paths (12-step threshold), and can be improved via ChEaP (chaining with early planning) to boost accuracy on complex tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the internal planning dynamics and reasoning capabilities of video diffusion models, using 2D maze solving as a controlled testbed to reveal how these models reason during generation.

Method: Study video diffusion models on 2D maze solving tasks, analyze denoising dynamics, identify early plan commitment patterns, measure difficulty predictors, and develop ChEaP (Chaining with Early Planning) - a method that selects promising seeds based on early plans and chains sequential generations.

Result: Two key findings: 1) Early plan commitment - models commit to motion plans within first few denoising steps; 2) Path length, not obstacle density, is dominant difficulty predictor with 12-step failure threshold. ChEaP improves accuracy from 7% to 67% on long-horizon mazes and 2.5x overall on hard tasks in Frozen Lake and VR-Bench across Wan2.2-14B and HunyuanVideo-1.5 models.

Conclusion: Current video models possess deeper reasoning capabilities than previously recognized, which can be elicited more reliably with better inference-time scaling. The ChEaP approach demonstrates practical benefits for tackling complex reasoning tasks by leveraging early planning signals and chaining mechanisms.
...
TL;DR: OmniRoam is a controllable panoramic video generation framework that enables long-horizon scene wandering by leveraging panoramic representations for better completeness and global consistency compared to perspective video models.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models for scene modeling rely on perspective views that synthesize limited observations, leading to issues with completeness and global consistency. There's a need for approaches that can generate more comprehensive scene representations with better spatial and temporal coherence.

Method: Two-stage framework: 1) Preview stage uses trajectory-controlled video generation to create scene overview from input image/video; 2) Refine stage temporally extends and spatially upsamples to produce long-range, high-resolution videos. Uses panoramic video datasets (synthetic and real-world) for training.

Result: OmniRoam consistently outperforms state-of-the-art methods in visual quality, controllability, and long-term scene consistency, both qualitatively and quantitatively. Framework extensions include real-time video generation and 3D reconstruction.

Conclusion: The panoramic video generation framework effectively addresses limitations of perspective models by exploiting panoramic representation's rich coverage and inherent consistency, enabling high-fidelity world wandering with better completeness and global coherence.
...
TL;DR: OptiMer decouples data mixture ratio selection from continual pre-training by training separate models per dataset, extracting distribution vectors, and optimizing composition weights post-hoc via Bayesian optimization, achieving better performance with dramatically lower search costs.


<details>
  <summary>Details</summary>
Motivation: Continual pre-training requires fixing data mixture ratios before training begins, which is expensive to tune and can waste weeks of compute if suboptimal. There's a need for a more flexible approach to data mixture ratio selection.

Method: Train one CPT model per dataset, extract each model's distribution vector (representing parameter shift induced by that dataset), then search for optimal composition weights post-hoc using Bayesian optimization. The same vector pool can be re-optimized for different objectives without retraining.

Result: OptiMer consistently outperforms data mixture and model averaging baselines with 15-35 times lower search cost. Optimized weights can be interpreted as data mixture ratios and improve retraining performance. The same vector pool can produce target-tailored models on demand.

Conclusion: Data mixture ratio selection, traditionally a pre-training decision, can be reformulated as post-hoc optimization over distribution vectors, establishing a more flexible paradigm for continual pre-training that decouples ratio selection from training.
...
TL;DR: Study evaluates off-the-shelf sentiment classifiers on Holocaust oral histories, finding low-to-moderate inter-model agreement primarily driven by neutrality boundary decisions, and proposes an agreement-based stability taxonomy for model divergence analysis.


<details>
  <summary>Details</summary>
Motivation: Polarity detection is challenging under domain shift, especially in heterogeneous, long-form narratives with complex discourse structure like Holocaust oral histories. There's a need to understand how off-the-shelf sentiment classifiers perform on such sensitive historical narratives and where they diverge.

Method: Corpus-scale diagnostic study using three pretrained transformer-based polarity classifiers on 107,305 utterances and 579,013 sentences from Holocaust oral histories. Introduced agreement-based stability taxonomy (ABC) to stratify inter-model output stability. Used pairwise percent agreement, Cohen kappa, Fleiss kappa, and row-normalized confusion matrices. Applied T5-based emotion classifier to stratified samples for auxiliary descriptive analysis.

Result: Inter-model agreement was low to moderate overall, primarily driven by boundary decisions around neutrality. The combination of multi-model label triangulation and ABC taxonomy provides a cautious framework for characterizing where and how sentiment models diverge in sensitive historical narratives.

Conclusion: The study demonstrates the limitations of off-the-shelf sentiment classifiers for domain-shifted, sensitive historical narratives and provides a methodological framework for analyzing model disagreement through multi-model triangulation and stability stratification.
...
TL;DR: CrossTrace is a novel cross-domain dataset of 1,389 grounded scientific reasoning traces for hypothesis generation, spanning biomedical, AI/ML, and cross-domain research, with structured reasoning chains from prior knowledge to novel hypotheses.


<details>
  <summary>Details</summary>
Motivation: Scientific hypothesis generation is a critical research bottleneck, but existing datasets are limited to single domains and lack explicit, grounded reasoning traces connecting prior knowledge to novel contributions.

Method: Created CrossTrace dataset with Input/Trace/Output schema extending Bit-Flip-Spark framework, including step-level verification and eight discovery patterns. Fine-tuned Qwen2.5-7B-Instruct via QLoRA on this dataset.

Result: Fine-tuning on CrossTrace substantially improved performance: IAScore rose from 0.828 to 0.968 (GPT-4o) and 0.716 to 0.888 (Claude Opus), structural compliance from 0% to 100%, spark cosine similarity from 0.221 to 0.620. Cross-domain training outperformed single-domain training.

Conclusion: CrossTrace is the first large-scale, cross-domain dataset with step-level grounded reasoning traces for hypothesis generation, demonstrating that such traces provide effective training signals with domain-general benefits.
...
TL;DR: Safety fine-tuning in LLMs suppresses mind-attribution to models/artifacts but preserves Theory of Mind capabilities, though it reduces mind-attribution to animals and spiritual beliefs.


<details>
  <summary>Details</summary>
Motivation: To investigate whether safety fine-tuning that suppresses LLMs' mind-attribution tendencies (like claiming consciousness or emotions) degrades related socio-cognitive abilities such as Theory of Mind.

Method: Conducted safety ablation studies and mechanistic analyses of representational similarity to examine behavioral and mechanistic dissociations between different types of mind-attribution.
...
Conclusion: Safety fine-tuning successfully suppresses problematic mind-attribution to models/artifacts without harming ToM, but inadvertently suppresses widely shared perspectives about non-human minds and spiritual beliefs, suggesting a trade-off between safety alignment and preserving human-like cognitive diversity.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-intent detection benchmarks inadequately test compositional generalization because train and test sets often share the same co-occurrence patterns. The authors aim to evaluate whether models can recover new combinations of familiar intents, which is crucial for real-world deployment.

Method: The authors introduce CoMIX-Shift, a controlled benchmark with five challenging test conditions: held-out intent pairs, discourse-pattern shift, longer/noisier wrappers, held-out clause templates, and zero-shot triples. They also present ClauseCompose, a lightweight decoder trained only on singleton intents, and compare it against whole-utterance baselines including a fine-tuned tiny BERT model.

Result: ClauseCompose significantly outperforms baselines across all test conditions: 95.7% exact match on unseen intent pairs, 93.9% on discourse-shifted pairs, 62.5% on longer/noisier pairs, 49.8% on held-out templates, and 91.1% on unseen triples. WholeMultiLabel and BERT baselines show substantially lower performance, especially on compositional tasks like zero-shot triples where they score 0.0%.

Conclusion: Multi-intent detection requires more rigorous compositional evaluation, and simple factorization approaches like ClauseCompose can achieve surprisingly strong performance when evaluation properly tests compositional generalization.
...
TL;DR: Bio-inspired memory framework for LLMs using cognitive psychology principles to address persistent memory limitations, focusing on emotional valence, dual-process retrieval, and active encoding to improve long-term interactions.


<details>
  <summary>Details</summary>
Motivation: LLMs lack persistent, structured memory for long-term interaction and context-sensitive retrieval. Expanding context windows alone degrades reasoning performance, even with perfect retrieval, necessitating a more sophisticated memory architecture.

Method: Proposes a bio-inspired memory framework based on complementary learning systems theory, cognitive behavioral therapy's belief hierarchy, dual-process cognition, and fuzzy-trace theory. Organized around three principles: (1) Memory has valence (emotional-associative summaries organized in belief hierarchy), (2) Retrieval defaults to System 1 with System 2 escalation (automatic spreading activation with deliberate retrieval only when needed), and (3) Active, present, feedback-dependent encoding with thalamic gateway and curiosity-driven gist formation.

Result: Framework specifies seven functional properties that any implementation must satisfy. System converges toward System 1 processing over time (computational analog of clinical expertise), producing interactions that become cheaper, not more expensive, with experience.

Conclusion: Bio-inspired memory framework addresses fundamental limitations of current LLM memory systems by incorporating cognitive psychology principles, enabling more efficient long-term interactions through structured memory with emotional valence, dual-process retrieval, and active encoding mechanisms.
...
TL;DR: LLMs fail when surface cues conflict with unstated feasibility constraints, showing systematic heuristic override failures across multiple models and domains.


<details>
  <summary>Details</summary>
Motivation: To understand and address systematic failures in large language models when salient surface cues conflict with unstated feasibility constraints, revealing fundamental reasoning vulnerabilities.

Method: Diagnose-measure-bridge-treat framework: causal-behavioral analysis of "car wash problem" across six models, development of Heuristic Override Benchmark (500 instances across 4 heuristic × 5 constraint families), parametric probes, and goal-decomposition prompting interventions.

Result: Models show approximately context-independent sigmoid heuristics with distance cues exerting 8.7-38× more influence than goals; under strict evaluation (10/10 correct), no model exceeds 75% accuracy; presence constraints hardest (44%); minimal hints recover +15 pp; 12/14 models perform worse when constraints removed (up to -39 pp); goal-decomposition prompting recovers +6 to 9 pp.

Conclusion: Heuristic override represents a systematic reasoning vulnerability in LLMs, characterized by over-reliance on surface cues and failure to infer unstated constraints; the HOB benchmark provides a tool for measuring progress toward resolving this fundamental limitation.
...
TL;DR: Parallel data in pretraining has minimal effect on cross-lingual alignment; alignment emerges similarly without explicit parallel signal.


<details>
  <summary>Details</summary>
Motivation: To investigate the impact of parallel data (translated sentences) as an explicit signal for triggering cross-lingually aligned representations during pretraining, and understand whether such explicit alignment signals are necessary for achieving multilingual representation sharing.

Method: Train reference models with varying proportions of parallel data, then evaluate cross-lingual alignment using multiple evaluation methods to assess the effect of parallel data on representation sharing and language-specific neuron distribution.

Result: Parallel data has only minimal effect on cross-lingual alignment; limited to potentially accelerating representation sharing in early pretraining phases and decreasing language-specific neurons. Cross-lingual alignment emerges to similar levels even without explicit parallel data signal.

Conclusion: Explicit parallel data is not essential for achieving cross-lingual alignment in multilingual representations; alignment emerges naturally during pretraining without this explicit signal, though parallel data may provide some acceleration benefits in early stages.
...
TL;DR: PhoneticXEUS achieves state-of-the-art multilingual phone recognition using large-scale data and systematic analysis of SSL representations, data scale, and training objectives across 100+ languages.


<details>
  <summary>Details</summary>
Motivation: Current phone recognition models have limitations: English-focused models don't generalize across languages, multilingual models underutilize pretrained representations, and there's insufficient understanding of how data scale, architecture, and training objectives affect multilingual performance.

Method: Developed PhoneticXEUS trained on large-scale multilingual data with controlled ablations to evaluate SSL representations, data scale, and loss objectives across 100+ languages under a unified evaluation scheme.

Result: Achieved state-of-the-art performance with 17.7% PFER on multilingual speech and 10.6% PFER on accented English speech, with systematic analysis of error patterns across language families, accented speech, and articulatory features.

Conclusion: PhoneticXEUS demonstrates effective multilingual phone recognition through optimized training recipes and provides empirical insights into the contributions of SSL representations, data scale, and objectives, with all data and code released openly.
...
TL;DR: LLMs show cultural biases in emotion attribution; a Generator-Interpreter framework reveals performance varies by emotion type and cultural context, with generator's cultural background having stronger impact than interpreter's.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based emotion attribution systems focus primarily on interpretation while overlooking the cultural background of emotion generators, assuming universal emotional expression patterns that neglect cross-cultural variations in how emotions are expressed and perceived.

Method: Proposed a Generator-Interpreter framework capturing dual perspectives of emotion attribution (expression and interpretation). Systematically evaluated six LLMs on emotion attribution tasks using data from 15 countries to analyze cultural influences.

Result: Performance variations depend on emotion type and cultural context. Generator-interpreter alignment effects exist, with the generator's country of origin having a stronger impact on performance than interpreter's cultural background.

Conclusion: Calls for culturally sensitive emotion modeling in LLM-based systems to improve robustness and fairness in emotion understanding across diverse cultural contexts, addressing current limitations in cross-cultural emotion attribution.
...
TL;DR: PolarQuant is a post-training weight quantization method for LLMs that uses block-wise normalization, Walsh-Hadamard rotation, and Gaussian-matched quantization to achieve near-lossless compression without calibration data.


<details>
  <summary>Details</summary>
Motivation: To develop a weight quantization method that exploits the distributional structure of neural network weights for near-lossless compression of LLMs without requiring calibration data, addressing the trade-off between model compression and performance preservation.

Method: Three-stage approach: (1) block-wise normalization to unit hypersphere, (2) Walsh-Hadamard rotation to transform coordinates into approximately Gaussian random variables, (3) quantization with centroids matched to Gaussian distribution. The method functions as a preprocessing step for downstream INT4 quantizers.

Result: PolarQuant reduces Qwen3.5-9B perplexity from 6.90 (absmax Q5) to 6.40 (Δ = +0.03 from FP16), making it practically lossless. When used as preprocessing for torchao INT4, achieves perplexity 6.56 vs 6.68 for direct absmax INT4, while maintaining 43.1 tok/s throughput at 6.5 GB VRAM. Hadamard rotation alone accounts for 98% of quality improvement.

Conclusion: PolarQuant effectively exploits weight distribution structure for near-lossless LLM quantization without calibration data, with Walsh-Hadamard rotation being the key component. It serves as an effective preprocessing step for downstream INT4 quantization, achieving better performance than direct quantization methods.
...
TL;DR: APEX-EM is a non-parametric online learning framework that enables LLM-based autonomous agents to accumulate, retrieve, and reuse structured procedural plans without modifying model weights, achieving significant performance gains on coding, knowledge graph, and reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: LLM-based autonomous agents lack persistent procedural memory, forcing them to re-derive solutions from scratch even when structurally identical tasks have been solved before, leading to inefficiency and wasted computational resources.

Method: APEX-EM introduces: (1) structured experience representation encoding full procedural-episodic traces; (2) Plan-Retrieve-Generate-Iterate-Ingest (PRGII) workflow with Task Verifiers providing multi-dimensional reward signals; (3) dual-outcome Experience Memory with hybrid retrieval combining semantic search, structural signature matching, and plan DAG traversal for cross-domain transfer.

Result: On KGQAGen-10k: 89.6% accuracy vs 41.3% baseline (+48.3pp), surpassing oracle-retrieval upper bound (84.9%). On BigCodeBench: 83.3% success rate from 53.9% baseline (+29.4pp), exceeding MemRL's +11.0pp gain. On HLE: entity graph retrieval reaches 48.0% from 25.2% (+22.8pp).

Conclusion: APEX-EM demonstrates that non-parametric procedural memory can dramatically improve LLM agent performance across diverse domains, with component effectiveness varying by task type - rich feedback is critical for structured queries but less important for code generation, while binary signals can partially compensate for weaker feedback.
...
TL;DR: The paper proposes concept-level supervision as an alternative to standard next-token prediction, training models to predict sets of semantically related tokens rather than single tokens, improving semantic alignment with human judgments.


<details>
  <summary>Details</summary>
Motivation: Standard next-token prediction (NTP) treats alternative valid continuations as mutually exclusive targets, failing to capture the semantic equivalence of different surface forms that express similar meanings. Natural language prefixes often have multiple valid continuations that are semantically related but lexically distinct.

Method: Introduces a framework that predicts concepts approximated as sets of semantically related tokens instead of single tokens. This concept-level supervision trains models to recognize semantic equivalence across different lexical forms rather than treating them as competing predictions.

Result: Models trained with concept supervision show stronger alignment with human semantic similarity judgments on multiple lexical benchmarks. They achieve lower perplexity on semantically meaningful words but modestly higher global token-level perplexity, reflecting a tradeoff between standard NTP optimization and concept-level supervision.

Conclusion: Concept-level objectives can improve semantic alignment while maintaining competitive language modeling performance, suggesting that moving beyond single-token prediction toward semantic concept prediction offers benefits for language model training.
...
TL;DR: Kwame 2.0 is a bilingual AI teaching assistant using retrieval-augmented generation with human-in-the-loop oversight, deployed in an African coding course to provide scalable, timely learning support while maintaining reliability through human facilitation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of providing timely and accurate learning support in large-scale online coding courses, particularly in resource-constrained contexts across Africa, where traditional human-only support systems struggle with scalability.

Method: Built Kwame 2.0 using retrieval-augmented generation (RAG) to retrieve relevant course materials and generate context-aware responses. Deployed in a human-in-the-loop forum within SuaCode, an introductory mobile-based coding course. Conducted a 15-month longitudinal study across 15 cohorts with 3,717 enrollments from 35 African countries.

Result: Kwame 2.0 provided high-quality and timely support with high accuracy on curriculum-related questions. Human facilitators and peers effectively mitigated errors, particularly for administrative queries. The system demonstrated successful deployment at scale across diverse African contexts.

Conclusion: Human-in-the-loop generative AI systems can effectively combine AI scalability and speed with human reliability, offering a viable approach for learning assistance in resource-constrained settings and for underrepresented populations at scale.
...
TL;DR: SyriSign is the first publicly available dataset for Syrian Arabic Sign Language (SyArSL) with 1500 video samples across 150 signs, addressing the lack of resources for low-resource sign languages and evaluated with three deep learning models for text-to-sign translation.


<details>
  <summary>Details</summary>
Motivation: Address the underrepresentation of low-resource sign languages like Arabic in existing benchmarks, specifically the complete absence of publicly available datasets for Syrian Arabic Sign Language (SyArSL), to reduce communication barriers for the deaf community in Syria where news is typically inaccessible in spoken/written Arabic.

Method: Created SyriSign dataset with 1500 video samples across 150 unique lexical signs. Evaluated using three deep learning architectures: MotionCLIP for semantic motion generation, T2M-GPT for text-conditioned motion synthesis, and SignCLIP for bilingual embedding alignment.

Result: Generative approaches show strong potential for sign representation, but the limited dataset size constrains generalization performance. The dataset will be publicly released to serve as an initial benchmark for SyArSL research.

Conclusion: SyriSign addresses a critical gap in low-resource sign language datasets and demonstrates the potential of generative models for sign language translation, though dataset size limitations highlight the need for further data collection and model development for SyArSL.
...
TL;DR: SiPaKosa is a large corpus of Sinhala and Pali Buddhist texts created via OCR and web scraping, used to evaluate language models and support cultural preservation.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive resource for Sinhala and Pali doctrinal texts to support language model development, historical language analysis, and Buddhist scholarship while preserving Sinhala cultural heritage.

Method: Combined high-quality OCR using Google Document AI on historical manuscripts with systematic web scraping of canonical repositories, followed by rigorous quality control and metadata annotation. Organized into language-specific subcorpora (Sinhala and Mixed Sinhala-Pali).

Result: Created a corpus of approximately 786K sentences and 9.25M words from 16 historical Buddhist documents plus complete Tripitaka texts. Evaluation of ten pretrained models showed perplexity scores ranging from 1.09 to 189.67, with proprietary models outperforming open-source alternatives by 3-6 times.

Conclusion: SiPaKosa corpus enables domain-adapted language model pretraining, facilitates historical language analysis, supports information retrieval systems for Buddhist scholarship, and contributes to Sinhala cultural heritage preservation.
...
TL;DR: Agenda-based narrative extraction integrates LLMs into Narrative Trails pathfinding to steer storylines toward user-specified perspectives while maintaining coherence, achieving better alignment than keyword matching with minimal coherence loss.


<details>
  <summary>Details</summary>
Motivation: Existing methods face trade-offs: Narrative Maps supports interactivity and multiple storylines but sacrifices individual path coherence, while Narrative Trails achieves high coherence but lacks user guidance and multi-perspective support. There's a need for a method that combines coherence with user-steerable, multi-perspective narrative extraction.

Method: Agenda-based narrative extraction integrates LLMs into the Narrative Trails pathfinding process. At each step, an LLM ranks candidate documents based on their alignment with a given user-specified agenda while maintaining narrative coherence. Different agendas yield different storylines through the same corpus.

Result: LLM-driven steering achieves 9.9% higher alignment than keyword matching on semantic agendas (p=0.017), with 13.3% improvement on "Regime Crackdown" specifically (p=0.037). Keyword matching remains competitive on agendas with literal keyword overlap. Coherence cost is minimal: LLM steering reduces coherence by only 2.2% compared to agenda-agnostic baseline. Counter-agendas that contradict source material score uniformly low (2.2-2.5), confirming steering cannot fabricate unsupported narratives.

Conclusion: Agenda-based narrative extraction successfully bridges the gap between coherence and user guidance, enabling multi-perspective narrative extraction from corpora while maintaining factual grounding and minimal coherence loss.
...
### [131] [Long-Document QA with Chain-of-Structured-Thought and Fine-Tuned SLMs](https://arxiv.org/abs/2603.29232)
*Zhuowen Liang,Xiaotian Lin,Zhengxuan Zhang,Yuyu Luo,Haixun Wang,Nan Tang*

Main category: cs.CL

TL;DR: LiteCoST is a framework that enables small language models (3B/7B) to achieve LLM-comparable accuracy on long-document QA by using structured outputs and two-stage fine-tuning, while reducing latency 2-4x.


<details>
  <summary>Details</summary>
Motivation: Direct reasoning over long, noisy documents with LLMs is brittle and error-prone. There's a need for document QA that consolidates dispersed evidence into structured outputs (tables, graphs, chunks) for reliable, verifiable QA with both high accuracy and low latency using smaller models.

Method: Two-pillar framework: 1) Chain-of-Structured-Thought (CoST) - schema-aware instruction guiding strong LLMs to produce step-wise traces and structured outputs with normalization, alignment, serialization, and verification. 2) SLM fine-tuning - two-stage training on LLM-generated CoST data: Supervised Fine-Tuning for structural alignment, followed by Group Relative Policy Optimization (GRPO) with triple rewards for answer/format quality and process consistency.

Result: Achieves LLM-comparable quality on multi-domain long-document QA using 3B/7B SLMs, while delivering 2-4x lower latency than GPT-4o and DeepSeek-R1 (671B).

Conclusion: LiteCoST successfully distills structure-first behavior into small language models, enabling efficient, accurate long-document QA with verifiable structured outputs and significantly reduced computational overhead.
...
TL;DR: This paper presents a weakly supervised method for detecting sentence-level cross-lingual text reuse in journalism without requiring full translations, analyzing where reused content appears within articles and using publication timing to identify likely sources.


<details>
  <summary>Details</summary>
Motivation: To address information overload for journalists by developing automated tools to detect sentence-level cross-lingual text reuse in multilingual journalism, moving beyond simple lexical matching that overlooks substantial editorial reuse involving paraphrase and compositional reuse from multiple sources.

Method: Weakly supervised method for detecting sentence-level cross-lingual reuse without requiring full translations; compares English articles from Slovenian Press Agency (STA) with reports from 15 foreign agencies in 7 languages; uses publication timestamps to retain earliest likely foreign source for each reused sentence; analyzes 1,037 STA and 237,551 FA articles from two time windows (Oct 7-Nov 2, 2023; Feb 1-28, 2025).

Result: Identified 1,087 aligned sentence pairs after filtering to earliest sources; reuse occurs in 52% of STA articles and 1.6% of FA articles; reuse is predominantly non-literal involving paraphrase and compositional reuse from multiple sources; reused content tends to appear in middle and end of English articles while leads are more often original.

Conclusion: The study demonstrates that simple lexical matching overlooks substantial editorial reuse, and presents a method that (i) detects reuse across languages without full translation, (ii) uses publication timing to identify likely sources, and (iii) analyzes where reused material is situated within articles, providing tools to reduce information overload for journalists.
...
### [133] [The Thiomi Dataset: A Large-Scale Multimodal Corpus for Low-Resource African Languages](https://arxiv.org/abs/2603.29244)
*Hillary Mutisya,John Mugane,Gavin Nyamboga,Brian Chege,Maryruth Gathoni*

Main category: cs.CL

TL;DR: Thiomi Dataset: A large-scale multimodal corpus for 10 African languages with over 601K text annotations and 385K audio recordings, collected via community platform with 86-100% quality approval, establishing new SOTA ASR results.


<details>
  <summary>Details</summary>
Motivation: Addressing the scarcity of high-quality, large-scale multimodal datasets for African languages to advance speech and language technology research and infrastructure development across diverse African linguistic communities.

Method: Community-driven data collection platform with 100+ contributors, multi-tier quality assurance pipeline, supplementation of Swahili with Common Voice data, and training/evaluation of ASR, MT, and TTS models for baseline establishment.

Result: Achieved 86-100% text approval rates for six primary languages; best ASR system reduced Swahili WER from 8.3% to 3.24% (61% relative reduction) and achieved 4.3% WER on Somali, establishing new state-of-the-art benchmarks.

Conclusion: The Thiomi Dataset successfully creates a valuable resource for African language technology, demonstrating effective community-driven collection methods and significant performance improvements, with potential to accelerate research and infrastructure development across diverse African linguistic communities.
...
TL;DR: GEO-SFE introduces a structural feature engineering framework for generative engine optimization, decomposing content into macro, meso, and micro structural levels to improve citation rates and quality in AI-powered search engines.


<details>
  <summary>Details</summary>
Motivation: The shift from traditional link-based retrieval to direct answer generation with selective source citation in AI-powered search engines creates new challenges for content visibility. Existing GEO approaches focus primarily on semantic content modification, leaving the role of structural features in influencing citation behavior underexplored.

Method: Proposes GEO-SFE, a systematic framework for structural feature engineering that decomposes content structure into three hierarchical levels: macro-structure (document architecture), meso-structure (information chunking), and micro-structure (visual emphasis). Develops architecture-aware optimization strategies and predictive models that preserve semantic integrity while improving structural effectiveness.

Result: Experimental evaluation across six mainstream generative engines demonstrates consistent improvements in citation rate (17.3%) and subjective quality (18.5%), validating the effectiveness and generalizability of the proposed framework.
...
TL;DR: MemRerank is a preference memory framework that distills user purchase history into concise signals for personalized product reranking, achieving +10.61% improvement in 1-in-5 selection accuracy.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based shopping agents struggle with personalization because naively appending raw purchase history to prompts is ineffective due to noise, length, and relevance mismatch issues.

Method: Proposes MemRerank framework that extracts query-independent preference signals from purchase history, trains memory extractor with reinforcement learning using downstream reranking performance as supervision, and builds an end-to-end benchmark with 1-in-5 selection task evaluation.

Result: MemRerank consistently outperforms no-memory, raw-history, and off-the-shelf memory baselines, achieving up to +10.61 absolute points improvement in 1-in-5 accuracy with two LLM-based rerankers.

Conclusion: Explicit preference memory is a practical and effective building block for personalization in agentic e-commerce systems, addressing limitations of raw history approaches.
...
TL;DR: Researchers developed a Japanese entity linking corpus with Japan-specific entity coverage, demonstrating high annotation consistency and containing non-trivial cases for evaluation.


<details>
  <summary>Details</summary>
Motivation: Entity linking resources are primarily available for English, while Japanese systems lack adequate evaluation corpora, especially for Japan-specific entities and linguistic expressions.

Method: Developed a corpus design policy for entity linking, constructed an annotated corpus with rich coverage of Japan-specific entities, evaluated inter-annotator agreement, and conducted preliminary experiments on entity disambiguation using string matching.

Result: High inter-annotator agreement confirmed annotation consistency; preliminary experiments showed the corpus contains substantial non-trivial cases, supporting its usefulness as an evaluation benchmark.

Conclusion: The constructed Japanese entity linking corpus addresses the resource gap, provides reliable annotations, and serves as a valuable benchmark for evaluating Japanese entity linking systems.
...
TL;DR: First comprehensive evaluation of open-source machine translation systems for Esperanto shows NLLB models achieve best performance across six language directions, with human evaluation confirming results.


<details>
  <summary>Details</summary>
Motivation: Esperanto is a widespread constructed language with regular grammar and productive word formation, but remains relatively underexplored in modern machine translation approaches despite having substantial online resources.

Method: Comprehensive evaluation of open-source MT systems including rule-based systems, encoder-decoder models, and LLMs across model sizes; evaluation across six language directions involving English, Spanish, Catalan, and Esperanto using multiple automatic metrics and human evaluation.

Result: NLLB family achieves best performance in all language pairs, followed closely by trained compact models and a fine-tuned general-purpose LLM; human evaluation confirms trend with NLLB translations preferred in approximately half of comparisons, though noticeable errors remain.

Conclusion: In line with Esperanto's tradition of openness and international collaboration, the authors release code and best-performing models publicly, providing first comprehensive MT evaluation for Esperanto.
...
TL;DR: L-ReLF is a reproducible methodology for creating structured lexical datasets for low-resource languages, addressing challenges like inconsistent terminology and OCR bias, with outputs compatible with Wikidata Lexemes.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized terminology in underserved languages like Moroccan Darija creates barriers to knowledge equity on platforms like Wikipedia, forcing editors to use inconsistent ad-hoc methods for creating new words.

Method: Developed a technical pipeline that systematically addresses low-resource data challenges: source identification, OCR processing (despite bias towards Modern Standard Arabic), and rigorous post-processing to correct errors and standardize the data model.

Result: Created a structured lexical dataset fully compatible with Wikidata Lexemes, providing a vital technical resource for underserved languages.

Conclusion: L-ReLF methodology is designed for generalizability, offering other language communities a clear path to build foundational lexical data for downstream NLP applications like Machine Translation and morphological analysis.
...
TL;DR: First systematic guidelines for applying Labovian narrative analysis to Japanese narratives, addressing language-specific challenges with explicit clause segmentation rules and achieving good inter-annotator agreement.


<details>
  <summary>Details</summary>
Motivation: Existing Labovian narrative analysis datasets are only available in English, which differs significantly from Japanese in grammar and discourse conventions. The labor-intensive nature of Labovian analysis combined with the lack of Japanese-specific guidelines creates a gap for qualitative research in Japanese contexts.

Method: Developed systematic guidelines for Labovian narrative analysis of Japanese data that retain all six Labovian categories while providing explicit rules for clause segmentation tailored to Japanese constructions. The guidelines cover a broader range of clause types and narrative types than previous approaches.

Result: Annotators achieved high agreement in clause segmentation (Fleiss' kappa = 0.80) and moderate agreement in two structural classification tasks (Krippendorff's alpha = 0.41 and 0.45, respectively). One classification agreement was slightly higher than prior work despite using finer-grained distinctions.

Conclusion: The paper successfully introduces the first systematic guidelines for Labovian narrative analysis of Japanese narratives, demonstrating their utility through good inter-annotator agreement. It discusses annotation challenges and prospects for developing larger Japanese narrative datasets for structural analysis in qualitative research.
...
TL;DR: CPB-Bench is a bilingual benchmark for evaluating LLMs on challenging patient behaviors in medical consultations, focusing on information contradiction, factual inaccuracy, self-diagnosis, and care resistance.


<details>
  <summary>Details</summary>
Motivation: Existing medical LLM evaluations assume idealized patient questions, lacking realism for real-world clinical scenarios where patients may provide unclear, inconsistent, or misleading information, creating safety risks.

Method: Defined four clinically grounded challenging patient behavior categories with concrete failure criteria. Built CPB-Bench using four existing medical dialogue datasets, creating 692 multi-turn bilingual dialogues annotated with these behaviors. Evaluated open- and closed-source LLMs and tested four intervention strategies.

Result: Models perform well overall but show consistent, behavior-specific failure patterns, particularly struggling with contradictory or medically implausible patient information. Intervention strategies yield inconsistent improvements and can introduce unnecessary corrections.

Conclusion: Current LLMs have significant limitations in handling challenging patient behaviors, highlighting the need for more realistic medical evaluation benchmarks and improved safety mechanisms for real-world clinical applications.
...
TL;DR: LLM evaluation framework using token-level perplexity to test if models rely on linguistically relevant cues via minimal sentence pair comparisons, revealing models use heuristics beyond expected linguistic ones.


<details>
  <summary>Details</summary>
Motivation: Standard LLM evaluations focus on task performance but offer limited insight into whether correct behavior reflects appropriate underlying mechanisms, risking confirmation bias. Need to test if models actually rely on linguistically relevant cues.

Method: Introduce interpretability framework based on token-level perplexity. Compare perplexity distributions over minimal sentence pairs differing in one or a few 'pivotal' tokens. Enables precise, hypothesis-driven analysis without unstable feature-attribution techniques.

Result: Experiments on controlled linguistic benchmarks with several open-weight LLMs show linguistically important tokens influence model behavior but never fully explain perplexity shifts. Models rely on heuristics other than expected linguistic ones.

Conclusion: Token-level perplexity analysis reveals LLMs use heuristics beyond expected linguistic cues, challenging assumptions about their underlying mechanisms and highlighting need for more nuanced interpretability methods.
...
TL;DR: CounselReflect is an end-to-end toolkit for auditing mental-health support dialogues that provides structured multi-dimensional reports with session summaries, turn-level scores, and evidence-linked excerpts instead of single opaque quality scores.


<details>
  <summary>Details</summary>
Motivation: Mental-health support is increasingly mediated by conversational systems (LLM-based tools), but users lack structured ways to audit the quality and potential risks of the support they receive.

Method: The system integrates two families of evaluation signals: (1) 12 model-based metrics from task-specific predictors, and (2) rubric-based metrics via a literature-derived library (69 metrics) and user-defined custom metrics, operationalized with configurable LLM judges. Available as web app, browser extension, and CLI.

Result: Human evaluation with 20 participants and expert review with 6 mental-health professionals suggests CounselReflect supports understandable, usable, and trustworthy auditing. Demo video and full source code provided.

Conclusion: CounselReflect provides a transparent, multi-dimensional auditing framework for mental-health support dialogues, addressing the need for structured quality assessment in LLM-mediated mental-health conversations.
...
TL;DR: LLM-generated authorial impersonations fail to bypass established forensic authorship verification systems, with some methods performing better against impersonations than genuine negative samples.


<details>
  <summary>Details</summary>
Motivation: To investigate whether prompted LLMs can generate convincing authorial impersonations that evade existing forensic authorship verification systems, addressing new challenges posed by LLM accessibility for adversarial impersonation.

Method: Used GPT-4o to generate impersonation texts under four prompting conditions across three genres (emails, text messages, social media posts), then evaluated against both non-neural (n-gram tracing, Ranking-Based Impostors Method, LambdaG) and neural (AdHominem, LUAR, STAR) AV methods within a likelihood-ratio framework.

Result: LLM-generated texts failed to sufficiently replicate authorial individuality to bypass established AV systems; some methods achieved higher accuracy when rejecting impersonation texts compared to genuine negative samples.

Conclusion: Current AV systems remain robust against entry-level LLM impersonation attempts across multiple genres, with resilience partly stemming from higher lexical diversity and entropy in LLM-generated texts.
...
TL;DR: M-MiniGPT4 is a multilingual vision-language model that achieves strong performance across 11 languages using mixed native/translated data and parallel text alignment, outperforming SOTA models in its weight class.


<details>
  <summary>Details</summary>
Motivation: To develop a multilingual vision-language understanding model that performs well across multiple languages, addressing the need for cross-lingual capabilities in vision-language tasks and improving performance in low-resource language settings.

Method: 1) Uses MiniGPT4 architecture as base; 2) Employs mixture of native multilingual and translated data for training; 3) Introduces multilingual alignment training stage using parallel text corpora to enhance cross-lingual capabilities.

Result: Achieves 36% accuracy on multilingual MMMU benchmark, outperforming state-of-the-art models in same weight class, including foundation models released after most of this work was completed.

Conclusion: M-MiniGPT4 demonstrates strong multilingual vision-language understanding across 11 languages, with released models, code, and translated datasets to support future research in multilingual and low-resource settings.
...
TL;DR: ConRad: A reinforcement learning framework for calibrating verbalized confidence estimates in radiology report generation to enable selective radiologist verification and reduce hallucination risks.


<details>
  <summary>Details</summary>
Motivation: Safe deployment of LVLMs in radiology requires clinically interpretable confidence indicators to enable selective verification and reduce risks from hallucinated findings influencing clinical decisions. Current models are often overconfident, and calibration research in multimodal radiology settings is limited.

Method: ConRad uses reinforcement learning (GRPO algorithm) to fine-tune medical LVLMs for calibrated verbalized confidence estimates. Two settings: report-level confidence scores and sentence-level confidence for each claim. Reward functions based on logarithmic scoring rule penalize miscalibration and guarantee optimal calibration under reward maximization.

Result: ConRad substantially improves calibration and outperforms competing methods. Clinical evaluation shows ConRad's report-level scores align well with clinicians' judgment. The framework enables highlighting full reports or low-confidence statements for targeted review.

Conclusion: ConRad supports safer clinical integration of AI-assisted radiology report generation by providing calibrated confidence estimates that enable selective radiologist verification, reducing risks from hallucinated findings influencing clinical decisions.
...
TL;DR: MemFactory is a unified framework for training and evaluating memory-augmented LLM agents, featuring modular memory lifecycle components and integrated GRPO optimization.


<details>
  <summary>Details</summary>
Motivation: Existing RL-based memory optimization approaches for LLMs are fragmented and task-specific, lacking a unified infrastructure for integrating, training, and evaluating complex memory agent pipelines.

Method: MemFactory abstracts memory lifecycle into atomic plug-and-play components with Lego-like architecture, integrates Group Relative Policy Optimization (GRPO) for fine-tuning memory management policies, and supports cutting-edge paradigms like Memory-R1, RMM, and MemAgent.

Result: Empirical validation on MemAgent architecture shows consistent performance improvements over base models across in-domain and out-of-distribution evaluations, with relative gains up to 14.8%.

Conclusion: MemFactory provides standardized, extensible infrastructure that lowers entry barriers and enables future innovations in memory-driven AI agents.
...
<details>
  <summary>Details</summary>
Motivation: Accurate privacy evaluation of textual data is critical but challenging; while LLMs can serve as reliable privacy evaluators, their computational cost and impracticality for processing sensitive data at scale limit real-world deployment.

Method: Distill privacy assessment capabilities from Mistral Large 3 (675B) into lightweight encoder models (as few as 150M parameters) using a large-scale dataset of privacy-annotated texts spanning 10 diverse domains, training efficient classifiers that preserve strong agreement with human annotations.

Result: Lightweight models achieve strong agreement with human annotations while dramatically reducing computational requirements; validated on human-annotated test data and demonstrated practical utility as an evaluation metric for de-identification systems.

Conclusion: The approach successfully addresses the gap between accurate privacy evaluation and computational practicality by distilling LLM capabilities into efficient models suitable for real-world deployment in privacy-preserving NLP applications.
...
TL;DR: LLM Probe is a lexicon-based framework for systematically evaluating LLMs' linguistic abilities in low-resource languages, tested on a Semitic language case study revealing performance differences between model types.


<details>
  <summary>Details</summary>
Motivation: Limited understanding of LLMs' linguistic capabilities in low-resource and morphologically rich languages due to scarce annotated resources and lack of standardized evaluation frameworks.

Method: Developed LLM Probe framework with four evaluation areas: lexical alignment, part-of-speech recognition, morphosyntactic probing, and translation accuracy. Created manually annotated benchmark dataset for a low-resource Semitic language with bilingual lexicons and linguistic annotations.

Result: Sequence-to-sequence models excel in morphosyntactic analysis and translation quality, while causal models perform well in lexical alignment but show weaker translation accuracy. Framework reveals notable performance differences across linguistic tasks.

Conclusion: Linguistically grounded evaluation is essential for understanding LLM limitations in low-resource settings. The open-source release of LLM Probe and benchmark dataset aims to promote reproducible benchmarking and support inclusive multilingual language technologies.
...
TL;DR: This paper analyzes whether providing task demonstrators (MR-sentence pairs) to NLG models improves generation quality across diverse datasets, finding benefits for complex tasks, small datasets, and zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: To investigate whether enriching NLG model inputs with task demonstrators (MR-sentence pairs from the dataset) enhances generation quality, particularly across different domains, dataset characteristics, and evaluation metrics.

Method: Used fine-tuned NLG models with enriched inputs containing MR-sentence demonstrator pairs. Conducted comparative analysis across four diverse datasets varying in domain, size, lexicon, MR variability, and acquisition process. Evaluated using five metrics focusing on different linguistic aspects.

Result: Enriched inputs are effective for complex tasks and small datasets with high MR/sentence variability, beneficial in zero-shot settings across domains. Semantic metrics (especially those trained with human ratings) capture generation quality more accurately than lexical metrics. Models show fast adaptability to tasks and robustness at semantic/communicative intention levels.

Conclusion: Task demonstrators improve NLG generation quality, particularly for challenging scenarios. Semantic evaluation metrics are superior to lexical ones, with human-trained metrics detecting subtle semantic issues. The approach enables robust adaptation to diverse tasks.
...
TL;DR: LMs trained on child-scale natural language data show acceptable grammar scaling but poorer semantic/world knowledge scaling compared to synthetic data, with performance variability across children's experiences and correlations between model likelihoods and children's word learning.


<details>
  <summary>Details</summary>
Motivation: To understand the "data gap" between language models requiring massive training data versus human children learning from limited natural input, by benchmarking LMs on human-scale datasets to investigate how linguistic knowledge emerges from children's natural training data.

Method: Used transcripts from BabyView dataset (videos from children ages 6-36 months) to investigate: (1) scaling performance at child-scale data regimes, (2) variability across different children's experiences and linguistic predictors of dataset quality, and (3) relationships between model and child language learning outcomes.

Result: LMs trained on child data show acceptable scaling for grammar tasks but lower scaling on semantic and world knowledge tasks than models trained on synthetic data; substantial variability across different children's data; performance associated with combination of distributional and interactional linguistic features; model likelihoods for individual words correlate with children's learning of those words.

Conclusion: Understanding properties that make language data efficient for learning can enable more powerful small-scale language models while shedding light on human language acquisition, with child-directed input properties influencing both model learning and human language development.
...
TL;DR: LLMs with linguistic resources (phonetic transcriptions, dialect features, vowel history, rules) can approach HuBERT's performance in Swiss German dialect classification, though human linguist baseline reveals room for improvement in automatic transcriptions.


<details>
  <summary>Details</summary>
Motivation: Audio dialect classification is challenging due to scarce labeled dialectal speech, especially for Swiss German. The paper explores whether LLMs can understand dialects and achieve performance comparable to models like HuBERT, while establishing both LLM and human linguist baselines.

Method: Uses phonetic transcriptions from ASR systems combined with linguistic resources including dialect feature maps, vowel history, and rules. Compares LLM performance with HuBERT and establishes human linguist baseline to evaluate the utility of automatically generated transcriptions.

Result: LLM predictions improve when provided with linguistic information, approaching HuBERT's performance. Human baseline shows that automatically generated transcriptions can be beneficial for dialect classification but also reveals opportunities for improvement in transcription quality.

Conclusion: LLMs with appropriate linguistic resources can be effective for dialect classification, though automatic transcriptions need refinement. The approach demonstrates the value of combining ASR outputs with linguistic knowledge for low-resource dialect analysis tasks.
...
TL;DR: Bilingual language models trained on controlled exposure conditions perform similarly to monolingual models in one language while gaining strong second language proficiency, suggesting no inherent bilingual acquisition disadvantages.


<details>
  <summary>Details</summary>
Motivation: To address theoretical and practical questions about multilingual acquisition, particularly whether bilingualism causes learning delays and how different exposure regimes affect language learning, using controlled computational simulations to overcome limitations of correlational human studies.

Method: Created matched 100M-word monolingual and bilingual datasets using synthetic data and machine translation, trained GPT-2 models on various bilingual exposure regimes, and evaluated performance on perplexity, grammaticality, and semantic knowledge across model scales.

Result: Bilingual models performed similarly to monolingual models in one language while achieving strong performance in the second language, with no significant differences between different bilingual exposure regimes.

Conclusion: Bilingual input poses no inherent challenges for statistical learners, and different bilingual exposure regimes yield comparable outcomes, suggesting no strong disadvantages to bilingual acquisition.
...
TL;DR: LLM-based grading confidence estimation: Self-reported confidence outperforms self-consistency voting in calibration despite lower cost, with larger models showing better calibration and GPT-OSS-120B achieving best results.


<details>
  <summary>Details</summary>
Motivation: LLMs show promise for automated grading but produce unreliable outputs. Instead of directly improving grading accuracy, the paper addresses the complementary problem of predicting when an LLM grader is likely to be correct, enabling selective automation where high-confidence predictions are processed automatically while uncertain cases are flagged for human review.

Method: Compared three confidence estimation methods (self-reported confidence, self-consistency voting, and token probability) across seven LLMs of varying scale (4B to 120B parameters) on three educational datasets: RiceChem (long-answer chemistry), SciEntsBank, and Beetle (short-answer science).

Result: Self-reported confidence consistently achieves the best calibration across all conditions (avg ECE 0.166 vs 0.229 for self-consistency). Self-consistency remains 38% worse despite requiring 5× the inference cost. Larger models exhibit substantially better calibration, with GPT-OSS-120B achieving the best calibration (avg ECE 0.100) and strong discrimination (avg AUC 0.668). Confidence is strongly top-skewed across methods, creating a "confidence floor" that practitioners must account for when setting thresholds.

Conclusion: Simply asking LLMs to report their confidence provides a practical approach for identifying reliable grading predictions, offering a cost-effective alternative to more computationally expensive methods like self-consistency voting.
...
TL;DR: DeToxR uses RL-aligned LLMs to fuse unstructured narratives with structured medical data for multi-label substance identification in acute poly-substance intoxication, outperforming both base LLMs and expert toxicologists.


<details>
  <summary>Details</summary>
Motivation: Acute poly-substance intoxication requires rapid decisions under uncertainty with incomplete information. Clinicians must fuse unstructured narratives (paramedic reports, unreliable patient histories) with structured medical data, but current LLMs struggle in this setting and often underperform simple baselines.

Method: DeToxR adapts Reinforcement Learning to emergency toxicology with a data-fusion engine for multi-label prediction across 14 substance classes. Uses LLM finetuned with Group Relative Policy Optimization (GRPO), optimizing reasoning directly using clinical performance reward based on multi-label agreement metric that penalizes missing co-ingested substances and hallucinating absent poisons.

Result: Model significantly outperforms unadapted base LLM counterpart and supervised baselines. In clinical validation study, outperforms expert toxicologist in identifying correct poisons (Micro-F1: 0.644 vs. 0.473).

Conclusion: RL-aligned LLMs can effectively synthesize unstructured pre-clinical narratives and structured medical data for decision support in high-stakes emergency toxicology environments, demonstrating clinical advantage over human experts.
...
TL;DR: The paper introduces a novel metric for detecting latent policy failures (near-misses) in LLM-based agentic workflows, where agents bypass required policy checks but still reach correct outcomes due to favorable circumstances.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for LLM-based agentic systems focus on comparing final system states against ground truth, which detects explicit policy violations but misses latent failures where agents bypass policy checks yet achieve correct outcomes due to favorable circumstances.

Method: Builds on ToolGuard framework to convert natural-language policies into executable guard code, then analyzes agent conversation traces to determine whether tool-calling decisions were sufficiently informed by policy checks. Evaluated on τ²-verified Airlines benchmark across contemporary open and proprietary LLMs.

Result: Latent failures occur in 8-17% of trajectories involving mutating tool calls, even when final outcomes match expected ground-truth states. This reveals a blind spot in current evaluation methodologies.

Conclusion: There is a need for metrics that assess not only final outcomes but also the decision process leading to them, as current evaluation methods overlook latent policy failures that could lead to serious issues under different circumstances.
...
TL;DR: ENEIDE is a silver standard NERL dataset for historical Italian texts, created from two digital editions spanning 1798-1978, with 2,111 documents and 8,000+ entity annotations covering multiple types linked to Wikidata.


<details>
  <summary>Details</summary>
Motivation: To address the lack of multi-domain, publicly available Named Entity Recognition and Linking (NERL) datasets for historical Italian texts, which is crucial for developing and evaluating models in this specialized domain.

Method: Semi-automatic annotation extraction from manually curated scholarly digital editions (Digital Zibaldone and Aldo Moro Digitale), including quality control and annotation enhancement procedures, with entity linking to Wikidata identifiers including NIL entities.

Result: Created ENEIDE dataset with 2,111 documents, 8,000+ entity annotations covering person, location, organization, and literary work types, with training/development/test splits, demonstrating challenge for NERL models and gap between zero-shot and fine-tuned approaches.

Conclusion: ENEIDE represents the first multi-domain, publicly available NERL dataset for historical Italian, suitable for temporal entity disambiguation and cross-domain evaluation, released under CC BY-NC-SA 4.0 license.
...
TL;DR: SNEAK benchmark evaluates LLMs' strategic communication under asymmetric information, measuring trade-off between utility (communicating to allies) and leakage (revealing to adversaries), with humans outperforming all models.


<details>
  <summary>Details</summary>
Motivation: Current LLM benchmarks don't measure strategic communication under asymmetric information, which is crucial for multi-agent deployments where agents must balance informativeness with secrecy when signaling to collaborators while preventing adversaries from inferring sensitive details.

Method: Introduces SNEAK benchmark where models are given a semantic category, candidate word set, and secret word, then must generate messages that indicate knowledge of the secret without clearly revealing it. Uses two simulated agents: ally (knows secret, identifies intended message) and chameleon (doesn't know secret, attempts inference), yielding utility and leakage metrics.

Result: Strategic communication under asymmetric information remains challenging for current LLMs. Human participants significantly outperform all evaluated models, achieving up to four times higher scores on the trade-off between informativeness and secrecy.

Conclusion: SNEAK provides a framework for evaluating selective information sharing in language models, revealing that current systems struggle with strategic communication under asymmetric information, highlighting an important capability gap that needs to be addressed for effective multi-agent deployments.
...
TL;DR: This paper analyzes readability of German ESG reports, finding they're generally perceived as easy by native speakers but readability is subjective. The study evaluates various readability scoring methods, showing that fine-tuned transformers outperform LLM prompting for predicting human readability.


<details>
  <summary>Details</summary>
Motivation: With growing sustainability concerns and mandatory ESG reporting, there's a need for these reports to be accessible to both financial experts and non-expert audiences. The paper addresses whether ESG reports are written clearly enough for public consumption.

Method: Extended an existing German ESG report dataset with crowdsourced readability annotations. Applied various readability scoring methods including LLM prompting and fine-tuned transformers, evaluating them based on prediction error and correlation with human rankings.

Result: Native speakers generally perceive ESG report sentences as easy to read, but readability is subjective. Fine-tuned transformers achieve the lowest error in predicting human readability, outperforming LLM prompting. Model averaging slightly improves performance at the cost of slower inference.

Conclusion: ESG reports are generally readable but subjective; fine-tuned transformers are most effective for readability prediction. The findings support using automated readability assessment to ensure ESG information is accessible to diverse audiences.
...
TL;DR: FLEURS-Kobani introduces the first public Northern Kurdish benchmark dataset for speech tasks, addressing the language's absence in existing multilingual benchmarks and providing baseline results for ASR and speech translation.


<details>
  <summary>Details</summary>
Motivation: Northern Kurdish (KMR) is not included in the FLEURS benchmark's 100+ language coverage, limiting benchmarking capabilities for automatic speech recognition and speech translation tasks in this under-resourced language.

Method: Created FLEURS-Kobani dataset with 5,162 validated utterances (18h24m) recorded by 31 native speakers. Used two-stage fine-tuning strategy (Common Voice to FLEURS-Kobani) for Whisper v3-large models for ASR and end-to-end speech-to-text translation.

Result: Best ASR performance: WER 28.11, CER 9.84 on test set. For end-to-end speech-to-text translation (KMR to EN): Whisper achieved 8.68 BLEU on test. Dataset provides first public benchmark for Northern Kurdish ASR, S2TT, and S2ST tasks.

Conclusion: FLEURS-Kobani successfully extends benchmark coverage to Northern Kurdish, an under-resourced language, providing a publicly available dataset (CC BY 4.0) for research in speech recognition and translation tasks.
...
TL;DR: YARN is a modular framework that uses LLMs to decompose narratives into units, abstract them at four levels, then maps these abstractions for analogical reasoning, outperforming end-to-end LLM baselines.


<details>
  <summary>Details</summary>
Motivation: Current approaches to analogical reasoning in narratives face challenges: cognitive engines require pre-extracted entities, while LLMs are sensitive to prompt format and surface similarity. This gap motivates investigating whether enhancing structural mapping with LLM-derived abstractions improves analogical reasoning in narratives.

Method: YARN is a modular framework with three main components: 1) uses LLMs to decompose narratives into units, 2) abstracts these units at four defined levels capturing both general meaning and story roles (grounded in framing theory), 3) passes abstractions to a mapping component that aligns elements across stories for analogical reasoning.

Result: Abstractions consistently improve model performance, achieving competitive or better results than end-to-end LLM baselines. Error analysis reveals remaining challenges: abstraction at appropriate levels, incorporating implicit causality, and emerging categorization of analogical patterns in narratives.

Conclusion: YARN enables systematic analysis of component contributions to analogical reasoning in narratives, provides insights into abstraction challenges, and offers an open-source framework for future research on narrative analogies.
...
TL;DR: ContextClaim introduces context-driven claim detection by retrieving Wikipedia information about entities in claims to improve verifiability judgments, showing variable effectiveness across domains and models.


<details>
  <summary>Details</summary>
Motivation: Existing claim detection methods rely solely on claim text, ignoring contextual information about referenced entities and events that could help determine verifiability. This is particularly limiting for verifiable claim detection where knowledge about what information exists for verification is crucial.

Method: ContextClaim extracts entity mentions from input claims, retrieves relevant information from Wikipedia as structured knowledge, and uses LLMs to produce concise contextual summaries for downstream classification. Evaluated on COVID-19 Twitter and political debate datasets across encoder/decoder models with fine-tuning, zero-shot, and few-shot settings.

Result: Context augmentation improves verifiable claim detection, but effectiveness varies across domains, model architectures, and learning settings. Component analysis, human evaluation, and error analysis examine when and why retrieved context contributes to more reliable verifiability judgments.

Conclusion: Retrieving contextual information from structured knowledge sources like Wikipedia can enhance verifiable claim detection, though the benefits depend on domain characteristics, model types, and learning paradigms. The approach advances retrieval to earlier stages of fact-checking pipelines.
...
TL;DR: Targeted speech-rate adjustments following specific temporal patterns improve vowel comprehension for both native and non-native listeners, while global slowing actually increases errors despite being perceived as clearer.


<details>
  <summary>Details</summary>
Motivation: To investigate whether common strategies like global speech slowing actually improve intelligibility for listeners with comprehension challenges (hard-of-hearing, non-native speakers), and to develop more effective, targeted speech-rate adjustments using precise machine-generated speech control.

Method: Used reverse-correlation experiments to identify temporal speech-rate patterns affecting vowel perception; tested these patterns on both L1-English and L2-English listeners; developed data-driven text-to-speech algorithm to replicate optimal temporal structure; compared targeted adjustments against global slowing across various sentences and vowel contrasts.

Result: Identified a scissor-like temporal pattern with opposite effects in early vs. late context windows that is stable across individuals and language backgrounds; targeted slowing based on this pattern improved comprehension for both L2 and native listeners in challenging conditions; global slowing increased comprehension errors despite being perceived as clearer; listeners remained unaware of targeted improvements.

Conclusion: Targeted speech-rate adjustments following specific temporal patterns significantly aid intelligibility under challenging conditions while often going unnoticed, providing a data-driven methodology to improve machine-generated speech accessibility that can be extended to other aspects of speech comprehension.
...
### [163] [OneComp: One-Line Revolution for Generative AI Model Compression](https://arxiv.org/abs/2603.28845)
...
TL;DR: OneComp is an open-source compression framework that automates post-training quantization of foundation models, transforming expert workflows into reproducible, hardware-adaptive pipelines with progressive refinement stages.


<details>
  <summary>Details</summary>
Motivation: Deploying foundation models faces constraints from memory footprint, latency, and hardware costs, while existing post-training compression approaches present fragmented, challenging workflows requiring expert navigation of quantization algorithms, precision budgets, calibration strategies, and hardware dependencies.

Method: OneComp automatically inspects models, plans mixed-precision assignments, and executes progressive quantization stages (layer-wise compression, block-wise refinement, global refinement) based on model identifier and available hardware, treating the first quantized checkpoint as a deployable pivot for incremental improvement.

Result: The framework bridges algorithmic innovation and production deployment by converting state-of-the-art compression research into an extensible, open-source, hardware-aware pipeline that ensures quality improves with increased compute investment.

Conclusion: OneComp transforms the fragmented expert workflow of model compression into a reproducible, resource-adaptive pipeline that enables practical implementation of post-training quantization for foundation model deployment.
...
### [164] [Structural Pass Analysis in Football: Learning Pass Archetypes and Tactical Impact from Spatio-Temporal Tracking Data](https://arxiv.org/abs/2603.28916)
*Oktay Karakuş,Hasan Arkadaş*

Main category: cs.LG

TL;DR: A framework for analyzing football passes based on their structural impact on defensive organization, using tracking data to quantify how passes alter defender configurations and reveal tactical patterns.


<details>
  <summary>Details</summary>
Motivation: Existing approaches evaluate passes primarily through outcome-based metrics like scoring probability or possession value, providing limited insight into how passes influence the opponent's defensive organization. There's a need for structural analysis that quantifies how passes alter the spatial configuration of defenders.

Method: Developed a structural framework using synchronized tracking/event data to derive three complementary metrics: Line Bypass Score, Space Gain Metric, and Structural Disruption Index. These quantify how passes alter defensive spatial configurations and are combined into a composite Tactical Impact Value (TIV). Applied unsupervised clustering to structural features and analyzed data from the 2022 FIFA World Cup.

Result: Identified four interpretable pass archetypes: circulatory, destabilising, line-breaking, and space-expanding passes. Passes with higher TIV are significantly more likely to lead to territorial progression (final third and penalty box entries). Revealed distinctive structural passing styles across teams, identified build-up defenders as key drivers of structural progression, and discovered impactful passing partnerships that amplify tactical progression.

Conclusion: The proposed structural framework demonstrates how representations derived from tracking data can reveal interpretable tactical patterns in football, moving beyond outcome-based metrics to quantify how passes influence defensive organization and create tactical advantages.
...
### [165] [Beta-Scheduling: Momentum from Critical Damping as a Diagnostic and Correction Tool for Neural Network Training](https://arxiv.org/abs/2603.28921)
*Ivan Pasichnyk*

Main category: cs.LG

TL;DR: Derives time-varying momentum schedule from critically damped harmonic oscillator, achieving faster convergence and enabling cross-optimizer invariant gradient attribution for identifying and correcting problem layers.


<details>
  <summary>Details</summary>
Motivation: Standard neural network training uses constant momentum (typically 0.9) with limited theoretical justification for its optimality. The paper aims to provide a principled, parameter-free momentum schedule and diagnostic tool for identifying problem layers in trained networks.

Method: Derives a time-varying momentum schedule from the critically damped harmonic oscillator: μ(t) = 1 - 2√α(t), where α(t) is the current learning rate. This beta-schedule requires zero free parameters beyond the existing learning rate schedule. Also proposes a hybrid schedule combining physics momentum for early convergence with constant momentum for final refinement.

Result: On ResNet-18/CIFAR-10, beta-scheduling delivers 1.9x faster convergence to 90% accuracy compared to constant momentum. Per-layer gradient attribution produces cross-optimizer invariant diagnostic identifying the same three problem layers regardless of optimizer (100% overlap). Surgical correction of only these layers fixes 62 misclassifications while retraining only 18% of parameters. Hybrid schedule reaches 95% accuracy fastest among five methods tested.

Conclusion: The main contribution is not an accuracy improvement but a principled, parameter-free tool for localizing and correcting specific failure modes in trained networks. The physics-derived momentum schedule provides faster convergence and enables consistent diagnostic analysis across different optimizers.
...
### [166] [A Neural Tension Operator for Curve Subdivision across Constant Curvature Geometries](https://arxiv.org/abs/2603.28937)
*Hassan Ugail,Newton Howard*

Main category: cs.LG

TL;DR: A learned subdivision scheme replaces global tension parameters with per-edge insertion angles predicted by a small neural network, working across Euclidean, spherical, and hyperbolic geometries with theoretical guarantees.


<details>
  <summary>Details</summary>
Motivation: Classical subdivision schemes use a single global tension parameter and require separate formulations for different geometries (Euclidean, spherical, hyperbolic). This limits adaptability and requires different implementations for each geometry.

Method: Introduces a shared learned tension predictor with a 140K-parameter network that takes local intrinsic features and a geometry embedding as input. It predicts per-edge insertion angles that drive geometry-specific insertion operators across all three spaces without architectural changes. Uses a constrained sigmoid output to enforce structural safety bounds.

Result: On 240 validation curves, achieves lower bending energy and angular roughness than fixed-tension and manifold-lift baselines. On ISS orbital ground-track example, reduces bending energy by 41% and angular roughness by 68% with modest Hausdorff distance increase. Riemannian manifold lifts retain pointwise-fidelity advantage.

Conclusion: The learned predictor provides a unified approach across geometries, offers theoretical guarantees (tangent-safe insertions, convergence certificate), and generalizes beyond training distribution while achieving better smoothness-fidelity trade-offs than classical methods.
...
### [167] [Foundations of Polar Linear Algebra](https://arxiv.org/abs/2603.28939)
*Giovanni Guasti*

Main category: cs.LG

TL;DR: Polar Linear Algebra introduces a spectral framework for operator learning using polar geometry with radial and angular components, reducing parameters/complexity while enabling parallelization through decoupled eigenmodes.


<details>
  <summary>Details</summary>
Motivation: To provide a new conceptual approach to operator learning by moving from spatial to spectral domains, enabling better interpretability, reduced computational complexity, and natural parallelization through decoupled spectral modes.

Method: Introduces Polar Linear Algebra framework based on polar geometry with linear radial and periodic angular components. Defines associated operators and analyzes their spectral properties. Imposes self-adjoint-inspired spectral constraints for stability. Evaluates on MNIST benchmark.

Result: Polar and fully spectral operators can be trained reliably. Self-adjoint-inspired constraints improve stability and convergence. Framework reduces parameter count and computational complexity while providing interpretable decoupled spectral representations. Enables additional dimension of model parallelization.

Conclusion: Polar Linear Algebra offers a novel spectral perspective for operator learning, particularly valuable for problems where spectral structure and parallel execution are important, providing interpretability, efficiency, and natural parallelization advantages.
...
TL;DR: ReproMIA is a novel membership inference attack framework that uses model reprogramming to amplify privacy leakage signals, achieving state-of-the-art performance with significantly lower computational costs than traditional shadow model approaches.


<details>
  <summary>Details</summary>
Motivation: Deep learning models' data memorization raises privacy concerns, but conventional Membership Inference Attacks (MIAs) face prohibitive computational costs from shadow model training and suffer performance degradation under low False Positive Rate constraints.

Method: Leverages model reprogramming as an active signal amplifier for privacy leakage, creating a unified proactive framework that induces and magnifies latent privacy footprints in model representations. Specialized implementations for LLMs, Diffusion Models, and Classification Models.

Result: Consistently outperforms state-of-the-art baselines across 10+ benchmarks and diverse architectures. For LLMs: 5.25% AUC and 10.68% TPR@1%FPR average increase. For Diffusion Models: 3.70% AUC and 12.40% TPR@1%FPR average increase. Particularly transformative in low-FPR regimes.

Conclusion: ReproMIA provides an efficient, proactive framework for membership inference that overcomes computational limitations of shadow model approaches and significantly improves detection performance, especially in practical low-FPR scenarios critical for privacy auditing.
...
### [169] [Differentiable Initialization-Accelerated CPU-GPU Hybrid Combinatorial Scheduling](https://arxiv.org/abs/2603.28943)
*Mingju Liu,Jiaqi Yin,Alvaro Velasquez,Cunxi Yu*

Main category: cs.LG

TL;DR: A hybrid CPU-GPU framework combining differentiable optimization with classical ILP solvers for combinatorial scheduling problems, achieving 10× speedup and <0.1% optimality gap.


<details>
  <summary>Details</summary>
Motivation: Combinatorial scheduling problems are NP-hard and challenging to solve optimally at scale, despite being fundamental to many optimization tasks in computing systems. Existing ILP solvers struggle with early pruning and solution quality.

Method: A hybrid CPU-GPU framework that uses differentiable presolving to rapidly generate high-quality partial solutions, which serve as warm-starts for commercial ILP solvers (CPLEX, Gurobi) and open-source solver HiGHS, enabling improved early pruning.

Result: Empirical results show up to 10× performance gain over baseline solvers, narrowing the optimality gap to <0.1% across industry-scale benchmarks.

Conclusion: This work demonstrates the first successful integration of differentiable optimization to initialize exact ILP solvers for combinatorial scheduling, creating new opportunities to combine machine learning infrastructure with classical exact optimization methods.
...
### [170] [Multi-Agent LLMs for Adaptive Acquisition in Bayesian Optimization](https://arxiv.org/abs/2603.28959)
*Andrea Carbonati,Mohammadsina Almasi,Hadis Anahideh*

Main category: cs.LG

TL;DR: LLMs struggle with exploration-exploitation trade-offs in optimization; single-agent approaches suffer cognitive overload, while multi-agent decomposition improves performance by making search policies explicit and adjustable.


<details>
  <summary>Details</summary>
Motivation: The exploration-exploitation trade-off is fundamental to sequential decision-making and black-box optimization, but how LLMs reason about and manage this trade-off remains poorly understood. Unlike Bayesian Optimization with explicit acquisition functions, LLM-based optimization relies on implicit, prompt-based reasoning over historical evaluations, making search behavior difficult to analyze or control.

Method: The paper proposes a multi-agent framework that decomposes exploration-exploitation control into strategic policy mediation and tactical candidate generation. A strategy agent assigns interpretable weights to multiple search criteria (informativeness, diversity, representativeness), while a generation agent produces candidates conditioned on the resulting search policy defined as weights. This decomposition makes exploration-exploitation decisions explicit, observable, and adjustable.

Result: Empirical results across various continuous optimization benchmarks indicate that separating strategic control from candidate generation substantially improves the effectiveness of LLM-mediated search. Single-agent LLM approaches, which jointly perform strategy selection and candidate generation within a single prompt, suffer from cognitive overload, leading to unstable search dynamics and premature convergence.

Conclusion: Decomposing exploration-exploitation control into separate strategic and tactical components addresses the cognitive overload in single-agent LLM approaches, making search policies interpretable and controllable, thereby improving optimization performance across continuous benchmarks.
...
### [171] [The Spectral Edge Thesis: A Mathematical Framework for Intra-Signal Phase Transitions in Neural Network Training](https://arxiv.org/abs/2603.28964)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: The spectral edge thesis identifies that neural network training phase transitions (grokking, capability gains, loss plateaus) are controlled by the spectral gap of the rolling-window Gram matrix of parameter updates, with the intra-signal gap at position k* being the dynamically privileged structure that governs learning dynamics.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental mechanisms controlling phase transitions in neural network training (grokking, capability gains, loss plateaus) and develop a unified theoretical framework that explains these phenomena through spectral properties of parameter updates.

Method: Developed the spectral edge thesis from three axioms, deriving: (1) gap dynamics governed by a Dyson-type ODE with curvature asymmetry, damping, and gradient driving; (2) spectral loss decomposition linking each mode's learning contribution to its Davis-Kahan stability coefficient; (3) the Gap Maximality Principle showing k* is the unique dynamically privileged position. Introduced adiabatic parameter A = ||ΔG||_F / (η g²) to control circuit stability. Tested across six model families (150K-124M parameters) with 24 grokking events.

Result: Gap dynamics precede every grokking event (24/24 with weight decay, 0/24 without), gap position is optimizer-dependent (Muon: k*=1, AdamW: k*=2 on same model), and 19/20 quantitative predictions are confirmed. The framework is consistent with edge of stability, Tensor Programs, Dyson Brownian motion, Lottery Ticket Hypothesis, and neural scaling laws.

Conclusion: The spectral edge thesis provides a unified framework explaining neural network training phase transitions through spectral properties of parameter updates, with the intra-signal gap at position k* being the fundamental control mechanism that governs learning dynamics and stability across different optimizers and model architectures.
...
### [172] [An Explicit Surrogate for Gaussian Mixture Flow Matching with Wasserstein Gap Bounds](https://arxiv.org/abs/2603.28992)
*Elham Rostami,Taous-Meriem Laleg-Kirati,Hamidou Tembine*

Main category: cs.LG

TL;DR: Training-free flow matching between Gaussian mixture models using explicit velocity fields, with a closed-form surrogate for pairwise kinetic transport cost that approximates the exact Gaussian Wasserstein cost.


<details>
  <summary>Details</summary>
Motivation: To develop efficient flow matching between Gaussian mixture models without training, providing a simpler alternative to exact Gaussian Wasserstein computations that require matrix square-root operations.

Method: Constructs component-wise Gaussian paths with affine velocity fields satisfying continuity equation, yielding closed-form surrogate cost. Analyzes approximation quality with second-order agreement proofs and cubic error bounds. Introduces path-splitting strategy for nonlocal regimes to localize covariance evolution.

Result: Derived analytic expression for surrogate cost, proved second-order agreement in local commuting regime with explicit cubic error bound. Developed practical regime map showing when surrogate is accurate versus when exact Gaussian Wasserstein geodesic method is preferable.

Conclusion: The training-free flow matching approach provides a practical alternative to exact Gaussian Wasserstein computations, with theoretical guarantees on approximation quality and a path-splitting strategy for handling nonlocal regimes, enabling efficient transport between Gaussian mixture models.
...
### [173] [Improving Efficiency of GPU Kernel Optimization Agents using a Domain-Specific Language and Speed-of-Light Guidance](https://arxiv.org/abs/2603.29010)
*Siva Kumar Sastry Hari,Vignesh Balaji,Sana Damani,Qijing Huang,Christos Kozyrakis*

Main category: cs.LG

TL;DR: μCUTLASS introduces a DSL for LLM-based GPU kernel optimization with SOL guidance to improve efficiency by reducing search space and preventing benchmark gaming.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based GPU kernel optimization suffers from inefficient search: operating at wrong abstraction levels wastes reasoning on low-impact details or misses optimization opportunities, and agents can't identify diminishing returns, leading to resource waste.

Method: Two design principles: (1) A compact domain-specific language (DSL) that enables higher-level reasoning while preserving optimization levers, and (2) Speed-of-Light (SOL) guidance using first-principles performance bounds to steer and budget search. Implemented as μCUTLASS, a DSL with compiler for CUTLASS-backed GPU kernels covering configuration, epilogue fusion, and multi-stage pipelines.

Result: On 59 KernelBench problems: DSL alone turns 0.40x regression into 1.27x speedup over PyTorch; adding SOL guidance raises to 1.56x. Weaker models with μCUTLASS + SOL outperform stronger baseline agents at lower token cost. SOL-guided budgeting saves 19-43% tokens while retaining ≥95% of geomean speedup, with best policy reaching 1.68x efficiency gain. SOL analysis detects benchmark-gaming kernels.

Conclusion: The combination of a compact DSL and SOL guidance significantly improves LLM-based GPU kernel optimization efficiency by enabling appropriate abstraction levels and intelligent search steering, reducing computational cost while maintaining or improving performance gains.
...
### [174] [From Astronomy to Astrology: Testing the Illusion of Zodiac-Based Personality Prediction with Machine Learning](https://arxiv.org/abs/2603.29033)
*Abhinna Sundar Samantaray,Finnja Annika Fluhrer,Dhruv Saini,Omkar Charaple,Anish Kumar Singh,Dhruv Vansraj Rathore*

Main category: cs.LG

TL;DR: Astrology lacks predictive validity; ML experiments show zodiac-based personality prediction performs at random chance levels, suggesting astrological success stems from cognitive biases rather than genuine predictive structure.


<details>
  <summary>Details</summary>
Motivation: Despite astrology's cultural persistence and influence on social decision-making (especially in South Asian societies), it lacks both physical plausibility and statistical reliability. The paper aims to empirically test zodiac-based personality prediction using controlled machine learning methods.

Method: Constructed synthetic dataset with individuals assigned zodiac signs and personality labels from 100 human traits. Each sign associated with 10 overlapping descriptors to mimic real astrological ambiguity. Trained Logistic Regression, Random Forest, and neural-network classifiers to predict personality from zodiac features and nuisance covariates, with shuffled-label controls.

Result: Predictive performance remained at or near random expectation across all experiments. Shuffled-label controls yielded comparable accuracies, indicating no genuine predictive structure in zodiac-based systems.

Conclusion: Astrology's apparent success arises from trait universality, category overlap, cognitive biases (Barnum effect, confirmation bias), and interpretive flexibility rather than measurable predictive structure. Zodiac systems function as culturally durable narrative frameworks rather than reliable predictors of human behavior.
...
### [175] [A Latent Risk-Aware Machine Learning Approach for Predicting Operational Success in Clinical Trials based on TrialsBank](https://arxiv.org/abs/2603.29041)
...
TL;DR: Hierarchical latent risk-aware ML framework predicts clinical trial operational success using pre-trial features, achieving high F1-scores (0.91-0.93) across phases.


<details>
  <summary>Details</summary>
Motivation: Clinical trials face high costs, extended timelines, and operational risks, but reliable prospective prediction methods are limited. Existing AI approaches often use metrics unavailable at trial design or focus on isolated aspects, limiting real-world applicability.

Method: Two-stage hierarchical framework: first predicts intermediate latent operational risk factors from 180+ drug- and trial-level features available pre-initiation, then integrates these predicted risks into downstream model to estimate operational success probability. Uses curated TrialsBank dataset (13,700 trials) with staged data-splitting to prevent leakage. Benchmarked with XGBoost, CatBoost, and Explainable Boosting Machines.

Result: Strong out-of-sample performance across Phase I-III: F1-scores of 0.93, 0.92, and 0.91 respectively. Latent risk drivers improve discrimination of operational failures. Performance remains robust under independent inference evaluation.

Conclusion: Clinical trial operational success can be prospectively forecasted using latent risk-aware AI framework, enabling early risk assessment and supporting data-driven clinical development decision-making.
...
### [176] [ARCS: Autoregressive Circuit Synthesis with Topology-Aware Graph Attention and Spec Conditioning](https://arxiv.org/abs/2603.29068)
...
<details>
  <summary>Details</summary>
Motivation: Current search-based methods for analog circuit generation require minutes per design and numerous SPICE simulations, creating a need for faster approaches that can enable rapid prototyping, design-space exploration, and warm-starting of optimization methods.

Method: Hybrid pipeline combining two learned generators (graph VAE and flow-matching model) with SPICE-based ranking; uses Group Relative Policy Optimization (GRPO) with per-topology advantage normalization to address REINFORCE failure modes; employs grammar-constrained decoding with topology-aware token masking for structural validity; includes Best-of-3 candidate selection for single-model inference.

Result: Achieves 99.9% simulation validity (reward 6.43/8.0) across 32 topologies using only 8 SPICE evaluations (40x fewer than genetic algorithms); single-model inference reaches 85% simulation validity in 97ms (600x faster than random search); GRPO improves simulation validity by +9.6pp over REINFORCE in only 500 RL steps; recovers 96.6% of GA quality with 49% fewer simulations.

Conclusion: ARCS provides >1000x speed advantage over search-based methods, enabling rapid prototyping and design-space exploration, though it doesn't yet match per-design quality of optimization-based approaches; the system effectively trades some quality for massive speed improvements and can warm-start search methods.
...
### [177] [On the Mirage of Long-Range Dependency, with an Application to Integer Multiplication](https://arxiv.org/abs/2603.29069)
*Zichao Wei*

Main category: cs.LG

TL;DR: Neural networks can perfectly generalize on integer multiplication using a 2D outer-product grid representation with local operations, challenging the belief that multiplication inherently requires long-range dependencies.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the conventional wisdom that integer multiplication is difficult for neural networks due to O(n) long-range dependencies from carry chains. The authors argue this is a mirage caused by the choice of computational representation rather than an intrinsic property of multiplication.

Method: The authors formalize the concept of "mirage" and provide a constructive proof: by representing two n-bit binary integers as a 2D outer-product grid, each step of long multiplication becomes a 3×3 local neighborhood operation. They implement this using a neural cellular automaton with only 321 learnable parameters.

Result: The neural cellular automaton achieves perfect length generalization up to 683× the training range. In contrast, five alternative architectures (Transformer with 6,625 parameters, Transformer+RoPE, and Mamba) all fail under the same representation.

Conclusion: The paper demonstrates that long-range dependency in integer multiplication is not intrinsic but induced by computational spacetime. It suggests that tasks diagnosed as requiring long-range dependency should first be examined to determine whether the dependency is inherent or representation-induced.
...
### [178] [Realistic Market Impact Modeling for Reinforcement Learning Trading Environments](https://arxiv.org/abs/2603.29086)
*Lucas Riera Abbade,Anna Helena Reali Costa*

Main category: cs.LG

TL;DR: The paper introduces three realistic trading environments with nonlinear market impact models based on Almgren-Chriss framework, showing that transaction cost modeling dramatically affects RL algorithm performance and trading behavior.


<details>
  <summary>Details</summary>
Motivation: Most existing RL trading environments assume negligible or fixed transaction costs, leading to agents learning unrealistic trading behaviors that fail under real market execution conditions with nonlinear market impact.

Method: Developed three Gymnasium-compatible trading environments (MACE stock trading, margin trading, portfolio optimization) integrating Almgren-Chriss framework and square-root impact law with pluggable cost models, permanent impact tracking, and trade-level logging. Evaluated five DRL algorithms (A2C, PPO, DDPG, SAC, TD3) on NASDAQ-100 comparing fixed 10 bps baseline against AC model with Optuna hyperparameter tuning.

Result: Cost models significantly change absolute performance and algorithm rankings; AC model produces dramatically different trading behavior (daily costs drop from $200k to $8k, turnover from 19% to 1%); hyperparameter optimization reduces costs up to 82%; algorithm-cost model interactions are strongly environment-specific (e.g., DDPG's OOS Sharpe jumps from -2.1 to 0.3 under AC in margin trading while SAC's drops from -0.5 to -1.2).

Conclusion: Realistic transaction cost modeling is essential for RL-based trading systems, as it fundamentally changes algorithm performance and trading behavior. The released open-source suite extends FinRL-Meta with market-impact-aware environments for more realistic trading algorithm development.
...
### [179] [Efficient Bilevel Optimization with KFAC-Based Hypergradients](https://arxiv.org/abs/2603.29108)
*Disen Liao,Felix Dangel,Yaoliang Yu*

Main category: cs.LG

TL;DR: KFAC-based hypergradient approximation for bilevel optimization provides curvature-aware updates with better efficiency than existing methods like CG or Neumann expansions, outperforming gradient unrolling across diverse tasks including meta-learning and AI safety.


<details>
  <summary>Details</summary>
Motivation: Current bilevel optimization methods for machine learning rely on crude approximations for hypergradients (inverse Hessian-vector products) that discard curvature information, limiting scaling and performance.

Method: Proposes incorporating Kronecker-factored approximate curvature (KFAC) into implicit function theorem-based algorithms to compute curvature-aware hypergradients with modest memory and runtime overhead.

Result: KFAC-based approach shows better performance-efficiency trade-off than Conjugate Gradient or Neumann methods, consistently outperforms gradient unrolling, and demonstrates value of curvature information at scale (up to BERT models).

Conclusion: Curvature information is valuable for scaling bilevel optimization, and KFAC provides an effective way to incorporate it with practical overhead, enabling better performance across diverse ML tasks.
...
### [180] [Quality-Controlled Active Learning via Gaussian Processes for Robust Structure-Property Learning in Autonomous Microscopy](https://arxiv.org/abs/2603.29135)
*Jawad Chowdhury,Ganesh Narasimha,Jan-Chi Yang,Yongtao Liu,Rama Vasudevan*

Main category: cs.LG

TL;DR: A gated active learning framework combining curiosity-driven sampling with physics-informed quality control improves autonomous materials experiments by filtering out noisy data during acquisition, outperforming standard methods in both simulated and real microscopy applications.


<details>
  <summary>Details</summary>
Motivation: Autonomous experimental systems in materials research suffer from performance limitations due to low-quality, noisy data, particularly in structure-property learning tasks like Image-to-Spectrum and Spectrum-to-Image translations. Standard active learning strategies often mistakenly prioritize poor-quality measurements by misinterpreting noise as uncertainty.

Method: A gated active learning framework that combines curiosity-driven sampling with a physics-informed quality control filter based on Simple Harmonic Oscillator model fits. The system automatically excludes low-fidelity data during acquisition by using the physics model to assess data quality before incorporating measurements into training.

Result: The proposed method outperforms random sampling, standard active learning, and multitask learning strategies on pre-acquired BEPS data from PbTiO3 thin films with spatially localized noise. It enhances both Im2Spec and Spec2Im tasks by handling noise during training and acquisition, leading to more reliable forward and inverse predictions. Real-time deployment on BiFeO3 thin films demonstrated effectiveness in autonomous microscopy experiments.

Conclusion: The work supports a shift toward hybrid autonomy in self-driving labs, where physics-informed quality assessment and active decision-making work together for more reliable scientific discovery. The gated approach prevents standard active learners from acquiring bad samples that hurt performance by misinterpreting noise as uncertainty.
...
### [181] [Efficient and Scalable Granular-ball Graph Coarsening Method for Large-scale Graph Node Classification](https://arxiv.org/abs/2603.29148)
*Guan Wang,Shuyin Xia,Lei Qian,Guoyin Wang,Yi Liu,Yi Wang,Wei Wang*

Main category: cs.LG

TL;DR: Proposes a granular-ball graph coarsening method for efficient large-scale GCN training with linear time complexity and multi-granularity graph structure preservation.


<details>
  <summary>Details</summary>
Motivation: GCNs face high computational overhead on large-scale graphs, especially with many convolutional layers. Existing sampling and coarsening methods either ignore multi-granularity information or have high time complexity.

Method: Uses multi-granularity granular-ball graph coarsening algorithm to coarsen original graph into subgraphs with linear time complexity, then randomly samples these granular-ball subgraphs to form minibatches for GCN training.
...
Conclusion: The proposed granular-ball graph coarsening method effectively addresses GCN scalability issues by adaptively reducing graph scale with linear time complexity while preserving multi-granularity structural information.
...
### [182] [Dummy-Aware Weighted Attack (DAWA): Breaking the Safe Sink in Dummy Class Defenses](https://arxiv.org/abs/2603.29182)
*Yunrui Yu,Xuxiang Feng,Pengda Qin,Pengyang Wang,Kafeng Wang,Cheng-zhong Xu,Hang Su,Jun Zhu*

Main category: cs.LG

TL;DR: Dummy Classes-based defenses exploit limitations in conventional adversarial robustness evaluation by using a dummy class as a safety sink, leading to overestimated robustness. The paper proposes DAWA, a dummy-aware weighted attack that simultaneously targets both true and dummy labels, significantly reducing measured robustness.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial robustness evaluation methods have limitations that can be exploited by new defense paradigms, particularly Dummy Classes-based defenses that use an additional dummy class as a safety sink for adversarial examples. Conventional attacks like AutoAttack focus only on misleading the true class label, which aligns perfectly with these defenses' mechanism, leading to significantly overestimated robustness measurements.

Method: The paper proposes Dummy-Aware Weighted Attack (DAWA), a novel evaluation method that simultaneously targets both the true label and dummy label during adversarial example synthesis. DAWA uses adaptive weighting to balance the attack objectives against both classes, effectively breaking the defense mechanism by not allowing successful attacks to be simply captured by the dummy class.

Result: DAWA significantly reduces the measured robustness of Dummy Classes-based defenses. In experiments, it reduced the robustness of a leading defense from 58.61% to 29.52% on CIFAR-10 under l_infty perturbation (epsilon=8/255), demonstrating that conventional evaluation methods substantially overestimate the actual robustness of these defenses.

Conclusion: The work provides a more reliable benchmark for evaluating Dummy Classes-based defenses and highlights the need for continuous evolution of robustness assessment methodologies. It reveals that defense mechanisms can exploit limitations in existing evaluation strategies, emphasizing the importance of developing attack methods that account for novel defense paradigms.
...
### [183] [IMPACT: Influence Modeling for Open-Set Time Series Anomaly Detection](https://arxiv.org/abs/2603.29183)
...
<details>
  <summary>Details</summary>
Motivation: Current open-set anomaly detection methods fail on time series data because they don't preserve sequential nature, generate trivial/unrealistic anomalies, and struggle with contaminated training data containing unlabeled anomalies.

Method: IMPACT learns an influence function to estimate individual training sample impacts, then uses influence scores to generate semantically divergent yet realistic unseen anomalies and repurposes high-influential samples as supervised anomalies for decontamination.

Result: Extensive experiments show IMPACT significantly outperforms existing state-of-the-art methods, demonstrating superior accuracy across varying OSAD settings and contamination rates.

Conclusion: IMPACT effectively addresses the limitations of existing methods for time series OSAD by leveraging influence modeling to generate realistic unseen anomalies and handle contaminated training data.
...
### [184] [Biomimetic PINNs for Cell-Induced Phase Transitions: UQ-R3 Sampling with Causal Gating](https://arxiv.org/abs/2603.29184)
*Anci Lin,Xiaohong Liu,Zhiwen Zhang,Weidong Zhao,Wenju Zhao*

Main category: cs.LG

TL;DR: Bio-PINNs: A biomimetic physics-informed neural network framework that addresses over-smoothing in cell-induced phase transitions by encoding temporal causality into spatial causality via progressive distance gates and using deformation-uncertainty proxies for adaptive collocation.


<details>
  <summary>Details</summary>
Motivation: Nonconvex multi-well energies in cell-induced phase transitions create sharp interfaces, fine-scale microstructures, and distance-dependent inter-cell coupling, posing significant challenges for physics-informed learning. Existing methods suffer from over-smoothing in near-field patterns, failing to capture the intricate microstructural features essential for accurate modeling of these complex biological phenomena.

Method: Bio-PINNs use a variational framework that encodes temporal causality into explicit spatial causality through a progressive distance gate. They leverage a deformation-uncertainty proxy for interfacial length scale to target microstructure-prone regions, providing efficient alternative to explicit second-derivative regularization. The method employs an uncertainty-driven "retain-resample-release" adaptive collocation strategy with theoretical guarantees for persistent coverage under gating and quantitative near-to-far growth bounds.

Result: Bio-PINNs consistently recover sharp transition layers and tether morphologies across single- and multi-cell benchmarks, diverse separations, and various regularization regimes. They significantly outperform state-of-the-art adaptive and ungated baselines, demonstrating superior capability in capturing the complex microstructural features of cell-induced phase transitions.

Conclusion: The Bio-PINNs framework successfully addresses the over-smoothing problem in physics-informed learning for cell-induced phase transitions by introducing biomimetic principles, progressive distance gating, and uncertainty-driven adaptive collocation. The method provides both theoretical guarantees and practical performance improvements for modeling complex biological systems with sharp interfaces and fine-scale microstructures.
...
### [185] [Improving Ensemble Forecasts of Abnormally Deflecting Tropical Cyclones with Fused Atmosphere-Ocean-Terrain Data](https://arxiv.org/abs/2603.29200)
*Qixiang Li,Shuwei Huo,Chong Wang,Xiaofeng Li,Yuan Zhou*

Main category: cs.LG

TL;DR: AOT-TCs dataset and model for tropical cyclone forecasting that integrates atmosphere-ocean-terrain data and handles both normal and abnormal deflected TCs in the Northwest Pacific basin.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for TC forecasting have limitations: they only process single-type sequential trajectory data or homogeneous meteorological variables, and fail to accurately forecast abnormal deflected TCs. Current approaches lack comprehensive multimodal integration of heterogeneous variables from different physical domains.

Method: Two main contributions: 1) Construction of AOT-TCs dataset - a multimodal, multi-source dataset integrating heterogeneous variables from atmosphere, ocean, and land for TC forecasting in the Northwest Pacific basin. 2) Development of a forecasting model with explicit atmosphere-ocean-terrain coupling architecture that can handle both normal and abnormally deflected TCs.

Result: Extensive experiments on all TC cases in the Northwest Pacific from 2017-2024 show state-of-the-art performance: significant improvement in forecasting accuracy for normal TCs and breakthrough in forecasting abnormally deflected TCs, which was previously a technical bottleneck.

Conclusion: The proposed AOT-TCs dataset and explicit atmosphere-ocean-terrain coupling model represent groundbreaking contributions that overcome key limitations of existing deep learning methods for TC forecasting, enabling comprehensive multimodal integration and accurate prediction of both normal and abnormal TC behavior.
...
### [186] [Derived Fields Preserve Fine-Scale Detail in Budgeted Neural Simulators](https://arxiv.org/abs/2603.29224)
*Wenshuo Wang,Fan Zhang*

Main category: cs.LG

TL;DR: DerivOpt improves fine-scale fidelity in neural PDE simulation by optimizing which physical fields to carry and how to allocate storage budget across them, addressing state construction as a primary bottleneck under fixed storage constraints.


<details>
  <summary>Details</summary>
Motivation: Fine-scale-faithful neural simulation under fixed storage budgets is challenging because existing methods focus on architectures, training objectives, or rollout strategies, while fine detail can be lost during state construction in budgeted coarsen-quantize-decode pipelines.
...
### [187] [Robust and Consistent Ski Rental with Distributional Advice](https://arxiv.org/abs/2603.29233)
*Jihwan Kim,Chenglin Fan*

Main category: cs.LG

TL;DR: A framework for ski rental problem that integrates full distributional predictions (not just point estimates) into both deterministic and randomized algorithms, with rigorous robustness guarantees and improved consistency.


<details>
  <summary>Details</summary>
Motivation: Classical ski rental algorithms focus on worst-case competitive ratios, while recent learning-augmented methods use point-estimate predictions. Neither fully exploits rich distributional predictions while maintaining robustness guarantees.

Method: For deterministic setting: formalize problem under perfect distributional prediction, derive optimal threshold-buy day algorithm, propose Clamp Policy with tunable parameter for imperfect predictions. For randomized setting: characterize stopping distribution via Water-Filling Algorithm that optimizes expected cost while satisfying robustness constraints.

Result: Identifies sufficient conditions where expected competitive ratio matches classic optimal randomized bound. Clamp Policy is robust to prediction errors and consistent with accurate predictions. Water-Filling Algorithm maintains robustness while optimizing expected cost. Experiments show significant consistency improvements over point-prediction baselines across Gaussian, geometric, and bi-modal distributions.

Conclusion: The framework successfully integrates distributional advice into ski rental algorithms, achieving better consistency than point-prediction methods while maintaining comparable robustness, bridging the gap between classical worst-case analysis and learning-augmented approaches.
...
### [188] [Stochastic Dimension Implicit Functional Projections for Exact Integral Conservation in High-Dimensional PINNs](https://arxiv.org/abs/2603.29237)
*Zhangyong Liang*

Main category: cs.LG

TL;DR: SDIFP is a framework for enforcing exact macroscopic conservation laws in neural PDE solvers using stochastic dimension implicit functional projection with doubly-stochastic unbiased gradient estimation for scalable, mesh-free high-dimensional PDE solving.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for enforcing exact macroscopic conservation laws (mass, energy) in neural PDE solvers face computational challenges: discrete projections rely on deterministic quadrature that scales poorly in high dimensions, restrict mesh-free formulations like PINNs, incur heavy memory overhead from high-order operators, and generic optimization lacks convergence guarantees for non-convex conservation manifolds.

Method: Proposes Stochastic Dimension Implicit Functional Projection (SDIFP) framework that applies global affine transformation to continuous network output instead of projecting discrete vectors, yielding closed-form solutions for integral constraints via detached Monte Carlo quadrature. Introduces doubly-stochastic unbiased gradient estimator (DS-UGE) that decouples spatial sampling from differential operator subsampling to reduce memory complexity from O(M × N_ℒ) to O(N × |ℐ|).

Result: SDIFP bypasses spatial grid dependencies, mitigates sampling variance, preserves solution regularity, maintains O(1) inference efficiency, and provides a scalable, mesh-free approach for solving conservative high-dimensional PDEs with exact macroscopic conservation law enforcement.

Conclusion: SDIFP offers a computationally efficient framework for enforcing exact conservation laws in neural PDE solvers, addressing scalability limitations of traditional methods while supporting mesh-free formulations and providing convergence guarantees through stochastic optimization techniques.
...
### [189] [Monodense Deep Neural Model for Determining Item Price Elasticity](https://arxiv.org/abs/2603.29261)
*Lakshya Garg,Sai Yaswanth,Deep Narayan Mishra,Karthik Kumaran,Anupriya Sharma,Mayank Uniyal*

Main category: cs.LG

TL;DR: Proposes a novel machine learning framework for estimating item-level price elasticity from transactional data without requiring treatment-control settings, with a Monodense-DL neural network outperforming other ML methods.


<details>
  <summary>Details</summary>
Motivation: Businesses need accurate price elasticity estimates to optimize pricing strategies and revenue management, but traditional methods often require controlled experimental settings that are impractical in real-world retail environments.

Method: A novel elasticity estimation framework that works without treatment-control settings, using three ML approaches: (1) Monodense-DL network (hybrid neural network with embedding, dense, and Monodense layers), (2) Double Machine Learning (DML) with regression models, and (3) Light Gradient Boosting Model (LGBM).

Result: Experimental evaluation on multi-category retail data with millions of transactions shows the proposed Monodense-DL neural network outperforms DML and LGBM methods in elasticity estimation accuracy.

Conclusion: The proposed framework enables practical price elasticity estimation from observational data, with the Monodense-DL architecture providing superior performance for revenue optimization and pricing strategy development.
...
### [190] [Lie Generator Networks for Nonlinear Partial Differential Equations](https://arxiv.org/abs/2603.29264)
*Shafayeth Jamil,Rehan Kapadia*

Main category: cs.LG

TL;DR: LGN-KM is a neural operator that learns continuous-time Koopman generators for nonlinear PDEs through a stable decomposition (skew-symmetric + positive-definite diagonal), enabling interpretable spectral analysis and guaranteed stability.


<details>
  <summary>Details</summary>
Motivation: For nonlinear systems governed by PDEs, there's no equivalent to the eigenspectrum theory available for linear dynamical systems. The authors aim to develop a framework that can lift nonlinear dynamics into a linear latent space while maintaining interpretability and stability guarantees.

Method: Lie Generator Network--Koopman (LGN-KM) learns the continuous-time Koopman generator L_k through a decomposition L_k = S - D_k, where S is skew-symmetric (conservative inter-modal coupling) and D_k is positive-definite diagonal (modal dissipation). This architectural decomposition enforces stability and enables direct spectral access.

Result: On 2D Navier-Stokes turbulence, the generator recovers known dissipation scaling and complete multi-branch dispersion relation from trajectory data alone. Independently trained models at different flow regimes recover matched gauge-invariant spectral structure, exposing gauge freedom in Koopman lifting. The provably stable generator enables guaranteed long-horizon stability, continuous-time evaluation at arbitrary time, and physics-informed cross-viscosity model transfer.

Conclusion: LGN-KM provides a neural operator framework that bridges the gap between linear and nonlinear dynamical systems by learning interpretable Koopman generators with stability guarantees, enabling spectral analysis of nonlinear PDEs and facilitating cross-regime model transfer.
...
### [191] [From Physics to Surrogate Intelligence: A Unified Electro-Thermo-Optimization Framework for TSV Networks](https://arxiv.org/abs/2603.29268)
*Mohamed Gharib,Leonid Popryho,Inna Partin-Vaisband*

Main category: cs.LG

TL;DR: A scalable electro-thermal modeling and optimization framework for TSV arrays using physics-informed analytical models and GNN surrogates, achieving 6 orders of magnitude speedup for design exploration.


<details>
  <summary>Details</summary>
Motivation: High-density TSVs enable 2.5D/3D heterogeneous integration but create signal-integrity and thermal-reliability challenges. Conventional FEM simulations are computationally prohibitive for large design-space exploration.

Method: Combines physics-informed analytical modeling, graph neural network (GNN) surrogates, and full-wave validation. Multi-conductor analytical model computes S-parameters and thermal conductivities. TSV-PhGNN surrogate trained on analytical data and fine-tuned with HFSS simulations.

Result: Analytical model achieves 5-10% RFE for arrays up to 15x15. GNN surrogate generalizes to larger arrays with RFE below 2%. Framework enables millions of TSV configurations to be explored within minutes with 6 orders of magnitude speedup.

Conclusion: The framework enables rapid electro-thermal co-design of TSV arrays, making exhaustive layout and geometric optimization feasible while maintaining accuracy through physics-informed modeling and surrogate techniques.
...
### [192] [LGFNet: Local-Global Fusion Network with Fidelity Gap Delta Learning for Multi-Source Aerodynamics](https://arxiv.org/abs/2603.29303)
*Qinye Zhu,Yu Xiang,Jun Zhang,Wenyong Wang*

Main category: cs.LG

TL;DR: LGFNet is a Local-Global Fusion Network that combines spatial perception with relational reasoning to extract multi-scale aerodynamic features from CFD, wind tunnel, and flight test data, using fidelity gap delta learning to preserve physical trends while capturing nonlinear discrepancies.


<details>
  <summary>Details</summary>
Motivation: Existing aerodynamic data fusion methods struggle to balance high-resolution local fidelity (e.g., shock waves) with wide-range global dependencies, often losing sharp discontinuities or failing to capture long-range topological correlations across the flight envelope.

Method: LGFNet combines: 1) spatial perception layer with sliding window mechanism for local feature extraction, 2) relational reasoning layer based on self-attention for capturing long-range flow information, and 3) fidelity gap delta learning (FGDL) strategy that treats CFD data as a "low-frequency carrier" to explicitly approximate nonlinear discrepancies while preserving physical trends.

Result: LGFNet achieves state-of-the-art performance in both accuracy and uncertainty reduction across diverse aerodynamic scenarios, demonstrating effective fusion of multi-source aerodynamic data while maintaining local discontinuities and global correlations.

Conclusion: The proposed LGFNet with FGDL strategy successfully addresses the local-global trade-off in aerodynamic data fusion, enabling comprehensive understanding of both localized flow structures and global aerodynamic trends across the entire flight envelope.
...
### [193] [Deep Learning-Based Anomaly Detection in Spacecraft Telemetry on Edge Devices](https://arxiv.org/abs/2603.29375)
*Christopher Goetze,Tim Schlippe,Daniel Lakey*

Main category: cs.LG

TL;DR: Multi-objective neural architecture optimization enables deployment of spacecraft anomaly detection models on highly constrained edge hardware, achieving 97.1% RAM reduction while maintaining 88.8% detection performance.


<details>
  <summary>Details</summary>
Motivation: Spacecraft anomaly detection is critical for mission safety, but deploying sophisticated models on-board is challenging due to hardware constraints of spacecraft edge computing platforms.
...
Result: Forecasting & threshold achieved best performance (92.7% CEF0.5). Optimized model preserved 88.8% CEF0.5 while reducing RAM by 97.1% to 59 KB and operations by 99.4%, requiring only 0.36-6.25% of CubeSat RAM.
...
### [194] [Finite-time analysis of Multi-timescale Stochastic Optimization Algorithms](https://arxiv.org/abs/2603.29380)
*Kaustubh Kartikey,Shalabh Bhatnagar*

Main category: cs.LG

TL;DR: Finite-time analysis of two smoothed functional stochastic approximation algorithms for simulation-based optimization: a two-time-scale gradient method and a three-time-scale Newton method with zeroth-order gradient/Hessian estimates.


<details>
  <summary>Details</summary>
Motivation: While asymptotic convergence of these algorithms has been established, finite-time guarantees for two-time-scale stochastic optimization algorithms in zeroth-order settings have not been provided previously. There's a need to understand the explicit interaction between multiple time-scales and propagation of estimation errors.

Method: Two algorithms: (1) Two time-scale gradient-based method using zeroth-order gradient estimates, (2) Three time-scale Newton-based algorithm estimating both gradient and Hessian using zeroth-order estimates. Analysis includes deriving mean-squared error bounds for Hessian estimator and establishing finite-time bound on gradient norm.

Result: Derived mean-squared error bounds for Hessian estimator; established finite-time bound on min₀≤m≤T 𝔼‖∇J(θ(m))‖² showing convergence to first-order stationary points; identified step-size choices that balance dominant error terms and achieve near-optimal convergence rates; validated results on Continuous Mountain Car environment.

Conclusion: The paper provides the first finite-time guarantees for two-time-scale stochastic optimization algorithms in zeroth-order settings, explicitly characterizing time-scale interactions and error propagation, with theoretical results validated experimentally.
...
### [195] [Causality-inspired Federated Learning for Dynamic Spatio-Temporal Graphs](https://arxiv.org/abs/2603.29384)
*Yuxuan Liu,Wenchao Xu,Haozhao Wang,Zhiming He,Zhaofeng Shi,Chongyang Xu,Peichao Wang,Boyuan Zhang*

Main category: cs.LG

TL;DR: SC-FSGL is a causality-inspired federated learning framework for dynamic spatio-temporal graphs that decouples transferable causal knowledge from client-specific noise through representation-level interventions and causal codebook alignment.


<details>
  <summary>Details</summary>
Motivation: Existing federated graph learning methods are designed for static graphs and rely on parameter averaging/distribution alignment, which assumes all features are equally transferable. This overlooks spatial-temporal heterogeneity and client-specific knowledge, creating issues with spurious representation entanglement, client-specific interference, and negative transfer that degrade performance in Federated Learning over Dynamic Spatio-Temporal Graphs (FSTG).

Method: Proposes SC-FSGL framework with: 1) Conditional Separation Module that simulates soft interventions through client-conditioned masks to disentangle invariant spatio-temporal causal factors from spurious signals, mitigating representation entanglement; 2) Causal Codebook that clusters causal prototypes and aligns local representations via contrastive learning to promote cross-client consistency and facilitate knowledge sharing across diverse spatio-temporal patterns.

Result: Experiments on five diverse heterogeneous Spatio-Temporal Graph datasets show that SC-FSGL outperforms state-of-the-art methods.

Conclusion: The causality-inspired approach effectively addresses the limitations of existing FGL methods for dynamic spatio-temporal graphs by explicitly decoupling transferable causal knowledge from client-specific noise, improving generalization performance in heterogeneous federated learning settings.
...
### [196] [Hybrid Quantum-Classical Spatiotemporal Forecasting for 3D Cloud Fields](https://arxiv.org/abs/2603.29407)
...
<details>
  <summary>Details</summary>
Motivation: Accurate 3D cloud field forecasting is challenging due to cross-layer interactions, nonlocal dependencies, and multiscale spatiotemporal dynamics. Existing models based on convolutions, recurrence, or attention struggle with locality-biased representations and preserving fine cloud structures in volumetric forecasting.

Method: QENO consists of four components: 1) classical spatiotemporal encoder for compact latent representation, 2) topology-aware quantum enhancement block for modeling nonlocal couplings in latent space, 3) dynamic fusion temporal unit for integrating measurement-derived quantum features with recurrent memory, and 4) decoder for reconstructing future cloud volumes.

Result: QENO consistently outperforms ConvLSTM, PredRNN++, Earthformer, TAU, and SimVP variants on CMA-MESO 3D cloud fields across MSE (0.2038), MAE, RMSE (0.4514), SSIM (0.6291), and threshold-based detection metrics while maintaining a compact parameter budget.

Conclusion: Topology-aware hybrid quantum-classical feature modeling is a promising direction for 3D cloud structure forecasting and atmospheric Earth observation data analysis, addressing limitations of locality-biased representations in existing methods.
...
### [197] [mtslearn: Machine Learning in Python for Medical Time Series](https://arxiv.org/abs/2603.29432)
*Zhongheng Jiang,Yuechao Zhao,Donglin Xie,Chenxi Sun,Rongchen Lu,Silu Luo,Zisheng Liang,Shenda Hong*

Main category: cs.LG

TL;DR: mtslearn is an end-to-end integrated toolkit designed to bridge the gap between AI technologies and clinical application by providing a unified framework for medical time-series data processing, feature engineering, model training, and visualization with minimal coding requirements.


<details>
  <summary>Details</summary>
Motivation: Real-world clinical time-series data is highly heterogeneous and inconsistently formatted, while existing machine learning tools have steep learning curves and fragmented workflows, creating a significant gap between cutting-edge AI technologies and clinical application.

Method: mtslearn provides: 1) a unified data interface that automates parsing and alignment of wide, long, and flat data formats; 2) a complete pipeline from data reading and feature engineering to model training and result visualization; 3) flexible interfaces for custom algorithms; and 4) a modular design that simplifies complex data engineering tasks into few lines of code.

Result: The toolkit significantly reduces data cleaning overhead, lowers the barrier to entry for clinicians with limited programming experience, and empowers them to focus more on exploring medical hypotheses while accelerating the translation of advanced algorithms into real-world clinical practice.

Conclusion: mtslearn addresses the critical need for accessible, integrated tools in medical time-series analysis, bridging the gap between AI research and clinical implementation by providing an end-to-end solution that simplifies complex workflows and makes advanced analytics accessible to clinical practitioners.
...
### [198] [An Isotropic Approach to Efficient Uncertainty Quantification with Gradient Norms](https://arxiv.org/abs/2603.29466)
*Nils Grünefeld,Jes Frellsen,Christian Hardmeier*

Main category: cs.LG

TL;DR: Lightweight method for quantifying predictive uncertainty in LLMs using gradient norm for epistemic uncertainty and Bernoulli variance for aleatoric uncertainty from single forward-backward pass.


<details>
  <summary>Details</summary>
Motivation: Existing uncertainty quantification methods for neural networks are computationally expensive for large language models or require training data access, which is often unavailable for pretrained models.

Method: Two approximations: 1) First-order Taylor expansion expressing uncertainty via gradient of prediction and parameter covariance, 2) Isotropy assumption on parameter covariance. This yields epistemic uncertainty as squared gradient norm and aleatoric uncertainty as Bernoulli variance of point prediction from single forward-backward pass.

Result: Method shows strong correspondence with reference MCMC estimates on synthetic problems, improving with model size. In QA benchmarks, combined uncertainty achieves high AUROC on TruthfulQA (questions with genuine answer conflicts) but performs near chance on TriviaQA (factual recall), indicating parameter-level uncertainty captures different signal than self-assessment methods.

Conclusion: The lightweight uncertainty quantification method provides efficient epistemic and aleatoric uncertainty estimates from single forward-backward pass, revealing benchmark-dependent utility where parameter-level uncertainty is informative for questions involving genuine answer conflicts but not for factual recall tasks.
...
### [199] [Survival In-Context: Prior-fitted In-context Learning Tabular Foundation Model for Survival Analysis](https://arxiv.org/abs/2603.29475)
*Dmitrii Seletkov,Paul Hager,Rickmer Braren,Daniel Rueckert,Raphael Rehms*

Main category: cs.LG

TL;DR: SIC is a prior-fitted in-context learning model for survival analysis that's pretrained on synthetic data, requiring no task-specific training and achieving competitive performance on real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Survival analysis faces challenges with limited data, censoring, and heterogeneous tabular covariates. While prior-fitted paradigms work for classification/regression, their suitability for time-to-event modeling remains unclear.

Method: Proposes a flexible survival data generation framework with explicit control over covariates and time-event distributions. Builds Survival In-Context (SIC), a prior-fitted in-context learning model pretrained exclusively on synthetic data that produces individualized survival predictions in a single forward pass without task-specific training.
...
Conclusion: The approach highlights the promise of prior-fitted foundation models for survival analysis, demonstrating that synthetic pretraining can effectively address challenges in time-to-event modeling.
...
### [200] [Why not to use Cosine Similarity between Label Representations](https://arxiv.org/abs/2603.29488)
*Beatrix M. G. Nielsen*

Main category: cs.LG

TL;DR: Cosine similarity between label representations in softmax classifiers provides no information about model probabilities due to translation ambiguity.


<details>
  <summary>Details</summary>
Motivation: Cosine similarity is commonly used to measure similarity between neural network representations, but it's unclear whether this metric provides meaningful information about model behavior, particularly for softmax classifiers where label representations are used.

Method: Theoretical proof showing that for any softmax classifier, given two label representations, it's possible to construct an equivalent model with the same probability outputs but where cosine similarity between representations is either 1 or -1. Provides specific examples demonstrating this translation ambiguity and shows that centering or fixing representation length doesn't resolve the issue.

Result: Cosine similarity between label representations in softmax classifiers is fundamentally ambiguous and cannot be used to explain model probabilities. The translation invariance property allows creating equivalent models with arbitrary cosine similarity values while maintaining identical probability distributions.

Conclusion: Cosine similarity values between label representations should not be used to explain model probabilities in softmax classifiers, as they provide no reliable information about the actual probability relationships between labels.
...
### [201] [Target-Aligned Reinforcement Learning](https://arxiv.org/abs/2603.29501)
*Leonard S. Pleiss,James Harrison,Maximilian Schiffer*

Main category: cs.LG

TL;DR: TARL (Target-Aligned Reinforcement Learning) addresses the stability-recency tradeoff in RL by focusing updates on transitions where target and online network estimates are well-aligned, accelerating convergence while maintaining stability.


<details>
  <summary>Details</summary>
Motivation: Target networks in RL provide stability but create a fundamental tradeoff: slower updates improve stability but reduce recency of learning signals, slowing convergence. The authors aim to mitigate adverse effects of stale target estimates while retaining stabilization benefits.

Method: Proposes TARL framework that emphasizes transitions where target and online network estimates are highly aligned. By focusing updates on well-aligned targets, it reduces negative impact of stale target estimates while preserving target network stability benefits.

Result: Theoretical analysis shows target alignment correction accelerates convergence. Empirical results demonstrate consistent improvements over standard RL algorithms across various benchmark environments.

Conclusion: TARL effectively addresses the stability-recency tradeoff in RL with target networks, providing a principled approach to accelerate learning while maintaining training stability through selective focus on well-aligned transitions.
...
### [202] [Variational Graph Neural Networks for Uncertainty Quantification in Inverse Problems](https://arxiv.org/abs/2603.29515)
*David Gonzalez,Alba Muixi,Beatriz Moya,Elias Cueto*

Main category: cs.LG

TL;DR: A variational graph neural network (VGNN) architecture is proposed for uncertainty quantification in computational mechanics, specifically for inverse problems in solid mechanics, providing confidence intervals for predictions at lower computational cost than full Bayesian networks.


<details>
  <summary>Details</summary>
Motivation: Deep learning in computational mechanics lacks reliable uncertainty quantification, which is critical for safety-sensitive applications like Digital Twins. Traditional deterministic methods and classic neural networks don't provide confidence measures, especially for inverse problems with non-unique solutions or noisy data.

Method: A variational graph neural network (VGNN) architecture that integrates variational layers only in the decoder to model weight probability distributions. This strategic placement allows estimation of both cognitive (model) uncertainty and statistical (data) uncertainty at lower computational cost than full Bayesian networks.

Result: The model successfully identifies elastic modulus with nonlinear distribution in 2D elastic problems and locates/quantifies loads applied to 3D hyperelastic beams using only displacement field data. It recovers physical parameters with high precision and provides physically consistent confidence intervals.

Conclusion: The proposed VGNN architecture provides an efficient solution for uncertainty quantification in computational mechanics inverse problems, offering reliable confidence intervals for predictions while maintaining computational efficiency compared to full Bayesian approaches.
...
### [203] [Capturing Multivariate Dependencies of EV Charging Events: From Parametric Copulas to Neural Density Estimation](https://arxiv.org/abs/2603.29554)
*Martin Výboh,Gabriela Grmanová*

Main category: cs.LG

TL;DR: First application of Vine copulas and CODINE framework for EV charging modeling, outperforming traditional methods by better capturing complex dependencies between charging variables.
...
Conclusion: These methods provide a robust framework for synthetic charging event generation in varied infrastructure contexts by explicitly focusing on modeling the joint dependence structure of EV charging variables.
...
### [204] [Total Variation Guarantees for Sampling with Stochastic Localization](https://arxiv.org/abs/2603.29555)
*Jakob Kellermann*

Main category: cs.LG

TL;DR: First rigorous convergence analysis for SLIPS diffusion sampling algorithm, showing linear dimension scaling for ε-accuracy in total variation distance.


<details>
  <summary>Details</summary>
Motivation: SLIPS (based on Stochastic Localization) shows strong empirical performance but lacks theoretical convergence guarantees; this work aims to provide rigorous analysis.

Method: Leverages techniques from score-based generative models theory to analyze SLIPS algorithm convergence in total variation distance under minimal assumptions.

Result: Establishes first convergence guarantee for SLIPS: number of steps scales linearly with dimension (up to logarithmic factors) to achieve ε-accuracy.

Conclusion: Provides theoretical foundation for SLIPS algorithm and insights into optimal discretization choices observed empirically.
...
### [205] [The Geometry of Polynomial Group Convolutional Neural Networks](https://arxiv.org/abs/2603.29566)
*Yacoub Hendi,Daniel Persson,Magdalena Larfors*

Main category: cs.LG

TL;DR: PGCNNs for finite groups analyzed via graded group algebras, revealing two parametrizations (Hadamard/Kronecker) with dimension depending only on layers and group size.


<details>
  <summary>Details</summary>
Motivation: To develop a rigorous mathematical framework for polynomial group convolutional neural networks (PGCNNs) for arbitrary finite groups, enabling systematic analysis of their architecture and parameter spaces.

Method: Introduce a new mathematical framework using graded group algebras to analyze PGCNNs. This yields two natural parametrizations based on Hadamard and Kronecker products, related by a linear map. Compute dimensions of neuromanifolds and describe fibers of parametrizations.

Result: The dimension of the neuromanifold depends only on the number of layers and group size. The general fiber of the Kronecker parametrization is described up to regular group action and rescaling. A conjecture is made for the Hadamard parametrization, supported by computations for small groups and shallow networks.

Conclusion: The graded group algebra framework provides a powerful mathematical foundation for analyzing PGCNNs, revealing fundamental properties of their parameter spaces and establishing connections between different architectural parametrizations.
...
### [206] [Disentangled Graph Prompting for Out-Of-Distribution Detection](https://arxiv.org/abs/2603.29644)
*Cheng Yang,Yu Hao,Qi Zhang,Chuan Shi*

Main category: cs.LG

TL;DR: DGP (Disentangled Graph Prompting) is a novel graph OOD detection method that uses pre-trained GNN encoders with prompt generators to capture fine-grained in-distribution patterns without OOD supervision during training.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks face safety risks when test data differs from training distributions. Existing graph OOD detection methods suffer from sub-optimal performance due to the absence of explicit OOD supervision signals during training, as OOD data is unavailable.

Method: Follows pre-training+prompting paradigm with pre-trained GNN encoders. Uses two prompt generators: one for class-specific and one for class-agnostic prompt graphs by modifying edge weights. Includes several effective losses to train prompt generators and prevent trivial solutions.

Result: Extensive experiments on ten datasets show DGP achieves 3.63% relative AUC improvement over the best graph OOD detection baseline. Ablation studies and hyper-parameter experiments demonstrate effectiveness.

Conclusion: DGP effectively addresses the OOD detection problem in graphs by leveraging pre-trained encoders with disentangled prompting, outperforming existing methods without requiring OOD data during training.
...
### [207] [Concept frustration: Aligning human concepts and machine representations](https://arxiv.org/abs/2603.29654)
*Enrico Parisini,Christopher J. Soelistyo,Ahab Isaac,Alessandro Barp,Christopher R. S. Banerji*

Main category: cs.LG

TL;DR: The paper introduces "concept frustration" - a geometric framework for detecting contradictions when unobserved concepts create inconsistent relationships between known concepts, and develops methods to align human concepts with foundation model representations.


<details>
  <summary>Details</summary>
Motivation: The central challenge is aligning human-interpretable concepts with internal representations learned by modern ML systems, particularly for interpretable AI in high-risk applications where understanding model reasoning is crucial.
...
Result: Concept frustration is detectable in foundation model representations using task-aligned geometry (where Euclidean comparisons fail). Experiments on synthetic data and real language/vision tasks show frustration can be detected, and incorporating frustrating concepts reorganizes geometry to better align human and machine reasoning.
...
### [208] [A Comprehensive Information-Decomposition Analysis of Large Vision-Language Models](https://arxiv.org/abs/2603.29676)
*Lixin Xiu,Xufang Luo,Hideki Nakayama*

Main category: cs.LG

TL;DR: A framework using partial information decomposition (PID) to quantitatively measure the information spectrum of large vision-language models, revealing task regimes and model strategies through cross-model, cross-task, and training dynamics analysis.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models achieve impressive performance, but their internal decision-making processes remain opaque, making it difficult to determine if success stems from true multimodal fusion or reliance on unimodal priors. There is an attribution gap in understanding how these models process multimodal information.

Method: Introduces a novel framework using partial information decomposition (PID) to quantitatively measure the "information spectrum" of LVLMs, decomposing decision-relevant information into redundant, unique, and synergistic components. Adapts a scalable estimator to modern LVLM outputs and creates a model-agnostic pipeline that profiles 26 LVLMs on four datasets across three dimensions: breadth (cross-model & cross-task), depth (layer-wise information dynamics), and time (learning dynamics across training).

Result: Analysis reveals: (i) two task regimes (synergy-driven vs. knowledge-driven), (ii) two stable, contrasting family-level strategies (fusion-centric vs. language-centric), (iii) a consistent three-phase pattern in layer-wise processing, and (iv) visual instruction tuning as the key stage where fusion is learned.

Conclusion: The framework provides a quantitative lens beyond accuracy-only evaluation and offers insights for analyzing and designing the next generation of LVLMs. The approach enables systematic understanding of multimodal fusion mechanisms in large vision-language models.
...
### [209] [Mind the Gap: A Framework for Assessing Pitfalls in Multimodal Active Learning](https://arxiv.org/abs/2603.29677)
*Dustin Eisenhardt,Yunhee Jeong,Florian Buettner*

Main category: cs.LG

TL;DR: Multimodal active learning faces unique challenges not present in unimodal settings, including missing modalities, varying modality difficulty, and different interaction structures. A new benchmarking framework reveals that models develop imbalanced representations favoring one modality, and existing query methods fail to address this issue.


<details>
  <summary>Details</summary>
Motivation: Active learning in multimodal settings presents distinct challenges absent in unimodal cases: missing modalities, differences in modality difficulty, and varying interaction structures. While unimodal active learning is well-characterized, multimodal active learning behavior remains poorly understood, necessitating systematic evaluation.

Method: Introduces a new framework for benchmarking multimodal active learning using synthetic datasets to isolate specific pitfalls without confounding noise. Compares unimodal and multimodal query strategies and validates findings on two real-world datasets.
...
Conclusion: Current active learning methods have limitations in multimodal settings, highlighting the need for modality-aware query strategies that explicitly address the unique challenges of multimodal learning, including modality imbalance and interaction structures.
...
### [210] [Nonnegative Matrix Factorization in the Component-Wise L1 Norm for Sparse Data](https://arxiv.org/abs/2603.29715)
*Giovanni Seraghiti,Kévin Dubrulle,Arnaud Vandaele,Nicolas Gillis*

Main category: cs.LG

TL;DR: L1-NMF is NP-hard even for r=1, enforces sparsity for interpretability, but may degrade with false zeros. Proposed weighted L1-NMF (wL1-NMF) controls sparsity via penalization, and sparse coordinate descent (sCD) scales with nonzero entries for efficient large-scale sparse data processing.


<details>
  <summary>Details</summary>
Motivation: Standard NMF using least squares is not robust to heavy-tailed noise, outliers, or sparse data with false zeros. L1-NMF addresses these issues but is NP-hard and may produce overly sparse solutions that degrade performance when data contains false zeros.

Method: 1) Prove NP-hardness of L1-NMF even for r=1. 2) Show L1-NMF enforces sparsity in factors for sparse input. 3) Propose weighted L1-NMF (wL1-NMF) with penalization parameter controlling sparsity of factorization entries associated with zeros in data. 4) Develop sparse coordinate descent (sCD) algorithm using weighted median to solve subproblems, with complexity scaling with number of nonzero entries.

Result: L1-NMF is NP-hard (contrasting with standard NMF). L1-NMF strongly enforces sparsity favoring interpretability. wL1-NMF provides better control over sparsity. sCD algorithm efficiently handles large-scale sparse data with complexity proportional to nonzero entries. Extensive experiments on synthetic and real-world data demonstrate effectiveness.

Conclusion: The paper presents theoretical analysis of L1-NMF hardness, proposes wL1-NMF model for better sparsity control in presence of false zeros, and develops efficient sCD algorithm for large-scale sparse data, providing robust NMF alternative for heavy-tailed noise and outlier scenarios.
...
TL;DR: One-for-All introduces Gaussian Rank-Stabilized Low-Rank Adapters (rsLoRA) for parameter-efficient fine-tuning of frozen LLMs for multivariate time-series analysis, achieving state-of-the-art efficiency-accuracy trade-offs with significantly reduced computational and memory demands.


<details>
  <summary>Details</summary>
Motivation: Pre-trained LLMs face prohibitive computational and memory demands when adapted for multivariate time-series analysis, hindering their deployment in practical applications like healthcare, finance, and environmental monitoring.

Method: Introduces Gaussian Rank-Stabilized Low-Rank Adapters (rsLoRA) with mathematically grounded rank-stabilization mechanism for provable gradient stability at low ranks. The framework injects trainable rank decomposition matrices (rank 16) into positional embeddings and output layers while keeping self-attention weights fixed.

Result: Achieves 5.5× higher parameter efficiency (MSE=5.50) than TimesNet and 21× better than GPT4TS while matching forecasting accuracy (MSE=0.33). Reduces trainable parameters by 6.8× (vs. TimesNet), 21× (vs. GPT4TS), and 11.8× (vs. TIME-LLM) with 168-1,776× smaller memory footprint (2.2MiB vs. 340MiB-4.18GiB).

Conclusion: One-for-All enables deployment of LLMs for time-series analysis on edge devices without compromising performance, validated through consistent performance across diverse horizons (96-720 steps) and datasets with 98.3% fewer parameters than conventional transformers.
...
TL;DR: DUME (Dynamic Upcycling MoE) reuses pre-trained dense domain experts to create a unified Mixture of Experts model without additional training, achieving strong multi-domain performance while preserving original expert capabilities.


<details>
  <summary>Details</summary>
Motivation: LLMs are expensive to train and lack domain expertise; fine-tuning causes overspecialization; multitask training suffers from interference and catastrophic forgetting; existing MoE approaches still require multitask fine-tuning.

Method: DUME reuses dense experts trained on different domains to construct a unified MoE model using ridge regression's closed-form solution, enabling dynamic expert addition without optimization while preserving original performance.

Result: DUME outperforms baselines in causal language modeling and reasoning, retaining 97.6% of dense expert performance in language modeling and achieving 102.1% in reasoning; can be further fine-tuned for improvement.

Conclusion: DUME provides a cost-efficient, scalable solution for building multi-domain expert models by reusing existing dense experts without additional training, addressing overspecialization and multitask interference issues.
...
TL;DR: Big2Small: A unified mathematical framework for model compression using measure theory, translating Implicit Neural Representations from data domain to network parameters for data-free compression.


<details>
  <summary>Details</summary>
Motivation: To elevate model compression from fragmented heuristics to a principled discipline by establishing a unifying mathematical framework grounded in measure theory, addressing the critical need for compression as foundational models grow larger.

Method: Constructs a measure theory-based mathematical framework showing equivalence between compression techniques and regularized neural networks. Proposes Big2Small framework that trains compact Implicit Neural Representations (INRs) to encode weights of larger models, with Outlier-Aware Preprocessing for extreme values and Frequency-Aware Loss for high-frequency detail preservation.

Result: Experiments on image classification and segmentation demonstrate competitive accuracy and compression ratios compared to state-of-the-art baselines, validating the effectiveness of the data-free compression approach.

Conclusion: Big2Small provides a principled, mathematically-grounded approach to model compression that unifies existing techniques and achieves practical compression performance through INR-based weight encoding and reconstruction.
...
TL;DR: Multimodal ML framework predicts metastasis risk 1 month before diagnosis using 6 months of EHR data from 4 cancer cohorts, with intermediate fusion achieving best performance across most cancer types.


<details>
  <summary>Details</summary>
Motivation: To develop a holistic patient status assessment by integrating structured and unstructured EHR data for early metastasis risk prediction, addressing the need for comprehensive multimodal approaches in clinical decision support.

Method: Proposed framework uses 6 months of clinical history from EHR data (demographics, comorbidities, lab results, medications, clinical text) across 4 cancer cohorts. Compared traditional vs deep learning classifiers across single modalities and multimodal combinations using various fusion strategies. Employed TRIPOD 2a design with 80-20 development-validation split and multimodal SHAP for interpretability analysis.

Result: Intermediate fusion achieved highest F1 scores: breast (0.845), colon (0.786), prostate (0.845). For lung cancer, text-only model achieved highest F1 (0.829) vs intermediate fusion (0.819). Deep learning consistently outperformed traditional models. Colon cancer (smallest cohort) had lowest performance. SHAP analysis showed modality importance varied across cancer types.

Conclusion: Multimodal ML effectively predicts metastasis risk with intermediate fusion generally performing best, though strategy choice should align with data characteristics and organizational needs. Deep learning outperforms traditional methods, and sufficient training data is crucial for performance.
...
TL;DR: The paper introduces the "2-datapoint reduced density matrix" (2RDM) as a unified observable for detecting and analyzing phase transitions during neural network training, providing early warning signals and interpretable insights into emergent capabilities.


<details>
  <summary>Details</summary>
Motivation: To develop a computationally efficient method for predicting and understanding emergent capabilities in AI models during training, inspired by quantum chemistry techniques for studying reactions.

Method: Proposes the 2-datapoint reduced density matrix (2RDM) and tracks its eigenvalue statistics over sliding windows to derive two complementary signals: spectral heat capacity (for early warning of second-order phase transitions via critical slowing down) and participation ratio (for revealing dimensionality of underlying reorganization).

Result: Validates the approach across four distinct settings: deep linear networks, induction head formation, grokking, and emergent misalignment, demonstrating that top eigenvectors of 2RDM are directly interpretable for studying transition nature.

Conclusion: The 2RDM provides a unified, computationally efficient observable for detecting and analyzing phase transitions during training, offering both early warning signals and interpretable insights into emergent capabilities, with promising directions for future work.
...
### [216] [Tracking Equivalent Mechanistic Interpretations Across Neural Networks](https://arxiv.org/abs/2603.30002)
*Alan Sun,Mariya Toneva*

Main category: cs.LG

TL;DR: The paper addresses scalability and generalization challenges in mechanistic interpretability by defining interpretive equivalence - determining if two models share a common interpretation without requiring explicit interpretation descriptions.


<details>
  <summary>Details</summary>
Motivation: Mechanistic interpretability (MI) faces scalability and generalization challenges due to: 1) lack of precise definition of valid interpretations, and 2) ad hoc interpretation generation processes. The paper aims to address these fundamental issues.

Method: Proposes interpretive equivalence framework where two interpretations are equivalent if all their possible implementations are equivalent. Develops algorithm to estimate interpretive equivalence based on representation similarity, with case studies on Transformer models.

Result: Introduces necessary and sufficient conditions for interpretive equivalence based on representation similarity. Provides theoretical guarantees relating algorithmic interpretations, circuits, and representations. Framework enables more rigorous MI evaluation.

Conclusion: The interpretive equivalence framework lays foundation for developing rigorous MI evaluation methods and automated, generalizable interpretation discovery methods, addressing key scalability challenges in mechanistic interpretability.
...
### [217] [AMShortcut: An Inference- and Training-Efficient Inverse Design Model for Amorphous Materials](https://arxiv.org/abs/2603.29812)
...
<details>
  <summary>Details</summary>
Motivation: Amorphous materials lack long-range order but have complex short/medium-range structures requiring large simulation cells. Inverse design using generative models is promising for applications like energy storage and thermal management, but existing methods suffer from inefficient inference requiring many sampling steps and inflexible property conditioning requiring separate models for each property combination.

Method: AMShortcut is a probabilistic generative model designed for inference and training efficiency. It enables accurate inference of diverse amorphous structures with only a few sampling steps (improving inference efficiency) and can be trained once with all relevant properties while performing inference conditioned on arbitrary property combinations (eliminating need for separate models).

Result: Experiments on three amorphous materials datasets with diverse structures and properties demonstrate that AMShortcut achieves its design goals of efficient inference and flexible property conditioning.

Conclusion: AMShortcut provides an efficient probabilistic generative modeling approach for inverse design of amorphous materials, addressing key limitations of existing methods in inference efficiency and property conditioning flexibility.
...
### [218] [Reward-Based Online LLM Routing via NeuralUCB](https://arxiv.org/abs/2603.30035)
*Ming-Hua Tsai,Phat Tran*

Main category: cs.LG

TL;DR: NeuralUCB-based routing for cost-aware LLM selection outperforms random/min-cost baselines, achieving lower inference cost while maintaining competitive quality compared to max-quality reference.


<details>
  <summary>Details</summary>
Motivation: Existing LLM routing approaches have tradeoffs between efficiency and adaptivity - supervised methods vs partial-feedback methods. Need for cost-aware routing that balances quality and inference cost.

Method: Implemented NeuralUCB-based routing policy and evaluated on RouterBench under simulated online setting.
...
Conclusion: NeuralUCB is a promising approach for cost-aware LLM routing, though challenges remain in action discrimination and exploration.
...
### [219] [Loss Gap Parity for Fairness in Heterogeneous Federated Learning](https://arxiv.org/abs/2603.29818)
*Brahim Erraji,Michaël Perrot,Aurélien Bellet*

Main category: cs.LG

TL;DR: EAGLE is a federated learning algorithm that ensures fairness by minimizing disparities in loss gaps across clients, particularly effective in heterogeneous settings where local optimal models are misaligned.


<details>
  <summary>Details</summary>
Motivation: Clients in federated learning are self-interested and expect the global model to perform well on their own data, motivating the need for an objective that ensures all clients achieve similar loss gaps (difference between global model performance and their best local model performance).

Method: EAGLE explicitly regularizes the global model to minimize disparities in loss gaps across clients, targeting fairness in relative improvements rather than loss parity, and prioritizes clients furthest from their local optimal loss.

Result: Theoretical convergence guarantees are provided under non-convex loss functions, with characterization of performance relative to standard federated learning using a novel heterogeneity measure. Empirically, EAGLE reduces disparity in loss gaps while maintaining competitive utility in both convex and non-convex cases.

Conclusion: EAGLE effectively addresses fairness concerns in federated learning by ensuring equitable relative improvements across clients, particularly valuable in heterogeneous settings where traditional loss parity methods may degrade performance for many clients.
...
### [220] [Curvature-Guided LoRA: Steering in the pretrained NTK subspace](https://arxiv.org/abs/2603.29824)
*Frédéric Zheng,Alexandre Proutière*

Main category: cs.LG

TL;DR: CG-LoRA improves PEFT performance by aligning predictions with full fine-tuning using curvature-aware second-order optimization, achieving better results than existing LoRA variants.


<details>
  <summary>Details</summary>
Motivation: Current parameter-efficient fine-tuning (PEFT) methods like LoRA often underperform compared to full fine-tuning. Existing approaches focus on aligning parameter updates rather than directly controlling model predictions, leading to suboptimal performance.

Method: Introduces prediction alignment problem to match PEFT predictor outputs with full fine-tuning. Derives curvature-aware second-order formulation where optimal low-rank updates correspond to Newton-like, curvature-whitened gradient. Proposes CG-LoRA which selects and scales adaptation directions using local curvature information without explicit second-order matrix construction.
...
Conclusion: Directly aligning predictions rather than parameter updates leads to more effective PEFT. The curvature-guided approach provides computationally efficient second-order optimization that bridges the performance gap between PEFT and full fine-tuning.
...
### [221] [DiSGMM: A Method for Time-varying Microscopic Weight Completion on Road Networks](https://arxiv.org/abs/2603.29837)
*Yan Lin,Jilin Hu,Shengnan Guo,Christian S. Jensen,Youfang Lin,Huaiyu Wan*

Main category: cs.LG

TL;DR: DiSGMM addresses time-varying microscopic road-network weight completion using sparsity-aware embeddings and spatiotemporal modeling to estimate Gaussian mixture distributions for missing segment weights.


<details>
  <summary>Details</summary>
Motivation: Microscopic road-network weights (e.g., travel speeds from individual vehicles) are crucial for traffic microsimulation and reliable vehicle routing, but suffer from two-layer sparsity: missing weights at both network level (many segments uncovered) and segment level (insufficient samples for distribution estimation), requiring flexible distribution representations that can handle complex conditions like heavy tails and multi-modality.

Method: DiSGMM combines sparsity-aware embeddings with spatiotemporal modeling to leverage sparse known weights, learned segment properties, and long-range correlations. It represents microscopic weight distributions as learnable Gaussian mixture models (GMMs), providing closed-form distributions capable of capturing complex traffic conditions flexibly.

Result: Experiments on two real-world datasets demonstrate that DiSGMM outperforms state-of-the-art methods for time-varying microscopic weight completion.

Conclusion: DiSGMM effectively addresses the dual challenges of sparsity in microscopic road-network weight completion by providing a flexible, closed-form distribution representation through Gaussian mixture models, enabling accurate traffic condition estimation for applications like traffic microsimulation and reliable routing.
...
### [222] [Task Scarcity and Label Leakage in Relational Transfer Learning](https://arxiv.org/abs/2603.29914)
*Francisco Galuppo Azevedo,Clarissa Lima Loures,Denis Oliveira Correa*

Main category: cs.LG

TL;DR: The paper addresses label leakage in relational foundation models caused by task scarcity, proposing gradient projection to suppress label-predictive directions and improve within-dataset transfer performance.


<details>
  <summary>Details</summary>
Motivation: Relational foundation models suffer from label leakage where learned representations encode task-specific shortcuts due to limited supervision (few prediction targets per database), degrading transfer performance even within the same schema.

Method: Uses K-Space architecture combining frozen pretrained tabular encoders with lightweight message-passing core, and introduces gradient projection method to remove label-predictive directions from representation updates to suppress leakage.

Result: On RelBench, gradient projection improves within-dataset transfer by +0.145 AUROC on average, often recovering near single-task performance, demonstrating effectiveness of leakage suppression.

Conclusion: Limited task diversity, not just limited data, constrains relational foundation models, and gradient projection effectively addresses label leakage to improve transfer learning performance.
...
### [223] [Real-Time Explanations for Tabular Foundation Models](https://arxiv.org/abs/2603.29946)
*Luan Borges Teodoro Reis Sena,Francisco Galuppo Azevedo*

Main category: cs.LG

TL;DR: ShapPFN is a tabular foundation model that integrates Shapley value regression directly into its architecture, enabling simultaneous predictions and high-fidelity explanations in a single forward pass, achieving 1000× speedup over KernelSHAP.


<details>
  <summary>Details</summary>
Motivation: Interpretability is crucial for scientific machine learning to understand why models make predictions for hypothesis generation and validation. While tabular foundation models show strong performance, existing explanation methods like SHAP are computationally expensive, limiting interactive exploration.

Method: ShapPFN integrates Shapley value regression directly into its foundation model architecture, producing both predictions and explanations in a single forward pass. This eliminates the need for separate post-hoc explanation methods.
...
Conclusion: ShapPFN enables efficient, high-quality interpretability for tabular foundation models by integrating explanation generation directly into the model architecture, overcoming computational bottlenecks of traditional SHAP methods.
...
### [224] [Meteorology-Driven GPT4AP: A Multi-Task Forecasting LLM for Atmospheric Air Pollution in Data-Scarce Settings](https://arxiv.org/abs/2603.29974)
*Prasanjit Dey,Soumyabrata Dev,Bianca Schoen-Phelan*

Main category: cs.LG

TL;DR: GPT4AP is a parameter-efficient air pollution forecasting framework using a frozen GPT-2 backbone with lightweight rsLoRA adaptations, achieving strong performance in few-shot, zero-shot, and long-term forecasting scenarios.


<details>
  <summary>Details</summary>
Motivation: Data-driven air pollution forecasting models often suffer from limited generalization in regions with sparse observations, creating a need for more data-efficient approaches that can perform well with limited supervision and handle domain shifts.

Method: The framework uses a pre-trained GPT-2 backbone with Gaussian rank-stabilized low-rank adaptation (rsLoRA). It freezes self-attention and feed-forward layers while adapting lightweight positional and output modules, substantially reducing trainable parameters. The model is evaluated in multi-task forecasting settings.

Result: In few-shot regime (10% training data): MSE/MAE of 0.686/0.442, outperforming DLinear (0.728/0.530) and ETSformer (0.734/0.505). In zero-shot cross-station transfer: MSE/MAE of 0.529/0.403, showing improved generalization. In long-term forecasting with full data: competitive MAE of 0.429, though specialized time-series models show slightly lower errors.

Conclusion: GPT4AP provides a data-efficient forecasting approach that performs robustly under limited supervision and domain shift while maintaining competitive accuracy in data-rich settings, offering a promising solution for air pollution forecasting in observation-sparse regions.
...
TL;DR: Multimodal cancer survival models show better performance comes from complementary signal aggregation rather than synergistic cross-modal interactions, with an inverse relationship between predictive accuracy and measured interaction strength.


<details>
  <summary>Details</summary>
Motivation: To test the common assumption that multimodal deep learning for cancer prognosis benefits from synergistic cross-modal interactions, particularly in survival prediction settings where this hasn't been directly evaluated.

Method: Adapted InterSHAP (Shapley interaction index-based metric) from classification to Cox proportional hazards models to quantify cross-modal interactions. Applied to glioma survival prediction using TCGA-GBM and TCGA-LGG data (n=575), evaluating four fusion architectures combining whole-slide image (WSI) and RNA-seq features.

Result: Found inverse relationship: architectures with superior discrimination (C-index 0.64→0.82) exhibited equivalent or lower cross-modal interaction (4.8%→3.0%). Variance decomposition showed stable additive contributions (WSI≈40%, RNA≈55%, Interaction≈4%), indicating performance gains arise from complementary signal aggregation rather than learned synergy.

Conclusion: Provides practical model auditing tool for comparing fusion strategies, reframes role of architectural complexity in multimodal fusion, and has implications for privacy-preserving federated deployment by showing simpler aggregation approaches may be sufficient.
...
### [226] [Aligning Validation with Deployment: Target-Weighted Cross-Validation for Spatial Prediction](https://arxiv.org/abs/2603.29981)
*Alexander Brenning,Thomas Suesse*

Main category: cs.LG

TL;DR: TWCV addresses CV bias in spatial prediction by weighting validation losses to match deployment task distributions, handling covariate and task-difficulty shifts through calibration weighting and buffered resampling.


<details>
  <summary>Details</summary>
Motivation: Standard cross-validation assumes validation and deployment tasks come from the same distribution, which is often violated in spatial prediction and structured data settings, leading to biased risk estimates.

Method: Target-Weighted CV (TWCV) uses calibration weighting to assign weights to validation losses so the weighted empirical distribution of validation tasks matches the target domain distribution. Combined with spatially buffered resampling to ensure adequate coverage of deployment distribution support.

Result: In simulations, conventional and spatial estimators showed substantial bias depending on sampling, while buffered TWCV remained approximately unbiased. Environmental pollution mapping case study confirmed TWCV better reflects prediction task over target domain.

Conclusion: Task distribution mismatch is a primary source of CV bias in spatial prediction, and calibration weighting combined with suitable validation task generation provides viable approach to estimating predictive risk under dataset shift.
...
### [227] [Refined Detection for Gumbel Watermarking](https://arxiv.org/abs/2603.30017)
*Tor Lattimore*

Main category: cs.LG

TL;DR: Proposes a near-optimal detection mechanism for Gumbel watermarking that works without access to the model's internal distributions


<details>
  <summary>Details</summary>
Motivation: To develop a simple, model-agnostic detection mechanism for Gumbel watermarking that achieves near-optimal performance without requiring knowledge of the underlying model's token distributions

Method: Proposes a new detection mechanism for the Gumbel watermarking scheme that operates under the assumption of i.i.d. next-token sampling, requiring only the watermarked text and no access to the model's internal probability distributions

Result: The proposed detection mechanism is proven to be near-optimal in a problem-dependent sense among all model-agnostic watermarking schemes

Conclusion: A simple, effective detection mechanism for Gumbel watermarking achieves near-optimal performance while remaining model-agnostic, making it practical for real-world applications
...
TL;DR: Tucker Attention is a parameter-efficient factorization method for self-attention that generalizes existing approaches (MHA, GQA, MLA) using Tucker decomposition, achieving comparable performance with an order of magnitude fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing memory-reduction methods for self-attention (GQA, MLA) use unconventional low-rank factorizations that lack clear interpretation from classical approximation theory. The paper aims to provide a generalized framework to understand what these methods actually approximate and to develop a more parameter-efficient factorization strategy.

Method: Proposes Tucker Attention, which applies Tucker decomposition to the weight tensors in self-attention layers. This provides a unified factorization framework that encompasses MHA, GQA, and MLA as special cases. The method is compatible with existing optimizations like flash-attention and RoPE.

Result: Tucker Attention requires an order of magnitude fewer parameters than GQA and MLA while achieving comparable validation metrics in both LLM and ViT test cases. The framework also provides insights into the actual ranks achieved by existing methods and enables simplifications for MLA.

Conclusion: Tucker Attention offers a principled, parameter-efficient generalization of self-attention factorization methods that unifies existing approaches and provides theoretical insights into their low-rank behavior while maintaining compatibility with standard optimizations.
...
### [229] [Aligned, Orthogonal or In-conflict: When can we safely optimize Chain-of-Thought?](https://arxiv.org/abs/2603.30036)
*Max Kaufmann,David Lindner,Roland S. Zimmermann,and Rohin Shah*

Main category: cs.LG

TL;DR: This paper proposes a framework to predict how Chain-of-Thought (CoT) monitorability is affected by training, showing that conflicting reward terms reduce monitorability while making optimization difficult.


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought monitoring is promising for AI oversight, but training can affect CoT monitorability when models learn to hide reasoning features. There's a need to understand when and why this occurs.

Method: Models LLM post-training as RL environment with reward decomposing into output-dependent and CoT-dependent terms. Classifies these terms as aligned, orthogonal, or in-conflict. Validates by classifying RL environments, training LLMs, and evaluating CoT monitorability effects.

Result: Training with "in-conflict" reward terms reduces CoT monitorability, and optimizing in-conflict reward terms is difficult.

Conclusion: The proposed framework successfully predicts how different reward term relationships affect CoT monitorability during training, with important implications for designing effective AI oversight systems.
...
### [230] [UltRAG: a Universal Simple Scalable Recipe for Knowledge Graph RAG](https://arxiv.org/abs/2603.28773)
*Dobrik Georgiev,Kheeran Naidu,Alberto Cattaneo,Federico Monti,Carlo Luschi,Daniel Justus*

Main category: cs.IR

TL;DR: ULTRAG is a novel framework for retrieving information from Knowledge Graphs that uses off-the-shelf neural query executing modules with LLMs, achieving state-of-the-art KGQA performance without retraining.


<details>
  <summary>Details</summary>
Motivation: LLMs often generate confident but factually incorrect content (hallucination). While RAG reduces errors by retrieving from knowledge corpora, adapting it to Knowledge Graphs is challenging, especially for multi-hop reasoning queries.

Method: ULTRAG shifts from classical RAG by endowing LLMs with off-the-shelf neural query executing modules. It enables LLMs to interface with KGs without retraining either the LLM or the executor.

Result: ULTRAG achieves better performance than state-of-the-art KG-RAG solutions and enables language models to interface with Wikidata-scale graphs (116M entities, 1.6B relations) at comparable or lower costs.

Conclusion: The framework demonstrates that readily available language models can achieve state-of-the-art results on KGQA tasks without retraining, providing an effective solution for knowledge graph retrieval and multi-hop reasoning.
...
### [231] [Calibrated Fusion for Heterogeneous Graph-Vector Retrieval in Multi-Hop QA](https://arxiv.org/abs/2603.28886)
*Andre Bacellar*

Main category: cs.IR

TL;DR: PhaseGraph addresses heterogeneous retrieval fusion in multi-hop QA by calibrating dense vector and graph-based scores (PPR) using percentile-rank normalization before fusion, improving last-hop retrieval performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Graph-augmented retrieval combines dense similarity scores with graph-based relevance signals like Personalized PageRank, but these scores have different distributions and are not directly comparable, creating a score calibration problem for heterogeneous retrieval fusion in multi-hop question answering.

Method: PhaseGraph maps vector and graph scores to a common unit-free scale using percentile-rank normalization (PIT) before fusion, enabling stable combination without discarding magnitude information. The method compares percentile-based calibration against min-max normalization and evaluates different post-calibration operators like Boltzmann weighting.

Result: Calibrated fusion improves held-out last-hop retrieval: LastHop@5 increases from 75.1% to 76.5% on MuSiQue (8W/1L, p=0.039) and from 51.7% to 53.6% on 2WikiMultiHopQA (11W/2L, p=0.023). Percentile-based calibration is more robust than min-max normalization, and Boltzmann weighting performs comparably to linear fusion after calibration.

Conclusion: Score commensuration through percentile-rank normalization is a robust design choice for heterogeneous retrieval fusion, with the exact post-calibration operator mattering less than the calibration itself. The approach effectively addresses distributional differences between dense and graph-based relevance signals.
...
### [232] [Zero-shot Cross-domain Knowledge Distillation: A Case study on YouTube Music](https://arxiv.org/abs/2603.28994)
...
TL;DR: Zero-shot cross-domain knowledge distillation from YouTube to a music app improves multi-task ranking models on low-traffic surfaces without training dedicated teachers.


<details>
  <summary>Details</summary>
Motivation: Knowledge distillation in low-traffic recommender systems is challenging due to limited data restricting teacher model size and high cost of training large dedicated teachers. Cross-domain KD offers a cost-effective alternative but faces technical difficulties from domain differences in features, interfaces, and tasks.

Method: Zero-shot cross-domain knowledge distillation from a 100x larger YouTube video recommendation platform to a music recommendation app, evaluating different KD techniques across two ranking models on the music application.

Result: Offline and live experiments demonstrate that zero-shot cross-domain KD is a practical and effective approach to improve performance of ranking models on low-traffic surfaces.

Conclusion: Zero-shot cross-domain knowledge distillation provides a viable solution for improving recommender systems with limited traffic by leveraging knowledge from data-rich source domains without the cost of training dedicated teacher models.
...
### [233] [Aligning Multimodal Sequential Recommendations via Robust Direct Preference Optimization with Sparse MoE](https://arxiv.org/abs/2603.29259)
*Hejin Huang,Jusheng Zhang,Kaitong Cai,Jian Wang,Rong Pan*

Main category: cs.IR

TL;DR: RoDPO improves DPO for implicit feedback by replacing hard negatives with stochastic sampling from dynamic top-K pool, achieving better ranking performance with minimal inference cost increase.


<details>
  <summary>Details</summary>
Motivation: Existing Direct Preference Optimization (DPO) methods perform poorly under implicit feedback where unobserved items are not reliable negatives, creating issues with false negatives and erroneous gradient suppression.

Method: Proposes RoDPO with two key modifications: 1) replaces deterministic hard negatives with stochastic sampling from dynamic top-K candidate pool, 2) optionally uses sparse Mixture-of-Experts encoder for efficient capacity scaling.

Result: Achieves up to 5.25% NDCG@5 improvement on three Amazon benchmarks with nearly unchanged inference cost, demonstrating consistent ranking performance gains.

Conclusion: Stochastic negative sampling from dynamic candidate pools effectively addresses false negative issues in DPO for implicit feedback, balancing informative hard signals with optimization smoothness.
...
### [234] [Drift-Aware Continual Tokenization for Generative Recommendation](https://arxiv.org/abs/2603.29705)
*Yuebo Feng,Jiahao Liu,Mingzhe Han,Dongsheng Li,Hansu Gu,Peng Zhang,Tun Lu,Ning Gu*

Main category: cs.IR

TL;DR: DACT is a drift-aware continual tokenization framework for generative recommendation that adapts to evolving collaborative signals while minimizing disruption to existing knowledge.


<details>
  <summary>Details</summary>
Motivation: Real-world recommendation environments continuously evolve with new items causing identifier collision/shifts and new interactions inducing collaborative drift in existing items. Fully retraining both tokenizer and generative recommender model is prohibitively expensive, while naive fine-tuning alters token sequences for most existing items, undermining learned token-embedding alignment.

Method: Two-stage framework: (1) tokenizer fine-tuning augmented with a jointly trained Collaborative Drift Identification Module (CDIM) that outputs item-level drift confidence for differentiated optimization of drifting vs. stationary items; (2) hierarchical code reassignment using a relaxed-to-strict strategy to update token sequences while limiting unnecessary changes.

Result: Experiments on three real-world datasets with two representative generative recommender models show DACT consistently achieves better performance than baselines, demonstrating effective adaptation to collaborative evolution with reduced disruption to prior knowledge.

Conclusion: DACT successfully balances plasticity and stability for collaborative tokenizers in evolving recommendation environments, enabling effective adaptation to collaborative evolution while minimizing disruption to existing learned knowledge.
...
### [235] [Cold-Starts in Generative Recommendation: A Reproducibility Study](https://arxiv.org/abs/2603.29845)
*Zhen Zhang,Jujia Zhao,Xinyu Ma,Xin Xin,Maarten de Rijke,Zhaochun Ren*

Main category: cs.IR

TL;DR: Systematic reproducibility study of generative recommendation models under unified cold-start protocols


<details>
  <summary>Details</summary>
Motivation: Cold-start recommendation remains challenging in dynamic platforms, but existing generative recommenders lack systematic evaluation under cold-start conditions with inconsistent design choices

Method: Systematic reproducibility study using unified suite of cold-start protocols to evaluate generative recommendation models

Result: Not specified in abstract (paper presents study methodology rather than results)

Conclusion: Need for standardized evaluation protocols to properly assess generative recommendation models for cold-start scenarios
...
### [236] [UnWeaving the knots of GraphRAG -- turns out VectorRAG is almost enough](https://arxiv.org/abs/2603.29875)
...
TL;DR: UnWeaver is a novel RAG framework that uses entity-based decomposition instead of chunk-based or graph-based approaches to improve multi-hop question answering while reducing complexity and preserving source fidelity.


<details>
  <summary>Details</summary>
Motivation: Traditional chunk-based RAG systems treat document chunks as atomic, isolated units without representing relations between them, making them ineffective for multi-hop questions. Graph-based RAG systems address this but introduce excessive complexity and rely on heuristics.

Method: UnWeaver disentangles document contents into entities using an LLM, where entities can span multiple chunks. During retrieval, entities serve as an intermediate layer to recover original text chunks, preserving source fidelity while creating a more distilled representation.

Result: Entity-based decomposition yields a more distilled representation of original information, reduces noise in indexing and generation processes, and simplifies the GraphRAG concept while maintaining effectiveness for multi-hop reasoning.

Conclusion: UnWeaver provides a simplified yet effective alternative to GraphRAG by using entity-based decomposition to handle multi-hop questions while reducing system complexity and preserving source material fidelity.
...
### [237] [Performance Evaluation of LLMs in Automated RDF Knowledge Graph Generation](https://arxiv.org/abs/2603.29878)
*Ioana Ramona Martin,Tudor Cioara,Ionut Anghel,Gabriel Arcas*

Main category: cs.IR

TL;DR: LLMs can effectively automate RDF knowledge graph generation from complex cloud logs, with Few-Shot learning achieving near-perfect performance (99.35% F1) using Llama, while prompt design significantly impacts extraction quality.


<details>
  <summary>Details</summary>
Motivation: Cloud systems generate heterogeneous log data that contains critical infrastructure, application, and security information. Transforming logs into RDF triples enables integration into knowledge graphs for improved interpretability, root-cause analysis, and cross-service reasoning. While LLMs offer promising automation for RDF generation, their effectiveness on complex cloud logs remains unexplored.

Method: Developed a controlled framework with two pipelines: 1) Extraction pipeline integrating multiple LLMs to identify entities and relationships and generate subject-predicate-object triples, and 2) Validation pipeline with syntactic and semantic metrics to assess accuracy, completeness, and quality. Created a reference Log-to-KG dataset from OpenStack logs using manual annotation and ontology-driven methods due to lack of public ground-truth datasets. Evaluated multiple LLM architectures (Llama, Qwen, NuExtract, Gemma) and prompting strategies (Few-Shot, One-Shot, Zero-Shot, Chain-of-Thought, Tree-of-Thought, Self-Critique, Generate-Multiple).

Result: Few-Shot learning was the most effective strategy, with Llama achieving 99.35% F1 score and 100% valid RDF output. Qwen, NuExtract, and Gemma also performed well under Few-Shot prompting, with Chain-of-Thought approaches maintaining similar accuracy. One-Shot prompting offered a lighter but effective alternative. Zero-Shot and advanced strategies (Tree-of-Thought, Self-Critique, Generate-Multiple) performed substantially worse.

Conclusion: LLMs can effectively automate RDF knowledge graph generation from complex cloud logs. Contextual examples and prompt design are crucial for accurate RDF extraction, with Few-Shot learning being optimal. Model-specific limitations exist across LLM architectures, highlighting the importance of appropriate model selection and prompt engineering for log-to-KG transformation tasks.
...
### [238] [A Hybrid Machine Learning Approach for Graduate Admission Prediction and Combined University-Program Recommendation](https://arxiv.org/abs/2603.29881)
*Melina Heidari Far,Elham Tabrizi*

Main category: cs.IR

TL;DR: Hybrid ML framework combining XGBoost with k-NN refinement achieves 87% accuracy for graduate admission prediction, with recommendation system improving acceptance probability by 70% for rejected applicants.


<details>
  <summary>Details</summary>
Motivation: Increasing competitiveness in graduate admissions necessitates data-driven prediction systems to help applicants make informed decisions and improve their chances of acceptance.

Method: Developed hybrid model combining XGBoost with residual refinement k-nearest neighbors module using enriched dataset of 13,000 GradCafe records (2021-2025) with features from OpenAlex API, QS rankings, and Wikidata SPARQL queries.

Result: Achieved 87% accuracy on test set; recommendation system for rejected applicants improved expected acceptance probability by 70%; university quality metrics identified as strong predictors.

Conclusion: Hybrid ML framework effectively predicts graduate admissions and provides actionable guidance through recommendations, with university quality metrics playing crucial role in competitive applicant pools.
...
### [239] [UniRank: End-to-End Domain-Specific Reranking of Hybrid Text-Image Candidates](https://arxiv.org/abs/2603.29897)
*Yupei Yang,Lin Yang,Wanxi Deng,Lin Qu,Shikui Tu,Lei Xu*

Main category: cs.IR

TL;DR: UniRank is a VLM-based multimodal reranking framework that natively handles hybrid text-image candidates without modality conversion, featuring domain adaptation through instruction tuning and RLHF-based preference alignment.


<details>
  <summary>Details</summary>
Motivation: Multimodal reranking faces challenges with hybrid text-image candidate sets due to modality gaps, computational overhead from treating text as images, and poor performance of general-domain models in specialized scenarios.

Method: UniRank uses VLM-based hybrid scoring without modality conversion, with two-stage domain adaptation: instruction tuning for calibrated cross-modal relevance scoring, and RLHF-based preference alignment using hard-negative-driven pairwise preferences.

Result: UniRank outperforms state-of-the-art baselines in scientific literature retrieval and design patent search, improving Recall@1 by 8.9% and 7.3% respectively.

Conclusion: UniRank effectively addresses multimodal reranking challenges through native hybrid scoring and domain adaptation, demonstrating superior performance in specialized domains.
