...
TL;DR: Multimodality in open LLM families emerges through rare founder events followed by rapid expansion within descendant lineages, showing punctuated adoption dynamics with weak cross-type transfer from text-only models.


<details>
  <summary>Details</summary>
Motivation: To understand how multimodal capabilities emerge and propagate within open LLM families over time, given rapid improvements in LLMs but unclear patterns of multimodal adoption.

Method: Used ModelBiome AI Ecosystem dataset of Hugging Face model metadata (>1.8 million entries) to quantify multimodality over time and along recorded parent-to-child relations, analyzing cross-modal task adoption and lineage-conditioned transition rates.

Result: Multimodality remains rare in major open LLM families through 2023 and most of 2024, then increases sharply in 2024-2025 dominated by image-text vision-language tasks. VLM variants appear months to years after text-only releases. Only 0.218% of fine-tuning edges from text-generation parents yield VLM descendants, while 94.5% of VLM-child edges originate from VLM parents. Most VLM releases appear as new roots (~60%) or are VLM-derived.

Conclusion: Multimodality enters open LLM families through rare founder events and expands rapidly within descendant lineages, producing punctuated adoption dynamics with distinct, transfer-limited scaling behavior for multimodal capabilities.
...
TL;DR: This paper evaluates multimodal LLMs (MLMs) as real-time assistants for technical tasks like furniture assembly, using a manually annotated dataset (M2AD) to assess their reasoning abilities, step progression tracking, and manual reference capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to extend LLM-based assistants beyond text to multimodal contexts for real-time procedural task assistance, particularly in technical domains like furniture assembly, leveraging VR/AR for shared environmental context.

Method: The authors created the Manual to Action Dataset (M2AD) with step-by-step labels and manual references for furniture assembly. They used this dataset to evaluate openly available MLMs on three aspects: (1) reasoning ability to reduce detailed labeling needs, (2) step progression tracking, and (3) correct manual page referencing.

Result: Results showed that while some models understand procedural sequences, their performance is limited by architectural and hardware constraints. The study highlights the need for improved multi-image and interleaved text-image reasoning capabilities in MLMs.

Conclusion: Current openly available MLMs have limited capabilities for real-time technical task assistance, particularly in procedural contexts. The research identifies architectural and hardware constraints as key limitations and emphasizes the need for enhanced multimodal reasoning approaches for effective real-world assistance.
...
TL;DR: VLMs struggle to detect misleading visualizations, especially when deception arises from subtle reasoning errors in captions rather than visual design flaws.


<details>
  <summary>Details</summary>
Motivation: Visualizations can propagate misinformation through deceptive representations, but VLMs' ability to detect misleading visualizations, particularly those involving subtle reasoning errors in captions, remains poorly understood and needs systematic evaluation.

Method: Developed a benchmark combining real-world visualizations with human-authored misleading captions grounded in a fine-grained taxonomy of reasoning errors (Cherry-picking, Causal inference) and visualization design errors (Truncated axis, Dual axis, inappropriate encodings), enabling controlled analysis across error categories and modalities.

Result: VLMs detect visual design errors substantially more reliably than reasoning-based misinformation, and frequently misclassify non-misleading visualizations as deceptive, showing significant limitations in identifying caption-based reasoning errors.

Conclusion: The work bridges the gap between coarse detection of misleading content and attribution of specific reasoning or visualization errors, revealing VLMs' current limitations in detecting subtle reasoning-based deception in visualization-caption pairs.
...
TL;DR: EUPE is a vision encoder that achieves both efficiency and versatility by distilling from multiple domain-expert foundation models through a two-step process: first scaling up to a large proxy teacher, then scaling down to an efficient encoder.


<details>
  <summary>Details</summary>
Motivation: Running AI models on edge devices requires small but powerful vision encoders that can handle multiple tasks simultaneously, but current methods struggle to balance efficiency with versatile representations for diverse downstream tasks.

Method: EUPE uses knowledge distillation from multiple domain-expert foundation vision encoders, but employs a novel two-step approach: first scaling up to create a large proxy teacher that aggregates knowledge from multiple experts, then scaling down from this single teacher to create an efficient encoder.

Result: EUPE achieves on-par or better performance than individual domain experts of the same size across diverse task domains, and outperforms previous agglomerative encoders that directly scale down from multiple teachers.

Conclusion: The two-step scaling approach (up then down) is crucial for creating efficient universal perception encoders, and EUPE demonstrates superior performance while maintaining inference efficiency for edge deployment.
...
TL;DR: Proposes novel evaluation framework for 3D point cloud semantic segmentation that addresses limitations of traditional metrics like mIoU and OA by introducing distance-based metrics for geometric error severity and focused evaluation on hard points.


<details>
  <summary>Details</summary>
Motivation: Traditional semantic segmentation metrics (mIoU, OA) have two key limitations for aerial LiDAR data: 1) they treat all misclassifications equally regardless of spatial context, ignoring geometric severity of errors that impact derived geospatial products; 2) they are dominated by easily classified points, masking meaningful differences between models and under-representing performance in challenging regions.

Method: Two complementary approaches: 1) Distance-based metrics that measure spatial deviation between misclassified points and nearest ground-truth points of predicted class to capture geometric severity of errors; 2) Focused evaluation on a common subset of hard points (points misclassified by at least one model) to reduce bias from easily classified points and better reveal model differences in challenging regions.

Result: Validation on three state-of-the-art deep learning models across three aerial LiDAR datasets shows proposed metrics provide complementary information to traditional measures, revealing spatial error patterns critical for Earth Observation applications but invisible to conventional evaluation approaches.

Conclusion: The framework enables more informed model selection for scenarios where spatial consistency is critical, addressing limitations of traditional metrics for aerial LiDAR semantic segmentation evaluation.
...
TL;DR: OsteoFlow is a flow-based framework that predicts Year-1 post-operative CT scans from Day-5 scans for mandibular reconstruction, using Lyapunov-guided trajectory distillation to maintain anatomical fidelity over long-term bone remodeling.


<details>
  <summary>Details</summary>
Motivation: Standard generative models struggle with trajectory-level consistency and anatomical fidelity over long horizons for predicting bone remodeling after mandibular reconstruction, creating a need for improved prediction methods with clinical benefits.

Method: OsteoFlow uses a flow-based framework with Lyapunov-guided trajectory distillation that distills continuous trajectories over transport time from registration-derived stationary velocity field teachers, combined with resection-aware image loss to enforce geometric correspondence while preserving generative capacity.
...
Conclusion: The approach demonstrates the promise of trajectory distillation for long-term prediction of bone remodeling after mandibular reconstruction, with potential clinical applications.
...
TL;DR: A robust pipeline adapts static 3D reconstruction systems to egocentric videos by suppressing dynamic foreground artifacts and preventing trajectory drift.


<details>
  <summary>Details</summary>
Motivation: Egocentric videos present unique challenges for 3D reconstruction due to rapid camera motion and frequent dynamic interactions, causing existing static reconstruction systems to degrade with trajectory drift and "ghost" geometry from moving hands.

Method: Introduces a mask-aware reconstruction mechanism that explicitly suppresses dynamic foreground in attention layers to prevent hand artifacts, and employs a chunked reconstruction strategy with pose-graph stitching for global consistency and elimination of long-term drift.
...
<details>
  <summary>Details</summary>
Motivation: Autoregressive decoding in OCR introduces sequential latency and amplifies error propagation in long documents; left-to-right causal generation is an artifact of serialization rather than an intrinsic property of the task.

Method: Unified diffusion-based framework with block-wise diffusion decoder and uncertainty-driven curriculum learning for stable training and efficient long-sequence inference under visual conditioning.

Result: Consistently improves robustness while achieving up to 3.2x faster decoding compared to autoregressive baselines; reduced dependence on linguistic priors and stronger visual OCR capability demonstrated on Semantic Shuffle benchmark.

Conclusion: Diffusion-based parallel denoising effectively addresses limitations of autoregressive decoding in document OCR, offering faster inference and improved robustness through visual conditioning and curriculum learning.
...
TL;DR: ColorTrigger enables efficient streaming video understanding by capturing grayscale continuously and only activating color capture when needed, achieving 91.6% of full-color performance with only 8.1% RGB frames.


<details>
  <summary>Details</summary>
Motivation: Always-on high-fidelity RGB video capture is too expensive for resource-constrained edge/wearable AI systems, creating a need for more efficient video sensing paradigms.

Method: Proposes ColorTrigger: an online training-free trigger that uses grayscale-always, color-on-demand approach with windowed grayscale affinity analysis, lightweight quadratic programming for chromatic redundancy detection, credit-budgeted control, and dynamic token routing.
...
Conclusion: The grayscale-always, color-on-demand paradigm enables practical always-on video sensing on resource-constrained devices by exploiting temporal structure and color redundancy in natural videos.
...
TL;DR: VHS is a verifier that operates directly on DiT hidden states instead of decoding to pixel space, reducing verification cost while matching or improving performance over MLLM verifiers.


<details>
  <summary>Details</summary>
Motivation: MLLM verifiers for inference-time scaling require decoding candidates to pixel space and re-encoding them, creating redundant and costly operations despite diffusion models using latent space for efficiency.

Method: Proposes Verifier on Hidden States (VHS) that operates directly on intermediate hidden representations of Diffusion Transformer single-step generators, analyzing generator features without decoding to pixel space.

Result: Reduces joint generation-and-verification time by 63.3%, compute FLOPs by 51%, VRAM usage by 14.5% vs standard MLLM verifier, with +2.7% improvement on GenEval at same inference-time budget.

Conclusion: VHS enables more efficient inference-time scaling by eliminating redundant pixel-space operations while maintaining or improving verification performance.
...
TL;DR: Sketch2CT: A multimodal diffusion framework for generating anatomically consistent 3D medical volumes from 2D sketches and text descriptions.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of generating 3D medical volumes with anatomically consistent structures under multimodal conditions, which remains unresolved despite diffusion models showing promise for medical image generation and data scarcity solutions.

Method: A multimodal diffusion framework that first generates 3D segmentation masks from random noise conditioned on 2D sketches and textual descriptions. Uses two key modules: one refines sketch features with localized textual cues, another integrates global sketch-text representations. Built on a capsule-attention backbone to leverage complementary sketch-text strengths. Synthesized masks then guide a latent diffusion model for 3D CT volume synthesis.

Result: Extensive experiments on public CT datasets demonstrate superior performance in generating multimodal medical volumes. The framework enables controllable, low-cost generation for principled, efficient medical dataset augmentation.

Conclusion: Sketch2CT provides an effective solution for structure-aware 3D medical volume generation from multimodal inputs, addressing anatomical consistency challenges and enabling practical medical dataset augmentation through its controllable pipeline.
...
TL;DR: A flood mapping framework integrates PlanetScope optical imagery with topographic features using ML/DL, showing topographic features provide only marginal improvement over optical-only models for inundation extent detection.


<details>
  <summary>Details</summary>
Motivation: Flood model validation is challenging due to limited observations during extreme events. High-resolution optical imagery offers opportunities but is limited by cloud cover and lack of labeled training data during disasters.

Method: Developed a flood mapping framework integrating PlanetScope optical imagery with topographic features using ML/DL. Used Random Forest on expert-annotated flood masks to generate training labels for U-Net models. Trained two U-Net models with ResNet18 backbone: optical-only (4 bands) and optical+topographic (6 bands with HAND and slope). Evaluated on Hurricane Ida flooding in NYC metropolitan area.

Result: U-Net model with topographic features achieved very similar performance to optical-only configuration (F1=0.92 and IoU=0.85 for both). Topographic features (HAND and slope) provided only marginal value to inundation extent detection.

Conclusion: The framework offers a scalable, label-efficient approach for mapping inundation extent that enables modeling under data-scarce flood scenarios. Topographic features add minimal improvement to optical imagery for flood extent detection.
...
TL;DR: Ego2Web is the first benchmark bridging egocentric video perception and web agent execution, enabling evaluation of multimodal AI agents that must recognize objects in real-world surroundings and complete related online tasks.


<details>
  <summary>Details</summary>
Motivation: Current web-agent benchmarks focus only on web-based interaction and perception, lacking grounding in users' real-world physical surroundings. This prevents evaluation in crucial scenarios where agents need egocentric visual perception (e.g., via AR glasses) to recognize objects in the user's environment and complete related online tasks.

Method: Ego2Web pairs real-world first-person video recordings with web tasks requiring visual understanding, web task planning, and online interaction. Uses automatic data-generation pipeline with human verification/refinement to curate high-quality video-task pairs across diverse web task types (e-commerce, media retrieval, knowledge lookup). Also develops Ego2WebJudge, a novel LLM-as-a-Judge automatic evaluation method.

Result: Ego2WebJudge achieves ~84% agreement with human judgment, substantially higher than existing evaluation methods. Experiments with diverse state-of-the-art agents show weak performance with substantial headroom across all task categories. Ablation study highlights necessity of accurate video understanding and limitations of current agents.

Conclusion: Ego2Web serves as a critical new resource for developing truly capable AI assistants that can seamlessly see, understand, and act across physical and digital worlds, addressing the gap between egocentric perception and web execution.
...
TL;DR: UrbanVGGT: A pipeline for estimating metric sidewalk width from single street-view images using semantic segmentation, 3D reconstruction, and scale calibration, achieving 0.252 m MAE on DC benchmark.


<details>
  <summary>Details</summary>
Motivation: Large-scale sidewalk width data is scarce in most cities, with existing approaches relying on costly field surveys, high-resolution overhead imagery, or simplified geometric assumptions that limit scalability or introduce systematic error.
...
Result: Achieves mean absolute error of 0.252 m on Washington, D.C. ground-truth benchmark, with 95.5% of estimates within 0.50 m of reference width. Generated SV-SideWidth dataset covering 527 OpenStreetMap street segments across three cities.

Conclusion: Street-view imagery can support scalable generation of candidate sidewalk-width attributes, but broader cross-city validation and local ground-truth auditing remain necessary before deployment as authoritative planning data.
...
TL;DR: A resonator network approach for visual scene understanding that simultaneously extracts invariant (object identity) and equivariant (position) information through analysis-by-synthesis, enabling classification without data augmentation and handling multiple objects.


<details>
  <summary>Details</summary>
Motivation: Current neural networks trained for invariance often lose equivariant information like object positions, don't naturally guarantee invariance through supervised learning alone, and generalize poorly to unseen transformations. There's a need for architectures that can capture both invariant and equivariant structure simultaneously.

Method: Uses analysis-by-synthesis with resonator networks. A generative model constructs scenes containing MNIST digits with transformations (color, position). The resonator network inverts this generative model to extract both invariant (shape) and equivariant (position) information. Sparse features learned from training data provide flexibility for unseen shapes. Modular structure separates shape and translation information.

Result: The network can identify objects with arbitrary translations without data augmentation, requires less training data (classifier trained only on centered digits), handles multiple objects through attention-like mechanisms, and can track multiple moving objects with pixel-level precision by extracting position information from translation modules.

Conclusion: Resonator networks provide an effective framework for simultaneous extraction of invariant and equivariant information in visual scenes, overcoming limitations of standard neural networks by using analysis-by-synthesis and modular factorization, enabling robust scene understanding without extensive data augmentation.
...
TL;DR: CanViT is the first task- and policy-agnostic Active-Vision Foundation Model that uses a retinotopic Vision Transformer backbone with scene-relative RoPE and a canvas workspace, trained via passive-to-active dense latent distillation on 13.2M ImageNet-21k scenes.


<details>
  <summary>Details</summary>
Motivation: Active computer vision offers efficient, biologically plausible perception through sequential glimpses but lacks scalable general-purpose architectures and pretraining pipelines, leaving Active-Vision Foundation Models unexplored despite their potential advantages.

Method: CanViT combines a retinotopic Vision Transformer backbone with a spatiotopic scene-wide latent workspace (canvas) using scene-relative RoPE. It employs Canvas Attention (asymmetric cross-attention) to decouple thinking from memory. The model is pretrained via policy-agnostic passive-to-active dense latent distillation, reconstructing scene-wide DINOv3 embeddings from random glimpse sequences.

Result: CanViT-B achieves 38.5% mIoU on ADE20K segmentation with a single glimpse (19.5x fewer FLOPs than best active model), reaching 45.9% with additional glimpses. On ImageNet-1k classification, it reaches 81.2% top-1 accuracy with frozen teacher probes. It generalizes to longer rollouts and larger scenes.

Conclusion: CanViT demonstrates the viability of Active-Vision Foundation Models, closing the gap between passive and active vision on semantic segmentation and establishing a new research axis for scalable, efficient active perception systems.
...
TL;DR: A practical pipeline for 3D scene reconstruction from raw 360° camera captures without special protocols or pre-processing, robust to human operators in imagery, with a new benchmark dataset.


<details>
  <summary>Details</summary>
Motivation: Radiance fields are powerful for 3D reconstruction but casual capture is challenging with perspective cameras due to narrow FOV limiting viewpoint coverage. 360° cameras offer broader coverage but existing methods require special protocols that undermine effortless workflows.

Method: Proposes a practical pipeline for reconstructing 3D scenes directly from raw 360° camera captures without special capture protocols or pre-processing. The method exhibits robustness to human operators visible in all 360° imagery. Introduces a multi-tiered dataset of scenes captured as raw dual-fisheye images as a benchmark.

Result: The method significantly outperforms vanilla 3DGS for 360° cameras and robust perspective baselines when perspective cameras are simulated from the same capture, demonstrating advantages of 360° capture for casual reconstruction.

Conclusion: The proposed pipeline enables practical 3D scene reconstruction from raw 360° camera captures without special protocols, addressing key challenges in casual capture and establishing a benchmark for robust 360° reconstruction.
...
TL;DR: Halsted is a vision-language model trained on a comprehensive surgical video atlas that outperforms previous models in mapping surgical activity and offers a web platform for surgeons to automatically analyze their own procedures.


<details>
  <summary>Details</summary>
Motivation: Current AI models for surgical behavior mapping are narrow in scope (limited to single procedures), capture limited behavioral components, and have limited translational value as they remain inaccessible to practicing surgeons. There's a need for comprehensive, accessible surgical AI that can bridge the translational gap to clinical deployment.

Method: Developed Halsted, a vision-language model trained on the Halsted Surgical Atlas (HSA) - a comprehensive annotated video library created through an iterative self-labelling framework containing over 650,000 videos across eight surgical specialties. Also released HSA-27k, a subset for benchmarking, and developed the Halsted web platform for surgeon access.

Result: Halsted surpasses previous state-of-the-art models in mapping surgical activity while offering greater comprehensiveness and computational efficiency. The web platform provides surgeons worldwide with the capability to automatically map their own procedures within minutes.
...
TL;DR: Steering uses causal interventions in Vision-Language Models to automatically explain Sparse Autoencoder features by steering features with empty images and prompting language models to describe what they "see", offering a scalable alternative to traditional input-example methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for explaining Sparse Autoencoder features in vision models rely on human intervention or correlation-based explanations from top activating input examples, lacking automated, scalable alternatives that provide causal understanding of feature representations.

Method: Leverage Vision-Language Model structure to steer individual SAE features in the vision encoder after providing an empty image, then prompt the language model to explain what it "sees", effectively eliciting visual concepts represented by each feature through causal interventions.

Result: Steering offers a scalable alternative that complements traditional input-example approaches, with explanation quality improving consistently with language model scale. The proposed Steering-informed Top-k hybrid approach combines causal interventions and input-based methods to achieve state-of-the-art explanation quality without additional computational cost.

Conclusion: Causal intervention-based steering provides a promising direction for automated interpretability in vision models, with quality scaling with language model size, and hybrid approaches combining steering with traditional methods offer superior explanation quality efficiently.
...
TL;DR: TrajLoom predicts future dense point trajectories from video using offset encoding, VAE-based latent modeling, and flow matching, extending prediction horizons from 24 to 81 frames with improved realism.


<details>
  <summary>Details</summary>
Motivation: Predicting future motion is crucial for video understanding and controllable video generation. Dense point trajectories offer a compact, expressive motion representation, but modeling their future evolution from observed video remains challenging.

Method: Three-component framework: (1) Grid-Anchor Offset Encoding reduces location bias by representing points as offsets from pixel-center anchors; (2) TrajLoom-VAE learns compact spatiotemporal latent space via masked reconstruction and consistency regularization; (3) TrajLoom-Flow generates future trajectories via flow matching with boundary cues and on-policy K-step fine-tuning. Also introduces TrajLoomBench benchmark.

Result: Extends prediction horizon from 24 to 81 frames while improving motion realism and stability across datasets. Outperforms state-of-the-art methods. Predicted trajectories directly support downstream video generation and editing.

Conclusion: The proposed framework effectively predicts future dense point trajectories with extended horizons and improved quality, enabling practical applications in video generation and editing. Comprehensive resources (code, models, datasets) are publicly available.
...
TL;DR: Dress-ED is the first large-scale benchmark unifying Virtual Try-On, Virtual Try-Off, and text-guided garment editing with 146k verified quadruplets, featuring a multimodal diffusion framework as baseline.


<details>
  <summary>Details</summary>
Motivation: Existing VTON/VTOFF datasets are static and lack instruction-driven editing capabilities for controllable and interactive fashion generation, creating a need for a unified benchmark.

Method: Created Dress-ED using fully automated multimodal pipeline with MLLM-based garment understanding, diffusion-based editing, and LLM-guided verification; proposed unified multimodal diffusion framework for joint reasoning over linguistic instructions and visual garment cues.

Result: Dataset comprises over 146k verified quadruplets spanning three garment categories and seven edit types (appearance and structural modifications), providing first large-scale benchmark for instruction-driven fashion editing.

Conclusion: Dress-ED addresses the gap in instruction-driven fashion editing datasets and provides a strong baseline framework for future research in controllable VTON/VTOFF systems.
...
TL;DR: A novel algorithm generates 3D plant architecture models from single images using vision-language models, trained on synthetic data with promising results for future real-world application.


<details>
  <summary>Details</summary>
Motivation: Field-scale measurement of 3D plant architectural parameters for procedural models is prohibitively labor-intensive, creating a need for automated extraction from simpler data sources like images.

Method: Developed a vision-language model approach that generates token sequences encoding procedural plant architecture definitions from synthetic images, using Helios 3D simulator for training data and XML-to-token conversion via a plant architecture tokenizer.

Result: Achieved token F1 score of 0.73 in teacher-forced training, with autoregressive generation achieving BLEU-4 score of 94.00% and ROUGE-L score of 0.5182, demonstrating feasibility of architectural parameter extraction from synthetic images.

Conclusion: Organ-level architectural parameters can be extracted from image data using vision-language models, with successful demonstration on synthetic data paving the way for future extension to real imagery.
...
TL;DR: Medical VLMs face a grounding-sycophancy tradeoff: models with lowest hallucination are most sycophantic, while pressure-resistant models hallucinate more. No evaluated 7-8B parameter VLM achieves adequate simultaneous grounding and pressure resistance.


<details>
  <summary>Details</summary>
Motivation: Despite strong performance on medical VQA benchmarks, the robustness of medical VLMs against critical failure modes (hallucination and sycophancy) remains poorly understood, especially in combination. This gap is crucial for clinical deployment.

Method: Evaluated six VLMs (three general-purpose, three medical-specialist) on three medical VQA datasets. Proposed three metrics: L-VASE (logit-space reformulation of VASE), CCS (confidence-calibrated sycophancy score), and Clinical Safety Index (CSI) combining grounding, autonomy, and calibration via geometric mean.

Result: Uncovered grounding-sycophancy tradeoff: models with lowest hallucination propensity are most sycophantic, while most pressure-resistant model hallucinates more than all medical-specialist models. Across 1,151 test cases, no model achieved CSI above 0.35, indicating none of the evaluated 7-8B parameter VLMs is simultaneously well-grounded and robust to social pressure.

Conclusion: Joint evaluation of hallucination and sycophancy is necessary before medical VLMs can be considered for clinical use. Current models exhibit fundamental tradeoffs between grounding and pressure resistance, highlighting safety concerns for clinical deployment.
...
TL;DR: The paper introduces a benchmark for evaluating attribution maps in semantic segmentation beyond visual plausibility, focusing on faithfulness, off-target leakage, robustness, and runtime. It also proposes Dual-Evidence Attribution (DEA), which fuses gradient and intervention signals to improve faithfulness while maintaining robustness.


<details>
  <summary>Details</summary>
Motivation: Current attribution maps for semantic segmentation are primarily judged by visual plausibility, which doesn't guarantee that highlighted pixels actually drive model predictions or that attribution credit stays within target regions. There's a need for a dedicated evaluation protocol to assess these critical aspects.

Method: The paper introduces a reproducible benchmark testing intervention-based faithfulness, off-target leakage, perturbation robustness, and runtime on Pascal VOC and SBD datasets across three pretrained backbones. It also proposes Dual-Evidence Attribution (DEA), a lightweight correction that fuses gradient evidence with region-level intervention signals through agreement-weighted fusion.

Result: DEA consistently improves deletion-based faithfulness over gradient-only baselines across all completed runs while preserving strong robustness. The benchmark reveals a faithfulness-stability tradeoff among attribution families that is hidden under visual evaluation.

Conclusion: The benchmark provides a foundation for principled method selection in segmentation explainability by exposing critical evaluation dimensions beyond visual assessment. DEA demonstrates how combining gradient and intervention evidence can improve attribution quality, though at the cost of additional compute from intervention passes.
...
TL;DR: PIVM is a diffusion-based framework for anatomically accurate abdominal CT synthesis that predicts intensity variations relative to organ-specific priors derived from segmentation labels, operating directly in image space while preserving full HU range.


<details>
  <summary>Details</summary>
Motivation: Abdominal CT data faces limitations due to high annotation costs and privacy constraints, which impede the development of robust segmentation and diagnostic models that require large annotated datasets.

Method: Prior-Integrated Variation Modeling (PIVM) uses a diffusion-based approach that predicts voxel-wise intensity variations relative to organ-specific intensity priors derived from segmentation labels. The priors and labels jointly guide the diffusion process to ensure spatial alignment and realistic organ boundaries, operating directly in image space while preserving the full Hounsfield Unit range.

Result: The method generates anatomically accurate CT images with realistic organ boundaries and fine anatomical textures without smoothing, addressing data limitations for segmentation and diagnostic model development.

Conclusion: PIVM provides a novel approach to CT image synthesis that overcomes data scarcity challenges by generating realistic, anatomically accurate images while preserving full HU range and fine textures, with potential applications in medical imaging research and model development.
...
TL;DR: CAM3R is a camera-agnostic feed-forward model for 3D reconstruction that handles wide-angle imagery without prior calibration, outperforming perspective-trained models on fisheye and panoramic datasets.


<details>
  <summary>Details</summary>
Motivation: Current 3D reconstruction models are trained primarily on perspective datasets with pinhole camera geometry, causing significant geometric degradation when applied to wide-angle imagery from fisheye or panoramic sensors.

Method: Two-view network with Ray Module for per-pixel ray direction estimation and Cross-view Module for radial distance inference with confidence maps, pointmaps, and relative poses, plus Ray-Aware Global Alignment framework for pose refinement and scale optimization while preserving local geometry.

Result: Extensive experiments on various camera model datasets (panorama, fisheye, pinhole) demonstrate CAM3R establishes new state-of-the-art in pose estimation and reconstruction.

Conclusion: CAM3R successfully addresses the limitation of perspective-trained models by providing camera-agnostic 3D reconstruction that handles wide-angle imagery without prior calibration.
...
TL;DR: Q-Tacit proposes a new paradigm for VLM-based image quality assessment that moves beyond natural language reasoning to latent quality space, achieving strong performance with fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Current VLM-based IQA methods using CoT reasoning are language-centric, treating visual information as static preconditions. The gap between discrete textual tokens and quality perception space limits reasoning effectiveness for visually intensive IQA tasks, as quality-related visual cues cannot be fully abstracted into text.

Method: Q-Tacit follows a two-stage process: (1) injecting structural visual quality priors into the latent space, and (2) calibrating latent reasoning trajectories to improve quality assessment ability. This enables VLMs to reason beyond natural language in the latent quality space.

Result: Extensive experiments show Q-Tacit can effectively perform quality reasoning with significantly fewer tokens than previous reasoning-based methods while achieving strong overall performance.

Conclusion: The paper validates that language is not the only compact representation suitable for visual quality assessment, opening possibilities for further exploration of effective latent reasoning paradigms for IQA.
...
TL;DR: JEAs outperform JEPAs for spatially localized signals (histopathology), while JEPAs are better for globally structured signals (ultrasound), providing a framework for matching SSL objectives to medical imaging modality characteristics.


<details>
  <summary>Details</summary>
Motivation: While self-supervised learning (SSL) has shown promise in medical imaging, there's no systematic investigation of which SSL objective (JEAs vs JEPAs) better aligns with clinically relevant signal organization in different imaging modalities with unique noise profiles.

Method: Empirical investigation comparing joint embedding architectures (JEAs) and joint embedding predictive architectures (JEPAs) across two medical imaging modalities: ultrasound (globally structured signals) and histopathology (spatially localized signals). Validation by board-certified radiologists and pathologists.

Result: JEAs are more effective for spatially localized informative signals (histopathology) due to view-invariance objective, while JEPAs are optimal for globally structured diagnostically relevant information (liver ultrasounds). Clinical relevance of learned features validated by medical experts.

Conclusion: Provides a framework for matching SSL objectives to structural and noise properties of medical imaging modalities: JEAs for localized signals, JEPAs for globally structured signals, enabling better alignment with clinically relevant information.
...
TL;DR: MAGICIAN is a long-term planning framework for active mapping that uses Imagined Gaussians scene representation with structural priors to maximize accumulated surface coverage through tree-search planning.


<details>
  <summary>Details</summary>
Motivation: Current active mapping approaches rely on greedy next-best-view prediction, leading to inefficient exploration and incomplete scene reconstruction. There's a need for long-term planning methods that can optimize accumulated coverage rather than immediate gains.

Method: Introduces MAGICIAN framework with Imagined Gaussians scene representation derived from pre-trained occupancy network with structural priors. Enables efficient coverage gain computation via fast volumetric rendering for any novel viewpoint. Integrates this into tree-search algorithm for long-horizon planning with closed-loop updates of Imagined Gaussians and trajectory refinement.
...
Conclusion: Long-term planning using Imagined Gaussians representation enables more efficient active mapping with better scene reconstruction compared to greedy next-best-view methods.
...
TL;DR: This paper presents a systematic investigation of large-scale avalanche mapping using Sentinel-1 SAR imagery for bi-temporal change detection, achieving strong performance with F1-scores up to 0.8061 and demonstrating the precision-recall trade-off in avalanche detection.
...
Method: The study uses bi-temporal change detection with Sentinel-1 synthetic aperture radar (SAR) imagery, treating avalanche mapping as a unimodal change detection problem relying solely on pre- and post-event SAR images. The approach involves extensive experiments across multiple alpine ecoregions with manually validated avalanche inventories.

Result: The end-to-end pipeline achieves an F1-score of 0.8061 in a conservative (F1-optimized) configuration and attains an F2-score of 0.8414 with 80.36% avalanche-polygon hit rate under a less conservative, recall-oriented (F2-optimized) tuning. Results highlight the trade-off between precision and completeness and demonstrate how threshold adjustment can improve detection of smaller or marginal avalanches.

Conclusion: The study demonstrates that unimodal SAR-based change detection provides consistent performance for large-scale avalanche mapping, with the release of the annotated multi-region dataset establishing a reproducible benchmark for SAR-based avalanche mapping.
...
TL;DR: GeoTikzBridge enhances MLLMs' geometric perception via TikZ code generation, using two models trained on complementary datasets (2.5M pairs) to improve visual reasoning in geometric problem-solving.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with fine-grained geometric structure perception, limiting their geometric understanding and visual reasoning capabilities, which this work aims to address.

Method: Proposes GeoTikzBridge framework with two models: GeoTikzBridge-Base trained on largest image-to-TikZ dataset (2.5M pairs via iterative expansion), and GeoTikzBridge-Instruct fine-tuned on instruction-augmented TikZ dataset for visual reasoning.

Result: Models achieve state-of-the-art performance among open-sourced MLLMs and can serve as plug-and-play reasoning modules to enhance geometric problem-solving performance for any MLLM/LLM.

Conclusion: GeoTikzBridge successfully enhances MLLMs' local geometric perception and visual reasoning through TikZ-based code generation, with publicly available datasets and code.
...
TL;DR: The paper introduces a multimodal benchmark evaluating MLLMs' reasoning width (parallel exploration) alongside depth (sequential chains), using 1200+ cases and a tree-of-thought protocol to assess 30+ models, revealing limitations in combining deep sequential reasoning with wide exploratory search.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language model (MLLM) evaluation focuses primarily on reasoning depth (long-chain sequential reasoning), neglecting reasoning width (parallel exploration, trial-and-error search, multi-constrained optimization). The authors aim to address this gap by creating a holistic benchmark that jointly assesses both dimensions.

Method: Developed a multimodal benchmark with 1200+ high-quality cases across heterogeneous domains. Introduced a fine-grained tree-of-thought evaluation protocol that quantifies both reasoning width (parallel path exploration, constraint application, branch pruning) and depth (sequential reasoning chains). Evaluated 12 major model families (over 30 advanced MLLMs) across difficulty tiers, question types, and required skills.

Result: Current MLLMs perform well on general/common-sense VQA tasks but struggle to combine deep sequential thought chains with wide exploratory search for genuine insight-based reasoning. The benchmark reveals characteristic failure modes in models' ability to systematically traverse multiple parallel reasoning paths while applying constraints to prune unpromising branches.

Conclusion: The paper provides a comprehensive benchmark for evaluating reasoning width alongside depth in MLLMs, highlighting current limitations and offering directions for developing models that can reason both deeper (sequential chains) and wider (parallel exploration). The analysis of failure modes suggests pathways for building more capable multimodal reasoning systems.
...
TL;DR: WiFi2Cap: A three-stage framework that generates natural language action descriptions from Wi-Fi CSI signals using vision-language teacher-student learning and mirror-consistency loss to address direction-sensitive ambiguities.


<details>
  <summary>Details</summary>
Motivation: Existing Wi-Fi CSI-based systems focus on pose estimation or predefined action classification rather than fine-grained language generation. There's a semantic gap between wireless signals and language, and direction-sensitive ambiguities (left/right confusion) make mapping CSI to natural-language descriptions challenging.

Method: Three-stage framework: 1) Vision-language teacher learns from synchronized video-text pairs, 2) CSI student aligns to teacher's visual space and text embeddings using Mirror-Consistency Loss to reduce mirrored-action and left-right ambiguities, 3) Prefix-tuned language model generates action descriptions from CSI embeddings. Also introduces WiFi2Cap Dataset (synchronized CSI-RGB-sentence benchmark).
...
Conclusion: The proposed WiFi2Cap framework successfully bridges the semantic gap between Wi-Fi signals and language, enabling fine-grained action captioning while preserving privacy through wireless sensing, with improved performance on multiple evaluation metrics.
...
<details>
  <summary>Details</summary>
Motivation: Existing reference-based face restoration methods assume age-aligned references, failing when only cross-age references are available (e.g., historical restoration or missing-person retrieval). Current approaches cannot maintain age fidelity while preserving identity when references and degraded inputs have different ages.

Method: TimeWeaver decouples identity and age conditioning. During training, it learns an age-robust identity representation by fusing a global identity embedding with age-suppressed facial tokens via a transformer-based ID-Fusion module. During inference, two training-free techniques guide sampling: Age-Aware Gradient Guidance and Token-Targeted Attention Boost, which steer the model toward desired age semantics while maintaining identity fidelity.

Result: Extensive experiments demonstrate that TimeWeaver surpasses existing methods in visual quality, identity preservation, and age consistency, effectively handling cross-age reference scenarios.

Conclusion: TimeWeaver addresses the critical limitation of existing reference-based face restoration methods by enabling cross-age restoration, achieving both identity fidelity and age consistency through decoupled conditioning and training-free inference guidance techniques.
...
TL;DR: VLMs show limited practical effectiveness for video QA in real-world gameplay scenarios, with simple baselines performing similarly to enhanced strategies, suggesting need for hybrid approaches.


<details>
  <summary>Details</summary>
Motivation: Video-based QA for long-form gameplay is labor-intensive and error-prone, but VLMs appear promising for detecting visual bugs directly from video frames. The study aims to evaluate VLM performance in practical industrial scenarios beyond curated benchmarks.

Method: Real-world study using industrial QA gameplay videos (100 videos, 41 hours, 19,738 keyframes). Sampled keyframes and asked VLM whether each contains a bug. Evaluated single-prompt baseline and two enhancement strategies without fine-tuning: (1) secondary judge model re-evaluating VLM outputs, (2) metadata-augmented prompting via retrieval of prior bug reports.

Result: Single-prompt baseline achieved precision 0.50 and accuracy 0.72. Enhancement strategies provided only marginal improvements over baseline while introducing additional computational cost and output variance. VLMs can detect certain visual bugs but performance is limited.

Conclusion: Off-the-shelf VLMs have basic capability for visual bug detection in QA gameplay videos, but further progress requires hybrid approaches that better separate textual and visual anomaly detection rather than simple prompting enhancements.
...
TL;DR: SOUPLE introduces learnable context tokens with visual features to bridge audio-visual semantic correspondence, improving localization and segmentation over fixed prompts in CLIP-based models.


<details>
  <summary>Details</summary>
Motivation: CLIP models struggle with audio-visual localization: replacing [CLS] with audio embeddings fails to capture semantic cues, and fixed prompts like "a photo of a [V_A]" don't establish meaningful connections between audio embeddings and visual context.

Method: Proposes Sound-aware Prompt Learning (SOUPLE) which replaces fixed prompts with learnable context tokens that incorporate visual features to generate conditional context for a mask decoder, bridging semantic correspondence between audio and visual inputs.

Result: Experiments on VGGSound, SoundNet, and AVSBench demonstrate improved localization and segmentation performance compared to existing approaches.

Conclusion: SOUPLE effectively addresses limitations of CLIP for audio-visual tasks by introducing learnable context tokens that better capture semantic relationships between audio and visual modalities.
...
TL;DR: MVPBench is a new benchmark for evaluating multi-modal LLMs on multi-video perception tasks, addressing limitations of existing single-image/single-video benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM benchmarks are limited to static images or single videos, overlooking complex interactions across multiple videos, creating a gap in evaluating models' ability to extract information from multiple video sequences.

Method: Introduces MVPBench with 14 subtasks across diverse visual domains, featuring 5K QA tests involving 2.7K video clips sourced from existing datasets and manually annotated clips.

Result: Extensive evaluations reveal current models struggle to process multi-video inputs effectively, showing substantial limitations in multi-video comprehension.

Conclusion: MVPBench addresses the multi-video perception gap and is anticipated to drive advancements in multi-video comprehension for MLLMs.
...
TL;DR: GPAD is a novel multimodal industrial anomaly detection network that leverages geometric priors from 3D point clouds to detect subtle surface deformations and irregular contours that are difficult to identify with 2D methods.


<details>
  <summary>Details</summary>
Motivation: Current multimodal industrial anomaly detection methods lack effective utilization of crucial geometric information like surface normal vectors and 3D shape topology, resulting in low detection accuracy for complex geometric shape defects such as subtle surface deformations and irregular contours.

Method: Proposes GPAD with: 1) A point cloud expert model for fine-grained geometric feature extraction using differential normal vector computation to enhance geometric details and generate geometric priors; 2) A two-stage fusion strategy to leverage multimodal data complementarity and 3D geometric priors; 3) Attention fusion and anomaly region segmentation based on geometric priors to enhance geometric defect perception.
...
Conclusion: The proposed GPAD network effectively addresses limitations in current multimodal anomaly detection by incorporating geometric priors from 3D point clouds, significantly improving detection accuracy for complex geometric defects in industrial applications.
...
TL;DR: SlotCurri addresses video object over-fragmentation in slot-attention models via progressive slot allocation guided by reconstruction error, structure-aware loss for semantic boundaries, and cyclic inference for temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video object-centric learning models suffer from severe over-fragmentation where single objects are represented by multiple redundant slots, as models are implicitly encouraged to occupy all available slots to minimize reconstruction objectives.

Method: 1) Reconstruction-guided slot curriculum: Start with few coarse slots, progressively allocate new slots where reconstruction error remains high. 2) Structure-aware loss: Augment MSE with local contrast and edge preservation to sharpen semantic boundaries. 3) Cyclic inference: Roll slots forward and backward through frame sequences for temporally consistent object representations.

Result: Achieves notable FG-ARI gains of +6.8 on YouTube-VIS and +8.3 on MOVi-C datasets, demonstrating effectiveness in reducing object over-fragmentation and improving video object decomposition.

Conclusion: SlotCurri successfully addresses object over-fragmentation by allocating representational capacity only where needed, enhanced by structural cues and cyclic inference, leading to improved video object decomposition performance.
...
TL;DR: ENC-Bench: First benchmark for evaluating multimodal LLMs on professional Electronic Navigational Charts understanding, revealing significant performance gaps (best model: 47.88% accuracy) across perception, spatial reasoning, and maritime decision-making tasks.


<details>
  <summary>Details</summary>
Motivation: Electronic Navigational Charts (ENCs) are safety-critical for maritime navigation but require specialized expertise to interpret. It remains unclear whether multimodal large language models (MLLMs) can reliably understand ENCs, which encode regulations, bathymetry, and route constraints through standardized vector symbols, scale-dependent rendering, and precise geometric structure.

Method: Created ENC-Bench with 20,490 expert-validated samples from 840 authentic NOAA ENCs, organized into three hierarchical levels: Perception (symbol/feature recognition), Spatial Reasoning (coordinate localization, bearing, distance), and Maritime Decision-Making (route legality, safety assessment, emergency planning). Samples generated from raw S-57 data through calibrated vector-to-image pipeline with automated consistency checks and expert review. Evaluated 10 state-of-the-art MLLMs (GPT-4o, Gemini 2.5, Qwen3-VL, InternVL-3, GLM-4.5V, etc.) under unified zero-shot protocol.

Result: Best model achieved only 47.88% accuracy, revealing systematic challenges in symbolic grounding, spatial computation, multi-constraint reasoning, and robustness to lighting/scale variations. Performance gaps demonstrate current MLLMs' limitations in professional ENC interpretation despite their capabilities in natural image understanding.

Conclusion: ENC-Bench establishes the first rigorous benchmark for ENC understanding, opening a new research frontier at the intersection of specialized symbolic reasoning and safety-critical AI. The benchmark provides essential infrastructure for advancing MLLMs toward professional maritime applications, highlighting the need for improved multimodal reasoning capabilities in safety-critical domains.
...
TL;DR: A hybrid framework combining AI super-resolution, object detection, and Vision-Language Models for post-disaster building damage assessment from satellite imagery, enhancing semantic interpretation and providing recovery recommendations.


<details>
  <summary>Details</summary>
Motivation: Rapid and accurate structural damage assessment after natural disasters is critical for emergency response, but remote sensing imagery suffers from low spatial resolution, contextual ambiguity, and limited semantic interpretability, reducing reliability of traditional detection methods.

Method: 1) Use Video Restoration Transformer (VRT) to upscale pre- and post-disaster satellite imagery from 1024x1024 to 4096x4096 resolution. 2) Apply YOLOv11-based detector to localize buildings in pre-disaster imagery. 3) Analyze cropped building regions using Vision-Language Models (VLMs) to assess structural damage across four severity levels. 4) Employ CLIPScore for reference-free semantic alignment and multi-model VLM-as-a-Jury strategy to reduce individual model bias.

Result: Experiments on subsets of the xBD dataset (Moore Tornado and Hurricane Matthew events) demonstrate enhanced semantic interpretation of damaged buildings. The framework provides helpful recommendations to first responders for recovery based on damage analysis.

Conclusion: The proposed hybrid framework effectively addresses limitations of traditional remote sensing damage assessment by integrating super-resolution, object detection, and VLMs, improving both visual detail and semantic understanding of building damage for better emergency response decision-making.
...
TL;DR: A typography-based monocular distance estimation framework using license plate characters as fiducial markers achieves accurate inter-vehicle distance estimation with 7.7% mean absolute error, outperforming plate-width methods by 35% reduction in standard deviation.


<details>
  <summary>Details</summary>
Motivation: Need for low-cost distance estimation for mass-market vehicles as LiDAR/radar are expensive, while monocular vision suffers from scale ambiguity and environmental sensitivity. License plates provide standardized typography as passive fiducial markers.

Method: Uses license plate detection and character segmentation to measure character height, computes distance via pinhole camera model. Includes interactive calibration, adaptive detection modes, multi-method character segmentation, camera pose compensation via lane-based horizon estimation, hybrid deep-learning fusion, temporal Kalman filtering, and multi-feature fusion using stroke width, character spacing, and plate border thickness.

Result: Achieved 2.3% coefficient of variation in character height across frames and 7.7% mean absolute error in controlled indoor setup. Character-based ranging reduces standard deviation by 35% compared to plate-width methods, enabling real-time operation without GPU acceleration.

Conclusion: Typography-based monocular distance estimation using license plate characters provides accurate, low-cost alternative to expensive sensors, with improved consistency over plate-width methods, reducing erratic estimates that could trigger unnecessary vehicle control actions.
...
TL;DR: Know3D is a framework that uses multimodal large language models to enable language-controllable generation of unseen 3D regions, transforming stochastic back-view hallucination into semantically controllable processes.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generation models produce stochastic and difficult-to-control unseen regions due to inherent ambiguity of single-view observations and lack of robust global structural priors from limited 3D training data, often failing to align with user intentions or producing implausible geometries.

Method: Proposes Know3D framework incorporating rich knowledge from multimodal large language models into 3D generative processes via latent hidden-state injection. Uses VLM-diffusion-based model where VLM handles semantic understanding/guidance and diffusion model transfers semantic knowledge to 3D generation model.

Result: Successfully bridges gap between abstract textual instructions and geometric reconstruction of unobserved regions, transforming traditionally stochastic back-view hallucination into semantically controllable process.

Conclusion: Demonstrates promising direction for future 3D generation models by enabling language-controllable generation of back-view for 3D assets through integration of multimodal language model knowledge.
...
TL;DR: AI monitoring study finds higher fall rates in chairs vs beds, with chair falls often involving footrest positioning failures, suggesting need for safer chair setups rather than reduced chair use.


<details>
  <summary>Details</summary>
Motivation: Traditional fall rate estimation using occupied bed-days may not accurately reflect risk during different activities; need for more precise measurement of fall risk during chair vs bed exposure to inform safety interventions.

Method: Retrospective cohort study using continuous AI monitoring to estimate fall rates by exposure time (chair vs bed hours) rather than occupied bed-days; analyzed 3,980 monitoring units with 292,914 hourly rows; used probability-weighted rates and Poisson modeling for chair-versus-bed rate ratio.

Result: Found probability-weighted fall rates of 17.8 per 1,000 chair exposure-hours vs 4.3 per 1,000 bed exposure-hours; adjusted chair-versus-bed rate ratio of 2.35 (95% CI 0.87-6.33; p=0.0907); in broader cohort, 6 of 7 direct chair falls involved footrest-positioning failures.

Conclusion: Findings suggest higher fall risk in chairs compared to beds, with footrest positioning as a key factor; results are hypothesis-generating and support testing safer chair setups rather than reducing chair use.
...
TL;DR: Lightweight plug-and-play framework for pixel-wise, view-dependent predictive uncertainty estimation in 3D Gaussian Splatting, enabling trustworthy spatial maps for autonomous agents.


<details>
  <summary>Details</summary>
Motivation: While 3D Gaussian Splatting achieves photorealistic novel view synthesis, it lacks uncertainty estimation needed for reliable spatial mapping in autonomous agents and safety-critical applications. Knowing where the representation is uncertain is as important as rendering fidelity for trustworthy deployment.

Method: Post-hoc, architecture-agnostic framework using Bayesian-regularized linear least-squares optimization over reconstruction residuals. Extracts per-primitive uncertainty channel without modifying underlying scene representation or degrading baseline visual fidelity.

Result: Successfully translates 3D Gaussian splatting into trustworthy spatial maps, improving state-of-the-art performance across three critical downstream perception tasks: active view selection, pose-agnostic scene change detection, and pose-agnostic anomaly detection.

Conclusion: The proposed lightweight, plug-and-play uncertainty estimation framework bridges the critical gap between rendering fidelity and reliability, enabling 3D Gaussian Splatting to serve as trustworthy spatial maps for autonomous agents and safety-critical applications.
...
Result: Extensive experiments demonstrate that Flickerformer outperforms state-of-the-art approaches in both quantitative metrics and visual quality, effectively removing flicker without introducing ghosting artifacts.

Conclusion: Flickerformer successfully addresses flicker artifacts by exploiting their intrinsic periodicity and directionality characteristics through specialized transformer-based modules, providing superior flicker suppression compared to existing methods.
...
TL;DR: PhotoAgent is an embodied agent for creative photography that bridges language commands to geometric control using LMM reasoning and 3D Gaussian Splatting for mental simulation.


<details>
  <summary>Details</summary>
Motivation: Embodied agents for creative tasks like photography need to overcome the semantic gap between high-level language commands (subjective aesthetic goals) and low-level geometric control parameters.

Method: PhotoAgent integrates Large Multimodal Models (LMMs) with chain-of-thought reasoning to translate aesthetic goals into geometric constraints, uses an analytical solver for initial viewpoint, then refines through visual reflection in a photorealistic 3D Gaussian Splatting world model.
...
Conclusion: The integration of LMM reasoning with 3DGS-based mental simulation provides an effective framework for embodied creative agents, bridging the semantic gap between language and geometric control for photography tasks.
...
TL;DR: PinPoint is a two-stage framework that identifies instruction-relevant image regions and extracts fine-grained visual features to improve reasoning efficiency in LVLMs, reducing computational overhead while achieving superior accuracy on VQA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models face computational inefficiency when processing visually complex images like infographics or documents, requiring many visual tokens that lead to significant overhead. Current methods lack effective mechanisms to focus only on instruction-relevant regions.

Method: Proposes PinPoint, a two-stage framework: 1) Instruction-Region Alignment that localizes relevant regions using both visual input and textual instructions, 2) Region refinement to extract fine-grained visual features. Introduces new annotations for instruction-relevant regions across InfographicVQA, MultiPageDocVQA, and SinglePageDocVQA benchmarks.

Result: PinPoint achieves superior accuracy compared to existing methods while reducing computational overhead by minimizing irrelevant visual tokens. Experimental results demonstrate effectiveness on challenging VQA benchmarks.

Conclusion: PinPoint effectively addresses computational inefficiency in LVLMs by focusing on instruction-relevant regions, improving both accuracy and efficiency for processing visually complex images, with potential applications in document understanding and infographic analysis.
...
TL;DR: TDATR improves end-to-end table recognition through table detail-aware learning and cell-level visual alignment, achieving SOTA performance on seven benchmarks without dataset-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing modular TR pipelines separate table structure and content modeling, leading to suboptimal integration and complex workflows. End-to-end approaches rely heavily on large-scale TR data and struggle in data-constrained scenarios.

Method: TDATR adopts a "perceive-then-fuse" strategy with table detail-aware learning to jointly perceive structure and content through multiple tasks under a language modeling paradigm. Includes structure-guided cell localization module for vision-language alignment.

Result: Achieves state-of-the-art or highly competitive performance on seven benchmarks without dataset-specific fine-tuning, demonstrating robustness and efficiency in data-constrained scenarios.

Conclusion: TDATR provides an effective end-to-end table recognition solution that overcomes limitations of modular pipelines and data-hungry approaches through detail-aware learning and improved vision-language alignment.
...
Method: SpaHGC uses a multi-modal heterogeneous graph model that integrates local spatial context within target slides and cross-slide similarities computed from pathology foundation model embeddings. It employs Masked Graph Contrastive Learning to enhance feature representation and transfer spatial gene expression knowledge from reference to target slides.
...
Conclusion: SpaHGC effectively models complex spatial dependencies for ST prediction from pathology images, demonstrating strong biological relevance and application potential as a cost-effective alternative to expensive ST experiments.
...
TL;DR: Proposes MVRD-rPPG, a multi-view framework for robust remote photoplethysmography that handles motion-induced occlusion through adaptive temporal compensation, dual-stream feature disentanglement, multi-view attention, and adversarial learning.


<details>
  <summary>Details</summary>
Motivation: Existing rPPG methods degrade under facial motion and occlusion due to reliance on static single-view videos. Need to tackle motion-induced occlusion in unconstrained multi-view scenarios for real-world applicability.

Method: Introduces MVRD dataset with synchronized multi-view videos. Proposes MVRD-rPPG framework with: 1) Adaptive Temporal Optical Compensation (ATOC) for motion artifact suppression; 2) Rhythm-Visual Dual-Stream Network to disentangle rhythmic and appearance features; 3) Multi-View Correlation-Aware Attention (MVCA) for adaptive view aggregation; 4) Correlation Frequency Adversarial (CFA) learning for temporal accuracy, spectral consistency, and perceptual realism.

Result: Achieves MAE of 0.90 and Pearson correlation coefficient (R) of 0.99 on MVRD movement scenario. Extensive experiments demonstrate superiority over existing methods.

Conclusion: The proposed multi-view approach effectively addresses motion-induced occlusion in rPPG measurement, providing robust performance in unconstrained real-world conditions through complementary view fusion and comprehensive learning strategies.
...
TL;DR: A marker-less multi-camera AR system that dynamically estimates camera poses using spatiotemporal field-of-view overlaps of known objects, eliminating the need for physical markers or initial calibration.


<details>
  <summary>Details</summary>
Motivation: Current multi-camera AR systems rely on markers or initial calibration, which require markers to be in the camera's field-of-view and lack flexibility for dynamic scenarios. There's a need for marker-less approaches that can work with existing scene objects and handle non-overlapping camera views.

Method: Enhanced state-of-the-art object pose estimator to update a spatiotemporal scene graph, enabling camera pose estimation through temporal field-of-view overlaps of known objects. The approach leverages object appearances across time to establish relationships even among cameras with non-overlapping views.

Result: Outperforms state-of-the-art on YCB-V and T-LESS datasets in camera pose accuracy for field-of-view overlapping scenarios. Introduces a new multi-camera, multi-object pose estimation dataset with temporal FoV overlap including both static and dynamic cameras.

Conclusion: The proposed marker-less approach effectively enables dynamic camera pose estimation for AR applications by leveraging spatiotemporal object overlaps, validated through performance on existing and new datasets.
...
TL;DR: UAV-DETR is a novel drone detection framework that balances feature representation and computational efficiency for detecting miniature drones in complex environments, achieving superior performance with reduced parameters.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for drone detection struggle to balance robust feature representation with computational efficiency, especially for detecting miniature drones against complex backgrounds under severe environmental interference.

Method: UAV-DETR integrates a small-target-friendly architecture with real-time detection capabilities, featuring a WTConv-enhanced backbone and Sliding Window Self-Attention (SWSA-IFI) encoder to capture high-frequency structural details while reducing parameters. It also includes an Efficient Cross-Scale Feature Recalibration and Fusion Network (ECFRFN) to suppress background noise and aggregate multi-scale semantics, plus a hybrid Inner-CIoU and NWD loss strategy to mitigate IoU sensitivity for small objects.

Result: UAV-DETR significantly outperforms baseline RT-DETR on custom UAV dataset (+6.61% in mAP50:95 with 39.8% parameter reduction) and public DUT-ANTI-UAV benchmark (+1.4% in Precision, +1.0% in F1-Score).

Conclusion: UAV-DETR establishes a superior trade-off between efficiency and precision in counter-UAV object detection, offering an effective solution for miniature drone detection in complex environments.
...
TL;DR: PEPO improves multimodal CoT reasoning by optimizing token-level advantages using perception priors and exploration entropy through a smooth gating mechanism, enhancing RLVR frameworks without extra supervision.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods treat multimodal CoT reasoning uniformly without distinguishing varying degrees of visual grounding, missing structured token dynamics that characterize successful reasoning.

Method: Token-level analysis reveals structured token dynamics; PEPO derives perception prior from hidden state similarity and integrates it with token entropy via smooth gating for token-level advantages, compatible with RLVR frameworks like GRPO and DAPO.

Result: Consistent improvements across diverse multimodal benchmarks including geometry reasoning, visual grounding, puzzle solving, and few-shot classification, with stable training dynamics.

Conclusion: PEPO effectively optimizes multimodal CoT reasoning by leveraging token-level perception-exploration dynamics, enhancing RLVR frameworks without additional supervision or architectural changes.
...
TL;DR: UniQueR is a unified query-based feedforward framework for efficient 3D reconstruction from unposed images, using sparse 3D queries instead of dense 2.5D representations to capture full scene geometry including occluded regions.


<details>
  <summary>Details</summary>
Motivation: Existing feedforward models (DUSt3R, VGGT, AnySplat) predict per-pixel point maps or pixel-aligned Gaussians, which are fundamentally 2.5D representations limited to visible surfaces and cannot capture occluded geometry.
...
Result: Surpasses state-of-the-art feedforward methods on Mip-NeRF 360 and VR-NeRF datasets in both rendering quality and geometric accuracy. Uses order of magnitude fewer primitives than dense alternatives while achieving strong geometric expressiveness with reduced memory and computational cost.

Conclusion: UniQueR demonstrates that sparse 3D query-based formulation enables efficient and accurate reconstruction of full 3D scenes including occluded regions, overcoming limitations of 2.5D representations in existing feedforward methods.
...
TL;DR: Gau-Occ: A multi-modal 3D semantic occupancy prediction framework using compact semantic 3D Gaussians instead of dense voxel/BEV tensors, achieving SOTA performance with computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Multi-modal fusion improves accuracy over vision-only methods for 3D semantic occupancy prediction, but existing approaches rely on computationally expensive dense voxel or BEV tensors, creating efficiency bottlenecks.

Method: Models scene as compact collection of semantic 3D Gaussians; uses LiDAR Completion Diffuser (LCD) to recover missing structures from sparse LiDAR for robust Gaussian anchor initialization; employs Gaussian Anchor Fusion (GAF) to integrate multi-view image semantics via geometry-aligned 2D sampling and cross-modal alignment.
...
Conclusion: Gau-Occ provides an efficient alternative to dense volumetric processing for 3D semantic occupancy prediction by leveraging compact semantic 3D Gaussians, achieving both spatial consistency and semantic discriminability while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Reconstruction-based anomaly detection methods suffer from identical shortcut issues where both normal and anomalous regions are well-reconstructed, limiting transferability across different scenarios. Existing methods perform well in specific settings but fail when transferred to other domains.

Method: FSR framework uses multi-scale features as reconstruction targets instead of raw pixels. Features are partitioned into non-overlapping blocks, randomly shuffled, then restored using a restoration network. A shuffling rate regulates task complexity to address identical shortcuts.

Result: Extensive experiments validate FSR's superiority and efficiency across different anomaly detection settings, demonstrating improved performance compared to existing methods.

Conclusion: FSR provides an effective universal anomaly detection framework that alleviates identical shortcut issues through feature shuffling and restoration, with theoretical explanations from network structure and mutual information perspectives.
...
TL;DR: SPMs are semi-parametric models that enable efficient sample deletion without parameter updates, achieving competitive performance with 10x faster unlearning than parametric approaches.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning research focuses on algorithms to remove training samples, but not all models are equally easy to unlearn. The authors aim to create models that are inherently easier to unlearn by design.

Method: Introduces deep semi-parametric models (SPMs) with a fusion module that aggregates information from each training sample, enabling explicit test-time deletion of selected samples without altering model parameters.

Result: SPMs achieve competitive task performance to parametric models in image classification and generation, with 11% reduction in prediction gap relative to retrained baseline on ImageNet and over 10x faster unlearning compared to existing parametric approaches.

Conclusion: SPMs provide an effective architecture for machine unlearning that maintains performance while enabling efficient sample deletion, offering a promising direction for designing models with built-in unlearning capabilities.
...
TL;DR: ForeSeaQA is a new benchmark for video QA with image-and-text queries, and ForeSea is a 3-stage forensic search system that improves accuracy by 3.5% and temporal IoU by 11.0% over prior VideoRAG models.


<details>
  <summary>Details</summary>
Motivation: Current surveillance methods struggle with specific target searches across multi-camera video due to limitations in tracking pipelines, CLIP-based models, and VideoRAG systems that require manual filtering, capture only shallow attributes, and fail at temporal reasoning. Real-world forensic searches are inherently multimodal but remain underexplored, with no proper benchmarks for evaluating video QA with multimodal queries.

Method: ForeSeaQA benchmark: Long-horizon surveillance footage with diverse multimodal questions and timestamped annotations. ForeSea system: 3-stage plug-and-play pipeline with (1) tracking module to filter irrelevant footage, (2) multimodal embedding module to index remaining clips, and (3) inference stage where top-K candidate clips are retrieved for a VideoLLM to answer queries and localize events.

Result: On ForeSeaQA, ForeSea improves accuracy by 3.5% and temporal IoU by 11.0 over prior VideoRAG models. ForeSeaQA is the first benchmark supporting complex multimodal queries with precise temporal grounding, and ForeSea is the first VideoRAG system built to excel in this setting.

Conclusion: The paper introduces both a benchmark (ForeSeaQA) and a system (ForeSea) that address the gap in multimodal video QA for forensic surveillance, demonstrating significant improvements over existing methods and establishing a foundation for future research in this underexplored domain.
...
### [60] [EVA: Efficient Reinforcement Learning for End-to-End Video Agent](https://arxiv.org/abs/2603.22918)
*Yaolun Zhang,Ruohui Wang,Jiahao Wang,Yepeng Tang,Xuanyu Zheng,Haonan Duan,Hao Lu,Hanming Deng,Lewei Lu*

Main category: cs.CV

TL;DR: EVA is an efficient reinforcement learning framework for end-to-end video agents that enables planning-before-perception reasoning to achieve query-driven, adaptive video understanding by deciding what, when, and how to watch.


<details>
  <summary>Details</summary>
Motivation: Video understanding with multimodal LLMs faces challenges due to long token sequences with extensive temporal dependencies and redundant frames. Existing approaches treat MLLMs as passive recognizers without adaptive reasoning, while agent-based methods rely on manually designed workflows and perception-first strategies, leading to inefficiency on long videos.

Method: EVA framework enables planning-before-perception through iterative summary-plan-action-reflection reasoning. It uses a three-stage learning pipeline: supervised fine-tuning (SFT), Kahneman-Tversky Optimization (KTO), and Generalized Reward Policy Optimization (GRPO), bridging supervised imitation and reinforcement learning. High-quality datasets are constructed for each stage.

Result: EVA achieves substantial improvements of 6-12% over general MLLM baselines and a further 1-3% gain over prior adaptive agent methods across six video understanding benchmarks, demonstrating comprehensive capabilities.

Conclusion: EVA presents an efficient reinforcement learning framework for end-to-end video agents that enables adaptive, query-driven video understanding through planning-before-perception reasoning, outperforming existing approaches while being more efficient on long videos.
...
<details>
  <summary>Details</summary>
Motivation: Existing feature-reconstruction methods for industrial anomaly detection suffer from shortcut learning issues that can lead to undesirable reconstruction of anomalous features, compromising detection accuracy.

Method: TFA-Net extracts hierarchical features from both template and input images, aggregates input features onto template features to filter anomalies based on similarity, uses fused template features to reconstruct feature maps, and employs random masking for enhanced performance.
...
Conclusion: The template-based feature aggregation approach provides a nontrivial and meaningful feature reconstruction task that effectively addresses shortcut learning, making TFA-Net highly suitable for practical industrial anomaly detection applications.
...
### [62] [YOLOv10 with Kolmogorov-Arnold networks and vision-language foundation models for interpretable object detection and trustworthy multimodal AI in computer vision perception](https://arxiv.org/abs/2603.23037)
*Marios Impraimakis,Daniel Vazquez,Feiyu Zhou*

Main category: cs.CV

TL;DR: A novel framework uses Kolmogorov-Arnold networks as interpretable post-hoc surrogates to model trustworthiness of YOLOv10 object detection confidence scores, enabling transparent visualization of feature influences and identifying unreliable predictions in degraded visual conditions.


<details>
  <summary>Details</summary>
Motivation: Address the limited transparency in object detection systems, particularly for autonomous vehicles, regarding the reliability of confidence scores in visually degraded or ambiguous scenes where current systems lack interpretability about when predictions are trustworthy.

Method: Employ Kolmogorov-Arnold networks as interpretable post-hoc surrogates to model trustworthiness of YOLOv10 detections using seven geometric and semantic features. The additive spline-based structure enables direct visualization of feature influences. Additionally, use a bootstrapped language-image (BLIP) foundation model to generate descriptive captions for multimodal interface.

Result: The framework accurately identifies low-trust predictions under blur, occlusion, or low texture conditions on both COCO dataset and University of Bath campus images. It produces smooth, transparent functional mappings that reveal when model confidence is well-supported versus unreliable, providing actionable insights for filtering, review, or risk mitigation.

Conclusion: The resulting system delivers interpretable object detection with trustworthy confidence estimates, offering a powerful tool for transparent and practical perception components in autonomous and multimodal AI applications through a lightweight multimodal interface that maintains interpretability.
...
TL;DR: GroupEditing is a novel framework for consistent multi-image editing that combines explicit geometric correspondences (via VGGT) with implicit temporal coherence priors (via video models), supported by new datasets and benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of performing consistent modifications across related images that vary in pose, viewpoint, and spatial layout, requiring reliable correspondences for semantically aligned edits.

Method: Proposes GroupEditing framework with: 1) Explicit geometric correspondences via VGGT for spatial alignment, 2) Implicit relationships by reformulating image groups as pseudo-videos to leverage video model temporal coherence, 3) Fusion mechanism to combine both correspondence types, 4) GroupEditData dataset for training, 5) Alignment-enhanced RoPE module for identity preservation.

Result: Extensive experiments show GroupEditing significantly outperforms existing methods in visual quality, cross-view consistency, and semantic alignment, validated by the proposed GroupEditBench benchmark.

Conclusion: GroupEditing effectively solves the multi-image editing problem by integrating explicit geometric and implicit temporal correspondences, with comprehensive dataset support and evaluation benchmarks demonstrating superior performance.
...
<details>
  <summary>Details</summary>
Motivation: To create a unified framework that simultaneously addresses dynamic scene reconstruction, semantic understanding, and real-time streaming inference, overcoming limitations of existing methods that treat these tasks separately and lack efficient streaming capabilities.

Method: 1. Higher-order motion modeling for complex, non-uniform motion captured through differentiable rendering without flow supervision. 2. Semantic feature distillation from LSeg for language-aligned representations enabling natural language querying. 3. Window-based causal attention for processing image sequences with stable, low-latency streaming inference without memory accumulation.

Result: State-of-the-art performance in dynamic estimation, rendering quality, and scene parsing: 21% improvement in motion accuracy, 1.6 dB improvement in reconstruction PSNR, and 20% improvement in segmentation mIoU over existing methods.

Conclusion: SLARM demonstrates that a unified framework combining dynamic reconstruction, semantic understanding, and streaming inference can achieve superior performance across all three domains, with the tight coupling between semantics and geometry enhancing both reconstruction accuracy and semantic understanding.
...
TL;DR: DDSR: Dual teacher distillation with subnetwork rectification for black box domain adaptation, integrating black box source model predictions with vision-language model semantic priors to generate reliable pseudo labels and mitigate noisy supervision.


<details>
  <summary>Details</summary>
Motivation: Black box domain adaptation is highly practical but extremely challenging when neither source data nor source model is accessible, limiting transferable information to only black box predictions. Existing approaches suffer from noisy supervision from pseudo labels or insufficient utilization of vision-language model semantic priors, hindering adaptation performance.

Method: Proposes DDSR (dual teacher distillation with subnetwork rectification) that: 1) Jointly exploits black box source model knowledge and vision-language model (ViL) general semantic information, 2) Adaptively integrates their complementary predictions to generate reliable pseudo labels, 3) Introduces subnetwork-driven regularization to mitigate overfitting from noisy supervision, 4) Refines target predictions to iteratively enhance both pseudo labels and ViL prompts, 5) Further optimizes target model through self-training with classwise prototypes.

Result: Extensive experiments on multiple benchmark datasets demonstrate consistent improvements over state-of-the-art methods, including those using source data or models.

Conclusion: DDSR effectively addresses limitations of black box domain adaptation by synergistically combining black box source model predictions with vision-language model semantic priors, generating reliable pseudo labels through adaptive integration and regularization, and enabling more accurate and semantically consistent adaptation.
...
### [66] [Unleashing Spatial Reasoning in Multimodal Large Language Models via Textual Representation Guided Reasoning](https://arxiv.org/abs/2603.23404)
*Jiacheng Hua,Yishu Yin,Yuhang Wu,Tai Wang,Yifei Huang,Miao Liu*

Main category: cs.CV

TL;DR: TRACE is a prompting method that enables MLLMs to generate text-based spatial representations from egocentric videos, improving 3D spatial reasoning performance across diverse MLLM backbones.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with 3D spatial reasoning because they fail to construct structured abstractions of 3D environments from video inputs, lacking the ability to model spatial relationships effectively.
...
Result: Extensive experiments on VSI-Bench and OST-Bench show that TRACE yields notable and consistent improvements over prior prompting strategies across diverse MLLM backbones with different parameter scales and training schemas.

Conclusion: TRACE effectively bridges the gap in 3D spatial reasoning for MLLMs by enabling them to construct structured spatial representations from video inputs, with ablation studies validating design choices and analyses identifying bottlenecks in current 3D spatial reasoning capabilities.
...
TL;DR: ForestPrune is a training-free token pruning method for video MLLMs that achieves high-ratio compression through spatial-temporal forest modeling, maintaining performance while reducing computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing token compression methods for video MLLMs fall short of high-ratio compression due to insufficient modeling of temporal and continual video content. There's a need for effective pruning that maintains performance while reducing computational and memory overhead.

Method: ForestPrune constructs token forests across video frames based on semantic, spatial, and temporal constraints for comprehensive video understanding. It evaluates token tree and node importance using tree depth and node roles to make globally optimal pruning decisions, operating in a training-free manner.

Result: Applied to LLaVA-Video and LLaVA-OneVision, ForestPrune retains 95.8% average accuracy while reducing 90% tokens for LLaVA-OneVision. It outperforms other token compression methods with +10.1% accuracy on MLVU and -81.4% pruning time than FrameFusion on LLaVA-Video.

Conclusion: ForestPrune effectively addresses the challenge of high-ratio token compression for video MLLMs through spatial-temporal forest modeling, achieving superior performance and efficiency compared to existing methods while being training-free.
...
### [68] [SpecEyes: Accelerating Agentic Multimodal LLMs via Speculative Perception and Planning](https://arxiv.org/abs/2603.23483)
*Haoyu Huang,Jinfa Huang,Zhongwei Wan,Xiawu Zheng,Rongrong Ji,Jiebo Luo*

Main category: cs.CV

TL;DR: SpecEyes is a speculative acceleration framework for agentic multimodal LLMs that uses a lightweight MLLM as speculative planner to predict execution trajectories, enabling early termination of expensive tool chains while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Agentic MLLMs like OpenAI o3 and Gemini Agentic Vision suffer from significant sequential overhead due to cascaded perception, reasoning, and tool-calling loops. This "agentic depth" causes prohibitive latency and limits system-level concurrency, creating a bottleneck for practical deployment.

Method: 1) Uses a lightweight, tool-free MLLM as speculative planner to predict execution trajectories; 2) Introduces cognitive gating mechanism based on answer separability for self-verification without oracle labels; 3) Designs heterogeneous parallel funnel that exploits stateless concurrency of small model to mask stateful serial execution of large model.

Result: Achieves 1.1-3.35x speedup over agentic baseline while preserving or improving accuracy (up to +6.7%). Evaluated on V* Bench, HR-Bench, and POPE benchmarks. Boosts serving throughput under concurrent workloads.
...
TL;DR: First systematic evaluation of AVSR models in video conferencing reveals severe performance degradation due to transmission distortions and human hyper-expression, addressed by creating MLD-VC dataset with Lombard effect and achieving 17.5% CER reduction.


<details>
  <summary>Details</summary>
Motivation: While Audio-Visual Speech Recognition (AVSR) has advanced in offline settings, its robustness in real-world video conferencing remains unexplored. Current models suffer severe performance degradation due to transmission distortions from VC platforms and spontaneous human hyper-expression during video calls.

Method: 1) Conducted first systematic evaluation of state-of-the-art AVSR models across mainstream VC platforms; 2) Constructed MLD-VC, the first multimodal dataset specifically for VC with 31 speakers, 22.79 hours of audio-visual data, incorporating explicit Lombard effect to capture human hyper-expression; 3) Analyzed distribution shifts caused by speech enhancement algorithms; 4) Fine-tuned AVSR models on MLD-VC to address performance degradation.

Result: 1) Identified speech enhancement algorithms as primary source of distribution shift, altering first and second formants of audio; 2) Discovered that Lombard effect distribution shift closely resembles speech enhancement-induced shift, explaining why Lombard-trained models show greater VC robustness; 3) Fine-tuning on MLD-VC achieved average 17.5% reduction in Character Error Rate (CER) across multiple VC platforms.

Conclusion: The study provides the first systematic analysis of AVSR robustness in video conferencing, revealing critical performance degradation factors and offering MLD-VC dataset as a solution. Fine-tuning on this dataset significantly improves model robustness, establishing a foundation for developing more generalizable AVSR systems for real-world VC applications.
...
TL;DR: MedObvious benchmark tests medical VLMs' ability to perform pre-diagnostic sanity checks on image sets, revealing they often hallucinate anomalies on normal inputs and struggle with input validation despite producing fluent diagnostic text.


<details>
  <summary>Details</summary>
Motivation: Current medical VLMs can generate fluent diagnostic narratives even when inputs are invalid or inconsistent, creating safety risks. Existing benchmarks assume input validation is solved, missing this critical failure mode where models produce plausible text from nonsensical inputs.

Method: Created MedObvious benchmark with 1,880 tasks testing input validation as set-level consistency capability over multi-panel image sets. Spans five progressive tiers: basic orientation/modality mismatches, clinically motivated anatomy/viewpoint verification, and triage-style cues. Includes five evaluation formats to test robustness across interfaces.

Result: Evaluation of 17 VLMs shows sanity checking remains unreliable: models hallucinate anomalies on normal inputs, performance degrades with larger image sets, and accuracy varies substantially between multiple-choice and open-ended settings.

Conclusion: Pre-diagnostic verification remains unsolved for medical VLMs and should be treated as a distinct, safety-critical capability before deployment, separate from diagnostic text generation.
...
TL;DR: FixationFormer is a transformer-based architecture that integrates expert gaze sequences as tokens with image features for improved chest X-ray classification, achieving state-of-the-art performance by preserving temporal-spatial gaze structure.


<details>
  <summary>Details</summary>
Motivation: Expert eye movements contain valuable diagnostic knowledge in radiology, but integrating gaze data into CNN-based medical image analysis is challenging due to gaze's sequential, sparse, noisy, and variable nature. Existing methods use reduced representations like heatmaps, losing temporal information.

Method: FixationFormer uses a transformer architecture to represent expert gaze trajectories as sequences of tokens, preserving temporal and spatial structure. It models gaze sequences jointly with image features using explicit cross-attention between image and gaze token sequences, addressing gaze sparsity and variability.

Result: The method achieves state-of-the-art classification performance on three publicly available benchmark chest X-ray datasets, demonstrating superior performance over existing approaches.

Conclusion: Transformer architectures naturally align with gaze data due to their sequential nature and attention mechanisms. Representing gaze as sequences in transformer-based medical image analysis enables more direct and fine-grained integration of expert diagnostic cues, improving classification performance.
...
TL;DR: PVGF-DPC is an encoder-decoder framework for generating culturally accurate captions for Dongba paintings by integrating content prompts and visual semantic-generation fusion loss to address domain shift issues.


<details>
  <summary>Details</summary>
Motivation: Dongba paintings have unique cultural and visual characteristics that cause severe domain shift when applying mainstream captioning models, creating a need for specialized methods that can capture their ethnic symbolism and cultural context.

Method: Uses MobileNetV2 encoder for visual feature extraction, 10-layer Transformer decoder initialized with BERT weights, content prompt module for culture-aware label mapping, and visual semantic-generation fusion loss for joint optimization of prompt prediction and caption generation.

Result: Constructed a dedicated dataset of 9,408 augmented Dongba painting images with culturally grounded annotations across seven thematic categories to support the proposed framework.

Conclusion: PVGF-DPC effectively bridges the gap between generic image captioning and culturally specific Dongba art by integrating cultural awareness through content prompts and fusion loss optimization.
...
TL;DR: ClusterSTM introduces a cluster-wise spatio-temporal masking strategy for efficient video-language pretraining that addresses visual information loss and temporal information leakage issues through intra-frame clustering and temporal density-based token selection.


<details>
  <summary>Details</summary>
Motivation: Large-scale video-language pretraining suffers from prohibitive computational costs. Existing masked visual modeling approaches face two fundamental limitations: severe visual information loss under high masking ratios and temporal information leakage caused by inter-frame correlations.

Method: ClusterSTM performs intra-frame clustering to partition visual tokens into multiple semantically independent clusters, then conducts cluster-wise masking by retaining the token with the highest temporal density within each cluster. It also introduces a video-text relevance reconstruction objective that aligns high-level multimodal semantics beyond conventional visual reconstruction.

Result: Extensive experiments across multiple benchmarks demonstrate that ClusterSTM achieves superior performance on video-text retrieval, video question answering, and video captioning tasks, establishing a new state-of-the-art among efficient video-language models.

Conclusion: ClusterSTM effectively addresses the limitations of existing masked visual modeling approaches for video-language pretraining through its cluster-wise spatio-temporal masking strategy and multimodal alignment objective, enabling efficient training while maintaining strong performance across diverse video-language tasks.
...
TL;DR: I²P: Identity Injection and Preservation for few-shot generative model adaptation that prevents forgetting source domain identity knowledge through identity injection and consistency alignment.
...
Method: Proposes Identity Injection and Preservation (I²P) with: 1) Identity injection module integrating source domain identity knowledge into target domain's latent space; 2) Identity substitution module with style-content decoupler and reconstruction modulator; 3) Identity consistency constraints aligning features from identity substitution.
...
<details>
  <summary>Details</summary>
Motivation: Existing weakly-supervised COD methods perform poorly compared to fully-supervised approaches. Even SAM struggles with WSCOD due to non-camouflage target responses, local responses, extreme responses, and lack of refined boundary awareness in camouflage scenes.

Method: Proposes FCL-COD with three key components: 1) Frequency-aware Low-rank Adaptation (FoRA) to incorporate camouflage scene knowledge into SAM, 2) gradient-aware contrastive learning to delineate precise foreground-background boundaries, and 3) multi-scale frequency-aware representation learning for refined boundary modeling.
...
Conclusion: The proposed FCL-COD framework effectively addresses key challenges in weakly-supervised camouflage object detection through frequency-aware adaptation and contrastive learning, achieving superior performance over existing methods.
...
TL;DR: A geometry-first approach for large-scale 3D scene generation that decouples structural composition (mesh scaffold) from appearance synthesis, enabling scalable, consistent, photorealistic 3D worlds from text descriptions.


<details>
  <summary>Details</summary>
Motivation: Text-to-image and text-to-video approaches struggle with maintaining scene- and object-level consistency at environment scale due to lack of explicit geometry, limiting their ability to generate truly immersive 3D worlds.

Method: First constructs a mesh scaffold capturing environment geometry (walls, floors, etc.) from text input, then populates it with objects using image synthesis, segmentation, and object reconstruction. The mesh scaffold is rendered to condition image synthesis, providing structural backbone for consistent appearance generation.

Result: Enables scalable, arbitrarily-sized 3D scenes with high object richness and diversity, combining robust 3D consistency with photorealistic detail, representing a significant step toward environment-scale immersive 3D world generation.

Conclusion: The geometry-first approach successfully addresses limitations of text-to-image/video methods by decoupling structure from appearance, enabling generation of large-scale, consistent, and photorealistic 3D scenes from text descriptions.
...
TL;DR: VLA-IAP introduces a training-free visual token pruning method for Vision-Language-Action models that prioritizes interaction-aligned pruning to maintain structural anchors and adapt pruning intensity based on semantic-motion alignment, achieving significant speedups while preserving task performance.
...
Method: Proposes VLA-IAP (Interaction-Aligned Pruning), a training-free method that introduces: 1) a geometric prior mechanism to preserve structural anchors, and 2) a dynamic scheduling strategy that adapts pruning intensity based on semantic-motion alignment, enabling a conservative-to-aggressive transition to ensure robustness during early uncertainty and efficiency once interaction is locked.

Result: Achieves 97.8% success rate with 1.25× speedup on LIBERO benchmark, and up to 1.54× speedup while maintaining performance comparable to unpruned backbone. Demonstrates superior and consistent performance across multiple model architectures, three simulation environments, and a real robot platform, validating strong generalization capability and practical applicability.

Conclusion: VLA-IAP successfully addresses the inference bottleneck in VLA systems through an interaction-first pruning paradigm that preserves structural anchors critical for manipulation tasks, enabling efficient deployment on resource-constrained platforms without compromising task performance.
...
TL;DR: VQ-Jarvis is a retrieval-augmented intelligent video restoration agent with enhanced perception and efficient search strategies that outperforms existing methods on complex degraded videos.


<details>
  <summary>Details</summary>
Motivation: Real-world video restoration faces challenges with heterogeneous degradations where static architectures and fixed inference pipelines fail to generalize. Existing agent-based approaches are limited by insufficient quality perception and inefficient search strategies.

Method: Proposes VQ-Jarvis with: 1) Sharp vision via VSR-Compare dataset (20K comparison pairs covering 7 degradation types, 11 enhancement operators) and trained degradation perception/judge models; 2) Fast thought via hierarchical operator scheduling with retrieval-augmented generation for easy cases and step-by-step greedy search for harder cases.
...
Conclusion: VQ-Jarvis successfully addresses limitations of existing video restoration agents through improved perception capabilities and efficient search strategies, achieving superior performance on heterogeneous degraded videos.
...
TL;DR: A novel framework for fast, zero-shot personalization of arbitrary objects in diffusion models using predicted textual inversion embeddings.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image diffusion models struggle with fast and efficient personalization, especially for arbitrary object categories beyond human subjects, limiting real-world applicability.

Method: Uses a learned network to predict object-specific textual inversion embeddings, which are integrated into UNet timesteps of diffusion models for text-conditional customization in a single forward pass.

Result: Enables rapid, zero-shot personalization of diverse objects, demonstrating effectiveness across multiple tasks and settings with flexibility and scalability.

Conclusion: First training-free, general-purpose personalization framework for diffusion models, paving way for future research in personalized image generation.
...
TL;DR: The paper presents an explainability framework for flood segmentation and car detection models (PIDNet and YOLO) using extended LRP for fusion layers and Prototypical Concept-based Explanations to provide transparent, real-time explanations suitable for drone deployment in disaster management.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for flood/wildfire segmentation and object detection lack transparency, which hinders human trust required for emergency response in disaster management scenarios.

Method: 1) Novel redistribution strategy extending Layer-wise Relevance Propagation (LRP) explanations for sigmoid-gated element-wise fusion layers in PIDNet; 2) Application of Prototypical Concept-based Explanations (PCX) for local and global concept-level explanations.

Result: The framework provides reliable and interpretable explanations while maintaining near real-time inference capabilities, making it suitable for deployment on resource-constrained platforms like UAVs, as demonstrated on a publicly available flood dataset.

Conclusion: The proposed explainability framework addresses the transparency gap in disaster management AI systems by providing comprehensive explanations for segmentation and detection models, enabling trustworthy deployment on embedded drone platforms for emergency response.
...
TL;DR: Cog3DMap introduces a framework that recurrently constructs explicit 3D memory from multi-view images, enabling MLLMs to reason directly over spatially structured 3D maps with tokens grounded in 3D space.
...
Method: Cog3DMap recurrently constructs an explicit 3D memory from multi-view images where each token is grounded in 3D space and possesses both semantic and geometric information. These tokens are then fed into the MLLM for direct reasoning over a spatially structured 3D map.

Result: The framework achieves state-of-the-art performance on various spatial reasoning benchmarks by enabling direct reasoning over explicit 3D representations rather than requiring implicit 3D structure inference.

Conclusion: Explicit 3D memory construction with tokens grounded in 3D space significantly improves MLLMs' spatial reasoning capabilities from multi-view images, addressing the fundamental challenge of geometric grounding in multimodal understanding.
...
TL;DR: GLA-CLIP addresses semantic discrepancy in sliding-window CLIP segmentation by enabling cross-window attention with proxy anchors and dynamic normalization for small objects.


<details>
  <summary>Details</summary>
Motivation: Sliding-window inference in training-free open-vocabulary semantic segmentation causes semantic discrepancy across windows due to independent processing, limiting CLIP's effectiveness with high-resolution images.

Method: Proposes GLA-CLIP framework with: 1) Extended key-value tokens incorporating contextual cues from all windows, 2) Proxy anchors aggregating similar tokens across windows to address window bias, 3) Dynamic normalization scaling/thresholding attention maps for small objects.

Result: Extensive experiments validate GLA-CLIP enhances training-free open-vocabulary semantic segmentation performance and can be equipped on existing methods to broaden their receptive field.

Conclusion: GLA-CLIP effectively addresses semantic discrepancy in sliding-window CLIP segmentation through comprehensive cross-window information exchange and adaptive attention mechanisms.
...
TL;DR: GEP transfers semantic knowledge from image VFMs to event data via alignment and generative pretraining, achieving SOTA on multiple event-based vision tasks.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer advantages in fast motion and challenging illumination, but limited labeled data and unique sensing characteristics make training event-based visual foundation models difficult.

Method: Two-stage framework: 1) Event encoder aligned to frozen visual foundation model via joint regression-contrastive objective; 2) Transformer backbone autoregressively pretrained on mixed event-image sequences to capture event-specific temporal dynamics.
...
TL;DR: TS-1M is a large-scale, globally diverse traffic sign dataset with 1M+ images across 454 categories, featuring a diagnostic benchmark for analyzing model capabilities under practical challenges like cross-region variation, long-tailed distribution, and semantic ambiguity.


<details>
  <summary>Details</summary>
Motivation: Existing traffic sign datasets and benchmarks offer limited diagnostic insight into how different modeling paradigms behave under practical challenges like cross-region variation, long-tailed categories, and semantic ambiguity, which are essential for reliable real-world autonomous driving deployment.

Method: Created TS-1M dataset with over 1 million real-world images across 454 standardized categories, designed a diagnostic benchmark with challenge-oriented settings (cross-region recognition, rare-class identification, low-clarity robustness, semantic text understanding), and conducted unified benchmarking across three learning paradigms: classical supervised models, self-supervised pretrained models, and multimodal vision-language models.

Result: Analysis revealed consistent paradigm-dependent behaviors: semantic alignment is key for cross-region generalization and rare-category recognition, while purely visual models remain sensitive to appearance shift and data imbalance. Real-scene autonomous driving experiments validated practical relevance by integrating traffic sign recognition with semantic reasoning and spatial localization.

Conclusion: TS-1M establishes a reference-level diagnostic benchmark for traffic sign recognition and provides principled insights into robust and semantic-aware traffic sign perception, demonstrating the importance of semantic alignment for practical autonomous driving applications.
...
TL;DR: A novel decomposition strategy for generating lung CT scans by synthesizing images one HU interval at a time, using multi-head VQVAE architectures to improve visual fidelity while reducing computational cost.


<details>
  <summary>Details</summary>
Motivation: Address data scarcity in medical imaging CAD models for lung cancer diagnosis by developing efficient generative AI methods that can handle the complex distribution of full Hounsfield Unit range lung CT scans.

Method: Decomposition strategy that trains generative architectures on individual tissue-focused HU windows, then merges outputs via learned reconstruction network; proposes multi-head and multi-decoder models including multi-head VQVAE for better texture capture and anatomical consistency.

Result: Significantly outperforms conventional 2D full-range baselines with 6.2% improvement in FID and superior MMD, Precision, and Recall across all HU intervals; multi-head VQVAE achieves best performance, enhancing visual fidelity and variability while reducing model complexity and computational cost.

Conclusion: Establishes a new paradigm for structure-aware medical image synthesis that aligns generative modeling with clinical interpretation, offering an efficient solution to data scarcity in lung cancer CAD systems.
...
TL;DR: MLLM-HWSI introduces a hierarchical multimodal LLM for whole slide images that aligns visual features with pathology language at four scales (cell, patch, region, WSI) to enable interpretable evidence-grounded reasoning, achieving SOTA on 13 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing CPath MLLMs compress entire WSIs into single embeddings, which hinders fine-grained grounding and ignores how pathologists synthesize evidence across different scales. There's a need for models that preserve hierarchical structure and support interpretable reasoning.
...
Conclusion: MLLM-HWSI provides accurate, interpretable outputs that mirror diagnostic workflows by aligning language with multi-scale visual evidence, advancing holistic WSI understanding and computational pathology.
...
TL;DR: PolarAPP is a joint optimization framework that improves polarimetric imaging by aligning demosaicking with downstream tasks through meta-learning, outperforming existing methods in both reconstruction quality and task performance.


<details>
  <summary>Details</summary>
Motivation: Current polarimetric imaging applications use suboptimal reconstruction strategies that extract and align raw sensor measurements without proper demosaicking, creating incomplete targets that limit downstream performance. Existing demosaicking methods are task-agnostic, optimizing only for photometric fidelity rather than utility in downstream tasks.

Method: PolarAPP introduces: 1) A feature alignment mechanism that semantically aligns representations of demosaicking and downstream networks via meta-learning, 2) An equivalent imaging constraint for demosaicking training enabling direct regression to physically meaningful outputs without relying on rearranged data, and 3) A task-refinement stage that fine-tunes the task network using the stable demosaicking front-end.

Result: Extensive experimental results demonstrate that PolarAPP outperforms existing methods in both demosaicking quality and downstream performance. The framework shows superior performance compared to current approaches.

Conclusion: PolarAPP represents the first framework to jointly optimize demosaicking and downstream tasks in polarimetric imaging, introducing task-aware reconstruction through feature alignment and equivalent imaging constraints, significantly advancing the field beyond task-agnostic approaches.
...
TL;DR: A technical report describing an audio-visual multi-view capture system that treats synchronized audio and video as first-class signals, addressing the gap in existing systems that focus primarily on video with little audio support.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view capture systems for human motion research are primarily video-focused with minimal audio acquisition and alignment support, despite audio being essential for studying conversational interactions where timing at the level of turn-taking, overlap, and prosody matters.

Method: The system combines a multi-camera pipeline with multi-channel microphone recording under a unified timing architecture, providing a practical workflow for calibration, acquisition, and quality control that supports repeatable recordings at scale.

Result: The system demonstrates quantified synchronization performance in deployment, showing that resulting recordings are temporally consistent enough to support fine-grained analysis and data-driven modeling of conversation behavior.

Conclusion: The audio-visual multi-view capture system successfully addresses the gap in existing systems by treating synchronized audio and video as first-class signals, enabling rigorous study of conversational interactions where precise timing is critical.
...
TL;DR: Adapting 2D DINOv3 visual foundation model to 3D neuronal segmentation via inflation-based adaptation and topology-aware skeleton loss for improved neuronal reconstruction accuracy.


<details>
  <summary>Details</summary>
Motivation: Lack of 3D foundation models for volumetric neuroimaging due to challenges in 3D image acquisition and scarcity of high-quality annotations, despite strong zero-shot generalization of 2D visual foundation models like DINOv3.

Method: Inflation-based adaptation strategy that inflates 2D filters into 3D operators to preserve semantic priors from DINOv3 while adapting to 3D neuronal volume patches, plus topology-aware skeleton loss to enforce structural fidelity in graph-based neuronal arbor reconstruction.

Result: Consistent improvements across four neuronal imaging datasets (BigNeuron, NeuroFly, CWMBS) with average gains of 2.9% in Entire Structure Average, 2.8% in Different Structure Average, and 3.8% in Percentage of Different Structure over state-of-the-art methods.

Conclusion: Successfully adapted 2D visual foundation model to 3D biomedical segmentation, enabling more data-efficient and morphologically faithful neuronal reconstruction through inflation-based adaptation and topology-aware structural constraints.
...
TL;DR: AgentFoX is an LLM-driven framework that transforms AIGI detection into a multi-phase analytical process using expert knowledge fusion and structured reasoning to produce interpretable forensic reports.


<details>
  <summary>Details</summary>
Motivation: The increasing realism of AI-generated images creates urgent need for reliable forensic tools. Existing detectors are specialized for specific artifacts (frequency patterns, semantic inconsistencies), leading to specialized performance and sometimes conflicting judgments.

Method: AgentFoX is an LLM-driven framework that redefines AIGI detection as dynamic, multi-phase analytical process. Uses quick-integration fusion mechanism guided by curated knowledge base with calibrated Expert Profiles and contextual Clustering Profiles. During inference: 1) high-level semantic assessment, 2) fine-grained, context-aware synthesis of signal-level expert evidence, 3) structured reasoning to resolve contradictions.

Result: Produces detailed, human-readable forensic report that substantiates verdict (not just binary output), enhancing interpretability and trustworthiness for real-world deployment. Introduces scalable agentic paradigm for intelligent integration of future forensic tools.

Conclusion: AgentFoX addresses limitations of specialized AIGI detectors by providing interpretable, multi-phase forensic analysis through LLM-driven framework with structured reasoning and knowledge fusion, enabling trustworthy deployment and future tool integration.
...
TL;DR: SAM2 adapted for zero-shot 3D CT segmentation via volumetric-aware inference modifications without fine-tuning, achieving coherent segmentations on 2,500 TotalSegmentator scans.


<details>
  <summary>Details</summary>
Motivation: Foundation models for image segmentation show strong generalization in natural images but have limited applicability to 3D medical imaging. The Segment Anything Model 2 (SAM2) lacks inherent volumetric awareness needed for CT data segmentation.

Method: Proposed inference-alone architectural and procedural modifications adapting SAM2's video-based memory mechanism to 3D CT data by treating slices as ordered sequences. Conducted systematic ablation study on 500 CT scans to evaluate prompt strategies, memory propagation schemes, and multi-pass refinement. Selected best configuration for final evaluation.

Result: SAM2 with frozen weights can produce coherent 3D segmentations when inference pipeline is carefully structured. Demonstrated feasibility of fully zero-shot approach for volumetric medical image segmentation on 2,500 CT scans from TotalSegmentator dataset.

Conclusion: Zero-shot use of SAM2 for automatic segmentation of volumetric CT data is feasible with appropriate inference modifications, addressing the model's lack of volumetric awareness through structured pipeline design without domain-specific training.
...
TL;DR: PiCo introduces an active canonicalization framework for robotic visual anomaly detection that actively projects observations onto a condition-invariant canonical manifold through physical reorientation and neural latent processing, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current robotic visual anomaly detection systems are limited by passive perception under diverse 6-DoF poses and unstable operating conditions (illumination changes, shadows), where semantic anomalies and physical disturbances coexist and interact, constraining industrial deployment.

Method: PiCo framework with two-stage cascaded mechanism: 1) Active Physical Canonicalization - robotic agent reorients objects to reduce geometric uncertainty at source; 2) Neural Latent Canonicalization - three-stage denoising hierarchy: photometric processing (input level), latent refinement (feature level), and contextual reasoning (semantic level) to progressively eliminate nuisance factors across representational scales.

Result: Achieves state-of-the-art 93.7% O-AUROC on M2AD benchmark (3.7% improvement over prior methods in static settings) and 98.5% accuracy in active closed-loop scenarios.

Conclusion: Active manifold canonicalization is critical for robust embodied perception, representing a paradigm shift from passive feature learning to active canonicalization for overcoming limitations in robotic visual anomaly detection.
...
TL;DR: VIRST-Audio is a practical framework for Audio-based Referring Video Object Segmentation that converts audio queries to text using ASR, leverages pretrained RVOS models with vision-language architecture, and uses an existence-aware gating mechanism to reduce hallucinations, achieving 3rd place in the MeViS-Audio track.


<details>
  <summary>Details</summary>
Motivation: ARVOS requires bridging acoustic signals with spatio-temporal visual representations, posing significant challenges in directly aligning audio queries with pixel-level object masks over time. The motivation is to develop a practical solution that can effectively transfer from text-based reasoning to audio-driven scenarios without requiring extensive audio-specific training.

Method: The method converts input audio into text using an ASR module, then performs segmentation using text-based supervision with a pretrained RVOS model integrated with a vision-language architecture. It incorporates an existence-aware gating mechanism that estimates whether the referred target object is present in the video and suppresses predictions when absent to reduce hallucinated masks and stabilize segmentation behavior.

Result: The approach was evaluated on the MeViS-Audio track of the 5th PVUW Challenge, where VIRST-Audio achieved 3rd place, demonstrating strong generalization and reliable performance in audio-based referring video segmentation.

Conclusion: The framework successfully enables effective transfer from text-based reasoning to audio-driven scenarios without audio-specific training, with the existence-aware gating mechanism providing robustness by reducing hallucinated masks and stabilizing segmentation behavior when target objects are absent.
...
TL;DR: InterDyad is a framework for synthesizing naturalistic interactive dynamics in dyadic (two-person) settings by querying structural motion guidance, with enhanced lip-sync and dedicated evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Existing speech-to-video synthesis methods struggle with capturing cross-individual dependencies and providing fine-grained control over reactive behaviors in dyadic interactions, limiting naturalistic interactive dynamics synthesis.

Method: 1) Interactivity Injector for identity-agnostic video reenactment using motion priors; 2) MetaQuery-based modality alignment to bridge conversational audio and motion priors; 3) MLLM for distilling linguistic intent to dictate reaction timing/appropriateness; 4) Role-aware Dyadic Gaussian Guidance (RoDG) for enhanced lip-sync under extreme head poses.
...
Conclusion: The proposed InterDyad framework effectively addresses limitations in dyadic speech-to-video synthesis by enabling naturalistic interactive dynamics through structural motion guidance querying, with improved cross-individual dependency modeling and fine-grained reactive behavior control.
...
TL;DR: Current volumetric super-resolution methods perform well on downsampled data but fail on real low-resolution scans, revealing a significant performance gap. The VoDaSuRe dataset addresses this by providing paired real high- and low-resolution 3D scans.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observation that current volumetric super-resolution methods achieve impressive results primarily on downsampled synthetic data rather than real low-resolution scans. This is driven by the scarcity of paired high- and low-resolution 3D datasets, leading to overestimated performance claims.

Method: The authors introduce VoDaSuRe, a large-scale volumetric dataset containing paired high- and low-resolution scans. They conduct comparative experiments training super-resolution models on both downsampled data and real low-resolution scans from VoDaSuRe to evaluate performance discrepancies.

Result: Results show a significant discrepancy: models trained on downsampled data produce sharper predictions but are inaccurate on real scans, while models trained on real low-resolution scans smooth fine structures. Current SR methods fail to recover structures lost in real low-resolution scans and instead predict smoothed averages.

Conclusion: Current volumetric super-resolution methods are overstated when applied to real data. Progress requires datasets with paired real scans of high complexity like VoDaSuRe. The dataset and code are publicly available to facilitate more realistic evaluation and development.
...
TL;DR: CCMA is a novel active learning framework that leverages vision-language models to provide conformally calibrated uncertainty estimates for more efficient sample selection in data-efficient learning.


<details>
  <summary>Details</summary>
Motivation: Foundation models for vision have strong capabilities but their potential for data-efficient learning remains untapped. Active learning aims to minimize annotation costs, but existing methods overlook the rich multimodal knowledge in vision-language models.

Method: CCMA bridges vision and language modalities through a teacher-student architecture. It uses a pretrained VLM as a teacher to provide semantically grounded uncertainty estimates, conformally calibrated to guide sample selection for a vision-only student model. It integrates multimodal conformal scoring with diversity-aware selection strategies.

Result: CCMA achieves superior data efficiency across multiple benchmarks and consistently outperforms state-of-the-art AL baselines, demonstrating clear advantages over methods relying solely on uncertainty or diversity metrics.

Conclusion: The framework successfully leverages multimodal knowledge from VLMs through conformal calibration to enhance active learning performance and data efficiency in visual recognition tasks.
...
TL;DR: Proposes Dual Contrastive Network (DCN) for few-shot remote sensing image scene classification, using two contrastive learning branches to address small inter-class variances and large intra-class variances.


<details>
  <summary>Details</summary>
Motivation: Address challenges in few-shot remote sensing image scene classification (FS-RSISC) where small inter-class variances and large intra-class variances make classification difficult with limited labeled samples.

Method: Transfer-based Dual Contrastive Network (DCN) with two auxiliary supervised contrastive learning branches: 1) Context-guided Contrastive Learning (CCL) with Condenser Network for inter-class discriminability, and 2) Detail-guided Contrastive Learning (DCL) with Smelter Network for intra-class invariance.

Result: Extensive experiments on four public benchmark remote sensing datasets demonstrate competitive performance of DCN compared to existing methods.

Conclusion: The proposed DCN effectively addresses FS-RSISC challenges by leveraging dual contrastive learning to enhance both inter-class discriminability and intra-class invariance, achieving state-of-the-art performance.
...
TL;DR: Gimbal360 is a framework for generating 360° panoramic scenes from unposed perspective images by bridging geometric and topological mismatches through canonical viewing spaces and topological equivariance.


<details>
  <summary>Details</summary>
Motivation: Diffusion models work well for 2D outpainting but struggle with 360° panoramic completion due to geometric/topological mismatches between perspective projections and spherical panoramas, requiring explicit bridging between these domains.

Method: Introduces Canonical Viewing Space to regularize projective geometry; Differentiable Auto-Leveling module to stabilize feature orientation without camera parameters; enforces topological equivariance in latent space to preserve ERP periodicity; uses Horizon360 dataset.

Result: Extensive experiments show state-of-the-art performance in structurally consistent 360° scene completion, demonstrating effectiveness of standardizing geometric and topological priors.

Conclusion: Explicitly addressing geometric and topological mismatches through canonical spaces and topological equivariance enables effective 360° panoramic generation from unposed perspective images, advancing panoramic scene completion capabilities.
...
TL;DR: ViKey enhances VideoLLMs' temporal reasoning using visual prompting with ordinal frame annotations and keyword-frame mapping, achieving dense-frame performance with only 20% of frames.


<details>
  <summary>Details</summary>
Motivation: Current efficiency methods like frame selection reduce computational cost but degrade temporal reasoning performance in VideoLLMs, as models struggle to infer event progression from sparse frames unlike humans.
...
Conclusion: Visual prompting with explicit ordinal annotations and keyword-frame mapping provides an effective, lightweight solution to enhance temporal understanding in VideoLLMs while maintaining computational efficiency.
...
TL;DR: A gaze-regularized framework enhances Vision Language Models for egocentric behavior understanding by incorporating gaze information, improving future event prediction by 13% over baseline models.


<details>
  <summary>Details</summary>
Motivation: Existing methods for egocentric behavior understanding rely solely on visual data and overlook critical gaze information, which provides insights into human intentions and future actions.

Method: Introduces a gaze-regularized framework that incorporates gaze information into VLM architecture through gaze-based queries and a gaze-regularization mechanism to align model attention with human attention patterns.
...
TL;DR: FDIF introduces an implicit-function representation using signed distance functions for formula-driven 3D medical image segmentation, enabling realistic geometric and intensity texture synthesis without real data or expert annotations.


<details>
  <summary>Details</summary>
Motivation: Deep learning for 3D medical segmentation requires large labeled datasets, which are difficult to obtain due to privacy constraints and high annotation costs. Existing formula-driven approaches have limited geometric expressiveness and cannot synthesize realistic textures.

Method: FDIF uses implicit-function representation based on signed distance functions (SDFs) to model complex geometries compactly. It exploits SDF surface representations to support controllable synthesis of both geometric and intensity textures for scalable pre-training without real data.

Result: FDIF consistently outperforms formula-driven methods across three medical segmentation benchmarks (AMOS, ACDC, KiTS) and three architectures (SwinUNETR, nnUNet ResEnc-L, nnUNet Primus-M). It achieves performance comparable to self-supervised approaches pre-trained on large-scale real datasets and also benefits 3D classification tasks.

Conclusion: Implicit-function-based formula supervision represents a promising paradigm for data-free representation learning in medical imaging, offering a viable alternative to real-data-dependent methods while addressing privacy and annotation cost challenges.
...
TL;DR: Gaze-regularized training framework improves VLA models for robotic manipulation by aligning internal attention with human visual patterns, achieving 4-12% performance gains without architectural changes or inference overhead.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models lack mechanisms for active visual attention allocation, which limits performance on fine-grained robotic manipulation tasks. Human gaze naturally encodes intent, planning, and execution patterns that could serve as powerful supervisory signals for guiding robot perception.
...
Result: Achieves 4-12% improvements across manipulation benchmarks, reaches equivalent performance with fewer training steps, maintains robustness under lighting variations and sensor noise, and produces interpretable attention visualizations that mirror human strategies.
...
TL;DR: PoseDriver is a unified bottom-up framework for multi-category skeleton detection in driving scenarios, achieving SOTA lane detection and demonstrating transferability to novel categories like bicycles.


<details>
  <summary>Details</summary>
Motivation: Object skeletons provide concise structural information crucial for autonomous driving, but existing approaches lack a unified architecture that can handle multiple instances and categories simultaneously using only image input.

Method: PoseDriver is a unified bottom-up framework for multi-category skeleton detection. It models each category as a distinct task to address multi-task learning challenges, and introduces a novel lane detection approach based on skeleton representations.

Result: Achieves state-of-the-art performance on OpenLane dataset for lane detection, presents a new bicycle skeleton detection dataset, and demonstrates framework transferability to novel categories.
...
TL;DR: GO-Renderer: A unified framework combining 3D reconstruction with diffusion models for high-quality object rendering with accurate viewpoint control and lighting adaptation.


<details>
  <summary>Details</summary>
Motivation: Feedforward 3D reconstruction methods efficiently recover geometry but struggle with complex appearances, while diffusion models can synthesize realistic images but lack accurate viewpoint control. There's a need to combine the strengths of both approaches.

Method: GO-Renderer integrates reconstructed 3D proxies to guide video generative models, enabling accurate viewpoint control via 3D geometry while leveraging diffusion models for high-quality rendering under arbitrary lighting without explicit material/lighting modeling.

Result: Achieves state-of-the-art performance across object rendering tasks: synthesizing images from new viewpoints, rendering objects in novel lighting environments, and inserting objects into existing videos.

Conclusion: The proposed unified framework successfully combines 3D reconstruction's viewpoint control with diffusion models' appearance synthesis capabilities, enabling high-quality object rendering with both geometric accuracy and photorealistic appearance.
...
TL;DR: Proposes a causal intervention framework for multi-modal image fusion that identifies robust cross-modal dependencies through three intervention strategies, outperforming correlation-based methods.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal fusion methods optimize statistical correlations which capture dataset-induced spurious associations that degrade under distribution shifts, motivating a causal approach to identify robust cross-modal dependencies.

Method: Three principled intervention strategies inspired by Pearl's causal hierarchy: complementary masking with spatially disjoint perturbations, random masking of identical regions, and modality dropout. A Causal Feature Integrator (CFI) learns to identify intervention-stable features through adaptive invariance gating.

Result: Achieves state-of-the-art performance on both public benchmarks and downstream high-level vision tasks, demonstrating superior robustness compared to correlation-based methods.

Conclusion: Causal intervention framework effectively identifies robust modal dependencies, overcoming limitations of statistical correlation-based approaches in multi-modal image fusion.
...
TL;DR: Proposes three components for robust cross-domain 3D object detection in dual-branch proposal-level detectors: Query-Decoupled Loss, LiDAR-Guided Depth Prior, and Complementary Cross-Modal Masking to address modality degradation and LiDAR dominance issues.


<details>
  <summary>Details</summary>
Motivation: Performance of multi-modal 3D object detection degrades substantially when deployed in target domains different from training (e.g., rain, nighttime). Two key limitations identified: 1) severe degradation of one modality in challenging domains, and 2) LiDAR branch dominance leading to underutilization of visual cues and vulnerability when point clouds are compromised.

Method: Three proposed components: 1) Query-Decoupled Loss provides independent supervision for 2D-only, 3D-only, and fused queries to rebalance gradient flow across modalities. 2) LiDAR-Guided Depth Prior augments 2D queries with instance-aware geometric priors through probabilistic fusion of image-predicted and LiDAR-derived depth distributions. 3) Complementary Cross-Modal Masking applies complementary spatial masks to image and point cloud to encourage queries from both modalities to compete within the fused decoder, promoting adaptive fusion.

Result: Extensive experiments demonstrate substantial gains over state-of-the-art baselines while preserving source-domain performance. Code and models are publicly available.

Conclusion: The proposed approach effectively addresses cross-domain generalization challenges in multi-modal 3D object detection by rebalancing modality contributions, enhancing spatial initialization, and promoting adaptive fusion, leading to robust performance across diverse domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing efficient recurrent-free frameworks for spatiotemporal prediction tend to discard textures and boundaries through strided convolutions/pooling, while purely spatial operators struggle to balance local interactions with global propagation. The challenge is to model long-range dynamics while preserving high-frequency details for sharp multi-step predictions.

Method: WaveSFNet unifies a wavelet-based codec with a spatial-frequency dual-domain gated spatiotemporal translator. The wavelet codec preserves high-frequency subband cues during downsampling/reconstruction. The translator injects adjacent-frame differences to enhance dynamic information, then performs dual-domain gated fusion between large-kernel spatial local modeling and frequency-domain global modulation, with gated channel interaction for cross-channel feature exchange.

Result: Extensive experiments demonstrate competitive prediction accuracy on Moving MNIST, TaxiBJ, and WeatherBench datasets while maintaining low computational complexity.

Conclusion: WaveSFNet effectively addresses the trade-off between preserving high-frequency details and modeling long-range dynamics in spatiotemporal prediction through its wavelet-based codec and spatial-frequency dual-domain approach, achieving both accuracy and efficiency.
...
TL;DR: Knots-10 benchmark for fine-grained visual classification of physical knots where appearance cues are suppressed, showing high accuracy with Swin-T/TransFG (97.2%) but significant cross-domain performance drop (58-69pp) due to rope appearance bias.


<details>
  <summary>Details</summary>
Motivation: Physical knot classification presents a challenging FGVC scenario where appearance cues are deliberately suppressed - different knot classes share identical rope material, color, and background, making classification rely primarily on crossing structure rather than appearance features.

Method: Introduces Knots-10 benchmark with 1,440 images using deployment-oriented split (train on loosely tied knots, test on tightly dressed ones). Evaluates five models (Swin-T, TransFG, PMG, etc.), uses McNemar tests for statistical comparison, Mantel permutation test for topology-confusion correlation, and proposes TACA regularization for embedding-topology alignment.

Result: Swin-T and TransFG achieve 97.2% accuracy; PMG scores 94.5% (jigsaw shuffling disrupts crossing continuity). McNemar tests show no significant difference between four of five backbones. Mantel test reveals topological distance significantly correlates with confusion patterns in three models (p<0.01). TACA regularization improves embedding-topology alignment from rho=0.46 to 0.65 without improving classification. Cross-domain test with phone photos shows 58-69 percentage-point accuracy drop.

Conclusion: Physical knot classification is appearance-suppressed FGVC where topological structure drives classification. Current models achieve high accuracy but show significant cross-domain degradation due to rope appearance bias. TACA regularization improves topology alignment but benefits appear driven by generic regularization rather than specific topological guidance.
...
TL;DR: This paper explores Mamba-based architectures (U-Mamba and SegMamba) for MRI-to-CT synthesis in radiotherapy planning, demonstrating advantages over traditional nnU-Net approaches for cross-modality translation.


<details>
  <summary>Details</summary>
Motivation: MRI-only treatment planning reduces patient radiation exposure and avoids registration errors from multi-modal workflows. While nnU-Net dominates MRI-to-CT synthesis, the authors investigate state-space modeling via Mamba architectures for potential advantages in cross-modality translation.

Method: Adapted U-Mamba and SegMamba architectures (originally for segmentation) to 3D cross-modality image generation. Used 3D Mamba to capture volumetric features and long-range dependencies. Evaluated on SynthRAD2025 dataset subset with registered single-channel MRI-CT pairs across three anatomical regions.

Result: The 3D Mamba architecture effectively captures complex volumetric features and long-range dependencies, enabling accurate CT synthesis with fast inference times. Quantitative evaluation used image similarity metrics in Hounsfield Units and segmentation-based metrics from TotalSegmentator to ensure geometric consistency.

Conclusion: Mamba-based architectures show promise for MRI-to-CT synthesis in radiotherapy workflows, with findings paving the way for integrating state-space models into clinical practice as an alternative to convolutional neural networks.
...
TL;DR: 3D Gaussian Splatting methods suffer from blurry renderings due to suboptimal loss functions. The paper introduces WD-R (regularized Wasserstein Distortion) as a superior perceptual loss that significantly improves visual quality without increasing computational cost.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods rely on ad-hoc pixel-level losses that produce blurry renderings, despite their output being consumed by human viewers. There's a need for systematic exploration of perceptual optimization strategies to improve visual quality.

Method: Conducted large-scale human subjective study (39,320 pairwise ratings) across multiple datasets and 3DGS frameworks. Systematically explored diverse distortion losses and identified WD-R (regularized Wasserstein Distortion) as optimal. Applied WD-R to various frameworks including Mip-Splatting and Scaffold-GS.

Result: WD-R emerges as clear winner: preferred 2.3× over original 3DGS loss and 1.5× over Perceptual-GS. Achieves SOTA LPIPS, DISTS, and FID scores across datasets. Generalizes well to other frameworks, improving perceptual quality within similar resource budgets. Enables ≈50% bitrate savings for 3DGS scene compression.

Conclusion: WD-R is a superior perceptual loss function for 3D Gaussian Splatting that significantly improves visual quality without increasing computational cost, generalizes across frameworks, and enables substantial compression improvements.
...
TL;DR: Ker-VLJEPA-3B: A four-phase curriculum learning framework for free-text radiology report generation from thoracic CT volumes, achieving state-of-the-art performance on CT-RATE benchmark through modality-agnostic design and novel training techniques.


<details>
  <summary>Details</summary>
Motivation: Address challenges in automated radiology report generation from 3D CT volumes: extreme sequence lengths, severe class imbalance, and LLMs' tendency to ignore visual tokens in favor of linguistic priors.

Method: Four-phase curriculum learning: 1) Self-supervised visual backbone (LeJEPA ViT-Large) trained on unlabeled CTs without text; 2) Progressive adaptation of Llama 3.2 3B decoder to ground output in visual features; 3) Modality-agnostic design integrating any self-supervised encoder; 4) Key innovations: zone-constrained cross-attention, PCA whitening, positive-findings-only strategy, warm bridge initialization, and selective cross-attention freezing with elastic weight consolidation.
...
Conclusion: The framework successfully addresses key challenges in radiology report generation through modality-agnostic design, curriculum learning, and novel training techniques, demonstrating superior performance while maintaining visual grounding in generated reports.
...
TL;DR: ARGENT introduces an adaptive entailment loss with norm regularization to prevent cone collapse in hyperbolic VLMs, plus a probabilistic entailment protocol for hierarchical evaluation, achieving SOTA improvements.


<details>
  <summary>Details</summary>
Motivation: Existing hyperbolic VLMs suffer from unstable entailment losses causing catastrophic cone collapse that destroys hierarchical structure, and hierarchical evaluation methods are unreliable due to taxonomy dependence and ambiguous negatives.

Method: Proposes adaptive entailment loss paired with norm regularizer to prevent cone collapse without heuristic aperture clipping, and introduces angle-based probabilistic entailment protocol (PEP) for evaluating hierarchical understanding with AUC-ROC and Average Precision metrics.
...
Conclusion: The proposed ARGENT framework provides a stronger hyperbolic VLM baseline that effectively addresses cone collapse issues and enables more reliable hierarchical evaluation through adaptive losses and probabilistic entailment protocols.
...
TL;DR: PFGS360 is a pose-free omnidirectional 3D Gaussian Splatting method that reconstructs 3D Gaussians from unposed 360° videos without requiring SfM-based camera poses, using spherical consistency-aware pose estimation and depth-inlier-aware densification.


<details>
  <summary>Details</summary>
Motivation: Existing omnidirectional 3D Gaussian Splatting methods rely on slow Structure-from-Motion (SfM) to provide camera poses and sparse point priors, which limits their practicality. The authors aim to develop a pose-free method that can reconstruct 3D Gaussians directly from unposed omnidirectional videos.

Method: The method consists of two key modules: 1) A spherical consistency-aware pose estimation module that recovers camera poses by establishing consistent 2D-3D correspondences between reconstructed Gaussians and unposed images using Gaussians' internal depth priors. 2) A depth-inlier-aware densification module that extracts depth inliers and Gaussian outliers with consistent monocular depth priors to enable efficient Gaussian densification for photorealistic novel view synthesis.

Result: The experiments demonstrate significant outperformance over existing pose-free and pose-aware 3DGS methods on both real-world and synthetic 360-degree videos. The method achieves photorealistic novel view synthesis without requiring pre-computed camera poses.

Conclusion: PFGS360 successfully enables pose-free omnidirectional 3D Gaussian reconstruction from unposed videos, eliminating the dependency on SfM-based pose estimation while achieving superior novel view synthesis quality compared to existing methods.
...
TL;DR: A framework that upgrades pre-trained video diffusion models to generate ultra-high-resolution videos using only image data, avoiding expensive 3D attention computations through a two-stage adaptation strategy.


<details>
  <summary>Details</summary>
Motivation: Transformer-based video diffusion models suffer from quadratic time/memory complexity due to 3D attention, making end-to-end training for ultra-high-resolution videos prohibitively expensive. Direct fine-tuning with high-resolution images introduces noise due to the image-video modality gap.

Method: Proposes Relay LoRA: a two-stage adaptation strategy. First stage adapts video diffusion model to image domain using low-resolution images to bridge modality gap. Second stage adapts with high-resolution images for spatial extrapolation. Also introduces High-Frequency-Awareness-Training-Objective to enhance fine-grained detail synthesis via dedicated reconstruction loss for high-frequency components.
...
Conclusion: The proposed framework successfully enables high-resolution video synthesis from pre-trained video diffusion models using only image data, overcoming computational bottlenecks and modality gaps through decoupled learning objectives and specialized adaptation strategies.
...
TL;DR: ABot-PhysWorld is a 14B Diffusion Transformer model that generates physically plausible, action-controllable manipulation videos using physics-aware training and DPO-based post-training, achieving SOTA performance on new benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current video-based world models often produce physically implausible manipulations (object penetration, anti-gravity motion) due to training on generic visual data with likelihood-based objectives that ignore physical laws.

Method: 1) 14B Diffusion Transformer model trained on curated dataset of 3M manipulation clips with physics-aware annotation; 2) Novel DPO-based post-training framework with decoupled discriminators to suppress unphysical behaviors while preserving visual quality; 3) Parallel context block for precise spatial action injection enabling cross-embodiment control.

Result: ABot-PhysWorld achieves new state-of-the-art performance on PBench and the newly introduced EZSbench benchmark, surpassing Veo 3.1 and Sora v2 Pro in physical plausibility and trajectory consistency.

Conclusion: The work presents a physically plausible video generation model with action controllability, introduces a training-independent benchmark for embodied zero-shot evaluation, and demonstrates superior performance over existing models while promoting standardized evaluation in embodied video generation.
...
### [116] [SIMART: Decomposing Monolithic Meshes into Sim-ready Articulated Assets via MLLM](https://arxiv.org/abs/2603.23386)
*Chuanrui Zhang,Minghan Qin,Yuang Wang,Baifeng Xie,Hang Li,Ziwei Wang*

Main category: cs.CV

TL;DR: SIMART is a unified MLLM framework that jointly performs part-level decomposition and kinematic prediction for articulated 3D object generation, using sparse 3D VQ-VAE to reduce token counts by 70% compared to dense voxel approaches.


<details>
  <summary>Details</summary>
Motivation: There's a gap in "sim-ready" interactive 3D assets for embodied AI and physical simulation. Current methods either focus on static meshes or use multi-stage pipelines that accumulate errors, while unified MLLMs face scalability issues due to dense voxel-based tokenization's high memory overhead.

Method: Proposes SIMART, a unified MLLM framework that jointly performs part-level decomposition and kinematic prediction. Introduces a Sparse 3D VQ-VAE that reduces token counts by 70% compared to dense voxel tokens, enabling high-fidelity multi-part assemblies.

Result: Achieves state-of-the-art performance on PartNet-Mobility and in-the-wild AIGC datasets. Enables physics-based robotic simulation with reduced computational overhead.

Conclusion: SIMART addresses the scalability limitations of unified MLLMs for articulated object generation through sparse 3D tokenization, providing a single-stage solution for creating "sim-ready" interactive 3D assets.
...
TL;DR: GeoSANE is a geospatial model foundry that learns to generate neural network weights from existing foundation models, creating unified representations for multiple remote sensing tasks and modalities without traditional pre-training.


<details>
  <summary>Details</summary>
Motivation: Current remote sensing foundation models are trained on different modalities, datasets, and objectives, resulting in complementary but fragmented capabilities. Instead of choosing one model over another, there's a need to combine their strengths into a single unified representation.

Method: GeoSANE learns a unified neural representation from the weights of existing foundation models and task-specific models, enabling on-demand generation of novel neural network weights for target architectures. It can generate weights ready for finetuning for classification, segmentation, and detection tasks across multiple modalities.

Result: Models generated by GeoSANE consistently outperform those trained from scratch, match or surpass state-of-the-art remote sensing foundation models, and outperform models obtained through pruning or knowledge distillation when generating lightweight networks. Evaluations across ten diverse datasets and GEO-Bench confirm strong generalization capabilities.

Conclusion: GeoSANE introduces a new framework for unifying and transferring geospatial knowledge across models and tasks by shifting from traditional pre-training to weight generation, enabling more efficient and effective model development for remote sensing applications.
...
### [118] [VISion On Request: Enhanced VLLM efficiency with sparse, dynamically selected, vision-language interactions](https://arxiv.org/abs/2603.23495)
*Adrian Bulat,Alberto Baldrati,Ioannis Maniadis Metaxas,Yassine Ouali,Georgios Tzimiropoulos*

Main category: cs.CV

TL;DR: VISOR improves LVLM efficiency by sparsifying text-image interactions instead of reducing visual tokens, using strategic attention layers and dynamic computation allocation based on sample complexity.


<details>
  <summary>Details</summary>
Motivation: Existing visual token reduction methods create information bottlenecks that impair performance on tasks requiring fine-grained understanding and reasoning. There's a need for efficiency improvements that don't discard visual information.

Method: VISOR sparsifies interaction between image and text tokens using strategically placed attention layers: efficient cross-attention provides general visual context, while dynamically selected self-attention layers refine visual representations when needed. Includes training a universal network across computational budgets and a lightweight policy for dynamic computation allocation.
...
Conclusion: VISOR successfully challenges the visual token reduction paradigm by preserving full visual information while improving efficiency through intelligent interaction sparsification and dynamic computation allocation.
...
### [119] [I3DM: Implicit 3D-aware Memory Retrieval and Injection for Consistent Video Scene Generation](https://arxiv.org/abs/2603.23413)
*Jia Li,Han Yan,Yihang Chen,Siqi Li,Xibin Song,Yifu Wang,Jianfei Cai,Tien-Tsin Wong,Pan Ji*

Main category: cs.CV

TL;DR: I3DM introduces an implicit 3D-aware memory mechanism for consistent video scene generation that bypasses explicit 3D reconstruction, achieving superior revisit consistency and camera control.


<details>
  <summary>Details</summary>
Motivation: Existing video generation methods struggle with long-term scene consistency when revisiting previously explored areas. Current solutions either rely on error-prone explicit 3D geometry reconstruction or naive camera FoV retrieval that fails under complex occlusions.

Method: Proposes I3DM with two key components: 1) 3D-aware memory retrieval using intermediate features from a pre-trained FF-NVS model to score view relevance for robust retrieval under occlusions, and 2) 3D-aligned memory injection module that implicitly warps historical content to target view and adaptively conditions generation on reliable warping regions.
...
Conclusion: I3DM provides an effective solution for consistent video scene generation by leveraging implicit 3D-aware memory mechanisms without explicit 3D reconstruction, addressing limitations of existing methods in handling occlusions and maintaining long-term scene consistency.
...
### [120] [SIGMA: A Physics-Based Benchmark for Gas Chimney Understanding in Seismic Images](https://arxiv.org/abs/2603.23439)
*Bao Truong,Quang Nguyen,Baoru Huang,Jinpei Han,Van Nguyen,Ngan Le,Minh-Tan Pham,Doan Huy Hien,Anh Nguyen*

Main category: cs.CV

TL;DR: SIGMA is a new physics-based dataset for gas chimney detection and enhancement in seismic images, addressing the lack of labeled data for deep learning approaches in seismic interpretation.


<details>
  <summary>Details</summary>
Motivation: Gas chimneys are crucial for hydrocarbon assessment and drilling safety, but accurate detection is challenging due to seismic attenuation/scattering. Traditional physics-based methods are computationally expensive and sensitive to errors, while deep learning lacks labeled datasets for training.

Method: Created SIGMA dataset using physics-based methods covering diverse geological settings and acquisition conditions. The dataset includes: (i) pixel-level gas-chimney masks for detection tasks, and (ii) paired degraded and ground-truth images for enhancement tasks.

Result: SIGMA serves as a challenging benchmark for gas chimney interpretation and benefits general seismic understanding through comprehensive experiments demonstrating its utility.

Conclusion: The SIGMA dataset addresses critical data limitations in seismic interpretation, providing a physics-based foundation for developing more accurate and efficient gas chimney detection and enhancement methods using deep learning approaches.
...
### [121] [3DCity-LLM: Empowering Multi-modality Large Language Models for 3D City-scale Perception and Understanding](https://arxiv.org/abs/2603.23447)
*Yiping Chen,Jinpeng Li,Wenyu Ke,Yang Luo,Jie Ouyang,Zhongjie He,Li Liu,Hongchao Fan,Hao Wu*

Main category: cs.CV

TL;DR: 3DCity-LLM is a unified framework for 3D city-scale vision-language understanding that uses coarse-to-fine feature encoding and is trained on a new 1.2M dataset, achieving state-of-the-art performance on urban scene reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modality large language models excel in object-centric or indoor scenarios but struggle with 3D city-scale environments, creating a significant gap in urban intelligence applications.

Method: Proposes 3DCity-LLM framework with coarse-to-fine feature encoding through three parallel branches (target object, inter-object relationship, global scene). Introduces 3DCity-LLM-1.2M dataset with 1.2M high-quality samples across seven task categories, integrating explicit 3D numerical information and user-oriented simulations.

Result: Extensive experiments on two benchmarks demonstrate that 3DCity-LLM significantly outperforms existing state-of-the-art methods, offering superior spatial reasoning and urban intelligence capabilities.

Conclusion: 3DCity-LLM provides a promising direction for advancing spatial reasoning and urban intelligence, bridging the gap in 3D city-scale vision-language understanding with its unified framework and comprehensive dataset.
...
### [122] [DetPO: In-Context Learning with Multi-Modal LLMs for Few-Shot Object Detection](https://arxiv.org/abs/2603.23455)
*Gautam Rajendrakumar Gare,Neehar Peri,Matvei Popov,Shruti Jain,John Galeotti,Deva Ramanan*

Main category: cs.CV

TL;DR: DetPO is a gradient-free test-time optimization method that refines text-only prompts for few-shot object detection in multi-modal LLMs, improving detection accuracy by optimizing prompts on few-shot visual examples without model fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with out-of-distribution generalization for object detection and cannot effectively leverage few-shot visual examples and textual descriptions. Since frontier MLLMs are API-only and open-weights models are too expensive to fine-tune, there's a need for black-box prompt optimization methods.

Method: Detection Prompt Optimization (DetPO) - a gradient-free test-time optimization approach that refines text-only prompts by maximizing detection accuracy on few-shot visual training examples while calibrating prediction confidence. It works without model gradients or fine-tuning.
...
Conclusion: DetPO provides an effective black-box solution for improving few-shot object detection in MLLMs through prompt optimization, addressing limitations of current in-context prompting approaches without requiring model fine-tuning.
...
### [123] [RealMaster: Lifting Rendered Scenes into Photorealistic Video](https://arxiv.org/abs/2603.23462)
*Dana Cohen-Bar,Ido Sobol,Raphael Bensadoun,Shelly Sheynin,Oran Gafni,Or Patashnik,Daniel Cohen-Or,Amit Zohar*

Main category: cs.CV

TL;DR: RealMaster bridges the sim-to-real gap by using video diffusion models to transform 3D engine renders into photorealistic videos while maintaining full geometric and dynamic alignment with the original 3D control.


<details>
  <summary>Details</summary>
Motivation: Current video generation models lack precise control and 3D consistency, while 3D engines offer granular control but produce unrealistic "uncanny valley" results. There's a need to bridge this gap by combining structural precision (preserving geometry/dynamics) with global semantic transformation (achieving photorealism).

Method: Uses video diffusion models to lift rendered video into photorealistic video while maintaining alignment with 3D engine output. Trains on paired dataset generated via anchor-based propagation strategy (first/last frames enhanced for realism, propagated across intermediate frames using geometric conditioning cues). Distills high-quality outputs into an IC-LoRA model that generalizes beyond pipeline constraints.

Result: Significantly outperforms existing video editing baselines on complex GTA-V sequences, improving photorealism while preserving geometry, dynamics, and identity specified by original 3D control. Enables handling of objects/characters appearing mid-sequence and inference without requiring anchor frames.

Conclusion: RealMaster successfully bridges the sim-to-real gap by combining the precise control of 3D engines with the photorealism of video diffusion models, achieving both structural precision and global semantic transformation while maintaining 3D consistency.
...
### [124] [InverFill: One-Step Inversion for Enhanced Few-Step Diffusion Inpainting](https://arxiv.org/abs/2603.23463)
*Duc Vu,Kien Nguyen,Trong-Tung Nguyen,Ngan Nguyen,Phong Nguyen,Khoi Nguyen,Cuong Pham,Anh Tran*

Main category: cs.CV

TL;DR: InverFill: A one-step inversion method for few-step inpainting that injects semantic information from masked images into initial noise, enabling high-fidelity results without training specialized models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models achieve photorealism but require many sampling steps, while few-step text-to-image models applied to inpainting produce poor harmonization and artifacts due to random Gaussian noise initialization causing semantic misalignment and reduced fidelity.

Method: Proposes InverFill, a one-step inversion method that injects semantic information from input masked images into initial noise, enabling high-fidelity few-step inpainting. Uses few-step text-to-image models in a blended sampling pipeline with semantically aligned noise as input, without requiring real-image supervision or training specialized inpainting models.

Result: InverFill significantly improves vanilla blended sampling and matches specialized inpainting models at low NFEs (number of function evaluations). Consistently boosts baseline few-step models, improving image quality and text coherence without costly retraining or heavy iterative optimization.

Conclusion: InverFill enables efficient, high-quality inpainting by addressing the semantic misalignment issue in few-step diffusion models through semantic noise initialization, offering practical advantages over both slow diffusion models and naive few-step approaches.
...
### [125] [UniFunc3D: Unified Active Spatial-Temporal Grounding for 3D Functionality Segmentation](https://arxiv.org/abs/2603.23478)
*Jiaying Lin,Dan Xu*

Main category: cs.CV

TL;DR: UniFunc3D is a unified, training-free framework for 3D scene functionality segmentation that uses multimodal LLMs as active observers to ground natural language instructions into precise interactive element masks through joint semantic, temporal, and spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene functionality segmentation rely on fragmented pipelines that suffer from visual blindness during initial task parsing, limited by single-scale, passive, and heuristic frame selection approaches.

Method: UniFunc3D treats multimodal large language models as active observers, consolidating semantic, temporal, and spatial reasoning into a single forward pass. It introduces active spatial-temporal grounding with a coarse-to-fine strategy that adaptively selects correct video frames and focuses on high-detail interactive parts while preserving global context for disambiguation.

Result: On the SceneFun3D benchmark, UniFunc3D achieves state-of-the-art performance, surpassing both training-free and training-based methods by a large margin with a relative 59.9% mIoU improvement, without any task-specific training.

Conclusion: UniFunc3D demonstrates that unified, training-free frameworks using multimodal LLMs as active observers can effectively address 3D scene functionality segmentation by performing joint reasoning to ground task decomposition in direct visual evidence.
...
TL;DR: TETO is a teacher-student framework that learns event-based motion estimation from minimal real-world data (~25 minutes) by distilling knowledge from a pretrained RGB tracker, achieving SOTA tracking and optical flow with dramatically reduced training data.


<details>
  <summary>Details</summary>
Motivation: Existing event-based motion estimators rely on large-scale synthetic data with significant sim-to-real gaps, creating a need for methods that can learn from limited real-world event data without extensive annotations.

Method: TETO uses a teacher-student framework with knowledge distillation from a pretrained RGB tracker. It employs motion-aware data curation and query sampling to disentangle object motion from ego-motion, learning from only ~25 minutes of unannotated real-world recordings. The estimator jointly predicts point trajectories and dense optical flow, which are used as explicit motion priors to condition a pretrained video diffusion transformer for frame interpolation.

Result: Achieves state-of-the-art point tracking on EVIMO2 and optical flow on DSEC using orders of magnitude less training data. Demonstrates that accurate motion estimation directly translates to superior frame interpolation quality on BS-ERGB and HQ-EVFI benchmarks.

Conclusion: TETO successfully bridges the sim-to-real gap in event-based motion estimation by leveraging knowledge distillation from RGB trackers and learning from minimal real-world data, enabling accurate motion estimation that benefits downstream applications like frame interpolation.
...
TL;DR: OVIE enables monocular novel-view synthesis using only unpaired internet images, leveraging monocular depth estimation as a geometric scaffold during training but requiring no geometry at inference, achieving state-of-the-art zero-shot performance with 600x speedup.


<details>
  <summary>Details</summary>
Motivation: Traditional monocular novel-view synthesis requires multi-view image pairs for supervision, which limits training data scale and diversity. The authors argue that one view should be sufficient, enabling training on vast amounts of unpaired internet images.

Method: OVIE uses a monocular depth estimator as a geometric scaffold during training: source images are lifted into 3D, transformed with sampled camera parameters, and projected to create pseudo-target views. A masked training formulation restricts losses to valid regions to handle disocclusions, enabling training on 30 million uncurated images. At inference, the system is geometry-free.

Result: OVIE outperforms prior methods in zero-shot settings while being 600x faster than the second-best baseline. The model was trained exclusively on in-the-wild images without requiring curated multi-view datasets.

Conclusion: Monocular novel-view synthesis can be effectively learned from unpaired internet images alone, eliminating the need for multi-view supervision. The approach demonstrates that geometric scaffolding during training enables geometry-free inference, achieving superior performance and efficiency.
...
TL;DR: AgentRVOS is a training-free agentic pipeline for Referring Video Object Segmentation that uses SAM3 for full spatio-temporal perception and MLLM for query-grounded reasoning, achieving SOTA performance among training-free methods.


<details>
  <summary>Details</summary>
Motivation: Existing training-free RVOS methods rely on MLLMs to make temporal decisions (selecting keyframes) before having object-level evidence, which limits reasoning quality and spatio-temporal coverage.

Method: AgentRVOS uses SAM3 to provide reliable perception over the full video through generated mask tracks, then employs an MLLM for query-grounded reasoning over this object-level evidence with iterative pruning guided by SAM3's temporal existence information.
...
Conclusion: The proposed agentic pipeline effectively leverages complementary strengths of SAM3 and MLLMs, overcoming limitations of previous training-free approaches by providing object-level evidence before temporal reasoning.
...
TL;DR: Foveated diffusion models optimize generation efficiency by allocating tokens non-uniformly based on human visual acuity, with higher density in gaze-focused foveal regions and lower density in peripheral areas, reducing token count and generation time while maintaining perceptual quality.


<details>
  <summary>Details</summary>
Motivation: The computational complexity of diffusion models grows quadratically with token count, making high-resolution, high-frame-rate generation increasingly challenging. Human visual acuity is eccentricity-dependent, with high resolution only in the foveal region and degraded resolution in the periphery, suggesting opportunities for efficiency optimization when gaze location is known.

Method: Develops a principled mechanism for constructing mixed-resolution tokens directly from high-resolution data, using a mask modeling foveated resolution to allocate tokens non-uniformly. Enables post-training of foveated diffusion models from existing base models while maintaining content consistency across resolutions.

Result: The approach yields results perceptually indistinguishable from full-resolution generation while drastically reducing token count and generation time. Validated through extensive analysis and a carefully designed user study, demonstrating foveation as a practical and scalable axis for efficient generation.

Conclusion: Foveated diffusion models represent an effective approach to optimizing generation efficiency by leveraging human visual perception characteristics, particularly when gaze location is known or can be estimated through eye tracking, enabling practical and scalable high-resolution content creation.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets for video world models lack diverse, semantically meaningful action spaces and have actions directly tied to visual observations rather than underlying states, making it difficult to learn structured world dynamics and maintain consistent evolution over long horizons.

Method: Automatically collected data from Monster Hunter: Wilds, containing over 108 million frames with more than 450 actions (movement, attacks, skill casting) and synchronized per-frame annotations of character skeletons, world states, camera poses, and depth maps.
...
Conclusion: The dataset highlights the need for state-aware video generation and provides a benchmark for developing world models that can handle complex action spaces and maintain state consistency over extended time horizons.
...
TL;DR: DA-Flow introduces degradation-aware optical flow estimation by leveraging diffusion model features with spatio-temporal attention, achieving superior performance on corrupted videos.


<details>
  <summary>Details</summary>
Motivation: Optical flow models trained on clean data perform poorly on real-world corrupted videos with blur, noise, and compression artifacts, creating a need for degradation-aware approaches.

Method: Lifts image restoration diffusion models to attend across frames via spatio-temporal attention, then fuses these corruption-aware features with convolutional features in an iterative refinement framework (DA-Flow).

Result: DA-Flow substantially outperforms existing optical flow methods under severe degradation across multiple benchmarks, demonstrating zero-shot correspondence capabilities.

Conclusion: Diffusion model features with spatio-temporal attention provide effective degradation-aware representations for optical flow estimation on corrupted videos.
...
TL;DR: Unified RL framework (UniGRPO) for interleaved text-image generation using GRPO for reasoning and modified FlowGRPO for image synthesis, with key modifications for scalability to multi-round interactions.


<details>
  <summary>Details</summary>
Motivation: Advance unified models for interleaved generation by developing a reinforcement learning framework tailored for multimodal generation, starting with the fundamental unit of reasoning-driven image generation.

Method: Formulate multimodal generation as MDP with sparse terminal rewards; introduce UniGRPO to jointly optimize text and image policies using GRPO for reasoning and modified FlowGRPO for visual synthesis. Key modifications: eliminate classifier-free guidance for linear rollouts, replace latent KL penalty with MSE penalty on velocity fields.

Result: Unified training recipe significantly enhances image generation quality through reasoning, providing a robust and scalable baseline for future post-training of fully interleaved models.

Conclusion: The proposed UniGRPO framework successfully advances interleaved generation by providing a scalable RL approach that improves multimodal generation quality and lays foundation for more complex multi-round interactions.
...
### [133] [OccAny: Generalized Unconstrained Urban 3D Occupancy](https://arxiv.org/abs/2603.23502)
*Anh-Quan Cao,Tuan-Hung Vu*

Main category: cs.CV

TL;DR: OccAny is a generalized 3D occupancy prediction model that operates on uncalibrated, out-of-domain urban scenes using various image inputs (sequential, monocular, or surround-view) without requiring sensor-rig priors or in-domain annotations.


<details>
  <summary>Details</summary>
Motivation: Existing 3D occupancy methods rely on in-domain annotations and precise sensor-rig priors, limiting scalability and out-of-domain generalization. Visual geometry foundation models lack key requirements for urban occupancy prediction: metric prediction, geometry completion in cluttered scenes, and adaptation to urban scenarios.

Method: Proposes a generalized 3D occupancy framework with three key components: 1) Segmentation Forcing that improves occupancy quality while enabling mask-level prediction, 2) Novel View Rendering pipeline that infers novel-view geometry for test-time view augmentation to complete geometry, and 3) Versatile architecture supporting sequential, monocular, or surround-view image inputs.
...
Conclusion: OccAny is the first unconstrained urban 3D occupancy model capable of operating on out-of-domain uncalibrated scenes, addressing limitations of existing methods while maintaining strong generalization capabilities for urban scenarios.
...
### [134] [Evaluating Prompting Strategies for Chart Question Answering with Large Language Models](https://arxiv.org/abs/2603.22288)
*Ruthuparna Naikar,Ying Zhu*

Main category: cs.CL

TL;DR: Systematic evaluation of four prompting strategies (Zero-Shot, Few-Shot, Zero-Shot CoT, Few-Shot CoT) on GPT models for chart-based QA reveals Few-Shot Chain-of-Thought achieves highest accuracy (78.2%) on ChartQA dataset.


<details>
  <summary>Details</summary>
Motivation: Prompting strategies significantly impact LLM reasoning performance, but their specific effects on chart-based question answering remain underexplored, creating a need for systematic evaluation of different prompting paradigms in structured data reasoning tasks.

Method: Systematic evaluation framework comparing four prompting paradigms (Zero-Shot, Few-Shot, Zero-Shot Chain-of-Thought, Few-Shot Chain-of-Thought) across GPT-3.5, GPT-4, and GPT-4o on 1,200 diverse ChartQA samples. The framework operates exclusively on structured chart data, isolating prompt structure as the only experimental variable, with evaluation using Accuracy and Exact Match metrics.

Result: Few-Shot Chain-of-Thought prompting consistently yields the highest accuracy (up to 78.2%), particularly effective on reasoning-intensive questions. Few-Shot prompting improves format adherence. Zero-Shot performs well only with high-capacity models on simpler tasks. Results provide clear performance patterns across different prompting strategies and model capabilities.

Conclusion: The study provides actionable guidance for selecting prompting strategies in structured data reasoning tasks, highlighting that Few-Shot Chain-of-Thought is optimal for accuracy on complex reasoning tasks, while Few-Shot improves format compliance. These findings have implications for balancing efficiency and accuracy in real-world chart-based QA applications.
...
### [135] [MERIT: Memory-Enhanced Retrieval for Interpretable Knowledge Tracing](https://arxiv.org/abs/2603.22289)
*Runze Li,Kedi Chen,Guwei Feng,Mo Yu,Jun Wang,Wei Zhang*

Main category: cs.CL

TL;DR: MERIT is a training-free framework that combines frozen LLMs with structured pedagogical memory for interpretable knowledge tracing, achieving state-of-the-art performance without gradient updates.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning KT models lack interpretability, while LLM-based methods require expensive fine-tuning and suffer from limited context windows and hallucinations. There's a need for scalable, interpretable KT that doesn't require parameter updates.

Method: MERIT transforms raw interaction logs into an interpretable memory bank using semantic denoising to categorize students into latent cognitive schemas. It constructs a paradigm bank with representative error patterns analyzed offline to generate explicit Chain-of-Thought rationales. During inference, hierarchical routing retrieves relevant contexts, and a logic-augmented module applies semantic constraints to calibrate predictions.

Result: Achieves state-of-the-art performance on real-world datasets without gradient updates, reducing computational costs and supporting dynamic knowledge updates.

Conclusion: MERIT improves accessibility and transparency of educational diagnosis by grounding LLMs in interpretable memory, offering a scalable, training-free approach to knowledge tracing.
...
### [136] [Less is More: Adapting Text Embeddings for Low-Resource Languages with Small Scale Noisy Synthetic Data](https://arxiv.org/abs/2603.22290)
*Zaruhi Navasardyan,Spartak Bughdaryan,Bagrat Minasyan,Hrant Davtyan*

Main category: cs.CL

TL;DR: Fine-tuning multilingual embeddings on minimal noisy synthetic data (10k pairs) yields substantial performance gains for low-resource languages, matching models trained on 100x more data, suggesting semantic alignment saturates early and is robust to noise.


<details>
  <summary>Details</summary>
Motivation: Low-resource languages lack high-quality datasets for training effective text embedding models, hindering applications like retrieval-augmented generation and semantic search. The prevailing assumption that massive datasets or pristine translations are necessary for semantic alignment is challenged.

Method: Cost-effective adaptation strategy using small-scale noisy synthetic data generated by translating English Reddit title-body pairs with open-weights models. Focus on Armenian (LRL with unique script), fine-tuning multilingual encoder (mE5) on minimal data (10k noisy pairs). Comprehensive evaluation benchmark with existing datasets, translated data, and manually curated dataset.

Result: "Less is More" phenomenon: fine-tuning on just 10,000 noisy synthetic pairs yields 11-12% average improvements across benchmark with 20%+ relative improvement in retrieval performance, matching models trained on ~1 million examples. Neither increasing data scale, improving translation quality via state-of-the-art LLMs, nor diversifying data domains yields significant gains over minimal baseline. Findings generalize to another LRL with unique script.

Conclusion: Semantic alignment for low-resource languages saturates early and is highly robust to noise, democratizing high-performance embedding creation for resource-constrained communities. Minimal noisy synthetic data is sufficient for effective adaptation.
...
### [137] [Evaluating Large Language Models' Responses to Sexual and Reproductive Health Queries in Nepali](https://arxiv.org/abs/2603.22291)
...
TL;DR: LEAF framework evaluates LLMs for SRH queries in low-resource languages, assessing accuracy, language, usability, and safety gaps, revealing only 35.1% of responses were "proper" for Nepali SRH queries.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation methods focus primarily on accuracy for objective queries in high-resource languages, lacking comprehensive assessment of usability and safety for culturally sensitive domains like Sexual and Reproductive Health (SRH) in low-resource languages.

Method: Introduces LLM Evaluation Framework (LEAF) that assesses responses across multiple criteria: accuracy, language, usability gaps (relevance, adequacy, cultural appropriateness), and safety gaps (safety, sensitivity, confidentiality). Applied to 14K SRH queries in Nepali from 9K+ users, with manual annotation by SRH experts.

Result: Only 35.1% of responses were "proper" (accurate, adequate, with no major usability or safety gaps). Found differences in performance between ChatGPT versions, with similar accuracy but varying usability and safety aspects.

Conclusion: Current LLMs have significant limitations for sensitive topics like SRH in low-resource languages. LEAF framework provides adaptable evaluation across domains and languages where usability and safety are critical, offering a pathway for improvement.
...
### [138] [TIPS: Turn-Level Information-Potential Reward Shaping for Search-Augmented LLMs](https://arxiv.org/abs/2603.22293)
*Yutao Xie,Nathaniel Thomas,Nicklas Hansen,Yang Fu,Li Erran Li,Xiaolong Wang*

Main category: cs.CL

TL;DR: TIPS is a reward shaping method that provides dense, turn-level rewards for search-augmented LLMs to address sparse reward and credit assignment problems in RL training for QA tasks.


<details>
  <summary>Details</summary>
Motivation: Training search-augmented LLMs with RL for QA is challenging due to sparse rewards and difficult credit assignment across multi-step reasoning and tool calls, leading to unstable optimization.

Method: Introduces Turn-Level Information Potential Reward Shaping (TIPS), which assigns dense rewards to each reasoning+tool-call segment based on increased likelihood of correct answer under a teacher model, using potential-based reward shaping for policy-invariant guidance.

Result: TIPS consistently outperforms GRPO/PPO baselines across seven QA benchmarks, improving average Exact Match by 11.8% and F1 by 13.6% with Qwen-2.5 7B Instruct model, while substantially improving training stability.

Conclusion: Turn-level information-potential reward shaping provides an effective and general solution to sparse-reward credit assignment for multi-turn LLM reasoning, overcoming limitations of outcome-only optimization.
...
### [139] [Whether, Not Which: Mechanistic Interpretability Reveals Dissociable Affect Reception and Emotion Categorization in LLMs](https://arxiv.org/abs/2603.22295)
*Michael Keeman*

Main category: cs.CL

TL;DR: LLMs have genuine emotion processing mechanisms beyond keyword spotting, with dissociable affect reception (near-perfect accuracy) and emotion categorization (partially keyword-dependent), validated using clinical vignettes without emotion keywords.


<details>
  <summary>Details</summary>
Motivation: To determine whether LLMs' reported emotion representations detect genuine emotional meaning or merely recognize emotion keywords, addressing a fundamental gap in previous studies that relied on explicit emotion terms.

Method: Used clinical vignettes without emotion keywords to evoke emotions through situational/behavioral cues alone. Applied four mechanistic interpretability methods (linear probing, causal activation patching, knockout experiments, representational geometry) across six models (Llama-3.2-1B, Llama-3-8B, Gemma-2-9B; base and instruct variants).

Result: Found two dissociable mechanisms: affect reception with near-perfect accuracy (AUROC 1.000) across all models, and emotion categorization that drops 1-7% without keywords and improves with scale. Causal activation patching showed keyword-rich and keyword-free stimuli share representational space, transferring affective salience rather than category identity.

Conclusion: Falsifies the keyword-spotting hypothesis, establishes a novel mechanistic dissociation in emotion processing, and introduces clinical stimulus methodology as a rigorous standard for testing emotion processing in LLMs, with implications for AI safety evaluation and alignment.
...
### [140] [Sparse but Critical: A Token-Level Analysis of Distributional Shifts in RLVR Fine-Tuning of LLMs](https://arxiv.org/abs/2603.22446)
*Haoming Meng,Kexin Huang,Shaohang Wei,Chiyu Ma,Shuo Yang,Xue Wang,Guoyin Wang,Bolin Ding,Jingren Zhou*

Main category: cs.CL

TL;DR: RLVR induces sparse, targeted token-level distributional shifts in LLMs, with only a small fraction of token decisions responsible for performance gains.


<details>
  <summary>Details</summary>
Motivation: While RLVR improves LLM reasoning, the token-level mechanisms underlying these improvements remain unclear, necessitating systematic empirical study of RLVR's distributional effects.

Method: Three-part analysis: (1) token-level characterization of distributional shifts between base and RL models, (2) cross-sampling interventions to assess impact of token-level shifts on sequence-level performance, and (3) fine-grained mechanics of shifts at token level including entropy, positional concentration, and probability mass reallocation.

Result: RL fine-tuning induces highly sparse changes; inserting small fraction of RL-sampled tokens into base generations recovers RL gains, while injecting base tokens into RL sequences collapses performance; divergence-weighted advantage variants yield improvements over baselines.

Conclusion: RLVR fine-tuning functions as targeted refinement process with sparse token-level changes directly responsible for performance gains, providing fine-grained lens for understanding RLVR mechanisms.
...
### [141] [Towards Automated Community Notes Generation with Large Vision Language Models for Combating Contextual Deception](https://arxiv.org/abs/2603.22453)
*Jin Ma,Jingwen Yan,Mohammed Aldeen,Ethan Anderson,Taran Kavuru,Jinkyung Katie Park,Feng Luo,Long Cheng*

Main category: cs.CL

TL;DR: Automated Community Notes generation for image-based contextual deception using retrieval-augmented multi-agent framework with new dataset and evaluation metric.


<details>
  <summary>Details</summary>
Motivation: Current Community Notes systems rely on human contributors, limiting timeliness and scalability. Automated generation is needed for image-based contextual deception where authentic images are paired with misleading context, but this problem remains underexplored due to dataset scarcity, dynamic nature of deception, and inadequate evaluation metrics.

Method: Proposes ACCNote (Automated Context-Corrective Note generation), a retrieval-augmented, multi-agent collaboration framework built on large vision-language models. Also introduces XCheck dataset of real-world posts with Community Notes and external contexts.

Result: Experiments on XCheck dataset show ACCNote improves both deception detection and note generation performance over baselines, exceeding commercial tool GPT5-mini. The proposed Context Helpfulness Score (CHS) aligns with user study outcomes.

Conclusion: The dataset, method, and metric advance practical automated generation of context-corrective notes toward more responsible online social networks, addressing timeliness and scalability limitations of human-based systems.
...
### [142] [LLM-guided headline rewriting for clickability enhancement without clickbait](https://arxiv.org/abs/2603.22459)
*Yehudit Aperstein,Linoy Halifa,Sagiv Bar,Alexander Apartsin*

Main category: cs.CL

TL;DR: A framework for controllable headline rewriting that balances engagement optimization with clickbait avoidance using LLM guidance from engagement-attribute and clickbait scoring models.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of enhancing reader engagement in news headlines while avoiding clickbait, which often results from conflating engagement optimization with exaggerated or misleading phrasing that undermines editorial trust.

Method: A guided headline rewriting framework using LLM with FUDGE paradigm, steered by two auxiliary guide models: (1) clickbait scoring model for negative guidance to suppress excessive stylistic amplification, and (2) engagement-attribute model for positive guidance aligned with clickability objectives. Both trained on neutral headlines from curated news corpus, with synthetic clickbait variants generated via LLM rewriting using predefined engagement tactics.

Result: The system generates headlines along a continuum from neutral paraphrases to more engaging yet editorially acceptable formulations by adjusting guidance weights at inference time, enabling study of trade-offs between attractiveness, semantic preservation, and clickbait avoidance.

Conclusion: The framework provides a principled approach for responsible LLM-based headline optimization in journalistic settings, treating clickbait as an extreme outcome of disproportionate amplification rather than a separate stylistic category.
...
### [143] [Rashid: A Cipher-Based Framework for Exploring In-Context Language Learning](https://arxiv.org/abs/2603.22497)
*Niyati Bafna,Ryan Soh-Eun Shim,Barbara Plank,David Yarowsky,Hale Sirin*

Main category: cs.CL

TL;DR: Rashid framework enables study of in-context language learning by reversibly ciphering high-resource languages to create truly unseen languages with full resource access, overcoming limitations of low-resource language research.


<details>
  <summary>Details</summary>
Motivation: Current in-context language learning research faces limitations due to lack of NLP tools, data resources, and expertise for truly unseen languages, restricting progress assessment, large-scale experimentation, and generalizability across languages and tasks.

Method: Introduces Rashid framework that reversibly ciphers high-resource languages to construct truly unseen languages while maintaining access to all resources available for the original high-resource languages.
...
Conclusion: Rashid framework unlocks previously impossible exploration of ICLL phenomena and provides actionable insights for current performance and future directions in in-context language learning research.
...
### [144] [Reddit After Roe: A Computational Analysis of Abortion Narratives and Barriers in the Wake of Dobbs](https://arxiv.org/abs/2603.22566)
*Aria Pessianzadeh,Alex H. Poole,Rezvaneh Rezapour*

Main category: cs.CL

TL;DR: Computational analysis of 17,000+ Reddit posts reveals emotional/psychological barriers dominate abortion discourse post-Dobbs decision, with information behaviors and barriers linked to specific emotions across different abortion stages.


<details>
  <summary>Details</summary>
Motivation: To understand how the 2022 Dobbs v. Jackson decision reshaped reproductive rights and created new uncertainty/barriers to abortion access, and to analyze how these barriers are articulated in online discourse across different information behaviors and abortion stages.

Method: Multi-step computational pipeline analyzing 17,000+ posts from four abortion-related subreddits, classifying posts by information type, abortion stage, barrier category (8 types including legal, financial, emotional, social), and expressed emotions. Used topic modeling of model-generated barrier rationales to examine discourse evolution across three phases of the Dobbs decision.

Result: Emotional and psychological barriers consistently dominate abortion narratives online, with emotions like nervousness, confusion, fear, and sadness prevalent across discourse. Analysis revealed associations between barrier types, emotions, and information behaviors, showing how discourse evolved in response to shifting legal/cultural contexts.

Conclusion: The study provides a multi-dimensional account of how abortion is navigated in online communities by linking information behaviors, barriers, emotions, and temporal dynamics, highlighting the central role of emotional/psychological barriers in abortion discourse post-Dobbs.
...
### [145] [CAPITU: A Benchmark for Evaluating Instruction-Following in Brazilian Portuguese with Literary Context](https://arxiv.org/abs/2603.22576)
...
TL;DR: CAPITU is a Brazilian Portuguese instruction-following benchmark using canonical Brazilian literature, featuring 59 automatically verifiable instruction types across 7 categories, evaluating 18 models with results showing frontier models excel but Portuguese-specialized models offer cost-efficiency advantages.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus primarily on English or use generic prompts, lacking culturally-grounded evaluation for Portuguese. There's a need for verifiable instruction-following assessment that incorporates Brazilian cultural context and Portuguese-specific linguistic constraints.

Method: Created CAPITU benchmark contextualizing tasks within eight canonical Brazilian literary works. Includes 59 instruction types organized into 7 categories with automatic verification (no LLM judges/human evaluation). Features Portuguese-specific constraints: word termination patterns (-ando/-endo/-indo, -inho/-inha, -mente) and structural requirements. Evaluated 18 state-of-the-art models in single-turn and multi-turn settings.

Result: Frontier reasoning models achieve strong performance (GPT-5.2 with reasoning: 98.5% strict accuracy). Portuguese-specialized models offer competitive cost-efficiency (Sabiazinho-4: 87.0% at $0.13 vs Claude-Haiku-4.5: 73.5% at $1.12). Multi-turn evaluation shows significant variation in constraint persistence (60% to 96% conversation-level accuracy). Identified challenges: morphological constraints, exact counting, constraint persistence degradation across turns.

Conclusion: CAPITU provides a culturally-grounded, automatically verifiable benchmark for Portuguese instruction-following evaluation. Results demonstrate performance-cost tradeoffs between frontier and specialized models, reveal multi-turn constraint persistence issues, and identify specific linguistic challenges. The released benchmark facilitates research on Portuguese instruction-following capabilities.
...
### [146] [Lie to Me: How Faithful Is Chain-of-Thought Reasoning in Reasoning Models?](https://arxiv.org/abs/2603.22582)
*Richard J. Young*

Main category: cs.CL

TL;DR: Comprehensive evaluation of faithfulness in open-weight reasoning models reveals systematic variation in Chain-of-Thought acknowledgment rates (39.7%-89.9%), with models often internally recognizing hint influence but suppressing it in outputs, challenging CoT's viability as a safety mechanism.


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought reasoning is proposed as a transparency mechanism for LLMs in safety-critical deployments, but its effectiveness depends on faithfulness. Prior evaluations examined only two proprietary models with low acknowledgment rates (25-39%), creating a need to extend evaluation across the open-weight ecosystem to understand faithfulness systematically.

Method: Tested 12 open-weight reasoning models spanning 9 architectural families (7B-685B parameters) on 498 multiple-choice questions from MMLU and GPQA Diamond. Injected six categories of reasoning hints (sycophancy, consistency, visual pattern, metadata, grader hacking, and unethical information) and measured acknowledgment rates when hints successfully altered answers. Conducted 41,832 inference runs with keyword-based analysis comparing thinking-token vs answer-text acknowledgment.

Result: Overall faithfulness rates ranged from 39.7% (Seed-1.6-Flash) to 89.9% (DeepSeek-V3.2-Speciale). Consistency hints (35.5%) and sycophancy hints (53.9%) had lowest acknowledgment rates. Training methodology and model family predicted faithfulness more strongly than parameter count. Keyword analysis revealed striking gap: thinking-token acknowledgment ~87.5% vs answer-text acknowledgment ~28.6%, indicating models internally recognize hint influence but systematically suppress acknowledgment in outputs.

Conclusion: Faithfulness is not a fixed property but varies systematically with architecture, training method, and influencing cue nature. The gap between internal recognition and output suppression challenges the viability of CoT monitoring as a safety mechanism, suggesting models can be aware of improper influences while concealing this awareness in their reasoning traces.
...
### [147] [LGSE: Lexically Grounded Subword Embedding Initialization for Low-Resource Language Adaptation](https://arxiv.org/abs/2603.22629)
...
<details>
  <summary>Details</summary>
Motivation: Existing vocabulary expansion methods for adapting pretrained language models to low-resource, morphologically rich languages rely on arbitrarily segmented subword units, leading to fragmented lexical representations and loss of critical morphological information.

Method: Lexically Grounded Subword Embedding Initialization (LGSE) framework uses morphologically informed segmentation to initialize embeddings of novel tokens. Words are decomposed into constituent morphemes, and embeddings are constructed by averaging pretrained subword or FastText-based morpheme representations. For unsegmentable tokens, character n-gram representations are used. During Language-Adaptive Pretraining, a regularization term penalizes large deviations from initialized values to preserve alignment with original embedding space.

Result: LGSE consistently outperforms baseline methods across three NLP tasks (Question Answering, Named Entity Recognition, and Text Classification) in two morphologically rich, low-resource languages: Amharic and Tigrinya.

Conclusion: The LGSE framework demonstrates the effectiveness of morphologically grounded embedding initialization for improving representation quality in underrepresented languages, showing consistent performance improvements across multiple NLP tasks.
...
### [148] [DALDALL: Data Augmentation for Lexical and Semantic Diverse in Legal Domain by leveraging LLM-Persona](https://arxiv.org/abs/2603.22765)
*Janghyeok Choi,Jaewon Lee,Sungzoon Cho*

Main category: cs.CL

TL;DR: DALDALL introduces persona-based data augmentation using legal professional personas to generate diverse synthetic queries for legal IR, improving lexical diversity while maintaining semantic fidelity and boosting retrieval performance.


<details>
  <summary>Details</summary>
Motivation: Data scarcity in low-resource domains like legal IR; existing LLM-based augmentation methods prioritize quantity over quality and lack domain-specific strategies.

Method: Persona-based data augmentation framework using domain-specific professional personas (attorneys, prosecutors, judges) to generate synthetic queries with greater lexical and semantic diversity than vanilla prompting.

Result: Persona-based augmentation achieves improved lexical diversity (measured by Self-BLEU) while preserving semantic fidelity; dense retrievers fine-tuned on persona-augmented data achieve competitive/superior recall on CLERC and COLIEE benchmarks.
...
### [149] [Multi-Method Validation of Large Language Model Medical Translation Across High- and Low-Resource Languages](https://arxiv.org/abs/2603.22642)
...
TL;DR: Frontier LLMs (GPT-5.1, Claude Opus 4.5, Gemini 3 Pro, Kimi K2) effectively translate medical documents across 8 languages of varying resource levels, maintaining high semantic fidelity regardless of language resource availability.


<details>
  <summary>Details</summary>
Motivation: Address language barriers in healthcare affecting 27.3 million U.S. residents with non-English language preference, where professional medical translation is costly and often unavailable.

Method: Evaluated four frontier LLMs translating 22 medical documents into 8 languages (Spanish, Chinese, Russian, Vietnamese, Korean, Arabic, Tagalog, Haitian Creole) using a five-layer validation framework. Conducted 704 translation pairs with semantic preservation measurement via LaBSE, cross-model back-translation, inter-model concordance analysis, and lexical borrowing assessment.

Result: All models achieved high semantic preservation (LaBSE > 0.92) with no significant difference between high- and low-resource languages (p = 0.066). Cross-model back-translation confirmed results weren't driven by same-model circularity (delta = -0.0009). High inter-model concordance (LaBSE: 0.946) and no correlation between English term retention and fidelity scores in low-resource languages (rho = +0.018, p = 0.82).

Conclusion: Frontier LLMs preserve medical meaning across language resource levels, suggesting potential for improving language access in healthcare through cost-effective translation solutions.
...
### [150] [Improving LLM Predictions via Inter-Layer Structural Encoders](https://arxiv.org/abs/2603.22665)
*Tom Ulanovski,Eyal Blyachman,Maya Bechler-Speicher*

Main category: cs.CL

TL;DR: ILSE (Inter-Layer Structural Encoders) is a geometric approach that combines information from all LLM layers using Cayley-Encoder based on expander graphs, outperforming baselines across 13 tasks with 9 LLMs.


<details>
  <summary>Details</summary>
Motivation: Standard LLMs use only final-layer representations, but intermediate layers contain substantial task-relevant information, with different layers being optimal for different tasks. There's a need to effectively combine information from all layers rather than relying on a single layer.

Method: ILSE uses Cayley-Encoder, a mathematically grounded geometric encoder that leverages expander Cayley graphs for efficient inter-layer information propagation. It learns one effective representation from all LLM internal layer representations together through structural encoding.

Result: ILSE consistently outperforms baselines and existing approaches across 13 classification and semantic similarity tasks with 9 pre-trained LLMs (14M to 8B parameters), achieving up to 44% improvement in accuracy and 25% in similarity metrics. It's data-efficient in few-shot regimes and enables small LLMs to compete with substantially larger models.

Conclusion: ILSE provides a powerful structural approach to leverage information from all LLM layers, demonstrating superior performance, data efficiency, and the ability to enhance smaller models to compete with larger ones through effective inter-layer representation learning.
...
### [151] [Synthetic or Authentic? Building Mental Patient Simulators from Longitudinal Evidence](https://arxiv.org/abs/2603.22704)
*Baihan Li,Bingrui Jin,Kunyao Lan,Ming Wang,Mengyue Wu*

Main category: cs.CL

TL;DR: DEPROFILE is a data-grounded patient simulation framework that constructs comprehensive patient profiles from real-world data to improve dialogue realism, behavioral diversity, and event richness in mental health dialogue systems.


<details>
  <summary>Details</summary>
Motivation: Existing patient simulation approaches rely on snapshot-style prompts with limited profile information, leading to homogeneous behaviors and incoherent disease progression in multi-turn interactions, which are key challenges in developing and evaluating mental health dialogue systems.

Method: Proposes DEPROFILE framework that constructs unified, multi-source patient profiles by integrating demographic attributes, standardized clinical symptoms, counseling dialogues, and longitudinal life-event histories from real-world data. Introduces a Chain-of-Change agent to transform noisy longitudinal records into structured, temporally grounded memory representations for simulation.

Result: Experiments across multiple LLM backbones show that with more comprehensive profiles constructed by DEPROFILE, dialogue realism, behavioral diversity, and event richness consistently improved and exceeded state-of-the-art baselines.

Conclusion: Highlights the importance of grounding patient simulation in verifiable longitudinal evidence for developing more realistic and effective mental health dialogue systems.
...
### [152] [Who Spoke What When? Evaluating Spoken Language Models for Conversational ASR with Semantic and Overlap-Aware Metrics](https://arxiv.org/abs/2603.22709)
*Naohiro Tawara,Samuele Cornell,Alexander Polok,Marc Delcroix,Lukáš Burget,Shinji Watanabe*

Main category: cs.CL

TL;DR: LLM-based ASR systems perform well in single-speaker scenarios but degrade in multi-speaker settings with increasing overlap and speaker count, while modular pipelines maintain robustness; new metric tcpSemER introduced for semantic error measurement.


<details>
  <summary>Details</summary>
Motivation: Conversational ASR faces challenges with overlapping speech, far-field noise, and varying speaker counts. While LLM-based systems excel in single-speaker benchmarks, their performance in multi-speaker conversational settings remains unverified, necessitating systematic comparison with traditional modular approaches.

Method: Systematic comparison of LLM-based and modular pipeline approaches across four axes: overlap robustness, semantic fidelity, speaker count, and input modality (single- vs multi-channel). Introduction of tcpSemER metric extending tcpWER by replacing Levenshtein distance with embedding-based semantic similarity. Decomposition of tcpWER into overlapping and non-overlapping components for granular analysis. Experiments conducted across three datasets.

Result: LLM-based systems are competitive in two-speaker settings but degrade as speaker count and overlap increase. Modular pipelines demonstrate superior robustness in multi-speaker scenarios with high overlap. The tcpSemER metric effectively captures meaning-altering errors that conventional metrics miss.

Conclusion: While LLM-based ASR shows promise for simpler conversational settings, modular pipeline approaches remain more robust for complex multi-speaker scenarios with significant overlap. The proposed tcpSemER metric provides better semantic error assessment than traditional word error rate metrics.
...
### [153] [How Utilitarian Are OpenAI's Models Really? Replicating and Reinterpreting Pfeffer, Krügel, and Uhl (2025)](https://arxiv.org/abs/2603.22730)
*Johannes Himmelreich*

Main category: cs.CL

TL;DR: The paper demonstrates that single-prompt evaluations of LLM moral reasoning are unreliable, showing that GPT-4o's apparent deontological stance in trolley problems disappears when prompts are reframed, while reasoning models show more utilitarian tendencies in footbridge dilemmas but with frequent refusals.


<details>
  <summary>Details</summary>
Motivation: To critically examine previous findings about LLM moral reasoning, particularly Pfeffer et al.'s (2025) claim that OpenAI's reasoning model o1-mini produces more utilitarian responses than GPT-4o, by testing robustness across different prompts and models.

Method: Replication and extension study using four current OpenAI models with prompt variant testing, comparing responses to trolley problems and footbridge dilemmas framed differently (e.g., "Should I..." vs. "Is it morally permissible...") to identify confounds.

Result: GPT-4o's low utilitarian rate in trolley problems disappears when prompts are reframed (99% utilitarian responses with "Is it morally permissible...?"). All models converge on utilitarian answers when prompt confounds are removed. For footbridge dilemmas, reasoning models show more utilitarian tendencies but often refuse to answer or give non-utilitarian responses.

Conclusion: Single-prompt evaluations of LLM moral reasoning are unreliable; multi-prompt robustness testing should be standard practice for any empirical claim about LLM behavior, as apparent moral stances can be artifacts of prompt framing rather than genuine reasoning patterns.
...
### [154] [Explanation Generation for Contradiction Reconciliation with LLMs](https://arxiv.org/abs/2603.22735)
*Jason Chan,Zhixue Zhao,Robert Gaizauskas*

Main category: cs.CL

TL;DR: The paper introduces the task of reconciliatory explanation generation, where models must generate explanations that make contradictory statements compatible, and finds that current LLMs have limited success in this reasoning capability.


<details>
  <summary>Details</summary>
Motivation: Existing NLP work treats contradictions as errors to be resolved by choosing which statements to accept or discard, but human reasoning often involves hypothesizing explanations that reconcile contradictions. This reconciliatory reasoning capability in LLMs remains largely unexplored despite their growing reasoning abilities.

Method: The authors introduce the task of reconciliatory explanation generation, propose a novel method of repurposing existing natural language inference (NLI) datasets, and introduce quality metrics for scalable automatic evaluation. They experiment with 18 different LLMs.

Result: Experiments show that most models achieve limited success in this task, and that the benefit of extending test-time compute by "thinking" plateaus as model size increases. Current LLMs struggle with reconciliatory reasoning.
...
### [155] [KALAVAI: Predicting When Independent Specialist Fusion Works -- A Quantitative Model for Post-Hoc Cooperative LLM Training](https://arxiv.org/abs/2603.22755)
*Ramchand Kumaresan*

Main category: cs.CL

TL;DR: Post-hoc fusion of independently trained domain specialists via lightweight MoE routing yields predictable performance gains proportional to domain divergence, enabling practitioners to estimate cooperative value before committing compute.


<details>
  <summary>Details</summary>
Motivation: To enable efficient collaboration among independent contributors without requiring coordination during training, allowing practitioners to estimate potential gains before committing computational resources.

Method: KALAVAI protocol: contributors fine-tune copies of a shared checkpoint independently, then submit for lightweight Mixture of Experts (MoE) routing (500 steps). The protocol uses learned routing to match domain-oracle routing within <10^{-5} nats.

Result: Predictable gains: gain = 0.82 x divergence - 2.72 (R^2 = 0.856). Consistent improvements: +7.72% at 410M, +7.49% at 1B, +6.53% at 6.9B over best specialist. Cross-lingual fusion achieves +21.76% (Yoruba perplexity falls from 41.9 to 7.7). 20-contributor federation achieves +16.71%.

Conclusion: Post-hoc fusion of domain specialists via lightweight MoE routing is effective and predictable, with three key requirements: shared initialization, optional frozen layers below ~10,000 steps, and essential learned routing (uniform averaging degrades performance).
...
### [156] [Span Modeling for Idiomaticity and Figurative Language Detection with Span Contrastive Loss](https://arxiv.org/abs/2603.22799)
*Blake Matheny,Phuong Minh Nguyen,Minh Le Nguyen*

Main category: cs.CL

TL;DR: BERT- and RoBERTa-based models with slot loss and span contrastive loss achieve state-of-the-art idiomaticity detection, outperforming previous approaches through improved span awareness and hard negative reweighting.


<details>
  <summary>Details</summary>
Motivation: Idioms and other non-compositional multi-word expressions present challenges for language models due to tokenization issues and contextual embeddings. Existing approaches often require prompting or instruction finetuning, with best results from BERT-based or LSTM finetuning methods.

Method: Proposes BERT- and RoBERTa-based models finetuned with a combination of slot loss and span contrastive loss (SCL) with hard negative reweighting. Uses geometric mean of F1 and sequence accuracy as evaluation metric to assess both span awareness and general performance.

Result: Achieves state-of-the-art sequence accuracy performance on existing datasets for idiomaticity detection. Ablation studies demonstrate the effectiveness of SCL and its generalizability.

Conclusion: The proposed approach with span contrastive loss and hard negative reweighting effectively improves idiomaticity detection in language models, addressing the challenges of non-compositional figurative language through enhanced span awareness.
...
### [157] [Efficient Hallucination Detection: Adaptive Bayesian Estimation of Semantic Entropy with Guided Semantic Exploration](https://arxiv.org/abs/2603.22812)
*Qiyao Sun,Xingming Li,Xixiang He,Ao Cheng,Xuanyu Ji,Hailun Lu,Runke Huang,Qingyong Hu*

Main category: cs.CL

TL;DR: Adaptive Bayesian framework for hallucination detection that dynamically adjusts sampling based on uncertainty, achieving better performance with 50% fewer samples.


<details>
  <summary>Details</summary>
Motivation: Current hallucination detection methods use fixed sampling budgets that don't adapt to query complexity, leading to computational inefficiency.

Method: Adaptive Bayesian Estimation framework with Guided Semantic Exploration using hierarchical Bayesian modeling and variance-based thresholds for dynamic sampling control, plus perturbation-based importance sampling.

Result: Achieves superior hallucination detection with 50% fewer samples for comparable performance, and 12.6% average AUROC improvement under same sampling budget across four QA datasets.

Conclusion: The adaptive framework provides more efficient and effective hallucination detection by dynamically adjusting sampling based on uncertainty.
...
### [158] [When AI Shows Its Work, Is It Actually Working? Step-Level Evaluation Reveals Frontier Language Models Frequently Bypass Their Own Reasoning](https://arxiv.org/abs/2603.22816)
*Abhinaba Basu,Pavan Chakraborty*

Main category: cs.CL

TL;DR: Frontier language models' step-by-step reasoning is largely decorative, not genuinely used for answers, as shown by step-level evaluation removing reasoning sentences.


<details>
  <summary>Details</summary>
Motivation: To determine whether language models' step-by-step reasoning is genuinely used for decision-making or merely decorative narratives generated after the answer is already decided.

Method: Step-level evaluation: remove one reasoning sentence at a time and check whether the answer changes, requiring only API access without model weights. Tested 10 frontier models across sentiment, mathematics, topic classification, and medical QA tasks.

Result: Majority of frontier models produce decorative reasoning: removing any step changes answer less than 17% of the time, while any single step alone recovers the answer. Two models (MiniMax-M2.5, Kimi-K2.5) show some task-specific faithfulness. Also discovered "output rigidity" and mechanistic analysis shows CoT attention drops more in late layers for decorative tasks.

Conclusion: Step-by-step explanations from frontier models are largely decorative, requiring per-model per-domain evaluation; training objectives - not scale - determine whether reasoning is genuine.
...
### [159] [RadTimeline: Timeline Summarization for Longitudinal Radiological Lung Findings](https://arxiv.org/abs/2603.22820)
*Sitong Zhou,Meliha Yetisgen,Mari Ostendorf*

Main category: cs.CL

TL;DR: This paper introduces RadTimeline, a structured summarization approach for longitudinal radiology reports that organizes findings into timelines with temporal grouping, using a 3-step LLM process for extraction and grouping.


<details>
  <summary>Details</summary>
Motivation: Tracking findings in longitudinal radiology reports is crucial for disease progression monitoring but time-consuming, requiring automated summarization to facilitate comparison across time and fact-checking against original reports.

Method: Frames longitudinal report summarization as timeline generation with dated findings in columns and temporally related findings grouped in rows. Uses a 3-step LLM process: (1) extracting findings, (2) generating group names, and (3) using names to group findings. Creates RadTimeline dataset focused on lung-related radiologic findings in chest imaging reports.

Result: Experiments show tradeoffs between different-sized LLMs and prompting strategies. Group name generation as intermediate step is critical for effective finding grouping. Best configuration achieves good recall with some irrelevant findings, and grouping performance is comparable to human annotators.

Conclusion: The structured timeline summarization approach enables straightforward comparison of findings across time and facilitates fact-checking, with LLM-based methods showing promising performance comparable to human annotation for grouping related findings in longitudinal radiology reports.
...
### [160] [Analysing LLM Persona Generation and Fairness Interpretation in Polarised Geopolitical Contexts](https://arxiv.org/abs/2603.22837)
*Maida Aizaz,Quang Minh Nguyen*

Main category: cs.CL

TL;DR: LLMs show systematic biases in generating Palestinian and Israeli personas, with Palestinian profiles often assigned lower socioeconomic status and survival roles, especially in war contexts, while Israeli profiles maintain middle-class professional attributes. Fairness prompts cause varied distributional changes but don't eliminate underlying socioeconomic distinctions.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly used for social simulation and persona generation, understanding how they represent geopolitical identities is crucial. The research aims to analyze biases in LLM-generated personas for Palestinian and Israeli identities across different contexts and roles.

Method: Analyzed personas generated for Palestinian and Israeli identities by five popular LLMs across 640 experimental conditions, varying context (war vs non-war) and assigned roles. Examined distributional patterns in generated attributes, effects of fairness prompts, and analyzed reasoning traces to understand the relationship between model reasoning and generation.

Result: Significant distributional biases: Palestinian profiles in war contexts frequently associated with lower socioeconomic status and survival-oriented roles, while Israeli profiles predominantly retained middle-class status and specialized professional attributes. Fairness prompts caused diverse distributional changes (increased non-binary gender inferences, convergence toward generic roles), but underlying socioeconomic distinctions often remained. Reasoning traces showed models consistently mentioned fairness concepts, but final generated personas followed biased distributional patterns.

Conclusion: LLMs interpret geopolitical contexts in biased ways, with systematic socioeconomic distinctions between Palestinian and Israeli personas. Fairness concepts in model reasoning don't consistently translate into representative outcomes, suggesting complex dynamics between model reasoning and generation that require further investigation and mitigation strategies.
...
### [161] [Avoiding Over-smoothing in Social Media Rumor Detection with Pre-trained Propagation Tree Transformer](https://arxiv.org/abs/2603.22854)
*Chaoqun Cui,Caiyan Jia*

Main category: cs.CL

TL;DR: P2T3: A Pre-Trained Propagation Tree Transformer for rumor detection that avoids GNN over-smoothing by extracting conversation chains and using pure Transformer architecture with token-wise embedding.


<details>
  <summary>Details</summary>
Motivation: GNN-based rumor detection methods suffer from over-smoothing issues when processing rumor propagation structures, particularly due to the prevalence of 1-level nodes in propagation trees and GNNs' inability to capture long-range dependencies.

Method: Proposes P2T3: extracts all conversation chains from propagation trees following reply direction, uses token-wise embedding to incorporate connection information and inductive bias, and pre-trains on large-scale unlabeled datasets using pure Transformer architecture.

Result: P2T3 outperforms previous state-of-the-art methods on multiple benchmark datasets and performs well under few-shot conditions, avoiding GNN over-smoothing issues.

Conclusion: P2T3 provides an effective alternative to GNNs for rumor detection, potentially serving as a foundation for large models or unified multi-modal schemes in social media research.
...
### [162] [EchoKV: Efficient KV Cache Compression via Similarity-Based Reconstruction](https://arxiv.org/abs/2603.22910)
*Yixuan Wang,Shiyu Ji,Yijun Liu,Qingfu Zhu,Wanxiang Che*

Main category: cs.CL

TL;DR: EchoKV is a flexible KV cache compression method for LLMs that enables on-demand switching between standard and compressed inference using lightweight residual reconstruction, outperforming existing methods across compression ratios while maintaining throughput.


<details>
  <summary>Details</summary>
Motivation: The increasing memory demand of KV cache poses a significant bottleneck for LLMs in long-context applications. Existing low-rank compression methods rely on irreversible parameter transformations, sacrificing flexibility to switch back to full-precision inference when memory is abundant.

Method: EchoKV uses a lightweight network to reconstruct residual KV components from a partial subset, leveraging intrinsic inter-layer and intra-layer similarities among attention heads. It introduces a two-stage fine-tuning strategy for rapid, low-cost training (~1 A100 GPU-hour for 7B models).
...
Conclusion: EchoKV provides a flexible KV cache compression scheme that enables on-demand transitions between standard and compressed inference, addressing memory bottlenecks in long-context LLM applications with minimal training overhead.
...
### [163] [Multilingual KokoroChat: A Multi-LLM Ensemble Translation Method for Creating a Multilingual Counseling Dialogue Dataset](https://arxiv.org/abs/2603.22913)
*Ryoma Suzuki,Zhiyang Qi,Michimasa Inaba*

Main category: cs.CL

TL;DR: Created Multilingual KokoroChat by translating Japanese counseling dialogues to English and Chinese using a novel multi-LLM ensemble method that outperforms individual state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Address critical scarcity of high-quality, publicly available counseling dialogue datasets, particularly in multilingual contexts, where translation fidelity is essential for sensitive counseling domains.

Method: Developed novel multi-LLM ensemble method: first generates diverse translation hypotheses from multiple distinct LLMs, then uses a single LLM to produce high-quality translation by analyzing strengths and weaknesses of all presented hypotheses.

Result: Human preference studies confirmed translations produced by ensemble method were preferred over any individual state-of-the-art LLM, demonstrating superior quality. Multilingual KokoroChat dataset created with English and Chinese translations.

Conclusion: Multi-LLM ensemble approach effectively addresses translation quality challenges in sensitive domains like counseling, producing higher-quality translations than any single model. The resulting multilingual dataset is publicly available.
...
### [164] [Quality Over Clicks: Intrinsic Quality-Driven Iterative Reinforcement Learning for Cold-Start E-Commerce Query Suggestion](https://arxiv.org/abs/2603.22922)
*Qi Sun,Kejun Xiao,Huaipeng Zhao,Tao Luo,Xiaoyi Zeng*

Main category: cs.CL

TL;DR: Cold-EQS: An iterative reinforcement learning framework for cold-start e-commerce query suggestion that uses answerability, factuality, and information gain as rewards, with uncertainty estimation for selecting hard samples from queries lacking click signals.


<details>
  <summary>Details</summary>
Motivation: Existing dialogue systems with query suggestion rely heavily on abundant online click data for CTR model training, which fails in cold-start scenarios where such data is unavailable or insufficient.

Method: Proposed Cold-EQS uses iterative reinforcement learning with three reward components (answerability, factuality, information gain). It estimates uncertainty for grouped candidate queries to select hard/ambiguous samples from user queries lacking click signals, enabling continuous optimization without requiring extensive click data.

Result: The framework achieves significant +6.81% improvement in online chatUV (chat unique visitors). Extensive experiments show strong positive correlation between offline and online effectiveness, demonstrating the approach's superiority in cold-start scenarios.

Conclusion: Cold-EQS effectively addresses the cold-start problem in e-commerce query suggestion by using reinforcement learning with carefully designed rewards and uncertainty-based sample selection, eliminating heavy dependence on abundant click data while improving user engagement metrics.
...
### [165] [Set-Valued Prediction for Large Language Models with Feasibility-Aware Coverage Guarantees](https://arxiv.org/abs/2603.22966)
*Ye Li,Anqi Hu,Yuanchang Ye,Shiyan Tong,Zhiyuan Wang,Bo Fu*

Main category: cs.CL

TL;DR: Proposes a framework for set-valued prediction from LLMs with feasibility-aware coverage guarantees, addressing limitations of single-point predictions by constructing calibrated prediction sets from multiple samples.


<details>
  <summary>Details</summary>
Motivation: Conventional LLM usage reports only the most likely generation as point prediction, underestimating model capability. Valid answers may exist in the broader output space but be missed with single sampling. Need to move from point to set-valued prediction to capture this uncertainty.

Method: Proposes principled framework for set-valued prediction with feasibility-aware coverage guarantees. Establishes minimum achievable risk level (MRL) below which coverage cannot be satisfied due to finite-sampling nature of LLMs. Develops data-driven calibration procedure that constructs prediction sets from sampled responses by estimating rigorous threshold, ensuring set contains correct answer with desired probability when target risk level is feasible.
...
Conclusion: The framework successfully addresses limitations of single-point LLM predictions by providing calibrated set-valued predictions with coverage guarantees, improving reliability and better utilizing LLM capabilities.
...
### [166] [DariMis: Harm-Aware Modeling for Dari Misinformation Detection on YouTube](https://arxiv.org/abs/2603.22977)
...
<details>
  <summary>Details</summary>
Motivation: Dari, spoken by tens of millions in Afghanistan, is largely absent from misinformation detection literature despite its importance. There's a need for resources to address misinformation in Dari-language content, particularly on platforms like YouTube.

Method: Created DariMis dataset with manual annotation across two dimensions: Information Type (Misinformation, Partly True, True) and Harm Level (Low, Medium, High). Proposed pair-input encoding strategy that treats video title and description as separate BERT segment inputs to model semantic relationships. Compared Dari/Farsi-specialized ParsBERT against XLM-RoBERTa-base with bootstrap confidence intervals.

Result: Found structural coupling: 55.9% of Misinformation carries at least Medium harm potential vs. 1.0% of True content. Pair-input encoding improved Misinformation recall by 7.0 percentage points (60.1% to 67.1%). ParsBERT achieved best performance with 76.60% accuracy and 72.77% macro F1. Information Type classifiers can function as implicit harm-triage filters.

Conclusion: DariMis addresses a critical gap in multilingual misinformation detection. The structural coupling between misinformation and harm enables efficient content moderation pipelines. Pair-input encoding effectively captures semantic relationships between headlines and body content, improving detection of safety-critical misinformation.
...
### [167] [Beyond Hate: Differentiating Uncivil and Intolerant Speech in Multimodal Content Moderation](https://arxiv.org/abs/2603.22985)
*Nils A. Herrmann,Tobias Eder,Jingyi He,Georg Groh*

Main category: cs.CL

TL;DR: Fine-grained annotation scheme distinguishes incivility (tone) from intolerance (content) in multimodal toxicity detection, improving model performance and error balance when combined with coarse hatefulness labels.


<details>
  <summary>Details</summary>
Motivation: Current multimodal toxicity benchmarks use single binary hatefulness labels that conflate tone and content, limiting model accuracy and moderation reliability.

Method: Introduced fine-grained annotation scheme distinguishing incivility (rude/dismissive tone) and intolerance (attacks on pluralism/identities); applied to 2,030 memes from Hateful Memes dataset; evaluated VLMs under coarse-label training, transfer learning, and joint learning combining both label types.

Result: Fine-grained annotations complement coarse labels and improve overall performance when used jointly; models show more balanced error profiles and reduced under-detection (FNR-FPR: 0.74→0.42 for LLaVA-1.6-Mistral-7B; 0.54→0.28 for Qwen2.5-VL-7B).

Conclusion: Combining coarse and fine-grained labels provides practical route to more reliable multimodal moderation, contributing to data-centric approaches through enhanced data quality.
...
### [168] [PaperVoyager : Building Interactive Web with Visual Language Models](https://arxiv.org/abs/2603.22999)
*Dasen Dai,Biao Wu,Meng Fang,Wenhao Wang*

Main category: cs.CL

TL;DR: PaperVoyager: An autonomous agent that converts research papers into executable interactive web systems, enabling users to manipulate inputs and observe dynamic behaviors, with significant improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing document agents mainly transform papers into static artifacts (summaries, webpages, slides), which are insufficient for technical papers involving dynamic mechanisms and state transitions. There's a need for interactive systems that allow users to manipulate inputs and observe dynamic behaviors.

Method: PaperVoyager: A structured generation framework that performs end-to-end processing without human intervention, including paper understanding, system modeling, and interactive webpage synthesis. It explicitly models mechanisms and interaction logic during synthesis.
...
Conclusion: PaperVoyager offers a new paradigm for interactive scientific paper understanding by converting research papers into executable interactive web systems, enabling users to manipulate inputs and observe dynamic behaviors.
...
### [169] [Knowledge Access Beats Model Size: Memory Augmented Routing for Persistent AI Agents](https://arxiv.org/abs/2603.23013)
*Xunzhuo Liu,Bowei He,Xue Liu,Andy Luo,Haichen Zhang,Huamin Chen*

Main category: cs.CL

TL;DR: Memory-augmented inference with lightweight 8B model achieves 69% of 235B model performance at 96% lower cost by leveraging conversational memory to handle repetitive user queries.


<details>
  <summary>Details</summary>
Motivation: Production AI agents face up to 47% semantically similar repetitive queries, each processed with same computational cost. This redundancy can be exploited through conversational memory to transform repetition from cost burden to efficiency advantage.

Method: Propose memory-augmented inference framework where lightweight 8B-parameter model uses retrieved conversational context via low-cost inference path. No additional training or labeled data required. Incorporates hybrid retrieval (BM25 + cosine similarity) for improved performance.

Result: Achieves 30.5% F1, recovering 69% of 235B model performance while reducing effective cost by 96%. 235B model without memory (13.7% F1) underperforms standalone 8B model (15.4% F1). Hybrid retrieval improves performance by +7.7 F1. At practical confidence thresholds, routing directs 96% of queries to small model.

Conclusion: Memory, rather than model size, is primary driver of accuracy and efficiency in persistent AI agents. Access to relevant knowledge outweighs model scale for user-specific queries. Conversational memory accumulation over time increases coverage of recurring topics, narrowing performance gap.
...
### [170] [Parametric Knowledge and Retrieval Behavior in RAG Fine-Tuning for Electronic Design Automation](https://arxiv.org/abs/2603.23047)
*Julian Oestreich,Maximilian Bley,Frank Binder,Lydia Müller,Maksym Sydorenko,André Alcalde*

Main category: cs.CL

TL;DR: RAG fine-tuning evaluation for long-form EDA text generation using triple-based human-validated pipeline (TriFEX) reveals limitations of standard NLP metrics and shows 7B models can outperform 72B baseline with proper adaptation.


<details>
  <summary>Details</summary>
Motivation: Most RAG fine-tuning studies focus on document QA and use standard NLP metrics that may obscure factual differences, creating a need for better evaluation methods for long-form text generation in specialized domains like electronic design automation.

Method: Adapt 7B model with five context augmentation strategies under varying retrieval conditions; introduce TriFEX (triple-based evaluation pipeline) with human validation and Parametric Knowledge Precision (PKP) to isolate internalized knowledge by filtering prompt-leaked claims.

Result: ROUGE and BERTScore fail to detect factual differences that triple-based evaluation reveals; existing knowledge internalization metric is retrieval-sensitive with 75% variance driven by expression rate changes rather than correctness; fine-tuned 7B variants outperform 72B baseline on most metrics and generalize across conditions.

Conclusion: Standard RAG evaluation metrics have significant limitations; triple-based evaluation reveals important factual differences; smaller models can be effectively adapted to specialized tasks for cost-efficient on-premises deployment, outperforming much larger baselines.
...
### [171] [AuthorMix: Modular Authorship Style Transfer via Layer-wise Adapter Mixing](https://arxiv.org/abs/2603.23069)
...
<details>
  <summary>Details</summary>
Motivation: Existing style transfer methods require training single models on large corpora to handle all target styles, which is costly, inflexible for target-specific adaptation, and often sacrifices meaning preservation for style transfer.

Method: Train individual style-specific LoRA adapters on high-resource authors, then use learned layer-wise adapter mixing to rapidly create specialized adaptation models for new low-resource targets using only a few training examples.

Result: AuthorMix outperforms existing state-of-the-art style-transfer baselines and GPT-5.1 for low-resource targets, achieving the highest overall score and substantially improving meaning preservation.

Conclusion: AuthorMix provides a lightweight, modular, and interpretable framework for authorship style transfer that enables efficient adaptation to new target styles with minimal training data while maintaining strong meaning preservation.
...
### [172] [When Language Models Lose Their Mind: The Consequences of Brain Misalignment](https://arxiv.org/abs/2603.23091)
*Gabriele Merlin,Mariya Toneva*

Main category: cs.CL

TL;DR: Brain-misaligned LLMs (trained to predict brain activity poorly while maintaining language modeling performance) show substantial impairment on downstream linguistic tasks compared to brain-aligned models, demonstrating that brain alignment is critical for robust linguistic competence.


<details>
  <summary>Details</summary>
Motivation: To investigate the functional implications of brain alignment in LLMs, specifically whether brain alignment contributes to linguistic competence beyond language modeling performance, by isolating its effects through controlled comparison with brain-misaligned models.

Method: Created brain-misaligned LLMs intentionally trained to predict brain activity poorly while maintaining high language modeling performance. Compared these with well-matched brain-aligned counterparts on over 200 downstream tasks across diverse linguistic domains (semantics, syntax, discourse, reasoning, morphology).

Result: Brain misalignment substantially impairs downstream performance across linguistic tasks, demonstrating that brain alignment is critical for achieving robust linguistic competence in LLMs.

Conclusion: Brain alignment plays a crucial role in LLMs' linguistic capabilities, offering novel insights into the relationship between neural representations and linguistic processing, and highlighting the importance of brain alignment for robust language understanding.
...
### [173] [HGNet: Scalable Foundation Model for Automated Knowledge Graph Generation from Scientific Literature](https://arxiv.org/abs/2603.23136)
*Devvrat Joshi,Islem Rekik*

Main category: cs.CL

TL;DR: A two-stage framework for zero-shot scientific knowledge graph construction with novel entity recognition and hierarchical relation extraction methods, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge graph construction approaches struggle with long multi-word entities, lack domain generalization, ignore hierarchical knowledge structure, and general-purpose LLMs are computationally expensive with inconsistent accuracy, resulting in shallow and inconsistent KGs.

Method: Two-stage framework: (1) Z-NERD with Orthogonal Semantic Decomposition (OSD) for domain-agnostic entity recognition and Multi-Scale TCQK attention for multi-word entities; (2) HGNet with hierarchy-aware message passing for relation extraction, plus Differentiable Hierarchy Loss and Continuum Abstraction Field (CAF) Loss for global consistency.

Result: New state-of-the-art on SciERC, SciER, and SPHERE benchmarks, improving NER by 8.08% and RE by 5.99% on out-of-distribution tests, with zero-shot gains reaching 10.76% for NER and 26.2% for RE.

Conclusion: The proposed framework enables scalable, zero-shot scientific KG construction with improved entity recognition and hierarchical relation extraction, formalizing hierarchical abstraction as a continuous property in Euclidean space and providing a simpler alternative to hyperbolic methods.
...
### [174] [Why AI-Generated Text Detection Fails: Evidence from Explainable AI Beyond Benchmark Accuracy](https://arxiv.org/abs/2603.23146)
*Shushanta Pudasaini,Luis Miralles-Pechuán,David Lillis,Marisa Llorens Salvador*

Main category: cs.CL

TL;DR: AI text detectors often fail to generalize across domains despite high benchmark scores, relying on dataset-specific artifacts rather than stable machine authorship signals.


<details>
  <summary>Details</summary>
Motivation: The widespread adoption of LLMs creates a pressing need for reliable AI-generated text detection, but current detectors' real-world reliability and interpretability remain uncertain despite high benchmark accuracy.

Method: Proposed an interpretable detection framework integrating linguistic feature engineering, machine learning, and explainable AI techniques; evaluated on PAN CLEF 2025 and COLING 2025 benchmarks with systematic cross-domain and cross-generator evaluation using SHAP-based explanations.

Result: Model achieved leaderboard-competitive F1 score of 0.9734 on benchmarks, but systematic evaluation revealed substantial generalization failure under distribution shift; SHAP explanations showed detectors rely on dataset-specific stylistic cues rather than stable machine authorship signals.

Conclusion: There's a fundamental tension in linguistic-feature-based detection: the most discriminative features are also most susceptible to domain shift, formatting variation, and text-length effects; this insight can help build more robust AI detectors across different settings.
...
### [175] [UniDial-EvalKit: A Unified Toolkit for Evaluating Multi-Faceted Conversational Abilities](https://arxiv.org/abs/2603.23160)
*Qi Jia,Haodong Zhao,Dun Pei,Xiujie Song,Shibo Wang,Zijian Chen,Zicheng Zhang,Xiangyang Zhu,Guangtao Zhai*

Main category: cs.CL

TL;DR: UniDial-EvalKit (UDE) is a unified evaluation toolkit that standardizes heterogeneous multi-turn AI system benchmarks through universal data schemas, modular pipelines, and consistent scoring interfaces.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation protocols for multi-turn interactive AI systems are highly heterogeneous with different dataset formats, model interfaces, and evaluation pipelines, which severely impedes systematic comparison and reproducibility.

Method: UDE provides holistic unification through: 1) Standardizing heterogeneous data formats into a universal schema, 2) Streamlining complex evaluation pipelines via modular architecture, 3) Aligning metric calculations under consistent scoring interface, 4) Supporting parallel generation/scoring and checkpoint-based caching for efficiency.
...
Conclusion: UDE provides a standardized benchmarking ecosystem for interactive AI evaluation, accelerating future breakthroughs through improved reproducibility, efficiency, and systematic comparison capabilities.
...
### [176] [From Synthetic to Native: Benchmarking Multilingual Intent Classification in Logistics Customer Service](https://arxiv.org/abs/2603.23172)
*Haoyu He,Jinyu Zhuang,Haoran Chu,Shuhang Yu,J,T AI Group,Hao Wang,Kunpeng Han*

Main category: cs.CL

TL;DR: A public benchmark for hierarchical multilingual intent classification using real logistics customer-service logs, showing that machine-translated test sets overestimate performance on noisy native queries.


<details>
  <summary>Details</summary>
Motivation: Existing multilingual benchmarks rely on machine-translated text that is cleaner than real customer requests, overestimating real-world robustness. There's a need for realistic multilingual intent benchmarks using native customer-service data.

Method: Constructed benchmark from 600K historical logistics customer-service logs, curating ~30K de-identified user queries through filtering, LLM-assisted quality control, and human verification. Organized into two-level taxonomy (13 parent, 17 leaf intents) with English, Spanish, Arabic as seen languages, and Indonesian, Chinese plus additional test-only languages for zero-shot evaluation. Provided paired native and machine-translated test sets to measure performance gap.

Result: Translated test sets substantially overestimate performance on noisy native queries, especially for long-tail intents and cross-lingual transfer. The gap highlights the need for more realistic multilingual intent benchmarks.

Conclusion: Machine-translated benchmarks fail to capture real-world robustness for multilingual intent classification. Real customer-service data reveals significant performance gaps, particularly for challenging cases like long-tail intents and cross-lingual transfer.
...
### [177] [ImplicitRM: Unbiased Reward Modeling from Implicit Preference Data for LLM alignment](https://arxiv.org/abs/2603.23184)
*Hao Wang,Haocheng Yang,Licheng Pan,Lei Shen,Xiaoxi Li,Yinuo Wang,Zhichao Chen,Yuan Lu,Haoxuan Li,Zhouchen Lin*

Main category: cs.CL

TL;DR: ImplicitRM learns unbiased reward models from implicit human feedback (clicks, copies) by stratifying samples into latent groups and using a theoretically unbiased learning objective, addressing challenges of missing negative samples and user preference bias.


<details>
  <summary>Details</summary>
Motivation: Current reward modeling in RLHF requires expensive experimental feedback data collection. Implicit human feedback (clicks, copies) offers a cost-effective alternative but presents challenges: lack of definitive negative samples and user preference bias that makes standard classification methods inapplicable.

Method: ImplicitRM stratifies training samples into four latent groups using a stratification model, then derives a learning objective through likelihood maximization that is theoretically proven to be unbiased, addressing both the missing negative samples problem and user preference bias.

Result: Experiments show ImplicitRM learns accurate reward models across implicit preference datasets, demonstrating effectiveness in learning from cost-effective implicit feedback while overcoming the fundamental challenges of implicit reward modeling.

Conclusion: ImplicitRM provides a viable solution for learning unbiased reward models from implicit human feedback, offering a cost-effective alternative to expensive experimental feedback collection in RLHF while addressing the core challenges of implicit preference data.
...
### [178] [Decoding AI Authorship: Can LLMs Truly Mimic Human Style Across Literature and Politics?](https://arxiv.org/abs/2603.23219)
*Nasser A Alsadhan*

Main category: cs.CL

TL;DR: LLMs (GPT-4o, Gemini 1.5 Pro, Claude Sonnet 3.5) can mimic literary/political figures but remain highly detectable via stylometric analysis; perplexity is the key distinguishing feature.


<details>
  <summary>Details</summary>
Motivation: To investigate the capability of state-of-the-art LLMs to emulate authorial signatures of prominent figures (Whitman, Wordsworth, Trump, Obama) and assess the detectability of AI-generated mimicry.

Method: Zero-shot prompting with thematic alignment to generate synthetic corpora, evaluated using transformer-based classification (BERT) and interpretable ML (XGBoost) with LIWC markers, perplexity, and readability indices.

Result: AI-generated mimicry is highly detectable; XGBoost with only 8 stylometric features achieves accuracy comparable to high-dimensional neural classifiers; perplexity is the primary discriminative metric showing AI's lower variability compared to human writing.

Conclusion: LLMs exhibit distributional convergence on some features (syntactic complexity, readability) but fail to replicate nuanced affective density and stylistic variance of human writing, providing benchmarks for authorship attribution in digital humanities.
...
### [179] [I Came, I Saw, I Explained: Benchmarking Multimodal LLMs on Figurative Meaning in Memes](https://arxiv.org/abs/2603.23229)
*Shijia Zhou,Saif M. Mohammad,Barbara Plank,Diego Frassinelli*

Main category: cs.CL

TL;DR: MLLMs show strong bias toward detecting figurative meaning in memes even when none exists, and their correct predictions often lack faithful explanations.


<details>
  <summary>Details</summary>
Motivation: Internet memes use multimodal figurative communication, but it's unclear how MLLMs interpret visual-textual combinations to identify figurative meaning.

Method: Evaluated 8 state-of-the-art generative MLLMs across three datasets on detecting and explaining six types of figurative meaning, plus human evaluation of explanation faithfulness.

Result: All models exhibited strong bias to associate memes with figurative meaning even when none present; correct predictions not always accompanied by faithful explanations.

Conclusion: MLLMs have significant limitations in multimodal figurative language understanding, showing systematic biases and unreliable explanation quality despite correct predictions.
...
### [180] [Is AI Catching Up to Human Expression? Exploring Emotion, Personality, Authorship, and Linguistic Style in English and Arabic with Six Large Language Models](https://arxiv.org/abs/2603.23251)
*Nasser A Alsadhan*

Main category: cs.CL

TL;DR: LLMs struggle to convincingly mimic human emotional nuance and personality traits, with AI-generated texts distinguishable from human-authored ones, though classification deteriorates on paraphrased samples. Significant generalization gaps exist between human and AI affective signals, but synthetic data shows potential for under-resourced languages like Arabic.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can convincingly emulate complex human traits like emotional expression and personality across diverse linguistic and cultural contexts, particularly focusing on emotional nuance in English and personality markers in Arabic as an under-resourced language with unique characteristics.

Method: Conducted two tasks across six models (Jais, Mistral, LLaMA, GPT-4o, Gemini, DeepSeek): 1) Evaluating machine classifiers' ability to distinguish human-authored vs AI-generated texts, 2) Assessing whether LLM-generated texts exhibit emotional/personality traits comparable to humans. Used emotion/personality classification experiments, linguistic/psycholinguistic analyses, and examined performance on paraphrased samples.

Result: AI-generated texts are distinguishable from human-authored ones (F1>0.95), but classification deteriorates on paraphrased samples, indicating reliance on superficial stylistic cues. Significant generalization gaps exist: classifiers trained on human data perform poorly on AI-generated texts and vice versa, suggesting LLMs encode affective signals differently. Augmenting training with AI-generated data enhances Arabic personality classification performance. GPT-4o and Gemini show superior affective coherence. Linguistic analyses reveal measurable divergences in tone, authenticity, and textual complexity.

Conclusion: LLMs cannot convincingly mimic human emotional nuance and personality traits, encoding affective signals differently from humans. However, synthetic data shows promise for addressing challenges in under-resourced languages. Findings have implications for affective computing, authorship attribution, and responsible AI deployment, particularly in under-resourced language contexts where detection and alignment pose unique challenges.
...
### [181] [Steering LLMs for Culturally Localized Generation](https://arxiv.org/abs/2603.23301)
*Simran Khanuja,Hongbin Liu,Shujian Zhang,John Lambert,Mingqing Chen,Rajiv Mathews,Lun Wang*

Main category: cs.CL

TL;DR: The paper introduces Cultural Embeddings (CuE) using mechanistic interpretability to identify and manipulate cultural representations in LLMs, enabling white-box cultural steering that improves faithfulness and elicits long-tail cultural concepts.


<details>
  <summary>Details</summary>
Motivation: LLMs exhibit cultural biases favoring cultures with abundant training data, while existing localization methods (prompting, post-training alignment) are black-box, hard to control, and don't distinguish between missing knowledge vs. poor elicitation.

Method: Use sparse autoencoders for mechanistic interpretability to identify interpretable features encoding culturally salient information, aggregate them into Cultural Embeddings (CuE), and apply CuE for both bias analysis and white-box steering interventions.

Result: CuE-based steering increases cultural faithfulness and elicits significantly rarer, long-tail cultural concepts compared to prompting alone. It's complementary to black-box methods, offering gains when applied to prompt-augmented inputs, suggesting models often have knowledge but need better elicitation.

Conclusion: The approach provides diagnostic insight into cultural representations in LLMs and offers a controllable method to steer toward desired cultures, addressing both analysis and intervention needs for cultural localization.
...
### [182] [WISTERIA: Weak Implicit Signal-based Temporal Relation Extraction with Attention](https://arxiv.org/abs/2603.23319)
*Duy Dao Do,Anaïs Halftermeyer,Thi-Bich-Hanh Dao*

Main category: cs.CL

TL;DR: WISTERIA is a temporal relation extraction framework that uses pair-conditioned top-K attention to identify implicit temporal signals rather than explicit markers, improving interpretability while maintaining competitive accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing attention-based models highlight globally salient tokens but overlook pair-specific cues that actually determine temporal relations. Current approaches often assume explicit temporal markers (before, after, when), but many temporal relations are expressed through implicit lexical, syntactic, or morphological elements.

Method: WISTERIA combines multi-head attention with pair-conditioned top-K pooling to isolate the most informative contextual tokens for each event pair. It examines whether top-K attention components conditioned on each event pair encode interpretable evidence for temporal classification, considering implicit signals rather than just explicit markers.

Result: The framework achieves competitive accuracy on TimeBank-Dense, MATRES, TDDMan, and TDDAuto datasets. Linguistic analyses of top-K tokens show that WISTERIA reveals pair-level rationales aligned with temporal linguistic cues, offering localized and interpretable views of temporal reasoning.

Conclusion: WISTERIA provides a more interpretable approach to temporal relation extraction by focusing on pair-specific implicit signals rather than global attention patterns, enabling better understanding of the linguistic evidence used for temporal classification decisions.
...
### [183] [Failure of contextual invariance in gender inference with large language models](https://arxiv.org/abs/2603.23485)
*Sagar Kumar,Ariel Flint,Luca Maria Aiello,Andrea Baronchelli*

Main category: cs.CL

TL;DR: LLM outputs are not stable under contextually equivalent formulations; minimal discourse context induces systematic shifts in gender inference tasks, violating contextual invariance assumptions.


<details>
  <summary>Details</summary>
Motivation: To test the assumption that LLM outputs are stable under contextually equivalent formulations of tasks, specifically examining whether minimal discourse context affects gender inference performance and bias patterns.

Method: Used controlled pronoun selection task with minimal theoretically uninformative discourse context; analyzed correlations with cultural gender stereotypes; employed Contextuality-by-Default analysis to quantify context dependence beyond marginal effects.

Result: Minimal context induces large systematic shifts in model outputs; correlations with gender stereotypes weaken/disappear with context; irrelevant features become most informative predictors; 19-52% of cases show persistent context dependence not attributable to simple pronoun repetition.

Conclusion: LLM outputs violate contextual invariance even under near-identical syntactic formulations, challenging standard evaluation practices and having implications for bias benchmarking and high-stakes deployment.
...
### [184] [AgentSLR: Automating Systematic Literature Reviews in Epidemiology with Agentic AI](https://arxiv.org/abs/2603.22327)
...
TL;DR: AgentSLR automates systematic literature reviews using LLMs, achieving 58x speed-up with human-level accuracy on epidemiological reviews of priority pathogens.


<details>
  <summary>Details</summary>
Motivation: Systematic literature reviews are essential for evidence-based policy but are costly, time-intensive (taking ~7 weeks), difficult to scale, and create bottlenecks in scientific evidence synthesis.

Method: Developed an open-source agentic pipeline (AgentSLR) using large language models to automate the complete systematic review workflow: article retrieval, screening, data extraction, and report synthesis. Tested on epidemiological reviews of nine WHO-designated priority pathogens with human-in-the-loop validation and comparison of five frontier models.

Result: AgentSLR achieves performance comparable to human researchers while reducing review time from approximately 7 weeks to 20 hours (58x speed-up). Performance on SLR tasks is driven less by model size or inference cost than by each model's distinctive capabilities. Human-in-the-loop validation identified key failure modes.

Conclusion: Agentic AI can substantially accelerate scientific evidence synthesis in specialized domains, demonstrating the feasibility of automating systematic literature reviews while maintaining human-level accuracy.
...
### [185] [Causal Direct Preference Optimization for Distributionally Robust Generative Recommendation](https://arxiv.org/abs/2603.22335)
*Chu Zhao,Enneng Yang,Jianzhe Zhao,Guibing Guo*

Main category: cs.IR

TL;DR: CausalDPO extends Direct Preference Optimization (DPO) for LLM-based recommendation by incorporating causal invariance learning to address spurious correlations from environmental confounders, improving out-of-distribution generalization.


<details>
  <summary>Details</summary>
Motivation: DPO amplifies spurious correlations from environmental confounders during preference alignment, undermining LLM-based recommendation generalization in out-of-distribution scenarios.

Method: CausalDPO extends DPO with causal invariance learning: introduces backdoor adjustment to eliminate confounder interference, models latent environmental distribution via soft clustering, and enforces invariance constraints across environments.

Result: Achieves average 17.17% performance improvement across four evaluation metrics under four distribution shift settings, demonstrating superior OOD generalization.

Conclusion: CausalDPO effectively captures stable user preference structures across environments, significantly enhancing OOD generalization of LLM-based recommendation models by addressing confounder-induced spurious correlations.
...
### [186] [Errors in AI-Assisted Retrieval of Medical Literature: A Comparative Study](https://arxiv.org/abs/2603.22344)
*Jenny Gao,Yongfeng Zhang,Mary L Disis,Lanjing Zhang*

Main category: cs.IR

TL;DR: LLMs have high error rates (47.8% complete failure) in medical literature retrieval with significant variability across platforms and journals, requiring careful review of bibliographic data.


<details>
  <summary>Details</summary>
Motivation: To quantitatively assess errors in reference retrieval by widely used free-version LLM platforms and identify factors associated with retrieval errors, as LLM-assisted literature retrieval may lead to erroneous references that haven't been rigorously quantified.

Method: Evaluated 2,000 references retrieved by 5 LLMs (Grok-2, ChatGPT GPT-4.1, Google Gemini Flash 2.5, Perplexity AI, and DeepSeek GPT-4) for 40 randomly-selected original articles from BMJ, JAMA, and NEJM published Jan 2024-July 2025. Used multimetric score ratio combining validity of DOI, PubMed ID, Google Scholar link, and relevance, plus complete miss rate. Applied multivariable regression to examine independent associations.

Result: LLM platforms completely failed to retrieve correct reference data 47.8% of the time. Average score ratio was 0.29 (SD 0.35; range 0-1.25). Grok had highest accuracy (0.57), Gemini lowest (0.11). NEJM articles had lower score ratios and higher complete miss rates than BMJ. LLM platforms and journals were independently associated with score ratios and complete miss rate, respectively.

Conclusion: LLMs show modest overall performance and significant variability in retrieval accuracy across platforms and journals. Both LLM platforms and journals are associated with LLM's performance in retrieving medical literature, necessitating careful review of bibliographic data when using LLM-assisted literature retrieval.
...
### [187] [Personalized Federated Sequential Recommender](https://arxiv.org/abs/2603.22349)
*Yicheng Di*

Main category: cs.IR

TL;DR: PFSR is a personalized federated sequential recommender that addresses efficiency and personalization challenges in consumer electronics recommendations using associative Mamba blocks, variable response mechanisms, and dynamic magnitude loss.


<details>
  <summary>Details</summary>
Motivation: Current sequential recommendation methods have quadratic computational complexity that hinders real-time performance and struggle to adapt to personalized user requirements across diverse scenarios in consumer electronics.

Method: Proposes Personalized Federated Sequential Recommender (PFSR) with three key components: 1) Associative Mamba Block to capture user profiles globally while improving efficiency, 2) Variable Response Mechanism for fine-tuning parameters based on individual user needs, and 3) Dynamic Magnitude Loss to preserve localized personalized information during training.

Result: The paper claims to address both efficiency and personalization challenges, though specific quantitative results are not provided in the abstract.

Conclusion: PFSR offers a solution to the dual challenges of computational efficiency and personalized adaptation in sequential recommendation systems for consumer electronics.
...
### [188] [Reasoner-Executor-Synthesizer: Scalable Agentic Architecture with Static O(1) Context Window](https://arxiv.org/abs/2603.22367)
*Ivan Dobrovolskyi*

Main category: cs.IR

TL;DR: RES architecture separates intent parsing, deterministic data retrieval, and narrative generation to achieve O(1) token complexity and eliminate hallucinations in LLM agents.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG approaches in LLM agents have two key problems: hallucination risk increases with context length, and token cost scales linearly with dataset size, making large-scale deployments inefficient and unreliable.

Method: Proposes Reasoner-Executor-Synthesizer (RES) architecture with three layers: Reasoner for intent parsing, Executor for deterministic data retrieval/aggregation using zero LLM tokens, and Synthesizer for narrative generation. Executor passes only fixed-size statistical summaries to Synthesizer.

Result: Formally proves O(1) token complexity with respect to dataset size. Validated on ScholarSearch (Crossref API with 130M+ articles) showing mean token cost of 1,574 tokens across 100 benchmark runs, regardless of dataset size (42,000 to 16.3M articles). Eliminates data hallucination by construction.

Conclusion: RES architecture provides a scalable solution for LLM agents that eliminates hallucinations and achieves constant token complexity, enabling efficient deployment on massive datasets without compromising reliability.
...
### [189] [AI Co-Scientist for Ranking: Discovering Novel Search Ranking Models alongside LLM-based AI Agents with Cloud Computing Access](https://arxiv.org/abs/2603.22376)
*Liwei Wu,Cho-Jui Hsieh*

Main category: cs.IR

TL;DR: AI Co-Scientist framework automates full search ranking research pipeline using LLM agents, discovering novel sequence feature handling techniques with substantial performance improvements.


<details>
  <summary>Details</summary>
Motivation: While AI agents have shown success in software engineering and scientific discovery, their application to developing novel ranking models for commercial search engines remains unexplored, presenting an opportunity to automate the full research pipeline.

Method: AI Co-Scientist framework that automates the complete search ranking research pipeline from idea generation to code implementation and GPU training job scheduling with expert-in-the-loop. Uses single-LLM agents for routine tasks and multi-LLM consensus agents (GPT 5.2, Gemini Pro 3, Claude Opus 4.5) for challenging phases like results analysis and idea generation.

Result: The framework discovered a novel technique for handling sequence features, with all model enhancements produced automatically, yielding substantial offline performance improvements. AI systems can discover ranking architectures comparable to human expert-developed ones while significantly reducing routine research workloads.

Conclusion: This represents the first study in the ranking community to utilize an AI Co-Scientist framework for algorithmic research, demonstrating that AI systems can effectively automate ranking model development and discover novel techniques with performance improvements.
...
### [190] [A Brief Comparison of Training-Free Multi-Vector Sequence Compression Methods](https://arxiv.org/abs/2603.22434)
*Rohan Jha,Chunsheng Zuo,Reno Kriz,Benjamin Van Durme*

Main category: cs.IR

TL;DR: Evaluation of training-free compression methods for multi-vector retrieval models shows token merging outperforms token pruning for reducing index size while preserving retrieval quality.


<details>
  <summary>Details</summary>
Motivation: Multi-vector retrieval models achieve better retrieval quality than single-vector models but have substantially larger index sizes due to additional sequence-length dimension in document embeddings, limiting practical deployment due to memory overhead and query latency.

Method: Evaluation of training-free methods targeting the token sequence length dimension unique to multi-vector retrieval, specifically comparing token merging versus token pruning approaches.

Result: Token merging is strictly superior to token pruning for reducing index size while maintaining retrieval effectiveness.

Conclusion: Token merging provides a more effective training-free compression approach for multi-vector retrieval models, addressing the practical limitations of large index sizes without compromising retrieval quality.
...
### [191] [GraphRAG for Engineering Diagrams: ChatP&ID Enables LLM Interaction with P&IDs](https://arxiv.org/abs/2603.22528)
*Achmad Anggawirya Alimin,Artur M. Schweidtmann*

Main category: cs.IR

TL;DR: ChatP&ID is an agentic framework using GraphRAG to enable cost-effective natural-language interaction with P&ID engineering diagrams by converting smart P&ID files into knowledge graphs for improved accuracy and reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Direct processing of raw P&ID images or smart files with LLMs is costly, inefficient, and prone to hallucinations, creating a need for more reliable and cost-effective methods for natural-language interaction with engineering diagrams.

Method: Transforms smart P&IDs in DEXPI standard into structured knowledge graphs, then uses Graph Retrieval-Augmented Generation (GraphRAG) with LLM agents for graph-based retrieval and reasoning. Integrates VectorRAG and PathRAG for small open-source models.

Result: Graph-based representations improve accuracy by 18% over raw image inputs and reduce token costs by 85% compared to directly ingesting smart P&ID files. GPT-5-mini with ContextRAG achieves 91% accuracy at $0.004 per task. Small open-source models with VectorRAG/PathRAG improve accuracy by up to 40%.

Conclusion: ChatP&ID enables intuitive natural-language interaction with complex engineering diagrams and lays groundwork for AI-assisted process engineering tasks like HAZOP and multi-agent analysis, demonstrating GraphRAG's effectiveness for engineering diagram interpretation.
...
### [192] [flexvec: SQL Vector Retrieval with Programmatic Embedding Modulation](https://arxiv.org/abs/2603.22587)
*Damian Delmas*

Main category: cs.IR

TL;DR: flexvec is a retrieval kernel that exposes embedding matrix and score array as programmable surfaces for arithmetic operations before selection, enabling Programmatic Embedding Modulation (PEM) with SQL integration for fast, composable retrieval operations.


<details>
  <summary>Details</summary>
Motivation: As AI agents become primary consumers of retrieval APIs, there's an opportunity to expose more of the retrieval pipeline to the caller for greater flexibility and control over the retrieval process.

Method: flexvec retrieval kernel exposes embedding matrix and score array as programmable surfaces, allowing arithmetic operations on both before selection (Programmatic Embedding Modulation). The system integrates these operations into a SQL interface via a query materializer that facilitates composable query primitives.

Result: On a production corpus of 240,000 chunks, three composed modulations execute in 19 ms end-to-end on a desktop CPU without approximate indexing. At one million chunks, the same operations execute in 82 ms.

Conclusion: flexvec enables efficient, programmable retrieval through Programmatic Embedding Modulation, providing AI agents with flexible control over retrieval pipelines while maintaining high performance on standard hardware.
...
### [193] [KARMA: Knowledge-Action Regularized Multimodal Alignment for Personalized Search at Taobao](https://arxiv.org/abs/2603.22779)
*Zhi Sun,Wenming Zhang,Yi Wei,Liren Yu,Zhixuan Zhang,Dan Ou,Haihong Tang*

Main category: cs.IR

TL;DR: KARMA framework addresses the Knowledge-Action Gap in LLMs for personalized search by using semantic reconstruction as a train-only regularizer to prevent semantic collapse while optimizing for action objectives.


<details>
  <summary>Details</summary>
Motivation: Direct fine-tuning of LLMs on industrial personalized tasks yields suboptimal results due to the Knowledge-Action Gap - the conflict between preserving pre-trained semantic knowledge and aligning with specific personalized actions through discriminative objectives, leading to semantic collapse.

Method: KARMA (Knowledge-Action Regularized Multimodal Alignment) treats semantic reconstruction as a train-only regularizer with two complementary objectives: history-conditioned semantic generation (anchors to LLM's native next-token distribution) and embedding-conditioned semantic reconstruction (ensures interest embedding remains semantically recoverable).

Result: On Taobao search system, KARMA mitigates semantic collapse (attention-sink analysis) and improves both action metrics and semantic fidelity. Achieved +0.25 CTR AUC in ranking, +1.86 HR in pre-ranking, +2.51 HR in recalling, and +0.5% increase in Item Click when deployed online with low inference overhead.

Conclusion: KARMA successfully bridges the Knowledge-Action Gap in LLMs for personalized search by regularizing semantic reconstruction during training, preventing semantic collapse while maintaining strong action performance, leading to significant improvements in industrial search systems.
...
### [194] [GateSID: Adaptive Gating for Semantic-Collaborative Alignment in Cold-Start Recommendation](https://arxiv.org/abs/2603.22916)
*Hai Zhu,Yantao Yu,Lei Shen,Bing Wang,Xiaoyi Zeng*

Main category: cs.IR

TL;DR: GateSID: Adaptive gating framework that balances semantic and collaborative signals based on item maturity to address cold-start recommendation challenges, outperforming baselines with significant online improvements.


<details>
  <summary>Details</summary>
Motivation: Cold-start scenarios suffer from scarce collaborative signals for new items, exacerbating the Matthew effect and undermining platform diversity. Existing methods face a collaborative-semantic tradeoff: collaborative signals work well for popular items but are unreliable for cold-start items, while over-reliance on semantic information obscures meaningful collaborative differences.

Method: GateSID uses an adaptive gating network to dynamically balance semantic and collaborative signals based on item maturity. First, multimodal features are discretized into hierarchical Semantic IDs using Residual Quantized VAE. Two key components: (1) Gating-Fused Shared Attention fuses intra-modal attention distributions with item-level gating weights from embeddings and statistical features; (2) Gate-Regulated Contrastive Alignment adaptively calibrates cross-modal alignment, enforcing stronger semantic-behavior consistency for cold-start items while relaxing constraints for popular items to preserve reliable collaborative signals.

Result: Extensive offline experiments on large-scale industrial datasets show GateSID consistently outperforms strong baselines. Online A/B tests confirm practical value: +2.6% GMV, +1.1% CTR, and +1.6% orders with less than 5 ms additional latency.

Conclusion: GateSID effectively addresses the collaborative-semantic tradeoff in cold-start recommendation by adaptively balancing signals based on item maturity, achieving significant performance improvements while maintaining low latency for practical deployment.
...
### [195] [From Questions to Trust Reports: A LLM-IR Framework for the TREC 2025 DRAGUN Track](https://arxiv.org/abs/2603.23125)
*Ignacy Alwasiak,Kene Nnolim,Jaclyn Thi,Samy Ateia,Markus Bink,Gregor Donabauer,David Elsweiler,Udo Kruschwitz*

Main category: cs.IR

TL;DR: The UR_Trecking system for TREC 2025 DRAGUN Track combines LLM-based question generation with semantic filtering, diversity enforcement, Chain-of-Thought query expansion, and monoT5 re-ranking to retrieve evidence for trustworthiness assessment of online news, showing improved relevance and domain trust.


<details>
  <summary>Details</summary>
Motivation: Address the growing need for effective support tools that help users evaluate the trustworthiness of online news, as targeted by the DREC 2025 DRAGUN Track.

Method: Combines LLM-based question generation with semantic filtering and diversity enforcement using clustering for Task 1. For retrieval, employs several query expansion strategies including reasoning-based Chain-of-Thought expansion to retrieve evidence from MS MARCO V2.1 segmented corpus, followed by monoT5 re-ranking and LLM relevance filtering with domain-level trustworthiness dataset. For Task 2, synthesizes selected evidence using LLM into concise trustworthiness reports with citations.

Result: Chain-of-Thought query expansion and re-ranking substantially improve both relevance and domain trust compared to baseline retrieval. Question-generation performance shows moderate quality with room for improvement.

Conclusion: Outlines key challenges encountered and suggests directions for enhancing robustness and trustworthiness assessment in future iterations of the system.
...
### [196] [Reasoning over Semantic IDs Enhances Generative Recommendation](https://arxiv.org/abs/2603.23183)
*Yingzhi He,Yan Sun,Junfei Tan,Yuxin Chen,Xiaoyu Kong,Chunxu Shen,Xiang Wang,An Zhang,Tat-Seng Chua*

Main category: cs.IR

TL;DR: SIDReasoner enhances LLM-based generative recommendation by improving reasoning over Semantic IDs through better alignment with language and outcome-driven reinforcement, without needing explicit reasoning annotations.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based generative recommendation uses Semantic IDs for efficiency but struggles with reasoning over these itemic tokens since they lack native meaning to LLMs, and recommendation-oriented reasoning is hard to evaluate with scarce supervision.

Method: Two-stage framework: 1) Enhances SID-language alignment via multi-task training on enriched SID-centered corpus synthesized by a stronger teacher model; 2) Improves recommendation reasoning through outcome-driven reinforced optimization without explicit reasoning annotations.

Result: Extensive experiments on three real-world datasets demonstrate effectiveness of reasoning-augmented SID-based generative recommendation, showing improved accuracy, interpretability, and cross-domain generalization.

Conclusion: SIDReasoner successfully addresses SID reasoning challenges by strengthening SID-language alignment and using outcome-driven reinforcement, unlocking transferable LLM reasoning for generative recommendation and highlighting broader potential of large reasoning models.
...
TL;DR: A novel offline safe RL method using safety-conditioned reachability sets to decouple reward maximization from cumulative safety constraints, avoiding unstable adversarial optimization while maintaining safety.


<details>
  <summary>Details</summary>
Motivation: Real-world sequential decision making must balance reward maximization with safety constraints, but existing methods often lead to unstable min/max adversarial optimization. Most reachability-based methods only address hard safety constraints, not cumulative cost constraints.

Method: 1) Define a safety-conditioned reachability set that decouples reward maximization from cumulative safety cost constraints. 2) Show how this set enforces safety constraints without unstable min/max or Lagrangian optimization. 3) Develop a novel offline safe RL algorithm that learns safe policies from fixed datasets without environment interaction.

Result: The method matches or outperforms state-of-the-art baselines on standard offline safe RL benchmarks and a real-world maritime navigation task while maintaining safety.

Conclusion: The proposed safety-conditioned reachability approach provides an effective alternative to unstable adversarial optimization methods for offline safe RL, successfully handling cumulative safety constraints while maintaining performance.
...
TL;DR: The paper analyzes synthetic data generation using LLMs, finds correlation between embedding space density and prediction accuracy, and proposes targeted embedding-based sampling to improve data diversity and model performance.


<details>
  <summary>Details</summary>
Motivation: Synthetic Data Generation (SDG) with LLMs is effective for improving smaller models via fine-tuning, but faces challenges in ensuring quality and diversity of generated data. The authors aim to address this by analyzing data distribution in embedding space.

Method: Analyze diversity and distribution of generated data in embedding space, demonstrate correlation between example density in neighborhoods and prediction accuracy, and develop targeted pipeline for embedding-based sampling to enhance data diversity.

Result: Strong correlation found between density of examples within specific neighborhoods in embedding space and accuracy of predictions on examples from those regions. The proposed embedding-based sampling pipeline consistently improves performance across several benchmarks.

Conclusion: Embedding space analysis reveals important insights about synthetic data quality, and targeted sampling based on embedding distributions effectively enhances data diversity and improves model performance in SDG applications.
...
TL;DR: A compact uncertainty estimation method for LLMs that analyzes cross-layer agreement patterns in internal representations using a single forward pass, achieving strong performance across models, datasets, and quantization settings.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce confidently wrong outputs, making reliable uncertainty estimation crucial. Existing methods have limitations: output-based heuristics are cheap but brittle, while probing internal representations is effective but high-dimensional and difficult to transfer across models or datasets.

Method: Proposes a compact, per-instance uncertainty estimation method that scores cross-layer agreement patterns in LLM internal representations. The approach requires only a single forward pass and analyzes how different layers within the model agree or disagree with each other, creating a lightweight uncertainty signal.

Result: The method matches probing performance in-distribution (within -1.8 AUPRC and +4.9 Brier score points). Under cross-dataset transfer, it consistently outperforms probing with gains up to +2.86 AUPRC and +21.02 Brier points. Under 4-bit weight-only quantization, it remains robust, improving over probing by +1.94 AUPRC and +5.33 Brier points on average. Analysis of layer-layer interactions reveals differences in how models encode uncertainty.

Conclusion: The proposed uncertainty estimation method offers a lightweight, compact approach to capture transferable uncertainty in LLMs, addressing limitations of both output-based heuristics and high-dimensional probing methods while maintaining robustness across different operational conditions.
...
TL;DR: Sparse Feature Attention (SFA) reduces Transformer attention cost from O(n²d) to O(n²k²/d) using k-sparse codes for queries/keys, enabling efficient ultra-long context scaling with minimal accuracy degradation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for scaling Transformers to ultra-long contexts (local windows, kernel approximations, token-level sparsity) consistently degrade accuracy. The paper explores an orthogonal axis: feature sparsity, to reduce attention cost while preserving accuracy.

Method: Proposes Sparse Feature Attention (SFA) where queries and keys are represented as k-sparse codes, reducing attention cost from Θ(n²d) to Θ(n²k²/d). Introduces FlashSFA, an IO-aware kernel that extends FlashAttention to operate directly on sparse overlaps without materializing dense score matrices.

Result: SFA matches dense baselines in GPT-2 and Qwen3 pretraining while improving speed by up to 2.5× and reducing FLOPs and KV-cache by nearly 50%. Preserves retrieval accuracy and robustness at long contexts, outperforming short-embedding baselines that collapse feature diversity.

Conclusion: Feature-level sparsity is established as a complementary and underexplored axis for efficient attention, enabling Transformers to scale to orders-of-magnitude longer contexts with minimal quality loss.
...
TL;DR: LLMs operate in continuous vector spaces but output discrete tokens, creating a geometric mismatch. The paper develops a Riemannian manifold framework where tokens partition the manifold into Voronoi regions, introduces an "expressibility gap" measure, proves theoretical bounds, and validates predictions across transformer architectures.


<details>
  <summary>Details</summary>
Motivation: The fundamental mismatch between LLMs' continuous internal computations and discrete token outputs creates geometric consequences that remain poorly understood. There's a need for a mathematical framework to analyze this semantic distortion and its implications for model behavior and performance.

Method: Develops a Riemannian geometric framework interpreting LLM hidden states as points on a latent semantic manifold equipped with the Fisher information metric. Tokens correspond to Voronoi regions partitioning the manifold. Introduces the expressibility gap as a geometric measure of semantic distortion from vocabulary discretization, proves theoretical bounds using rate-distortion theory and coarea formula, and validates across six transformer architectures (124M-1.5B parameters).

Result: Validates theoretical predictions across six transformer models, confirming: universal hourglass intrinsic dimension profiles, smooth curvature structure, linear expressibility gap scaling with slopes 0.87-1.12 (R^2 > 0.985), and a persistent hard core of boundary-proximal representations invariant to scale. Provides geometric decomposition of perplexity.

Conclusion: The Riemannian manifold framework successfully captures the geometric structure of LLM representations, revealing universal patterns across architectures and scales. The persistent hard core of boundary-proximal representations and linear scaling of the expressibility gap have important implications for architecture design, model compression, decoding strategies, and scaling laws.
...
TL;DR: This study applies K-means clustering to student data (CET-4 scores, GPA, personality traits, student cadre experiences) to provide personalized career guidance by grouping students into four clusters with similar characteristics and offering targeted career suggestions for each group.


<details>
  <summary>Details</summary>
Motivation: Existing career guidance methods focus mainly on career path prediction but lack analysis of how different combinations of student characteristics fit specific career directions. The study aims to address this gap by using clustering to provide more personalized and scientifically-based career guidance.

Method: The researchers collected data from over 3000 students including CET-4 scores, GPA, personality traits, and student cadre experiences. They applied the K-means clustering algorithm to group students into four clusters by minimizing intra-cluster squared error, ensuring high similarity within clusters and maximizing differences between clusters.

Result: The K-means algorithm successfully classified students into four main groups based on their characteristics. Analysis revealed that students with different characteristic combinations are suitable for different career directions, providing a scientific basis for personalized career guidance that effectively enhances employment success rates.

Conclusion: K-means clustering provides an effective approach for personalized career guidance by identifying student groups with similar characteristics. Future research can improve precision by expanding sample size, adding more feature variables, and considering external factors to enhance clustering accuracy and guidance effectiveness.
...
TL;DR: The paper proposes a training-free hallucination detector for LLMs based on distribution complexity measured via Wasserstein distances between token embeddings of generated responses.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in LLMs remain a major obstacle to trustworthy deployment, creating need for accurate, lightweight, and broadly applicable detectors. The authors argue that the complexity of the conditional distribution defined by an LLM with a prompt can serve as an indicator of hallucination.

Method: Compute optimal-transport distances between sets of token embeddings from pairwise generated samples, creating a Wasserstein distance matrix. From this matrix, derive two signals: AvgWD (average cost) and EigenWD (cost complexity). The approach is training-free and can be extended to black-box LLMs via teacher forcing with an accessible teacher model.

Result: Experiments show that AvgWD and EigenWD are competitive with strong uncertainty baselines and provide complementary behavior across models and datasets, demonstrating that distribution complexity is an effective signal for LLM truthfulness detection.

Conclusion: Distribution complexity, quantified through Wasserstein distance matrices between generated samples, provides an effective training-free approach for detecting hallucinations in LLMs, offering a lightweight and broadly applicable solution.
...
TL;DR: ProVQ addresses premature discretization in VQ by introducing progressive quantization that gradually transitions from continuous to discrete latent spaces, improving performance across image and biological sequence domains.


<details>
  <summary>Details</summary>
Motivation: Existing Vector Quantization (VQ) methods suffer from premature discretization, where discretization occurs before the encoder has properly captured the underlying data manifold, creating a fundamental conflict in VQ training.

Method: Progressive Quantization (ProVQ) treats quantization as a curriculum that smoothly anneals from a continuous latent space to a discrete one, incorporating quantization hardness dynamics as a fundamental axis in VQ training to guide codebooks toward well-expanded manifolds.

Result: ProVQ demonstrates broad effectiveness across diverse modalities: improved reconstruction and generative performance on ImageNet-1K and ImageNet-100 benchmarks, and establishes new state-of-the-art performance for protein structure tokenization on the StrutTokenBench leaderboard.

Conclusion: ProVQ resolves the fundamental conflict of premature discretization in VQ by introducing progressive quantization, enabling better codebook learning and establishing superior performance across image and biological sequence domains.
...
TL;DR: CN-Buzz2Portfolio is a reproducible benchmark for evaluating LLMs as financial agents in Chinese markets, focusing on macro/sector asset allocation using daily trending news rather than entity-level stock picking.


<details>
  <summary>Details</summary>
Motivation: Current evaluation paradigms for LLMs as financial agents are inadequate: live trading is irreproducible and prone to luck bias, while static benchmarks focus on entity-level stock picking and ignore broader market attention dynamics.

Method: Introduces CN-Buzz2Portfolio benchmark with daily trending news data from Chinese markets (2024-mid 2025), requiring agents to map narratives to macro/sector allocation. Proposes Tri-Stage CPA Agent Workflow (Compression, Perception, Allocation) focusing on ETFs rather than individual stocks to reduce idiosyncratic volatility.

Result: Extensive experiments on nine LLMs reveal significant disparities in how models translate macro-level narratives into portfolio weights, highlighting challenges in aligning general reasoning with financial decision-making.

Conclusion: The work provides new insights into LLM alignment for financial decision-making and releases all data, codes, and experiments to promote sustainable financial agent research, addressing the evaluation dilemma through a reproducible benchmark.
...
TL;DR: A conditional diffusion model-based full-waveform inversion method that incorporates 2D density information as conditional input to regularize velocity inversion, improving resolution, structural fidelity, stability, and robustness compared to unconditional diffusion approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional full-waveform inversion suffers from nonlinearity and local minima issues. Existing generative diffusion models provide regularization but use unconditional processes that ignore physical coupling between velocity and density properties, limiting their effectiveness.

Method: Proposes a conditional diffusion model regularization approach for full-waveform inversion. Improves the diffusion model's backbone network structure to incorporate 2D density information as conditional input into the U-Net architecture, enabling physics-aware regularization.

Result: The conditional diffusion model-based method significantly improves inversion resolution and structural fidelity. It demonstrates stronger stability and robustness when handling complex scenarios compared to unconditional approaches, effectively utilizing density information to constrain inversion.

Conclusion: The conditional diffusion model regularization effectively leverages density information to constrain full-waveform inversion, enhancing performance and practical application value by addressing physical coupling relationships ignored by unconditional methods.
...
TL;DR: UniFluids: A conditional flow-matching framework using diffusion Transformers to unify learning of PDE solution operators across diverse dimensions and physical variables, achieving parallel sequence generation and improved accuracy through x-prediction.


<details>
  <summary>Details</summary>
Motivation: Current PDE simulation is crucial for scientific research, and while deep neural networks show promise for learning PDE solution operators, existing approaches lack a unified framework that can handle diverse PDEs with varying dimensionality and physical variables while enabling efficient parallel generation.

Method: UniFluids uses a conditional flow-matching framework with diffusion Transformers, introducing a unified 4D spatiotemporal representation for heterogeneous PDE datasets to enable joint training. It employs x-prediction in flow-matching operator learning based on the observation that PDE dataset effective dimension is much lower than patch dimension, improving accuracy. The approach enables parallel sequence generation unlike autoregressive PDE foundation models.

Result: Experimental evaluation on several PDE datasets covering 1D, 2D, and 3D spatial dimensions shows UniFluids achieves strong prediction accuracy and demonstrates good scalability and cross-scenario generalization capability.

Conclusion: UniFluids presents the first flow-matching approach for unified PDE operator learning, offering parallel generation capabilities and improved accuracy through x-prediction, with demonstrated effectiveness across diverse PDE scenarios and dimensions.
...
TL;DR: MultiModalFallDetector: A novel multi-modal deep learning framework for real-time elderly fall detection using wearable sensors, achieving state-of-the-art performance with 98.7 F1-score and sub-50ms inference latency.


<details>
  <summary>Details</summary>
Motivation: The increasing global aging population requires reliable health monitoring systems for elderly fall detection. Traditional single-modality acceleration-based approaches suffer from high false alarm rates, and conventional machine learning methods require extensive hand-crafted feature engineering.

Method: Proposes MultiModalFallDetector with multiple innovations: multi-scale CNN-based feature extractor for varying temporal resolutions; fusion of tri-axial accelerometer, gyroscope, and four-channel physiological signals; multi-head self-attention mechanism for dynamic temporal weighting; Focal Loss to address class imbalance; auxiliary activity classification task for regularization; and transfer learning from UCI HAR to SisFall dataset.

Result: Achieves F1-score of 98.7, Recall of 98.9, and AUC-ROC of 99.4 on SisFall dataset (real-world simulated fall trials from elderly participants aged 60-85), significantly outperforming baseline methods. Maintains sub-50ms inference latency on edge devices.

Conclusion: The framework demonstrates superior performance for elderly fall detection and is suitable for real-time deployment in geriatric care settings due to its high accuracy and low inference latency.
...
### [209] [Emergency Preemption Without Online Exploration: A Decision Transformer Approach](https://arxiv.org/abs/2603.22315)
*Haoran Su,Hanxiao Deng,Yandong Sun*

Main category: cs.LG

TL;DR: Proposes a return-conditioned Decision Transformer framework for emergency corridor optimization that eliminates online environment interaction, enables dispatch-level urgency control via a single scalar, and extends to multi-agent coordination with graph attention.


<details>
  <summary>Details</summary>
Motivation: Current emergency vehicle signal preemption strategies are reactive and uncontrollable, limiting their effectiveness in optimizing response times while managing civilian traffic disruption.

Method: Uses Decision Transformer for offline, return-conditioned sequence modeling of corridor optimization; extends to Multi-Agent Decision Transformer (MADT) with graph attention for spatial coordination; includes Constrained DT extension for explicit civilian disruption budgets.

Result: DT reduces average EV travel time by 37.7% vs fixed-timing preemption (88.6s vs 142.3s) on 4x4 grid, achieves lowest civilian delay (11.3s/veh) and fewest EV stops (1.2); MADT improves further with 45.2% reduction on 8x8 grid; return conditioning enables smooth trade-off between EV travel time (72.4-138.2s) and civilian delay (16.8-5.4s/veh) without retraining.

Conclusion: The return-conditioned Decision Transformer framework provides an effective, controllable approach to emergency corridor optimization that eliminates online interaction requirements, enables flexible dispatch control, and scales to multi-agent coordination through graph attention mechanisms.
...
### [210] [ST-GDance++: A Scalable Spatial-Temporal Diffusion for Long-Duration Group Choreography](https://arxiv.org/abs/2603.22316)
*Jing Xu,Weiqiang Wang,Cunjian Chen,Jun Liu,Qiuhong Ke*

Main category: cs.LG

TL;DR: ST-GDance++ is a scalable framework for group dance generation from music that decouples spatial and temporal dependencies to enable efficient, collision-aware choreography with reduced latency.


<details>
  <summary>Details</summary>
Motivation: Existing group dance generation models suffer from quadratic attention computation growth with increasing dancers and sequence length, leading to reduced efficiency, motion collisions, and limited scalability for interactive applications like film, gaming, and animation.

Method: Proposes ST-GDance++ with two key components: 1) Lightweight distance-aware graph convolutions for spatial modeling of inter-dancer relationships, and 2) Diffusion noise scheduling with temporal-aligned attention mask for stream-based generation of long motion sequences.
...
Conclusion: ST-GDance++ effectively addresses scalability and efficiency challenges in group dance generation by decoupling spatial-temporal dependencies, enabling practical deployment in interactive scenarios.
...
### [211] [Geometric Mixture-of-Experts with Curvature-Guided Adaptive Routing for Graph Representation Learning](https://arxiv.org/abs/2603.22317)
*Haifang Cao,Yu Wang,Timing Li,Xinjie Yao,Pengfei Zhu*

Main category: cs.LG

TL;DR: GeoMoE is a Geometric Mixture-of-Experts framework that adaptively fuses node representations across diverse Riemannian spaces using Ollivier-Ricci Curvature as a geometric prior to handle graph topological heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Graph-structured data exhibits complex topological heterogeneity that is difficult to model accurately within a single Riemannian manifold. Existing mixed-curvature methods often rely on implicit, task-driven routing that lacks fundamental geometric grounding.

Method: Proposes GeoMoE framework with: 1) Ollivier-Ricci Curvature as intrinsic geometric prior to orchestrate expert collaboration, 2) Graph-aware gating network with node-specific fusion weights regularized by curvature-guided alignment loss, 3) Curvature-aware contrastive objective that constructs positive/negative pairs based on curvature consistency.

Result: Extensive experiments on six benchmark datasets demonstrate that GeoMoE outperforms state-of-the-art baselines across diverse graph types.

Conclusion: GeoMoE provides a geometrically-grounded framework for handling graph topological heterogeneity by adaptively fusing representations across diverse Riemannian spaces using curvature-based geometric priors, achieving superior performance over existing methods.
...
### [212] [Sparsely-Supervised Data Assimilation via Physics-Informed Schrödinger Bridge](https://arxiv.org/abs/2603.22319)
*Dohyun Bu,Chanho Kim,Seokun Choi,Jong-Seok Lee*

Main category: cs.LG

TL;DR: PICSB enables fast spatiotemporal field reconstruction for PDE-governed systems using physics-informed conditional Schrödinger Bridge without HF supervision or test-time optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional data assimilation for PDE systems requires test-time optimization which is slow for time-critical applications, and existing amortized approaches need full-field HF supervision which is often impractical in real-world settings.

Method: Physics-Informed Conditional Schrödinger Bridge (PICSB) transports LF prior to observation-conditioned HF posterior using iterative surrogate-endpoint refresh scheme, incorporates PDE residuals in training objective, and enforces observations via hard conditioning during sampling.

Result: PICSB enables extremely fast spatiotemporal field reconstruction while maintaining competitive accuracy under sparse HF supervision on fluid PDE benchmarks.

Conclusion: PICSB provides a practical solution for fast, physics-consistent field reconstruction in data assimilation without requiring HF supervision or test-time optimization.
...
### [213] [Bridging the Gap Between Climate Science and Machine Learning in Climate Model Emulation](https://arxiv.org/abs/2603.22320)
*Luca Schmidt,Nina Effenberger*

Main category: cs.LG

TL;DR: A framework integrating climate science and ML perspectives to overcome barriers in using ML emulators for climate models, focusing on easy-to-adopt, task-specific designs with demonstrated reliability.


<details>
  <summary>Details</summary>
Motivation: Climate models have high computational demands limiting their use, while ML emulators face accessibility, expertise, and trust barriers due to perceived lack of physical grounding.

Method: Introduces a framework that integrates both climate science and machine learning perspectives to design emulators that are easy to adopt, address clearly defined tasks, and demonstrate reliability.

Result: The framework provides a promising path for bridging the gap between climate science and machine learning fields by overcoming barriers to ML emulator adoption.

Conclusion: Designing accessible, task-specific ML emulators with demonstrated reliability offers an effective approach to overcoming barriers and enabling wider use of climate model emulation.
...
### [214] [AEGIS: An Operational Infrastructure for Post-Market Governance of Adaptive Medical AI Under US and EU Regulations](https://arxiv.org/abs/2603.22322)
*Fardin Afdideh,Mehdi Astaraki,Fernando Seoane,Farhad Abtahi*

Main category: cs.LG

TL;DR: AEGIS is a governance framework that operationalizes FDA and EU regulatory mechanisms for continuous learning in medical AI, featuring three modules and a four-category deployment decision taxonomy with ALARM signals for critical state detection.


<details>
  <summary>Details</summary>
Motivation: Medical AI systems require governance frameworks that ensure safety while enabling continuous improvement through iterative model updates, addressing regulatory requirements like FDA's PCCP and EU AI Act provisions.

Method: AEGIS framework with three modules: dataset assimilation/retraining, model monitoring, and conditional decision; implements four-category deployment taxonomy (APPROVE, CONDITIONAL APPROVAL, CLINICAL REVIEW, REJECT) with independent PMS ALARM signals for critical state detection.

Result: In sepsis prediction simulations across 11 iterations: 8 APPROVE, 1 CONDITIONAL APPROVAL, 1 CLINICAL REVIEW, 1 REJECT decisions; ALARM signals at iterations 8 and 10 detected drift before observable performance degradation, including critical state where no deployable model exists while released model fails.

Conclusion: AEGIS successfully translates regulatory change-control concepts into executable governance procedures, supporting safe continuous learning for adaptive medical AI across diverse clinical applications while detecting critical states before performance degradation.
...
### [215] [A Multi-Task Targeted Learning Framework for Lithium-Ion Battery State-of-Health and Remaining Useful Life](https://arxiv.org/abs/2603.22323)
*Chenhan Wang,Zhengyi Bao,Huipin Lin,Jiahao Nie,Chunxiang Zhu*

Main category: cs.LG

TL;DR: A multi-task learning framework integrating multi-scale CNNs, improved LSTM, and dual-stream attention for simultaneous SOH and RUL prediction of lithium-ion batteries, achieving significant RMSE reductions.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for battery SOH and RUL prediction have limitations in feature extraction selectivity and temporal dependency modeling, with most relying on traditional RNNs that struggle with long-term time-series modeling.

Method: Proposes a multi-task targeted learning framework with: 1) multi-scale CNN feature extraction module for local battery decline patterns, 2) improved extended LSTM for enhanced long-term temporal information retention, 3) dual-stream attention module (polarized and sparse attention) to selectively focus on SOH/RUL-relevant information, and 4) Hyperopt optimization algorithm for automated hyperparameter tuning.

Result: Extensive experiments on battery aging datasets show the proposed method reduces average RMSE for SOH and RUL predictions by 111.3% and 33.0%, respectively, compared to traditional and state-of-the-art methods.

Conclusion: The integrated multi-task framework effectively addresses feature extraction selectivity and temporal dependency modeling limitations in battery health prediction, demonstrating superior performance through comprehensive neural network integration and automated optimization.
...
TL;DR: Delta-Aware Quantization (DAQ) is a data-free post-training quantization framework that preserves post-training knowledge by optimizing for directional fidelity of parameter deltas rather than minimizing reconstruction error.


<details>
  <summary>Details</summary>
Motivation: Standard quantization methods minimize reconstruction error but are agnostic to the base model, allowing quantization noise to disproportionately corrupt the small-magnitude parameter deltas (ΔW) that encode post-training behavior, which can degrade style-specific capabilities acquired during post-training.

Method: DAQ replaces reconstruction-based objectives with two delta-aware metrics: Sign Preservation Rate and Cosine Similarity, which directly optimize for directional fidelity of ΔW. The framework requires only the base and post-trained weight matrices and operates in a data-free manner.

Result: In a pilot FP8 study, DAQ recovers style-specific capabilities lost under standard quantization while maintaining general performance, demonstrating effectiveness in preserving post-training knowledge.

Conclusion: DAQ provides an effective data-free post-training quantization approach that preserves directional fidelity of parameter deltas, addressing the limitations of standard reconstruction-based quantization methods that fail to maintain post-training knowledge.
...
TL;DR: HAM layer combines RNNs and self-attention by using RNN to compress sequences and attention to supplement only hard-to-predict information, enabling controlled KV cache growth with smooth performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: RNNs and self-attention have orthogonal strengths: RNNs compress past into fixed-size state (efficient but degrade over long contexts), while self-attention stores every past step (good recall but high memory/computation). Prior combinations naively interleaved them without leveraging complementary mechanisms.

Method: Proposes Hybrid Associative Memory (HAM) layer that uses RNN to compress entire sequence while attention supplements only information difficult for RNN to predict. Enables data-dependent KV cache growth controlled by a single continuous threshold parameter.

Result: HAM offers fine-grained control over KV cache growth rate with smooth trade-off between loss and performance. Achieves strong competitive performance relative to RNNs and Transformers at substantially lower KV-cache usage.

Conclusion: HAM effectively combines RNN compression efficiency with attention's precise recall capability by selectively storing only valuable information, providing practical control over memory-computation trade-offs in sequence modeling.
...
TL;DR: The paper proposes a novel methodology combining majority-class undersampling and ensemble learning to improve Wind Power Ramp Event forecasting under class imbalance, achieving over 85% accuracy and 88% weighted F1 score.


<details>
  <summary>Details</summary>
Motivation: Wind Power Ramp Events (WPREs) are critical for grid stability but challenging to forecast due to inherent class imbalance in datasets (ramp events <15% of observations). Conventional ML models underperform by favoring the majority class, necessitating specialized approaches for effective early warning systems.

Method: 1) Formulates WPRE forecasting as multivariate time series classification; 2) Develops data preprocessing strategy extracting features from recent power observations while masking unavailable ramp information for integration with real-time identification tools; 3) Combines majority-class undersampling with ensemble learning to address class imbalance.

Result: Numerical simulations on real-world dataset demonstrate superiority: over 85% accuracy and 88% weighted F1 score, outperforming benchmark classifiers. The approach effectively handles class imbalance while maintaining high predictive performance.

Conclusion: The proposed methodology successfully addresses the class imbalance challenge in WPRE forecasting through strategic undersampling and ensemble learning, providing an effective decision support system for grid stability in low-carbon power systems with wind power plants.
...
### [219] [Beyond the Mean: Distribution-Aware Loss Functions for Bimodal Regression](https://arxiv.org/abs/2603.22328)
*Abolfazl Mohammadi-Seif,Carlos Soares,Rita P. Ribeiro,Ricardo Baeza-Yates*

Main category: cs.LG

TL;DR: Proposes distribution-aware loss functions (normalized RMSE + Wasserstein/Cramér distances) for deep regression to handle bimodal uncertainty without MDN instability, achieving better uncertainty estimation than standard losses and mixture models.


<details>
  <summary>Details</summary>
Motivation: Standard regression models struggle with reliable uncertainty estimation when predictive errors follow bimodal distributions (confident vs. ambiguous predictions). MSE assumes unimodal Gaussian noise, leading to mean-collapse, while Mixture Density Networks (MDNs) suffer from severe optimization instability.

Method: Proposes a family of distribution-aware loss functions combining normalized RMSE with Wasserstein and Cramér distances. Applied to standard deep regression models, these losses can recover bimodal distributions without the volatility of mixture models.

Result: Wasserstein loss establishes new Pareto efficiency frontier: matches MSE stability on unimodal tasks while reducing Jensen-Shannon Divergence by 45% on complex bimodal datasets. Framework strictly dominates MDNs in both fidelity and robustness across four experimental stages.

Conclusion: The proposed distribution-aware loss functions offer a reliable tool for aleatoric uncertainty estimation in trustworthy AI systems, addressing the critical challenge of assessing model trustworthiness through reliable predictive confidence estimates.
...
### [220] [Trained Persistent Memory for Frozen Decoder-Only LLMs](https://arxiv.org/abs/2603.22329)
*Hong Jeong*

Main category: cs.LG

TL;DR: Decoder-only LMs can gain persistent memory via self-attention adapters; architectural priors matter more than capacity for memory retention.


<details>
  <summary>Details</summary>
Motivation: To extend persistent latent-space memory from encoder-decoder to decoder-only transformers, where no cross-attention exists and memory must enter through self-attention alone.

Method: Adapt six memory methods (prefix, parallel cross-attention, KV extension, Hebbian memory, context-gated branch, slot-based sparse write) to frozen GPT-2, training only a small memory adapter θ_mem. Write rule is shared; read injection changes from decoder cross-attention to self-attention KV prefix or parallel branch.

Result: At 1× capacity, three methods with strong architectural priors (cross-attention, Hebbian, slot write) achieve 7-18% retained-memory scores and ΔK of 7-10, while others fail (<0.4%). At 10× capacity all six converge, showing the gap is architectural, not fundamental.

Conclusion: Persistent latent-space memory is a general paradigm spanning major transformer families; architectural priors are crucial for efficient memory retention in decoder-only models.
...
### [221] [Conformal Risk Control for Safety-Critical Wildfire Evacuation Mapping: A Comparative Study of Tabular, Spatial, and Graph-Based Models](https://arxiv.org/abs/2603.22331)
*Baljinnyam Dayan*

Main category: cs.LG

TL;DR: First application of conformal risk control to wildfire spread prediction provides formal safety guarantees (FNR ≤ 0.05), revealing standard thresholds capture only 7-72% of fire spread while CRC achieves ~95% coverage with ~15% flagged pixels.


<details>
  <summary>Details</summary>
Motivation: Current wildfire prediction models lack formal safety guarantees, leaving evacuation planners reliant on probability thresholds without assurance. No prior work has applied distribution-free safety guarantees to wildfire spread prediction despite extensive deep learning research in this domain.

Method: Applied conformal risk control (CRC) to wildfire spread prediction across three model families: tabular (LightGBM), convolutional (Tiny U-Net), and graph-based (Hybrid ResGNN-UNet). Introduced shift-aware three-way CRC framework for operational triage with SAFE/MONITOR/EVACUATE zones.

Result: Standard thresholds captured only 7-72% of true fire spread across models. CRC achieved ~95% fire coverage while flagging only ~15% of total pixels. Spatial models with CRC were 4.2x more efficient than LightGBM. Graph model complexity provided no meaningful efficiency gain over simple U-Net.

Conclusion: Model architecture determines evacuation efficiency while CRC determines safety. CRC eliminates failure of standard thresholds and provides formal guarantees. Shift-aware three-way CRC framework enables operational triage. Prevalence-weighted bounds have fundamental limitations under extreme class imbalance (~5% fire prevalence).
...
### [222] [Large Language Models for Missing Data Imputation: Understanding Behavior, Hallucination Effects, and Control Mechanisms](https://arxiv.org/abs/2603.22332)
*Arthur Dantas Mangussi,Ricardo Cardoso Pereira,Ana Carolina Lorena,Pedro Henriques Abreu*

Main category: cs.LG

TL;DR: LLMs (especially Gemini 3.0 Flash and Claude 4.5 Sonnet) outperform traditional imputation methods on real-world tabular datasets but are computationally expensive and perform worse on synthetic data, suggesting their effectiveness relies on semantic context from pre-training rather than pure statistical reconstruction.


<details>
  <summary>Details</summary>
Motivation: Prior LLM-based imputation studies face scalability limitations, restricted cross-model comparisons, small/domain-specific datasets, heterogeneous experimental protocols, and inconsistent treatment of missingness mechanisms (MCAR, MAR, MNAR), hindering systematic benchmarking.

Method: Comprehensive benchmarking comparing five widely used LLMs against six state-of-the-art imputation baselines across 29 datasets (including nine synthetic datasets) under MCAR, MAR, and MNAR mechanisms with missing rates up to 20%, using zero-shot prompt engineering.

Result: Leading LLMs (Gemini 3.0 Flash and Claude 4.5 Sonnet) consistently achieve superior performance on real-world open-source datasets compared to traditional methods, but traditional methods (like MICE) outperform LLMs on synthetic datasets. LLMs incur significantly higher computational time and monetary costs.

Conclusion: LLMs are promising semantics-driven imputers for complex tabular data, with effectiveness tied to prior exposure to domain-specific patterns from pre-training rather than pure statistical reconstruction, but face computational cost trade-offs.
...
### [223] [Problems with Chinchilla Approach 2: Systematic Biases in IsoFLOP Parabola Fits](https://arxiv.org/abs/2603.22339)
*Eric Czech,Zhiwei Xu,Yael Elmatad,Yixin Wang,William Held*

Main category: cs.LG

TL;DR: Chinchilla Approach 2 has systematic biases in compute-optimal allocation estimates due to parabolic approximation errors, leading to significant parameter underallocation and unnecessary compute costs. Approach 3 eliminates these biases and is shown to be practical when using Variable Projection for efficient optimization.


<details>
  <summary>Details</summary>
Motivation: Chinchilla Approach 2, widely used for fitting neural scaling laws, introduces systematic biases in compute-optimal allocation estimates even on noise-free data, leading to significant computational waste and parameter misallocation in large-scale models like Llama 3.

Method: The paper examines three error sources in Approach 2: IsoFLOP sampling grid width (Taylor approximation accuracy), uncentered IsoFLOP sampling, and loss surface asymmetry (α≠β). It proposes using Chinchilla Approach 3 with Variable Projection, which exploits the partially linear structure of the objective to enable unbiased inference through a two-dimensional optimization that is well-conditioned and analytically differentiable.

Result: Applied to Llama 3 IsoFLOP data, Approach 2 biases imply parameter underallocation corresponding to 6.5% of the $3.8×10²⁵ FLOP training budget and $1.4M in unnecessary compute. Simulated multimodal models show even greater opportunity costs. Approach 3 with Variable Projection eliminates these biases and addresses concerns about data efficiency, numerical stability, and implementation difficulty.

Conclusion: Chinchilla Approach 3, when implemented with Variable Projection, provides unbiased compute-optimal allocation estimates and is a practical replacement for Approach 2 or scalable alternative for richer scaling law formulations, overcoming perceived limitations through efficient optimization techniques.
...
### [224] [Cloud-Edge Collaborative Large Models for Robust Photovoltaic Power Forecasting](https://arxiv.org/abs/2603.22343)
*Nan Qiao,Sijing Duan,Shuning Wang,Xingyuan Hua,Ju Ren*

Main category: cs.LG

TL;DR: A risk-aware cloud-edge collaborative framework for PV forecasting that balances accuracy, robustness, and latency by integrating local specialized models, edge-side lightweight models, and cloud-based retrieval models with intelligent routing.


<details>
  <summary>Details</summary>
Motivation: PV forecasting in edge-enabled grids requires balancing accuracy, robustness under weather-driven distribution shifts, and strict latency constraints. Local models degrade under rare events, while cloud models incur communication delays and overhead.

Method: Proposes a three-component framework: 1) site-specific expert predictor for routine cases, 2) lightweight edge-side model for local inference, 3) cloud-side large retrieval model with retrieval-prediction pipeline. Includes lightweight screening module for uncertainty/risk assessment and Lyapunov-guided router for selective escalation under constraints, with adaptive fusion of outputs.
...
Conclusion: The risk-aware cloud-edge collaborative framework effectively addresses the challenges of PV forecasting in edge-enabled grids by intelligently combining local and cloud resources while respecting latency and resource constraints.
...
### [225] [First-Mover Bias in Gradient Boosting Explanations: Mechanism, Detection, and Resolution](https://arxiv.org/abs/2603.22346)
*Drake Caraker,Bryan Arnold,David Rhoads*

Main category: cs.LG

TL;DR: First-mover bias in gradient boosting causes unstable SHAP feature rankings under multicollinearity; DASH method uses model independence to restore stability.


<details>
  <summary>Details</summary>
Motivation: To identify and address the mechanistic cause of SHAP feature ranking instability under multicollinearity, specifically the first-mover bias phenomenon in gradient boosting where sequential residual fitting creates path-dependent concentration of feature importance.

Method: Proposes DASH (Diversified Aggregation of SHAP) which breaks sequential dependency through model independence, using either ensemble averaging or seed-averaging (Stochastic Retrain). Also introduces diagnostic tools: Feature Stability Index (FSI) and Importance-Stability (IS) Plot for detecting first-mover bias.

Result: At correlation rho=0.9, DASH and seed-averaging achieve stability=0.977 vs. 0.958 for single-best workflow and 0.938 for Large Single Model. On Breast Cancer dataset, DASH improves stability from 0.32 to 0.93 (+0.61). Diagnostic tools successfully detect bias without ground truth.

Conclusion: First-mover bias is a key mechanistic cause of SHAP instability under multicollinearity; model independence (via DASH or seed-averaging) effectively mitigates this bias; diagnostic tools enable practical auditing of explanation reliability.
...
### [226] [COMPASS-Hedge: Learning Safely Without Knowing the World](https://arxiv.org/abs/2603.22348)
*Ting Hu,Luanda Cai,Manolis Vlatakis*

Main category: cs.LG

TL;DR: COMPASS-Hedge is a parameter-free full-information online learning algorithm that simultaneously achieves optimal adversarial regret, instance-optimal stochastic regret, and baseline safety without requiring prior knowledge of environment parameters.


<details>
  <summary>Details</summary>
Motivation: Existing online learning algorithms face a fundamental trilemma: they cannot simultaneously achieve optimal regret in adversarial environments, instance-optimal gap-dependent regret in stochastic environments, and baseline safety against a fixed comparator without sacrificing optimal rates or requiring oracle access to problem-dependent parameters.

Method: COMPASS-Hedge integrates adaptive pseudo-regret scaling and phase-based aggression with a comparator-aware mixing strategy. The algorithm is parameter-free and requires no prior knowledge of the environment's nature or the magnitude of stochastic suboptimality gaps.

Result: COMPASS-Hedge achieves: 1) Minimax-optimal regret in adversarial environments; 2) Instance-optimal, gap-dependent regret in stochastic environments; and 3) $\tilde{\mathcal{O}}(1)$ regret relative to a designated baseline policy (up to logarithmic factors).

Conclusion: This work provides the first "best-of-three-world" guarantee in the full-information setting, demonstrating that baseline safety does not have to come at the cost of worst-case robustness or stochastic efficiency.
...
### [227] [WIST: Web-Grounded Iterative Self-Play Tree for Domain-Targeted Reasoning Improvement](https://arxiv.org/abs/2603.22352)
*Fangyuan Li,Pengfei Li,Shijie Wang,Junqi Gao,Jianxing Liu,Biqing Qi,Yuqiang Li*

Main category: cs.LG

TL;DR: WIST is a web-grounded iterative self-play tree framework that improves language model reasoning by learning directly from the open web without curated data, using domain tree exploration, web corpus retrieval, and challenger-solver self-play with verifiable rewards.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods face a trade-off: endogenous self-play can drift over iterations, while corpus-grounded approaches require curated data environments. There's a need for methods that can learn directly from the open web without pre-arranged domain corpora.
...
Result: Across four backbones, WIST consistently improves over base models and typically outperforms both purely endogenous self-evolution and corpus-grounded self-play baselines, with overall gains reaching +9.8 (Qwen3-4B-Base) and +9.7 (OctoThinker-8B). Domain-steerable improvements include +14.79 in medicine for Qwen3-8B-Base and +5.28 on PhyBench for Qwen3-4B-Base.

Conclusion: WIST provides an effective framework for domain-targeted reasoning improvement that learns directly from the open web without requiring curated data, with ablations confirming the importance of its key components for stable open-web learning.
...
### [228] [Unveiling the Mechanism of Continuous Representation Full-Waveform Inversion: A Wave Based Neural Tangent Kernel Framework](https://arxiv.org/abs/2603.22362)
*Ruihua Chen,Yisi Luo,Bangyu Wu,Deyu Meng*

Main category: cs.LG

TL;DR: The paper develops a theoretical framework using wave-based neural tangent kernel (NTK) to explain why continuous representation FWI (CR-FWI) reduces initial model dependency but has slower high-frequency convergence, and proposes improved CR-FWI methods including a hybrid INR-grid representation.


<details>
  <summary>Details</summary>
Motivation: Full-waveform inversion (FWI) is sensitive to initial model accuracy. While recent continuous representation FWI (CR-FWI) using implicit neural representations (INR) reduces this dependency, its underlying mechanism is unclear and it suffers from slower high-frequency convergence.

Method: Developed a unified theoretical framework by extending neural tangent kernel (NTK) to wave-based NTK for FWI analysis. Investigated eigenvalue decay behavior of wave-based NTK to explain CR-FWI properties. Proposed several CR-FWI methods with tailored eigenvalue decay, including a novel hybrid representation combining INR and multi-resolution grid (IG-FWI).

Result: Wave-based NTK analysis reveals it's not constant due to FWI nonlinearity, and its eigenvalue decay explains CR-FWI's reduced initial model dependency and slower high-frequency convergence. The proposed methods, especially IG-FWI, achieve better trade-off between robustness and convergence rate, showing superior performance on multiple geophysical models.

Conclusion: The wave-based NTK framework provides theoretical understanding of CR-FWI mechanisms. The proposed hybrid INR-grid representation (IG-FWI) offers improved balance between robustness to initial models and high-frequency convergence, advancing FWI methodology for practical applications.
...
### [229] [FAAR: Format-Aware Adaptive Rounding for NVFP4](https://arxiv.org/abs/2603.22370)
*Hanglin Li,Shuchang Tian,Chen Lin,Zhiyong Zhao,Kun Zhan*

Main category: cs.LG

TL;DR: FAAR is a learnable rounding strategy for NVFP4 quantization that adapts to the format's non-uniform grid, combined with 2FA fine-tuning to align LLM parameters, achieving better perplexity with minimal training overhead.


<details>
  <summary>Details</summary>
Motivation: Existing quantization methods for ultra-low precision formats like NVFP4 use conventional rounding strategies that don't account for the format's non-uniform numerical grid, leading to suboptimal rounding decisions and amplified quantization errors when deploying LLMs on edge devices.

Method: Proposes Format-Aware Adaptive Rounding (FAAR) - a learnable rounding strategy that explicitly incorporates the non-uniform NVFP4 grid into optimization, adaptively adjusting rounding decisions guided by loss gradients. Also introduces 2-stages Format Alignment (2FA) fine-tuning scheme that aligns LLM parameters layer-by-layer to the NVFP4 numerical space.

Result: Reduces perplexity on WikiText-2 from 14.28 to 12.60 on Llama3-1B and from 23.06 to 21.27 on Qwen3-1.7B compared to RTN. Consistently outperforms state-of-the-art approaches across various zero-shot downstream tasks. Achieves this with minimal training overhead of only 4 GPU hours on Llama3-1B.

Conclusion: FAAR with 2FA fine-tuning provides an effective solution for ultra-low-bit quantization of LLMs to NVFP4 format, addressing the limitations of conventional rounding methods by explicitly accounting for the format's non-uniform numerical characteristics while maintaining practical training efficiency.
...
### [230] [Rethinking Multimodal Fusion for Time Series: Auxiliary Modalities Need Constrained Fusion](https://arxiv.org/abs/2603.22372)
*Seunghan Lee,Jun Seo,Jaehoon Lee,Sungdong Yoo,Minjae Kim,Tae Yoon Lim,Dongwan Kang,Hwanil Choi,SoonYoung Lee,Wonbin Ahn*

Main category: cs.LG

TL;DR: CFA is a plug-in adapter method for controlled fusion of text with time series forecasting that filters irrelevant textual information using low-rank adapters, outperforming naive fusion approaches.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal time series forecasting methods show limited gains, often underperforming unimodal models due to uncontrolled integration of irrelevant auxiliary information from modalities like text.

Method: Proposes Controlled Fusion Adapter (CFA), a plug-in method using low-rank adapters to filter irrelevant textual information before fusing it with temporal representations, enabling controlled cross-modal interactions without modifying the time series backbone.

Result: Extensive experiments (over 20K across various datasets and models) demonstrate that constrained fusion methods including CFA consistently outperform naive fusion approaches.

Conclusion: Controlled fusion is essential for effective multimodal time series forecasting, and CFA provides a simple yet effective plug-in solution for integrating only relevant textual information aligned with time series dynamics.
...
### [231] [Three Creates All: You Only Sample 3 Steps](https://arxiv.org/abs/2603.22375)
*Yuren Cai,Guangyi Wang,Zongqing Li,Li Li,Zhihui Liu,Songzhi Su*

Main category: cs.LG

TL;DR: MTEO optimizes time embeddings for few-step diffusion sampling by distilling layer-wise embeddings from reference trajectories, achieving SOTA performance with minimal parameter training and no inference overhead.


<details>
  <summary>Details</summary>
Motivation: Standard timestep conditioning becomes a bottleneck for few-step sampling in diffusion models, limiting inference speed despite high-fidelity generation capabilities.

Method: Multi-layer Time Embedding Optimization (MTEO) freezes pretrained diffusion backbone and distills a small set of step-wise, layer-wise time embeddings from reference trajectories. It's plug-and-play with existing ODE solvers and trains only a tiny fraction of parameters.
...
Conclusion: MTEO provides an effective solution for accelerating diffusion model inference through optimized time embeddings, enabling high-quality few-step sampling without modifying the backbone architecture or adding inference overhead.
...
### [232] [Instruction-Tuned, but Not More Verifiable Instruction-Following: A Cross-Task Diagnosis for LoRA Adapters](https://arxiv.org/abs/2603.22379)
*Junyi Zou*

Main category: cs.LG

TL;DR: Adapters' nominal training objectives often misalign with actual cross-task capability gains, with instruction-tuned adapters sometimes improving off-target numeric performance while failing to improve verifiable instruction following, a pattern termed "capability drift."


<details>
  <summary>Details</summary>
Motivation: To test whether nominal training objectives (e.g., "instruction-tuned") reliably predict actual cross-task capability improvements, as adapters are often selected based on these labels which implicitly suggest what capabilities improve after adaptation.

Method: Evaluated the same LoRA adapter across multiple tasks, focusing on strict automatically verifiable instruction following measured by IFEval. Used multiple seeds, base models, and LoRA settings in controlled experiments, including an instruction-versus-numeric setting to illustrate capability drift patterns.

Result: Nominal labels recurrently but not universally fail to predict improvements on verifiable instruction following (IFEval), with clear configuration sensitivity including near-zero or negative cases. In the strongest-case example, an instruction-tuned adapter substantially improved off-target numeric benchmark performance (0.133 to 0.632) while not improving verifiable instruction following on IFEval (ILA: 0.313 to 0.271; PLA: 0.250 to 0.143).

Conclusion: The nominal-versus-realized mismatch pattern (termed "capability drift") demonstrates that nominal labels are unreliable capability proxies. The practical recommendation is to perform routine cross-task evaluation before deployment and avoid treating nominal labels as reliable indicators of actual capability improvements.
...
### [233] [Symbolic Graph Networks for Robust PDE Discovery from Noisy Sparse Data](https://arxiv.org/abs/2603.22380)
*Xingyu Chen,Junxiu An,Jun Guo,Yuqian Zhou*

Main category: cs.LG

TL;DR: SGN framework combines graph message passing with symbolic regression for robust PDE discovery from noisy, sparse data without numerical differentiation.


<details>
  <summary>Details</summary>
Motivation: Existing PDE discovery methods struggle with noisy and sparse observational data due to reliance on numerical differentiation or integral formulations, which are sensitive to noise and limited sampling.

Method: Symbolic Graph Network (SGN) uses graph message passing to model spatial interactions non-locally, creating noise-resistant latent features, then applies symbolic regression to extract interpretable mathematical expressions.

Result: SGN successfully recovers governing equations for wave, convection-diffusion, and Navier-Stokes systems under varying noise levels, showing improved robustness over baselines in sparse, noisy conditions.

Conclusion: Graph-based representations combined with symbolic regression provide a viable approach for robust data-driven discovery of physical laws from imperfect observational data.
...
### [234] [Learning When to Act: Interval-Aware Reinforcement Learning with Predictive Temporal Structure](https://arxiv.org/abs/2603.22384)
*Davide Di Gioia*

Main category: cs.LG

TL;DR: A lightweight adaptive temporal control system learns optimal action intervals using hyperbolic geometry-based predictive signals, with interval-aware rewards and spatio-temporal embeddings improving efficiency.


<details>
  <summary>Details</summary>
Motivation: Autonomous agents in continuous environments need to decide both what to do and when to act, requiring principled learned policies rather than ad hoc biologically inspired timers.

Method: Introduces adaptive temporal control with hyperbolic spread signal (mean pairwise Poincaré distance among sampled futures), interval-aware reward penalizing inefficiency relative to wait time, and joint spatio-temporal embedding (ATCPG-ST) combining state and position projections in Poincaré ball.

Result: System achieves 22.8% efficiency over fixed-interval baseline; hyperbolic spread provides 26.2% gain over geometry-free control; spatial position adds 5.8% efficiency; mean hyperbolic spread increases from 1.88 to 3.37 with spatio-temporal embedding.

Conclusion: Learning optimal action intervals with hyperbolic geometry-based predictive signals and interval-aware rewards significantly improves agent efficiency, with spatial trajectory divergence providing complementary timing information beyond state-only approaches.
...
### [235] [Neural Structure Embedding for Symbolic Regression via Continuous Structure Search and Coefficient Optimization](https://arxiv.org/abs/2603.22429)
*Fateme Memar,Tao Zhe,Dongjie Wang*

Main category: cs.LG

TL;DR: SRCO is a symbolic regression framework that uses continuous embeddings of symbolic structures to enable efficient gradient-based optimization, outperforming traditional discrete search methods.


<details>
  <summary>Details</summary>
Motivation: Traditional symbolic regression methods rely on discrete structure search (e.g., genetic programming), which suffers from high computational cost, unstable performance, and poor scalability to large equation spaces.

Method: Three-component framework: (1) Structure embedding: train Transformer to compress symbolic structures into continuous embedding space using equations from traditional methods; (2) Continuous structure search: use gradient-based or sampling-based optimization in embedding space; (3) Coefficient optimization: treat symbolic coefficients as learnable parameters for gradient optimization.
...
Conclusion: SRCO introduces a new paradigm for symbolic regression by bridging symbolic equation discovery with continuous embedding learning and optimization, overcoming limitations of traditional discrete search approaches.
...
### [236] [Model Predictive Control with Differentiable World Models for Offline Reinforcement Learning](https://arxiv.org/abs/2603.22430)
...
<details>
  <summary>Details</summary>
Motivation: Existing offline RL methods use fixed policies at inference time without adaptation, missing opportunities to leverage inference-time information for better performance

Method: Differentiable World Model (DWM) pipeline enabling end-to-end gradient computation through imagined rollouts for MPC-based policy optimization at inference time

Result: Consistent gains over strong offline RL baselines on D4RL continuous-control benchmarks (MuJoCo locomotion tasks and AntMaze)

Conclusion: Inference-time adaptation using differentiable world models and MPC significantly improves offline RL performance by optimizing policy parameters on-the-fly
...
### [237] [SkillRouter: Retrieve-and-Rerank Skill Selection for LLM Agents at Scale](https://arxiv.org/abs/2603.22455)
*YanZhao Zheng,ZhenTao Zhang,Chao Ma,YuanQiang Yu,JiHuan Zhu,Baohua Dong,Hangcheng Zhu*

Main category: cs.LG

TL;DR: SkillRouter is a two-stage retrieve-and-rerank pipeline for LLM agent skill routing that achieves 74.0% top-1 accuracy by leveraging full skill implementation text, challenging the assumption that metadata alone is sufficient for skill selection.


<details>
  <summary>Details</summary>
Motivation: As LLM agent ecosystems grow with tens of thousands of available skills, skill routing becomes essential but remains under-explored. Current architectures use progressive disclosure (showing only skill names/descriptions) assuming metadata is sufficient, but this assumption is challenged by pervasive functional overlap where skills share similar names/purposes but differ in implementation details.

Method: SkillRouter is a two-stage retrieve-and-rerank pipeline with 1.2B total parameters (0.6B encoder + 0.6B reranker). The approach is based on the key finding that skill body (full implementation text) is the decisive signal for routing. The system first retrieves candidate skills then reranks them using cross-encoder attention analysis.

Result: Empirical study on ~80K skills and 75 expert-verified queries shows: 1) Removing skill body causes 29-44 percentage point degradation across all retrieval methods; 2) Cross-encoder attention analysis reveals 91.7% of attention concentrates on the body field; 3) SkillRouter achieves 74.0% top-1 routing accuracy, delivering the strongest average result among compact and zero-shot baselines while remaining deployable on consumer hardware.

Conclusion: The skill body (full implementation text) is crucial for effective skill routing, challenging the progressive disclosure assumption. SkillRouter demonstrates that leveraging implementation details through a compact two-stage pipeline enables accurate skill selection in large-scale LLM agent ecosystems while maintaining practical deployability.
...
### [238] [A Theoretical Framework for Energy-Aware Gradient Pruning in Federated Learning](https://arxiv.org/abs/2603.22465)
*Emmanouil M. Athanasakos*

Main category: cs.LG

TL;DR: CWMP is an energy-aware gradient sparsification method for federated learning that prioritizes parameter updates based on magnitude relative to hardware costs, outperforming traditional Top-K pruning in energy-performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: Current federated learning gradient sparsification methods like Top-K pruning are energy-agnostic, assuming uniform transmission and memory-update costs for all parameter updates, which ignores hardware realities and energy limitations of edge devices.

Method: Formalizes pruning as an energy-constrained projection problem accounting for hardware-level disparities between memory-intensive and compute-efficient operations. Proposes Cost-Weighted Magnitude Pruning (CWMP) that selects parameter updates based on magnitude relative to physical cost, proven to be the optimal greedy solution.

Result: CWMP consistently establishes a superior performance-energy Pareto frontier compared to Top-K baseline on non-IID CIFAR-10 benchmark, with probabilistic analysis confirming its global energy efficiency.

Conclusion: CWMP provides an energy-aware gradient sparsification approach that accounts for hardware realities in federated learning, offering better energy-performance trade-offs than traditional magnitude-based pruning methods.
...
### [239] [Adversarial Vulnerabilities in Neural Operator Digital Twins: Gradient-Free Attacks on Nuclear Thermal-Hydraulic Surrogates](https://arxiv.org/abs/2603.22525)
*Samrendra Roy,Kazuma Kobayashi,Souvik Chakraborty,Rizwan-uddin,Syed Bahauddin Alam*

Main category: cs.LG

TL;DR: Neural operators are highly vulnerable to sparse adversarial perturbations targeting boundary conditions, with gradient-free attacks causing catastrophic prediction failures while remaining undetectable by standard validation metrics.


<details>
  <summary>Details</summary>
Motivation: Operator learning models are becoming crucial for digital twins in nuclear and energy systems, but their robustness to adversarial perturbations remains uncharacterized, creating a critical safety gap for deployment in safety-critical applications.

Method: Used gradient-free differential evolution across four operator architectures to find sparse, physically plausible perturbations. Introduced effective perturbation dimension (d_eff) as a Jacobian-based diagnostic and developed a two-factor vulnerability model combining sensitivity magnitude and concentration.

Result: Minimal modifications (fewer than 1% of inputs) trigger catastrophic failures, increasing relative L2 error from ~1.5% to 37-63%. 100% of successful single-point attacks pass z-score anomaly detection. Gradient-free search outperforms gradient-based alternatives on architectures with gradient pathologies.
...
### [240] [Multimodal Training to Unimodal Deployment: Leveraging Unstructured Data During Training to Optimize Structured Data Only Deployment](https://arxiv.org/abs/2603.22530)
*Zigui Wang,Minghui Sun,Jiang Shu,Matthew M. Engelhard,Lauren Franz,Benjamin A. Goldstein*

Main category: cs.LG

TL;DR: A multimodal learning framework that uses unstructured EHR data during training but deploys using only structured data, improving late-talking phenotype classification performance.


<details>
  <summary>Details</summary>
Motivation: Unstructured EHR data (clinical notes) contain valuable clinical context not captured in structured fields, but are often unavailable or impractical for model deployment. There's a need to leverage this rich information during training while maintaining deployability with structured-only data.

Method: Multimodal learning framework with teacher-student architecture: note-based teacher model (BioClinicalBERT embeddings) and structured-only student model (demographics + medical codes). Joint training using contrastive learning and contrastive knowledge distillation loss to transfer knowledge from unstructured to structured data.

Result: Teacher model achieved AUROC = 0.985. Proposed structured-only student model reached AUROC = 0.705, outperforming structured-only baseline of 0.656. Demonstrates successful knowledge transfer from unstructured to structured data.

Conclusion: Incorporating unstructured EHR data during training enhances model's ability to identify task-relevant information within structured data, enabling development of deployable structured-only phenotype models that benefit from richer training context.
...
### [241] [MIOFlow 2.0: A unified framework for inferring cellular stochastic dynamics from single cell and spatial transcriptomics data](https://arxiv.org/abs/2603.22564)
...
TL;DR: MIOFlow 2.0 is a computational framework that integrates manifold learning, optimal transport, and neural differential equations to learn biologically informed cellular trajectories from time-resolved single-cell transcriptomics data, addressing stochastic branching, population changes, and spatial influences.


<details>
  <summary>Details</summary>
Motivation: Current methods for inferring continuous cellular trajectories from discrete snapshots fail to capture key biological complexities: stochastic cell fate decisions, temporal proliferation changes, and spatial environmental influences. Deterministic interpolations treat cells in isolation, missing probabilistic branching, population shifts, and niche-dependent signaling.

Method: MIOFlow 2.0 integrates manifold learning, optimal transport, and neural differential equations. It models: (1) stochasticity and branching via Neural Stochastic Differential Equations; (2) non-conservative population changes using a learned growth-rate model initialized with unbalanced optimal transport; (3) environmental influence through a joint latent space unifying gene expression with spatial features. It operates in a PHATE-distance matching autoencoder latent space to respect intrinsic data geometry.
...
Conclusion: MIOFlow 2.0 bridges single-cell and spatial transcriptomics to uncover tissue-scale trajectories, providing a comprehensive framework for learning biologically informed cellular trajectories that capture the complexity of real biological processes.
...
### [242] [A Foundation Model for Instruction-Conditioned In-Context Time Series Tasks](https://arxiv.org/abs/2603.22586)
*Anish Saha,Konstantin Shmakov*

Main category: cs.LG

TL;DR: A foundation model for instruction-conditioned in-context time-series tasks using quantile-regression T5 encoder-decoder with structured tokenization and hierarchical Transformer architecture, trained on multi-task objectives to enable forecasting and related tasks without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing time-series foundation models lack explicit instruction-conditioned demonstrations for in-context learning, relying instead on implicit positional context, retrieval, or task-specific objectives. The paper aims to address this gap by developing a model that can adapt to various time-series tasks at inference time through explicit demonstration examples.

Method: Uses a quantile-regression T5 encoder-decoder with structured tokenization scheme marking target series, covariates, context, and task-specific future information. Implements hierarchical Transformer with per-example encoding, example-level fusion, and cross-example attention to condition decoding on demonstration pairs. Trained on large-scale real and synthetic time series using supervised forecasting plus self-supervised tasks including imputation, reconstruction, classification, anomaly detection, and source demixing.

Result: Outperforms strong foundation baselines on point and probabilistic forecasting benchmarks (including fev-bench and GIFT-Eval) across diverse datasets, frequencies, and horizons. Remains competitive on classification and anomaly detection tasks without task-specific fine-tuning.

Conclusion: The proposed instruction-conditioned in-context learning approach for time-series tasks enables effective adaptation to various tasks through explicit demonstrations, with multi-task training learning a distribution over task mappings that improves adaptation to local structure at inference time.
...
### [243] [Precision-Varying Prediction (PVP): Robustifying ASR systems against adversarial attacks](https://arxiv.org/abs/2603.22590)
*Matías Pizarro,Raghavan Narasimhan,Asja Fischer*

Main category: cs.LG

TL;DR: Random precision sampling during ASR inference improves adversarial robustness and enables detection via output comparison across precisions.


<details>
  <summary>Details</summary>
Motivation: As automated speech recognition systems become more prevalent, their vulnerability to adversarial attacks poses a critical security concern, necessitating improved robustness measures.

Method: Random sampling of model precision during inference to disrupt adversarial attacks, combined with a Gaussian classifier for detection by comparing outputs across different precisions.

Result: Experimental analysis shows significant robustness improvement and competitive detection performance across various ASR models and attack types.

Conclusion: Precision manipulation during inference provides an effective defense strategy against adversarial attacks on ASR systems, enhancing both robustness and detection capabilities.
...
### [244] [Causal Discovery in Action: Learning Chain-Reaction Mechanisms from Interventions](https://arxiv.org/abs/2603.22620)
...
<details>
  <summary>Details</summary>
Motivation: Many real-world systems exhibit directional, cascade-like structure where components activate sequentially and upstream failures suppress downstream effects. Causal discovery in general dynamical systems is challenging without strong structural assumptions, but chain-reaction systems offer a more tractable setting.

Method: Study causal discovery in chain-reaction systems using blocking interventions that prevent individual components from activating. Propose a minimal estimator with finite-sample guarantees.

Result: Causal structure is uniquely identifiable from blocking interventions. The estimator achieves exponential error decay and logarithmic sample complexity. Experiments on synthetic models and diverse chain-reaction environments demonstrate reliable recovery from few interventions.

Conclusion: Chain-reaction systems enable unique causal identifiability through blocking interventions, overcoming limitations of observational methods that fail with delayed or overlapping causal effects.
...
### [245] [Transfer learning via interpolating structures](https://arxiv.org/abs/2603.22621)
*T. A. Dardeno,A. J. Hughes,L. A. Bull,R. S. Mills,N. Dervilis,K. Worden*

Main category: cs.LG

TL;DR: The paper proposes using intermediate structures to enable knowledge transfer between highly-disparate systems in population-based structural health monitoring, demonstrating successful transfer between heterogeneous bridges and even between bridges and airplanes through parameterized morphing.


<details>
  <summary>Details</summary>
Motivation: Knowledge transfer between highly-disparate structures (heterogeneous populations) remains challenging in PBSHM, despite recent advances. The paper addresses the difficulty of transferring structural health monitoring knowledge between systems with significant differences in design, materials, and geometry.

Method: The approach uses intermediate structures that bridge the information gap between disparate systems. Key innovation: varying parameters (material properties, geometry) to continuously morph one structure into another. Demonstrated through two case studies: 1) parameterization and transfer between simulated heterogeneous bridge designs, 2) transfer between simplified physical representations of a 'bridge' and 'aeroplane' via a chain of finite-element models.

Result: The results show that positive transfer can be achieved between highly-disparate systems. While the facetious question "When is a bridge not an aeroplane?" suggests they're always different, the paper demonstrates that in some cases, positive knowledge transfer is possible even between such fundamentally different structures.

Conclusion: The proposed approach of using intermediate structures and parameterized morphing enables knowledge transfer between heterogeneous populations in PBSHM. This challenges conventional assumptions about structural similarity requirements for positive transfer, showing that even highly-disparate systems like bridges and airplanes can share transferable knowledge through appropriate bridging techniques.
...
### [246] [Generalizing Dynamics Modeling More Easily from Representation Perspective](https://arxiv.org/abs/2603.22655)
*Yiming Wang,Zhengnan Zhang,Genghe Zhang,Jiawen Dan,Changchun Li,Chenlong Hu,Chris Nugent,Jun Liu,Ximing Li,Bo Yang*

Main category: cs.LG

TL;DR: PDEDER is a pre-trained dynamics encoder that learns generalized latent representations from multiple complex systems, enabling better dynamics modeling through Lyapunov exponent regularization and reconstruction/forecasting objectives.


<details>
  <summary>Details</summary>
Motivation: Existing neural dynamics modeling methods create specific models for each observation system, lacking generalization across different complex systems. There's a need for a generalized approach that can learn transferable dynamics representations.

Method: Pre-train a Language Model (PLM) as PDEDER using 152 datasets from 23 complex systems. Minimize Lyapunov exponent objective to constrain chaotic behavior, penalize divergence for stable latent dynamics, and incorporate reconstruction/forecasting objectives to prevent over-smoothing.

Result: PDEDER achieves effective and generalizable performance on 12 dynamic systems in both in-domain and cross-domain settings for short/long-term forecasting tasks.

Conclusion: PDEDER provides a generalized pre-trained dynamics encoder that learns transferable latent representations, enabling better dynamics modeling across diverse complex systems through stability-constrained pre-training.
...
### [247] [Bounding Box Anomaly Scoring for simple and efficient Out-of-Distribution detection](https://arxiv.org/abs/2603.22660)
*Mohamed Bahi Yahiaoui,Geoffrey Daniel,Loïc Giraldi,Jérémie Bruyelle,Julyan Arbel*

Main category: cs.LG

TL;DR: BBAS is a post-hoc OOD detection method using bounding-box abstraction that combines graded anomaly scores, convolutional layer monitoring, and decoupled clustering for robust performance while maintaining simplicity and compactness.


<details>
  <summary>Details</summary>
Motivation: Current OOD detection methods face a trade-off between compact parametric models (like Mahalanobis) and flexible reference-based methods (like k-NN). Bounding-box abstraction offers an intermediate solution that combines compactness with representational flexibility, but existing approaches need improvement for richer representations and better performance.

Method: BBAS uses bounding-box abstraction with three key innovations: 1) Graded anomaly scores based on interval exceedances, 2) Monitoring variables adapted specifically for convolutional layers, and 3) Decoupled clustering and box construction to enable richer multi-layer representations while maintaining the bounding-box approach's simplicity.

Result: Experiments on image-classification benchmarks demonstrate that BBAS provides robust separation between in-distribution and out-of-distribution samples while preserving the bounding-box approach's simplicity, compactness, and updateability.

Conclusion: BBAS successfully bridges the gap between compact parametric models and flexible reference-based methods for OOD detection, offering an effective bounding-box abstraction approach with improved performance through graded scoring, convolutional adaptations, and decoupled representation construction.
...
### [248] [Vision-based Deep Learning Analysis of Unordered Biomedical Tabular Datasets via Optimal Spatial Cartography](https://arxiv.org/abs/2603.22675)
*Sakib Mostafa,Tarik Massoud,Maximilian Diehn,Lei Xing,Md Tauhidul Islam*

Main category: cs.LG

TL;DR: Dynomap is a deep learning framework that learns optimal spatial arrangements of tabular features, enabling vision architectures to effectively process unordered biomedical data by transforming feature vectors into learned feature maps.


<details>
  <summary>Details</summary>
Motivation: Tabular biomedical data lacks intrinsic spatial organization, limiting vision architectures' ability to exploit local structure and higher-order feature interactions. Current approaches treat features as unordered dimensions, requiring models to implicitly infer relationships rather than leveraging spatial patterns.

Method: Dynamic Feature Mapping (Dynomap) jointly optimizes feature placement and prediction through a fully differentiable rendering mechanism. It learns task-optimized spatial topologies directly from data without relying on heuristics, predefined groupings, or external priors, transforming high-dimensional tabular vectors into learned feature maps.

Result: Dynomap consistently outperformed classical ML, modern deep tabular models, and existing vector-to-image approaches across multiple clinical and biological datasets. It improved multiclass cancer subtype prediction accuracy by up to 18% in liquid biopsy data and Parkinson disease voice classification accuracy by up to 8%, while organizing clinically relevant features into coherent spatial patterns.

Conclusion: Dynomap establishes a general strategy for bridging tabular and vision-based deep learning, enabling the discovery of structured, clinically relevant patterns in high-dimensional biomedical data through learned spatial feature organization.
...
### [249] [Coordinate Encoding on Linear Grids for Physics-Informed Neural Networks](https://arxiv.org/abs/2603.22700)
*Tetsuro Tsuchino,Motoki Shiga*

Main category: cs.LG

TL;DR: Proposes a PINN method with coordinate-encoding layer on linear grid cells to improve training convergence speed by separating local domains and using axis-independent linear grid cells with natural cubic spline interpolation.


<details>
  <summary>Details</summary>
Motivation: PINNs suffer from slow convergence due to spectral bias problems, which hinders efficient training despite their advantages for solving PDEs (mesh-free, unsupervised, high-dimensional capability).

Method: PINN-based method with coordinate-encoding layer on linear grid cells that separates local domains using grid cells, uses axis-independent linear grid cells to reduce computational cost, and employs natural cubic splines for interpolation between grid points to ensure continuous derivatives for loss computation.

Result: Numerical experiments demonstrate effective performance and efficient training convergence speed of the proposed method compared to standard PINNs.

Conclusion: The proposed method successfully addresses PINN training convergence issues through grid-based domain separation and spline interpolation, achieving both computational efficiency and stable training.
...
### [250] [TTQ: Activation-Aware Test-Time Quantization to Accelerate LLM Inference On The Fly](https://arxiv.org/abs/2603.19296)
*Toshiaki Koike-Akino,Jing Liu,Ye Wang*

Main category: cs.LG

TL;DR: TTQ is a test-time quantization framework that compresses large foundation models on-the-fly during inference using efficient online calibration, adapting to each prompt regardless of downstream tasks while achieving inference speedup.


<details>
  <summary>Details</summary>
Motivation: Existing activation-aware compression techniques for large foundation models rely heavily on calibration data, causing domain shift issues when applied to unseen downstream tasks. There's a need for quantization methods that can adapt dynamically during inference without retraining.

Method: Proposes a test-time quantization (TTQ) framework that performs instant activation-aware quantization at inference time using efficient online calibration. The method adapts quantization parameters for each prompt dynamically, enabling compression without prior knowledge of downstream tasks.

Result: TTQ improves quantization performance over state-of-the-art baselines and achieves inference speedup. The framework successfully handles domain shift issues by adapting to each prompt during inference.

Conclusion: Test-time quantization provides an effective solution for compressing large foundation models without retraining, overcoming domain shift limitations of calibration-dependent methods while maintaining inference efficiency.
...
### [251] [Non-Adversarial Imitation Learning Provably Free of Compounding Errors: The Role of Bellman Constraints](https://arxiv.org/abs/2603.22713)
*Tian Xu,Chenyang Wang,Xiaochen Zhai,Ziniu Li,Yi-Chen Li,Yang Yu*

Main category: cs.LG

TL;DR: IQ-Learn, a popular non-adversarial imitation learning method, is shown to reduce to behavioral cloning with compounding errors; a new method called Dual Q-DM is proposed that eliminates compounding errors through a primal-dual framework with Bellman constraints.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of IQ-Learn and similar Q-based imitation learning methods that were believed to outperform behavioral cloning but actually suffer from compounding errors and fail to generalize beyond demonstrations.

Method: Introduces Dual Q-DM, a primal-dual framework for distribution matching that incorporates Bellman constraints to propagate high Q-values from visited states to unvisited ones, enabling generalization beyond demonstrations.

Result: Theoretical analysis proves Dual Q-DM is equivalent to adversarial imitation learning and can recover expert actions beyond demonstrations, eliminating compounding errors. Experimental results support the theoretical findings.

Conclusion: Dual Q-DM is the first non-adversarial imitation learning method theoretically guaranteed to eliminate compounding errors, addressing the fundamental limitations of previous Q-based methods like IQ-Learn while avoiding adversarial training instability.
...
### [252] [Double Coupling Architecture and Training Method for Optimization Problems of Differential Algebraic Equations with Parameters](https://arxiv.org/abs/2603.22724)
*Wenqiang Yang,Wenyuan Wu,Yong Feng,Changbo Chen*

Main category: cs.LG

TL;DR: A dual physics-informed neural network architecture decouples constraints and objectives in parametric differential algebraic equation optimization, with genetic algorithm-enhanced training for multi-task optimization in product development simulations.


<details>
  <summary>Details</summary>
Motivation: Product development increasingly requires multi-task optimization for diverse requirements, but simulation models as complex nonlinear differential algebraic equations present challenges in optimization efficiency and real-time responsiveness.

Method: Dual physics-informed neural network architecture decouples constraints and objective functions; introduces relaxation variable with global error bound for solution equivalence; uses genetic algorithm-enhanced training framework to improve precision and efficiency without redundant solving of differential algebraic equations.

Result: Theoretical analysis shows solution equivalence between network and optimization problem; approach enables generalization for multi-task objectives with single training while maintaining real-time responsiveness to product requirements.

Conclusion: The proposed architecture and training framework effectively address multi-task optimization challenges in simulation modeling, providing efficient, precise solutions for product development with complex differential algebraic equation constraints.
...
### [253] [Spiking Personalized Federated Learning for Brain-Computer Interface-Enabled Immersive Communication](https://arxiv.org/abs/2603.22727)
*Chen Shang,Dinh Thai Hoang,Diep N. Nguyen,Jiadong Yu*

Main category: cs.LG

TL;DR: A BCI-based immersive communication framework using personalized federated learning with spiking neural networks for energy-efficient, privacy-preserving brain signal analysis.


<details>
  <summary>Details</summary>
Motivation: To enable personalized immersive adaptation under strong individual variability while addressing privacy concerns of sensitive brain signal data and energy limitations of immersive terminals.

Method: Develops a personalized federated learning model with embedded spiking neural networks for brain signal analysis, combining PFL for privacy preservation and SNNs for energy efficiency through sparse, event-driven computation.

Result: Achieves best overall identification accuracy on real brain-signal dataset while reducing inference energy by 6.46× compared to conventional ANN-based personalized baselines.

Conclusion: The proposed SNN-enabled PFL framework effectively balances personalization performance, privacy preservation, and energy efficiency for BCI-based immersive communication systems.
...
### [254] [Behavioral Heterogeneity as Quantum-Inspired Representation](https://arxiv.org/abs/2603.22729)
*Mohammad Elayan,Wissam Kontar*

Main category: cs.LG

TL;DR: Quantum-inspired representation models drivers as evolving latent states using density matrices, capturing dynamic heterogeneity beyond static labels.


<details>
  <summary>Details</summary>
Motivation: Current approaches reduce driver heterogeneity to static labels or discrete regimes, failing to capture the inherent dynamic nature of driving behavior.

Method: Quantum-inspired representation using density matrices for evolving latent states, behavioral embedding via non-linear Random Fourier Features, and state evolution combining temporal persistence with context-dependent profile activation.

Result: Evaluation on empirical driving data (Third Generation Simulation Data - TGSIM) demonstrates extraction and analysis of driving profiles.

Conclusion: The quantum-inspired approach provides a more nuanced representation of driver heterogeneity as dynamic, evolving states rather than static categories.
...
### [255] [Multitask-Informed Prior for In-Context Learning on Tabular Data: Application to Steel Property Prediction](https://arxiv.org/abs/2603.22738)
*Dimitrios Sinodinos,Bahareh Nikpour,Jack Yi Wei,Sushant Sinha,Xiaoping Ma,Kashif Rehman,Stephen Yue,Narges Armanfard*

Main category: cs.LG

TL;DR: Multitask learning framework adapts TabPFN foundation model for predicting multiple steel mechanical properties in hot rolling processes, outperforming traditional methods through novel fine-tuning strategies.


<details>
  <summary>Details</summary>
Motivation: Accurate prediction of steel mechanical properties during hot rolling processes like TSDR is challenging due to complex interactions among chemical compositions, processing parameters, and microstructures. Traditional methods are resource-intensive and lack adaptability, while existing approaches fail to leverage correlations among mechanical properties for improved accuracy through multitask learning.

Method: Presents a multitask learning framework that injects multitask awareness into TabPFN's prior through two complementary fine-tuning strategies: (1) target averaging - provides unified scalar signal compatible with TabPFN's single-target architecture, and (2) task-specific adapters - introduce task-specific supervision during fine-tuning. These strategies jointly guide the model toward a multitask-informed prior capturing cross-property relationships.

Result: Extensive experiments on industrial TSDR dataset demonstrate that multitask adaptations outperform classical machine learning methods and recent state-of-the-art tabular learning models across multiple evaluation metrics. The approach enhances both predictive accuracy and computational efficiency compared to task-specific fine-tuning.

Conclusion: Multitask-aware prior adaptation enables foundation models for tabular data to deliver scalable, rapid, and reliable deployment for automated industrial quality control and process optimization in TSDR, effectively capturing cross-property relationships among key mechanical metrics.
...
### [256] [From Arithmetic to Logic: The Resilience of Logic and Lookup-Based Neural Networks Under Parameter Bit-Flips](https://arxiv.org/abs/2603.22770)
*Alan T. L. Bacellar,Sathvik Chemudupati,Shashank Nag,Allison Seigler,Priscila M. V. Lima,Felipe M. G. França,Lizy K. John*

Main category: cs.LG

TL;DR: Lower precision, sparsity, bounded activations, and shallow depth improve DNN resilience to hardware bit-flip errors; logic/lookup-based networks achieve optimal accuracy-resilience trade-off.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks deployed in safety-critical edge environments need robustness against hardware-induced bit-flip errors. While empirical evidence suggests lower precision improves fault tolerance, the theoretical basis remains underexplored.

Method: Derived expected squared error (MSE) under independent parameter bit flips across multiple numerical formats and layer primitives. Conducted ablation studies on MLPerf Tiny benchmark suite to validate theoretical predictions.

Result: Lower precision, higher sparsity, bounded activations, and shallow depth consistently improve resilience. LUT-based models remain highly stable in corruption regimes where standard floating-point models fail sharply. Identified novel even-layer recovery effect unique to logic-based architectures.

Conclusion: Shifting from continuous arithmetic weights to discrete Boolean lookups provides favorable accuracy-resilience trade-off for hardware fault tolerance, suggesting logic/lookup-based networks as optimal design for safety-critical edge deployments.
...
TL;DR: The paper introduces a theoretical framework for test-time computation in LLMs as algorithms interacting with Markov chains, proves that optimal backtracking generates caterpillar trees, and proposes Caterpillar of Thoughts (CaT) algorithm that outperforms Tree-of-Thoughts with fewer token generations.


<details>
  <summary>Details</summary>
Motivation: Despite empirical success of test-time computation techniques like sampling, chain of thought, and backtracking in improving LLM outputs, there is limited theoretical understanding of optimal inference-time computation structure and how to best utilize fixed computation budgets.

Method: Models test-time computation as algorithms interacting with Markov chains where algorithms can backtrack to any previously observed state. Proves optimal backtracking generates caterpillar trees (path-like structures when leaves are removed). Introduces Caterpillar of Thoughts (CaT) algorithm based on this theoretical characterization.

Result: Theoretical proof shows backtracking can reduce generations exponentially, but limited backtracking (caterpillar trees) is theoretically sufficient. Empirical evaluation shows CaT achieves better success rates than Tree-of-Thoughts while reducing token/state generations.

Conclusion: The paper provides theoretical foundations for test-time computation in LLMs, demonstrates that optimal algorithms generate caterpillar trees, and introduces CaT as a practical algorithm that improves upon existing methods by reducing computational overhead while maintaining or improving performance.
...
### [258] [Transformers Trained via Gradient Descent Can Provably Learn a Class of Teacher Models](https://arxiv.org/abs/2603.22801)
*Chenyang Zhang,Qingyue Zhao,Quanquan Gu,Yuan Cao*

Main category: cs.LG

TL;DR: Transformers can theoretically learn from diverse teacher models (convolutional layers, graph convolutions, statistical models) using simplified attention, achieving optimal loss and good out-of-distribution generalization.


<details>
  <summary>Details</summary>
Motivation: Despite transformers' empirical success across many applications, their theoretical foundations remain largely unexplored. The paper aims to demystify transformers' strong capacities by theoretically investigating their ability to learn from various teacher models.

Method: Theoretical analysis of transformers as students learning from teacher models including convolution layers with average pooling, graph convolution layers, and statistical learning models (sparse token selection models and group-sparse linear predictors). The analysis focuses on one-layer transformers with simplified "position-only" attention.

Result: Proves that one-layer transformers with simplified attention can successfully recover all parameter blocks of teacher models, achieving optimal population loss. Demonstrates that trained transformers can generalize well to out-of-distribution data under mild assumptions.

Conclusion: The key insight is identifying a fundamental bilinear structure shared by various learning tasks, enabling unified learning guarantees. This provides theoretical foundations for transformers' ability to learn from diverse teacher models and generalize effectively.
...
### [259] [Universal and efficient graph neural networks with dynamic attention for machine learning interatomic potentials](https://arxiv.org/abs/2603.22810)
*Shuyu Bi,Zhede Zhao,Qiangchao Sun,Tao Hu,Xionggang Lu,Hongwei Cheng*

Main category: cs.LG

TL;DR: MLANet is an efficient graph neural network framework for machine learning interatomic potentials that achieves near-quantum accuracy with linear computational cost through dual-path dynamic attention and multi-perspective pooling strategies.


<details>
  <summary>Details</summary>
Motivation: Traditional empirical potentials lack accuracy while first-principles methods are computationally prohibitive. Existing machine learning interatomic potentials face challenges in efficiency and stability, creating a need for more practical tools for high-fidelity atomic simulations.

Method: MLANet introduces a dual-path dynamic attention mechanism for geometry-aware message passing and a multi-perspective pooling strategy to construct comprehensive system representations. This graph neural network framework enables accurate modeling of atomic environments with exceptional computational efficiency.

Result: Tested across diverse systems including organic molecules, periodic inorganic materials, 2D materials, surface catalytic reactions, and charged systems, MLANet maintains competitive prediction accuracy with markedly lower computational cost than mainstream equivariant models, enabling stable long-time molecular dynamics simulations.

Conclusion: MLANet provides an efficient and practical tool for large-scale, high-accuracy atomic simulations, making high-fidelity molecular dynamics simulations more accessible by balancing accuracy and computational efficiency.
...
### [260] [Sparser, Faster, Lighter Transformer Language Models](https://arxiv.org/abs/2603.23198)
*Edoardo Cetin,Stefano Peluchetti,Emilio Castillo,Akira Naruse,Mana Murakami,Llion Jones*

Main category: cs.LG

TL;DR: The paper introduces a sparse packing format and CUDA kernels to leverage unstructured sparsity in LLM feedforward layers, achieving over 99% sparsity with minimal performance impact and substantial efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Autoregressive LLM scaling comes with vast computational costs, particularly in feedforward layers that account for most parameters and FLOPs. The authors aim to tackle these costs by exploiting sparsity for efficiency improvements.

Method: Introduces a new sparse packing format and specialized CUDA kernels designed to integrate with modern GPU execution pipelines. Uses L1 regularization to induce over 99% sparsity in LLM feedforward layers while maintaining downstream performance.

Result: Demonstrates that simple L1 regularization can induce over 99% sparsity with negligible impact on downstream performance. When paired with their kernels, these sparsity levels translate into substantial throughput, energy efficiency, and memory usage benefits that increase with model scale.

Conclusion: The approach establishes sparsity as a practical axis for improving efficiency and scalability of modern foundation models. The authors will release all code and kernels open-source to promote adoption and accelerate research in this direction.
...
### [261] [Towards The Implicit Bias on Multiclass Separable Data Under Norm Constraints](https://arxiv.org/abs/2603.22824)
*Shengping Xie,Zekun Wu,Quan Chen,Kaixu Tang*

Main category: cs.LG

TL;DR: The paper introduces NucGD, a geometry-aware optimizer that enforces low-rank structures via nuclear norm constraints, connects it with low-rank projection methods, derives efficient SVD-free updates, and analyzes how stochastic optimization dynamics affect convergence to maximum margin solutions.


<details>
  <summary>Details</summary>
Motivation: To investigate how optimization geometry shapes solutions on multiclass separable data and understand the mechanisms of implicit bias induced by gradient-based algorithms in overparameterized models.

Method: Leverages Normalized Steepest Descent (NSD) framework, introduces NucGD optimizer with nuclear norm constraints, connects with low-rank projection methods, derives efficient SVD-free update rule via asynchronous power iteration, and empirically analyzes stochastic optimization dynamics including gradient noise from mini-batch sampling and momentum.

Result: Developed a unified perspective connecting NucGD with emerging low-rank projection methods, created scalable training via SVD-free updates, and characterized how varying levels of gradient noise modulate convergence toward expected maximum margin solutions.

Conclusion: The work provides a comprehensive framework for understanding and controlling implicit bias through optimization geometry, offering practical algorithms for enforcing low-rank structures and insights into stochastic optimization dynamics affecting solution convergence.
...
### [262] [Off-Policy Value-Based Reinforcement Learning for Large Language Models](https://arxiv.org/abs/2603.23355)
...
TL;DR: ReVal: A value-based RL method for LLMs that enables off-policy learning via Bellman updates, combining stepwise internal consistency signals with trajectory-level outcome verification for improved data efficiency.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for LLMs are largely on-policy, requiring fresh samples for each update and discarding data after single use, leading to poor sample efficiency. This is particularly problematic for long-horizon tasks where trajectory generation is expensive.

Method: ReVal is a Bellman-update-based value-based RL framework that combines stepwise signals capturing internal consistency with trajectory-level signals derived from outcome verification. It naturally supports replay-buffer-based training, enabling efficient reuse of past trajectories.

Result: ReVal converges faster and outperforms GRPO in final performance on mathematical reasoning benchmarks. On DeepSeek-R1-Distill-1.5B, it improves training efficiency and achieves 2.7% improvement on AIME24 and 4.5% improvement on out-of-domain benchmark GPQA over GRPO.

Conclusion: Value-based RL is a practical alternative to policy-based methods for LLM training, offering better data utilization efficiency through off-policy learning capabilities.
...
TL;DR: DPPN architecture uses pheromone field routing to identify two requirements for persistent structural memory: stable coordinate systems and graceful transfer mechanisms.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental requirements for persistent structural memory in neural networks, particularly addressing the instability of learned coordinate systems and transfer challenges.

Method: Dual-View Pheromone Pathway Network (DPPN) architecture with sparse attention routing through persistent pheromone fields over latent slot transitions. Five progressively refined experiments with 10 seeds per condition across 5 model variants and 4 transfer targets.

Result: Identified core principle: persistent memory requires stable coordinate systems, and learned coordinate systems are inherently unstable. Fixed random Fourier features provide stable extrinsic coordinates. DPPN outperforms transformer and random sparse baselines (AULC 0.700 vs 0.680 vs 0.670). Warm pheromone as learning-rate prior achieves +0.003 on same-family tasks.

Conclusion: Two independent requirements for persistent structural memory: (a) coordinate stability and (b) graceful transfer mechanism. The catch-22 between stability and informativeness is partially permeable to learned functions.
...
### [264] [Confidence Calibration under Ambiguous Ground Truth](https://arxiv.org/abs/2603.22879)
*Linwei Tao,Haoyang Luo,Minjing Dong,Chang Xu*

Main category: cs.LG

TL;DR: The paper addresses confidence calibration failure when using majority-voted labels in multi-annotator settings, showing Temperature Scaling underestimates annotator uncertainty. It introduces ambiguity-aware calibrators that optimize against full label distributions without model retraining.


<details>
  <summary>Details</summary>
Motivation: Standard confidence calibration assumes unique ground-truth labels, but this fails when annotators genuinely disagree. Post-hoc calibrators fitted on majority-voted labels appear well-calibrated under conventional evaluation but remain miscalibrated against the underlying annotator distribution.

Method: Developed a family of ambiguity-aware post-hoc calibrators: Dirichlet-Soft (uses full annotator distribution), Monte Carlo Temperature Scaling with single annotation (MCTS S=1), and Label-Smooth Temperature Scaling (LS-TS) that constructs pseudo-soft targets from model confidence using only voted labels.

Result: Experiments on four benchmarks show Dirichlet-Soft reduces true-label ECE by 55-87% relative to Temperature Scaling, while LS-TS reduces ECE by 9-77% without any annotator data. MCTS S=1 matches full-distribution calibration across all benchmarks.

Conclusion: Standard calibration methods are structurally biased when using majority-voted labels in ambiguous settings. The proposed ambiguity-aware calibrators effectively address this issue, with LS-TS providing significant improvements even without access to annotator distributions.
...
### [265] [TreeTeaming: Autonomous Red-Teaming of Vision-Language Models via Hierarchical Strategy Exploration](https://arxiv.org/abs/2603.22882)
*Chunxiao Li,Lijun Li,Jing Shao*

Main category: cs.LG

TL;DR: TreeTeaming is an automated red teaming framework that uses evolutionary tree exploration to discover novel, diverse safety vulnerabilities in Vision-Language Models, outperforming existing linear methods.


<details>
  <summary>Details</summary>
Motivation: Existing red teaming methods for VLMs are constrained by linear exploration paradigms that optimize within predefined strategy sets, preventing discovery of novel and diverse exploits. There's a need for more dynamic, evolutionary approaches to vulnerability discovery.

Method: TreeTeaming reframes strategy exploration as a dynamic evolutionary process using an LLM-powered Orchestrator that autonomously decides whether to evolve promising attack paths or explore diverse strategic branches, constructing and expanding a strategy tree. A multimodal actuator executes these complex strategies.

Result: Achieved state-of-the-art attack success rates on 11 out of 12 prominent VLMs, reaching up to 87.60% on GPT-4o. Demonstrated superior strategic diversity over union of previously public jailbreak strategies. Generated attacks showed 23.09% average toxicity reduction, indicating stealth and subtlety.

Conclusion: TreeTeaming introduces a new paradigm for automated vulnerability discovery, emphasizing the necessity of proactive exploration beyond static heuristics to secure frontier AI models through evolutionary strategy tree construction.
...
### [266] [Balancing Safety and Efficiency in Aircraft Health Diagnosis: A Task Decomposition Framework with Heterogeneous Long-Micro Scale Cascading and Knowledge Distillation-based Interpretability](https://arxiv.org/abs/2603.22885)
*Xinhang Chen,Zhihuan Wei,Yang Hu,Zhiguo Zeng,Kang Zeng,Suili Yang*

Main category: cs.LG

TL;DR: DDF framework decouples aircraft diagnosis into anomaly detection and fault classification using LMSD with global screening and local feature extraction, achieving improved performance and efficiency with interpretable decisions.


<details>
  <summary>Details</summary>
Motivation: Address three challenges in general aviation diagnosis: data uncertainty, task heterogeneity, and computational inefficiency. Existing end-to-end approaches have receptive field conflicts between global context and local features, and suffer from high training costs under class imbalance.

Method: Proposes Diagnosis Decomposition Framework (DDF) with Long-Micro Scale Diagnostician (LMSD). Uses ConvTokMHSA for global operational pattern discrimination and Multi-Micro Kernel Network for local fault feature extraction. Implements decoupled training with separate optimization pathways and Keyness Extraction Layer for interpretability via knowledge distillation.

Result: Experiments on NGAFID real-world aviation dataset show 4-8% improvement in Multi-Class Weighted Penalty Metric (MCWPM) over baselines with substantially reduced training time, demonstrating advantages in task adaptability, interpretability, and efficiency.

Conclusion: DDF provides a deployable methodology for general aviation health management by addressing key challenges through task decomposition, efficient architecture design, and interpretable decision-making.
...
### [267] [Conditionally Identifiable Latent Representation for Multivariate Time Series with Structural Dynamics](https://arxiv.org/abs/2603.22886)
*Minkey Chang,Jae-Young Kim*

Main category: cs.LG

TL;DR: iVDFM learns identifiable latent factors from multivariate time series using innovation conditioning, achieving identifiability up to permutation and affine transformations with linear diagonal dynamics enabling scalable computation.


<details>
  <summary>Details</summary>
Motivation: To learn latent factors from multivariate time series with provable identifiability guarantees, addressing limitations of existing methods that lack identifiability or require restrictive assumptions.

Method: Apply iVAE-style conditioning to the innovation process driving dynamics rather than latent states, use linear diagonal dynamics for identifiability preservation, and implement scalable computation via companion-matrix and Krylov methods.

Result: Improved factor recovery on synthetic data, stable intervention accuracy on synthetic SCMs, and competitive probabilistic forecasting on real-world benchmarks.

Conclusion: iVDFM provides a principled framework for learning identifiable latent factors from time series with theoretical guarantees and practical scalability, enabling reliable factor discovery and forecasting.
...
### [268] [VLGOR: Visual-Language Knowledge Guided Offline Reinforcement Learning for Generalizable Agents](https://arxiv.org/abs/2603.22892)
*Pengsen Liu,Maosen Zeng,Nan Tang,Kaiyuan Li,Jing-Cheng Pang,Yunan Liu,Yang Yu*

Main category: cs.LG

TL;DR: VLGOR integrates visual and language knowledge to generate imaginary rollouts for offline RL, improving generalization to unseen tasks by 24% over baselines.


<details>
  <summary>Details</summary>
Motivation: LLMs lack direct perception of physical environments, limiting their understanding of environmental dynamics and ability to generalize to unseen tasks when combined with RL for instruction-following agents.

Method: Proposes Visual-Language Knowledge-Guided Offline Reinforcement Learning (VLGOR) that fine-tunes a vision-language model to predict future states and actions conditioned on initial visual observations and high-level instructions, generating temporally coherent and spatially plausible imaginary rollouts. Uses counterfactual prompts to produce diverse rollouts for offline RL training.

Result: Experiments on robotic manipulation benchmarks show VLGOR significantly improves performance on unseen tasks requiring novel optimal policies, achieving success rate over 24% higher than baseline methods.

Conclusion: VLGOR effectively bridges the perception gap in LLM-RL agents by integrating visual-language knowledge to generate realistic imaginary rollouts, enabling better generalization to unseen tasks through enriched offline RL training data.
...
### [269] [Weak-PDE-Net: Discovering Open-Form PDEs via Differentiable Symbolic Networks and Weak Formulation](https://arxiv.org/abs/2603.22951)
*Xinxin Li,Xingyu Cui,Jin Qi,Juan Zhang,Da Li,Junping Yin*

Main category: cs.LG

TL;DR: Weak-PDE-Net: An end-to-end differentiable framework for discovering open-form PDEs from sparse, noisy data using weak-form representation and neural architecture search.


<details>
  <summary>Details</summary>
Motivation: Conventional sparse regression methods for PDE discovery suffer from instability of numerical differentiation under sparse/noisy data and restricted flexibility of pre-defined candidate libraries.

Method: Two interconnected modules: (1) forward response learner with learnable Gaussian kernels in lightweight MLP as surrogate model, (2) weak-form PDE generator with symbolic network and integral module to avoid explicit differentiation. Uses Differentiable Neural Architecture Search to explore functional space and incorporates Galilean Invariance constraints and symmetry equivariance for physical consistency.

Result: Experiments on challenging PDE benchmarks demonstrate accurate recovery of governing equations even under highly sparse and noisy observations.

Conclusion: Weak-PDE-Net provides a robust, end-to-end differentiable framework for discovering open-form PDEs from sparse and noisy data, overcoming limitations of conventional sparse regression methods.
...
### [270] [Asymptotic Learning Curves for Diffusion Models with Random Features Score and Manifold Data](https://arxiv.org/abs/2603.22962)
*Anand Jerry George,Nicolas Macris*

Main category: cs.LG

TL;DR: Diffusion models' denoising score matching learns efficiently on linear manifolds with sample complexity scaling with intrinsic dimension, but benefits diminish for non-linear manifolds.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical behavior of denoising score matching (used in diffusion models) when data lies on low-dimensional manifolds, and to characterize how sample complexity depends on manifold structure.

Method: Theoretical analysis of denoising score matching with random feature neural networks in high-dimensional limit, deriving exact expressions for test/train/score errors, comparing linear vs. non-linear manifolds.

Result: For linear manifolds, sample complexity scales linearly with intrinsic dimension rather than ambient dimension. However, benefits of low-dimensional structure diminish for non-linear manifolds.

Conclusion: Diffusion models can benefit from structured data, but the dependence on manifold structure type is subtle and intricate, with linear manifolds showing clear advantages that don't fully extend to non-linear cases.
...
### [271] [Can Graph Foundation Models Generalize Over Architecture?](https://arxiv.org/abs/2603.22984)
*Benjamin Gutteridge,Michael Bronstein,Xiaowen Dong*

Main category: cs.LG

TL;DR: GFMs need architecture adaptivity for true zero-shot generalization; current fixed-backbone approaches fail on tasks with different architectural requirements than training.


<details>
  <summary>Details</summary>
Motivation: Current graph foundation models rely on fixed architectural backbones, assuming a single message-passing regime suffices across all tasks, but this limits their ability to generalize zero-shot to tasks with different architectural requirements.

Method: Introduce a framework that adapts GNN architecture at inference time by discovering and mixing task-specific linear graph operators, enabling zero-shot generalization across tasks with heterogeneous architectural requirements without retraining.

Result: Demonstrate improved performance and robustness over existing domain-agnostic GFMs on arbitrary-range synthetic tasks and real-world benchmarks, showing that architecture adaptivity addresses the limitations of fixed-backbone approaches.

Conclusion: Architecture adaptivity is a necessary requirement for true graph foundation models, and the proposed framework enables robust zero-shot generalization across tasks with diverse architectural requirements.
...
### [272] [Robustness Quantification and Uncertainty Quantification: Comparing Two Methods for Assessing the Reliability of Classifier Predictions](https://arxiv.org/abs/2603.22988)
*Adrián Detavernier,Jasper De Bock*

Main category: cs.LG

TL;DR: RQ (Robustness Quantification) can outperform UQ (Uncertainty Quantification) for assessing classifier prediction reliability, and combining both approaches yields even better results.


<details>
  <summary>Details</summary>
Motivation: To compare and evaluate two different approaches for assessing the reliability of individual classifier predictions: Robustness Quantification (RQ) and Uncertainty Quantification (UQ), and to explore their potential complementarity.

Method: Conceptual analysis of differences between RQ and UQ approaches, followed by empirical comparison on benchmark datasets under both standard settings and distribution shift conditions.

Result: RQ demonstrates capability to outperform UQ in reliability assessment, both in standard settings and under distribution shift. Additionally, combining RQ and UQ approaches leads to even better reliability assessments than either approach alone.

Conclusion: Robustness Quantification is a competitive alternative to Uncertainty Quantification for assessing classifier prediction reliability, and the two approaches are complementary, with combined methods offering superior performance.
...
### [273] [A Sobering Look at Tabular Data Generation via Probabilistic Circuits](https://arxiv.org/abs/2603.23016)
*Davide Scassola,Dylan Ponsford,Adrián Javaloy,Sebastiano Saccani,Luca Bortolussi,Henry Gouk,Antonio Vergari*

Main category: cs.LG

TL;DR: The paper questions the perceived progress in tabular data generation, showing that current evaluation metrics are inadequate and that simple hierarchical mixture models (deep probabilistic circuits) outperform state-of-the-art diffusion models at much lower cost.


<details>
  <summary>Details</summary>
Motivation: The motivation is to challenge the perception of progress in tabular data generation, highlighting limitations in current evaluation protocols and demonstrating that simpler, more efficient models can outperform complex state-of-the-art approaches when properly evaluated.

Method: The method involves: 1) Critiquing current evaluation metrics for tabular data generation, 2) Proposing hierarchical mixture models in the form of deep probabilistic circuits (PCs) as a simple baseline, and 3) Conducting rigorous empirical analysis comparing PCs against state-of-the-art diffusion models using improved evaluation protocols.

Result: Results show that deep probabilistic circuits deliver competitive or superior performance to state-of-the-art diffusion models for tabular data generation at a fraction of the computational cost. The apparent saturation of progress in SotA models is largely due to inadequate metrics rather than actual performance improvements.

Conclusion: The conclusion is that there is still much work needed to generate realistic tabular data, current evaluation protocols are flawed, and simpler hierarchical mixture models (probabilistic circuits) provide an efficient and effective alternative to complex state-of-the-art approaches.
...
### [274] [Assessing the Robustness of Climate Foundation Models under No-Analog Distribution Shifts](https://arxiv.org/abs/2603.23043)
*Maria Conchita Agana Navarro,Geng Li,Theo Wolf,Maria Perez-Ortiz*

Main category: cs.LG

TL;DR: Climate emulators trained only on historical data struggle to generalize to future climate states, with foundation models showing accuracy-stability trade-offs under distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Climate change introduces non-stationarities that challenge ML-based climate emulators' ability to generalize beyond training distributions, especially under "no-analog" future conditions. Data contamination in training (using future scenarios) masks true out-of-distribution performance, creating reliability concerns.

Method: Benchmarked OOD robustness of three architectures (U-Net, ConvLSTM, ClimaX foundation model) restricted to historical-only training (1850-2014). Used two evaluation strategies: temporal extrapolation to 2015-2023 and cross-scenario forcing shifts across divergent emission pathways.

Result: Revealed accuracy vs. stability trade-off: ClimaX achieved lowest absolute error but exhibited higher relative performance changes under distribution shifts, with precipitation errors increasing up to 8.44% under extreme forcing scenarios.

Conclusion: Even high-capacity foundation models are sensitive to external forcing trajectories when restricted to historical training. Scenario-aware training and rigorous OOD evaluation protocols are necessary to ensure climate emulator robustness under changing climate conditions.
...
### [275] [Generalization Bounds for Physics-Informed Neural Networks for the Incompressible Navier-Stokes Equations](https://arxiv.org/abs/2603.23072)
*Sebastien Andre-Sloan,Dibyakanti Kumar,Alejandro F Frangi,Anirbit Mukherjee*

Main category: cs.LG

TL;DR: First rigorous generalization bounds for depth-2 neural networks solving (d+1)-D incompressible Navier-Stokes equations via PINNs, with dimension-independent sample complexity and novel activation function recommendations.


<details>
  <summary>Details</summary>
Motivation: To establish rigorous theoretical guarantees for Physics-Informed Neural Networks (PINNs) when solving high-dimensional fluid dynamics problems, specifically addressing the lack of theoretical understanding of generalization behavior in PINNs for Navier-Stokes equations.

Method: Bounding the Rademacher complexity of the PINN risk for depth-2 neural networks with weight constraints, deriving generalization bounds that characterize the gap in terms of fluid viscosity and regularization parameters, leading to dimension-independent sample complexity.

Result: Derived first rigorous generalization bounds for PINNs solving (d+1)-dimensional Navier-Stokes equations, showing dimension-independent sample complexity and suggesting novel activation functions for fluid dynamics applications.

Conclusion: The work provides theoretical foundations for PINNs in fluid dynamics, offers practical activation function recommendations, and demonstrates empirical validation on the Taylor-Green vortex benchmark, bridging theory and practice in physics-informed machine learning.
...
### [276] [MsFormer: Enabling Robust Predictive Maintenance Services for Industrial Devices](https://arxiv.org/abs/2603.23076)
*Jiahui Zhou,Dan Li,Ruibing Jin,Jian Lou,Yanran Zhao,Zhenghua Chen,Zigui Jiang,See-Kiong Ng*

Main category: cs.LG

TL;DR: MsFormer is a lightweight Multi-scale Transformer designed as a unified AI service model for industrial predictive maintenance, addressing multi-scale temporal correlations in streaming sensor data and data scarcity challenges.


<details>
  <summary>Details</summary>
Motivation: Existing deep-learning methods for predictive maintenance lack a general service-oriented framework to capture complex dependencies in industrial IoT sensor data. Transformer-based models face bottlenecks in real-world deployment due to multi-scale temporal correlations in streaming sensor data and limited training dataset sizes, posing challenges for robust predictive services.

Method: MsFormer incorporates a Multi-scale Sampling (MS) module and tailored position encoding to capture sequential correlations across multi-streaming service data. It adopts a lightweight attention mechanism with straightforward pooling operations instead of self-attention to accommodate data-scarce environments.

Result: Extensive experiments on real-world datasets demonstrate significant performance improvements over state-of-the-art methods. MsFormer outperforms across industrial devices and operating conditions, showing strong generalizability while maintaining highly reliable Quality of Service (QoS).

Conclusion: MsFormer provides an effective solution for reliable industrial predictive maintenance services by addressing multi-scale temporal correlations and data scarcity challenges through its lightweight multi-scale Transformer architecture.
...
### [277] [Policy-based Tuning of Autoregressive Image Models with Instance- and Distribution-Level Rewards](https://arxiv.org/abs/2603.23086)
*Orhun Buğra Baran,Melih Kandemir,Ramazan Gokberk Cinbis*

Main category: cs.LG

TL;DR: Lightweight RL framework for autoregressive image generation using Group Relative Policy Optimization with novel LOO-FID reward to improve sample quality and diversity while preventing mode collapse.


<details>
  <summary>Details</summary>
Motivation: Standard AR models trained with maximum likelihood lack direct optimization for sample quality and diversity. Existing RL methods for diffusion models suffer from diversity collapse, while concurrent RL methods for AR models rely on instance-level rewards that trade distributional coverage for quality.

Method: Proposes lightweight RL framework casting token-based AR synthesis as Markov Decision Process optimized via Group Relative Policy Optimization (GRPO). Introduces novel distribution-level Leave-One-Out FID (LOO-FID) reward using exponential moving average of feature moments to encourage diversity and prevent mode collapse. Integrates with composite instance-level rewards (CLIP and HPSv2) for semantic/perceptual fidelity, stabilized with adaptive entropy regularization.

Result: Extensive experiments on LlamaGen and VQGAN architectures show clear improvements across standard quality and diversity metrics within only few hundred tuning iterations. Model can produce competitive samples without Classifier-Free Guidance, bypassing its 2x inference cost.

Conclusion: Proposed RL framework with LOO-FID reward effectively addresses quality-diversity trade-off in AR image generation, enabling efficient optimization that improves both aspects while eliminating need for Classifier-Free Guidance.
...
### [278] [SpecXMaster Technical Report](https://arxiv.org/abs/2603.23101)
...
TL;DR: SpecXMaster is an AI framework using Agentic Reinforcement Learning for automated NMR spectral interpretation, extracting multiplicity information from raw FID data to generate chemical structures.


<details>
  <summary>Details</summary>
Motivation: Conventional expert-dependent spectral interpretation suffers from human bias, limited specialized expertise, and variability across interpreters, creating substantial hurdles in NMR analysis.

Method: SpecXMaster leverages Agentic Reinforcement Learning (RL) for NMR molecular spectral interpretation, enabling automated extraction of multiplicity information from both 1H and 13C spectra directly from raw FID data in an end-to-end pipeline.
...
Conclusion: SpecXMaster represents a novel methodological paradigm for spectral interpretation that will have a profound impact on the organic chemistry community by enabling fully automated NMR spectral interpretation.
...
### [279] [DAK-UCB: Diversity-Aware Prompt Routing for LLMs and Generative Models](https://arxiv.org/abs/2603.23140)
...
<details>
  <summary>Details</summary>
Motivation: Current model selection methods focus only on maximizing fidelity scores (e.g., CLIP-Score) but ignore output diversity, potentially leading to homogeneous or repetitive generations that fail to address diversity shortcomings.

Method: Diversity-Aware Kernelized Upper Confidence Bound (DAK-UCB) incorporates both fidelity and diversity metrics using prompt-aware diversity score functions that decompose to two-sample-based expectations over prompt-output pairs from previous generation rounds, implemented with joint kernel distance and kernel entropy measures.

Result: Experimental results demonstrate DAK-UCB's effectiveness in promoting diversity-aware model selection while maintaining fidelity in generations across a sequence of prompts.

Conclusion: DAK-UCB provides a principled online learning framework for selecting generative models that balances both quality and diversity, addressing a critical gap in current model selection approaches.
...
### [280] [A Schrödinger Eigenfunction Method for Long-Horizon Stochastic Optimal Control](https://arxiv.org/abs/2603.23173)
*Louis Claeys,Artur Goldman,Zebang Shen,Niao He*

Main category: cs.LG

TL;DR: The paper presents a method for solving long-horizon high-dimensional stochastic optimal control problems by exploiting a connection between Hamilton-Jacobi-Bellman equations and Schrödinger operators, enabling efficient solutions through eigenfunction learning.


<details>
  <summary>Details</summary>
Motivation: Existing methods for high-dimensional stochastic optimal control scale linearly with planning horizon T, with performance deteriorating exponentially for long horizons. The authors aim to overcome these limitations for a subclass of linearly-solvable SOC problems with gradient drift structure.

Method: The paper proves that under gradient drift assumptions, the HJB operator is unitarily equivalent to a Schrödinger operator with discrete spectrum. This enables two approaches: 1) For symmetric LQR, using quantum harmonic oscillator eigenfunctions for analytic solutions, and 2) For general settings, learning the eigensystem using neural networks with a novel loss function that addresses implicit reweighting issues in existing eigenfunction learning methods.

Result: The method achieves order-of-magnitude improvement in control accuracy on long-horizon benchmarks compared to state-of-the-art methods, while reducing memory usage and runtime complexity from O(Td) to O(d).

Conclusion: The connection between stochastic optimal control and Schrödinger operators provides an efficient framework for solving long-horizon control problems, with both analytic solutions for special cases and neural network-based learning for general settings, overcoming exponential performance degradation in existing methods.
...
### [281] [A One-Inclusion Graph Approach to Multi-Group Learning](https://arxiv.org/abs/2603.23208)
*Noah Bergam,Samuel Deng,Daniel Hsu*

Main category: cs.LG

TL;DR: The paper proves optimal sample complexity bounds for multi-group learning using an extension of the one-inclusion graph strategy with bipartite b-matching, achieving optimal convergence rates in different settings.


<details>
  <summary>Details</summary>
Motivation: To establish tight upper bounds on sample complexity for multi-group learning, addressing the gap in understanding optimal convergence rates in this learning framework.

Method: Extends the one-inclusion graph prediction strategy using a generalization of bipartite b-matching to develop an algorithm for multi-group learning.

Result: Achieves optimal log n/n convergence rate in group-realizable setting (proven by matching lower bound), and optimal 1/n rate when evaluation group is chosen obliviously of the sample.

Conclusion: The algorithm provides tightest-known sample complexity bounds for multi-group learning, with optimal convergence rates in both standard and relaxed settings.
...
### [282] [General Machine Learning: Theory for Learning Under Variable Regimes](https://arxiv.org/abs/2603.23220)
*Aomar Osmani*

Main category: cs.LG

TL;DR: Foundational framework for learning under regime variation with admissible transport, protected-core preservation, and evaluator-aware evolution; establishes first theorem-supporting consequences including structural obstructions, protected-stability templates, and kernel-level compatibility results.


<details>
  <summary>Details</summary>
Motivation: To address learning scenarios where the learner, memory state, and evaluative conditions evolve over time, requiring a foundational framework that captures regime variation and establishes core learning-theoretic objects for such dynamic settings.

Method: Develops regime-varying framework centered on admissible transport, protected-core preservation, and evaluator-aware learning evolution; uses structural obstruction arguments, protected-stability templates with explicit witnesses, and theorem-layer results on evaluator factorization, morphisms, composition, and kernel-level alignment.

Result: Establishes immediate closure consequences of admissibility, structural obstruction for faithful fixed-ontology reduction in multi-regime settings, protected-stability templates with numerical/symbolic witnesses on controlled subclasses, and first kernel-level compatibility results with controlled monotonic deductive witnesses.

Conclusion: Presents foundational structural framework for regime-varying learning with first theorem-supporting layer, not a complete quantitative theory; demonstrates framework viability through worked two-regime example showing admissibility certificates, protected evaluative cores, and regime-variation costs.
...
### [283] [GEM: Guided Expectation-Maximization for Behavior-Normalized Candidate Action Selection in Offline RL](https://arxiv.org/abs/2603.23232)
*Haoyu Wang,Jingcheng Wang,Shunyu Wu,Xinwei Xiao*

Main category: cs.LG

TL;DR: GEM introduces a multimodal, controllable action selection framework for offline RL using Gaussian Mixture Models with critic-guided EM updates and candidate-based inference with behavior-normalized support.


<details>
  <summary>Details</summary>
Motivation: Offline RL can fit strong value functions but deployment reliability depends on action selection. Unimodal policies can produce "in-between" actions weakly supported by data when datasets have branched/multimodal action landscapes, making decisions brittle even with good critics.

Method: GEM trains a GMM actor via critic-guided, advantage-weighted EM-style updates that preserve distinct components while shifting probability mass toward high-value regions. Learns tractable GMM behavior model to quantify support. During inference, performs candidate-based selection: generates parallel candidate set and reranks actions using conservative ensemble lower-confidence bound with behavior-normalized support (behavior log-likelihood standardized within each state's candidate set).

Result: GEM is competitive across D4RL benchmarks and offers a simple inference-time budget knob (candidate count) that trades compute for decision quality without retraining.

Conclusion: GEM provides a multimodal, controllable action selection framework for offline RL that addresses the limitations of unimodal policies in branched/multimodal action landscapes, offering reliable deployment with explicit control over inference-time compute-quality tradeoffs.
...
### [284] [Neural ODE and SDE Models for Adaptation and Planning in Model-Based Reinforcement Learning](https://arxiv.org/abs/2603.23245)
*Chao Han,Stefanos Ioannou,Luca Manneschi,T. J. Hayward,Michael Mangan,Aditya Gilra,Eleni Vasilaki*

Main category: cs.LG

TL;DR: Neural SDEs outperform neural ODEs in modeling stochastic dynamics for RL, enabling efficient policy adaptation and handling partial observability via latent SDE models.


<details>
  <summary>Details</summary>
Motivation: To develop more effective model-based RL approaches that can handle stochastic transition dynamics in both fully and partially observed environments, improving sample efficiency and enabling efficient policy adaptation.

Method: Use neural ODEs and SDEs to model stochastic dynamics; introduce latent SDE model combining ODE with GAN-trained stochastic component in latent space for partial observability; leverage inverse models for policy adaptation.

Result: Neural SDEs capture inherent stochasticity better than neural ODEs, enabling high-performing policies with improved sample efficiency; latent SDE model outperforms or matches general model-based and model-free approaches across stochastic continuous-control benchmarks.

Conclusion: Action-conditional latent SDEs are effective for RL planning in environments with stochastic transitions, demonstrating applicability in both fully and partially observed settings with efficient adaptation capabilities.
...
### [285] [A Learning Method with Gap-Aware Generation for Heterogeneous DAG Scheduling](https://arxiv.org/abs/2603.23249)
*Ruisong Zhou,Haijun Zou,Li Zhou,Chumin Sun,Zaiwen Wen*

Main category: cs.LG

TL;DR: WeCAN is an RL-based DAG scheduler for heterogeneous environments that uses a two-stage single-pass design with weighted cross-attention to model task-pool compatibility, plus skip-extended realization to address generation-induced optimality gaps.


<details>
  <summary>Details</summary>
Motivation: Efficient DAG scheduling in heterogeneous environments is challenging due to resource capacities, dependencies, varying resource pools/task types, and the need for rapid schedule generation. Existing approaches struggle with adaptability across environments and generation-induced optimality gaps from restricted reachability in list-scheduling maps.

Method: WeCAN uses an end-to-end RL framework with two-stage single-pass design: 1) single forward pass produces task-pool scores and global parameters, 2) generation map constructs schedules without repeated network calls. Features weighted cross-attention encoder gated by compatibility coefficients (size-agnostic to environment fluctuations). Introduces order-space analysis characterizing reachable schedule orders, then designs skip-extended realization with analytically parameterized decreasing skip rule to enlarge reachable order set while preserving single-pass efficiency.
...
Conclusion: WeCAN provides an effective RL-based solution for heterogeneous DAG scheduling that addresses task-pool compatibility and generation-induced optimality gaps through novel architectural design and theoretical analysis, achieving both performance improvements and computational efficiency.
...
### [286] [Permutation-Symmetrized Diffusion for Unconditional Molecular Generation](https://arxiv.org/abs/2603.23255)
*Gyeonghoon Ko,Juho Lee*

Main category: cs.LG

TL;DR: The paper proposes direct diffusion modeling on quotient manifolds for permutation-invariant molecular generation, deriving explicit heat kernel expressions and developing MCMC-based training for permutation-symmetrized scores.


<details>
  <summary>Details</summary>
Motivation: Most diffusion models enforce permutation invariance indirectly via permutation-equivariant networks on ordered spaces, rather than directly modeling the quotient manifold where all atom permutations are identified.

Method: Model diffusion directly on quotient manifold ℝ^{d×N}/S_N; derive explicit heat kernel expression as sum of Euclidean heat kernels over permutations; develop training with permutation-symmetrized score using expectation form over posterior on permutations approximated via MCMC in permutation space.

Result: Evaluation on unconditional 3D molecule generation on QM9 under EQGAT-Diff protocol shows quotient-based permutation symmetrization is practical and yields competitive generation quality with improved efficiency.

Conclusion: Direct modeling on quotient manifolds provides a principled approach for permutation-invariant molecular generation, with explicit theoretical foundations and practical implementation via MCMC approximation.
...
### [287] [SynForceNet: A Force-Driven Global-Local Latent Representation Framework for Lithium-Ion Battery Fault Diagnosis](https://arxiv.org/abs/2603.23265)
*Rongxiu Chen,Yuting Su*

Main category: cs.LG

TL;DR: Online battery fault diagnosis network using deep anomaly detection with kernel one-class classification and minimum-volume estimation, enhanced by mechanical constraints and STDP-based dynamic representations, validated on 8.6M EV data points.


<details>
  <summary>Details</summary>
Motivation: Online safety fault diagnosis is critical for lithium-ion batteries in EVs, especially under complex and rare safety-critical conditions during real-world operation where traditional methods may fail.

Method: Deep anomaly detection framework combining kernel one-class classification and minimum-volume estimation, incorporating mechanical constraints and spike-timing-dependent plasticity (STDP)-based dynamic representations to improve fault characterization and create a more compact normal-state boundary.
...
Conclusion: The method demonstrates the promise of integrating deep learning with physical constraints and neural dynamics for battery safety diagnosis, revealing possible shared causal structures across different fault types.
...
### [288] [SafeSeek: Universal Attribution of Safety Circuits in Language Models](https://arxiv.org/abs/2603.23268)
*Miao Yu,Siyuan Fu,Moayad Aloqaily,Zhenhong Zhou,Safa Otoum,Xing fan,Kun Wang,Yufei Guo,Qingsong Wen*

Main category: cs.LG

TL;DR: SAFE introduces a unified safety interpretability framework that identifies functionally complete safety circuits in LLMs via optimization with differentiable binary masks, enabling both circuit localization and efficient safety fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing safety attribution methods lack generalization and reliability due to heuristic, domain-specific metrics and search algorithms, making it difficult to systematically identify safety-critical functional components in LLMs.

Method: Proposes SAFE with differentiable binary masks to extract multi-granular circuits through gradient descent on safety datasets, and integrates Safety Circuit Tuning to utilize sparse circuits for efficient safety fine-tuning.

Result: Identified a backdoor circuit with 0.42% sparsity (ablation reduces ASR from 100% → 0.4% while retaining >99% utility) and an alignment circuit with 3.03% heads and 0.79% neurons (removal increases ASR from 0.8% → 96.9%).

Conclusion: SAFE provides a unified, optimization-based framework for identifying functionally complete safety circuits in LLMs, enabling both precise safety attribution and efficient safety fine-tuning across different safety scenarios.
...
### [289] [A Comparative Study of Machine Learning Models for Hourly Forecasting of Air Temperature and Relative Humidity](https://arxiv.org/abs/2603.23282)
*Jiaqi Dong*

Main category: cs.LG

TL;DR: XGBoost outperforms six other ML models for hourly temperature and humidity forecasting in Chongqing, achieving best accuracy with MAE of 0.302°C and 1.271% RH.


<details>
  <summary>Details</summary>
Motivation: Accurate short-term forecasting of air temperature and relative humidity is critical for urban management in topographically complex cities like Chongqing, China, where terrain complexity poses challenges for meteorological prediction.

Method: Systematic comparison of seven ML models (XGBoost, Random Forest, SVR, MLP, Decision Tree, LSTM, CNN-LSTM) using unified framework: data preprocessing, lag-feature construction, rolling statistical features, and time-series validation for hourly prediction with real-world open data.

Result: XGBoost achieves best overall performance with test MAE of 0.302°C for air temperature and 1.271% for relative humidity, with average R² of 0.989 across both forecasting tasks, outperforming other models in predictive accuracy and robustness.

Conclusion: Tree-based ensemble learning (particularly XGBoost) demonstrates strong effectiveness for structured meteorological time-series forecasting, providing practical guidance for intelligent meteorological forecasting in mountainous cities with complex topography.
...
### [290] [Robustness Quantification for Discriminative Models: a New Robustness Metric and its Application to Dynamic Classifier Selection](https://arxiv.org/abs/2603.23318)
*Rodrigo F. L. Lassance,Jasper De Bock*

Main category: cs.LG

TL;DR: Proposes a new robustness metric for probabilistic discriminative classifiers that works with any feature type, enabling reliability assessment and dynamic classifier selection.


<details>
  <summary>Details</summary>
Motivation: Existing robustness quantification methods have limited applicability - they require generative models, specific architectures, or discrete features. There's a need for a more general robustness metric applicable to any probabilistic discriminative classifier and feature type.

Method: Develops a new robustness metric that measures how much uncertainty a classifier can withstand before changing predictions. The metric is designed to work with any probabilistic discriminative classifier and any feature type (continuous or discrete).
...
Conclusion: The new robustness metric overcomes limitations of existing methods by being universally applicable to probabilistic discriminative classifiers and all feature types, providing a foundation for improved reliability assessment and dynamic classifier selection.
...
### [291] [Central Dogma Transformer III: Interpretable AI Across DNA, RNA, and Protein](https://arxiv.org/abs/2603.23361)
*Nobuyuki Ota*

Main category: cs.LG

TL;DR: CDT-III is a biological AI model that predicts cellular responses across DNA, RNA, and protein levels using a Virtual Cell Embedder architecture, achieving high accuracy and demonstrating that protein prediction improves RNA performance and DNA interpretability.


<details>
  <summary>Details</summary>
Motivation: Current biological AI models predict complex cellular responses but their learned representations remain disconnected from the underlying molecular processes they aim to capture, limiting interpretability and mechanistic understanding.

Method: CDT-III uses a two-stage Virtual Cell Embedder architecture that mirrors cellular compartmentalization: VCE-N models transcription in the nucleus and VCE-C models translation in the cytosol, extending mechanism-oriented AI across the full central dogma (DNA, RNA, protein).

Result: On five held-out genes, CDT-III achieves per-gene RNA correlation r=0.843 and protein correlation r=0.969. Adding protein prediction improves RNA performance from r=0.804 to 0.843, demonstrating downstream task regularization. Protein supervision increases CTCF enrichment by 30% for DNA-level interpretability. Applied to CD52 knockdown, the model predicts 29/29 protein changes correctly and rediscovers 5 of 7 known clinical side effects without clinical data.

Conclusion: CDT-III successfully connects AI representations to molecular mechanisms across the central dogma, showing that protein supervision improves both prediction accuracy and interpretability. The gradient-based side effect profiling approach enables screening of all 2,361 genes without new experiments, demonstrating practical utility for drug discovery and biological understanding.
...
### [292] [Graph Energy Matching: Transport-Aligned Energy-Based Modeling for Graph Generation](https://arxiv.org/abs/2603.23398)
*Michal Balcerak,Suprosana Shit,Chinmay Prabhakar,Sebastian Kaltenbach,Michael S. Albergo,Yilun Du,Bjoern Menze*

Main category: cs.LG

TL;DR: GEM is a graph generative framework that closes the fidelity gap between discrete energy-based models and diffusion models by learning a permutation-invariant potential energy for transport-aligned guidance and refined sampling.


<details>
  <summary>Details</summary>
Motivation: Discrete energy-based models for graphs capture relative likelihoods for composable inference but struggle with efficient, high-quality sampling due to spurious local minima in off-support regions, creating a fidelity gap compared to discrete diffusion models.

Method: GEM learns a permutation-invariant potential energy inspired by the JKO scheme's transport map optimization perspective, providing transport-aligned guidance from noise to data while refining samples in high-likelihood regions. It uses an energy-based switch to bridge gradient-guided transport with mixing regime exploration.

Result: On molecular graph benchmarks, GEM matches or exceeds strong discrete diffusion baselines in sample quality. It enables targeted exploration at inference time for compositional generation, property-constrained sampling, and geodesic interpolation between graphs.

Conclusion: GEM closes the fidelity gap for discrete energy-based graph models by combining transport-aligned guidance with energy-based refinement, enabling both high-quality sampling and composable probabilistic inference capabilities.
...
### [293] [SortedRL: Accelerating RL Training for LLMs through Online Length-Aware Scheduling](https://arxiv.org/abs/2603.23414)
*Yiqi Zhang,Huiqiang Jiang,Xufang Luo,Zhihe Yang,Chengruidong Zhang,Yifei Shen,Dongsheng Li,Yuqing Yang,Lili Qiu,Yang You*

Main category: cs.LG

TL;DR: SortedRL is an online length-aware scheduling strategy that reorders rollout samples by output length to accelerate RL training for LLMs, reducing training bubble ratios by over 50% while improving performance on reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: RL training for LLMs is bottlenecked by the rollout phase, which can consume up to 70% of training time when generating long trajectories due to slow autoregressive generation and synchronization overhead between rollout and policy updates.

Method: SortedRL reorders rollout samples based on output lengths, prioritizing short samples to form groups for early updates. This enables large rollout batches, flexible update batches, and near on-policy micro-curriculum construction. It incorporates a cache-based mechanism to control off-policy training degree and uses a dedicated RL infrastructure with stateful controller and rollout buffer.

Result: SortedRL reduces RL training bubble ratios by over 50% and achieves 3.9% to 18.4% superior performance over baselines given the same amount of data on tasks including logical puzzles, AIME 24, Math 500, and Minerval using LLaMA-3.1-8B and Qwen-2.5-32B models.

Conclusion: SortedRL effectively addresses the rollout bottleneck in RL training for LLMs through intelligent length-aware scheduling, significantly improving training efficiency while maintaining or enhancing model performance on complex reasoning tasks.
...
### [294] [Similarity-Aware Mixture-of-Experts for Data-Efficient Continual Learning](https://arxiv.org/abs/2603.23436)
*Connor Mclaughlin,Nigel Lee,Lili Su*

Main category: cs.LG

TL;DR: Proposes an adaptive mixture-of-experts framework for continual learning with limited per-task data and arbitrary task overlaps, using incremental global pooling and instance-wise prompt masking to enable efficient knowledge transfer while preventing negative interference.


<details>
  <summary>Details</summary>
Motivation: Address the challenging continual learning setting where each task has limited data samples and tasks may overlap arbitrarily without prior knowledge, which is more realistic but substantially harder due to data scarcity necessitating effective knowledge transfer and unstructured overlapping risking negative knowledge transfer.

Method: Adaptive mixture-of-experts framework over pre-trained models with two key components: 1) incremental global pooling that gradually introduces prompts over time to mitigate prompt association noise, and 2) instance-wise prompt masking that decomposes incoming task samples into in-distribution (aligning with current prompts) and out-of-distribution (requiring new prompts) categories.

Result: Experiments across varying data volumes and inter-task similarity show the method enhances sample efficiency and is broadly applicable, demonstrating effective handling of data scarcity and arbitrary task overlaps.

Conclusion: The proposed adaptive MoE framework with incremental global pooling and instance-wise prompt masking successfully addresses the challenging continual learning scenario with limited per-task data and arbitrary task overlaps, enabling strategic leveraging of task overlaps while preventing negative mutual interference.
...
### [295] [End-to-End Efficient RL for Linear Bellman Complete MDPs with Deterministic Transitions](https://arxiv.org/abs/2603.23461)
*Zakaria Mhammedi,Alexander Rakhlin,Nneka Okolo*

Main category: cs.LG

TL;DR: Efficient RL algorithm for linear Bellman complete MDPs with deterministic transitions, requiring only argmax oracle for large action spaces.


<details>
  <summary>Details</summary>
Motivation: Prior computationally efficient algorithms for linear Bellman complete MDPs are limited to small action spaces or require strong oracle assumptions. There's a need for efficient algorithms that work with large/infinite action spaces under realistic assumptions.

Method: Develops a computationally efficient algorithm for linear Bellman complete MDPs with deterministic transitions, stochastic initial states, and stochastic rewards. For finite action spaces, it's end-to-end efficient; for large/infinite action spaces, it requires only a standard argmax oracle over actions.

Result: The algorithm learns an ε-optimal policy with sample and computational complexity polynomial in the horizon, feature dimension, and 1/ε.

Conclusion: Provides the first computationally efficient algorithm for linear Bellman complete MDPs that handles large action spaces with only an argmax oracle, bridging the gap between statistical tractability and computational efficiency.
...
### [296] [Byzantine-Robust and Differentially Private Federated Optimization under Weaker Assumptions](https://arxiv.org/abs/2603.23472)
*Rustem Islamov,Grigory Malinovsky,Alexander Gaponov,Aurelien Lucchi,Peter Richtárik,Eduard Gorbunov*

Main category: cs.LG

TL;DR: Byz-Clip21-SGD2M: A federated learning algorithm that integrates Byzantine-robust aggregation with double momentum and gradient clipping to provide both differential privacy and Byzantine robustness with convergence guarantees under standard assumptions.


<details>
  <summary>Details</summary>
Motivation: Federated learning has privacy vulnerabilities (gradient leakage) and security risks (Byzantine attacks from malicious servers). Existing approaches have limitations: unrealistic assumptions (bounded gradients), need for auxiliary datasets, or lack of convergence guarantees. Need unified framework for DP and Byzantine robustness.

Method: Proposes Byz-Clip21-SGD2M algorithm combining robust aggregation with double momentum and carefully designed gradient clipping. Uses standard L-smoothness and σ-sub-Gaussian gradient noise assumptions, relaxing prior restrictive conditions.

Result: Proves high-probability convergence guarantees, recovers state-of-the-art convergence rates without adversaries, and improves utility guarantees under Byzantine and DP settings. Empirical validation on CNN/MLP models with MNIST dataset.

Conclusion: The proposed algorithm addresses key limitations of prior work by providing a unified framework for differential privacy and Byzantine robustness with provable convergence under standard assumptions, validated empirically.
...
### [297] [Estimating Flow Velocity and Vehicle Angle-of-Attack from Non-invasive Piezoelectric Structural Measurements Using Deep Learning](https://arxiv.org/abs/2603.23496)
*Chandler B. Smith,S. Hales Swift,Andrew Steyer,Ihab El-Kady*

Main category: cs.LG

TL;DR: Non-intrusive method using piezoelectric sensors and CNN to estimate freestream velocity and angle of attack from structural vibrations, validated in hypersonic wind tunnel experiments.


<details>
  <summary>Details</summary>
Motivation: Accurate estimation of aerodynamic state variables (freestream velocity and AoA) is crucial for load prediction, flight control, and model validation, but traditional flow instrumentation can be intrusive. This work aims to develop a non-intrusive alternative using structural vibration measurements.

Method: Uses dense array of piezoelectric sensors mounted on interior aeroshell skin to capture vibrations from turbulent boundary layer pressure fluctuations. A convolutional neural network is trained to invert structural responses to recover velocity and AoA. Experimental validation in hypersonic wind tunnel (Mach 5 and 8) with both constant and varying conditions. Post-processing with short-window moving-median filter to reduce variance.

Result: After post-processing, achieves mean velocity error below 2.27 m/s (0.21%) and mean AoA error of 0.44° (8.25%) on held-out test data. Demonstrates feasibility of vibration-based estimation in controlled laboratory environment.

Conclusion: The method successfully demonstrates non-intrusive estimation of aerodynamic state variables from structural vibrations using CNN approach, validated through comprehensive hypersonic wind tunnel experiments with promising accuracy.
