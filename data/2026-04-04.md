...
TL;DR: DOne框架通过解耦结构理解和元素渲染来解决VLMs在Design-to-Code生成中的"整体瓶颈"问题，在复杂布局数据集上显著提升视觉相似度和元素对齐精度


<details>
  <summary>Details</summary>
Motivation: 当前Vision Language Models在Design-to-Code生成中存在"整体瓶颈"问题，无法同时兼顾高层次结构层次和细粒度视觉细节，导致布局失真或使用通用占位符

Method: 提出DOne端到端框架，包含：(1)学习型布局分割模块分解复杂设计，避免启发式裁剪限制；(2)专门化混合元素检索器处理UI组件的极端宽高比和密度；(3)模式引导的生成范式连接布局和代码

Result: 在HiFi2Code基准测试中，DOne在高层视觉相似度（如GPT Score提升超过10%）和细粒度元素对齐方面均优于现有方法；人工评估显示生产力提升3倍且视觉保真度更高

Conclusion: DOne通过解耦结构理解和元素渲染的方法有效解决了VLMs在Design-to-Code生成中的瓶颈问题，在复杂布局场景下实现了显著的性能提升和生产力增益
...
TL;DR: Look Twice (LoT)是一个无需训练、推理时优化的框架，通过注意力模式识别相关视觉区域和文本证据，使用提示标记重新关注关键信息，提升多模态大语言模型在知识密集型视觉问答中的表现。
...
Method: 提出Look Twice (LoT)框架，利用模型注意力模式估计与查询相关的视觉区域和检索到的文本元素，然后基于这些高亮证据生成答案。通过轻量级的提示级标记来突出显示选定的线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的VQA基准测试中，相比零样本MLLMs取得了持续改进。在无文本上下文的纯视觉中心和幻觉导向基准测试上的额外评估进一步表明，仅视觉证据高亮就能提升模型性能，且无需额外训练或架构修改。

Conclusion: LoT是一个有效的推理时优化框架，通过注意力引导的证据高亮机制，显著提升了预训练多模态大语言模型在整合多模态证据方面的能力，特别是在知识密集型视觉问答任务中。
...
TL;DR: MedQwen：一种参数高效的医学视觉语言模型，通过谱路由混合专家（MoE）架构和理论驱动的缩放规则，在保持基础架构不变的情况下，实现接近全参数微调的性能，同时大幅减少可训练参数并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在医学影像领域缺乏鲁棒性，存在跨数据集干扰和数据体制敏感性问题。在临床工作流中，数据和任务按顺序到达，传统的持续训练会导致灾难性遗忘。需要开发参数高效且能适应医学领域特殊需求的模型。

Method: 提出MedQwen模型，结合谱路由混合专家（MoE）架构：1）从预训练权重的非重叠奇异值分解（SVD）片段初始化每个专家；2）引入残差补偿和缩放方案，确保在分布偏移下稳定的专家专业化和一致路由；3）采用理论驱动的缩放规则，使低秩更新与全秩全参数微调的MoE对齐。

Result: 在23个医学数据集（涵盖视觉问答、报告生成、放射学分类和幻觉缓解）上表现优异：1）零样本分类任务中，仅用全参数微调1/339的可训练参数即达到相近性能；2）将顺序遗忘降至约5%，而基线模型性能下降20-50%。

Conclusion: MedQwen通过谱路由MoE架构和理论驱动的缩放规则，成功解决了医学视觉语言模型中的跨数据集干扰、数据体制敏感性和灾难性遗忘问题，实现了参数高效且鲁棒的持续学习能力。
...
TL;DR: 该研究提出了一种基于视觉Transformer的模型，用于从美式足球训练视频中检测危险擒抱动作，并扩展了相关数据集，通过不平衡感知训练显著提高了危险动作的召回率。


<details>
  <summary>Details</summary>
Motivation: 在接触性运动中及早识别危险动作能够及时干预并提高运动员安全性。美式足球中的擒抱技术评估对于预防伤害至关重要，但现有数据集有限且检测方法有待改进。

Method: 研究扩展了数据集至733个单运动员-假人擒抱视频片段，每个片段围绕首次接触点进行时间定位，并使用标准化擒抱技术评估(SATT-3)的击打区域组件进行标注。采用基于视觉Transformer的模型，结合不平衡感知训练来处理类别不平衡问题。

Result: 在交叉验证下获得危险召回率0.67和危险F1分数0.59。相较于先前在较小数据集上的基线(危险召回率0.58；危险F1分数0.56)，危险召回率提高了超过8个百分点，且是在更大数据集上实现的。
...
<details>
  <summary>Details</summary>
Motivation: 现有姿态估计模型在蹦床体操这类包含极端人体姿态和非常规视角的场景中表现不佳，需要专门优化

Method: 开发合成蹦床姿态数据集(STP)：从动作捕捉数据拟合参数化人体模型，生成多视角逼真图像；用此数据微调ViTPose模型

Result: 2D姿态估计达到SOTA，弥合了常规姿态与极端姿态的性能差距；3D三角化中MPJPE降低12.5mm，相比预训练模型提升19.6%

Conclusion: 合成数据微调能有效解决特定领域（如蹦床体操）的姿态估计问题，显著提升模型在极端姿态和非常规视角下的性能
...
Conclusion: 自助法可作为实用的正则化工具，当使用注意力分数作为ViT解释时，能有效提高注意力图的解释性和可靠性。
...
TL;DR: 研究发现CNN作为视觉系统模型的质量与其纹理感知对齐度无关，表明纹理感知机制不同于基于物体识别训练的CNN常用建模方法


<details>
  <summary>Details</summary>
Motivation: 探究CNN特征相关性（Gram矩阵）是否自发对齐于纹理的感知内容，以及被视为更好视觉系统模型的CNN是否拥有更接近人类的纹理表示

Method: 比较多样化CNN池中特征相关性捕获的感知内容，并将其与Brain-Score测量的模型与哺乳动物视觉系统的感知对齐度进行对比

Result: 令人惊讶地发现，CNN作为视觉系统模型的常规质量度量与其与人类纹理感知的对齐度之间没有关联
...
TL;DR: AffordTissue：用于胆囊切除术中预测工具-动作特定组织可操作性区域的密集热图预测框架，通过多模态架构显著提升手术自动化安全性


<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在局限性：缺乏对器械在组织表面交互位置的预测能力，缺少强制执行工具-动作特定安全交互区域的显式条件输入，这阻碍了手术自动化向临床应用的转化

Method: 提出AffordTissue多模态框架，结合：1）时序视觉编码器捕捉多视角下的工具运动和组织动态；2）语言条件化实现不同器械-动作对的泛化；3）DiT风格解码器进行密集可操作性预测。建立了首个组织可操作性基准数据集，包含103个胆囊切除术的15,638个视频片段，涵盖4种器械（钩、抓钳、剪刀、夹钳）和6种工具-动作对

Result: 实验显示显著优于视觉语言模型基线（20.6像素ASSD vs. 60.2像素的Molmo-VLM），任务特定架构在大规模基础模型上表现出色，为密集手术可操作性预测提供了更好的性能

Conclusion: AffordTissue通过预测工具-动作特定组织可操作性区域，为安全手术自动化提供了显式空间推理，可能实现向适当组织区域的显式策略指导，并在器械偏离预测安全区域时实现早期安全停止
...
TL;DR: GRAZE：一种无需训练、无需标注接触示例的框架，用于在美式足球训练视频中定位首次接触点（FPOC），通过目标检测、运动感知时序推理和像素级接触验证实现鲁棒定位。


<details>
  <summary>Details</summary>
Motivation: 美式足球训练产生大量视频，但关键交互（接触）只占长视频中的短暂窗口。可靠的生物力学分析需要时空定位来识别交互实体和接触起始点。现有方法面临相机运动、杂乱场景、多个相似装备运动员以及冲击时快速姿态变化等挑战。

Method: GRAZE采用训练免费的三阶段流水线：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时序推理进行细化；3) 使用SAM2作为显式像素级接触验证器，而非仅依赖检测置信度。候选发现与接触确认的分离使方法对杂乱场景和冲击附近不稳定的目标检测具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的视频中产生有效输出，在77.5%的视频中将FPOC定位在±10帧内，在82.7%的视频中定位在±20帧内。这表明无需任务特定训练即可在真实训练视频中实现帧级准确的接触起始定位。

Conclusion: GRAZE证明了在无约束训练视频中实现帧级准确接触起始定位的可行性，无需任务特定训练或标注接触示例。该方法通过分离候选发现和接触确认，对杂乱场景和冲击附近不稳定的目标检测具有鲁棒性，为生物力学分析提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的开放词汇3D场景理解方法存在两个关键限制：1) 非结构化重叠高斯分布导致的空间模糊性，需要概率特征注册；2) 基于物体级掩码的特征池化导致的多级语义模糊性，稀释了细粒度细节。

Method: 提出基于稀疏体素光栅化的结构化、不相交几何表示框架，通过单目深度和法线先验进行正则化，建立稳定几何基础。实现确定性、置信感知的特征注册过程，并利用基础模型AM-RADIO的密集对齐特性解决多级模糊性，避免分层训练的计算开销。

Result: 在开放词汇3D物体检索和点云理解基准测试中达到最先进性能，特别是在特征注册方法通常失败的细粒度查询任务上表现优异，有效抑制了3DGS中常见的语义渗漏伪影。

Conclusion: 稀疏体素光栅化作为结构化几何表示优于非结构化3D高斯泼溅，能够提供确定性特征注册并解决多级语义模糊性，为开放词汇3D场景理解提供了更稳定、精确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 从第一人称视角视频理解和预测物体运动对具身感知和交互至关重要。现有生成模型缺乏显式物理推理，难以生成物理一致的6DoF轨迹，面临遮挡、快速运动和物理合理性等挑战。
...
Conclusion: EgoFlow展示了基于流的生成模型在可扩展和物理基础的第一人称运动理解方面的潜力，通过结合生成建模和可微物理约束，实现了连贯可控的运动生成。
...
Result: 通过三个案例研究展示了流程的灵活性：涵盖面部和全身运动、2D和3D数据、个体与多智能体行为，证明同一分析流程可适应不同实验环境

Conclusion: 该通用分析流程能够从复杂姿态时间序列中提取理论上有意义的洞察，支持跨不同实验环境的人类运动行为分析
...
TL;DR: UniRecGen是一个统一框架，将前馈重建和扩散生成相结合，通过共享规范空间和分离协作学习解决稀疏视图3D建模中重建保真度与生成合理性之间的基本矛盾。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的基本矛盾：前馈重建方法虽然高效且与输入对齐，但缺乏全局先验导致结构不完整；而基于扩散的生成方法能提供丰富的几何细节，但难以保持多视图一致性。

Method: 1. 将两种模型在共享规范空间中对齐，解决坐标空间、3D表示和训练目标的内在冲突；2. 采用分离协作学习，保持训练稳定性并在推理时实现无缝协作；3. 重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和完成几何结构。

Result: 实验结果表明，UniRecGen在从稀疏观测创建完整一致的3D模型方面，实现了卓越的保真度和鲁棒性，优于现有方法。

Conclusion: UniRecGen通过统一重建和生成范式，有效解决了稀疏视图3D建模的核心挑战，实现了保真度与合理性的平衡，为从稀疏观测创建高质量3D模型提供了有效解决方案。
...
TL;DR: TAG框架通过非参数纹理恢复解决热成像中的鬼影效应，首次实现幽灵人脸的细节恢复，超越现有HADAR技术并揭示其有效性边界。


<details>
  <summary>Details</summary>
Motivation: 热成像中的鬼影效应导致细节纹理丢失，现有HADAR技术仅适用于均匀材料场景，而现实世界中材料非均匀性普遍存在，需要通用抗鬼影成像方案。

Method: 提出TAG（热抗鬼影）通用计算热成像框架，利用高光谱光子流进行非参数纹理恢复，解决材料非均匀性问题。

Result: 首次实现幽灵人脸的细节和表情恢复，在各种场景中普遍优于HADAR，揭示材料非均匀性对HADAR有效性的影响，并演示热3D拓扑对齐和情绪检测。
...
TL;DR: PBSeg：一种针对无人机图像的高效原型分割框架，通过原型交叉注意力和多尺度特征提取，在保持分割质量的同时降低计算复杂度


<details>
  <summary>Details</summary>
Motivation: 无人机图像语义分割面临极端尺度变化、复杂边界和边缘设备计算资源有限的挑战。现有方法要么计算开销大（如transformer），要么难以捕捉高分辨率航拍场景的细节（轻量级方法）。
...
Conclusion: PBSeg为无人机图像语义分割提供了一种高效解决方案，在计算开销和分割精度之间取得了良好平衡，适用于边缘设备部署。
...
TL;DR: ReFlow：用于单目动态场景重建的统一框架，通过自校正流匹配机制学习3D运动，无需外部密集运动引导


<details>
  <summary>Details</summary>
Motivation: 现有方法在动态区域初始化不完整，导致重建和运动估计不稳定，通常需要依赖预计算光流等外部密集运动引导，这会引入额外复杂性和误差传播

Method: 1) 完整规范空间构建模块增强静态和动态区域初始化；2) 基于分离的动态场景建模模块解耦静态和动态组件；3) 核心的自校正流匹配机制：全流匹配对齐3D场景流与时间变化的2D观测，相机流匹配确保静态对象的多视角一致性

Result: 在多种场景下的广泛实验表明，ReFlow实现了优越的重建质量和鲁棒性，为单目4D重建建立了新的自校正范式

Conclusion: ReFlow通过自校正流匹配机制，无需外部密集运动引导，实现了鲁棒准确的动态场景重建，为单目动态场景重建提供了统一框架
...
TL;DR: VideoZeroBench：一个用于长视频问答的分层基准测试，通过严格的时空证据验证来评估视频多模态大语言模型，揭示当前模型在基于证据的推理方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 夸大的分数掩盖了细粒度视觉理解和推理的缺陷；(2) 答案正确性评估通常不验证模型是否识别了支持其预测的精确时空证据。需要开发能严格验证时空证据的基准测试来评估模型的真实推理能力。

Method: 提出了VideoZeroBench分层基准测试，包含500个手动标注的问题，涵盖13个领域，每个问题都配有作为证据的时间区间和空间边界框。引入五级评估协议，逐步收紧证据要求，以分离答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro的正确回答率也低于17%。当施加定位约束时，性能急剧下降：在要求正确答案和精确时空定位时（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。这揭示了表面答案正确性与真实基于证据的推理之间存在显著差距。
...
TL;DR: 提出卫星图像无关训练框架，通过多视角无人机序列重建3D场景并生成伪正射影像，实现无卫星数据训练的跨视图地理定位


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角地理定位方法依赖卫星图像进行训练（配对监督或无监督对齐），但在卫星数据不可用或受限的实际部署场景中存在局限性

Method: 三阶段框架：1) 使用3D高斯溅射从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像，并用轻量级几何引导修复完善纹理；3) 从无人机数据学习Fisher向量聚合模型，提取DINOv3补丁特征用于跨视图检索

Result: 在University-1652和SUES-200数据集上，SFT框架显著优于卫星无关泛化基线，缩小了与使用卫星图像训练方法的性能差距

Conclusion: 提出的卫星无关训练框架能够在没有卫星数据的情况下实现有效的跨视图地理定位，通过几何重建和伪正射影像生成解决了卫星数据依赖问题，具有实际部署价值
...
TL;DR: ROSETTA：一种鲁棒的开集测试时适应方法，通过角度损失和特征范数损失解决熵最小化与最大化之间的内在冲突，在保持ID分类性能的同时提升OOD检测能力。


<details>
  <summary>Details</summary>
Motivation: 开集测试时适应（OSTTA）面临的主要挑战是：在存在分布偏移的情况下，模型需要同时处理协变量偏移的ID样本和OOD样本。现有方法通常结合熵最小化（保持ID性能）和熵最大化（增强OOD检测），但这两种目标存在内在冲突，导致在csID分类和csOOD检测之间需要权衡。

Method: 提出了ROSETTA方法：1）分析熵最大化在OSTTA中的局限性；2）引入角度损失来调节特征范数大小；3）使用特征范数损失来抑制csOOD样本的logits。这些目标共同构成一个鲁棒的开集测试时适应框架。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C等数据集上实现了强大的OOD检测能力，同时保持了较高的ID分类性能。在Cityscapes数据集上的实验验证了该方法在真实世界语义分割中的有效性，在HAC数据集上的结果证明了其在不同开集TTA设置中的适用性。

Conclusion: ROSETTA通过引入角度损失和特征范数损失，有效解决了开集测试时适应中熵最小化与最大化之间的冲突，在多个基准数据集和实际应用场景中展现了优越的性能，为处理协变量偏移下的开集适应问题提供了有效的解决方案。
...
TL;DR: F3DGS是一个用于去中心化多智能体3D重建的联邦3D高斯泼溅框架，通过共享几何骨架和可见性感知聚合解决分布式机器人系统中的通信开销和几何不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅（3DGS）流水线假设集中式访问所有观测数据，这在分布式机器人设置中不适用，因为智能体独立运行且集中式数据聚合可能受限。直接将集中式训练扩展到多智能体系统会引入通信开销和几何不一致问题。

Method: 首先通过注册来自多个客户端的局部合并LiDAR点云构建共享几何骨架，初始化全局3DGS模型。在联邦优化期间，固定高斯位置以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度和球谐系数）。服务器使用可见性感知聚合更新，根据每个客户端观察每个高斯的频率加权其贡献，解决多智能体探索中的部分可观测性挑战。

Result: 实验表明F3DGS在实现与集中式训练相当的重建质量的同时，支持跨智能体的分布式优化。收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集进行评估。

Conclusion: F3DGS为分布式机器人系统中的3D重建提供了一种有效的联邦学习方法，解决了现有集中式方法的局限性。数据集、开发工具包和源代码将公开发布。
...
TL;DR: 该论文提出了一种基于稀疏自编码器和视觉语言提示的自动化形态特征标注方法，构建了包含8万条标注的Bioscan-Traits数据集，为大规模生态研究提供了可扩展的形态特征提取方案。


<details>
  <summary>Details</summary>
Motivation: 形态特征是生物与环境相互作用的重要指标，但传统提取方法依赖专家标注，成本高、效率低，限制了在大规模生态研究中的应用。主要瓶颈在于缺乏连接生物图像与特征标注的高质量数据集。

Method: 使用在基础模型特征上训练的稀疏自编码器，获得单语义、空间定位的神经元，这些神经元能一致地在有意义的形态部位上激活。基于此特性，开发了一个特征标注流程：首先定位显著区域，然后使用视觉语言提示生成可解释的特征描述。

Result: 构建了Bioscan-Traits数据集，包含8万条特征标注，覆盖BIOSCAN-5M中的1.9万张昆虫图像。人工评估确认了生成形态描述的生物合理性。通过全面的消融研究评估了设计敏感性，系统性地变化关键设计选择并测量其对特征描述质量的影响。

Conclusion: 通过模块化流水线而非昂贵的人工标注来标注特征，为大规模形态分析提供了可扩展的方法，为基础模型注入了具有生物学意义的监督，弥合了生态相关性与机器学习实用性之间的差距。
...
TL;DR: LivingWorld是一个从单张图像生成具有环境动态的4D世界的交互式框架，通过渐进构建全局一致的运动场来解决场景扩展中的运动一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D场景生成方法主要关注静态几何重建，而忽略了云、水、烟雾等场景尺度的环境动态建模。这些动态建模的挑战在于：运动需要在扩展的场景中保持一致性，同时支持低延迟的用户反馈。

Method: 1. 渐进构建全局一致的运动场；2. 引入几何感知对齐模块解决视角间的方向和尺度模糊性；3. 使用基于哈希的紧凑运动场表示，支持高效查询和稳定传播；4. 支持渲染时的双向运动传播，无需昂贵的视频细化。

Result: 在单个RTX 5090 GPU上，每个新场景扩展步骤需要9秒生成，运动对齐和运动场更新需要3秒，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架成功解决了从单张图像生成具有环境动态的4D世界的挑战，通过全局一致的运动场表示和高效的计算方法，实现了交互式的4D世界生成。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但在现实场景中，由于3D标注成本高昂，通常只有部分目标被标注，这种稀疏标注设置使得现有方法难以有效工作

Method: 提出包含两个关键模块的框架：1) 道路感知补丁增强(RAPA)：利用稀疏标注，将分割的目标补丁增强到道路区域，同时保持3D几何一致性；2) 原型过滤(PBF)：通过原型相似性和深度不确定性过滤预测结果，生成高质量伪标签，维护全局2D RoI特征原型，选择既与学习原型特征一致又具有可靠深度估计的伪标签
...
<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分。需要一种基于物理原理的可靠方法来验证视频的真实性。

Method: 利用莫尔效应：相机拍摄紧凑双层光栅结构时产生的干涉条纹。推导莫尔运动不变量，证明条纹相位与光栅图像位移通过光学几何线性耦合，与观看距离和光栅结构无关。验证器从视频中提取两个信号并测试其相关性。

Result: 在真实拍摄和多个最先进生成器产生的AI生成视频上验证不变量，发现真实视频与AI生成视频产生显著不同的相关性签名，表明该方法能有效区分两者。

Conclusion: 确定性光学现象可作为物理基础的可验证签名来对抗AI生成视频，为视频真实性认证提供了一种稳健方法。
...
TL;DR: DynaVid：利用合成运动数据（光流）训练视频扩散模型，通过两阶段生成框架（运动生成器+运动引导视频生成器）提升动态运动生成的真实性和可控性
...
Method: 提出DynaVid框架：1）利用计算机图形流水线渲染的合成光流数据作为运动表示；2）采用两阶段生成框架：运动生成器先合成运动（光流），然后运动引导的视频生成器基于该运动条件生成视频帧。这种解耦设计使模型能从合成数据学习动态运动模式，同时从真实视频保持视觉真实性。

Result: 在剧烈人体运动生成和极端相机运动控制两个具有挑战性的场景中验证了框架有效性。大量实验表明DynaVid在动态运动生成和相机运动控制方面显著提升了真实性和可控性。

Conclusion: 通过利用合成运动数据（光流）并采用解耦的两阶段生成框架，DynaVid成功解决了视频扩散模型在动态运动生成中的局限性，实现了从合成数据学习运动模式同时保持真实视觉外观的目标。
...
TL;DR: 提出了一种无域标识符和无示例的增量学习框架，用于具身多媒体系统，通过解耦表示机制和权重融合策略实现鲁棒的连续环境适应


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间中持续交互时面临动态环境分布漂移的挑战。现有域增量感知方法通常依赖测试阶段预先获取的域标识符，限制了在未知交互场景中的实用性。同时，模型容易过拟合到特定上下文感知噪声，导致泛化能力不足和灾难性遗忘。

Method: 1) 设计解耦表示机制，去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2) 使用权重融合策略，在参数空间中动态整合新旧环境知识，无需存储历史数据即可适应新分布，同时最大程度保留旧环境的判别能力。

Result: 在多个标准基准数据集上的大量实验表明，该方法在完全无示例和无域标识符的设置下显著减少了灾难性遗忘，其准确率优于现有的最先进方法。

Conclusion: 该方法为具身多媒体系统提供了一种实用的连续环境适应解决方案，通过解耦表示和权重融合实现了鲁棒的增量学习，解决了域标识符依赖和灾难性遗忘问题。
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，通过转移低频频谱成分来增强rPPG模型对跨域外观变化的鲁棒性
...
Method: 提出频率域适应(FDA)策略，通过转移低频频谱成分来建模外观变化；进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性指导原始和FDA转移表示之间的对齐

Result: 广泛的跨数据集实验表明，FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力

Conclusion: FDA和HOT框架为解决rPPG跨域泛化问题提供了原则性策略，通过建模外观变化和利用生理信号特性来提升模型的实际部署效果
...
Method: 1) 使用时序对齐的实例掩码和MLLM生成的句子嵌入监督高斯语义特征；2) 将2D光流与4D高斯桥接以微调运动；3) 引入几何感知的SDF约束和正则化项增强表面连续性

Result: 实现了时序一致的4D重建，同时支持实例分割和开放词汇查询，在动态前景建模中增强了时间连贯性
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频对象移除方法难以消除对象引发的副作用（阴影、反射、光照变化），同时保持整体一致性，这源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 提出双视角理解方法：1）外部：通过知识蒸馏方案，将视觉基础模型中对象与其引发效果的关系迁移到视频扩散模型；2）内部：提出帧间上下文交叉注意力机制，使每个去噪块基于目标区域周围未遮挡的上下文信息。
...
TL;DR: 提出一种端到端的共享注意力估计方法，通过群体检测同时实现群体识别和共享注意力估计，解决了现有方法不检测实际群体或假设单点共享注意力的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有共享注意力估计方法存在两个主要问题：1) 不检测实际关注共享注意力的群体；2) 假设图像中只有一个共享注意力点。这些问题限制了共享注意力检测的实际应用并影响了性能。

Method: 采用两步处理流程：1) 基于群体推理估计的个体注视注意力热图和群体成员标量生成共享注意力热图；2) 考虑初始共享注意力热图对初始群体成员关系进行细化，并最终预测共享注意力热图。

Result: 实验表明，该方法在群体检测和共享注意力估计方面优于其他方法。附加分析验证了所提出组件的有效性。

Conclusion: 提出的端到端方法通过同时进行群体检测和共享注意力估计，有效解决了现有方法的局限性，提高了共享注意力检测的实际应用性和性能。
...
Conclusion: SteerFlow是一个具有强理论保证的模型无关编辑框架，在保持源保真度的同时实现高质量文本引导图像编辑，并能扩展到复杂多轮编辑场景。
...
TL;DR: 该论文提出DPMO和RPS框架，从点标注生成密集人群实例分割，并在多个数据集上取得SOTA性能，同时设计新损失函数提升计数精度。


<details>
  <summary>Details</summary>
Motivation: 人群实例分割在监控和交通等领域有广泛应用，但现有数据集多为点标注，区域标注（如边界框）稀缺且不准确。直接应用SAM等大模型在密集人群场景效果不佳，需要从点标注生成高质量掩码标注并提升分割性能。
...
Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上取得最先进的人群实例分割性能。掩码监督的新损失函数在不同模型上均提升了计数性能。

Conclusion: DPMO和RPS框架有效解决了从点标注生成密集人群实例分割的挑战，掩码标注对提升计数精度有重要作用。该方法为人群分析任务提供了高质量的标注和强大的分割框架。
...
TL;DR: 提出几何感知的无人机跨视图地理定位框架，通过3D场景重建和虚拟鸟瞰图生成，统一粗粒度地点识别和细粒度姿态估计，在GNSS拒止环境下实现米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有无人机跨视图地理定位方法通常采用解耦的检索-姿态估计流程，将透视畸变视为外观噪声而非显式几何变换，难以处理倾斜无人机图像与正交卫星地图之间的严重几何差异。

Method: 提出几何感知框架：1) 使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景；2) 渲染虚拟鸟瞰图作为几何中介，将无人机视角正交化以对齐卫星图像；3) 引入卫星注意力块隔离候选位置交互，保持线性计算复杂度；4) 发布重新校准的University-1652数据集。

Result: 在重新校准的University-1652基准和SUES-200数据集上显著优于现有方法，实现鲁棒的米级定位精度，在复杂城市环境中表现出更好的泛化能力。

Conclusion: 通过显式建模3D场景几何，将透视畸变转化为可学习的几何变换，统一了地点识别和姿态估计，为GNSS拒止环境下的无人机地理定位提供了有效的端到端解决方案。
...
TL;DR: 本文构建了US-365K大规模超声图像-文本数据集和超声诊断分类法，提出了Ultrasound-CLIP语义感知对比学习框架，在超声图像分类和检索任务上取得SOTA性能。
...
Method: 1) 构建US-365K数据集（36.5万配对样本，52个解剖类别）；2) 建立超声诊断分类法（UDT），包含层次解剖分类和9维诊断属性框架；3) 提出Ultrasound-CLIP框架，引入语义软标签和语义损失；4) 构建基于UDAF文本表示的异构图模态，支持病灶-属性关系结构化推理。

Result: 在患者级数据划分下，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务上表现出强大的泛化能力。

Conclusion: 通过构建大规模超声数据集和结构化知识框架，结合语义感知对比学习，成功将视觉-语言预训练模型适配到超声领域，为超声诊断AI提供了有效的预训练解决方案。
...
TL;DR: 论文提出了一种使用自监督学习特征作为预训练视频扩散模型条件信号的方法，通过轻量级架构和训练策略解耦外观特征，实现视频域迁移和3D到视频生成等任务。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要使用感知、几何或简单语义信号作为条件，而大规模自监督学习获得的高维特征虽然包含丰富的场景信息，但风格、光照和语义信息相互纠缠，限制了其生成能力。需要探索如何将这些特征作为更通用的条件信号用于预训练视频扩散模型。

Method: 提出轻量级架构和训练策略，解耦外观特征与其他需要保留的特征，使模型能够鲁棒地控制外观变化如风格化和重光照。同时发现低空间分辨率可以通过更高的特征维度来补偿，从而提高从显式空间表示进行生成渲染的可控性。

Result: 该方法成功应用于视频域迁移和从3D生成视频等任务，能够有效分离外观信息，实现风格化、重光照等外观变化，同时保持其他场景特征的完整性。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号，通过适当的解耦架构和训练策略，能够实现鲁棒的外观控制，扩展视频生成模型的应用范围。
...
TL;DR: 该论文提出了RebusBench基准测试，用于评估大视觉语言模型在解决需要神经符号推理的谜语谜题方面的能力，发现现有模型表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步推理的问题上存在认知鸿沟，特别是当视觉输入仅作为线索而非答案时，模型难以进行神经符号推理。

Method: 引入RebusBench基准测试，包含1,164个谜语谜题，旨在评估模型整合感知和知识的能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，测试了模型缩放和上下文学习的影响。

Result: 模型表现严重不足：精确匹配率低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型缺乏连接视觉和语言组件的认知推理能力。

Conclusion: 虽然大视觉语言模型具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理"胶水"，无法有效进行神经符号推理来解决谜语谜题这类复杂问题。
...
Conclusion: 通过集成几何感知的世界表示，DriveDreamer-Policy能够产生更连贯的想象未来和更明智的驾驶动作，同时保持模块化和可控延迟。显式深度学习为视频想象提供互补优势并提高规划鲁棒性。
...
TL;DR: FSKD：基于知识蒸馏的LiDAR到RGBI框架，通过多模态教师融合LiDAR与RGBI数据，训练仅使用RGBI的学生模型，实现零样本高精度森林结构监测


<details>
  <summary>Details</summary>
Motivation: 航空LiDAR成本高、采集频率低，但却是森林结构监测（如CHM、PAI、FHD）的金标准。需要开发仅使用更易获取的RGBI图像就能达到LiDAR精度的方法，以实现可扩展的高分辨率森林监测。

Method: 提出FSKD知识蒸馏框架：多模态教师模型通过交叉注意力融合RGBI图像与LiDAR衍生的平面指标和垂直剖面，训练仅使用RGBI的SegFormer学生模型来复现这些输出。在德国萨克森州384平方公里森林数据（20cm GSD）上训练，在八个地理独立测试区评估。

Result: 学生模型在零样本CHM预测上达到SOTA性能（MedAE 4.17m，R²=0.51，IoU 0.87），比HRCHM/DAC基线MAE提升29-46%（5.81m vs. 8.14-10.84m），相关性更强（0.713 vs. 0.166-0.652）。多模态融合比仅RGBI训练提升10-26%性能，能同时预测CHM、PAI、FHD，且在时间不匹配（冬季LiDAR、夏季RGBI）下仍有效。

Conclusion: FSKD框架通过知识蒸馏实现了仅使用RGBI图像就能达到接近LiDAR精度的森林结构监测，打破了严格同步采集的限制，为德国数字孪生和国家数字正射影像项目等应用提供了可扩展的20cm分辨率操作监测方案。
...
TL;DR: 提出GardenDesigner框架，通过智能体链和程序化建模自动生成江南园林数字资产，解决人工建模耗时且依赖专家经验的问题
...
Result: 开发Unity交互界面，非专家用户可通过文本输入在1分钟内构建江南园林，实验和人工评估表明能生成多样且美观的江南园林

Conclusion: GardenDesigner框架成功实现江南园林的自动化生成，通过智能体链编码美学原则，显著降低建模门槛和时间成本，为数字内容创作提供有效工具
...
TL;DR: 提出一种利用轮式里程计实现时间一致性的单目深度估计框架，通过光流三角化估计相机位姿和稀疏深度，使用递归贝叶斯估计更新度量尺度，重新标定预训练深度估计基础模型的相对深度输出。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间缺乏时间一致性，导致深度估计抖动，在深度范围突变时可能失效。这种不一致性会影响自动驾驶和移动机器人感知系统的稳定性。

Method: 利用移动机器人的轮式里程计信息，通过连续帧间的光流进行三角化估计相机位姿和稀疏深度。使用稀疏深度估计递归更新度量尺度的贝叶斯估计，然后将该尺度应用于预训练深度估计基础模型输出的相对深度，实现重新标定。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，证明了该方法能够实现鲁棒且准确的深度估计性能，特别是在保持时间一致性方面表现优异。

Conclusion: 提出的基于轮式里程计的一致性感知单目深度估计框架能够有效解决现有方法的时间一致性问题，为自动驾驶和移动机器人提供了稳定可靠的深度感知能力。
...
TL;DR: STRIVE是一个用于视频问答的时空强化学习框架，通过构建多个时空变体视频并进行联合归一化，解决传统方法奖励方差低的问题，提升策略更新的稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统基于组的策略优化方法在视频问答中存在局限性：当回答具有相似正确性时，奖励方差较低，导致优势估计弱或不稳定，影响强化学习效果。

Method: 1. 为每个输入视频构建多个时空变体；2. 在文本生成和视觉变体上进行联合归一化；3. 引入重要性感知采样机制，优先选择与问题最相关的帧，同时保持时间覆盖；4. 通过结构化视觉扰动扩展组间比较，丰富奖励信号。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上，相比现有强化学习基线方法，STRIVE在多个大型多模态模型中均取得了持续改进。

Conclusion: 结构化时空探索是稳定多模态强化学习、提升视频推理性能的原则性机制，STRIVE通过扩展组间比较到结构化视觉扰动，促进了跨互补视觉视角的鲁棒推理。
...
TL;DR: SafeRoPE：针对MMDiT的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间，提出头部级RoPE扰动来抑制不安全内容


<details>
  <summary>Details</summary>
Motivation: 现有基于U-Net的去噪器安全缓解方法计算开销大，且不适用于基于transformer的扩散模型（如MMDiT）。研究发现不安全语义集中在注意力头级别的可解释低维子空间中，扰动RoPE可以有效修改特定概念

Method: 1) 分析MMDiT注意力机制，发现不安全语义集中在安全关键头的低维子空间；2) 构建头部级不安全子空间，计算潜在风险分数；3) 引入头部级RoPE扰动，结合LRS对查询和键向量嵌入执行风险特定的头部级旋转

Result: SafeRoPE在平衡有害内容缓解和效用保持方面达到SOTA性能，有效抑制不安全输出同时保持生成保真度，计算开销小

Conclusion: SafeRoPE为MMDiT提供了一种轻量级、细粒度的安全生成框架，通过头部级RoPE扰动和潜在风险分数机制，实现了精确的不安全语义抑制而不损害图像质量
...
TL;DR: 论文提出随机标签桥接训练方法，通过无需人工标注的方式让大语言模型参数适应视觉基础任务，发现部分桥接训练效果更佳，为跨模态适应提供新途径
...
Method: 提出随机标签桥接训练方法，作为模态适应学习器，无需人工标注，帮助LLM参数适应视觉基础任务。特别强调部分桥接训练的优势，即只对部分层进行适应训练

Result: 研究发现添加桥接训练阶段可以有效对齐LLM参数与视觉任务，部分桥接训练通常更有优势，因为LLM中的某些层表现出强大的基础特性，即使不对视觉任务进行微调也保持有益

Conclusion: 语言预训练参数可以直接在视觉模型中利用，部分桥接训练是跨模态适应的实用途径，为语言和视觉模态之间的桥接开辟了新方向
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督域自适应方法在处理医学图像严重程度分类时面临挑战，因为严重程度分类具有自然有序的类别标签，导致类别边界模糊，使得域自适应更加困难
...
Conclusion: 该方法通过利用类别顺序信息进行跨域排序和对齐，有效解决了医学图像严重程度分类中的域自适应问题，为处理有序类别标签的域自适应任务提供了新思路
...
TL;DR: 提出了一种纹理感知的Transformer模型，用于纹理3D网格的语义分割，通过融合纹理和几何信息，在城市场景和文化遗产数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 纹理3D网格同时包含几何、拓扑和外观信息，但其不规则结构给深度学习语义分割带来挑战。现有方法通常忽略丰富的纹理信息，需要一种能有效利用纹理和几何特征的解决方案。

Method: 提出纹理感知Transformer：1) 纹理分支将每个面片级别的像素汇总为可学习token；2) 与几何描述符融合；3) 通过两阶段Transformer块(TSTB)处理，支持局部和全局信息流；4) 采用新的分层学习方案进行多尺度特征聚合。

Result: 在Semantic Urban Meshes(SUM)基准上达到81.9% mF1和94.3% OA，在新建的文化遗产数据集(纹理屋顶瓦片)上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 该方法有效利用了纹理3D网格中的纹理信息，通过纹理感知Transformer和分层学习方案，在城市场景和文化遗产保护等应用中展现了优越的语义分割性能。
...
TL;DR: PI-VQ提出了一种位置无关的向量量化自编码器，通过消除潜在代码中的位置信息来捕获全局语义特征，并引入匹配量化算法提升信息容量，实现单次前向传播的图像合成。
...
Method: 提出位置无关向量量化自编码器（PI-VQ），约束潜在代码不携带位置信息。引入匹配量化算法，基于最优二分匹配进行向量量化，相比朴素最近邻量化提升3.5倍有效瓶颈容量。利用学习代码的组合结构实现基于插值的采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖度指标。位置无关表示鼓励代码捕获全局语义特征，支持无需学习先验的直接图像插值，并通过单次前向传播合成新图像。

Conclusion: 位置无关表示在可分离性和可解释性方面存在权衡，为未来工作指明了方向。PI-VQ展示了消除位置信息可以促进更全局、语义化的表示学习，同时通过匹配量化算法解决了信息容量减少的问题。
...
TL;DR: FaCT-GS是一个用于快速灵活CT重建的框架，通过优化体素化和光栅化流程，显著提升了基于高斯泼溅的CT重建速度，比现有方法快4-13倍。


<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅(GS)在图像渲染中表现出色并已应用于CT重建，但其优势不足以推动从成熟算法转向。现有GS方法在CT重建中仍存在显著的速度和可扩展性限制。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流程，实现快速CT重建。改进的体素化技术能够快速将高斯分布拟合到现有体积数据，可作为先验进行热启动或作为压缩表示。

Result: 在标准512×512投影上比现有最佳GS CT重建方法快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能够适应不同投影和输出体积大小。

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了快速、灵活且可扩展的解决方案，显著提升了重建效率，为实际应用提供了实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 研究最先进的视觉语言模型在基本几何变换下的脆弱性，揭示其缺乏鲁棒的空间不变性和等变性，尽管在语义任务上表现出色

Method: 通过系统评估方法，在不同视觉领域（符号草图、自然照片、抽象艺术）测试VLMs对旋转、缩放和恒等变换的鲁棒性，涵盖多种架构、模型容量和提示策略

Result: VLMs在语义内容稀疏时性能急剧下降，这种脆弱性在不同架构、模型容量和提示策略中普遍存在，表明当前模型缺乏基本的几何推理能力

Conclusion: 当前VLMs在语义理解和空间推理之间存在系统性差距，未来多模态系统需要更强的几何基础来提升鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 当前时序动作分割方法过度依赖复杂架构，不利于实际部署。需要一种轻量级方法在保持帧级准确性的同时提升细粒度分割质量。

Method: 提出双损失训练框架：1) 边界回归损失通过单通道边界预测提升时间定位精度；2) 基于CDF的片段级正则化损失通过匹配预测与真实片段的累积分布来增强片段内结构一致性。该框架与架构无关，可作为训练时损失函数集成到现有TAS模型中。

Result: 在三个基准数据集上，该方法提升了片段级一致性和边界质量，在三种不同模型上均获得更高的F1和Edit分数。帧级准确率基本保持不变，表明通过简单损失设计而非复杂架构或推理时优化即可实现精确分割。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型在文本到图像生成中面临两个主要挑战：1) 生成的图像质量可能不符合人类期望标准；2) 难以处理具有多种有效解释的模糊提示。需要同时提升生成质量和对模糊语义的处理能力。

Method: 提出MAR-MAER分层自回归框架，包含两个核心组件：1) 度量感知嵌入正则化方法，使用轻量级投影头和自适应核回归损失函数，将模型内部表示与人类偏好度量（如CLIPScore和HPSv2）对齐；2) 条件变分模块，在分层token生成过程中引入受控随机性，处理模糊语义。
...
Conclusion: MAR-MAER通过结合度量感知嵌入正则化和概率潜在建模，有效解决了自回归文本到图像生成中的质量控制和模糊提示处理问题，在保持高质量生成的同时增强了语义灵活性。
...
TL;DR: HieraVid是一个分层剪枝框架，通过分段级、帧级和层级三个层次逐步动态减少视频冗余，仅保留30%的token就能在视频理解基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型(VideoLLMs)在处理大量输入视频token时计算负担重，现有方法主要在输入级剪枝，忽略了视频固有的分段-帧结构和LLM内部的多模态信息单向传播特性。

Method: 提出HieraVid分层剪枝框架：1) 分段级：将视频token按时间分段并空间合并；2) 帧级：在同一分段内联合剪枝相似帧以保持多样性；3) 层级：随着LLM层数增加逐渐减少冗余而不影响性能。

Result: 在四个广泛使用的视频理解基准上进行了广泛实验，仅保留30%的token就能达到新的SOTA性能，同时保持LLaVA-Video-7B 98%以上和LLaVA-OneVision-7B 99%以上的性能。

Conclusion: HieraVid通过分层渐进式剪枝有效减少了视频token的计算负担，同时保持了视频理解性能，为高效视频大语言模型部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将可供性推理视为一次性预测问题，假设静态观察已提供足够证据。但在复杂3D场景中，许多失败案例源于固定观察下任务相关证据的不完整性，而非预测能力不足

Method: 将细粒度可供性推理重新定义为序列证据获取过程，通过互补的3D几何和2D语义证据逐步减少歧义。提出A3R框架：基于MLLM的策略迭代选择证据获取动作，通过跨维度证据获取更新可供性信念。采用GRPO策略学习方法优化序列决策

Result: 在场景级基准测试上的广泛实验表明，A3R始终优于静态一次性基线方法，证明了智能体跨维度证据获取在复杂3D高斯场景中细粒度可供性推理的优势

Conclusion: 通过将可供性推理重新定义为序列证据获取过程，并采用智能体跨维度证据获取框架，能够有效解决复杂3D场景中证据不完整的问题，提高推理准确性和效率
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射(3DGS)在实时渲染方面表现出色，但大量高斯点导致高内存成本。现有剪枝方法虽然减少内存，但会损害空间一致性并产生渲染伪影，需要一种既能压缩表示又能保持高质量渲染的解决方案

Method: 提出GS^2框架：1) 基于证据下界(ELBO)的自适应致密化策略，自动控制高斯点生成过程；2) 透明度感知渐进剪枝策略，动态移除低透明度高斯点；3) 基于图的特征编码模块，通过特征引导的点位移调整空间分布

Result: 与原始3DGS相比，GS^2仅使用约12.5%的高斯点就实现了更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑高斯表示与高质量渲染的平衡

Conclusion: GS^2通过优化高斯点的空间分布，有效解决了3DGS内存消耗高的问题，在显著减少高斯点数量的同时提升了渲染质量，为紧凑3D高斯表示提供了有效解决方案
...
TL;DR: 该论文揭示了现代文生图模型仅通过自然语言提示就能绕过安全过滤器的漏洞，提出了多种无需模型访问或优化的提示攻击策略，成功率高达74.47%。


<details>
  <summary>Details</summary>
Motivation: 文生图模型广泛应用于创意工具和在线平台，但现有的安全过滤器和审核管道仍存在漏洞。研究旨在揭示仅通过自然语言提示就能绕过安全措施的简易攻击方法，暴露当前安全系统的根本缺陷。

Method: 提出系统性的提示攻击策略分类法，包括艺术重构、材质替换、伪教育框架、生活美学伪装和模糊动作替换等技术。这些策略通过将恶意意图隐藏在良性语义上下文中，利用提示审核和视觉安全过滤的弱点。在多个先进文生图系统上进行评估。

Result: 在所有测试模型和攻击类别中，攻击成功率（ASR）高达74.47%。简单的语言修改就能可靠地规避现有安全措施并生成受限图像，表明当前安全系统存在严重缺陷。

Conclusion: 研究揭示了文生图模型安全系统中的关键差距：表面级提示过滤与检测生成媒体系统中对抗意图所需的语义理解之间存在不匹配。需要更强大的语义理解和上下文感知的安全机制来有效防御此类攻击。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法主要依赖句子级视觉语言对齐，难以利用空间关系和对象属性等细粒度语言线索，而这些线索在不同定位阶段扮演不同角色，对区分相似特征对象至关重要。

Method: 1. 将语言表达解耦为全局上下文、空间关系和对象属性；2. 采用渐进式跨模态调制器，通过"调查-定位-验证"方案动态调制视觉注意力；3. 引入跨尺度融合模块处理遥感图像的大尺度变化；4. 使用语言引导校准解码器在预测时细化跨模态对齐；5. 统一多任务头支持指代表达理解和分割任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有形状合成方法在捕捉复杂表面细节、计算效率和模型简洁性方面存在不足，需要一种能够处理任意拓扑结构（任意亏格）且能高效准确重建几何细节的方法。

Method: 1. 在表面内部体积中优化放置参考点，最大化稀疏性、中心性和表面可见性；2. 通过光线投射采样可见距离场；3. 使用快速球谐变换计算SH系数；4. 应用可配置低通滤波器增强几何保真度；5. 基于邻近性使用局部一致性约束细化输出。

Result: 与最先进方法相比，SHARC在重建精度和时间效率方面均表现更优，同时保持了模型的简洁性。

Conclusion: SHARC框架通过球谐函数表示距离场和优化参考点放置，实现了高效准确的任意拓扑形状合成，在精度、效率和简洁性方面取得了良好平衡。
...
Method: 提出FTPFusion方法，将特征表示分解为高频和低频分量进行协同建模。高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；低频分支引入时序扰动策略以增强对复杂视频变化（如闪烁、抖动和局部错位）的鲁棒性；设计偏移感知时序一致性约束以显式稳定时序扰动下的跨帧表示。

Result: 在多个公共基准测试上的广泛实验表明，FTPFusion在空间保真度和时间一致性方面均优于现有最先进方法，并在多个指标上表现一致。

Conclusion: FTPFusion通过频率分解、稀疏跨模态交互和时序扰动策略，有效解决了红外与可见光视频融合中的时空稳定性与细节保留的平衡问题，为智能监控和低光监测提供了更优的解决方案。
...
TL;DR: 提出一种基于2D几何和星座匹配的多光斑检测与匹配管道，专注于可重复性和清晰评估，适用于多LED眼动追踪系统
...
Method: 引入2D几何驱动的星座匹配管道，将光斑视为结构化星座而非独立斑点；提出相似性-布局对齐（SLA）程序，将星座匹配适配到多LED眼动追踪的特定约束；结合受控过检测、自适应候选回退、外观感知评分和可选语义布局先验，保持检测与对应关系明确分离

Result: 在公开多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系；发布代码、预设和评估脚本以实现透明复现、比较和数据集标注

Conclusion: 提出了一种可重复、可评估的多光斑检测与匹配框架，解决了眼动追踪中光斑检测的复现性问题，为透明比较和数据集标注提供了工具
...
TL;DR: KnowMVG通过知识增强提示和全局-局部注意力机制，提升视觉语言模型在医学视觉定位任务中的空间定位精度，在多个基准测试中显著超越现有方法。
...
Method: 提出KnowMVG框架：1）知识增强提示策略，将医学知识编码为紧凑嵌入；2）全局-局部注意力机制，联合利用粗粒度全局信息和细粒度局部线索来引导精确区域定位，无需额外文本推理开销。

Result: 在四个医学视觉定位基准测试中，KnowMVG一致优于现有方法，AP50提升3.0%，mIoU提升2.6%，超过先前的最先进方法。定性和消融研究进一步验证了各组件有效性。

Conclusion: KnowMVG通过显式增强解码过程中的空间感知能力，成功弥合了高级语义理解和细粒度视觉感知之间的差距，为医学视觉定位提供了更精确的解决方案。
...
TL;DR: 该研究探索了直接从波束形成前的天线级距离-多普勒张量学习空间结构的可行性，绕过了传统的角度域构建过程，使用双啁啾共享权重编码器进行端到端学习。
...
Method: 使用6发8收（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列调频连续波发射方案。构建双啁啾共享权重编码器，对波束形成前的天线级距离-多普勒张量进行端到端数据驱动训练。监督信号基于激光雷达，采用可见性感知的跨模态监督，明确建模雷达视场和基于射线的可见性分析。通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线，评估发射配置对几何可恢复性的影响。

Result: 实验结果表明，空间结构可以直接从波束形成前的天线级距离-多普勒张量中学习，无需显式的角度域构建或手工信号处理阶段。不同发射配置（啁啾A、B或组合）对几何可恢复性有显著影响，验证了直接从原始天线数据学习空间表示的可行性。

Conclusion: 该研究证明了直接从波束形成前的天线级雷达测量中学习空间结构的可行性，为汽车雷达感知提供了一种新的表示学习方法，避免了传统角度域构建的局限性，为端到端雷达感知系统设计提供了新思路。
...
### [63] [Curia-2: Scaling Self-Supervised Learning for Radiology Foundation Models](https://arxiv.org/abs/2604.01987)
...
TL;DR: Curia-2是一个用于CT和MRI分析的医学影像基础模型，通过改进预训练策略和表示质量，首次将视觉Transformer扩展到十亿参数级别，并在2D和3D评估基准上表现出色。


<details>
  <summary>Details</summary>
Motivation: 医学影像的快速增长给放射科医生带来了不可持续的工作负担，需要开发基础模型来减轻这种压力。虽然现有的基础模型在CT和MRI分析中显示出大规模预训练的潜力，但在如何从复杂的放射学体积数据中学习方面仍有很大的优化空间。

Method: 基于Curia框架，提出了Curia-2，改进了原始预训练策略和表示质量，以更好地捕捉放射学数据的特性。该方法首次将架构扩展到十亿参数的视觉Transformer，适用于多模态CT和MRI基础模型。同时，通过扩展和重构CuriaBench，将其分为两个独立的评估轨道：针对基于切片的视觉模型的2D轨道和用于体积基准测试的3D轨道。

Result: Curia-2在视觉聚焦任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型竞争相当。模型权重将公开以促进进一步研究。

Conclusion: Curia-2通过改进的预训练策略和表示学习，显著提升了医学影像基础模型的性能，特别是在CT和MRI分析方面。该工作为医学影像分析提供了更强大的基础模型，并通过标准化的评估框架促进了该领域的研究进展。
...
TL;DR: IndoorCrowd是一个用于室内人群检测、实例分割和多目标跟踪的大规模多场景数据集，包含31个视频和9,913帧图像，覆盖四个校园场景，提供人工验证的实例分割掩码和身份跟踪数据。
...
Method: 在四个校园位置（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频，以5fps采样获得9,913帧，提供人工验证的每实例分割掩码。使用620帧控制子集评估SAM3、GroundingSAM和EfficientGroundingSAM三种基础模型自动标注器的性能，使用Cohen's κ、AP、精度、召回率和掩码IoU指标。另外2,552帧子集以MOTChallenge格式提供连续身份跟踪数据。使用YOLOv8n、YOLOv26n和RT-DETR-L结合ByteTrack、BoT-SORT和OC-SORT建立检测、分割和跟踪基线。

Result: 不同场景的难度差异显著，主要由人群密度、尺度大小和遮挡程度驱动。ACS-EC场景具有79.3%的密集帧和平均实例尺度60.8像素，是最具挑战性的场景。自动标注器评估显示基础模型在室内人群场景中的性能表现。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，为检测、分割和跟踪任务提供了基准测试平台。场景间难度差异的分析强调了考虑环境特定因素的重要性，数据集支持基础模型评估和算法开发。
...
TL;DR: 该论文提出了ECAC基准数据集和RSRS混合训练框架，用于解决幼儿教育图像描述任务中缺乏领域特定数据和专业对象描述能力不足的问题，最终开发了KinderMM-Cap-3B模型并在TTS指标上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 幼儿教育图像描述任务面临两个关键挑战：1) 缺乏大规模领域特定数据集，导致模型无法捕捉幼儿教育场景的细粒度语义概念，产生通用且不精确的描述；2) 传统训练范式在提升专业对象描述能力方面存在局限，监督学习偏向高频表达，而强化学习在困难样本上优化不稳定。

Method: 1) 提出ECAC基准数据集，包含256,121张真实世界图像，配有专家级描述和细粒度标签，并引入领域导向的评估指标TTS；2) 提出RSRS混合训练框架，动态切换强化学习和监督微调，将零奖励的困难样本重定向到监督微调，缓解优势崩溃并实现稳定优化；3) 基于ECAC和RSRS开发KinderMM-Cap-3B领域适应多模态大语言模型。

Result: KinderMM-Cap-3B模型在TTS指标上达到51.06，显著超越最先进的基线方法，同时保持优越的描述质量，展示了其在专业教育应用中的潜力。

Conclusion: ECAC基准数据集和RSRS混合训练框架有效解决了幼儿教育图像描述中的领域适应问题，KinderMM-Cap-3B模型在专业对象识别和描述质量方面表现出色，为专业教育应用提供了有前景的解决方案。
...
TL;DR: 该论文提出AMIMV方法，通过非对称多图像多视图增强策略解决医学图像分类中的数据稀缺和类别不平衡问题，并在11个MedMNIST数据集上评估了8种SSL方法在长尾分布下的鲁棒性。
...
Method: 1) 扩展先前提出的MIMV方法，引入新的增强策略构建非对称多图像多视图(AMIMV)对，以同时应对数据稀缺和数据集不平衡问题。2) 进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性。3) 在11个MedMNIST数据集上评估8种代表性SSL方法在长尾分布和有限监督下的表现。

Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。AMIMV方法在医学图像分类中有效改善了数据不平衡问题下的性能。

Conclusion: AMIMV方法通过非对称多图像多视图增强策略，有效解决了医学图像分类中的数据稀缺和类别不平衡问题。该方法在多个医学图像数据集上表现出优于传统SSL方法的性能，特别是在长尾分布和有限监督条件下。
...
TL;DR: LatentUM提出了一种在共享语义潜在空间中表示所有模态的统一模型，消除了视觉理解和生成之间像素空间中介的需求，实现了高效的交错跨模态推理和生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解和生成之间不连续表示的桥梁，这种方法既低效又低效。需要一种更有效的跨模态表示方法来支持密集视觉思维、自我反思的视觉生成以及物理世界视觉动态建模。

Method: LatentUM将所有模态表示在共享的语义潜在空间中，消除了像素空间中介。这种设计自然支持灵活的交错跨模态推理和生成，减少编解码器偏差并增强跨模态对齐。

Result: LatentUM在视觉空间规划基准上实现了最先进的性能，通过自我反思推动了视觉生成的极限，并支持在共享语义潜在空间中预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间的统一模型设计显著提高了计算效率，减轻了编解码器偏差，增强了跨模态对齐，为实现高效的交错跨模态推理和生成提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外与可见光视频融合方法大多针对静态图像设计，无法有效处理视频中的帧间运动。当前视频融合方法通过引入跨帧交互提高时间一致性，但计算成本高昂。需要一种既能保持高质量融合又能高效处理视频动态内容的方法。

Method: 提出MAVFusion端到端视频融合框架，采用运动感知稀疏交互机制：1）利用光学流识别多模态序列中的动态区域；2）自适应地将计算密集的跨模态注意力分配到这些稀疏动态区域，以捕捉显著过渡并促进模态间信息交换；3）对静态背景区域使用轻量级弱交互模块保持结构和外观完整性；4）通过解耦动态和静态区域处理，同时保持时间一致性和细节。

Result: 在多个红外与可见光视频基准测试中达到最先进性能，在640×480分辨率下实现14.16 FPS的推理速度，显著加速推理过程。

Conclusion: MAVFusion通过运动感知稀疏交互机制有效平衡了融合质量与计算效率，为红外与可见光视频融合提供了一种高效且高性能的解决方案，源代码将在GitHub上公开。
...
TL;DR: 提出基于nnU-Net v2框架的深度学习模型，用于多参数MRI中前列腺腺体的自动分割，在PI-CAI数据集上训练，在外部验证集上表现出良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: 手动分割前列腺耗时且存在观察者间差异，通用分割工具在前列腺特定任务中精度不足，需要开发专门的多模态分割方法。

Method: 使用nnU-Net v2框架，整合T2加权成像、扩散加权成像和表观扩散系数图等多模态MRI数据，在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证。

Result: 交叉验证平均Dice分数0.96±0.00，外部测试集Dice分数0.82；相比之下，通用分割工具TotalSegmentator仅得0.15分，主要由于腺体欠分割。

Conclusion: 任务特定的多模态分割策略对前列腺分割至关重要，提出的方法具有良好的泛化能力和临床研究应用潜力，模型已容器化并可作为即用推理工具。
...
Method: 构建MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类。在多种MLLMs变体上进行基准测试，包括开源vs专有、思考vs非思考、小vs大规模模型。

Result: 顶级闭源和开源模型（如GPT-5和Qwen3-VL）准确率仅为~46%和36%，远低于人类表现（差距约40%和50%）。显式推理和模型缩放未带来一致改进，提供相关证据时模型有所提升但随时间增益下降。

Conclusion: 自我定位和长期记忆对于自我中心视频中的个性化问答至关重要，当前MLLMs在这方面存在显著局限。MyEgo数据集和分析将推动该领域发展，助力自我中心个性化辅助系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本条件3D场景生成方法在短文本描述下存在物理合理性差和细节丰富性不足的问题，主要原因是过度依赖显式的组合对象语义关系线索，需要增强3D推理能力特别是先验整合和空间锚定能力

Method: 提出SDesc3D框架：1）多视角场景先验增强，通过聚合多视角结构知识丰富欠指定的文本输入；2）功能感知布局锚定，利用区域功能锚定进行隐式空间锚定和分层布局推理；3）迭代反思-修正方案，通过自修正进行渐进式结构合理性优化

Result: 在短文本条件3D室内场景生成任务上，该方法优于现有方法，代码将公开提供

Conclusion: SDesc3D通过整合多视角结构先验和区域功能含义，实现了在稀疏文本指导下的3D布局推理，显著提升了短文本描述下3D场景生成的物理合理性和语义一致性
...
Method: 1) 构建InteractTrack基准，包含150个视频、密集边界框标注和时间戳语言指令；2) 提出评估协议并测试25个代表性跟踪器；3) 提出IMAT基线方法，采用动态记忆机制从用户反馈中学习并更新跟踪行为

Result: 评估显示现有最先进方法在交互场景中表现失败，传统基准的强性能无法迁移；IMAT基线方法为交互式跟踪提供了有效解决方案

Conclusion: 建立了交互式跟踪的研究基础，包括基准、协议和基线方法，为开发更智能、自适应和协作的跟踪系统铺平道路，弥合自动感知与人类指导之间的差距
...
TL;DR: ActionParty是一个用于生成式视频游戏的动作可控多主体世界模型，通过引入主体状态令牌解决现有视频扩散模型中动作绑定的基本问题，能够同时控制最多7个玩家。
...
Conclusion: ActionParty成功解决了多主体视频扩散模型中的动作绑定问题，实现了对多个智能体的同时控制，为生成式视频游戏提供了有效的世界建模方法。
...
TL;DR: 本文提出IVE方法解决多模态大语言模型中视觉注意力的惯性问题，该惯性阻碍了组合式认知推理，导致认知幻觉。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中的视觉注意力表现出明显的惯性，一旦在早期解码步骤中稳定后就基本保持静态，无法支持认知推理所需的组合式理解。现有幻觉缓解方法主要针对感知幻觉（对象存在或属性），但对需要对象间关系推理的认知幻觉效果有限。

Method: 提出无需训练的惯性感知视觉激发（IVE）方法，通过将认知推理建模为视觉注意力的动态响应来打破惯性模式。IVE选择相对于历史注意力趋势动态出现的视觉标记，同时区分表现出惯性行为的标记。为促进组合推理，IVE引入惯性感知惩罚，防止注意力过度集中并限制在局部区域的持久性。

Result: 大量实验表明，IVE在各种基础MLLM和多个幻觉基准测试中均有效，特别是对于认知幻觉。该方法能够显著改善模型对对象间关系的推理能力。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，IVE方法通过打破这种惯性模式，有效提升了模型对组合式认知推理的能力，为缓解认知幻觉提供了训练免费的解决方案。
...
TL;DR: Resonance4D：一种物理驱动的4D动态模拟框架，通过双域运动监督和全参数物理恢复，在单消费级GPU上实现高保真4D模拟，大幅降低计算成本和内存占用。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的4D动态模拟方法存在两个主要问题：1）运动监督依赖计算成本高昂的在线视频扩散或光流管道，其开销超过模拟器本身；2）现有方法仅优化部分材料参数，限制了复杂材料和动态场景的真实性。

Method: 提出Resonance4D框架，将3D高斯溅射与材料点方法耦合，通过轻量级但物理表达性强的监督实现。核心创新包括：1）双域运动监督（DMS），结合空间结构一致性（局部变形）和频域谱一致性（振荡和全局动态模式）；2）零样本文本提示分割与模拟引导初始化，自动将高斯分解为对象部件级区域，支持全材料参数的联合优化。

Result: 在合成和真实场景上的实验表明，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，使高保真物理驱动的4D模拟能够在单消费级GPU上运行。

Conclusion: Resonance4D通过创新的双域运动监督和全参数物理恢复方法，解决了现有4D动态模拟中计算成本高和物理参数优化不完整的问题，实现了高效、高保真的物理驱动4D模拟，为复杂动态场景的模拟提供了实用解决方案。
...
TL;DR: ProDiG：一种渐进式扩散引导框架，通过多阶段扩散和几何感知注意力机制，从纯航拍图像生成逼真的地面级视图和一致的3D场景模型，无需地面真值数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从纯航拍图像生成地面级视图和3D场景模型时面临挑战：极端视角变化、中间观测缺失和大尺度变化导致几何不一致；现有方法要么后处理渲染效果不佳，要么依赖难以获取的多高度地面真值数据；高斯溅射和扩散细化在小变化下有效，但在大视角差距下失败。

Method: 提出ProDiG框架：1）渐进式高度高斯溅射，逐步将航拍3D表示向地面级保真度转换；2）合成中间高度视图，每阶段使用几何感知因果注意力模块细化高斯表示，该模块将极线结构注入参考视图扩散；3）距离自适应高斯模块，根据相机距离动态调整高斯尺度和不透明度，确保大视角差距下的稳定重建。

Result: 在合成和真实数据集上的广泛实验表明，ProDiG能生成视觉逼真的地面级渲染和一致的3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式扩散引导框架，无需额外地面真值视角即可实现几何基础的渐进细化，成功解决了从纯航拍图像生成地面级视图和3D模型的挑战，为极端视角变化下的3D重建提供了有效解决方案。
...
TL;DR: DR-Seg提出了一种解耦-校正框架，用于遥感图像开放词汇语义分割，通过分离CLIP特征为语义主导和结构主导子空间，在保持语言对齐语义的同时增强结构细节。


<details>
  <summary>Details</summary>
Motivation: 遥感开放词汇语义分割需要同时实现语言对齐识别和细粒度空间划分。CLIP虽然提供强大的语义泛化能力，但其全局对齐的视觉表示难以捕捉结构细节。现有方法引入RS预训练的DINO特征进行补偿，但将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，既不能有效划分边界，又可能破坏CLIP的语义完整性。

Method: DR-Seg基于CLIP特征通道具有功能异质性的观察，将CLIP特征解耦为语义主导和结构主导两个子空间。通过解耦实现有针对性的DINO结构增强而不扭曲语言对齐语义。随后，先验驱动的图校正模块在DINO指导下注入高保真结构先验形成精炼分支，不确定性引导的自适应融合模块动态整合精炼分支与原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征为语义和结构子空间，实现了有针对性的结构增强，在保持语言对齐语义的同时显著提升了遥感开放词汇语义分割的边界划分能力，为这一领域提供了有效的解决方案。
...
Method: 构建LinkS²Bench基准，将1,022分钟动态无人机视频与覆盖200+ km²的高分辨率卫星图像关联；通过LMM辅助流程和严格人工标注，创建17.9k高质量问答对，涵盖感知、定位、关系和推理四个维度的12个细粒度任务；设计跨视图对齐适配器来改善模型性能。

Result: 评估18个代表性VLM显示，与人类基线相比存在显著差距，准确跨视图动态对齐是关键瓶颈；跨视图对齐适配器显著提升模型性能；微调实验证明LinkS²Bench在推进VLM适应复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估VLM在宽域动态跨视图空间智能方面的空白，揭示了现有模型在跨视图动态对齐上的不足，提出的对齐适配器有效改善性能，为推进VLM在复杂空间推理任务中的应用提供了重要基准。
...
TL;DR: 该论文提出了一种解决自编码器中空间不平衡问题的方法，通过自熵损失和样本传播机制来改善对罕见空间位置的重建质量。


<details>
  <summary>Details</summary>
Motivation: 自编码器在处理空间非均匀采样的图像数据时面临挑战，特别是在医学成像、生物学和物理学领域，信息丰富的模式在特定图像坐标上很少出现，而背景在这些位置占据主导地位，导致重建偏向多数外观，丢失细粒度细节，在空间数据不平衡下产生模糊重建。

Method: 提出两个互补组件：(1) 基于自熵的损失函数，对统计上不常见的空间位置进行加权；(2) 样本传播，一种重放机制，在训练过程中选择性地将难以重建的样本重新暴露给模型。

Result: 在具有受控空间不平衡条件的模拟数据集以及三个不受控制的真实世界数据集（物理、生物和天文领域）上进行验证。该方法在各种重建指标上优于基线方法，特别是在空间不平衡分布下表现更佳。

Conclusion: 该方法通过针对空间不平衡问题，鼓励模型关注统计上罕见的位置，提高了重建一致性，强调了在无监督图像重建中批次数据表示和罕见样本的重要性。
...
TL;DR: WISE提出了一种高效的多模态推理范式，通过"思考两次"原则（一次学习，一次加速），在推理时仅生成简洁推理而非详细解释，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维推理在语言引导分割中表现优异，但生成冗长推理文本导致计算成本过高，限制了实际应用。需要一种既能保持推理能力又能显著降低计算开销的方法。

Method: WISE训练模型生成结构化序列：简洁推理→最终答案→详细解释。通过自回归条件约束确保简洁推理能充分支撑详细解释生成。采用自蒸馏目标联合奖励语义保真度和简洁性。推理时使用WISE-S策略，通过简洁性指令提示激活学习到的简洁策略。

Result: 在ReasonSeg基准测试中实现58.3 cIoU的零样本性能，达到最先进水平，同时将平均推理长度从112个token减少到23个token，降低近5倍。

Conclusion: WISE通过"思考两次"范式成功平衡了推理质量和计算效率，在保持高性能的同时大幅降低计算成本，为实际应用提供了可行的解决方案。
...
TL;DR: Jagle是迄今为止最大的日本多模态预训练数据集，包含约920万个实例，通过多种策略生成VQA对，用于训练高质量的日语视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 构建多语言和非英语视觉语言模型面临的主要障碍是其他语言的VQA数据集规模有限、领域覆盖不足。日语尤其缺乏大规模、多样化的多模态训练数据，这阻碍了高质量日语VLMs的开发。

Method: 收集异构源数据（图像、图像-文本对、PDF文档），通过多种策略生成VQA对：基于VLM的QA生成、翻译和文本渲染。构建了包含约920万个实例的Jagle数据集。

Result: 使用Jagle训练的2.2B模型在日语任务上表现优异，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。结合Jagle和FineVision训练不仅不降低英语性能，反而比单独使用FineVision表现更好。

Conclusion: Jagle数据集有效解决了日语多模态训练数据稀缺问题，通过创新的数据生成策略构建了大规模多样化数据集，显著提升了日语VLMs性能，同时保持甚至提升了英语能力。作者开源了数据集、模型和代码以促进可复现性和未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中多个物体可能具有相同的功能可供性，但只有特定物体适合特定任务上下文（如"切苹果"时选择刀而非剪刀）。现有3D可供性方法主要评估孤立单物体，且通常依赖查询中显式类别名称，无法处理这种混淆物体对的选择问题。

Method: 提出CompassNet框架：1) Instance-bounded Cross Injection (ICI)：将语言-几何对齐约束在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR)：在几何组级别和点级别实施对比学习，增强目标表面与混淆表面的区分度。

Result: 构建了CompassAD基准数据集，包含30个混淆物体对、16种可供性类型、6,422个场景和88K+查询-答案对。实验表明CompassNet在已见和未见查询上均达到最先进性能，并在机器人操作器上验证了其在真实混淆多物体场景中的有效抓取能力。

Conclusion: 该研究形式化了基于意图驱动的多物体可供性接地任务，通过专用基准和框架解决了混淆物体对的选择问题，实现了从隐式自然语言意图到正确物体可供性掩码的预测，在真实机器人场景中展示了实用价值。
...
TL;DR: PLUME提出了一种潜在推理框架，用连续潜在状态的自回归展开替代显式思维链，在保持推理优势的同时显著减少推理开销，实现30倍加速


<details>
  <summary>Details</summary>
Motivation: 现有通用多模态嵌入方法使用显式思维链推理，但存在推理开销大、将丰富多模态证据压缩为狭窄文本瓶颈的问题

Method: PLUME采用潜在推理框架：1)用连续潜在状态的自回归展开替代显式思维链；2)引入语义锚点引导的转换适配器，在固定计算预算下引导不同推理轨迹；3)采用渐进式显式到潜在课程学习，使用显式推理作为训练支架并逐步转移到隐藏状态计算

Result: 在78任务的MMEB-v2基准上，PLUME优于强显式思维链基线，同时将推理从数百个生成token减少到少于10个潜在步骤，实现超过30倍的推理加速

Conclusion: 结构化潜在计算可以在不产生显式推理生成开销的情况下保留中间推理的优势，为实际检索系统提供更强、更高效的范式，特别适用于证据密集、结构复杂且难以通过显式中间推理组织的检索场景
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的滑块编辑方法依赖辅助模块和合成监督，存在训练开销大、分布偏移下可靠性降低的问题

Method: 将FlowEdit更新分解为保真度项（源条件稳定器）和转向项（语义转换驱动），利用正交性仅缩放转向项实现强度控制

Result: 无需后训练即可提供平滑可靠的编辑控制，在多种任务中提升连续编辑质量

Conclusion: FlowSlider为Rectified Flow提供了一种无需训练、基于几何分析的连续编辑解决方案，解决了现有方法的训练依赖和可靠性问题
...
TL;DR: GroundVTS：一种用于视频时间定位的视觉语言模型架构，通过查询引导的细粒度视觉令牌采样机制，专注于最信息丰富的时间片段，解决了现有方法因均匀帧采样导致的关键帧稀疏和时序信息丢失问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（Vid-LLMs）依赖均匀帧采样提取视频信息，导致关键帧分布稀疏且丢失重要时序线索。视频时间定位（VTG）作为视频理解的关键任务，需要更精细的时序信息处理能力。

Method: 提出Grounded Visual Token Sampling（GroundVTS）架构：1）采用细粒度、查询引导的机制在输入LLM前过滤视觉令牌，保留关键时空信息并维持时序连贯性；2）引入渐进优化策略，使LLM能适应视觉特征的非均匀分布，增强时序依赖建模能力。

Result: 在三个标准VTG基准测试上全面评估：在时刻检索任务上mIoU提升7.7个百分点，在高光检测任务上mAP提升12.0个百分点，显著优于现有方法。

Conclusion: GroundVTS通过查询引导的视觉令牌采样和渐进优化策略，有效解决了视频时间定位中的时序信息保留问题，为视频大语言模型在更广泛应用中的扩展提供了关键技术支撑。
...
<details>
  <summary>Details</summary>
Motivation: 传统变化检测方法假设固定标签空间，无法回答用户自定义查询。开集词汇变化检测(OVCD)需要查询特定概念的变化掩码，但在无需训练的场景下，跨时间的密集概念响应直接比较存在困难：外观变化、弱跨概念竞争、以及土地覆盖类别的空间连续性导致噪声、碎片化和语义不可靠的变化证据。

Method: CoRegOVCD框架包含：1) 竞争性后验校准(CPC)和语义后验差异(SPD)，将原始概念响应转换为竞争感知的查询概念后验并量化跨时间差异；2) 几何标记一致性门(GeoGate)和区域共识差异(RCD)，通过几何感知结构验证和区域共识抑制不支持响应并提高空间一致性。

Result: 在四个基准测试（包括建筑导向和多类设置）上，CoRegOVCD相比之前最强的无需训练基线持续提升2.24到4.98 F1_C分数，在SECOND数据集上达到六类平均47.50% F1_C。

Conclusion: CoRegOVCD通过后验校准和一致性正则化有效解决了开集词汇变化检测中跨时间概念响应可比性问题，在无需训练的场景下显著提升了变化检测性能。
...
TL;DR: 论文分析了面部动作单元检测中标准评估协议（受试者排他交叉验证）的局限性，发现交叉验证本身引入了可测量的随机方差，而留一数据集交叉验证能提供更稳定和可解释的结果。


<details>
  <summary>Details</summary>
Motivation: 面部动作单元检测研究中，受试者排他交叉验证是标准评估协议，但报告的性能提升往往很小。作者怀疑交叉验证本身可能引入随机方差，导致结果不稳定，需要更可靠的评估方法来准确衡量模型性能。

Method: 1. 在BP4D+数据集上进行重复3折受试者排他分割，量化交叉验证的随机方差；2. 使用留一数据集交叉验证协议，在五个动作单元数据集上进行跨数据集鲁棒性评估；3. 比较操作点指标（如F1）与阈值无关指标（如AUC）的波动性。

Result: 1. 重复3折受试者排他分割产生±0.065的平均F1经验噪声底限，低流行度动作单元的方差更大；2. F1等操作点指标比AUC等阈值无关指标波动更大，模型排名可能因不同折分配而改变；3. 留一数据集交叉验证消除了分区随机性，揭示了单数据集交叉验证中不可见的领域级不稳定性。

Conclusion: 交叉验证本身引入的协议方差可能掩盖了模型性能的真实差异，许多研究中报告的性能提升可能落在此方差范围内。留一数据集交叉验证提供了更稳定、可解释的评估结果，建议在面部动作单元检测研究中采用这种更严格的评估协议。
...
TL;DR: 该论文提出了一种反射生成方法，通过向基础扩散模型注入反射位置和外观的先验信息，并采用类型感知的模型设计，构建了首个大规模物体反射数据集DEROBA，实现了物理一致且视觉逼真的反射效果。


<details>
  <summary>Details</summary>
Motivation: 图像合成涉及将前景对象插入背景并合成环境一致的效果（如阴影和反射）。虽然阴影生成已被广泛研究，但反射生成在很大程度上尚未被充分探索。本研究专注于反射生成，旨在填补这一研究空白。
...
Conclusion: 该研究成功解决了反射生成这一未充分探索的问题，通过创新的方法设计和数据集构建，实现了高质量的反射合成效果，为图像合成领域提供了重要的技术贡献。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但在视觉场景中理解模型如何工作仍具挑战性。现有可解释性工具通常关注孤立组件或专家导向分析，缺乏对完整推理流程的引导式端到端理解。

Method: 开发ViT-Explainer系统，提供Vision Transformer推理的集成可视化，包括动画演示、分块级注意力叠加和视觉适应的Logit Lens，支持引导和自由探索两种模式。

Result: 6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了Vision Transformer端到端可解释性工具的空白，通过交互式可视化系统帮助用户更好地理解模型推理过程。
...
TL;DR: UniDriveVLA通过专家解耦和混合Transformer架构解决自动驾驶VLA模型中空间感知与语义推理的冲突，实现统一的驾驶视觉-语言-动作模型。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶VLA模型面临空间感知与语义推理的冲突：直接采用2D视觉语言模型空间感知有限，而增强3D空间表示又会损害VLM的推理能力。这种困境源于空间感知和语义推理在共享模型参数中的耦合优化。
...
Result: 在nuScenes开放环评估和Bench2Drive封闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛任务上表现优异，展示了作为统一自动驾驶模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构有效解决了自动驾驶VLA模型中空间感知与语义推理的冲突，实现了高性能的统一模型，为自动驾驶系统提供了更强的认知能力。
...
TL;DR: SCALE是一个轻量级确定性框架，用于零样本骨架动作识别，通过语义和置信度感知的列表能量损失来改进未见类的识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，这在动作名称无法充分描述细粒度动态以及未见类语义混淆时表现脆弱。需要更鲁棒的框架来处理这些挑战。

Method: 提出SCALE框架：1) 构建文本条件变分自编码器，冻结文本表示参数化先验和编码器；2) 引入语义和置信度感知的列表能量损失，强调语义相似的困难负样本并纳入后验不确定性；3) 使用潜在原型对比目标对齐后验均值与文本导出的潜在原型。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE持续优于先前的VAE和对齐基线方法，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过将ZSAR建模为类条件能量排序问题，结合语义和置信度感知的损失函数，实现了更鲁棒的零样本骨架动作识别，无需在测试时生成样本。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中效果有限，主要受限于固定的预训练分辨率和粗粒度的patch级表示。在密集预测任务（如开放词汇分割）中，高分辨率输入对像素级推理至关重要，但现有滑动窗口方法计算成本高昂。

Method: 提出SPAR（Single-Pass Any-Resolution ViT），通过特征回归损失将精细步长的滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中。该方法无需架构修改或像素级监督，实现分辨率无关的密集特征提取。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全方法通常依赖模型微调或精选数据集，这会降低生成质量或限制可扩展性。需要一种无需训练、模块化的安全控制方法，能够在不牺牲生成质量的前提下实现可靠的安全控制。

Method: 提出推理时引导框架，利用视觉语言基础模型作为现成的监督信号。通过在每个采样步骤中注入基于干净潜在估计的梯度反馈，将安全引导构建为基于能量的采样问题。该方法兼容扩散模型和流匹配模型。

Result: 在NSFW红队测试基准上达到最先进的鲁棒性，能够有效实现多目标引导，同时在良性非目标提示上保持高质量生成。框架展示了良好的泛化能力。

Conclusion: 该框架为基础模型作为语义能量估计器提供了原则性方法，实现了可靠且可扩展的文本到图像生成安全控制，无需训练即可实现模块化控制。
...
TL;DR: Omni123是一个3D原生基础模型，在单一自回归框架内统一了文本到2D和文本到3D生成，利用2D图像作为几何先验来改善3D表示，通过跨模态一致性约束解决3D数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在文本和图像理解与生成方面表现出色，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺。现有方法通常依赖间接的2D编辑和优化提升到3D的流程，牺牲了几何一致性。需要开发能够直接处理3D内容并保持跨模态一致性的原生3D基础模型。

Method: 提出Omni123模型，将文本、图像和3D表示为共享序列空间中的离散标记，在单一自回归框架内统一文本到2D和文本到3D生成。采用交错X到X训练范式，协调异构配对数据集上的多样化跨模态任务，无需完全对齐的文本-图像-3D三元组。通过语义-视觉-几何循环（如文本→图像→3D→图像）在自回归序列中联合强制执行语义对齐、外观保真度和多视角几何一致性。

Result: 实验表明，Omni123显著改善了文本引导的3D生成和编辑性能，证明了该方法为构建多模态3D世界模型提供了可扩展的路径。

Conclusion: Omni123通过利用2D图像作为几何先验，在共享序列空间中统一处理文本、图像和3D内容，成功解决了3D数据稀缺问题，实现了跨模态一致性约束，为3D原生基础模型的发展提供了有效方案。
...
TL;DR: 提出了一种基于概率度量表示和切片Wasserstein距离的快速表面配准方法，使用AdamFlow优化算法在概率空间进行配准
...
Result: 理论上分析了AdamFlow的渐近收敛性；实验证明在多种解剖结构的仿射和非刚性表面配准中具有优越性能
...
<details>
  <summary>Details</summary>
Motivation: 现有视频物体移除方法擅长修复物体"背后"的内容和纠正外观层面的伪影，但当移除的物体与其他物体有更显著的交互（如碰撞）时，当前模型无法纠正这些交互并产生不合理的输出。

Method: 1. 使用Kubric和HUMOTO生成包含反事实物体移除的配对数据集；2. 推理时使用视觉语言模型识别受移除物体影响的场景区域；3. 使用视频扩散模型生成物理一致的反事实结果。

Result: 在合成和真实数据上的实验表明，与现有视频物体移除方法相比，VOID方法能更好地保持物体移除后场景动态的一致性。
...
TL;DR: 论文提出SimpleStream基线方法，仅使用最近N帧输入现成VLM，性能即可匹配或超越复杂记忆机制的流式视频理解模型，挑战了当前依赖复杂记忆机制的趋势。


<details>
  <summary>Details</summary>
Motivation: 挑战当前流式视频理解方法过度依赖复杂记忆机制的趋势，发现简单的滑动窗口基线方法已能取得同等或更好的性能，质疑复杂记忆模块的必要性。

Method: 提出SimpleStream基线方法：仅将最近N帧输入现成的视觉语言模型(VLM)，采用滑动窗口机制，不依赖复杂的记忆、检索或压缩模块。

Result: 在OVO-Bench和StreamingBench上评估13个主流离线/在线视频LLM基线，仅使用4帧时达到67.7%平均准确率(OVO-Bench)和80.59%(StreamingBench)，性能稳定强劲。

Conclusion: 发现感知-记忆权衡：增加历史上下文可改善记忆但可能削弱实时感知；未来流式基准应将近期场景感知与长期记忆分离，以更清晰评估复杂模块的实际价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D化身建模面临保真度与泛化性的关键权衡：基于多视角工作室数据的方法保真度高但泛化性差，而基于大规模野外数据训练的方法泛化性好但质量低。需要一种既能保持高质量又能泛化到大规模人群的解决方案。

Method: 提出预训练/后训练范式：先在100万个野外视频上进行预训练学习外观和几何的广泛先验，然后在高质量策划数据上进行后训练以增强表现力和保真度。采用大规模编解码器化身架构，实现前馈推理。

Result: LCA能够泛化到不同发型、服装和人口统计特征，同时提供精确的细粒度面部表情和手指级关节控制，具有强身份保持能力。观察到对重新光照和宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA首次展示了大规模3D化身建模的预训练/后训练范式，成功解决了保真度与泛化性的权衡问题，为高质量、可泛化的全身体3D化身建模提供了有效解决方案。
...
TL;DR: 论文提出Referring Scenario Comprehension (RSC)基准，用于评估基于场景的视觉定位，该任务比传统的字面指称表达更具挑战性，要求模型从角色、意图和关系上下文中推断目标对象。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型通常可以通过匹配显著命名类别来成功。作者探索了一个互补且更具挑战性的基于场景的视觉定位设置，其中目标必须从角色、意图和关系上下文中推断，而非显式命名。
...
Result: RSC包含约31k训练样本、4k域内测试样本和3k域外分布样本（包含未见对象类别）。实验表明，基于场景的查询暴露了当前模型在标准基准中未揭示的系统性失败，课程训练提高了在挑战性切片上的性能，并能迁移到标准基准。

Conclusion: 基于场景的视觉定位是一个比传统字面指称更具挑战性的设置，RSC基准为这一领域提供了评估框架，揭示了现有模型的局限性，提出的课程推理方法为这一设置提供了参考点。
...
TL;DR: 提出可操控视觉表示方法，通过早期融合文本指导视觉特征聚焦任意感兴趣对象，同时保持通用表示质量


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）的特征关注最显著视觉线索，无法引导至次要概念；而多模态LLM虽可文本引导，但特征偏向语言中心，在通用视觉任务上效果下降

Method: 提出可操控视觉表示，通过轻量级交叉注意力在视觉编码器层中直接注入文本（早期融合），而非像CLIP等模型在编码后融合文本（晚期融合）

Result: 方法能引导视觉特征聚焦图像中任意目标对象，同时保持底层表示质量；在异常检测和个性化对象识别任务上匹配或超越专用方法，并展示对分布外任务的零样本泛化能力

Conclusion: 提出了一种新的可操控视觉表示类别，通过早期文本融合实现自然语言指导的视觉特征操控，在保持通用视觉表示质量的同时实现特定概念聚焦
...
TL;DR: ModMap是一个原生多视角多模态的3D异常检测与分割框架，通过跨模态特征映射和视角依赖的特征调制实现多视角协同，在SiM3D基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常独立处理不同视角，缺乏对多视角和多模态信息的有效整合。工业3D异常检测需要同时利用多视角图像和深度信息，但现有方法未能充分利用这些数据的协同效应。
...
Conclusion: ModMap通过原生多视角多模态设计、跨模态特征映射和视角依赖调制，有效解决了3D异常检测中的多视角协同问题，为工业应用提供了强大的解决方案。
...
TL;DR: 提出一个从AAA游戏提取的大规模动态数据集，包含4M连续帧的RGB和G-buffer数据，用于提升双向渲染性能，并提出基于VLM的无监督评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实感和时序一致性方面存在局限，导致生成式逆渲染和前向渲染在真实场景中应用存在领域差距。需要高质量、大规模的真实世界数据来推动双向渲染技术的发展。

Method: 1. 采用新颖的双屏拼接捕获方法，从视觉复杂的AAA游戏中提取4M连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道数据；2. 提出基于视觉语言模型（VLM）的评估协议，用于无监督评估逆渲染性能，测量语义、空间和时间一致性。

Result: 1. 数据集显著提升逆渲染器的跨数据集泛化能力和可控生成质量；2. VLM评估结果与人类判断高度相关；3. 结合工具包，用户可以通过文本提示从G-buffer编辑AAA游戏的风格。

Conclusion: 该大规模动态数据集有效缩小了合成数据与真实世界的领域差距，推动了双向渲染技术的发展。提出的VLM评估协议为无监督逆渲染评估提供了可靠方法，整体方案在几何材质分解、可控视频生成和游戏风格编辑等方面展现出实际应用价值。
...
TL;DR: 该论文研究了在线沟通中常见的重复延长形式（RLF）对情感分析的重要性，创建了首个RLF多领域数据集Lengthening，并提出了可解释指令调优框架ExpInstruct来提升LLM对RLF的理解和解释能力。


<details>
  <summary>Details</summary>
Motivation: 在线交流中人们经常使用非正式表达方式（如表情包、表情符号），但一种独特的强调风格——重复延长形式（RLF）多年来一直被忽视。研究者希望探索RLF对情感分析的重要性，以及语言模型是否能理解这种表达方式。

Method: 1) 基于先前语言学研究，创建了首个专注于RLF的多领域数据集Lengthening（85万样本）；2) 提出了可解释指令调优框架ExpInstruct，这是一个两阶段的指令调优框架，旨在提升LLM对RLF的性能和可解释性；3) 提出了一种统一的方法来量化语言模型对非正式表达的理解能力。

Result: 研究发现：1) RLF句子是富有表现力的表达方式，可以作为文档级情感的特征标记；2) RLF对在线内容分析具有潜在价值；3) 微调的预训练语言模型在性能上可以超越零样本GPT-4，但在解释能力上不如GPT-4；4) ExpInstruct能够用有限样本提升开源LLM，使其在RLF任务上达到与零样本GPT-4相当的性能和可解释性。

Conclusion: RLF是情感分析中重要但被忽视的非正式表达形式，通过专门的数据集和指令调优框架，可以显著提升语言模型对这种表达方式的理解和解释能力，为在线内容分析提供了新的工具和方法。
...
TL;DR: 该论文研究如何通过训练时强化学习和测试时并行思考来扩展竞争性编程的推理令牌预算，提出了多轮并行思考管道，显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 竞争性编程需要大量推理令牌，但单次生成的推理在强化学习中成本高昂。需要探索更高效的扩展推理预算的方法，以提升模型在复杂编程问题上的表现。

Method: 采用两种互补方法：1) 训练时强化学习，包括验证RL预热和随机剪枝技术；2) 测试时多轮并行思考管道，将令牌预算分配到多个线程和轮次中，进行生成、验证和精炼。基于Seed-OSS-36B模型进行端到端训练。

Result: 系统使用16线程和每线程16轮，平均每个问题使用760万令牌，在pass@1指标上匹配了底层RL模型的oracle pass@16性能，并在AetherCode的456个困难竞争性编程问题上超越了GPT-5-high。

Conclusion: 通过结合训练时强化学习和测试时并行思考，可以有效扩展推理令牌预算，显著提升模型在复杂编程任务上的性能，为大规模推理系统提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏足够的规模、领域多样性和视觉复杂性来真实评估科学主张与多模态证据之间的一致性，需要更全面的数据集来填补这一空白。

Method: 从PubMed和arXiv收集数据，构建包含超过469K实例的M2-Verify数据集，涵盖16个领域，并通过专家审核进行严格验证，进行基线实验评估现有模型性能。

Result: 最先进模型在一致性检查上表现不佳：在低复杂度医学扰动上达到85.8% Micro-F1，但在高复杂度挑战（如解剖结构变化）上性能降至61.6%，专家评估还发现模型在生成科学解释时存在幻觉问题。

Conclusion: M2-Verify数据集填补了现有基准的不足，揭示了当前模型在科学主张一致性检查方面的局限性，特别是面对复杂视觉挑战时的性能下降和幻觉问题，为未来研究提供了有价值的资源和指导。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型中人类偏好的学习仍然具有根本性挑战，因为奖励建模依赖于微妙、主观的比较而非明确标签。当前方法在标准配对偏好设置下表现有限（ROC AUC低于0.74），需要更好地捕捉人类判断的多维性质。

Method: 提出混合方法：在文本表示基础上增强可解释信号，包括响应长度、拒绝指标、毒性分数和提示-响应语义相似度。使用Anthropic HHRLHF数据集评估10个不同的大型语言模型，并集成SHAP和LIME进行细粒度可解释性分析。

Result: 特征增强框架在所有模型上带来一致改进，ROC AUC最高达0.84，配对准确率显著提高。DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化安全性和支持性框架而非孤立关键词。特征交互影响偏好学习。

Conclusion: 通过整合结构化特征增强文本表示能有效改进人类偏好学习，提供更准确的奖励建模。可解释性分析揭示了模型决策机制，特征交互在偏好学习中起重要作用，为未来偏好对齐研究提供了新方向。
...
Method: 从逐步推理轨迹语料库中分解出3200万个自包含的子问题-子程序对，构建程序性知识库；推理时通过轻量级提示让模型表达核心子问题，检索相关子程序作为隐式程序先验

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于基于文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线；在高推理预算下，相比无检索提升达19.2%，相比最强基线提升7.9%

Conclusion: Reasoning Memory通过广泛的程序性知识覆盖以及分解和检索设计，有效提取和重用程序性知识，显著提升语言模型在复杂推理任务上的表现
...
TL;DR: 该研究通过定位语言模型中实体选择性的MLP神经元，揭示了实体事实检索的内部机制，发现早期层存在稀疏的、因果可操作的实体检索点。


<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答许多以实体为中心的事实性问题，但其内部机制尚不清楚。本研究旨在探索语言模型在实体事实检索过程中涉及的内部机制。

Method: 使用模板化提示定位多个语言模型中对特定实体具有选择性的MLP神经元，然后在PopQA基准的QA示例上进行因果干预验证。研究选取了PopQA中的200个实体进行系统分析。

Result: 定位的神经元主要集中在早期层；负向消融导致实体特异性遗忘，而在占位符处注入控制信号能改善答案检索；对许多实体，激活单个神经元就足以恢复实体一致的预测；机制对别名、缩写、拼写错误和多语言形式具有鲁棒性；效果显著但不普遍，覆盖率对流行实体更高。

Conclusion: 研究识别了稀疏的、因果可操作的访问点，可用于分析和调节实体条件化的事实行为，支持紧凑实体检索而非纯粹深度渐进富集的解释。
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被用来区分自动化翻译过程与需要认知努力的翻译过程。然而，如何确定区分这两种过程的停顿阈值一直存在争议，需要更可靠的方法来识别翻译中的生产单元中断。

Method: 比较了三种现有的停顿阈值计算方法，并提出了一种新的生产单元中断计算方法，并对这些方法进行了评估。

Result: 通过比较分析，展示了不同方法在确定翻译停顿阈值方面的差异，并验证了新提出的生产单元中断计算方法的有效性。

Conclusion: 确定翻译过程中的停顿阈值对于区分自动化与认知性翻译过程至关重要，新提出的生产单元中断计算方法为这一领域提供了更可靠的工具，有助于更准确地分析翻译认知过程。
...
### [110] [Diagnosing Translated Benchmarks: An Automated Quality Assurance Study of the EU20 Benchmark Suite](https://arxiv.org/abs/2604.01957)
*Klaudia Thellmann,Bernhard Stadler,Michael Färber*

Main category: cs.CL

TL;DR: 论文提出自动化质量保证方法评估机器翻译基准数据集质量，发现COMET分数与翻译错误率相关，并发布了清理后的EU20数据集
...
Method: 采用三步自动化质量保证方法：1) 结构性语料库审计与针对性修复；2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与DeepL/ChatGPT/Google翻译服务比较；3) 基于LLM的片段级翻译错误分析

Result: 趋势一致：COMET分数较低的数据集在片段级别显示出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上与人编辑样本的对比指向相同方向
...
### [111] [Optimizing RAG Rerankers with LLM Feedback via Reinforcement Learning](https://arxiv.org/abs/2604.02091)
...
Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的基线模型；框架具有良好泛化性、正交集成能力和噪声鲁棒性

Conclusion: RRPO通过直接对齐重排序与LLM生成质量，有效解决了传统重排序模型与下游生成任务之间的错位问题，为检索增强生成提供了更有效的重排序解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有大量基准测试评估大语言模型，但研究表明模型性能可由少数潜在能力因子解释，这为更高效、原则化的基准测试提供了可能，但缺乏比较不同方法质量的统一框架。

Method: 收集WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务上的109,564个独特项目响应对；提出基于预测效度的评估框架，结合改进的多维IRT模型和最优实验设计驱动的自适应项目选择，并引入成本感知折扣因子。

Result: 改进的多维IRT模型结合自适应项目选择能在仅观察16个项目后，以小于7%的平均绝对误差预测112个保留基准任务性能；加入成本感知折扣因子可将达到7%MAE所需总token从141,000减少到22,000，降低85%评估成本。

Conclusion: 基于预测效度的基准测试评估框架结合多维IRT和自适应项目选择能显著提高评估效率，大幅降低评估成本，为更高效、原则化的模型基准测试提供了可行方案。
...
TL;DR: 使用词嵌入和随机森林模型，仅基于分布数据就能区分印地语中梵语和波斯-阿拉伯语源同义词，表明使用模式保留了词源信号，同义词反映了与起源相关的系统差异。


<details>
  <summary>Details</summary>
Motivation: 研究同义词现象，特别是绝对同义词理论上不应存在，因为它们不扩展语言表达能力。但即使同义词表示相同概念，它们可能反映不同视角或携带不同文化关联，这些主张很少被定量测试。印地语中梵语和波斯-阿拉伯语源词共存形成众多同义词对，为研究提供了理想案例。

Method: 使用基于分布数据的词嵌入技术，训练随机森林分类器来区分印地语中梵语和波斯-阿拉伯语源的同义词。该方法不依赖语义内容，仅基于词语在上下文中的使用模式。

Result: 随机森林模型成功分类了梵语和波斯-阿拉伯语源的词语，即使这些词语在语义上无关。这表明使用模式保留了词源痕迹，上下文编码了词源信号。

Conclusion: 研究提供了定量证据，表明上下文编码词源信号，同义词可能反映与起源相关的微妙但系统性的区别。同义词可以提供不同视角，词源相关的词语可能形成不同的概念子空间，创造由历史起源塑造的新型语义框架。结果突显了上下文在捕捉超越传统语义相似性的细微区别方面的能力。
...
TL;DR: 研究发现，在属性生成中，引用粒度（句子级、段落级或文档级）对模型性能有显著影响，精细粒度引用会降低属性质量16-276%，而段落级引用效果最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索引用粒度（句子级、段落级或文档级）对属性生成模型性能的影响。虽然精细粒度引用通常被认为便于人类验证，但其对模型性能的具体影响尚未充分研究。

Method: 方法包括分析四个不同规模的模型（8B-120B），比较不同引用粒度（句子级、段落级、文档级）对属性质量和答案正确性的影响，并研究模型规模与最优引用粒度之间的关系。

Result: 结果显示：1) 强制精细粒度引用使属性质量下降16-276%；2) 属性质量在中间粒度（段落级）达到峰值；3) 精细粒度引用会破坏证据与答案之间的语义依赖关系；4) 过粗的引用引入干扰噪声；5) 性能差距随模型规模非单调变化，精细粒度约束对大型模型影响更大；6) 最优引用粒度在提升属性质量的同时保持甚至改善答案正确性。

Conclusion: 结论是：仅为了便于人类验证而优化精细粒度引用忽视了模型约束，会损害属性忠实性和生成可靠性。有效的属性生成需要将引用粒度与模型的自然语义范围对齐，段落级引用通常是最佳选择。
...
TL;DR: 论文对LLM中过度自信的言语表达进行电路级机制分析，发现特定MLP块和注意力头在中间到后期层中写入过度自信信号，并通过针对性干预改善校准


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常不仅给出错误答案，而且"过度自信地错误"：当产生事实不正确的答案时，它们倾向于表达过高的置信度而不是表示不确定性。这种言语化的过度自信会误导用户并削弱置信分数作为可靠不确定性信号的作用，但其内部机制仍不清楚。

Method: 提出围绕三个轴心的电路级机制分析：1) 将言语化置信度捕获为可微分的内部信号；2) 识别因果性夸大该信号的电路；3) 利用这些见解进行针对性推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 发现一组紧凑的MLP块和注意力头（集中在中间到后期层）持续在最终标记位置写入置信度夸大信号。进一步表明对这些电路的针对性推理时干预显著改善了校准。

Conclusion: LLM中的言语化过度自信由可识别的内部电路驱动，可以通过针对性干预来缓解。这为理解和改善LLM的校准提供了机制性见解。
...
TL;DR: 提出Magic, Madness, Heaven, Sin框架，将大语言模型输出变化建模为同质-异质轴，根据任务规范目标进行价值评估，分析四个规范上下文及其交互影响


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于输出变化的术语碎片化，缺乏明确的任务规范目标，需要统一的框架来理解不同任务背景下输出变化的价值和意义

Method: 引入Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质连续体，基于任务规范目标进行价值评估。将任务组织为四个规范上下文：认知性（事实性）、交互性（用户效用）、社会性（代表性）、安全性（鲁棒性）。分析每个上下文的失败模式和词汇，并应用框架分析所有成对跨上下文交互

Result: 框架揭示了优化一个目标（如安全性）可能无意中损害人口统计代表性或创造性多样性。输出变化应被视为由任务目标塑造的属性，而非模型的固有特征

Conclusion: 需要基于上下文感知的输出变化评估，将其重新定义为由任务目标塑造的属性，而非模型的固有特征，以解决不同规范目标之间的潜在冲突
...
<details>
  <summary>Details</summary>
Motivation: 探究指令遗忘范式是否适用于扩散模型等非语言生成模型，验证自然语言指令在图像生成中的遗忘效果

Method: 通过多概念多提示变体的控制实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态

Result: 扩散模型无法通过自然语言遗忘指令有效抑制目标概念，目标概念表示在生成过程中持续存在

Conclusion: 扩散模型的指令遗忘存在根本性限制，有效遗忘需要超越推理时语言控制的干预措施
...
TL;DR: 提出模型合并框架，通过合并临床基础模型与通用指令模型，解决医学领域微调中的灾难性遗忘问题，在保持指令跟随能力的同时提升临床任务性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学领域应用时，使用特定医学数据集微调会导致严重的指令跟随能力遗忘，这是临床应用中采用通用LLMs的关键挑战。

Method: 提出模型合并框架，通过基于插值的合并方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，获得领域适应模型。

Result: 合并模型能有效缓解灾难性遗忘，保留临床领域专业知识，同时保持指令跟随能力；在严重受限监督下(如64-shot vs 256-shot)性能与完全微调基线相当。
...
TL;DR: DeltaMem：一种将角色中心记忆管理作为端到端任务的智能记忆管理系统，通过强化学习框架提升性能，在多个长期记忆基准测试中超越现有产品级基线


<details>
  <summary>Details</summary>
Motivation: 现有角色中心记忆管理框架存在信息丢失问题，在不同场景下表现脆弱，导致性能不佳。需要更鲁棒的记忆管理系统来解决这些问题。

Method: 1) 提出DeltaMem系统，将角色中心记忆管理作为单智能体设置中的端到端任务；2) 合成用户-助手对话数据集及操作级记忆更新标签；3) 引入基于记忆的Levenshtein距离来形式化记忆更新奖励；4) 设计定制化强化学习框架增强管理能力

Result: DeltaMem（包括免训练版和RL训练版）在多个长期记忆基准测试（LoCoMo、HaluMem、PersonaMem）中均优于所有产品级基线方法

Conclusion: DeltaMem通过端到端的智能记忆管理方法，有效解决了现有框架的信息丢失和场景脆弱性问题，在长期记忆管理任务中表现出色
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，通过增量协方差聚合和单次特征值分解，实现训练无关、快速且理论最优的层间低秩近似，显著提升LLM压缩效率。
...
Result: 在6个LLM和8个数据集上的实验表明，Swift-SVD优于现有最优基线，在保证最优压缩精度的同时，端到端压缩时间提升3-70倍。

Conclusion: Swift-SVD同时保证了理论最优性、实际效率和数值稳定性，为LLM的硬件友好压缩提供了有效解决方案。
...
TL;DR: 印尼全国349名K-12教师调查显示：AI在教学中应用增加但分布不均，主要用于减轻教学准备负担，但面临通用输出、基础设施不足和情境适配有限等挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在印尼课堂中逐渐应用，但缺乏大规模、以教师为中心的实证研究，无法了解AI实际使用情况和教师需求，这阻碍了开发适合当地情境的AI系统和政策制定。

Method: 对印尼全国349名K-12教师（涵盖小学、初中和高中）进行全国性问卷调查，收集教师使用AI的实践经验和需求数据。

Result: 研究发现：1）AI在教学法、内容开发和教学媒体方面的使用增加，但采用不均衡；2）小学教师使用更一致，高中教师参与较少；3）职业生涯中期教师更重视AI，东部印尼教师感知价值更高；4）各层级教师主要用AI减轻教学准备负担（评估、课程规划、材料开发）；5）通用输出、基础设施限制和情境适配不足阻碍有效课堂整合。

Conclusion: 印尼教师AI应用呈现增长但分布不均的模式，主要功能是减轻教学准备负担。未来AI系统开发需要解决通用性、基础设施和情境适配问题，以支持更有效的课堂整合。
...
TL;DR: 论文分析了9个声明验证数据集的推理模式，发现现有基准主要测试证据提取能力，而多句合成和数值推理严重不足，高基准分数主要反映检索+蕴涵能力而非复杂推理。


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对现有基准实际测试的推理能力的系统性理解。研究者希望揭示这些基准主要测试什么类型的推理，以及它们在推理复杂性方面存在的偏差。

Method: 使用GPT-4o-mini为9个数据集的24K声明验证示例生成结构化推理轨迹，分析推理模式分布。使用紧凑的1B参数推理验证器识别五种错误类型，并分析不同领域的错误特征。

Result: 发现直接证据提取占主导地位（约80%），多句合成和数值推理严重不足（各约5%）。数据集层面存在显著偏差：某些数据集几乎完全测试词汇匹配，而其他数据集约一半情况需要信息合成。错误模式因领域而异：通用领域验证主要受词汇重叠偏差影响，科学验证因过度谨慎而失败，数学验证则因算术推理失败。

Conclusion: 高基准分数主要反映检索+蕴涵能力，而非复杂推理能力。需要构建更具挑战性的评估套件，更好地测试验证系统所需的推理能力，特别是多句合成和数值推理。
...
TL;DR: PRCCF框架通过人物角色引导的检索和因果感知认知过滤，提升情感支持对话中的上下文理解和情感推理能力
...
Method: 提出PRCCF框架，包含人物角色引导的检索机制（联合建模语义兼容性和人物角色对齐）和因果感知认知过滤模块（优先考虑因果相关的外部知识）

Result: 在ESConv数据集上的实验表明，PRCCF在自动指标和人工评估方面均优于现有最先进基线方法

Conclusion: PRCCF通过结合人物角色引导的检索和因果感知认知过滤，有效提升了情感支持对话中的上下文理解和情感推理能力
...
TL;DR: 研究发现监督微调中使用不同来源的思维链轨迹存在泛化性能悖论：训练损失更低不一定泛化更好，DeepSeek-R1数据训练损失更低但泛化性能更差，原因在于其发散式探索模式导致模型陷入冗余分支。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探究不同来源的思维链轨迹如何影响大型推理模型的泛化性能。虽然监督微调在长思维链轨迹上已成为关键阶段，但不同来源轨迹对模型泛化的具体影响尚不明确。

Method: 采用对比研究方法，使用DeepSeek-R1-0528和gpt-oss-120b两个竞争模型生成的已验证思维链轨迹，控制问题集相同。进行多层面分析，包括词元级监督微调损失和步骤级推理行为分析。提出通过过滤频繁分支轨迹的简单有效方法来改进泛化。

Result: 发现训练损失与泛化性能的悖论：DeepSeek-R1数据训练损失更低但泛化性能显著更差。分析显示gpt-oss-120b轨迹收敛且演绎性强，而DeepSeek-R1轨迹发散且分支多。过滤频繁分支轨迹后，在AIME25上提升5.1%，BeyondAIME上提升5.5%，五个基准平均提升3.6%。

Conclusion: 思维链轨迹的质量不仅取决于正确性，还取决于推理模式。发散式探索模式虽然训练损失低，但会导致模型继承低效探索行为，陷入冗余分支。通过过滤频繁分支轨迹可以显著提升泛化性能，为监督微调数据选择提供了重要指导。
...
TL;DR: EndoASR：专为胃肠内窥镜设计的领域自适应语音识别系统，通过两阶段适应策略显著提升临床环境下的转录准确性和实时性能


<details>
  <summary>Details</summary>
Motivation: 传统ASR系统在胃肠内窥镜临床环境中可靠性有限，主要受限于领域特定术语和复杂声学条件，需要专门针对内窥镜工作流程优化的实时ASR系统

Method: 基于合成内窥镜报告开发两阶段适应策略：1）领域特定语言建模；2）噪声鲁棒性增强。系统采用Paraformer架构，模型参数量220M，支持边缘部署

Result: 回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%；前瞻性多中心研究中，CER从16.20%降至14.97%，Med ACC从61.63%提升至84.16%；实时因子0.005，远优于Whisper-large-v3的0.055

Conclusion: 领域自适应ASR可作为胃肠内窥镜中人机协作的可靠接口，在多中心真实临床环境中验证了其一致性能，改进的ASR质量直接提升下游结构化信息提取和临床医生-AI交互效果
...
TL;DR: 本文系统综述了LLM智能体中的记忆方法，提出了统一框架，在标准基准上全面比较现有方法，并设计出优于SOTA的新记忆方法，最后指出了未来研究方向。
...
Method: 1. 从高层视角总结了一个统一框架，整合所有现有智能体记忆方法；2. 在两个知名基准上广泛比较代表性智能体记忆方法；3. 通过分析现有方法模块，设计了一种新的记忆方法；4. 基于研究发现提出未来研究方向。

Result: 1. 建立了统一的智能体记忆方法框架；2. 全面评估了现有方法的有效性并进行了深入分析；3. 设计的新记忆方法在实验中优于当前最先进方法；4. 为未来研究提供了有价值的见解。

Conclusion: 对现有智能体记忆方法的深入理解能为未来研究提供宝贵的新见解。通过系统比较和统一框架的建立，本文不仅评估了现有方法，还设计了更优的新方法，并为记忆模块在LLM智能体中的进一步发展指明了方向。
...
TL;DR: 提出人机协作框架，结合LLM推理与声学特征模型，通过置信度路由机制处理模糊样本，在越南语语音情感识别任务上达到86.59%准确率


<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临声学模式模糊和标注数据不足的挑战，特别是在真实场景中情感边界不清晰。传统数据驱动模型难以处理这些模糊样本，需要结合人类知识来提升识别性能。

Method: 1. 人机协作框架：集成人类知识到学习过程，而非仅依赖数据驱动模型
2. LLM推理中心：使用声学特征模型提供置信度和特征级证据作为辅助信号
3. 置信度路由机制：区分简单和模糊样本，将不确定案例委托给LLM进行深度推理
4. 结构化规则指导：基于人类标注行为推导的规则指导LLM推理
5. 迭代精炼策略：通过错误分析和规则更新持续改进系统性能

Result: 在包含2,764个样本的越南语语音数据集上进行实验，涵盖三种情感类别（平静、愤怒、恐慌），标注者间一致性高（Fleiss Kappa = 0.8574）。提出的方法达到最高86.59%的准确率，Macro F1分数约0.85-0.86，在处理模糊和难以分类的案例上表现出色。

Conclusion: 该工作强调了数据驱动模型与人类推理结合的重要性，为低资源环境下的语音情感识别提供了鲁棒且模型无关的方法。人机协作框架能有效处理模糊样本，在资源有限的情况下实现高性能情感识别。
...
TL;DR: 本文提出了一种更细致的保加利亚语文本毒性检测方法，通过构建本体和数据集，训练BERT模型实现毒性分类，同时保护医疗术语和少数群体相关文本不被误判。


<details>
  <summary>Details</summary>
Motivation: 当前在线内容毒性检测方案经常误判有价值信息，包括医学术语和少数群体相关文本。需要一种更细致的方法来识别保加利亚语中的毒性内容，同时保留对重要信息的访问。
...
Conclusion: 该方法为保加利亚语毒性检测提供了更细致的解决方案，能够准确识别毒性内容，同时避免误判医学术语和少数群体相关文本，具有在多种在线平台和内容审核系统中应用的潜力。
...
TL;DR: LiveMathematicianBench是一个基于arXiv最新论文的动态选择题基准测试，用于评估大语言模型的研究级数学推理能力，通过证明草图引导的干扰项设计和替换抵抗机制来区分真实理解与表面匹配。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准测试存在合成设置和数据污染的限制，无法真实评估大语言模型的研究级数学能力。随着大语言模型越来越多地集成到科学工作流程中，对其数学能力进行严格评估成为实际需求。

Method: 1. 基于模型训练截止日期后发布的arXiv论文构建动态选择题基准；2. 引入13类逻辑分类法（蕴含、等价、存在、唯一性等）；3. 采用证明草图引导的干扰项生成管道，使用高层次证明策略构建看似合理但无效的答案选项；4. 引入替换抵抗机制以区分答案识别与实质性推理；5. 实施双模式协议评估证明草图访问对推理的影响。

Result: 基准测试远未饱和：最佳模型Gemini-3.1-pro-preview仅达到43.5%准确率。在替换抵抗评估下，准确率大幅下降：GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问能带来一致的准确率提升，表明模型可以利用高层次证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的测试平台，用于研究大语言模型的研究级数学推理能力，揭示了当前模型在实质性数学理解方面仍存在显著局限性。
...
TL;DR: 本文提出基于指令微调和离散控制令牌的领域无关可控自动文本简化框架，发现小模型可竞争但可控性依赖训练数据中目标属性的变化程度，并指出现有评估指标不足，需基于误差的度量方法。
...
Result: 较小模型（1-3B）可达到竞争性表现，但可靠的可控性强烈依赖于训练数据是否编码足够的目标属性变化。可读性控制（FKGL、ARI、Dale-Chall）学习一致，而压缩控制因现有语料库中信号变异性有限而表现不佳。标准简化和相似性指标不足以衡量控制程度。

Conclusion: 需要基于误差的度量方法来评估目标-输出对齐，且朴素的数据分割可能引入分布不匹配，从而破坏训练和评估效果。数据质量和评估方法对CATS系统至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法（如PPO）成本高且不稳定，替代方法需要大量数据且可能削弱LLM的泛化能力，需要更高效的对齐框架

Method: 提出DEFT框架：1）基于语言模型输出分布和偏好数据差异分布计算微分分布奖励；2）使用该奖励从原始数据中筛选高质量子集；3）将筛选数据集成到现有对齐方法中指导模型输出分布
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感、以及缺乏全局token级关系建模等问题，需要更有效且理论扎实的解决方案

Method: PLOT通过将偏好学习建模为最优传输问题，设计token级损失函数，利用token嵌入捕捉语义关系，实现全局优化的对齐方法

Result: 在人类价值观和逻辑问题解决两大类共七个子偏好上的实验表明，PLOT能持续提升对齐性能，同时保持文本的流畅性和连贯性

Conclusion: 最优传输为LLM偏好学习提供了理论扎实的框架，PLOT方法在保持模型稳定性的同时显著提升了对齐效果，为偏好学习提供了新见解
...
TL;DR: 提出自适应占位符补全(APC)框架，通过在高熵位置输出显式占位符来替代硬补全，降低编辑成本同时保持传统补全性能


<details>
  <summary>Details</summary>
Motivation: 传统硬补全(HC)范式在上下文不足时强制生成完全具体的代码，导致61%的建议被编辑或拒绝，即使与用户后续代码有80%以上相似度，表明模型在特定token位置容易做出错误预测

Method: 1) 将代码补全形式化为不确定性下的成本最小化问题；2) 基于填充占位符成本低于纠正错误的观察，证明存在临界熵阈值，超过该阈值APC的期望成本严格低于HC；3) 从过滤的真实编辑日志构建训练数据；4) 设计基于成本的奖励函数进行强化学习

Result: 在1.5B-14B参数模型上的广泛评估显示，APC将期望编辑成本降低了19%到50%，同时保持了标准HC性能
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于强调融合而忽视了噪声特征的不确定性和细粒度推理，需要更鲁棒和上下文感知的解决方案
...
Method: 在法国心力衰竭队列上评估基于transformer的模型，比较四种方法：1) 仅文本方法；2) 仅结构化数据方法；3) 多模态融合方法；4) 大语言模型方法。特别研究了临床文本中实体级表示与CLS嵌入的比较，以及不同模态融合策略。

Result: 1) 用实体级表示丰富临床文本比仅使用CLS嵌入能改善预测；2) 文本和结构化变量的监督多模态融合获得最佳整体性能；3) 大语言模型在不同模态和解码策略中表现不一致，仅文本提示优于结构化或多模态输入。

Conclusion: 实体感知多模态transformer为心力衰竭短期结局预测提供了最可靠的解决方案，而当前的大语言模型提示方法在临床决策支持方面仍有限制。
...
Result: 相比纯文本方法有所改进，消融研究和可解释性分析证实了添加特征的有效性

Conclusion: 神经符号方法通过结合文本嵌入和符号概念特征，能够更有效地检测宣传性新闻，提高分类的鲁棒性和泛化能力
...
TL;DR: 提出衡量词序变化相对于交换距离最小化最优性的数学框架，证明跨语言手势至少77%最优，引入二次分配问题作为语言优化问题的统一框架


<details>
  <summary>Details</summary>
Motivation: 检验语言中词序变化是否遵循交换距离最小化假设，即源顺序与目标顺序在置换多面体中的距离越近，认知成本越低，出现概率越高

Method: 建立基于置换多面体交换距离的数学框架来衡量词序最优性，应用二次分配问题作为统一优化框架，分析跨语言手势数据

Result: 跨语言手势至少达到77%的最优性，多次达到最优的情况不太可能是偶然，为词序/手势顺序优化研究奠定理论基础

Conclusion: 提出最优分配一般原则，统一包括交换距离最小化在内的多种语言优化原则，将二次分配问题引入语言研究作为优化问题的统一框架
...
TL;DR: SAFE是一个动态基准测试框架，通过可验证的实体序列替代无根据的思维链，解决多跳QA基准测试中LLMs虚假正确性问题，包含训练时验证和推理时验证两个阶段。


<details>
  <summary>Details</summary>
Motivation: 多跳QA基准测试经常奖励大型语言模型虚假的正确性，掩盖了无根据或有缺陷的推理步骤。需要转向更严格的推理评估，确保推理轨迹的可验证性。

Method: 提出SAFE框架，包含两个阶段：1）训练时验证：建立原子错误分类法和基于知识图谱的验证流程，消除标准基准中的噪声监督，识别不可回答实例；2）推理时验证：基于验证数据集训练反馈模型，实时检测无根据的推理步骤。

Result: 实验结果表明：1）SAFE在训练时暴露了现有基准的关键缺陷，识别出高达14%的实例为不可回答；2）在推理时显著优于标准基线，平均准确率提升8.4个百分点，同时保证推理轨迹的可验证性。

Conclusion: SAFE框架通过严格的验证机制解决了多跳QA基准测试中的虚假正确性问题，不仅揭示了现有基准的缺陷，还提供了可验证推理轨迹的高性能解决方案。
...
TL;DR: 论文研究了高斯扩散模型在离散数据生成中的局限性，发现DDPM求解器在处理离散分布时会在临界采样区间遇到多模态密度问题，导致采样质量下降，并提出结合自条件和q-sampling的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但在离散数据上的应用仍然具有挑战性。本文旨在探究高斯扩散模型（使用DDPM求解器）为何难以从离散分布中采样，这些分布在连续空间中表示为δ分布的混合。

Method: 使用玩具随机层次模型识别临界采样区间，在该区间内噪声化数据的密度变为多模态。分析DDPM在该区间内进入低密度区域的问题，并评估现有启发式方法（包括自条件和q-sampling求解器）的缓解效果。提出将自条件与在临界区间内从DDPM切换到q-sampling相结合的方法。

Result: 发现DDPM在临界采样区间会偶尔进入模态之间的低密度区域，产生模型分布外的输入并降低采样质量。自条件和q-sampling有助于缓解此问题。在真实数据上，结合自条件并在临界区间切换到q-sampling的方法提高了生成质量，在文本、编程代码和蛋白质等多个领域的条件和非条件任务中验证了这些发现。

Conclusion: 高斯扩散模型在处理离散分布时存在固有的采样困难，特别是在临界区间内面临多模态密度问题。通过识别这一机制并提出结合自条件和自适应求解器切换的策略，可以有效改善离散数据生成质量，为扩散模型在离散领域的应用提供了理论见解和实用解决方案。
...
TL;DR: 研究自监督语音模型训练过程中语言结构的编码何时出现，分析不同语言结构在层间和学习轨迹上的差异，以及预训练目标对结构组织的影响


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能学习有效的语音表征，但语言结构在模型训练过程中何时出现尚不清楚。需要研究不同语言结构在模型训练中的编码模式和学习轨迹

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语语音数据上的中间检查点，研究多种语言结构在不同层间的编码模式和学习轨迹，考察预训练目标定义层级的影响

Result: 不同层级的语言结构在层间分布和学习轨迹上存在显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释。高阶预测任务（迭代精炼的伪标签）能诱导更大的并行性

Conclusion: 语言结构的编码模式和学习轨迹受其抽象程度和时间尺度影响，预训练目标的定义层级强烈影响语言结构的层间组织和学习轨迹，高阶预测任务能促进更并行的结构编码
...
TL;DR: 提出BidirLM方法，将因果生成式语言模型转化为双向编码器，通过系统消融实验确定关键因素，引入权重合并和多领域数据混合策略，并融合专业模型能力，在文本、视觉和音频表征任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成式语言模型转化为双向编码器的方法存在三个主要局限：缺乏最优训练目标的共识、大规模训练时的灾难性遗忘问题、以及难以灵活集成专业生成模型生态系统。需要开发一种通用、可扩展且能融合专业能力的转换方法。

Method: 1. 在Gemma3和Qwen3模型上进行系统消融实验，识别成功适应的关键因素，特别强调常被忽略的先验掩码阶段的重要性；2. 引入双重策略：线性权重合并结合轻量级多领域数据混合，以缓解灾难性遗忘；3. 通过合并专业因果模型来增强编码器，实现模态和领域特定能力的无缝迁移。

Result: 提出的开源方法适用于任何因果解码器LLM，产生了BidirLM系列五个编码器，在文本、视觉和音频表征基准测试中超越了现有替代方法。

Conclusion: 该方法成功解决了当前因果生成模型向双向编码器转换的三大局限，提供了一种通用、可扩展的解决方案，能够有效整合专业模型能力，在多种模态的表征任务上取得优异性能。
...
TL;DR: GOOSE是一种无需训练的解码框架，通过构建自适应脊柱树（深度链+宽分支）来优化推测解码，利用不同token来源的质量差异实现更高效的加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在构建候选token树时，通常采用平衡树结构，未能区分不同token来源（如上下文n-gram匹配和统计预测）的质量差异。研究发现这两种来源的接受率存在显著差距（中位数6倍，范围2-18倍），这为优化树结构提供了机会。

Method: GOOSE提出自适应脊柱树结构：将高接受率的上下文匹配token组织成深度链（脊柱），在每个节点上为低接受率的统计预测token添加宽分支。这种各向异性（不对称）结构突破了平衡树的深度限制，同时利用了不同token来源的质量特性。

Result: 在5个LLM（7B-33B）和5个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线性能提升12-33%。理论证明GOOSE每个步骤接受的token数至少与单独使用任一token来源时相同。

Conclusion: 通过识别和利用不同token来源的质量差异，构建各向异性的自适应脊柱树结构，可以显著提升推测解码的效率，突破传统平衡树的深度限制，实现更高的加速比。
...
TL;DR: 提出prosodic ABX框架，扩展ABX判别任务来评估自监督语音模型表示对韵律对比的敏感性，构建英日韵律最小对数据集，并在低资源条件下验证模型评估的可行性。


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型（S3Ms）的表示对音位对比敏感，但对韵律对比的敏感性尚未直接测量。ABX判别任务已用于通过最小对测量S3M表示中的音位对比，但缺乏针对韵律对比的系统评估框架。

Method: 提出prosodic ABX框架，扩展ABX判别任务来评估韵律对比敏感性；构建并发布英语和日语韵律最小对数据集；使用该数据集和现有普通话数据集评估英语重音、日语音高重音和普通话声调的对比敏感性；在多种实验条件下验证模型和层级排名的稳定性。

Result: 成功开发了prosodic ABX评估框架；构建了英语和日语韵律最小对数据集；评估了S3M表示对英语重音、日语音高重音和普通话声调的敏感性；发现模型和层级排名在不同实验条件下通常保持稳定，证明该方法在低资源设置下的实用性。

Conclusion: prosodic ABX框架能够有效评估自监督语音模型表示对韵律对比的敏感性，仅需少量示例且无需显式标签；构建的数据集和验证的方法为低资源环境下的模型评估提供了实用工具，模型排名稳定性确保了评估结果的可靠性。
...
TL;DR: 该论文提出稳定性过滤方法，通过筛选行为稳定的边界来构建更有效的引导向量，显著提升数学推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于关键词匹配检测推理行为边界的方法存在严重缺陷：大多数检测到的边界是行为不稳定的，无法在相同前缀下重现目标行为，这导致引导信号被稀释，影响引导向量的效果。

Method: 1. 提出概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件；2. 开发稳定性过滤方法，仅保留模型能一致重现目标行为的边界；3. 结合内容子空间投影去除残差问题特定噪声。

Result: 在541个关键词检测的边界中，93.3%是行为不稳定的；经过稳定性过滤和内容子空间投影后，在MATH-500上达到0.784准确率（比最强基线提升5.0%）；引导向量可在同架构模型间迁移，提升Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）性能。

Conclusion: 稳定性过滤能有效识别真正的行为信号，构建的引导向量质量更高，且具有跨模型迁移能力，为训练无关的行为控制提供了更可靠的方法。
...
TL;DR: GaelEval：首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答任务，评估19个LLM发现前沿模型在盖尔语语法任务上超越人类基线。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在未官方支持的语言中表现出"影子"能力，但性能参差不齐且缺乏有效测量。对于形态句法丰富的少数民族语言如苏格兰盖尔语，现有翻译基准无法捕捉结构能力，需要更全面的评估框架。

Method: 开发GaelEval基准，包含三个部分：1) 专家编写的形态句法多项选择题任务；2) 文化背景的翻译基准；3) 大规模文化知识问答任务。评估19个LLM，与30名流利说话者的人类基线对比，测试不同提示策略（英语vs盖尔语）。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超越人类基线(78.1%)。专有模型持续优于开源模型，盖尔语提示带来小幅稳定优势(+2.4%)。在文化任务上，领先模型超过90%准确率，但多数系统在盖尔语提示下表现更差，且绝对分数相对于人工基准被夸大。
...
TL;DR: 该论文介绍了在SOMD 2026跨文档软件提及共指消解共享任务中获得第二名的系统，比较了无需微调的模糊匹配和上下文感知表示两种方法，分析了它们在噪声环境和不同规模语料下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索跨文档软件提及共指消解任务的有效方法，该任务目前研究较少。作者旨在比较两种无需微调的方法在不同噪声条件和语料规模下的表现，为实际应用提供指导。

Method: 采用两种无需微调的方法：1) 模糊匹配(FM)：基于词汇字符串相似性的方法；2) 上下文感知表示(CAR)：结合提及级别和文档级别嵌入的方法。通过噪声注入实验和推理时间分析评估两种方法。

Result: 在SOMD 2026任务中，两种方法都取得了竞争性性能(CoNLL F1 0.94-0.96)，CAR始终比FM高1分。噪声实验显示：边界噪声下CAR更稳健(F1仅降0.07 vs FM降0.20)，提及替换下FM更稳健(F1降0.52 vs CAR降0.63)。CAR的推理时间随语料规模近似线性增长，而FM呈超线性增长。

Conclusion: 系统选择应综合考虑上游提及检测器的噪声特征和目标语料的规模。软件名称的高表面规律性降低了对复杂语义推理的需求。CAR在大规模语料下是更高效的选择。作者发布了代码以支持这一研究不足的任务。
...
TL;DR: 提出Neuro-RIT框架，通过神经元引导的鲁棒指令微调解决检索增强语言模型对噪声上下文的脆弱性问题，从密集适应转向精确神经元对齐


<details>
  <summary>Details</summary>
Motivation: 检索增强语言模型在知识密集型任务中表现良好，但对不相关或噪声检索上下文仍易出现性能下降。现有方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性。

Method: 提出Neuro-RIT框架：1) 使用基于归因的神经元挖掘显式分离处理相关与不相关上下文的神经元；2) 引入两阶段指令微调策略：通过功能失活仅处理不相关上下文的神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏。

Result: 在多样化QA基准测试上的广泛实验表明，Neuro-RIT始终优于强基线方法和鲁棒性增强方法。

Conclusion: Neuro-RIT通过从密集适应转向精确神经元对齐的范式转变，有效提升了检索增强语言模型对噪声上下文的鲁棒性，为模型鲁棒性优化提供了新方向。
...
### [148] [Towards Position-Robust Talent Recommendation via Large Language Models](https://arxiv.org/abs/2604.02200)
*Silin Du,Hongyan Liu*

Main category: cs.CL

TL;DR: L3TR是一个基于大语言模型的列表式人才推荐框架，通过块注意力机制和局部位置编码解决位置偏见和并发令牌偏见问题，相比传统点式方法在效率和推荐质量上均有提升。


<details>
  <summary>Details</summary>
Motivation: 传统人才推荐系统采用点式范式，需要大语言模型重复处理文本，无法捕捉候选人列表中的关系，导致令牌消耗高且推荐效果次优。此外，大语言模型在处理多选题和长文档时存在位置偏见和"迷失在中间"问题。

Method: 提出L3TR框架，采用隐式策略利用大语言模型的潜在输出进行推荐。引入块注意力机制和局部位置编码方法增强文档间处理并缓解位置偏见和并发令牌偏见。使用ID采样方法解决训练和推理阶段候选集大小不一致问题。设计评估方法检测位置偏见和令牌偏见，并提供无需训练的去偏见方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法显示出持续改进。

Conclusion: L3TR框架通过列表式处理、块注意力机制和局部位置编码，有效解决了大语言模型在人才推荐中的位置偏见、令牌偏见和效率问题，为人才推荐系统提供了更优的解决方案。
...
### [149] [CV-18 NER: Augmented Common Voice for Named Entity Recognition from Arabic Speech](https://arxiv.org/abs/2604.02209)
*Youssef Saidi,Haroun Elleuch,Fethi Bougares*

Main category: cs.CL

TL;DR: 该论文介绍了CV-18 NER，这是首个公开的阿拉伯语语音命名实体识别数据集，并展示了端到端模型在阿拉伯语语音NER任务上优于级联系统。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语音命名实体识别研究不足，主要由于阿拉伯语的形态复杂性、缺少短元音以及标注资源有限。现有研究显示端到端方法在英语、法语和中文上优于级联系统，但阿拉伯语领域尚未充分探索。

Method: 1. 创建CV-18 NER数据集：基于阿拉伯语Common Voice 18语料库，按照Wojood模式（21种实体类型）进行人工NER标注。2. 对比评估：测试级联系统（ASR + 文本NER）和基于Whisper、AraBEST-RQ的端到端模型。3. 分析阿拉伯语特定自监督预训练与多语言弱监督对任务的影响。

Result: 端到端系统在测试集上显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。分析表明：阿拉伯语特定自监督预训练在ASR任务上表现强劲，而多语言弱监督在联合语音到实体学习上转移效果更好；在低资源设置下，更大模型可能更难适应。

Conclusion: 端到端方法在阿拉伯语语音NER任务上具有优势，CV-18 NER数据集和模型的公开为阿拉伯语语音NER研究提供了首个开放基准，促进了该领域的发展。
...
### [150] [No Single Best Model for Diversity: Learning a Router for Sample Diversity](https://arxiv.org/abs/2604.02319)
*Yuhan Liu,Fangyuan Xu,Vishakh Padmakumar,Daphne Ippolito,Eunsol Choi*

Main category: cs.CL

TL;DR: 论文研究如何从多个LLMs中生成全面的有效答案集，提出多样性覆盖率指标，发现不同模型在不同提示上表现各异，开发了基于查询的路由器来选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 当面对允许多种有效答案的提示时，全面生成这些答案是满足广泛用户需求的第一步。目前缺乏评估模型生成全面答案集能力的方法，且不同LLMs在不同类型的开放提示上表现存在差异。

Method: 1) 提出多样性覆盖率指标，衡量预测答案集中每个唯一答案的质量得分相对于相同数量最佳可能答案集的比例；2) 评估18个LLMs在生成多样化响应方面的表现；3) 开发基于查询的路由器，预测每个查询的最佳模型；4) 在NB-Wildchat数据集上训练路由器，并在NB-Curated数据集上测试泛化能力。

Result: 1) 没有单一模型在所有开放提示上都能生成最全面的答案集；2) 每个提示都存在一个明显优于其他模型的特定模型；3) 训练的路由器在NB-Wildchat上优于单一最佳模型基线（26.3% vs 23.8%）；4) 路由器能泛化到域外数据集和不同的答案生成提示策略。

Conclusion: 该研究为在拥有多个模型套件时生成全面答案奠定了基础，证明了基于查询的路由方法能有效利用不同模型的优势，提高答案生成的多样性和全面性。
...
### [151] [Grounded Token Initialization for New Vocabulary in LMs for Generative Recommendation](https://arxiv.org/abs/2604.02324)
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型扩展新词汇（如推荐系统中的语义ID令牌）时，通常采用均值初始化策略，但这种初始化方式将所有新令牌压缩到退化的子空间中，抹去了令牌间的区别，导致后续微调难以完全恢复这些差异。作者认为令牌初始化是词汇扩展的关键瓶颈。

Method: 通过谱分析和几何诊断揭示均值初始化的问题，提出"基于语言的令牌初始化假设"：在微调前将新令牌基于语言基础映射到预训练嵌入空间中的不同、有语义意义的位置。具体实现为GTI（基于语言的令牌初始化），这是一个轻量级的基于语言基础阶段，仅使用配对的语言监督将新令牌映射到预训练嵌入空间。

Result: GTI在多个生成推荐基准测试（包括工业规模和公共数据集）的大多数评估设置中，优于均值初始化和现有的辅助任务适应方法。分析表明，基于语言基础的嵌入产生了更丰富的令牌间结构，这些结构在微调过程中得以保持。

Conclusion: 初始化质量是词汇扩展的关键瓶颈，基于语言的令牌初始化（GTI）通过将新令牌映射到预训练嵌入空间中有语义意义的位置，能够更好地利用模型的通用知识来处理新令牌领域，从而获得更好的性能。
...
### [152] [ReFormeR: Learning and Applying Explicit Query Reformulation Patterns](https://arxiv.org/abs/2604.01417)
*Amin Bigdeli,Mert Incesu,Negar Arabzadeh,Charles L. A. Clarke,Ebrahim Bagheri*

Main category: cs.IR

TL;DR: ReFormeR：一种基于模式引导的查询重构方法，通过从查询对中提取重构模式，构建模式库，然后根据检索上下文为新查询选择合适模式，实现可控的查询重构操作。


<details>
  <summary>Details</summary>
Motivation: 现有基于语言模型的查询重构方法通常直接生成重构查询，缺乏可控性和可解释性。需要一种更明确、可控制的重构策略，使语言模型能够进行有针对性和有效的查询重构。

Method: 1. 从初始查询和实证更强的重构查询对中提取简短的重构模式；2. 将模式整合成紧凑的可转移模式库；3. 根据新查询的检索上下文选择合适重构模式；4. 使用选定模式约束语言模型进行可控操作（如词义消歧、词汇接地、区分性方面添加等）。

Result: 在TREC DL 2019、DL 2020和DL Hard数据集上的广泛实验表明，ReFormeR相比经典反馈方法和近期基于语言模型的查询重构与扩展方法，取得了持续改进。

Conclusion: ReFormeR通过重构模式使重构策略明确化，引导语言模型进行有针对性和有效的查询重构，在多个基准测试中表现出优于现有方法的性能。
...
### [153] [STABLE: Efficient Hybrid Nearest Neighbor Search via Magnitude-Uniformity and Cardinality-Robustness](https://arxiv.org/abs/2604.01617)
*Qianyun Yang,Zhiwei Chen,Yupeng Hu,Zixu Li,Zhiheng Fu,Liqiang Nie*

Main category: cs.IR

TL;DR: STABLE框架通过AUTO度量、HELP索引和动态异构路由解决混合近似最近邻搜索中的数据分布异质性挑战


<details>
  <summary>Details</summary>
Motivation: 当前混合近似最近邻搜索方法忽视数据分布的异质性，存在相似性幅度异质性的兼容性障碍和属性基数容忍度瓶颈两大挑战

Method: 提出STABLE框架：1) AUTO度量联合测量特征相似性和属性一致性；2) HELP索引组织异构语义关系；3) 动态异构路由方法确保高效搜索

Result: 在五个具有不同属性基数的特征向量基准测试中，STABLE表现出优越性能

Conclusion: STABLE框架能够为具有各种分布的数据集提供准确、高效且鲁棒的混合近似最近邻搜索
...
### [154] [From BM25 to Corrective RAG: Benchmarking Retrieval Strategies for Text-and-Table Documents](https://arxiv.org/abs/2604.01733)
*Meftun Akarsu,Recep Kaan Karaman,Christopher Mierbach*

Main category: cs.IR

TL;DR: 对包含文本和表格数据的异构文档，系统比较了十种检索策略在金融QA基准上的表现，发现两阶段混合检索+神经重排序效果最佳，BM25优于密集检索，查询扩展对数值查询帮助有限。
...
Method: 在包含23,088个查询和7,318个混合文本表格文档的金融QA基准上，对十种检索策略进行基准测试：稀疏检索（BM25）、密集检索、混合融合、交叉编码器重排序、查询扩展（HyDE、多查询）、索引增强和自适应检索。使用Recall@k、MRR、nDCG评估检索质量，使用Number Match评估端到端生成质量，并进行配对自助显著性检验。

Result: 1. 两阶段管道（混合检索+神经重排序）表现最佳：Recall@5达到0.816，MRR@3达到0.605，显著优于所有单阶段方法。2. BM25在金融文档上优于最先进的密集检索，挑战了语义搜索普遍占优的假设。3. 查询扩展方法（HyDE、多查询）和自适应检索对精确数值查询帮助有限，而上下文检索则带来一致增益。

Conclusion: 对于包含文本和表格的异构金融文档，两阶段混合检索+神经重排序是最有效的检索策略。BM25等传统方法在特定领域仍具竞争力，而查询扩展对数值查询效果有限。研究提供了融合方法和重排序深度的消融研究、成本-准确性建议，并发布了完整的基准代码。
...
### [155] [Do We Need Bigger Models for Science? Task-Aware Retrieval with Small Language Models](https://arxiv.org/abs/2604.01965)
*Florian Kelber,Matthias Jobst,Yuni Susanti,Michael Färber*

Main category: cs.IR

TL;DR: 检索增强框架结合任务感知路由和紧凑语言模型，在科学应用中部分补偿模型规模，但复杂推理仍需模型能力


<details>
  <summary>Details</summary>
Motivation: 当前科学知识发现过度依赖大型专有模型，限制了研究的可复现性和可访问性，需要探索检索设计能否补偿模型规模

Method: 设计轻量级检索增强框架，包含任务感知路由选择专用检索策略，整合全文科学论文和结构化元数据，使用紧凑指令调优语言模型生成带引用的回答

Result: 检索和模型规模是互补而非可互换的，检索设计可部分补偿小模型，但复杂推理任务仍需模型容量
...
### [156] [Multi-Agent Video Recommenders: Evolution, Patterns, and Open Challenges](https://arxiv.org/abs/2604.02211)
*Srivaths Ranganathan,Abhishek Dharmaratnakar,Anushree Sinha,Debanshu Das*

Main category: cs.IR

TL;DR: 该论文综述了多智能体视频推荐系统(MAVRS)的发展历程，从传统的单模型推荐系统到基于多智能体架构的系统，再到新兴的LLM驱动的MAVRS，分析了协作模式分类、协调机制，并讨论了代表性框架和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统单模型视频推荐系统仅优化静态参与度指标，难以满足现代平台的动态需求。多智能体架构通过协调专门负责视频理解、推理、记忆和反馈的智能体，能够提供更精确、可解释的推荐，从而重新定义视频推荐系统的服务、学习和适应方式。

Method: 论文采用综述研究方法，追溯多智能体视频推荐系统的演进历程。结合多智能体推荐系统、基础模型和对话AI的思想，提出LLM驱动的MAVRS新兴领域。建立了协作模式分类法，分析了跨不同视频领域（从短视频到教育平台）的协调机制。通过分析代表性框架（如早期MARL系统MMRF和近期LLM驱动架构MACRec、Agent4Rec）来阐释这些模式。

Result: 论文系统梳理了MAVRS的发展脉络，提出了多智能体视频推荐系统的分类体系，识别了不同协作模式和协调机制。通过分析具体框架展示了多智能体架构如何提升推荐系统的精确性和可解释性，同时指出了从传统MARL到LLM驱动的技术演进趋势。

Conclusion: 多智能体视频推荐系统代表了视频推荐技术的重要发展方向，能够更好地满足现代平台的动态需求。LLM驱动的MAVRS结合了多智能体推荐系统、基础模型和对话AI的优势，展现出巨大潜力。未来研究应关注可扩展性、多模态理解、激励对齐等挑战，并探索混合强化学习-LLM系统、终身个性化和自改进推荐系统等方向。
...
### [157] [DySCo: Dynamic Semantic Compression for Effective Long-term Time Series Forecasting](https://arxiv.org/abs/2604.01261)
...
Method: 提出动态语义压缩框架DySCo：1) 熵引导动态采样机制自主识别并保留高熵片段，压缩冗余趋势；2) 分层频率增强分解策略分离高频异常和低频模式；3) 跨尺度交互混合器动态融合全局上下文与局部表征。

Result: 实验表明DySCo可作为通用即插即用模块，显著增强主流模型捕捉长时相关性的能力，同时降低计算成本。

Conclusion: DySCo框架通过动态语义压缩有效解决了时间序列预测中长时依赖捕捉的挑战，在保持关键细节的同时减少计算冗余，具有广泛的适用性。
...
### [158] [Forecasting Supply Chain Disruptions with Foresight Learning](https://arxiv.org/abs/2604.01298)
*Benjamin Turtel,Paul Wilczewski,Kris Skotheim*

Main category: cs.LG

TL;DR: 提出端到端框架训练LLM生成校准的概率预测，用于供应链中断预测，显著优于GPT-5等基线
...
Result: 模型在准确性、校准性和精确度上显著优于包括GPT-5在内的强基线，训练可诱导更结构化可靠的概率推理
...
### [159] [An Online Machine Learning Multi-resolution Optimization Framework for Energy System Design Limit of Performance Analysis](https://arxiv.org/abs/2604.01308)
...
<details>
  <summary>Details</summary>
Motivation: 工业过程可靠集成能源系统设计需要在多个保真度级别进行优化和验证，但不同保真度模型之间的不匹配会掩盖性能损失来源，并使架构到运行性能差距的量化变得复杂

Method: 提出在线机器学习加速的多分辨率优化框架：1）多目标架构优化选择系统配置和组件容量；2）ML加速的多分辨率滚动时域最优控制策略，基于预测不确定性自适应调度优化分辨率，使用精英低保真解热启动高保真求解

Result: 在1 MW工业热负荷试点案例中，多分辨率策略相对于基于规则的控制器将架构-运行性能差距减少42%，相对于无ML指导的多保真方法减少34%的高保真模型评估需求

Conclusion: 该框架使高保真验证变得可行，提供了实际可达到操作性能的上限，实现了更快更可靠的设计验证
...
TL;DR: 提出ReDiRect框架，通过无监督方法将大型交易图模糊分区为可管理组件，以高效检测洗钱活动，减少误报并提高实际应用性。


<details>
  <summary>Details</summary>
Motivation: 现有洗钱检测方法存在局限性：洗钱者通过模仿交易模式逃避监控，现有算法难以处理大规模复杂数据，基于风险规则的刚性系统产生过多误报信号。

Method: 提出ReDiRect框架（REduce, DIstribute, and RECTify），采用无监督方法将大型交易图模糊分区为更小、可管理的组件，实现分布式快速处理，并定义改进的评估指标来更好捕捉洗钱模式。

Result: 在真实Libra数据集和IBM Watson合成数据集上的实验表明，该框架在效率和实际应用性方面优于现有和最先进技术，实现了卓越性能。

Conclusion: ReDiRect框架有效解决了洗钱检测中的规模、复杂性和误报问题，通过无监督图分区方法和改进评估指标，为实际金融监控提供了更高效的解决方案。
...
### [161] [Efficient and Principled Scientific Discovery through Bayesian Optimization: A Tutorial](https://arxiv.org/abs/2604.01328)
...
TL;DR: 本教程介绍贝叶斯优化(BO)作为科学发现的概率驱动框架，通过替代模型和采集函数自动化假设-实验-优化循环，应用于催化、材料科学等领域。


<details>
  <summary>Details</summary>
Motivation: 传统科学发现依赖直觉的假设-实验-优化循环，存在资源浪费、设计低效和关键洞察缺失的问题。需要一种原则性的概率驱动框架来形式化和自动化这一核心科学循环。

Method: 贝叶斯优化使用替代模型（如高斯过程）将经验观察建模为演化假设，利用采集函数指导实验选择，平衡已知知识的利用和未知领域的探索。教程涵盖BO的核心组件、端到端工作流程，以及针对科学应用的技术扩展。

Result: 通过催化、材料科学、有机合成和分子发现等案例研究展示了BO在现实世界中的有效性。教程提供了分层内容，使跨学科研究人员能够设计更高效的实验。

Conclusion: 贝叶斯优化为科学发现提供了一个原则性的概率驱动框架，能够消除猜测和手动试错，加速原则性科学发现，并桥接AI进展与自然科学实际应用。
...
### [162] [Model Merging via Data-Free Covariance Estimation](https://arxiv.org/abs/2604.01329)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法存在两个问题：1）启发式方法缺乏理论依据；2）基于干扰最小化的优化方法需要数据来估计协方差矩阵，而数据在实际合并时可能不可得。本文旨在开发一种既具有理论依据又无需数据的模型合并方法

Method: 重新审视干扰最小化框架，发现在特定条件下，可以直接从模型差异矩阵估计协方差矩阵，从而消除对数据的需求。该方法将模型合并表述为逐层优化问题，直接最小化任务间的干扰

Result: 在视觉和语言基准测试上验证了该方法，模型规模从8600万参数到70亿参数不等，性能优于先前最先进的无数据合并方法

Conclusion: 提出了一种理论上有依据且无需数据的模型合并方法，通过从差异矩阵直接估计协方差矩阵，既保持了理论优势又具有实际可行性，在多种基准测试中表现出色
...
### [163] [SECURE: Stable Early Collision Understanding via Robust Embeddings in Autonomous Driving](https://arxiv.org/abs/2604.01337)
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在事故预测方面取得显著进展，但这些安全关键系统在真实世界扰动下的鲁棒性仍然是一个重大挑战。研究发现即使像CRASH这样的最先进模型，在面对微小输入扰动时也会表现出预测和潜在表示的不稳定性，这带来了严重的可靠性风险。

Method: 提出SECURE框架，基于四个关键属性：预测空间和潜在特征空间的一致性和稳定性。采用原则性训练方法，通过多目标损失函数对基线模型进行微调，该损失函数最小化与参考模型的差异，并惩罚对对抗性扰动的敏感性。

Result: 在DAD和CCD数据集上的实验表明，该方法不仅显著增强了对各种扰动的鲁棒性，还提高了在清洁数据上的性能，取得了新的最先进结果。
...
### [164] [Massively Parallel Exact Inference for Hawkes Processes](https://arxiv.org/abs/2604.01342)
...
<details>
  <summary>Details</summary>
Motivation: 多元Hawkes过程的最大似然估计计算复杂度为O(N²)，现有线性指数Hawkes过程的O(N)递归算法无法充分利用现代GPU的并行计算能力。

Method: 将Hawkes过程强度表示为稀疏转移矩阵的乘积，通过线性时间关联乘法实现并行前缀扫描计算，支持大规模并行处理。

Result: 在模拟和真实数据集上实现了数量级的加速，可扩展到数千个节点和数千万个事件，远超先前工作的规模。

Conclusion: 提出的并行前缀扫描算法为线性指数Hawkes过程提供了精确、高效且可扩展的最大似然估计方法，保持了模型的简单性和可解释性。
...
### [165] [Malliavin Calculus for Counterfactual Gradient Estimation in Adaptive Inverse Reinforcement Learning](https://arxiv.org/abs/2604.01345)
...
Method: 提出被动Langevin算法，利用Malliavin微积分将反事实条件转化为无条件期望的比值，推导Malliavin导数及其伴随Skorohod积分形式，提供具体算法实现反事实梯度估计

Result: 通过Malliavin微积分方法能够有效估计反事实梯度，恢复标准估计速率，克服传统方法的效率问题

Conclusion: 提出的基于Malliavin微积分的被动Langevin算法为自适应逆强化学习提供了有效的反事实梯度估计方法，解决了传统方法在效率和收敛速度方面的局限性
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习在无需与环境交互的情况下从先前收集的数据中学习策略，这对于高风险应用尤为重要。现有方法通常依赖于数据覆盖的严格假设，且容易受到分布偏移的影响。

Method: 提出了基于残差的离线RL框架，定义了一个基于残差的贝尔曼最优算子，该算子通过利用经验残差将转移动态估计误差显式地纳入策略优化。进一步开发了基于残差的离线深度Q学习（DQN）算法。

Result: 证明了该贝尔曼算子是压缩映射，并确定了其固定点具有渐近最优性和有限样本保证的条件。在随机CartPole环境中验证了基于残差的离线DQN算法的有效性。

Conclusion: 提出的基于残差的框架为一般状态和动作空间的离线强化学习提供了一种有效方法，能够处理分布偏移问题并具有理论保证。
...
TL;DR: 该报告介绍了Qumphy项目中用于量化医疗机器学习算法不确定性的六个PPG相关基准问题及其数据集


<details>
  <summary>Details</summary>
Motivation: 开发量化机器学习算法在医疗应用中不确定性的方法，特别是在PPG信号分析领域，需要标准化的基准问题和数据集来评估算法性能

Method: 识别并定义了六个与PPG信号相关的医疗问题作为基准问题，同时描述了适用的基准数据集及其使用方法

Result: 建立了包含六个PPG相关医疗问题的基准问题集，并提供了相应的基准数据集描述和使用指南

Conclusion: 该报告为评估医疗机器学习算法在PPG信号分析中的不确定性提供了标准化的基准框架，有助于推动该领域的研究和算法比较
...
<details>
  <summary>Details</summary>
Motivation: 现有预训练缩放定律（如Chinchilla）未考虑测试时缩放（如重复采样）带来的推理成本，这导致模型大小和推理样本数之间存在权衡。需要新的缩放定律来在固定端到端预算下联合优化预训练和测试时决策。

Method: 提出T²缩放定律，将预训练缩放定律现代化，加入用于测试时缩放的pass@k建模，然后联合优化预训练和测试时决策。通过任务损失和任务准确率两个维度验证预测的稳健性。

Result: 在八个下游任务上发现：考虑推理成本时，最优预训练决策会大幅转向过训练区域，远超标准预训练缩放套件的范围。在T²预测的最优区域预训练过训练模型，确认其性能显著优于仅基于预训练缩放的模型。后训练阶段的结果也验证了T²缩放的有效性。

Conclusion: T²缩放定律为现代LLM部署提供了更全面的优化框架，表明在考虑端到端成本时，过训练策略可能比传统预训练缩放更优，这对实际部署具有重要指导意义。
...
TL;DR: 该论文研究如何通过测试时计算（"思考"）来改善语言模型的潜在泛化能力，使用强化学习训练模型生成长思维链以解决反转诅咒等潜在泛化失败问题。


<details>
  <summary>Details</summary>
Motivation: 语言模型存在两种知识获取机制：权重内学习和上下文学习。权重内学习在促进内部知识的演绎推理方面存在困难，表现为潜在泛化能力不足（如反转诅咒）。现有方法依赖训练时数据增强，但这些方法任务特定、扩展性差且无法泛化到分布外知识。因此需要研究如何通过测试时计算来改善潜在泛化。

Method: 使用基于正确性反馈的强化学习训练模型生成长思维链，通过测试时计算（"思考"）来改善潜在泛化。该方法不依赖任务特定的数据增强，而是训练模型在推理时进行更深入的思考。

Result: 思考方法不仅解决了分布内知识的许多潜在泛化失败实例，而且与增强基线不同，能够泛化到未进行强化学习的新知识。在纯反转任务中，思考虽未实现直接知识反转，但通过生成-验证能力使性能显著高于随机水平。然而，事实自验证的脆弱性意味着思考模型在此任务上的性能仍远低于上下文学习。

Conclusion: 测试时思考是改善语言模型潜在泛化能力的灵活且有前景的方向。该方法不依赖任务特定的数据增强，能够泛化到新知识，为改善模型推理能力提供了新途径。
...
TL;DR: 提出soft MPCritic框架，将强化学习与模型预测控制结合，在值函数空间学习的同时使用基于采样的规划进行在线控制和价值目标生成，通过摊销预热策略实现计算效率


<details>
  <summary>Details</summary>
Motivation: 强化学习（RL）和模型预测控制（MPC）各有优势但难以大规模结合，现有方法在计算上具有挑战性，需要一种既能利用MPC的规划能力又能通过RL学习价值函数的实用框架

Method: 使用模型预测路径积分控制（MPPI）实例化MPC，通过拟合价值迭代训练终端Q函数；引入摊销预热策略，重用在线观测的计划开环动作序列计算批量MPPI价值目标；采用基于场景的规划，使用经过训练的动态模型集合进行下一步预测

Result: soft MPCritic在经典和复杂控制任务中通过鲁棒的短视界规划有效学习，在保持解质量的同时实现计算可行性，为在策略提取和直接长视界规划可能失败的环境中合成MPC策略提供了实用蓝图

Conclusion: soft MPCritic通过将RL与MPC在值函数空间结合，提供了可扩展的框架，能够处理传统方法难以应对的控制场景，为强化学习与模型预测控制的融合提供了实用解决方案
...
TL;DR: ES与GRPO在LLM微调中达到相似任务性能，但产生几何结构不同的参数空间解决方案，ES更新幅度更大且方向更分散，而GRPO更新更局部化，两者解之间存在线性连接无损失障碍。
...
Method: 在四个任务上比较ES和GRPO，包括单任务和顺序持续学习设置；开发ES分析理论来解释观察到的现象；通过实验测量模型更新幅度、KL散度漂移和解决方案几何结构。

Result: ES在单任务准确率上匹配或超过GRPO，在持续学习中迭代预算受控时保持竞争力；ES产生更大、更分散的更新和更广泛的离任务KL漂移，而GRPO更新更小、更局部化；ES和GRPO解决方案线性连接且无损失障碍，尽管更新方向几乎正交。

Conclusion: 梯度自由和基于梯度的微调方法可以达到相似准确率但几何结构不同的解决方案，这对遗忘和知识保存有重要影响，表明不同优化方法可能通过不同路径达到类似性能。
...
TL;DR: 论文提出REPAIR方法，通过贝叶斯最优重排序解决长尾分类问题，将残差校正分解为类间常数项和输入依赖的成对项，优于固定偏移方法


<details>
  <summary>Details</summary>
Motivation: 现有后处理方法（如logit adjustment）使用固定的类间偏移来校正长尾分类中的偏差，但这种固定偏移无法适应不同输入间的变化需求。当相同标签对在不同上下文中产生不兼容的排序约束时，固定偏移无法恢复贝叶斯最优排序

Method: 提出REPAIR（Reranking via Pairwise residual correction）方法：1）基于基础模型top-k候选列表进行贝叶斯最优重排序；2）将残差校正分解为类间常数项和成对项；3）使用收缩稳定的类间项和由候选列表竞争特征驱动的线性成对项构建轻量级后处理重排序器

Result: 在图像分类、物种识别、场景识别和罕见疾病诊断等五个基准测试上的实验证实，该分解能够解释成对校正何时有效、何时类间校正单独足够。REPAIR方法在需要成对校正的场景中表现优于仅使用固定偏移的方法

Conclusion: 长尾分类中的校正需求可以分解为类间常数项和输入依赖的成对项。当相同标签对在不同上下文中产生不兼容排序约束时，固定偏移方法失效，需要成对校正。REPAIR方法通过结合这两种校正实现了更好的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型对齐方法主要优化帮助性和无害性，但忽视了关系风险（如依赖强化、过度保护、强制指导等），这些风险会损害用户自主性。需要一种能平衡帮助性和自主性保护的框架。

Method: 提出Care-Conditioned Neuromodulation（CCN）状态依赖控制框架：1）从结构化用户状态和对话上下文中学习标量信号；2）该信号调节响应生成和候选选择；3）形式化为自主性保护对齐问题，定义奖励自主性支持和帮助性、惩罚依赖和强制的效用函数；4）结合基于效用的重排序。

Result: 在构建的关系失败模式基准测试中（包括安慰依赖、操纵性关怀、过度保护、边界不一致等），CCN结合效用重排序相比监督微调将自主性保护效用提高了+0.25，相比偏好优化基线提高了+0.07，同时保持相当的帮助性。初步人类评估和零样本迁移到真实情感支持对话显示与自动化指标方向一致。

Conclusion: 状态依赖控制结合基于效用的选择是处理自主敏感性对话中多目标对齐的实用方法，能够在保持帮助性的同时更好地保护用户自主性。
...
<details>
  <summary>Details</summary>
Motivation: 现有序列模型（如LSTM、状态空间模型和Transformer变体）在长序列任务中的泛化能力有限，需要一种能够学习稳定内部结构并随时间演化的方法

Method: 在慢速观测更新之间插入快速循环潜在更新，利用自组织能力学习随时间演化的稳定内部结构，保持表示的一致性和聚类特性

Result: 在强化学习和算法任务中，相比LSTM、状态空间模型和Transformer变体等基线方法，该方法展现出更好的分布外泛化能力

Conclusion: 通过交替快速循环潜在更新和慢速观测更新，结合自组织能力，能够学习稳定且随时间演化的内部结构，显著提升序列模型的长时程表示一致性和泛化性能
...
### [176] [Optimizing EEG Graph Structure for Seizure Detection: An Information Bottleneck and Self-Supervised Learning Approach](https://arxiv.org/abs/2604.01595)
*Lincan Li,Rikuto Kotoge,Xihao Piao,Zheng Chen,Yushun Dong*

Main category: cs.LG

TL;DR: 提出IRENE方法，通过信息瓶颈引导的图结构学习和自监督学习，解决EEG癫痫检测中的噪声、动态图构建和标签稀缺问题。


<details>
  <summary>Details</summary>
Motivation: EEG癫痫检测面临复杂时空动态性和患者间高度变异性挑战。现有方法通过统计相关性、预定义相似度或隐式学习构建动态图，但很少考虑EEG的噪声特性，导致图包含冗余或任务无关连接，影响模型性能。

Method: 提出IRENE方法：1) 联合学习去噪动态图结构和信息瓶颈引导的时空表征；2) 图构建器显式考虑EEG噪声特性，产生紧凑可靠的连接模式；3) 采用自监督图掩码自编码器，基于动态图上下文重构掩码EEG信号，促进结构感知的紧凑表征；4) 解决三个核心挑战：识别最信息节点/边、解释癫痫传播、增强对标签稀缺和患者间变异性的鲁棒性。

Result: 在基准EEG数据集上的大量实验表明，该方法在癫痫检测方面优于最先进的基线方法，并为癫痫动态提供了临床有意义的见解。

Conclusion: IRENE通过信息瓶颈引导的图结构学习和自监督学习，有效解决了EEG癫痫检测中的关键挑战，提供了性能优越且具有临床解释性的解决方案。
...
### [177] [Learning from the Right Rollouts: Data Attribution for PPO-based LLM Post-Training](https://arxiv.org/abs/2604.01597)
*Dong Shu,Denghui Zhang,Jessica Hullman*

Main category: cs.LG

TL;DR: I-PPO：一种通过梯度影响分数筛选有害推理轨迹的RL框架，提升训练效率和模型忠实性
...
Method: 提出Influence-Guided PPO (I-PPO)，将数据归因集成到RL后训练循环中。通过基于梯度的近似计算每个轨迹的影响分数，识别并消除与验证梯度反对齐的轨迹。

Result: 实验表明I-PPO持续优于SFT和PPO基线。筛选过程充当内在的早停机制，加速训练效率，同时有效减少不忠实的思维链推理。

Conclusion: I-PPO通过选择性训练轨迹提供了一种更高效的RL训练方法，能够提升模型性能并减少有害推理的影响。
...
### [178] [Training In-Context and In-Weights Mixtures Via Contrastive Context Sampling](https://arxiv.org/abs/2604.01601)
*Deeptanshu Malu,Deevyanshu Malu,Aditya Nemiwal,Sunita Sarawagi*

Main category: cs.LG

TL;DR: 本文研究了上下文学习(ICL)与权重学习(IWL)的协同训练策略，发现上下文示例与目标输入之间的相似性结构对ICL演化至关重要，并提出对比上下文训练方法来解决现有问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然同时具备上下文学习和权重学习能力，但标准的任务特定微调往往会削弱上下文学习能力。现有的上下文训练方法中，上下文示例与目标输入的相似性结构对上下文学习的演化具有重要影响，但尚未得到充分研究。

Method: 提出对比上下文训练方法，包含两种对比：(1)在单个上下文中混合相似和随机示例，以演化正确的上下文学习形式；(2)在不同上下文中使用不同相似度等级的示例，以演化上下文学习与权重学习的混合。通过理论分析和在四个大语言模型及多个任务上的实证评估进行验证。

Result: 随机上下文会导致上下文学习能力丧失和权重学习主导，而仅使用相似示例则使上下文学习退化为不考虑相关性的标签复制。对比上下文训练能够产生稳定的上下文学习-权重学习混合，避免坍缩为纯上下文学习、纯权重学习或简单复制。

Conclusion: 上下文示例与目标输入之间的相似性结构是影响上下文学习演化的关键因素。提出的对比上下文训练方法能够有效协同发展上下文学习和权重学习能力，并实现基于上下文相关性的模式切换。
...
### [179] [Expert-Choice Routing Enables Adaptive Computation in Diffusion Language Models](https://arxiv.org/abs/2604.01622)
...
TL;DR: 该论文提出在扩散语言模型（DLM）的混合专家（MoE）系统中使用专家选择（EC）路由替代传统的令牌选择（TC）路由，通过时间步相关的专家容量分配优化计算效率，实现更好的负载均衡和性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有DLM的MoE模型继承了自回归系统中的令牌选择路由机制，导致负载不均衡和计算分配僵化，需要更适合DLM特性的路由方案来提升并行生成效率和收敛速度。

Method: 提出专家选择路由方案，该方案天生具有确定性负载均衡特性；引入时间步相关的专家容量分配，根据去噪步骤动态调整专家分配；展示如何将预训练的TC DLM通过仅替换路由器改造为EC路由。

Result: EC路由相比TC路由实现了更高的吞吐量和更快的收敛速度；在低掩码率步骤分配更多容量能在相同FLOPs下获得最佳性能；改造后的EC DLM在多种下游任务中实现了更快的收敛和更高的准确率。

Conclusion: 专家选择路由是DLM MoE模型的优越范式，DLM中的计算可以视为自适应策略而非固定架构常数，EC路由为DLM提供了更好的负载均衡和计算效率优化方案。
...
### [180] [CRIT: Graph-Based Automatic Data Synthesis to Enhance Cross-Modal Multi-Hop Reasoning](https://arxiv.org/abs/2604.01634)
*Junyoung Sung,Seungwoo Lyu,Minjun Kim,Sumin An,Arsha Nagrani,Paul Hongsuck Seo*

Main category: cs.LG

TL;DR: CRIT是一个新的多模态推理数据集和基准，专注于跨模态多跳推理任务，通过图基自动流水线生成复杂推理问题，解决现有基准中单模态推理的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界推理需要跨模态信息整合，但现有多模态基准主要依赖单图像或图像集，答案可以从单一模态推断，训练数据中交错的图像-文本内容也很少强制互补的多跳推理，导致视觉语言模型经常产生幻觉和缺乏视觉证据支持的推理轨迹。

Method: 采用图基自动流水线生成复杂的跨模态推理任务，构建CRIT数据集，涵盖自然图像、视频和文本丰富来源等多个领域，并包含手动验证的测试集以确保可靠评估。

Result: 实验表明，即使最先进的模型在此类推理任务上也表现不佳。在CRIT上训练的模型在跨模态多跳推理方面显示出显著提升，包括在SPIQA和其他标准多模态基准上的强劲改进。

Conclusion: CRIT填补了多模态推理基准的空白，通过强制跨模态多跳推理，能够更好地评估和提升视觉语言模型的真实世界推理能力，训练后的模型在标准基准上也表现出改进。
...
### [181] [Label Shift Estimation With Incremental Prior Update](https://arxiv.org/abs/2604.01651)
*Yunrui Zhang,Gustavo Batista,Salil S. Kanhere*

Main category: cs.LG

TL;DR: 提出一种新的后验标签偏移估计方法，通过增量更新每个样本的先验概率来调整后验分布，相比现有方法基于更弱的校准假设，适用于任何黑盒概率分类器。


<details>
  <summary>Details</summary>
Motivation: 现实场景中训练集和测试集的标签分布往往不同（标签偏移），如医疗诊断、欺诈检测、社交媒体分类等任务中标签分布会随时间、地点、趋势变化。现有标签偏移估计方法通常基于验证集估计混淆矩阵进行矩匹配，或使用期望最大化算法最大化新数据似然，存在局限性。

Method: 提出一种后验标签偏移估计方法，通过增量更新每个样本的先验概率来调整后验分布。该方法基于对现代概率分类器的直观假设，相比其他方法依赖更弱的校准概念。作为后验方法，可应用于任何黑盒概率分类器，无需访问模型内部结构。

Result: 在CIFAR-10和MNIST数据集上的实验表明，该方法在不同校准设置和不同强度的标签偏移下，始终优于当前最先进的基于最大似然的方法。

Conclusion: 提出的后验标签偏移估计方法具有更好的性能、更弱的假设要求和更广泛的适用性，为处理现实世界中的标签偏移问题提供了有效的解决方案。
...
### [182] [Cognitive Energy Modeling for Neuroadaptive Human-Machine Systems using EEG and WGAN-GP](https://arxiv.org/abs/2604.01653)
...
<details>
  <summary>Details</summary>
Motivation: 虽然GAN已广泛用于EEG数据增强，但尚不清楚合成EEG是否保留了基于转换分析所需的底层动态结构。需要评估合成EEG是否保持分布几何结构，以支持基于能量的认知状态转换建模。

Method: 使用SBP衍生的传输成本作为评估指标，比较真实和合成EEG在Stroop任务中提取的转换能量。在群体和参与者层面进行分析，验证合成EEG是否保留SBP建模所需的转换结构。

Result: 真实和合成EEG在Stroop任务中提取的转换能量显示出强烈的一致性，表明合成EEG保留了SBP建模所需的转换结构，可用于数据高效的神经自适应系统。

Conclusion: 合成EEG保留了认知状态转换所需的动态结构，支持其在数据高效神经自适应系统中的应用。SBP衍生的认知能量可作为自适应人机系统的控制信号，实现基于用户认知和情感状态的实时系统行为调整。
...
### [183] [MiCA Learns More Knowledge Than LoRA and Full Fine-Tuning](https://arxiv.org/abs/2604.01694)
*Sten Rüdiger,Sebastian Raschka*

Main category: cs.LG

TL;DR: MiCA是一种新颖的参数高效微调方法，专注于调整LLM中未充分利用的表示子空间，通过SVD识别与最小奇异值相关的次要奇异向量方向，相比LoRA在知识获取上提升5.9倍，参数占用仅6-60%。


<details>
  <summary>Details</summary>
Motivation: 传统参数高效微调方法（如LoRA）主要针对模型表示中的主导子空间，但可能忽略了未充分利用的次要子空间。MiCA旨在通过调整这些次要子空间来更有效地整合新知识到预训练语言模型中。

Method: MiCA使用奇异值分解（SVD）识别与最小奇异值相关的次要奇异向量方向，将微调过程中的参数更新约束在这些次要方向上，从而调整模型表示中未充分利用的子空间。

Result: 在优化训练超参数下，MiCA在知识获取方面相比LoRA提升高达5.9倍，同时保持极小的参数占用（仅为LoRA的6-60%）。这表明约束在次要奇异方向上的适应提供了更高效稳定的知识整合机制。

Conclusion: 针对次要奇异向量方向的参数约束适应策略比传统主导子空间适应方法更高效，为预训练语言模型的知识整合提供了更稳定有效的机制，在保持参数效率的同时显著提升性能。
...
### [184] [Transformer self-attention encoder-decoder with multimodal deep learning for response time series forecasting and digital twin support in wind structural health monitoring](https://arxiv.org/abs/2604.01712)
...
Method: 采用基于Transformer的人工智能模型，利用系统的时间特性训练预测模型。首先训练预测模型，然后将振动预测值与实测值比较以检测大偏差，最后将识别出的案例用作结构变化的早期预警指标。

Result: 该方法在挪威科技大学监测的Hardanger Bridge真实测量数据上进行了严格检验。模型在现实条件下准确捕捉结构行为，并能适应系统激励的变化。相比传统方法，该AI模型在响应预测方面表现更优，无需对风平稳性或结构正常振动行为做出假设。
...
### [185] [MATA-Former & SIICU: Semantic Aware Temporal Alignment for High-Fidelity ICU Risk Prediction](https://arxiv.org/abs/2604.01727)
*Zhichong Zheng,Xiaohang Nie,Xueqi Wang,Yuanjin Zhao,Haitao Zhang,Yichao Tang*

Main category: cs.LG

TL;DR: MATA-Former：基于医学语义的时间注意力机制，结合Plateau-Gaussian软标签，用于临床风险预测，在SIICU和MIMIC-IV数据集上表现优异


<details>
  <summary>Details</summary>
Motivation: 现有临床风险预测方法依赖时间邻近性而非病理学依赖关系，且受限于粗糙的二元监督和物理时间戳，无法与临床逻辑对齐

Method: 提出医学语义感知的时间注意力Transformer（MATA-Former），利用事件语义动态参数化注意力权重，优先考虑因果有效性而非时间滞后；引入Plateau-Gaussian软标签（PSL），将二元分类重构为连续多时间范围回归，实现全轨迹风险建模

Result: 在SIICU（新构建数据集，包含50.6万+事件，专家验证的细粒度标注）和MIMIC-IV数据集上评估，框架在捕获文本密集、不规则临床时间序列风险方面表现出优越效能和鲁棒泛化能力

Conclusion: 通过语义感知的注意力机制和连续风险建模，MATA-Former能够更好地捕捉临床风险演变的内在病理学依赖关系，为临床风险预测提供了更符合临床逻辑的解决方案
...
### [186] [Koopman-Based Nonlinear Identification and Adaptive Control of a Turbofan Engine](https://arxiv.org/abs/2604.01730)
*David Grasev*
...
<details>
  <summary>Details</summary>
Motivation: 研究Koopman算子方法在涡扇发动机多变量控制中的应用，解决传统线性控制方法在处理非线性发动机动力学时的局限性，提高控制系统的鲁棒性和适应性。

Method: 1. 建立基于物理的部件级模型生成训练数据和验证控制器；2. 开发元启发式扩展动态模态分解方法，构建能同时准确捕捉转子转速和发动机压力比的单一Koopman模型；3. 基于时变Koopman模型开发两种控制器：带扰动观测器的自适应Koopman模型预测控制器（AKMPC）和作为基准的Koopman反馈线性化控制器（K-FBLC）；4. 在海平面和变化飞行条件下评估两种控制策略（转子转速配置和EPR配置）。

Result: 1. 提出的辨识方法能准确预测转子转速和发动机压力比，使Koopman模型可在不同控制方案中灵活重用；2. 两种控制策略在稳态条件下性能相当，但在变化飞行条件下，AKMPC因能补偿模型失配而表现出比K-FBLC更优的鲁棒性；3. EPR控制策略改善了推力响应。

Conclusion: 该研究验证了基于Koopman的控制方法在涡扇发动机控制中的适用性，并证明了AKMPC框架在鲁棒控制方面的优势，为复杂非线性系统的控制提供了有效解决方案。
...
### [187] [FourierMoE: Fourier Mixture-of-Experts Adaptation of Large Language Models](https://arxiv.org/abs/2604.01762)
*Juyong Jiang,Fan Wang,Hong Qi,Sunghun Kim,Jing Tang*

Main category: cs.LG

TL;DR: FourierMoE：一种在谱域进行频率感知适配的参数高效微调方法，通过傅里叶变换和MoE架构解决多任务微调中的任务干扰和参数限制问题。


<details>
  <summary>Details</summary>
Motivation: 标准PEFT方法在多任务微调中面临任务干扰和表示能力不足的问题，现有MoE方法主要在空间域操作，存在结构冗余和参数开销。谱域分析显示不同任务具有不同的频率能量分布，LLM层表现出异质的频率敏感性，这为谱域适配提供了理论基础。

Method: 提出FourierMoE，将MoE架构与逆离散傅里叶变换（IDFT）结合进行频率感知适配。使用频率自适应路由器将token分发到专门处理不同频带的专家，每个专家学习一组共轭对称的复系数，通过IDFT无损重构为实值空间权重。

Result: 在28个基准测试、多种模型架构和规模上的广泛评估表明，FourierMoE在单任务和多任务设置中均优于竞争基线，同时使用显著更少的可训练参数。

Conclusion: 谱域专家适配是LLM微调的有效且参数高效的范式，FourierMoE通过频率感知的MoE架构解决了多任务PEFT中的关键挑战。
...
TL;DR: 该研究提出一个结合预测与运营分析的三步框架，用于处理零售需求数据中的季节性、不规则波动和噪声问题，通过比较N-BEATS MSTL和N-HiTS模型，最终选择N-BEATS进行优化预测，并结合整数线性规划生成成本最优的配送计划。


<details>
  <summary>Details</summary>
Motivation: 零售需求数据常呈现不可调和的季节性、不规则尖峰和噪声，使得精确预测几乎无法实现，这对供应链规划构成重大挑战。研究旨在开发一个结合预测与运营分析的框架，以应对这些数据质量问题并提升物流决策效果。

Method: 提出三步分析框架：1) 探索性数据分析，对180,519笔交易数据进行分区，分析长期趋势、季节性和配送相关属性；2) 比较统计时间序列分解模型N-BEATS MSTL与深度学习架构N-HiTS的预测性能；3) 选择最优模型（N-BEATS）预测未来4周需求（1918单位），结合整数线性规划在预算、容量和服务约束下最小化总配送时间。

Result: N-HiTS和N-BEATS均显著优于统计基准模型，其中N-BEATS被选为预测误差最低的最优模型。基于N-BEATS预测结果，整数线性规划生成了可行且成本最优的配送计划解决方案。

Conclusion: 该研究展示了精确预测与简单、高度可解释的模型优化在物流实践中的重要影响，为处理复杂零售数据提供了有效的分析框架，并证明了深度学习模型在需求预测中的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界系统中，由于成本、可达性和环境限制，密集仪器部署通常不可行。基于物理的求解器计算延迟和功耗高，无法在资源受限的监控系统中实时使用。

Method: 提出VIRSO（虚拟不规则实时稀疏算子），一种基于图的神经算子，用于不规则几何上的稀疏到密集重建；以及V-KNN（可变K近邻）算法，用于构建网格感知图。将推理重构为测量过程，结合谱分析和空间分析。

Result: 在三个核热工水力基准测试中，重建比从47:1到156:1，平均相对L2误差低于1%，参数更少。在NVIDIA H200上，10层配置将能量延迟积从约206 J·ms降至10.1 J·ms。在Jetson Orin Nano上，所有配置功耗低于10W，延迟低于1秒。
...
### [190] [CANDI: Curated Test-Time Adaptation for Multivariate Time-Series Anomaly Detection Under Distribution Shift](https://arxiv.org/abs/2604.01845)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中分布偏移普遍存在，导致预训练异常检测器性能严重下降。测试时自适应（TTA）仅使用未标记测试数据在线更新预训练模型，为解决这一挑战提供了可能。

Method: 提出CANDI框架，包含：1）假阳性挖掘（FPM）策略，基于异常分数和潜在相似性筛选适应样本；2）即插即用的时空感知正态性适应（SANA）模块，实现结构感知的模型更新。

Result: CANDI显著提升了分布偏移下MTSAD的性能，AUROC提升高达14%，同时使用更少的适应样本。

Conclusion: CANDI通过选择性适应潜在假阳性并保持预训练知识，有效解决了分布偏移对多元时间序列异常检测的影响，为实际部署提供了鲁棒的解决方案。
...
### [191] [Towards Intrinsically Calibrated Uncertainty Quantification in Industrial Data-Driven Models via Diffusion Sampler](https://arxiv.org/abs/2604.01870)
*Yiran Ma,Jerome Le Ny,Zhichao Chen,Zhihuan Song*

Main category: cs.LG

TL;DR: 提出基于扩散的后验采样框架，用于工业过程监测中的不确定性量化，无需后校准即可产生良好校准的预测不确定性


<details>
  <summary>Details</summary>
Motivation: 现代过程工业中，数据驱动模型对实时监测至关重要，但关键性能指标难以直接测量。虽然准确预测很重要，但可靠的不确定性量化对于安全、可靠性和决策制定同样关键，而当前数据驱动方法在这方面仍面临重大挑战
...
### [192] [Robust Graph Representation Learning via Adaptive Spectral Contrast](https://arxiv.org/abs/2604.01878)
...
<details>
  <summary>Details</summary>
Motivation: 现有谱图对比学习方法在处理混合图时面临谱困境：高频信号对异质性编码至关重要，但在谱集中扰动下具有更高方差。全局谱融合策略在节点级频率偏好分离的混合图上存在非零遗憾，证明是次优的。

Method: 提出ASPECT框架，采用可靠性感知的谱门控机制。通过极小极大博弈实现：节点级门控根据频率通道对专门构建的对抗器的稳定性进行动态重加权，对抗器通过瑞利商惩罚显式针对谱能量分布，迫使编码器学习既具有结构判别性又具有谱鲁棒性的表示。

Result: 在9个基准测试中的8个达到新的最先进性能，有效解耦有意义的异质性和偶然噪声。理论分析证明ASPECT能够规避全局融合的遗憾下界。

Conclusion: ASPECT通过节点级可靠性感知谱门控解决了谱图对比学习中的谱困境，实现了对混合图中同质性和异质性的统一处理，同时保证了表示的鲁棒性和判别性。
...
### [193] [Enhancing the Reliability of Medical AI through Expert-guided Uncertainty Modeling](https://arxiv.org/abs/2604.01898)
*Aleksei Khalin,Ekaterina Zaychenkova,Aleksandr Yugay,Andrey Goncharov,Sergey Korchagin,Alexey Zaytsev,Egor Ershov*

Main category: cs.LG

TL;DR: 提出利用专家分歧训练机器学习模型来改进医疗AI不确定性估计的新方法


<details>
  <summary>Details</summary>
Motivation: 医疗AI系统需要可靠的不确定性估计来识别高风险案例，但现有方法在量化数据模糊性引起的不确定性方面存在局限

Method: 利用专家响应的分歧生成训练目标，结合标准标签通过两集成方法分别估计两种不确定性成分，并提供轻量级变体

Result: 在图像分类、分割和问答任务中，结合专家知识可将不确定性估计质量提升9%到50%

Conclusion: 专家知识是构建医疗风险感知AI系统的宝贵信息来源，能显著提升不确定性估计质量
...
### [194] [annbatch unlocks terabyte-scale training of biological data in anndata](https://arxiv.org/abs/2604.01949)
...
Method: 开发annbatch——一个原生集成于anndata的mini-batch加载器，能够直接在磁盘支持的数据集上进行out-of-core训练，同时保持与scverse生态系统的完全兼容性。
...
### [195] [Learn by Surprise, Commit by Proof](https://arxiv.org/abs/2604.01951)
*Kang-Sin Choi*

Main category: cs.LG

TL;DR: LSCP是一种自门控后训练框架，通过异常损失检测、自问答链生成和基于置信度的β₂调整，实现自主知识获取，防止死记硬背并保护相邻知识。
...
Method: 1) 检测异常高损失段落；2) 生成自问答链让模型表达知识并识别缺口；3) 根据置信深度k调整AdamW的β₂参数(β₂=0.999·r^k)；4) 整个学习强度由单个参数r控制。

Result: 标准微调产生死记硬背(扰动比11.6±0.2倍基线)，而所有LSCP条件都学习语义(2.7-3.0倍)。r=1.0条件证实训练数据格式是防止记忆的主要机制，门控保护相邻知识免受污染(93±7%准确率vs 90%基线)。

Conclusion: LSCP框架通过自门控机制实现自主知识获取，防止死记硬背，保护已有知识，模拟生物记忆巩固，为后训练提供了更智能的方法。
...
### [196] [World Action Verifier: Self-Improving World Models via Forward-Inverse Asymmetry](https://arxiv.org/abs/2604.01985)
*Yuejiang Liu,Fan Feng,Lingjing Kong,Weifeng Lu,Jinzhou Tang,Kun Zhang,Kevin Murphy,Chelsea Finn,Yilun Du*

Main category: cs.LG

TL;DR: WAV框架通过将状态预测分解为状态合理性和动作可达性两个因素，并分别验证，使世界模型能够自我识别预测错误并自我改进，显著提升了样本效率和策略性能。


<details>
  <summary>Details</summary>
Motivation: 通用世界模型需要比策略学习更广泛的鲁棒性，必须对次优动作范围可靠预测，但动作标记的交互数据通常覆盖不足。现有方法在未充分探索的区域容易失败。

Method: 提出World Action Verifier框架：1) 将动作条件状态预测分解为状态合理性和动作可达性；2) 利用动作自由数据的广泛可用性和动作相关特征的较低维度这两个不对称性；3) 通过视频语料库获得多样化子目标生成器；4) 使用稀疏逆模型从状态特征子集推断动作；5) 通过生成子目标、推断动作和前向展开之间的循环一致性提供验证机制。

Result: 在MiniGrid、RoboMimic和ManiSkill的9个任务中，WAV实现了2倍的样本效率提升，下游策略性能提高了18%。

Conclusion: 通过分解验证和利用数据与特征的不对称性，WAV框架能够有效识别世界模型的预测错误并在未充分探索的区域自我改进，为构建更鲁棒的通用世界模型提供了有效途径。
...
### [197] [Apriel-Reasoner: RL Post-Training for General-Purpose and Efficient Reasoning](https://arxiv.org/abs/2604.02007)
...
<details>
  <summary>Details</summary>
Motivation: 现有通用推理模型通常不公开训练配方和领域混合策略，且跨领域联合优化面临挑战：不同领域在轨迹长度、问题难度和样本效率上差异巨大。长链式推理会增加推理成本和延迟，影响实际部署效率。

Method: 在Apriel-Base（15B参数开源LLM）基础上，使用公开数据集在五个领域进行可复现的多领域RL后训练。引入自适应领域采样机制保持目标领域比例，并提出难度感知长度惩罚，鼓励困难问题更长推理、简单问题更短推理，无额外训练开销。

Result: 在16K令牌输出预算下训练，推理时可泛化到32K令牌。在AIME 2025、GPQA、MMLU-Pro和LiveCodeBench上超越Apriel-Base，同时产生30-50%更短的推理轨迹。在相似规模的开源模型中达到相当性能但令牌成本更低，推进了准确性与令牌预算的帕累托前沿。

Conclusion: Apriel-Reasoner通过创新的自适应领域采样和难度感知长度惩罚，实现了跨领域的高效推理优化，在保持准确性的同时显著降低推理成本，为实际部署提供了有效的解决方案。
...
### [198] [Feature Weighting Improves Pool-Based Sequential Active Learning for Regression](https://arxiv.org/abs/2604.02019)
*Dongrui Wu*
...
<details>
  <summary>Details</summary>
Motivation: 传统的主动学习回归方法在计算样本间距离时没有考虑不同特征的重要性，导致样本选择不够优化。代表性（representativeness）和多样性（diversity）是主动学习中的重要考量因素，但现有方法在距离计算中未能有效利用特征权重信息。

Method: 提出了三种特征加权的单任务主动学习回归方法和两种特征加权的多任务主动学习回归方法。核心思想是：利用少量已标记样本训练的岭回归系数来对相应特征进行加权，从而改进样本间距离计算。这种方法易于实现，可以增强现有主动学习回归方法的性能。

Result: 实验表明，这种易于实现的增强方法几乎总是能提升四种现有主动学习回归方法的性能，无论是在单任务还是多任务回归问题中。特征加权策略还可以轻松扩展到基于流的主动学习和分类算法。

Conclusion: 通过引入特征加权机制来改进样本间距离计算，能够显著提升主动学习回归的性能。这种方法简单有效，具有很好的扩展性，可以应用于更广泛的主动学习场景。
...
### [199] [AA-SVD : Anchored and Adaptive SVD for Large Language Model Compression](https://arxiv.org/abs/2604.02119)
...
Method: 提出双锚定方法：将每个压缩层锚定到原始输出，同时显式建模输入分布偏移。在单个层压缩基础上，对每个Transformer块进行端到端细化，最小化块级输出失真，让压缩层共同补偿累积误差。

Result: 在大型语言模型上的实验表明，该方法在所有压缩比下均优于现有SVD基线，在激进压缩预算下优势尤为明显，而竞争方法性能显著下降或完全崩溃。
...
Method: 采用模块化框架：每个目标对应一个自私局部策略，通过拍卖机制协调——策略根据当前状态紧迫性出价，最高出价者选择动作；将问题转化为一般和博弈，使用PPO并发训练策略

Result: 在Atari Assault和网格世界路径规划任务上评估，相比PPO训练的单体策略获得显著更好的性能

Conclusion: 提出的拍卖机制框架能够有效处理动态多目标强化学习问题，实现运行时快速适应目标变化，且具有可解释性
...
### [201] [Smoothing the Landscape: Causal Structure Learning via Diffusion Denoising Objectives](https://arxiv.org/abs/2604.02250)
...
Method: 提出Denoising Diffusion Causal Discovery (DDCD)框架，利用扩散模型的去噪分数匹配目标平滑梯度以加速收敛，并引入自适应k-hop无环约束避免矩阵求逆，从而改进运行时性能。与生成式扩散模型不同，DDCD使用反向去噪过程推断参数化因果结构而非生成数据。

Result: 在合成基准数据上展示了DDCD的竞争性能，并在两个真实世界案例中进行了定性分析，证明了方法的实际效用。

Conclusion: DDCD通过结合扩散模型的去噪分数匹配和自适应无环约束，为高维观测数据中的因果发现提供了更高效、更稳定的解决方案，在合成和真实数据上都表现出良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的控制方法通常假设系统动态是平稳的，但现实世界系统由于漂移、磨损或运行条件变化而违反这一假设。论文研究时间变化动态下的强化学习控制问题。

Method: 采用基于高斯过程动态模型的持续模型强化学习方法，提出具有自适应数据缓冲机制的乐观模型强化学习算法，通过限制过时数据的影响来维持校准的不确定性。

Result: 分析表明持久非平稳性需要明确限制过时数据的影响以维持校准的不确定性和有意义的动态遗憾保证。提出的算法在具有非平稳动态的连续控制基准测试中表现出改进性能。

Conclusion: 对于非平稳动态系统中的强化学习控制，需要采用自适应数据管理策略来有效处理系统动态的时间变化，提出的缓冲机制方法在实践中表现良好。
...
### [203] [SKILL0: In-Context Agentic Reinforcement Learning for Skill Internalization](https://arxiv.org/abs/2604.02268)
...
<details>
  <summary>Details</summary>
Motivation: 当前推理时技能增强方法存在三个主要问题：检索噪声引入无关指导、注入技能内容带来大量token开销、模型仅遵循而非真正掌握知识。研究者希望探索技能能否内化到模型参数中，实现无需运行时技能检索的零样本自主行为。

Method: SKILL0框架包含：1）离线按类别分组技能，将交互历史渲染为紧凑视觉上下文；2）动态课程学习，从完整技能上下文开始，逐步减少依赖；3）评估每个技能文件对当前策略的帮助程度，在线性衰减的预算内仅保留仍有帮助的技能，最终实现完全零样本设置。

Result: 在ALFWorld任务上比标准RL基线提升9.7%，在Search-QA任务上提升6.6%，同时保持每步少于0.5k tokens的高效上下文使用。

Conclusion: SKILL0成功实现了技能内化，使智能体能够在零样本设置下自主执行任务，显著优于传统推理时技能增强方法，同时保持了上下文效率。
...
TL;DR: SRPO是一种结合GRPO和SDPO优势的强化学习框架，通过样本路由机制将正确样本分配给GRPO进行奖励对齐，将失败样本分配给SDPO进行针对性修正，解决了SDPO长期训练不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在局限性：GRPO采用粗粒度信用分配，对失败rollout进行统一惩罚，缺乏对特定偏差的token级关注；SDPO虽然提供更密集的logit级监督，能实现快速早期改进，但在长期训练中经常崩溃。需要一种既能快速改进又保持长期稳定的方法。

Method: 提出样本路由策略优化(SRPO)：1) 样本路由机制：将正确样本路由到GRPO进行奖励对齐的强化学习，将失败样本路由到SDPO进行针对性logit级修正；2) 熵感知动态加权机制：抑制高熵、不可靠的蒸馏目标，强调置信度高的目标；3) 统一的on-policy框架。

Result: 在五个基准测试和两种模型规模上，SRPO同时实现了SDPO的快速早期改进和GRPO的长期稳定性。在Qwen3-8B模型上，五个基准测试平均性能比GRPO提高3.4%，比SDPO提高6.3%，同时产生适度的响应长度，每步计算成本降低高达17.2%。

Conclusion: SRPO成功解决了SDPO的长期训练不稳定性问题，通过智能样本路由和熵感知加权，结合了两种方法的优势，实现了既快速又稳定的性能改进，同时降低了计算成本。
...
TL;DR: 提出go-mHC方法，通过广义正交随机矩阵理论实现双随机矩阵的精确参数化，在计算效率和表达能力之间取得平衡，显著提升流混合任务的性能。


<details>
  <summary>Details</summary>
Motivation: 双随机矩阵能够实现残差流之间的学习混合，但现有方法存在限制：精确方法随流数量d呈阶乘级扩展，而Kronecker分解方法虽然高效但表达能力有限。需要一种既能精确参数化Birkhoff多面体，又具有计算效率的方法。

Method: 基于广义正交随机矩阵理论提出新颖的精确参数化方法，复杂度为O(d³)。引入超参数s在计算效率边界和完全表达能力之间连续插值。在流形约束超连接(mHC)框架基础上实例化为go-mHC方法，可与Kronecker分解方法自然组合，以相似计算成本大幅恢复表达能力。

Result: 谱分析表明go-mHC比Kronecker分解基线更完整地填充Birkhoff多面体。在合成流混合任务中达到理论最小损失，收敛速度提升高达10倍。在3000万参数的GPT风格语言模型中验证了方法的有效性。

Conclusion: go-mHC方法的表达能力、效率和精确性为将流数量d作为模型容量的新维度提供了实用途径，平衡了计算复杂度和表达能力之间的矛盾。
...
### [206] [Taming the Exponential: A Fast Softmax Surrogate for Integer-Native Edge Inference](https://arxiv.org/abs/2604.02292)
...
<details>
  <summary>Details</summary>
Motivation: 在Transformer模型的MHA块中，softmax在低精度推理时成为计算瓶颈，特别是对于小模型，指数运算和归一化带来显著开销。AMD Versal AI Engines平台的现有实现使用bfloat16或LUT进行指数运算，无法充分利用平台的高吞吐量整数向量处理单元。

Method: 提出HCCS方法：使用剪裁线性映射替代指数函数，包含轻量级校准参数，这些参数基于代表性数据集离线优化，并为每个注意力头单独校准以保持统计特性。该方法针对AMD AI Engines的int8乘累加单元进行硬件优化实现。
...
Conclusion: HCCS为Transformer模型中的softmax计算瓶颈提供了有效的硬件优化解决方案，特别适合低精度推理场景，能够充分利用现代AI加速器的整数处理能力，在保持精度的同时显著提升推理速度。
...
TL;DR: BCR通过批量上下文强化训练，让模型在共享上下文中同时解决多个问题，实现了推理效率提升而精度不降甚至提升的"免费午餐"效果。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维推理方法虽然性能强，但存在token消耗过多、推理成本高的问题。现有的效率优化方法如显式长度惩罚、难度估计或多阶段课程学习要么会降低推理质量，要么需要复杂的训练流程。

Method: 提出批量上下文强化训练范式：训练模型在共享上下文窗口中同时解决N个问题，仅以每个实例的准确率作为奖励。这种结构修改创建了隐式token预算，无需显式长度监督。

Result: 1) 发现任务缩放定律：推理时并发问题数N增加，每个问题的token使用单调减少，精度下降比基线更平缓；2) 在1.5B和4B模型上，token使用减少15.8%-62.6%，同时在五个主要数学基准上保持或提升精度；3) 模型自主消除冗余元认知循环；4) 隐式预算约束避免了显式长度惩罚的对抗梯度和灾难性优化崩溃。

Conclusion: BCR证明了简单的结构激励能够解锁LLMs中潜在的高密度推理能力，提供了一种高度稳定、基于约束的长度控制替代方案，挑战了传统的精度-效率权衡关系。
